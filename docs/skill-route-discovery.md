...
Source digest: `github-growth-20260712T205308.160735Z` (selected proposal
`prop-skill-reverse-flow-test-lane`).

After
`skill_route_discovery_unlocked_local_test_lane_apply` is `ready`, the pipeline
emits `skill_route_discovery_focused_local_test_validation`:

1. Decision while commands are scheduled is
   `run_focused_local_test_validation_with_body_free_command_hashes`
2. Focused validation exports command hashes only; raw evidence URLs, upstream
   bodies, and command stdout stay out of the packet
3. Optional operator/result rows (`command_hash` + `passed`) mark the surface
   `passed` or `failed` without enabling activation
4. On full pass the decision becomes
   `record_focused_local_test_validation_pass_and_keep_activation_external` and
   supervisor next action is
...
6. Adjacent fortress-style rows remain outside this surface; agent-chief privacy
   rows stay review-only
7. While this surface is `ready`/unrecorded or `failed`, residual fortress/Hy3
   stages stay held (`residual_adjacent_hold_until_recorded=true` while
   unrecorded; `residual_adjacent_hold_active=true` for both unrecorded and
   failed). Pipeline render keeps supervisor_next on reverse-flow focused
   validation and does not advertise a residual selected proposal until residual
   work is residual-active
8. Residual stage packets themselves leave
   `selected_residual_proposal_id` / residual `proposal_id` empty while only
...
11. Durable pipeline `operator_state` (also top-level fields) exports
   ...
   `package_reverse_flow_focused_validation_continue_dispatch_inventory` packages
   a body-free inventory dispatch packet (`action`, `execute_recommended`, residual
   ...
   `focused_validation_residual_adjacent_queue` into body-free
   `residual_open_line` (for example
   ...
   residual_export=false
   helper=build_skill_route_discovery_residual_adjacent_harness_eval_local_apply`)
   plus `residual_entry` / `selected_residual_proposal_id` so residual selection
   is legible without nested residual-apply re-assembly. Residual export stays
   denied on continue surfaces even when residual entry is ready; selected residual
   IDs stay empty while residual open is blocked.
   ...
   plus nested `continue_cascade_transition` /
   ...
   plus nested `continue_cascade_wake_route` /
   `continue_cascade_wake_route_line` / `continue_cascade_wake_route_action` /
   `continue_cascade_wake_route_call_execute` so supervisors pin one route
   action enum and preferred helper instead of re-assembling wake_outcome +
   follow policy. Residual export stays denied on continue surfaces even when
   ...
   `continue_cascade_wake_route_apply_line` with pre→post route action
   ...
   `continue_cascade_wake_route_apply_follow_call_execute` so supervisors pin
   one follow receipt instead of re-mapping applied route actions after
   continue wakes. Residual export stays denied on continue surfaces even when
   residual route opens after residual_open_ready.
   ...
   `continue_cascade_wake_route_apply_follow_pin_call_next_line` with next
   invoke policy (for example
   ...
   supervisors pin one deep call receipt instead of re-comparing nested deep pin
   packets after continue wakes. Residual export stays denied on continue surfaces
   even when residual route opens after residual_open_ready.
   Preferred
   ...
   `continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call`,
   `continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow`,
   `continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin`, and
   `continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call` after
   run/record.
   ...
   `continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow`,
   `continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin`, and
   `continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call`. Durable
   ...
   `continue_cascade_wake_route_apply_follow_pin_call_next_preferred_helper`,
   `continue_cascade_wake_route_apply_follow_pin_call_next_call_execute`,
   `continue_cascade_wake_route_apply_follow_pin_call_next_ready`, nested
   `continue_cascade_wake_route_apply_follow_pin_call_next_call`,
   `continue_cascade_wake_route_apply_follow_pin_call_next_call_helper`,
   `continue_cascade_wake_route_apply_follow_pin_call_next_call_line`,
   `continue_cascade_wake_route_apply_follow_pin_call_next_action_transition`,
   `continue_cascade_wake_route_apply_follow_pin_call_next_invoke_transition`, and
   `continue_cascade_wake_route_apply_follow_pin_call_next_advanced` while reverse-flow
...
skill-route evidence and keeps `rnskill` in the documentation lane as a generic
SKILL.md-compatible skill collection. Both rows are limited to documentation,
config, test, or code_patch and require local validation before activation.

The Blender/Seedance workflow-usecase collection, `agent-chief`, and Hy3 remain
adjacent `agent_harness_eval_required` rows. They inherit no
`skill_route_discovery` lane, expose no direct local lane before harness
evaluation, and may only produce documentation, test, or code_patch follow-up
after that gate. The pass records proposal IDs, item IDs, lane names, source
hashes, rollback metadata, and activation denials only. Raw source URLs,
evidence URLs, replay commands, target paths, upstream bodies, install, run,
provider launch, external harness execution, promotion, restart, and remote
execution remain disabled. Replay with `python -m pytest
tests/test_skill_routing.py -q -k 20260709T103527`.

For source digest `github-growth-20260709T101527.212913Z`, pass 1 exposes
`skill_route_discovery_current_digest_20260709T101527_pass1_local_skill_route_discovery`.
The packet maps `reverse-flow-skill` to the local test lane as Codex
workflow-gate skill evidence and maps `rnskill` to the documentation lane as a
generic SKILL.md-compatible skill collection. Both rows are bounded to
...
`skill_route_discovery` lane, expose no direct local lane before harness
evaluation, and may only produce documentation, test, or code_patch follow-up
after that local gate. The pass records selected proposal IDs, item IDs, lane
names, source hashes, rollback metadata, and activation denials only. Raw source
URLs, evidence URLs, replay commands, target paths, upstream bodies, install,
run, provider launch, external harness execution, promotion, restart, and
remote execution remain disabled. Replay with `python -m pytest
tests/test_skill_routing.py -q -k 20260709T101527`.
...
skill-workflow evidence and keeps `rnskill` in the documentation lane as
generic SKILL.md-compatible workflow evidence. Both rows remain limited to
documentation, config, test, or code_patch and require local validation before
activation.

`agent-chief` and `Hy3` remain adjacent `agent_harness_eval_required` rows with
no direct local lane before harness evaluation. Hy3's API quickstart and MCP
server evidence is represented only as body-free provider preflight control:
the operator packet records recovery hint codes for missing provider preflight
samples and MCP harness evaluation, but exports no raw provider config, secret
values, source URLs, evidence URLs, replay commands, upstream bodies, provider
launch, MCP server launch, network call, remote execution, promotion, push, or
restart authority. Replay with `python -m pytest tests/test_skill_routing.py
-q -k 20260709T093527`.
...
slice before pass 4: `reverse-flow-skill` maps to the local test lane because
the evidence is a Codex/AI Agent skill workflow with a SKILL.md directory,
local sandbox framing, staged workflow, references, and diagnostic scripts;
`rnskill` maps to documentation as a generic SKILL.md-compatible collection
with docs, tools, and plugin/marketplace metadata. Both rows remain limited to
documentation, config, test, or code_patch lanes and keep local validation
required.

`agent-chief` and `Hy3` remain adjacent `agent_harness_eval_required` rows
with no direct local lane before harness evaluation. Hy3 provider preflight is
kept review-only for the privacy boundary: no provider launch, network call,
raw provider config, secret value, token, API key, evidence URL, source URL,
replay command, target path, upstream body, promotion, push, restart, or remote
execution is exported or enabled. Replay with `python -m pytest
tests/test_skill_routing.py -q -k 20260709T081527`.

For source digest `github-growth-20260709T075527.280528Z`, pass 2 exposes
`skill_route_discovery_current_digest_20260709T075527_pass2_validation_lane`.
The lane continues the reverse-flow/rnskill route-discovery slice as a bounded
local validation surface. `reverse-flow-skill` maps to the local test lane
because the reviewed evidence is a Codex/AI Agent skill workflow with a skill
directory, activation phrase, local sandbox framing, staged workflow, and
diagnostic scripts. `rnskill` maps to documentation as a generic
SKILL.md-compatible skills collection. Both rows remain limited to
documentation, config, test, or code_patch lanes with local validation required.

`agent-chief` and `Hy3` remain adjacent `agent_harness_eval_required` rows.
They inherit no skill-route lane, expose no direct local lane before harness
evaluation, and may only produce documentation, test, or code_patch follow-up
after that local gate. The pass records proposal IDs, selected item IDs, lane
names, source hashes, rollback metadata, and activation denials only. Raw source
URLs, evidence URLs, replay commands, target paths, upstream bodies, install,
run, provider launch, external harness execution, promotion, restart, and remote
execution remain disabled. Replay with `python -m pytest tests/test_skill_routing.py
tests/test_harness_eval.py -q -k 20260709T075527`.

For source digest `github-growth-20260709T071527.122161Z`, pass 4 exposes
`skill_route_discovery_current_digest_20260709T071527_pass4_completion_handoff`.
The handoff completes the current reverse-flow/rnskill slice as an
operator-visible route surface before any supervisor activation. The
`reverse-flow-skill` row maps to the local test lane because the evidence is a
triggered Codex/AI Agent skill workflow with local sandbox framing, staged
workflow phases, and diagnostic scripts. The `rnskill` row maps to
documentation as a generic SKILL.md-compatible skills collection. Both rows
remain bounded to documentation, config, test, or code_patch outputs and keep
runtime action set to none.

`agent-chief` and `Hy3` remain adjacent `agent_harness_eval_required` rows.
They inherit no skill-route lane, expose no direct local lane before harness
evaluation, and may only produce documentation, test, or code_patch follow-up
after that local gate. The handoff records proposal IDs, selected item IDs,
lane names, source hashes, rollback metadata, validation-command hashes, and
activation denials only. Raw source URLs, evidence URLs, replay commands,
...
For source digest `github-growth-20260709T065527.238783Z`, pass 3 exposes
`skill_route_discovery_current_digest_20260709T065527_pass3_validation_lane`.
The lane keeps `reverse-flow-skill` in the local test lane as Codex workflow
gate evidence and keeps `rnskill` in the documentation lane as generic
SKILL.md-compatible workflow evidence. Both rows must pass through
`skill_route_discovery` first, require local validation, and may use only
documentation, config, test, or code_patch lanes.

`agent-chief` and `Hy3` remain adjacent `agent_harness_eval_required` rows.
They inherit no skill-route lane, expose no direct local lane before harness
evaluation, and may only produce documentation, test, or code_patch follow-up
after that local gate. The pass records proposal IDs, item IDs, lane names,
hashes, rollback metadata, and activation denials only; raw source URLs,
replay commands, target paths, upstream bodies, install, run, provider launch,
external harness execution, promotion, restart, and remote execution remain
disabled. Replay with `python -m pytest tests/test_skill_routing.py -q -k
20260709T065527`.

For source digest `github-growth-20260709T061527.151662Z`, pass 1 exposes
`skill_route_discovery_current_digest_20260709T061527_pass1_validation_lane`.
The lane turns `reverse-flow-skill` into a local test candidate for Codex
workflow-gate skill evidence and `rnskill` into a documentation candidate for
generic SKILL.md-compatible workflow evidence. Both rows remain bounded to
documentation, config, test, or code_patch lanes and require local validation
before any activation.

`agent-chief` and `Hy3` are retained as adjacent
`agent_harness_eval_required` rows because the focused evidence shows general
agent orchestration and model capability rather than a skill package route.
They expose no direct local lane before harness evaluation and may only produce
documentation, test, or code_patch follow-up after that local gate. The pass
records proposal IDs, item IDs, lane names, hashes, rollback metadata, and
activation denials only; raw source URLs, replay commands, target paths,
upstream bodies, install, run, provider launch, external harness execution,
promotion, restart, and remote execution remain disabled. Replay with
`python -m pytest tests/test_skill_routing.py tests/test_harness_eval.py -q -k
20260709T061527`.
...
bounded local validation only: reverse-flow selects the test lane for Codex
workflow-gate plus generic skill workflow evidence, and rnskill selects the
documentation lane for a generic SKILL.md-compatible skills collection.

`agent-chief` and `Hy3` are retained as adjacent `agent_harness_eval_required`
rows because their public evidence is general agent orchestration or model
evidence, not a skill package route. They expose no direct local lane before
harness evaluation and may only produce documentation, test, or code_patch
follow-up after that local gate. The pass records proposal IDs, selected item
IDs, lane names, source hashes, rollback metadata, and activation denials only.
Raw source URLs, evidence URLs, replay commands, target paths, upstream bodies,
install, run, provider launch, external harness execution, promotion, restart,
and remote execution remain disabled. Replay with `python -m pytest
tests/test_skill_routing.py tests/test_harness_eval.py -q -k 20260709T045527`.

For source digest `github-growth-20260709T043527.568573Z`, pass 4 exposes
`skill_route_discovery_current_digest_20260709T043527_pass4_completion_handoff`.
The handoff closes the current reverse-flow/rnskill skill-route slice as an
operator-visible behavior before supervisor activation. `reverse-flow-skill`
maps to the local test lane as Codex workflow-gate plus generic skill workflow
evidence. `rnskill` maps to documentation as a generic SKILL.md-compatible
skills collection. Both rows remain bounded to documentation, config, test, or
code_patch lanes and keep runtime action set to none.

Generic skill repositories are interpreted as local route evidence, not as
packages to install, enable, or run. Any runtime execution, provider launch,
or external harness path requires a separate validated lane. `Hy3`,
`agent-chief`, and workflow-usecase observations remain adjacent
`agent_harness_eval_required` rows with no direct local lane before harness
evaluation and only documentation, test, or code_patch follow-up after that
gate. The handoff records rollback metadata, selected item IDs, lane names,
hashes, and activation denials only. Raw source URLs, evidence URLs, replay
commands, target paths, upstream bodies, install, run, provider launch,
external harness execution, promotion, restart, and remote execution remain
disabled. Replay with `python -m pytest tests/test_skill_routing.py
tests/test_harness_eval.py -q -k 20260709T043527`.

For source digest `github-growth-20260709T041527.127710Z`, pass 3 exposes
`skill_route_discovery_current_digest_20260709T041527_pass3_operator_handoff`.
The handoff converts the current reverse-flow/rnskill/Hy3 window into an
operator-visible validation surface before pass 4. `reverse-flow-skill` maps
to the local test lane as Codex workflow-gate plus generic skill workflow
evidence, and `rnskill` maps to the documentation lane as generic
SKILL.md-compatible collection evidence. Both rows keep local validation
required and may use only documentation, config, test, or code_patch lanes.

`Hy3`, `agent-chief`, and workflow-usecase observations remain adjacent
...
produce documentation, test, or code_patch follow-up after that local gate. The
handoff records rollback metadata, selected local lanes, local evidence item
IDs, and hashes only. Raw source URLs, evidence URLs, replay commands, target
paths, upstream bodies, install, run, provider launch, external harness
execution, promotion, restart, and remote execution remain disabled. Replay
with `python -m pytest tests/test_skill_routing.py tests/test_harness_eval.py
-q -k 20260709T041527`.
...
surface: `reverse-flow-skill` maps to documentation for the local route catalog,
`rnskill` maps to the test lane for generic SKILL.md fixture coverage, and
`Cognitive-Core-Skills` remains a skill-route test row before any benchmark or
harness follow-up. Adjacent general-agent or workflow-usecase projects such as
`agent-chief`, `Hy3`, and the Blender/Seedance workflow-usecase repository
remain `agent_harness_eval_required`; they inherit no skill-route lane, expose
no direct local lanes before harness evaluation, and may only produce
documentation, test, or code_patch follow-up after that gate.

The pass-4 handoff is body-free and activation-free. It records proposal IDs,
selected item IDs, lane names, source hashes, rollback metadata, a validation
command hash, and explicit activation denials only. Raw source URLs, evidence
URLs, replay commands, target paths, upstream bodies, install, runtime,
provider launch, external harness execution, promotion, restart, and remote
execution remain disabled. Replay with
`python -m pytest tests/test_skill_routing.py -q -k 20260709T031527`.

For source digest `github-growth-20260709T025529.673051Z`, pass 3 exposes
`skill_route_discovery_current_digest_20260709T025529_pass3_validation_packet`.
The packet keeps `reverse-flow-skill` in the local test lane as Codex
workflow-gate evidence, keeps `rnskill` in the documentation lane as a generic
SKILL.md-style workflow collection, and treats `Cognitive-Core-Skills` as a
skill-route test candidate before any benchmark or harness follow-up. Adjacent
general-agent projects such as `agent-chief` and `Hy3` remain
...
produce documentation, test, or code_patch follow-up after that gate.

The pass-3 packet is discovery-only and body-free. It exports selected item
IDs, route profiles, lane names, hashes, rollback metadata, and activation
denials only; raw source URLs, replay commands, upstream bodies, install,
runtime, provider launch, external harness execution, remote execution,
promotion, and restart remain disabled. Replay with
`python -m pytest tests/test_skill_routing.py -q -k 20260709T025529`.
...
replays the active reverse-flow/rnskill/Shepherd/Hy3 plus Blender/Seedance
workflow-usecase window before any activation path. `reverse-flow-skill` maps
to the local test lane as Codex workflow-gate plus generic skill workflow
evidence, while `rnskill` maps to the documentation lane as a generic
SKILL.md-compatible workflow collection. Both rows may use only
documentation, config, test, or code_patch lanes and keep runtime action set
to none.

`shepherd`, `Hy3`, and the Blender/Seedance workflow-usecase repository remain
adjacent `agent_harness_eval_required` rows. They inherit no
`skill_route_discovery` lane, expose no direct local implementation lane
before local harness evaluation, and may only produce documentation, test, or
code_patch follow-up after that gate. The local route eval gate records the
selected source digest hash, skill-route proposal IDs, adjacent agent-harness
proposal IDs, bounded lane checks, rollback metadata, and activation denials
...
For source digest `github-growth-20260708T231850.689945Z`, pass 1 exposes
`skill_route_discovery_current_digest_20260708T231850_pass1_validation_lane`.
The lane converts the active reverse-flow/rnskill/Shepherd/Hy3 plus
Blender/Seedance workflow-usecase window into bounded local validation lanes
before activation. `trend:lingbol088-spec/reverse-flow-skill-1` maps to the
local test lane as Codex workflow-gate plus generic skill workflow evidence,
and `trend:Pluviobyte/rnskill-1` maps to the documentation lane as generic
SKILL.md-compatible workflow collection evidence. Both skill rows may use only
documentation, config, test, or code_patch lanes and keep runtime action set
to none.

`trend:shepherd-agents/shepherd-1`, `trend:Tencent-Hunyuan/Hy3-1`, and
`trend:Evolink-AI/Awesome-Blender-Seedance-Workflow-Usecases-1` remain
adjacent `agent_harness_eval_required` rows. They inherit no
`skill_route_discovery` lane, expose no direct local implementation lane
before local harness evaluation, and may only produce documentation, test, or
code_patch follow-up after that gate. The packet exports item IDs, proposal
IDs, lane names, hashes, rollback metadata, and activation denials only; raw
source URLs, evidence URLs, replay commands, target paths, upstream bodies,
install, enable, run, provider launch, external harness execution, remote
execution, promotion, restart, profile writes, and memory writes remain
disabled. Replay with:
...
bounded local route probe. `lingbol088-spec/reverse-flow-skill` maps to
`p1-skill-route-discovery-reverse-flow` in the local test lane because its
public repository shape shows a Codex/AI Agent skill workflow with
`skills/reverse-flow`, `SKILL.md`, local sandbox framing, staged workflow
language, and diagnostic scripts. `Pluviobyte/rnskill` maps to
`p2-skill-route-discovery-rnskill` in the documentation lane as generic
SKILL.md-compatible workflow collection evidence.

`shepherd-agents/shepherd` and `Tencent-Hunyuan/Hy3` remain adjacent
`agent_harness_eval_required` rows. They inherit no `skill_route_discovery`
lane, expose no direct local implementation lane before local harness
evaluation, and may only produce documentation, test, or code_patch follow-up
after that gate. The packet exports selected item IDs, proposal IDs, lane
names, hashes, rollback metadata, and activation denials only; raw source
URLs, evidence URLs, replay commands, target paths, upstream bodies, install,
...
For source digest `github-growth-20260708T213850.601257Z`, pass 4 exposes
`skill_route_discovery_current_digest_20260708T213850_pass4_completion_checkpoint`.
The checkpoint completes the active reverse-flow/rnskill/Shepherd/Hy3 window
as an operator-visible replay surface. `Pluviobyte/rnskill` maps to
`p1-rnskill-skill-route-discovery` in the documentation lane as generic
SKILL.md-compatible workflow evidence. `lingbol088-spec/reverse-flow-skill`
maps to `p2-reverse-flow-codex-workflow-gate` in the local test lane and keeps
the Codex workflow-gate evidence behind `skill_route_discovery` before any
code_patch route can be considered.

`shepherd-agents/shepherd` and `Tencent-Hunyuan/Hy3` remain adjacent
`p3-general-agent-harness-eval` rows. They inherit no `skill_route_discovery`
lane, expose no direct implementation lane before local agent-harness
evaluation, and may only produce documentation, test, or code_patch follow-up
after that gate. The checkpoint exports selected item IDs, proposal IDs, lane
names, hashes, rollback metadata, validation-command hashes, and activation
denials only; raw source URLs, evidence URLs, replay commands, target paths,
upstream bodies, install, enable, run, provider launch, external harness
execution, remote execution, promotion, and restart remain disabled. Replay
with: `python -m pytest tests/test_skill_routing.py -q -k 20260708T213850`.
...
bounded route probe before activation. `lingbol088-spec/reverse-flow-skill`
maps to `p1-skill-route-discovery-reverse-flow` in the local test lane because
its public metadata presents a Codex/AI-agent skill workflow with
`skills/reverse-flow`, `SKILL.md`, sandbox framing, staged workflow language,
and scripts. `Pluviobyte/rnskill` maps to
`p2-skill-route-discovery-rnskill` in the documentation lane as generic
SKILL.md-compatible workflow evidence.
...
before local agent-harness evaluation, and may only produce documentation,
test, or code_patch follow-up after that gate. The
`p5-agent-workflow-usecase-eval` anchor stays queued without a selected item in
this digest. The lane exports expected triggers, bounded local lanes, minimal
acceptance checks, selected item IDs, hashes, rollback metadata, and activation
denials only; raw source URLs, evidence URLs, replay commands, target paths,
upstream bodies, install, provider launch, external harness execution, remote
execution, promotion, and restart remain disabled. Replay with:
`python -m pytest tests/test_skill_routing.py -q -k 20260708T203850`.

For source digest `github-growth-20260708T201850.924336Z`, pass 4 exposes
`skill_route_discovery_current_digest_20260708T201850_pass4_operator_handoff`.
The handoff completes the active reverse-flow/rnskill/Hy3/Shepherd window as
a supervisor-visible replay surface. `lingbol088-spec/reverse-flow-skill`
maps to `p1-skill-route-discovery-reverse-flow` in the local test lane because
its public metadata shows a Codex/AI-agent skill package with a
`skills/reverse-flow` layout, `SKILL.md`, sandbox framing, staged workflow
language, and scripts. `Pluviobyte/rnskill` maps to
`p2-generic-skill-route-discovery-rnskill` in the documentation lane as a
generic SKILL.md-compatible workflow collection.

`Tencent-Hunyuan/Hy3` and `shepherd-agents/shepherd` remain adjacent
`p3-agent-harness-eval-general-projects` rows. They inherit no
`skill_route_discovery` lane, expose no direct local implementation lane
before local agent-harness evaluation, and may only produce documentation,
test, or code_patch follow-up after that gate. The handoff exports selected
item IDs, proposal IDs, lane names, hashes, rollback metadata, and activation
denials only; raw source URLs, evidence URLs, replay commands, target paths,
upstream bodies, install, provider launch, external harness execution, remote
execution, promotion, and restart remain disabled. Replay with:
`python -m pytest tests/test_skill_routing.py -q -k 20260708T201850`.

For source digest `github-growth-20260708T195850.396172Z`, pass 3 exposes
`skill_route_discovery_current_digest_20260708T195850_pass3_validation_packet`.
The packet reuses the pass-3 activation-packet machinery as a bounded local
validation surface for the active reverse-flow/rnskill window.
...
`p2-generic-skill-workflow-discovery` in the local test lane. Both rows expose
only documentation, config, test, or code_patch lanes, preserve local
validation before implementation, and keep runtime action disabled.

`shepherd-agents/shepherd`, `Tencent-Hunyuan/Hy3`, and the
Blender/Seedance workflow-usecase repository remain adjacent
`p3-agent-harness-eval-general-projects` rows. They inherit no
`skill_route_discovery` lane, expose no direct local implementation lane before
local agent-harness evaluation, and may only produce documentation, test, or
code_patch follow-up after that gate. The packet exports body-free selected
item IDs, lane names, hashes, rollback metadata, and activation denials only;
raw source URLs, evidence URLs, replay commands, upstream bodies, install,
provider launch, external harness execution, remote execution, promotion, and
restart remain disabled. Replay with:
`python -m pytest tests/test_harness_eval.py tests/test_skill_routing.py -q -k 20260708T195850`.

For source digest `github-growth-20260708T191850.475615Z`, pass 1 exposes the
active reverse-flow/rnskill/general-agent window through
...
public repository shape includes a `skills/reverse-flow` package, `SKILL.md`,
local sandbox framing, staged workflow language, install examples, and script
examples. `Pluviobyte/rnskill` maps to
`p2_skill_route_discovery_rnskill` in the documentation lane as generic
SKILL.md-style workflow evidence. Both rows expose only documentation, config,
test, or code_patch lanes.

`shepherd-agents/shepherd`, `Tencent-Hunyuan/Hy3`, and the Blender/Seedance
workflow-usecase repository remain adjacent
`p3_agent_harness_eval_general_projects` rows. They inherit no
`skill_route_discovery` lane, expose no direct local implementation lane before
local agent-harness evaluation, and may only produce documentation, test, or
code_patch follow-up after that gate. The packet exports body-free route
metadata and activation denials only; raw source URLs, replay commands,
upstream bodies, install, provider launch, external harness execution, remote
execution, promotion, and restart remain disabled. Replay with:
`python -m pytest tests/test_harness_eval.py -q -k 20260708T191850`.

For source digest `github-growth-20260708T185850.414401Z`, pass 4 exposes
`skill_route_discovery_current_digest_20260708T185850_pass4_completion_handoff`.
The handoff completes the active reverse-flow/rnskill skill-route slice as a
bounded local replay surface. `lingbol088-spec/reverse-flow-skill` maps to the
local test lane for Codex workflow-gate validation, while `Pluviobyte/rnskill`
maps to the documentation lane as generic SKILL.md workflow evidence. Both
rows expose only documentation, config, test, or code_patch lanes.

`shepherd-agents/shepherd`, `Tencent-Hunyuan/Hy3`, and the Blender/Seedance
workflow-usecase repository remain adjacent `agent_harness_eval_required`
rows. They inherit no `skill_route_discovery` lane, expose no direct local
implementation lane before local agent-harness evaluation, and may only
produce documentation, test, or code_patch follow-up after that gate. The
top-level pass-4 handoff is ready, but the nested local-kernel replay remains
blocked by the review-only automation/bug checklist for the reverse-flow
signal; that preserves the offensive-behavior boundary without discarding the
bounded local route evidence. Replay with:
...
`skill_route_discovery_current_digest_pass1_validation_lane` and
`skill_route_discovery_current_run_pass1_activation_readiness`. The lane binds
`lingbol088-spec/reverse-flow-skill` to
`p1-skill-route-discovery-reverse-flow` in the bounded local test lane because
the public repository presents a Codex/AI Agent skill package, local sandbox
framing, staged workflow, and diagnostic scripts. It binds
`Pluviobyte/rnskill` to `p2-generic-skill-route-discovery-rnskill` in the
documentation lane because it is a generic SKILL.md-compatible skill
collection. Install, run, enable, provider, runtime, and external harness
pressure remains diagnostic only.

`shepherd-agents/shepherd` and `Tencent-Hunyuan/Hy3` remain adjacent
`agent_harness_eval_required` rows. They receive no direct local lane before a
local harness evaluation, inherit no skill-route discovery lane, and cannot
enable external skill activation, external harness execution, provider launch,
remote execution, promotion, or restart from discovery alone. The replay check
is `python -m pytest tests/test_harness_eval.py -q -k 20260708T175850`.

For source digest `github-growth-20260708T173850.570889Z`, pass 4 exposes
`skill_route_discovery_current_digest_20260708T173850_pass4_operator_handoff`.
The handoff completes the active reverse-flow/rnskill/Hy3/Shepherd window as a
supervisor-visible replay surface. `lingbol088-spec/reverse-flow-skill` maps to
`p1-skill-route-discovery-codex-workflow-gate` in the local test lane, while
`Pluviobyte/rnskill` maps to `p2-generic-skill-route-discovery-config` with a
documentation-first selected lane until local validation proves a config
change. Both rows expose only documentation, config, test, or code_patch lanes
and keep runtime action disabled.

`Tencent-Hunyuan/Hy3` and `shepherd-agents/shepherd` remain adjacent
`agent_harness_eval_required` rows under
`p3-agent-harness-eval-for-general-agent-projects`. They inherit no
`skill_route_discovery` lane, expose no direct implementation lane before
local harness evaluation, and may only produce documentation, test, or
code_patch follow-up after that gate. The handoff exports selected item IDs,
proposal IDs, lane names, hashes, rollback artifact metadata, and activation
denials only; raw source URLs, evidence URLs, replay commands, target paths,
upstream bodies, install, provider launch, external harness execution, remote
execution, promotion, restart, profile writes, and memory writes remain
disabled. Replay with:
...
For source digest `github-growth-20260708T171850.612077Z`, pass 3 exposes
`skill_route_discovery_current_digest_20260708T171850_pass3_validation_lane`.
The lane advances the active reverse-flow/rnskill/Hy3/Shepherd window into an
operator-visible validation packet before pass 4. `lingbol088-spec/reverse-flow-skill`
maps to `p1-skill-route-discovery-codex-workflow-gate` in the local test lane
with `skill_route_discovery_first` preserved before any Codex workflow-gate
interpretation. `Pluviobyte/rnskill` maps to
`p2-generic-skill-workflow-fixture` as generic skill-workflow evidence and
selects the documentation lane while still exposing only documentation,
config, test, or code_patch as bounded local lanes.

`Tencent-Hunyuan/Hy3` and `shepherd-agents/shepherd` remain adjacent
`agent_harness_eval_required` rows under
`p3-agent-harness-eval-for-general-agent-projects`. Their direct allowed lanes
remain empty until local agent-harness evaluation recomputes scope; only after
that gate may documentation, test, or code_patch follow-up be considered. The
packet exports selected item IDs, proposal IDs, lane names, route profiles,
hashes, rollback metadata, and activation denials only; raw source URLs,
...
For source digest `github-growth-20260708T165850.561086Z`, pass 2 exposes
`skill_route_discovery_current_digest_20260708T165850_pass2_validation_lane`.
The lane advances the active reverse-flow/rnskill/Hy3/Shepherd window from
pass-1 discovery into a bounded operator-visible validation packet.
`lingbol088-spec/reverse-flow-skill` maps to
`p1-skill-route-discovery-codex-workflow-gate` in the local test lane as Codex
workflow-gate skill evidence. `Pluviobyte/rnskill` maps to
`p2-generic-skill-route-discovery-config`; because it is still generic
SKILL.md workflow evidence, the selected replay lane remains documentation
until a concrete activation-safe config change is locally proven.

`Tencent-Hunyuan/Hy3` and `shepherd-agents/shepherd` remain adjacent
`agent_harness_eval_required` rows under
`p3-agent-harness-eval-for-general-agent-projects`. They inherit no
`skill_route_discovery` lane, expose no direct implementation lane before local
harness evaluation, and may only produce documentation, test, or code_patch
follow-up after that gate. The packet exports proposal IDs, selected item IDs,
lane names, route profiles, hashes, rollback metadata, and activation denials
only; raw source URLs, evidence URLs, replay commands, target paths, upstream
bodies, install, provider launch, external harness execution, remote execution,
promotion, and restart remain disabled. Replay with:
`python -m pytest tests/test_skill_routing.py -q -k 20260708T165850`.

For source digest `github-growth-20260708T163850.664161Z`, pass 1 exposes the
active reverse-flow/rnskill/Hy3/Shepherd window through the existing
`current_digest_pass1_validation_lane` and
`skill_route_discovery_current_run_pass1_activation_readiness` surfaces.
`lingbol088-spec/reverse-flow-skill` maps to
`p1-skill-route-discovery-codex-workflow-gate` in the local test lane as Codex
workflow-gate skill evidence. `Pluviobyte/rnskill` maps to
`p2-generic-skill-route-discovery-config` as generic SKILL.md workflow evidence
while still selecting the documentation-first local lane until a concrete
activation-safe config change is proven.

`Tencent-Hunyuan/Hy3` and `shepherd-agents/shepherd` remain adjacent
`agent_harness_eval_required` rows. They inherit no `skill_route_discovery`
lane, expose no direct local lanes before local harness evaluation, and may
only produce documentation, test, or code_patch follow-up after that gate. The
surfaces export selected item IDs, proposal IDs, lane names, route profiles,
hashes, and activation denials only; raw source URLs, evidence URLs, replay
commands, target paths, upstream bodies, install, provider launch, external
harness execution, remote execution, promotion, and restart remain disabled.
Replay with:
`python -m pytest tests/test_skill_routing.py -q -k 20260708T163850`.

For source digest `github-growth-20260708T161850.608560Z`, pass 4 exposes
`skill_route_discovery_current_digest_20260708T161850_pass4_operator_handoff`.
The handoff completes the active reverse-flow/rnskill/Shepherd window through
the existing pass-4 operator path rather than a standalone fixture. The
`lingbol088-spec/reverse-flow-skill` row remains a Codex workflow-gate skill
route in the local test lane, and `Pluviobyte/rnskill` remains a generic
skill-workflow row in the documentation lane. Both rows allow only
documentation, config, test, or code_patch and keep `runtime_action` as `none`
until a later controller or supervisor approval path exists.
...
The handoff exports selected item IDs, proposal IDs, lane names, hashes,
rollback artifact metadata, and activation denials only; raw source URLs,
evidence URLs, replay commands, target paths, upstream bodies, install,
provider launch, external harness execution, remote execution, promotion,
restart, profile writes, and memory writes remain disabled. Replay with:
`python -m pytest tests/test_skill_routing.py -q -k 20260708T161850`.

For source digest `github-growth-20260708T155850.641176Z`, pass 3 exposes
`skill_route_discovery_current_digest_20260708T155850_pass3_validation_lane`.
The lane turns the current reverse-flow/rnskill/Shepherd window into an
operator-visible validation and recovery workflow before pass 4. The
`lingbol088-spec/reverse-flow-skill` row remains a Codex workflow-gate skill
route in the local test lane; its install, run, script, runtime, and external
harness wording is downgraded to route pressure. `Pluviobyte/rnskill` remains a
generic skill-workflow collection in the documentation lane. Both rows allow
only documentation, config, test, or code_patch and require local validation
before activation.

`shepherd-agents/shepherd` and the Blender/Seedance workflow-usecase evidence
remain adjacent `agent_harness_eval_required` rows. The nested
`shepherd_release_activity_gate` records that Shepherd release metadata,
branch-creation observations, and push activity are evaluation-only until local
agent-harness tests pass. The `operator_recovery_workflow` orders replay as
route recomputation, reverse-flow lane validation, rnskill documentation-lane
validation, Shepherd holdback, then pass-4 continuation. Raw source URLs,
evidence URLs, replay commands, target paths, upstream bodies, install,
provider launch, external harness execution, remote execution, promotion,
...
reusing the generic pass-4 completion lane as an operator-visible replay
surface. `lingbol088-spec/reverse-flow-skill` maps to the local test lane as
Codex workflow-gate skill evidence, and `Pluviobyte/rnskill` maps to the
documentation lane as a generic `SKILL.md` collection. Both rows preserve only
documentation, config, test, or code_patch as allowed local lanes and require
local validation before activation.

`shepherd-agents/shepherd`, `Tencent-Hunyuan/Hy3`, and the
Blender/Seedance workflow-usecase repository remain adjacent
`agent_harness_eval_required` rows. They inherit no `skill_route_discovery`
lane, have no direct allowed lanes before local harness evaluation, and may
only produce documentation, test, or code_patch follow-up after that gate.
The handoff exports selected item IDs, proposal IDs, lane names, hashes,
rollback artifact metadata, and activation denials only; raw source URLs,
evidence URLs, replay commands, target paths, upstream bodies, install,
...
For source digest `github-growth-20260708T141851.976489Z`, pass 2 exposes
`skill_route_discovery_current_digest_20260708T141851_pass2_validation_lane`.
The lane binds the active reverse-flow/rnskill proposals to bounded local
validation before activation. `lingbol088-spec/reverse-flow-skill` remains in
the local test lane as Codex workflow-gate evidence: the focused public review
found a `skills/` layout, `SKILL.md`, local sandbox/CTF framing, staged
workflow language, install examples, and script examples. Those signals are
validation pressure only and do not authorize install, execution, external
harness use, provider launch, or remote execution.

`Pluviobyte/rnskill` remains a generic `SKILL.md` collection in the
documentation lane, even though the current proposal is test-shaped; the local
test validates route metadata and documentation conventions before activation.
`shepherd-agents/shepherd` is routed to
`p3_agent_harness_eval_shepherd_integration` as adjacent
`agent_harness_eval_required` evidence for capability declarations, permission
boundaries, routing expectations, and failure handling. Hy3 and the
Blender/Seedance workflow-usecase repository remain adjacent harness-eval or
triage rows with no inherited skill-route lane. The packet exports selected
item IDs, proposal IDs, lane names, hashes, rollback metadata, and activation
denials only; raw source URLs, evidence URLs, replay commands, target paths,
upstream bodies, install, provider launch, external harness execution, remote
execution, promotion, and restart remain disabled. Replay with:
`python -m pytest tests/test_skill_routing.py -q -k 20260708T141851`.

For source digest `github-growth-20260708T131852.174738Z`, pass 3 exposes
`skill_route_discovery_current_digest_20260708T131852_pass3_validation_lane`.
The lane advances the active reverse-flow/rnskill route window from focused
review into an operator-visible validation packet. `lingbol088-spec/reverse-flow-skill`
is kept in the local test lane as Codex workflow-gate skill evidence because
the public review found a `skills/` layout, `SKILL.md`, local sandbox/CTF
framing, staged workflow language, install examples, and script examples.
`Pluviobyte/rnskill` is kept in the documentation lane as a generic
`SKILL.md`-compatible agent skill collection.

`shepherd-agents/shepherd`, `Tencent-Hunyuan/Hy3`, and the Blender/Seedance
workflow-usecase repository remain adjacent `agent_harness_eval_required` rows.
They inherit no `skill_route_discovery` lane, expose no direct lanes before
local harness evaluation, and may only produce documentation, test, or
code_patch follow-up after that gate. The pass-3 packet exports selected item
ids, proposal ids, lane names, hashes, and activation denials only; raw source
URLs, evidence URLs, replay commands, target paths, upstream bodies, install,
provider launch, external harness execution, remote execution, promotion, and
restart remain disabled. Replay with:
