...
Observed this run (digest `github-growth-20260714T054752.485772Z`,
...
- Prior: continue cascade wake classifies `wake_outcome`; wake route maps
  wake_outcome → durable `route_action` + preferred helper; wake route apply
  collapses pre/post routes into body-free apply receipt; apply follow maps
  applied route → preferred helper; apply follow pin classifies pin mode
  (`execute_helper` / `package_helper` / `inventory_only`); pin_call collapses
  pre/post pin into body-free call receipt; pin_call_next seals pin_call into
  a next-invoke recipe without residual_export
- New: `package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call`
  collapses pre/post pin_call_next into body-free
  `continue_cascade_wake_route_apply_follow_pin_call_next_call_line` with
  pre→post next action/invoke transition (for example
  ...
  so supervisors pin one next-call receipt instead of re-comparing nested
  pin_call_next packets after continue wakes
- Pin call next call fields: `pre_next_action`, `post_next_action`,
  `next_action_transition`, `applied_next_action`, `pre_next_invoke`,
  `post_next_invoke`, `next_invoke_transition`, `applied_next_invoke`,
  `next_advanced`, `pre_call_next_with_execute`, `post_call_next_with_execute`,
  `pre_next_ready`, `post_next_ready`, reverse/residual progress transitions
- Identity / inventory snapshot: pin_call_next_call
  `pre_action=execute_now post_action=execute_now
  action=execute_now→execute_now invoke=execute_helper→execute_helper
  call_execute=true→true next_advanced=false`, residual_export denied
- Full follow after reverse-flow pass: pin_call_next_call reports
  `action=execute_now→keep_activation_external
  invoke=execute_helper→package_helper call_execute=true→false
  next_advanced=true` (or residual open
  `action=keep_activation_external→open_residual_entry
  residual_route=false→true`), residual_export denied
- New: follow and dispatch attach `continue_cascade_wake_route_apply_follow_pin_call_next_call`,
  ...
  `wake_route_apply_follow_pin_call_next_action_transition`,
  `wake_route_apply_follow_pin_call_next_invoke_transition`, and
  `wake_route_apply_follow_pin_call_next_advanced` after cascade_wake_route_apply_follow_pin_call_next
  packaging
- New: operator_state exports nested `continue_cascade_wake_route_apply_follow_pin_call_next_call`,
  `continue_cascade_wake_route_apply_follow_pin_call_next_call_helper`,
  `continue_cascade_wake_route_apply_follow_pin_call_next_call_line`,
  ...
  `continue_cascade_wake_route_apply_follow_pin_call_next_advanced` (alongside pin_call_next /
  pin_call / pin / follow / apply / wake route / wake / transition / cascade /
  residual cascade surfaces)
- Ready/unrecorded continue cascade wake route apply follow pin call next call:
  identity `action=execute_now→execute_now invoke=execute_helper→execute_helper
  next_advanced=false` (or inventory_only when execute is not recommended);
...
- Full follow after reverse-flow pass: continue cascade wake route still reports
  ...
  continue_cascade_wake_route_apply/continue_cascade_wake_route_apply_follow/
  ...
  continue_cascade_wake_route_apply_follow_pin_call_next_call surfaces themselves
