...
Observed this run (digest `github-growth-20260715T025132.803155Z`,
...
agent-chief proposals retained review-only for privacy):
...
- Prior: pin_call_next_call_follow_pin_call collapses pre/post next_call_follow
  pin recipes into a body-free call receipt with pre→post pin action/mode
  transition so supervisors do not re-compare nested pin packets after continue
  wakes
- New: `package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next`
  seals that pin_call receipt into body-free
...
- Pin call next call follow pin call next fields: `next_action`, `next_invoke`,
  `next_ready`, `call_next_with_execute`, `applied_pin_action`,
  `applied_pin_mode`, `pin_advanced`, reverse/residual progress transitions
- Identity / inventory snapshot: pin_call_next_call_follow_pin_call_next
  `action=execute_now invoke=execute_helper next_ready=true advanced=false`
  (or inventory_only when execute is not recommended); residual_export denied
- Full follow after reverse-flow pass: pin_call_next_call_follow_pin_call_next
...
- New: follow and dispatch attach
  `continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next`,
  `continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_line`,
  `continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_helper`,
  `wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_action`,
  `wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_invoke`, and
  `wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_ready` after
  pin_call packaging
- New: operator_state exports nested
  ...
  and `continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_ready`
  (alongside pin_call_next_call_follow_pin_call / pin_call_next_call_follow_pin /
...
- Ready/unrecorded continue cascade wake route apply follow pin call next call
  follow pin call next: identity `action=execute_now invoke=execute_helper
  next_ready=true advanced=false` (or inventory_only when execute is not
