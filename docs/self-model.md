...
Observed this run (digest `github-growth-20260714T064753.067377Z`,
...
- Prior: continue cascade wake classifies `wake_outcome`; wake route maps
  ...
  into a body-free next-call receipt without residual_export
- New: `package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow`
  maps pin_call_next_call into body-free
  ...
  so supervisors pin one next-call follow receipt instead of re-mapping applied
  next action/invoke after continue wakes
- Pin call next call follow fields: `applied_next_action`, `applied_next_invoke`,
  `next_action_transition`, `next_invoke_transition`, `next_advanced`,
  `follow_action`, `follow_mode`, `follow_ready`, `call_follow_with_execute`,
  `post_call_next_with_execute`, `post_next_ready`, reverse/residual progress
  transitions
- Identity / inventory snapshot: pin_call_next_call_follow
  `applied=execute_now invoke=execute_helper mode=execute_helper
  advanced=false call_execute=true follow_ready=true`, residual_export denied
- Full follow after reverse-flow pass: pin_call_next_call_follow reports
  `applied=keep_activation_external invoke=package_helper mode=package_helper
  advanced=true call_execute=false` (or residual open
  `applied=open_residual_entry residual_route=true`), residual_export denied
- New: follow and dispatch attach
  ...
  `wake_route_apply_follow_pin_call_next_call_follow_action`,
  `wake_route_apply_follow_pin_call_next_call_follow_mode`,
  `wake_route_apply_follow_pin_call_next_call_follow_helper`,
  `wake_route_apply_follow_pin_call_next_call_follow_call_execute`, and
  `wake_route_apply_follow_pin_call_next_call_follow_ready` after
  cascade_wake_route_apply_follow_pin_call_next_call packaging
- New: operator_state exports nested
  ...
  and `continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_ready`
  (alongside pin_call_next_call / pin_call_next / pin_call / pin / follow / apply /
  wake route / wake / transition / cascade / residual cascade surfaces)
- Ready/unrecorded continue cascade wake route apply follow pin call next call
  follow: identity `applied=execute_now mode=execute_helper call_execute=true
  follow_ready=true` (or inventory_only when execute is not recommended);
...
- Full follow after reverse-flow pass: continue cascade wake route still reports
  ...
  continue_cascade_wake_route_apply_follow_pin_call_next_call_follow surfaces
  themselves
