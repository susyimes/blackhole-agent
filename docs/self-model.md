...
Observed this run (digest `github-growth-20260715T035131.989105Z`,
...
- Prior: pin_call_next_call_follow_pin_call_next seals pin_call into a body-free
  next-invoke recipe (`next_action` / `next_invoke` / `call_next_with_execute` /
  `next_ready`) so supervisors do not re-derive execute vs package policy from
  nested pin_call transitions after continue wakes
- New: `package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call`
  collapses pre/post pin_call_next recipes into body-free
...
- Pin call next call follow pin call next call fields: `next_action_transition`,
  `next_invoke_transition`, `next_advanced`, `applied_next_action`,
  `applied_next_invoke`, `pre_next_action`/`post_next_action`,
  `pre_next_invoke`/`post_next_invoke`, reverse/residual progress transitions
- Identity / inventory snapshot: pin_call_next_call_follow_pin_call_next_call
  `action=execute_now→execute_now invoke=execute_helper→execute_helper
  next_advanced=false` (or inventory_only→inventory_only when execute is not
  recommended); residual_export denied
- Full follow after reverse-flow pass: pin_call_next_call_follow_pin_call_next_call
  reports `action=execute_now→keep_activation_external
  invoke=execute_helper→package_helper next_advanced=true` (or residual open
...
- New: follow and dispatch attach
  `continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call`,
  `continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_line`,
  `continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_helper`,
  `wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_action_transition`,
  `wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_invoke_transition`, and
  `wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_advanced` after
  pin_call_next packaging
- New: operator_state exports nested
  ...
  and `continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_advanced`
  (alongside pin_call_next_call_follow_pin_call_next / pin_call_next_call_follow_pin_call /
...
- Ready/unrecorded continue cascade wake route apply follow pin call next call
  follow pin call next call: identity `action=execute_now→execute_now
  invoke=execute_helper→execute_helper next_advanced=false` (or
  inventory_only→inventory_only when execute is not recommended);
  residual_export denied
