...
from blackhole_agent.addrpolicy_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_addrpolicy_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_addrpolicy_header,
    independent_policydigest,
    parse_http_request,
    parse_http_response,
    parse_message,
    parse_addrpolicy,
    parse_addrpolicy_header,
    public_request,
    public_response,
    run_addrpolicy_workflow,
    serialize_addrpolicy,
    temporary_request,
    temporary_response,
)
...
def test_goal_binds_addrpolicy_actuation_plane() -> None:
    ...
    assert leftover_marker_ids(ADDRPOLICY_LEFTOVER) == (ADDRPOLICY_ACTUATION_ID,)
    assert leftover_marker_ids(FIRSTHOP_ACTUATION_GOAL) == (FIRSTHOP_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert ADDRPOLICY_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(ADDRPOLICY_ACTUATION_GOAL)
...
def test_opted_in_addrpolicy_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("addrpolicy",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, ADDRPOLICY_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["addrpolicy"]

    missing = run_addrpolicy_workflow(with_policyid=False)
    skip_bind = run_addrpolicy_workflow(skip_bind=True)
    skip_temporary = run_addrpolicy_workflow(do_temporary=False)
    skip_public = run_addrpolicy_workflow(do_public=False)
    skip_policydigest = run_addrpolicy_workflow(do_policydigest=False)
    skip_replay = run_addrpolicy_workflow(replay=False)
    skip_policyid = run_addrpolicy_workflow(use_policyid=False)
    live = run_addrpolicy_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_policyid"
    ...
    assert skip_policydigest["ok"] is False
    assert skip_policydigest["error"] == "policydigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_policyid["ok"] is False
    assert skip_policyid["error"] == "policyid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_policydigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["policydigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["policyid_bound"] is True
    assert row["digest"]
    assert live["policyid"] == DEFAULT_POLICYID
    assert live["policydigest"] == DEFAULT_POLICYDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, policyid=DEFAULT_POLICYID, policydigest=DEFAULT_POLICYDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["policyid"] == DEFAULT_POLICYID
    assert queried["policydigest"] == DEFAULT_POLICYDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == ADDRPOLICY_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, policyid=DEFAULT_POLICYID, policydigest=DEFAULT_POLICYDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["policyid"] == DEFAULT_POLICYID
    assert answered["policydigest"] == DEFAULT_POLICYDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, policyid=DEFAULT_POLICYID, include_policyid=False)
    )
    assert bare["has_policyid"] is False
    assert bare["policyid"] == EMPTY_POLICYID
    publicised = serialize_addrpolicy(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_addrpolicy(publicised) == DEFAULT_SOURCE
    assert parse_addrpolicy(RFC_DEST_FIELD) == DEST_POLICY
    header = parse_addrpolicy_header(encode_addrpolicy_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_POLICYID, DEFAULT_POLICYDIGEST))
    ...
    assert asked["method"] == "POLICY"
    assert asked["addrpolicy_kind"] == "policy"
    assert listed["addrpolicy_kind"] == "table"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_POLICY
    assert canonical_temporary(SENTINEL, DEFAULT_POLICYID).startswith("POLICY")
    assert "policydigest=" in canonical_public(SENTINEL, DEFAULT_POLICYID, DEFAULT_POLICYDIGEST)


def test_builtin_proof_seals_addrpolicy_actuation() -> None:
    report = builtin_addrpolicy_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "addrpolicy_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_addrpolicy"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_policyid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_policydigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_policyid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_policydigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_addrpolicy"]
    ...
    assert report["checks"]["leftover_text_binds_addrpolicy"]
    assert report["checks"]["proved_addrpolicy_consumes_leftover"]
    assert report["mission_goal"] == ADDRPOLICY_ACTUATION_GOAL
    assert report["done_when"] == ADDRPOLICY_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[ADDRPOLICY_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "addrpolicy" in capability.tags
    assert "rfc7078" in capability.tags
    assert "http" in capability.tags
    assert "policyid" in capability.tags
    assert "policydigest" in capability.tags
    assert "policy" in capability.tags
    assert "table" in capability.tags


def test_selection_gate_rejects_ledger_only_addrpolicy_family(tmp_path: Path) -> None:
    ...
    assert "addrpolicy" in family.split("/")
    assert "rfc7078" in family
    assert "policyid" in family
    assert "policydigest" in family
