...
def test_goal_binds_pwldp_actuation_plane() -> None:
    assert leftover_marker_ids(PWLDP_ACTUATION_GOAL) == (PWLDP_ACTUATION_ID,)
    ...
    assert PWLDP_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(PWLDP_LEFTOVER) == (PWLDP_ACTUATION_ID,)
    assert leftover_marker_ids(PWLDP_ACTUATION_GOAL) == (PWLDP_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert PWLDP_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(PWLDP_ACTUATION_GOAL)
...
def test_opted_in_pwldp_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("pwldp",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, PWLDP_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["pwldp"]

    missing = run_pwldp_workflow(with_pwldpid=False)
    skip_bind = run_pwldp_workflow(skip_bind=True)
    skip_temporary = run_pwldp_workflow(do_temporary=False)
    skip_public = run_pwldp_workflow(do_public=False)
    skip_pwldpdigest = run_pwldp_workflow(do_pwldpdigest=False)
    skip_replay = run_pwldp_workflow(replay=False)
    skip_pwldpid = run_pwldp_workflow(use_pwldpid=False)
    live = run_pwldp_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_pwldpid"
    ...
    assert skip_pwldpdigest["ok"] is False
    assert skip_pwldpdigest["error"] == "pwldpdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_pwldpid["ok"] is False
    assert skip_pwldpid["error"] == "pwldpid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_pwldpdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["pwldpdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["pwldpid_bound"] is True
    assert row["digest"]
    assert live["pwldpid"] == DEFAULT_PWLDPID
    assert live["pwldpdigest"] == DEFAULT_PWLDPDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, pwldpid=DEFAULT_PWLDPID, pwldpdigest=DEFAULT_PWLDPDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["pwldpid"] == DEFAULT_PWLDPID
    assert queried["pwldpdigest"] == DEFAULT_PWLDPDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == PWLDP_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, pwldpid=DEFAULT_PWLDPID, pwldpdigest=DEFAULT_PWLDPDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["pwldpid"] == DEFAULT_PWLDPID
    assert answered["pwldpdigest"] == DEFAULT_PWLDPDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, pwldpid=DEFAULT_PWLDPID, include_pwldpid=False)
    )
    assert bare["has_pwldpid"] is False
    assert bare["pwldpid"] == EMPTY_PWLDPID
    publicised = serialize_pwldp(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_pwldp(publicised) == DEFAULT_SOURCE
    assert parse_pwldp(RFC_DEST_FIELD) == DEST_HOP
    header = parse_pwldp_header(encode_pwldp_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_PWLDPID, DEFAULT_PWLDPDIGEST))
    ...
    assert listed["disc_kind"] == "status"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_PWLDPID).startswith("LABEL")
    assert "pwldpdigest=" in canonical_public(SENTINEL, DEFAULT_PWLDPID, DEFAULT_PWLDPDIGEST)


def test_builtin_proof_seals_pwldp_actuation() -> None:
    report = builtin_pwldp_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "pwldp_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_pwldp"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_pwldpid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_pwldpdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_pwldpid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_pwldpdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_pwldp"]
    ...
    assert report["checks"]["leftover_text_binds_pwldp"]
    assert report["checks"]["proved_pwldp_consumes_leftover"]
    assert report["mission_goal"] == PWLDP_ACTUATION_GOAL
    assert report["done_when"] == PWLDP_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[PWLDP_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "pwldp" in capability.tags
    assert "rfc4447" in capability.tags
    assert "http" in capability.tags
    assert "pwldpid" in capability.tags
    assert "pwldpdigest" in capability.tags
    assert "pwldp" in capability.tags
    assert "status" in capability.tags


def test_selection_gate_rejects_ledger_only_pwldp_family(tmp_path: Path) -> None:
    ...
    assert "pwldp" in family.split("/")
    assert "rfc4447" in set(semantic_tokens(PWLDP_ACTUATION_GOAL))
    assert "pwldpid" in set(semantic_tokens(PWLDP_ACTUATION_GOAL))
    assert "pwldpdigest" in family
