...
def test_goal_binds_ilpce_actuation_plane() -> None:
    assert leftover_marker_ids(ILPCE_ACTUATION_GOAL) == (ILPCE_ACTUATION_ID,)
    ...
    assert ILPCE_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(ILPCE_LEFTOVER) == (ILPCE_ACTUATION_ID,)
    assert leftover_marker_ids(ILPCE_ACTUATION_GOAL) == (ILPCE_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert ILPCE_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(ILPCE_ACTUATION_GOAL)
...
def test_opted_in_ilpce_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("ilpce",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, ILPCE_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["ilpce"]

    missing = run_ilpce_workflow(with_ilpceid=False)
    skip_bind = run_ilpce_workflow(skip_bind=True)
    skip_temporary = run_ilpce_workflow(do_temporary=False)
    skip_public = run_ilpce_workflow(do_public=False)
    skip_ilpcedigest = run_ilpce_workflow(do_ilpcedigest=False)
    skip_replay = run_ilpce_workflow(replay=False)
    skip_ilpceid = run_ilpce_workflow(use_ilpceid=False)
    live = run_ilpce_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_ilpceid"
    ...
    assert skip_ilpcedigest["ok"] is False
    assert skip_ilpcedigest["error"] == "ilpcedigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_ilpceid["ok"] is False
    assert skip_ilpceid["error"] == "ilpceid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_ilpcedigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["ilpcedigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["ilpceid_bound"] is True
    assert row["digest"]
    assert live["ilpceid"] == DEFAULT_ILPCEID
    assert live["ilpcedigest"] == DEFAULT_ILPCEDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, ilpceid=DEFAULT_ILPCEID, ilpcedigest=DEFAULT_ILPCEDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["ilpceid"] == DEFAULT_ILPCEID
    assert queried["ilpcedigest"] == DEFAULT_ILPCEDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == ILPCE_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, ilpceid=DEFAULT_ILPCEID, ilpcedigest=DEFAULT_ILPCEDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["ilpceid"] == DEFAULT_ILPCEID
    assert answered["ilpcedigest"] == DEFAULT_ILPCEDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, ilpceid=DEFAULT_ILPCEID, include_ilpceid=False)
    )
    assert bare["has_ilpceid"] is False
    assert bare["ilpceid"] == EMPTY_ILPCEID
    publicised = serialize_ilpce(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_ilpce(publicised) == DEFAULT_SOURCE
    assert parse_ilpce(RFC_DEST_FIELD) == DEST_HOP
    header = parse_ilpce_header(encode_ilpce_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_ILPCEID, DEFAULT_ILPCEDIGEST))
    ...
    assert asked["method"] == "VNTM"
    assert asked["disc_kind"] == "vntm"
    assert listed["disc_kind"] == "layer"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_ILPCEID).startswith("VNTM")
    assert "ilpcedigest=" in canonical_public(SENTINEL, DEFAULT_ILPCEID, DEFAULT_ILPCEDIGEST)


def test_builtin_proof_seals_ilpce_actuation() -> None:
    report = builtin_ilpce_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "ilpce_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_ilpce"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_ilpceid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_ilpcedigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_ilpceid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_ilpcedigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_binds_ilpce"]
    ...
    assert report["checks"]["leftover_text_binds_ilpce"]
    assert report["checks"]["proved_ilpce_consumes_leftover"]
    assert report["mission_goal"] == ILPCE_ACTUATION_GOAL
    assert report["done_when"] == ILPCE_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[ILPCE_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "ilpce" in capability.tags
    assert "rfc5623" in capability.tags
    assert "http" in capability.tags
    assert "ilpceid" in capability.tags
    assert "ilpcedigest" in capability.tags
    assert "vntm" in capability.tags
    assert "layer" in capability.tags


def test_selection_gate_rejects_ledger_only_ilpce_family(tmp_path: Path) -> None:
    ...
    assert "ilpce" in family.split("/")
    assert "rfc5623" in set(semantic_tokens(ILPCE_ACTUATION_GOAL))
    assert "ilpceid" in set(semantic_tokens(ILPCE_ACTUATION_GOAL))
    assert "ilpcedigest" in family
