...
from blackhole_agent.objfun_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_objfun_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_objfun_header,
    independent_objfundigest,
    parse_http_comm,
    parse_http_response,
    parse_message,
    parse_objfun,
...
def test_goal_binds_objfun_actuation_plane() -> None:
    assert leftover_marker_ids(OBJFUN_ACTUATION_GOAL) == (OBJFUN_ACTUATION_ID,)
    ...
    assert OBJFUN_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(OBJFUN_LEFTOVER) == (OBJFUN_ACTUATION_ID,)
    assert leftover_marker_ids(OBJFUN_ACTUATION_GOAL) == (OBJFUN_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert OBJFUN_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(OBJFUN_ACTUATION_GOAL)
...
def test_opted_in_objfun_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("objfun",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, OBJFUN_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["objfun"]

    missing = run_objfun_workflow(with_objfunid=False)
    skip_bind = run_objfun_workflow(skip_bind=True)
    skip_temporary = run_objfun_workflow(do_temporary=False)
    skip_public = run_objfun_workflow(do_public=False)
    skip_objfundigest = run_objfun_workflow(do_objfundigest=False)
    skip_replay = run_objfun_workflow(replay=False)
    skip_objfunid = run_objfun_workflow(use_objfunid=False)
    live = run_objfun_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_objfunid"
    ...
    assert skip_objfundigest["ok"] is False
    assert skip_objfundigest["error"] == "objfundigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_objfunid["ok"] is False
    assert skip_objfunid["error"] == "objfunid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_objfundigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["objfundigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["objfunid_bound"] is True
    assert row["digest"]
    assert live["objfunid"] == DEFAULT_OBJFUNID
    assert live["objfundigest"] == DEFAULT_OBJFUNDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, objfunid=DEFAULT_OBJFUNID, objfundigest=DEFAULT_OBJFUNDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["objfunid"] == DEFAULT_OBJFUNID
    assert queried["objfundigest"] == DEFAULT_OBJFUNDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == OBJFUN_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, objfunid=DEFAULT_OBJFUNID, objfundigest=DEFAULT_OBJFUNDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["objfunid"] == DEFAULT_OBJFUNID
    assert answered["objfundigest"] == DEFAULT_OBJFUNDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, objfunid=DEFAULT_OBJFUNID, include_objfunid=False)
    )
    assert bare["has_objfunid"] is False
    assert bare["objfunid"] == EMPTY_OBJFUNID
    publicised = serialize_objfun(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_objfun(publicised) == DEFAULT_SOURCE
    assert parse_objfun(RFC_DEST_FIELD) == DEST_HOP
    header = parse_objfun_header(encode_objfun_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_OBJFUNID, DEFAULT_OBJFUNDIGEST))
    ...
    assert asked["method"] == "OBJ"
    assert asked["disc_kind"] == "obj"
    assert listed["disc_kind"] == "fun"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_OBJFUNID).startswith("OBJ")
    assert "objfundigest=" in canonical_public(SENTINEL, DEFAULT_OBJFUNID, DEFAULT_OBJFUNDIGEST)


def test_builtin_proof_seals_objfun_actuation() -> None:
    report = builtin_objfun_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "objfun_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_objfun"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_objfunid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_objfundigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_objfunid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_objfundigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_binds_objfun"]
    ...
    assert report["checks"]["leftover_text_binds_objfun"]
    assert report["checks"]["proved_objfun_consumes_leftover"]
    assert report["mission_goal"] == OBJFUN_ACTUATION_GOAL
    assert report["done_when"] == OBJFUN_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[OBJFUN_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "objfun" in capability.tags
    assert "rfc5541" in capability.tags
    assert "http" in capability.tags
    assert "objfunid" in capability.tags
    assert "objfundigest" in capability.tags
    assert "obj" in capability.tags
    assert "fun" in capability.tags


def test_selection_gate_rejects_ledger_only_objfun_family(tmp_path: Path) -> None:
    ...
    assert "objfun" in family.split("/")
    assert "rfc5541" in set(semantic_tokens(OBJFUN_ACTUATION_GOAL))
    assert "objfunid" in set(semantic_tokens(OBJFUN_ACTUATION_GOAL))
    assert "objfundigest" in family
