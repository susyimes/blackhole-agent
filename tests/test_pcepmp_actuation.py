...
from blackhole_agent.pcepmp_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_pcepmp_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_pcepmp_header,
    independent_pcepmpdigest,
    parse_http_comm,
    parse_http_response,
    parse_message,
    parse_pcepmp,
...
def test_goal_binds_pcepmp_actuation_plane() -> None:
    assert leftover_marker_ids(PCEPMP_ACTUATION_GOAL) == (PCEPMP_ACTUATION_ID,)
    ...
    assert PCEPMP_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(PCEPMP_LEFTOVER) == (PCEPMP_ACTUATION_ID,)
    assert leftover_marker_ids(PCEPMP_ACTUATION_GOAL) == (PCEPMP_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert PCEPMP_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(PCEPMP_ACTUATION_GOAL)
...
def test_opted_in_pcepmp_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("pcepmp",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, PCEPMP_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["pcepmp"]

    missing = run_pcepmp_workflow(with_pcepmpid=False)
    skip_bind = run_pcepmp_workflow(skip_bind=True)
    skip_temporary = run_pcepmp_workflow(do_temporary=False)
    skip_public = run_pcepmp_workflow(do_public=False)
    skip_pcepmpdigest = run_pcepmp_workflow(do_pcepmpdigest=False)
    skip_replay = run_pcepmp_workflow(replay=False)
    skip_pcepmpid = run_pcepmp_workflow(use_pcepmpid=False)
    live = run_pcepmp_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_pcepmpid"
    ...
    assert skip_pcepmpdigest["ok"] is False
    assert skip_pcepmpdigest["error"] == "pcepmpdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_pcepmpid["ok"] is False
    assert skip_pcepmpid["error"] == "pcepmpid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_pcepmpdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["pcepmpdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["pcepmpid_bound"] is True
    assert row["digest"]
    assert live["pcepmpid"] == DEFAULT_PCEPMPID
    assert live["pcepmpdigest"] == DEFAULT_PCEPMPDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, pcepmpid=DEFAULT_PCEPMPID, pcepmpdigest=DEFAULT_PCEPMPDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["pcepmpid"] == DEFAULT_PCEPMPID
    assert queried["pcepmpdigest"] == DEFAULT_PCEPMPDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == PCEPMP_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, pcepmpid=DEFAULT_PCEPMPID, pcepmpdigest=DEFAULT_PCEPMPDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["pcepmpid"] == DEFAULT_PCEPMPID
    assert answered["pcepmpdigest"] == DEFAULT_PCEPMPDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, pcepmpid=DEFAULT_PCEPMPID, include_pcepmpid=False)
    )
    assert bare["has_pcepmpid"] is False
    assert bare["pcepmpid"] == EMPTY_PCEPMPID
    publicised = serialize_pcepmp(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_pcepmp(publicised) == DEFAULT_SOURCE
    assert parse_pcepmp(RFC_DEST_FIELD) == DEST_HOP
    header = parse_pcepmp_header(encode_pcepmp_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_PCEPMPID, DEFAULT_PCEPMPDIGEST))
    ...
    assert asked["method"] == "P2MP"
    assert asked["disc_kind"] == "p2mp"
    assert listed["disc_kind"] == "endpoints"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_PCEPMPID).startswith("P2MP")
    assert "pcepmpdigest=" in canonical_public(SENTINEL, DEFAULT_PCEPMPID, DEFAULT_PCEPMPDIGEST)


def test_builtin_proof_seals_pcepmp_actuation() -> None:
    report = builtin_pcepmp_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "pcepmp_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_pcepmp"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_pcepmpid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_pcepmpdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_pcepmpid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_pcepmpdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_binds_pcepmp"]
    ...
    assert report["checks"]["catalog_names_pcepmp"]
    assert report["checks"]["catalog_names_wson"]
    assert report["checks"]["leftover_text_binds_pcepmp"]
    assert report["checks"]["proved_pcepmp_consumes_leftover"]
    assert report["mission_goal"] == PCEPMP_ACTUATION_GOAL
    assert report["done_when"] == PCEPMP_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[PCEPMP_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "pcepmp" in capability.tags
    assert "rfc6006" in capability.tags
    assert "http" in capability.tags
    assert "pcepmpid" in capability.tags
    assert "pcepmpdigest" in capability.tags
    assert "p2mp" in capability.tags
    assert "endpoints" in capability.tags


def test_selection_gate_rejects_ledger_only_pcepmp_family(tmp_path: Path) -> None:
    ...
    assert "pcepmp" in family.split("/")
    assert "rfc6006" in set(semantic_tokens(PCEPMP_ACTUATION_GOAL))
    assert "pcepmpid" in set(semantic_tokens(PCEPMP_ACTUATION_GOAL))
    assert "pcepmpdigest" in family
