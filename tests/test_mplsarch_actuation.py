...
from blackhole_agent.mplsarch_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_mplsarch_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_mplsarch_header,
    independent_mplsdigest,
    parse_http_comm,
    parse_http_response,
    parse_message,
    parse_mplsarch,
    parse_mplsarch_header,
    public_comm,
    public_response,
    run_mplsarch_workflow,
    serialize_mplsarch,
    temporary_comm,
    temporary_response,
)
...
def test_goal_binds_mplsarch_actuation_plane() -> None:
    assert leftover_marker_ids(MPLSARCH_ACTUATION_GOAL) == (MPLSARCH_ACTUATION_ID,)
    ...
    assert MPLSARCH_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(MPLSARCH_LEFTOVER) == (MPLSARCH_ACTUATION_ID,)
    assert leftover_marker_ids(MPLSARCH_ACTUATION_GOAL) == (MPLSARCH_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert MPLSARCH_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(MPLSARCH_ACTUATION_GOAL)
...
def test_opted_in_mplsarch_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("mplsarch",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, MPLSARCH_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["mplsarch"]

    missing = run_mplsarch_workflow(with_mplsid=False)
    skip_bind = run_mplsarch_workflow(skip_bind=True)
    skip_temporary = run_mplsarch_workflow(do_temporary=False)
    skip_public = run_mplsarch_workflow(do_public=False)
    skip_mplsdigest = run_mplsarch_workflow(do_mplsdigest=False)
    skip_replay = run_mplsarch_workflow(replay=False)
    skip_mplsid = run_mplsarch_workflow(use_mplsid=False)
    live = run_mplsarch_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_mplsid"
    ...
    assert skip_mplsdigest["ok"] is False
    assert skip_mplsdigest["error"] == "mplsdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_mplsid["ok"] is False
    assert skip_mplsid["error"] == "mplsid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_mplsdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["mplsdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["mplsid_bound"] is True
    assert row["digest"]
    assert live["mplsid"] == DEFAULT_MPLSID
    assert live["mplsdigest"] == DEFAULT_MPLSDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, mplsid=DEFAULT_MPLSID, mplsdigest=DEFAULT_MPLSDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["mplsid"] == DEFAULT_MPLSID
    assert queried["mplsdigest"] == DEFAULT_MPLSDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == MPLSARCH_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, mplsid=DEFAULT_MPLSID, mplsdigest=DEFAULT_MPLSDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["mplsid"] == DEFAULT_MPLSID
    assert answered["mplsdigest"] == DEFAULT_MPLSDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, mplsid=DEFAULT_MPLSID, include_mplsid=False)
    )
    assert bare["has_mplsid"] is False
    assert bare["mplsid"] == EMPTY_MPLSID
    publicised = serialize_mplsarch(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_mplsarch(publicised) == DEFAULT_SOURCE
    assert parse_mplsarch(RFC_DEST_FIELD) == DEST_HOP
    header = parse_mplsarch_header(encode_mplsarch_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_MPLSID, DEFAULT_MPLSDIGEST))
    ...
    assert asked["method"] == "FEC"
    assert asked["disc_kind"] == "fec"
    assert listed["disc_kind"] == "nhlfe"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_MPLSID).startswith("FEC")
    assert "mplsdigest=" in canonical_public(SENTINEL, DEFAULT_MPLSID, DEFAULT_MPLSDIGEST)


def test_builtin_proof_seals_mplsarch_actuation() -> None:
    report = builtin_mplsarch_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "mplsarch_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_mplsarch"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_mplsid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_mplsdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_mplsid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_mplsdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_mplsarch"]
    ...
    assert report["checks"]["leftover_text_binds_mplsarch"]
    assert report["checks"]["proved_mplsarch_consumes_leftover"]
    assert report["mission_goal"] == MPLSARCH_ACTUATION_GOAL
    assert report["done_when"] == MPLSARCH_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[MPLSARCH_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "mplsarch" in capability.tags
    assert "rfc3031" in capability.tags
    assert "http" in capability.tags
    assert "mplsid" in capability.tags
    assert "mplsdigest" in capability.tags
    assert "fec" in capability.tags
    assert "nhlfe" in capability.tags


def test_selection_gate_rejects_ledger_only_mplsarch_family(tmp_path: Path) -> None:
    ...
    assert "mplsarch" in family.split("/")
    assert "rfc3031" in set(semantic_tokens(MPLSARCH_ACTUATION_GOAL))
    assert "mplsid" in set(semantic_tokens(MPLSARCH_ACTUATION_GOAL))
    assert "mplsdigest" in family
