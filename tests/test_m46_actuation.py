...
def test_goal_binds_m46_actuation_plane() -> None:
    assert leftover_marker_ids(M46_ACTUATION_GOAL) == (M46_ACTUATION_ID,)
    ...
    assert M46_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(M46_LEFTOVER) == (M46_ACTUATION_ID,)
    assert leftover_marker_ids(M46_ACTUATION_GOAL) == (M46_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert M46_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(M46_ACTUATION_GOAL)
...
def test_opted_in_m46_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("m46",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, M46_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["m46"]

    missing = run_m46_workflow(with_m46id=False)
    skip_bind = run_m46_workflow(skip_bind=True)
    skip_temporary = run_m46_workflow(do_temporary=False)
    skip_public = run_m46_workflow(do_public=False)
    skip_m46digest = run_m46_workflow(do_m46digest=False)
    skip_replay = run_m46_workflow(replay=False)
    skip_m46id = run_m46_workflow(use_m46id=False)
    live = run_m46_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_m46id"
    ...
    assert skip_m46digest["ok"] is False
    assert skip_m46digest["error"] == "m46digest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_m46id["ok"] is False
    assert skip_m46id["error"] == "m46id_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_m46digest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["m46digest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["m46id_bound"] is True
    assert row["digest"]
    assert live["m46id"] == DEFAULT_M46ID
    assert live["m46digest"] == DEFAULT_M46DIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, m46id=DEFAULT_M46ID, m46digest=DEFAULT_M46DIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["m46id"] == DEFAULT_M46ID
    assert queried["m46digest"] == DEFAULT_M46DIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == M46_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, m46id=DEFAULT_M46ID, m46digest=DEFAULT_M46DIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["m46id"] == DEFAULT_M46ID
    assert answered["m46digest"] == DEFAULT_M46DIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, m46id=DEFAULT_M46ID, include_m46id=False)
    )
    assert bare["has_m46id"] is False
    assert bare["m46id"] == EMPTY_M46ID
    publicised = serialize_m46(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_m46(publicised) == DEFAULT_SOURCE
    assert parse_m46(RFC_DEST_FIELD) == DEST_HOP
    header = parse_m46_header(encode_m46_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_M46ID, DEFAULT_M46DIGEST))
    ...
    assert asked["method"] == "ASM"
    assert asked["disc_kind"] == "asm"
    assert listed["disc_kind"] == "ssm"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_M46ID).startswith("ASM")
    assert "m46digest=" in canonical_public(SENTINEL, DEFAULT_M46ID, DEFAULT_M46DIGEST)


def test_builtin_proof_seals_m46_actuation() -> None:
    report = builtin_m46_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "m46_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_m46"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_m46id_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_m46digest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_m46id_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_m46digest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_m46"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_m46"]
    assert report["checks"]["proved_m46_consumes_leftover"]
    assert report["mission_goal"] == M46_ACTUATION_GOAL
    assert report["done_when"] == M46_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[M46_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "m46" in capability.tags
    assert "rfc8114" in capability.tags
    assert "http" in capability.tags
    assert "m46id" in capability.tags
    assert "m46digest" in capability.tags
    assert "asm" in capability.tags
    assert "ssm" in capability.tags


def test_selection_gate_rejects_ledger_only_m46_family(tmp_path: Path) -> None:
    ...
    assert "m46" in family.split("/")
    assert "rfc8114" in family
    assert "m46id" in family
    assert "m46digest" in family
    ...
    assert "prefix64" not in family.split("/")
    assert "rfc8115" not in family
    assert "prefix64id" not in family
    assert "prefix64digest" not in family
