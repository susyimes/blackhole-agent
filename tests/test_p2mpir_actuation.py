...
from blackhole_agent.p2mpir_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_p2mpir_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_p2mpir_header,
    independent_p2mpirdigest,
    parse_http_comm,
    parse_http_response,
    parse_message,
    parse_p2mpir,
...
def test_goal_binds_p2mpir_actuation_plane() -> None:
    assert leftover_marker_ids(P2MPIR_ACTUATION_GOAL) == (P2MPIR_ACTUATION_ID,)
    ...
    assert P2MPIR_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(P2MPIR_LEFTOVER) == (P2MPIR_ACTUATION_ID,)
    assert leftover_marker_ids(P2MPIR_ACTUATION_GOAL) == (P2MPIR_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert P2MPIR_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(P2MPIR_ACTUATION_GOAL)
...
def test_opted_in_p2mpir_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("p2mpir",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, P2MPIR_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["p2mpir"]

    missing = run_p2mpir_workflow(with_p2mpirid=False)
    skip_bind = run_p2mpir_workflow(skip_bind=True)
    skip_temporary = run_p2mpir_workflow(do_temporary=False)
    skip_public = run_p2mpir_workflow(do_public=False)
    skip_p2mpirdigest = run_p2mpir_workflow(do_p2mpirdigest=False)
    skip_replay = run_p2mpir_workflow(replay=False)
    skip_p2mpirid = run_p2mpir_workflow(use_p2mpirid=False)
    live = run_p2mpir_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_p2mpirid"
    ...
    assert skip_p2mpirdigest["ok"] is False
    assert skip_p2mpirdigest["error"] == "p2mpirdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_p2mpirid["ok"] is False
    assert skip_p2mpirid["error"] == "p2mpirid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_p2mpirdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["p2mpirdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["p2mpirid_bound"] is True
    assert row["digest"]
    assert live["p2mpirid"] == DEFAULT_P2MPIRID
    assert live["p2mpirdigest"] == DEFAULT_P2MPIRDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, p2mpirid=DEFAULT_P2MPIRID, p2mpirdigest=DEFAULT_P2MPIRDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["p2mpirid"] == DEFAULT_P2MPIRID
    assert queried["p2mpirdigest"] == DEFAULT_P2MPIRDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == P2MPIR_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, p2mpirid=DEFAULT_P2MPIRID, p2mpirdigest=DEFAULT_P2MPIRDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["p2mpirid"] == DEFAULT_P2MPIRID
    assert answered["p2mpirdigest"] == DEFAULT_P2MPIRDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, p2mpirid=DEFAULT_P2MPIRID, include_p2mpirid=False)
    )
    assert bare["has_p2mpirid"] is False
    assert bare["p2mpirid"] == EMPTY_P2MPIRID
    publicised = serialize_p2mpir(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_p2mpir(publicised) == DEFAULT_SOURCE
    assert parse_p2mpir(RFC_DEST_FIELD) == DEST_HOP
    header = parse_p2mpir_header(encode_p2mpir_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_P2MPIRID, DEFAULT_P2MPIRDIGEST))
    ...
    assert asked["disc_kind"] == "p2mpir"
    assert listed["disc_kind"] == "ir"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_P2MPIRID).startswith("P2MP")
    assert "p2mpirdigest=" in canonical_public(SENTINEL, DEFAULT_P2MPIRID, DEFAULT_P2MPIRDIGEST)


def test_builtin_proof_seals_p2mpir_actuation() -> None:
    report = builtin_p2mpir_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "p2mpir_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_p2mpir"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_p2mpirid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_p2mpirdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_p2mpirid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_p2mpirdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_p2mpir"]
    ...
    assert report["checks"]["leftover_text_binds_p2mpir"]
    assert report["checks"]["proved_p2mpir_consumes_leftover"]
    assert report["mission_goal"] == P2MPIR_ACTUATION_GOAL
    assert report["done_when"] == P2MPIR_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[P2MPIR_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "p2mpir" in capability.tags
    assert "rfc10018" in capability.tags
    assert "http" in capability.tags
    assert "p2mpirid" in capability.tags
    assert "p2mpirdigest" in capability.tags
    assert "p2mpir" in capability.tags
    assert "ir" in capability.tags


def test_selection_gate_rejects_ledger_only_p2mpir_family(tmp_path: Path) -> None:
    ...
    assert "p2mpir" in family.split("/")
    assert "rfc10018" in set(semantic_tokens(P2MPIR_ACTUATION_GOAL))
    assert "p2mpirid" in set(semantic_tokens(P2MPIR_ACTUATION_GOAL))
    assert "p2mpirdigest" in family
