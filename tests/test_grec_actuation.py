...
def test_goal_binds_grec_actuation_plane() -> None:
    assert leftover_marker_ids(GREC_ACTUATION_GOAL) == (GREC_ACTUATION_ID,)
    ...
    assert GREC_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(GREC_LEFTOVER) == (GREC_ACTUATION_ID,)
    assert leftover_marker_ids(GREC_ACTUATION_GOAL) == (GREC_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert GREC_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(GREC_ACTUATION_GOAL)
...
def test_opted_in_grec_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("grec",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, GREC_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["grec"]

    missing = run_grec_workflow(with_grecid=False)
    skip_bind = run_grec_workflow(skip_bind=True)
    skip_temporary = run_grec_workflow(do_temporary=False)
    skip_public = run_grec_workflow(do_public=False)
    skip_grecdigest = run_grec_workflow(do_grecdigest=False)
    skip_replay = run_grec_workflow(replay=False)
    skip_grecid = run_grec_workflow(use_grecid=False)
    live = run_grec_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_grecid"
    ...
    assert skip_grecdigest["ok"] is False
    assert skip_grecdigest["error"] == "grecdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_grecid["ok"] is False
    assert skip_grecid["error"] == "grecid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_grecdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["grecdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["grecid_bound"] is True
    assert row["digest"]
    assert live["grecid"] == DEFAULT_GRECID
    assert live["grecdigest"] == DEFAULT_GRECDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, grecid=DEFAULT_GRECID, grecdigest=DEFAULT_GRECDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["grecid"] == DEFAULT_GRECID
    assert queried["grecdigest"] == DEFAULT_GRECDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == GREC_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, grecid=DEFAULT_GRECID, grecdigest=DEFAULT_GRECDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["grecid"] == DEFAULT_GRECID
    assert answered["grecdigest"] == DEFAULT_GRECDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, grecid=DEFAULT_GRECID, include_grecid=False)
    )
    assert bare["has_grecid"] is False
    assert bare["grecid"] == EMPTY_GRECID
    publicised = serialize_grec(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_grec(publicised) == DEFAULT_SOURCE
    assert parse_grec(RFC_DEST_FIELD) == DEST_HOP
    header = parse_grec_header(encode_grec_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_GRECID, DEFAULT_GRECDIGEST))
    ...
    assert asked["method"] == "NOTIFY"
    assert asked["disc_kind"] == "notify"
    assert listed["disc_kind"] == "restore"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_GRECID).startswith("NOTIFY")
    assert "grecdigest=" in canonical_public(SENTINEL, DEFAULT_GRECID, DEFAULT_GRECDIGEST)


def test_builtin_proof_seals_grec_actuation() -> None:
    report = builtin_grec_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "grec_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_grec"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_grecid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_grecdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_grecid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_grecdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_grec"]
    ...
    assert report["checks"]["leftover_text_binds_grec"]
    assert report["checks"]["proved_grec_consumes_leftover"]
    assert report["mission_goal"] == GREC_ACTUATION_GOAL
    assert report["done_when"] == GREC_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[GREC_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "grec" in capability.tags
    assert "rfc4426" in capability.tags
    assert "http" in capability.tags
    assert "grecid" in capability.tags
    assert "grecdigest" in capability.tags
    assert "notify" in capability.tags
    assert "restore" in capability.tags


def test_selection_gate_rejects_ledger_only_grec_family(tmp_path: Path) -> None:
    ...
    assert "grec" in family.split("/")
    assert "rfc4426" in set(semantic_tokens(GREC_ACTUATION_GOAL))
    assert "grecid" in set(semantic_tokens(GREC_ACTUATION_GOAL))
    assert "grecdigest" in family
