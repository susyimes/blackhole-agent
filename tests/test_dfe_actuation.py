...
def test_goal_binds_dfe_actuation_plane() -> None:
    assert leftover_marker_ids(DFE_ACTUATION_GOAL) == (DFE_ACTUATION_ID,)
    ...
    assert DFE_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(DFE_LEFTOVER) == (DFE_ACTUATION_ID,)
    assert leftover_marker_ids(DFE_ACTUATION_GOAL) == (DFE_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert DFE_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(DFE_ACTUATION_GOAL)
...
def test_opted_in_dfe_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("dfe",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, DFE_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["dfe"]

    missing = run_dfe_workflow(with_dfeid=False)
    skip_bind = run_dfe_workflow(skip_bind=True)
    skip_temporary = run_dfe_workflow(do_temporary=False)
    skip_public = run_dfe_workflow(do_public=False)
    skip_dfedigest = run_dfe_workflow(do_dfedigest=False)
    skip_replay = run_dfe_workflow(replay=False)
    skip_dfeid = run_dfe_workflow(use_dfeid=False)
    live = run_dfe_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_dfeid"
    ...
    assert skip_dfedigest["ok"] is False
    assert skip_dfedigest["error"] == "dfedigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_dfeid["ok"] is False
    assert skip_dfeid["error"] == "dfeid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_dfedigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["dfedigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["dfeid_bound"] is True
    assert row["digest"]
    assert live["dfeid"] == DEFAULT_DFEID
    assert live["dfedigest"] == DEFAULT_DFEDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, dfeid=DEFAULT_DFEID, dfedigest=DEFAULT_DFEDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["dfeid"] == DEFAULT_DFEID
    assert queried["dfedigest"] == DEFAULT_DFEDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == DFE_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, dfeid=DEFAULT_DFEID, dfedigest=DEFAULT_DFEDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["dfeid"] == DEFAULT_DFEID
    assert answered["dfedigest"] == DEFAULT_DFEDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, dfeid=DEFAULT_DFEID, include_dfeid=False)
    )
    assert bare["has_dfeid"] is False
    assert bare["dfeid"] == EMPTY_DFEID
    publicised = serialize_dfe(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_dfe(publicised) == DEFAULT_SOURCE
    assert parse_dfe(RFC_DEST_FIELD) == DEST_HOP
    header = parse_dfe_header(encode_dfe_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_DFEID, DEFAULT_DFEDIGEST))
    ...
    assert asked["method"] == "DF"
    assert asked["disc_kind"] == "dfe"
    assert listed["disc_kind"] == "ndf"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_DFEID).startswith("DF")
    assert "dfedigest=" in canonical_public(SENTINEL, DEFAULT_DFEID, DEFAULT_DFEDIGEST)


def test_builtin_proof_seals_dfe_actuation() -> None:
    report = builtin_dfe_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "dfe_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_dfe"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_dfeid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_dfedigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_dfeid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_dfedigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_dfe"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_dfe"]
    assert report["checks"]["proved_dfe_consumes_leftover"]
    assert report["mission_goal"] == DFE_ACTUATION_GOAL
    assert report["done_when"] == DFE_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[DFE_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "dfe" in capability.tags
    assert "rfc8584" in capability.tags
    assert "http" in capability.tags
    assert "dfeid" in capability.tags
    assert "dfedigest" in capability.tags
    assert "dfe" in capability.tags
    assert "ndf" in capability.tags


def test_selection_gate_rejects_ledger_only_dfe_family(tmp_path: Path) -> None:
    ...
    assert "dfe" in family.split("/")
    assert "rfc8584" in family
    assert "dfeid" in set(semantic_tokens(DFE_ACTUATION_GOAL))
    assert "dfedigest" in family
