...
def test_goal_binds_dns64_actuation_plane() -> None:
    ...
    assert leftover_marker_ids(DNS64_LEFTOVER) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(XLAT_ACTUATION_GOAL) == (XLAT_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    assert leftover_marker_ids(RDNSS_ACTUATION_GOAL) == (RDNSS_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert DNS64_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(DNS64_ACTUATION_GOAL)
...
def test_opted_in_dns64_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("dns64",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, DNS64_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["dns64"]

    missing = run_dns64_workflow(with_dns64id=False)
    skip_bind = run_dns64_workflow(skip_bind=True)
    skip_temporary = run_dns64_workflow(do_temporary=False)
    skip_public = run_dns64_workflow(do_public=False)
    skip_dns64digest = run_dns64_workflow(do_dns64digest=False)
    skip_replay = run_dns64_workflow(replay=False)
    skip_dns64id = run_dns64_workflow(use_dns64id=False)
    live = run_dns64_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_dns64id"
    ...
    assert skip_dns64digest["ok"] is False
    assert skip_dns64digest["error"] == "dns64digest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_dns64id["ok"] is False
    assert skip_dns64id["error"] == "dns64id_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_dns64digest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["dns64digest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["dns64id_bound"] is True
    assert row["digest"]
    assert live["dns64id"] == DEFAULT_DNS64ID
    assert live["dns64digest"] == DEFAULT_DNS64DIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, dns64id=DEFAULT_DNS64ID, dns64digest=DEFAULT_DNS64DIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["dns64id"] == DEFAULT_DNS64ID
    assert queried["dns64digest"] == DEFAULT_DNS64DIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == DNS64_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, dns64id=DEFAULT_DNS64ID, dns64digest=DEFAULT_DNS64DIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["dns64id"] == DEFAULT_DNS64ID
    assert answered["dns64digest"] == DEFAULT_DNS64DIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, dns64id=DEFAULT_DNS64ID, include_dns64id=False)
    )
    assert bare["has_dns64id"] is False
    assert bare["dns64id"] == EMPTY_DNS64ID
    publicised = serialize_dns64(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_dns64(publicised) == DEFAULT_SOURCE
    assert parse_dns64(RFC_DEST_FIELD) == DEST_HOP
    header = parse_dns64_header(encode_dns64_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_DNS64ID, DEFAULT_DNS64DIGEST))
    ...
    assert asked["method"] == "DNS64"
    assert asked["dns64_kind"] == "dns64"
    assert listed["dns64_kind"] == "synth"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_DNS64ID).startswith("DNS64")
    assert "dns64digest=" in canonical_public(SENTINEL, DEFAULT_DNS64ID, DEFAULT_DNS64DIGEST)


def test_builtin_proof_seals_dns64_actuation() -> None:
    report = builtin_dns64_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "dns64_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_dns64"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_dns64id_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_dns64digest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_dns64id_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_dns64digest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_dns64"]
    ...
    assert report["checks"]["leftover_text_binds_dns64"]
    assert report["checks"]["proved_dns64_consumes_leftover"]
    assert report["mission_goal"] == DNS64_ACTUATION_GOAL
    assert report["done_when"] == DNS64_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[DNS64_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "dns64" in capability.tags
    assert "rfc6147" in capability.tags
    assert "http" in capability.tags
    assert "dns64id" in capability.tags
    assert "dns64digest" in capability.tags
    assert "dns64" in capability.tags
    assert "synth" in capability.tags


def test_selection_gate_rejects_ledger_only_dns64_family(tmp_path: Path) -> None:
    ...
    assert "dns64" in family.split("/")
    assert "rfc6147" in family
    assert "dns64id" in family
    assert "dns64digest" in family
