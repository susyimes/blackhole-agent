...
def test_goal_binds_vpls_actuation_plane() -> None:
    assert leftover_marker_ids(VPLS_ACTUATION_GOAL) == (VPLS_ACTUATION_ID,)
    ...
    assert VPLS_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(VPLS_LEFTOVER) == (VPLS_ACTUATION_ID,)
    assert leftover_marker_ids(VPLS_ACTUATION_GOAL) == (VPLS_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert VPLS_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(VPLS_ACTUATION_GOAL)
...
def test_opted_in_vpls_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("vpls",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, VPLS_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["vpls"]

    missing = run_vpls_workflow(with_vplsid=False)
    skip_bind = run_vpls_workflow(skip_bind=True)
    skip_temporary = run_vpls_workflow(do_temporary=False)
    skip_public = run_vpls_workflow(do_public=False)
    skip_vplsdigest = run_vpls_workflow(do_vplsdigest=False)
    skip_replay = run_vpls_workflow(replay=False)
    skip_vplsid = run_vpls_workflow(use_vplsid=False)
    live = run_vpls_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_vplsid"
    ...
    assert skip_vplsdigest["ok"] is False
    assert skip_vplsdigest["error"] == "vplsdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_vplsid["ok"] is False
    assert skip_vplsid["error"] == "vplsid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_vplsdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["vplsdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["vplsid_bound"] is True
    assert row["digest"]
    assert live["vplsid"] == DEFAULT_VPLSID
    assert live["vplsdigest"] == DEFAULT_VPLSDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, vplsid=DEFAULT_VPLSID, vplsdigest=DEFAULT_VPLSDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["vplsid"] == DEFAULT_VPLSID
    assert queried["vplsdigest"] == DEFAULT_VPLSDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == VPLS_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, vplsid=DEFAULT_VPLSID, vplsdigest=DEFAULT_VPLSDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["vplsid"] == DEFAULT_VPLSID
    assert answered["vplsdigest"] == DEFAULT_VPLSDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, vplsid=DEFAULT_VPLSID, include_vplsid=False)
    )
    assert bare["has_vplsid"] is False
    assert bare["vplsid"] == EMPTY_VPLSID
    publicised = serialize_vpls(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_vpls(publicised) == DEFAULT_SOURCE
    assert parse_vpls(RFC_DEST_FIELD) == DEST_HOP
    header = parse_vpls_header(encode_vpls_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_VPLSID, DEFAULT_VPLSDIGEST))
    ...
    assert asked["method"] == "VE"
    assert asked["disc_kind"] == "ve"
    ...
    assert canonical_temporary(SENTINEL, DEFAULT_VPLSID).startswith("VE")
    assert "vplsdigest=" in canonical_public(SENTINEL, DEFAULT_VPLSID, DEFAULT_VPLSDIGEST)


def test_builtin_proof_seals_vpls_actuation() -> None:
    report = builtin_vpls_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "vpls_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_vpls"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_vplsid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_vplsdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_vplsid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_vplsdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_vpls"]
    ...
    assert report["checks"]["leftover_text_binds_vpls"]
    assert report["checks"]["proved_vpls_consumes_leftover"]
    assert report["mission_goal"] == VPLS_ACTUATION_GOAL
    assert report["done_when"] == VPLS_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[VPLS_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "vpls" in capability.tags
    assert "rfc4761" in capability.tags
    assert "http" in capability.tags
    assert "vplsid" in capability.tags
    assert "vplsdigest" in capability.tags
    assert "vpls" in capability.tags
    assert "nlri" in capability.tags


def test_selection_gate_rejects_ledger_only_vpls_family(tmp_path: Path) -> None:
    ...
    assert "vpls" in family.split("/")
    assert "rfc4761" in set(semantic_tokens(VPLS_ACTUATION_GOAL))
    assert "vplsid" in set(semantic_tokens(VPLS_ACTUATION_GOAL))
    assert "vplsdigest" in family
