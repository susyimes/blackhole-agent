...
def test_goal_binds_lpd_actuation_plane() -> None:
    assert leftover_marker_ids(LPD_ACTUATION_GOAL) == (LPD_ACTUATION_ID,)
    assert leftover_marker_ids(LPD_LEFTOVER) == (LPD_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert LPD_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(LPD_ACTUATION_GOAL)
...
def test_opted_in_lpd_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("lpd",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, LPD_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["lpd"]

    missing = run_lpd_workflow(with_lpdid=False)
    skip_bind = run_lpd_workflow(skip_bind=True)
    skip_print = run_lpd_workflow(do_print=False)
    skip_queue = run_lpd_workflow(do_queue=False)
    skip_lpddigest = run_lpd_workflow(do_lpddigest=False)
    skip_replay = run_lpd_workflow(replay=False)
    skip_lpdid = run_lpd_workflow(use_lpdid=False)
    live = run_lpd_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_lpdid"
    assert skip_bind["ok"] is False
    assert skip_bind["error"] == "not_bound"
    assert skip_print["ok"] is False
    assert skip_print["error"] == "print_required"
    assert skip_queue["ok"] is False
    assert skip_queue["error"] == "queue_required"
    assert skip_lpddigest["ok"] is False
    assert skip_lpddigest["error"] == "lpddigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_lpdid["ok"] is False
    assert skip_lpdid["error"] == "lpdid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_lpddigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["print_frame"] is True
    assert row["queue_frame"] is True
    assert row["lpddigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["lpdid_bound"] is True
    assert row["digest"]
    assert live["lpdid"] == DEFAULT_LPDID
    assert live["lpddigest"] == DEFAULT_LPDDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_print(identity=SENTINEL, lpdid=DEFAULT_LPDID, lpddigest=DEFAULT_LPDDIGEST)
    )
    assert queried["is_print"] is True and queried["is_queue"] is False
    assert queried["identity"] == SENTINEL and queried["lpdid"] == DEFAULT_LPDID
    assert queried["lpddigest"] == DEFAULT_LPDDIGEST
    assert queried["type"] == FRAME_PRINT
    assert queried["first_byte"] == LPD_FIRST
    answered = parse_message(
        encode_queue(identity=SENTINEL, lpdid=DEFAULT_LPDID, lpddigest=DEFAULT_LPDDIGEST)
    )
    assert answered["is_queue"] is True and answered["is_queue"] is True
    assert answered["lpdid"] == DEFAULT_LPDID
    assert answered["lpddigest"] == DEFAULT_LPDDIGEST
    ...
    bare = parse_message(
        encode_print(identity=SENTINEL, lpdid=DEFAULT_LPDID, include_lpdid=False)
    )
    assert bare["has_lpdid"] is False
    assert bare["lpdid"] == EMPTY_LPDID
    advertised = serialize_lpd(DEFAULT_PRINT)
    assert advertised == RFC_PRINT_FIELD
    assert parse_lpd(advertised) == DEFAULT_PRINT
    assert parse_lpd(RFC_QUEUE_FIELD) == QUEUE_POLICY
    header = parse_lpd_header(encode_lpd_header(DEFAULT_PRINT))
    assert header["field_value"] == RFC_PRINT_FIELD
    assert header["header"] == PRINT_HEADER
    asked = parse_http_request(print_request(SENTINEL, DEFAULT_LPDID))
    listed = parse_http_request(queue_request(SENTINEL, DEFAULT_LPDID, DEFAULT_LPDDIGEST))
    ...
    assert asked["method"] == "PRINT"
    assert asked["lpd_kind"] == "print"
    assert listed["lpd_kind"] == "queue"
    assert got["status"] == 200
    assert preload_reply["status"] == 200
    assert got["policy"] == DEFAULT_PRINT
    assert preload_reply["policy"] == QUEUE_POLICY
    assert canonical_print(SENTINEL, DEFAULT_LPDID).startswith("PRINT")
    assert "lpddigest=" in canonical_queue(SENTINEL, DEFAULT_LPDID, DEFAULT_LPDDIGEST)


def test_builtin_proof_seals_lpd_actuation() -> None:
    report = builtin_lpd_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "lpd_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_lpd"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_lpdid_is_forbidden"]
    assert report["checks"]["skip_print_stays_empty"]
    assert report["checks"]["skip_queue_stays_empty"]
    assert report["checks"]["skip_lpddigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_lpdid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_lpddigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_lpd"]
    ...
    assert report["checks"]["leftover_text_binds_lpd"]
    assert report["checks"]["proved_lpd_consumes_leftover"]
    assert report["mission_goal"] == LPD_ACTUATION_GOAL
    assert report["done_when"] == LPD_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[LPD_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "lpd" in capability.tags
    assert "rfc1179" in capability.tags
    assert "http" in capability.tags
    assert "lpdid" in capability.tags
    assert "lpddigest" in capability.tags
    assert "print" in capability.tags


def test_selection_gate_rejects_ledger_only_lpd_family(tmp_path: Path) -> None:
    ...
    assert "lpd" in family
    assert "rfc1179" in family
    assert "lpdid" in family
    assert "lpddigest" in family
