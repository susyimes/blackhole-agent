...
from blackhole_agent.pref64_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_pref64_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_pref64_header,
    independent_pref64digest,
    parse_http_request,
    parse_http_response,
    parse_message,
    parse_pref64,
...
def test_goal_binds_pref64_actuation_plane() -> None:
    assert leftover_marker_ids(PREF64_ACTUATION_GOAL) == (PREF64_ACTUATION_ID,)
    ...
    assert leftover_marker_ids(PREF64_LEFTOVER) == (PREF64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    assert leftover_marker_ids(RDNSS_ACTUATION_GOAL) == (RDNSS_ACTUATION_ID,)
    ...
    assert PREF64_ACTUATION_ID in LOCAL_DENYLIST
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert PREF64_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(PREF64_ACTUATION_GOAL)
...
def test_opted_in_pref64_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("pref64",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, PREF64_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["pref64"]

    missing = run_pref64_workflow(with_pref64id=False)
    skip_bind = run_pref64_workflow(skip_bind=True)
    skip_temporary = run_pref64_workflow(do_temporary=False)
    skip_public = run_pref64_workflow(do_public=False)
    skip_pref64digest = run_pref64_workflow(do_pref64digest=False)
    skip_replay = run_pref64_workflow(replay=False)
    skip_pref64id = run_pref64_workflow(use_pref64id=False)
    live = run_pref64_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_pref64id"
    ...
    assert skip_pref64digest["ok"] is False
    assert skip_pref64digest["error"] == "pref64digest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_pref64id["ok"] is False
    assert skip_pref64id["error"] == "pref64id_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_pref64digest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["pref64digest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["pref64id_bound"] is True
    assert row["digest"]
    assert live["pref64id"] == DEFAULT_PREF64ID
    assert live["pref64digest"] == DEFAULT_PREF64DIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, pref64id=DEFAULT_PREF64ID, pref64digest=DEFAULT_PREF64DIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["pref64id"] == DEFAULT_PREF64ID
    assert queried["pref64digest"] == DEFAULT_PREF64DIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == PREF64_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, pref64id=DEFAULT_PREF64ID, pref64digest=DEFAULT_PREF64DIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["pref64id"] == DEFAULT_PREF64ID
    assert answered["pref64digest"] == DEFAULT_PREF64DIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, pref64id=DEFAULT_PREF64ID, include_pref64id=False)
    )
    assert bare["has_pref64id"] is False
    assert bare["pref64id"] == EMPTY_PREF64ID
    publicised = serialize_pref64(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_pref64(publicised) == DEFAULT_SOURCE
    assert parse_pref64(RFC_DEST_FIELD) == DEST_HOP
    header = parse_pref64_header(encode_pref64_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_PREF64ID, DEFAULT_PREF64DIGEST))
    ...
    assert asked["method"] == "PREF64"
    assert asked["pref64_kind"] == "pref64"
    assert listed["pref64_kind"] == "prefix"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_PREF64ID).startswith("PREF64")
    assert "pref64digest=" in canonical_public(SENTINEL, DEFAULT_PREF64ID, DEFAULT_PREF64DIGEST)


def test_builtin_proof_seals_pref64_actuation() -> None:
    report = builtin_pref64_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "pref64_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_pref64"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_pref64id_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_pref64digest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_pref64id_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_pref64digest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_pref64"]
    ...
    assert report["checks"]["leftover_text_binds_pref64"]
    assert report["checks"]["proved_pref64_consumes_leftover"]
    assert report["mission_goal"] == PREF64_ACTUATION_GOAL
    assert report["done_when"] == PREF64_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[PREF64_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "pref64" in capability.tags
    assert "rfc8781" in capability.tags
    assert "http" in capability.tags
    assert "pref64id" in capability.tags
    assert "pref64digest" in capability.tags
    assert "pref64" in capability.tags
    assert "prefix" in capability.tags


def test_selection_gate_rejects_ledger_only_pref64_family(tmp_path: Path) -> None:
    ...
    assert "pref64" in family.split("/")
    assert "rfc8781" in family
    assert "pref64id" in family
    assert "pref64digest" in family
