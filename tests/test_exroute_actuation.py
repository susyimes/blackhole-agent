...
from blackhole_agent.exroute_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_exroute_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_exroute_header,
    independent_exroutedigest,
    parse_http_comm,
    parse_http_response,
    parse_message,
    parse_exroute,
    parse_exroute_header,
    public_comm,
    public_response,
    run_exroute_workflow,
    serialize_exroute,
    temporary_comm,
    temporary_response,
)
...
def test_goal_binds_exroute_actuation_plane() -> None:
    assert leftover_marker_ids(EXROUTE_ACTUATION_GOAL) == (EXROUTE_ACTUATION_ID,)
    ...
    assert EXROUTE_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(EXROUTE_LEFTOVER) == (EXROUTE_ACTUATION_ID,)
    assert leftover_marker_ids(EXROUTE_ACTUATION_GOAL) == (EXROUTE_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert EXROUTE_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(EXROUTE_ACTUATION_GOAL)
...
def test_opted_in_exroute_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("exroute",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, EXROUTE_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["exroute"]

    missing = run_exroute_workflow(with_exrouteid=False)
    skip_bind = run_exroute_workflow(skip_bind=True)
    skip_temporary = run_exroute_workflow(do_temporary=False)
    skip_public = run_exroute_workflow(do_public=False)
    skip_exroutedigest = run_exroute_workflow(do_exroutedigest=False)
    skip_replay = run_exroute_workflow(replay=False)
    skip_exrouteid = run_exroute_workflow(use_exrouteid=False)
    live = run_exroute_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_exrouteid"
    ...
    assert skip_exroutedigest["ok"] is False
    assert skip_exroutedigest["error"] == "exroutedigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_exrouteid["ok"] is False
    assert skip_exrouteid["error"] == "exrouteid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_exroutedigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["exroutedigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["exrouteid_bound"] is True
    assert row["digest"]
    assert live["exrouteid"] == DEFAULT_EXROUTEID
    assert live["exroutedigest"] == DEFAULT_EXROUTEDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, exrouteid=DEFAULT_EXROUTEID, exroutedigest=DEFAULT_EXROUTEDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["exrouteid"] == DEFAULT_EXROUTEID
    assert queried["exroutedigest"] == DEFAULT_EXROUTEDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == EXROUTE_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, exrouteid=DEFAULT_EXROUTEID, exroutedigest=DEFAULT_EXROUTEDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["exrouteid"] == DEFAULT_EXROUTEID
    assert answered["exroutedigest"] == DEFAULT_EXROUTEDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, exrouteid=DEFAULT_EXROUTEID, include_exrouteid=False)
    )
    assert bare["has_exrouteid"] is False
    assert bare["exrouteid"] == EMPTY_EXROUTEID
    publicised = serialize_exroute(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_exroute(publicised) == DEFAULT_SOURCE
    assert parse_exroute(RFC_DEST_FIELD) == DEST_HOP
    header = parse_exroute_header(encode_exroute_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_EXROUTEID, DEFAULT_EXROUTEDIGEST))
    ...
    assert listed["disc_kind"] == "route"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_EXROUTEID).startswith("EXCLUDE")
    assert "exroutedigest=" in canonical_public(SENTINEL, DEFAULT_EXROUTEID, DEFAULT_EXROUTEDIGEST)


def test_builtin_proof_seals_exroute_actuation() -> None:
    report = builtin_exroute_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "exroute_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_exroute"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_exrouteid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_exroutedigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_exrouteid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_exroutedigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_exroute"]
    ...
    assert report["checks"]["leftover_text_binds_exroute"]
    assert report["checks"]["proved_exroute_consumes_leftover"]
    assert report["mission_goal"] == EXROUTE_ACTUATION_GOAL
    assert report["done_when"] == EXROUTE_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[EXROUTE_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "exroute" in capability.tags
    assert "rfc4874" in capability.tags
    assert "http" in capability.tags
    assert "exrouteid" in capability.tags
    assert "exroutedigest" in capability.tags
    assert "exclude" in capability.tags
    assert "route" in capability.tags


def test_selection_gate_rejects_ledger_only_exroute_family(tmp_path: Path) -> None:
    ...
    assert "exroute" in family.split("/")
    assert "rfc4874" in set(semantic_tokens(EXROUTE_ACTUATION_GOAL))
    assert "exrouteid" in set(semantic_tokens(EXROUTE_ACTUATION_GOAL))
    assert "exroutedigest" in family
