...
def test_goal_binds_asbw_actuation_plane() -> None:
    assert leftover_marker_ids(ASBW_ACTUATION_GOAL) == (ASBW_ACTUATION_ID,)
    ...
    assert ASBW_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(ASBW_LEFTOVER) == (ASBW_ACTUATION_ID,)
    assert leftover_marker_ids(ASBW_ACTUATION_GOAL) == (ASBW_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert ASBW_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(ASBW_ACTUATION_GOAL)
...
def test_opted_in_asbw_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("asbw",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, ASBW_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["asbw"]

    missing = run_asbw_workflow(with_asbwid=False)
    skip_bind = run_asbw_workflow(skip_bind=True)
    skip_temporary = run_asbw_workflow(do_temporary=False)
    skip_public = run_asbw_workflow(do_public=False)
    skip_asbwdigest = run_asbw_workflow(do_asbwdigest=False)
    skip_replay = run_asbw_workflow(replay=False)
    skip_asbwid = run_asbw_workflow(use_asbwid=False)
    live = run_asbw_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_asbwid"
    ...
    assert skip_asbwdigest["ok"] is False
    assert skip_asbwdigest["error"] == "asbwdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_asbwid["ok"] is False
    assert skip_asbwid["error"] == "asbwid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_asbwdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["asbwdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["asbwid_bound"] is True
    assert row["digest"]
    assert live["asbwid"] == DEFAULT_ASBWID
    assert live["asbwdigest"] == DEFAULT_ASBWDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, asbwid=DEFAULT_ASBWID, asbwdigest=DEFAULT_ASBWDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["asbwid"] == DEFAULT_ASBWID
    assert queried["asbwdigest"] == DEFAULT_ASBWDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == ASBW_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, asbwid=DEFAULT_ASBWID, asbwdigest=DEFAULT_ASBWDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["asbwid"] == DEFAULT_ASBWID
    assert answered["asbwdigest"] == DEFAULT_ASBWDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, asbwid=DEFAULT_ASBWID, include_asbwid=False)
    )
    assert bare["has_asbwid"] is False
    assert bare["asbwid"] == EMPTY_ASBWID
    publicised = serialize_asbw(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_asbw(publicised) == DEFAULT_SOURCE
    assert parse_asbw(RFC_DEST_FIELD) == DEST_HOP
    header = parse_asbw_header(encode_asbw_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_ASBWID, DEFAULT_ASBWDIGEST))
    ...
    assert asked["method"] == "ASYM"
    assert asked["disc_kind"] == "asym"
    assert listed["disc_kind"] == "bidir"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_ASBWID).startswith("ASYM")
    assert "asbwdigest=" in canonical_public(SENTINEL, DEFAULT_ASBWID, DEFAULT_ASBWDIGEST)


def test_builtin_proof_seals_asbw_actuation() -> None:
    report = builtin_asbw_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "asbw_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_asbw"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_asbwid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_asbwdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_asbwid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_asbwdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_binds_asbw"]
    ...
    assert report["checks"]["catalog_names_wson"]
    assert report["checks"]["leftover_text_binds_asbw"]
    assert report["checks"]["proved_asbw_consumes_leftover"]
    assert report["mission_goal"] == ASBW_ACTUATION_GOAL
    assert report["done_when"] == ASBW_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[ASBW_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "asbw" in capability.tags
    assert "rfc6387" in capability.tags
    assert "http" in capability.tags
    assert "asbwid" in capability.tags
    assert "asbwdigest" in capability.tags
    assert "asym" in capability.tags
    assert "bidir" in capability.tags


def test_selection_gate_rejects_ledger_only_asbw_family(tmp_path: Path) -> None:
    ...
    assert "asbw" in family.split("/")
    assert "rfc6387" in set(semantic_tokens(ASBW_ACTUATION_GOAL))
    assert "asbwid" in set(semantic_tokens(ASBW_ACTUATION_GOAL))
    assert "asbwdigest" in family
