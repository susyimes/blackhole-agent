...
from blackhole_agent.pathkey_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_pathkey_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_pathkey_header,
    independent_pathkeydigest,
    parse_http_comm,
    parse_http_response,
    parse_message,
    parse_pathkey,
    parse_pathkey_header,
    public_comm,
    public_response,
    run_pathkey_workflow,
    serialize_pathkey,
    temporary_comm,
    temporary_response,
)
...
def test_goal_binds_pathkey_actuation_plane() -> None:
    assert leftover_marker_ids(PATHKEY_ACTUATION_GOAL) == (PATHKEY_ACTUATION_ID,)
    ...
    assert PATHKEY_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(PATHKEY_LEFTOVER) == (PATHKEY_ACTUATION_ID,)
    assert leftover_marker_ids(PATHKEY_ACTUATION_GOAL) == (PATHKEY_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert PATHKEY_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(PATHKEY_ACTUATION_GOAL)
...
def test_opted_in_pathkey_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("pathkey",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, PATHKEY_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["pathkey"]

    missing = run_pathkey_workflow(with_pathkeyid=False)
    skip_bind = run_pathkey_workflow(skip_bind=True)
    skip_temporary = run_pathkey_workflow(do_temporary=False)
    skip_public = run_pathkey_workflow(do_public=False)
    skip_pathkeydigest = run_pathkey_workflow(do_pathkeydigest=False)
    skip_replay = run_pathkey_workflow(replay=False)
    skip_pathkeyid = run_pathkey_workflow(use_pathkeyid=False)
    live = run_pathkey_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_pathkeyid"
    ...
    assert skip_pathkeydigest["ok"] is False
    assert skip_pathkeydigest["error"] == "pathkeydigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_pathkeyid["ok"] is False
    assert skip_pathkeyid["error"] == "pathkeyid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_pathkeydigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["pathkeydigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["pathkeyid_bound"] is True
    assert row["digest"]
    assert live["pathkeyid"] == DEFAULT_PATHKEYID
    assert live["pathkeydigest"] == DEFAULT_PATHKEYDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, pathkeyid=DEFAULT_PATHKEYID, pathkeydigest=DEFAULT_PATHKEYDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["pathkeyid"] == DEFAULT_PATHKEYID
    assert queried["pathkeydigest"] == DEFAULT_PATHKEYDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == PATHKEY_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, pathkeyid=DEFAULT_PATHKEYID, pathkeydigest=DEFAULT_PATHKEYDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["pathkeyid"] == DEFAULT_PATHKEYID
    assert answered["pathkeydigest"] == DEFAULT_PATHKEYDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, pathkeyid=DEFAULT_PATHKEYID, include_pathkeyid=False)
    )
    assert bare["has_pathkeyid"] is False
    assert bare["pathkeyid"] == EMPTY_PATHKEYID
    publicised = serialize_pathkey(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_pathkey(publicised) == DEFAULT_SOURCE
    assert parse_pathkey(RFC_DEST_FIELD) == DEST_HOP
    header = parse_pathkey_header(encode_pathkey_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_PATHKEYID, DEFAULT_PATHKEYDIGEST))
    ...
    assert asked["method"] == "PATH"
    assert asked["disc_kind"] == "path"
    assert listed["disc_kind"] == "key"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_PATHKEYID).startswith("PATH")
    assert "pathkeydigest=" in canonical_public(SENTINEL, DEFAULT_PATHKEYID, DEFAULT_PATHKEYDIGEST)


def test_builtin_proof_seals_pathkey_actuation() -> None:
    report = builtin_pathkey_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "pathkey_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_pathkey"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_pathkeyid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_pathkeydigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_pathkeyid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_pathkeydigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_binds_pathkey"]
    ...
    assert report["checks"]["leftover_text_binds_pathkey"]
    assert report["checks"]["proved_pathkey_consumes_leftover"]
    assert report["mission_goal"] == PATHKEY_ACTUATION_GOAL
    assert report["done_when"] == PATHKEY_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[PATHKEY_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "pathkey" in capability.tags
    assert "rfc5520" in capability.tags
    assert "http" in capability.tags
    assert "pathkeyid" in capability.tags
    assert "pathkeydigest" in capability.tags
    assert "path" in capability.tags
    assert "key" in capability.tags


def test_selection_gate_rejects_ledger_only_pathkey_family(tmp_path: Path) -> None:
    ...
    assert "pathkey" in family.split("/")
    assert "rfc5520" in set(semantic_tokens(PATHKEY_ACTUATION_GOAL))
    assert "pathkeyid" in set(semantic_tokens(PATHKEY_ACTUATION_GOAL))
    assert "pathkeydigest" in family
