...
def test_goal_binds_xlat_actuation_plane() -> None:
    assert leftover_marker_ids(XLAT_ACTUATION_GOAL) == (XLAT_ACTUATION_ID,)
    ...
    assert leftover_marker_ids(XLAT_LEFTOVER) == (XLAT_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    assert leftover_marker_ids(RDNSS_ACTUATION_GOAL) == (RDNSS_ACTUATION_ID,)
    ...
    assert XLAT_ACTUATION_ID in LOCAL_DENYLIST
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert XLAT_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(XLAT_ACTUATION_GOAL)
...
def test_opted_in_xlat_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("xlat",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, XLAT_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["xlat"]

    missing = run_xlat_workflow(with_clatid=False)
    skip_bind = run_xlat_workflow(skip_bind=True)
    skip_temporary = run_xlat_workflow(do_temporary=False)
    skip_public = run_xlat_workflow(do_public=False)
    skip_clatdigest = run_xlat_workflow(do_clatdigest=False)
    skip_replay = run_xlat_workflow(replay=False)
    skip_clatid = run_xlat_workflow(use_clatid=False)
    live = run_xlat_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_clatid"
    ...
    assert skip_clatdigest["ok"] is False
    assert skip_clatdigest["error"] == "clatdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_clatid["ok"] is False
    assert skip_clatid["error"] == "clatid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_clatdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["clatdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["clatid_bound"] is True
    assert row["digest"]
    assert live["clatid"] == DEFAULT_CLATID
    assert live["clatdigest"] == DEFAULT_CLATDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, clatid=DEFAULT_CLATID, clatdigest=DEFAULT_CLATDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["clatid"] == DEFAULT_CLATID
    assert queried["clatdigest"] == DEFAULT_CLATDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == XLAT_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, clatid=DEFAULT_CLATID, clatdigest=DEFAULT_CLATDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["clatid"] == DEFAULT_CLATID
    assert answered["clatdigest"] == DEFAULT_CLATDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, clatid=DEFAULT_CLATID, include_clatid=False)
    )
    assert bare["has_clatid"] is False
    assert bare["clatid"] == EMPTY_CLATID
    publicised = serialize_xlat(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_xlat(publicised) == DEFAULT_SOURCE
    assert parse_xlat(RFC_DEST_FIELD) == DEST_HOP
    header = parse_xlat_header(encode_xlat_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_CLATID, DEFAULT_CLATDIGEST))
    ...
    assert asked["method"] == "CLAT"
    assert asked["xlat_kind"] == "clat"
    assert listed["xlat_kind"] == "plat"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_CLATID).startswith("CLAT")
    assert "clatdigest=" in canonical_public(SENTINEL, DEFAULT_CLATID, DEFAULT_CLATDIGEST)


def test_builtin_proof_seals_xlat_actuation() -> None:
    report = builtin_xlat_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "xlat_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_xlat"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_clatid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_clatdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_clatid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_clatdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_xlat"]
    ...
    assert report["checks"]["leftover_text_binds_xlat"]
    assert report["checks"]["proved_xlat_consumes_leftover"]
    assert report["mission_goal"] == XLAT_ACTUATION_GOAL
    assert report["done_when"] == XLAT_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[XLAT_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "xlat" in capability.tags
    assert "rfc6877" in capability.tags
    assert "http" in capability.tags
    assert "clatid" in capability.tags
    assert "clatdigest" in capability.tags
    assert "clat" in capability.tags
    assert "plat" in capability.tags


def test_selection_gate_rejects_ledger_only_xlat_family(tmp_path: Path) -> None:
    ...
    assert "xlat" in family.split("/")
    assert "rfc6877" in family
    assert "clatid" in family
    assert "clatdigest" in family
