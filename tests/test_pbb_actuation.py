...
def test_goal_binds_pbb_actuation_plane() -> None:
    assert leftover_marker_ids(PBB_ACTUATION_GOAL) == (PBB_ACTUATION_ID,)
    ...
    assert PBB_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(PBB_LEFTOVER) == (PBB_ACTUATION_ID,)
    assert leftover_marker_ids(PBB_ACTUATION_GOAL) == (PBB_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert PBB_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(PBB_ACTUATION_GOAL)
...
def test_opted_in_pbb_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("pbb",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, PBB_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["pbb"]

    missing = run_pbb_workflow(with_pbbid=False)
    skip_bind = run_pbb_workflow(skip_bind=True)
    skip_temporary = run_pbb_workflow(do_temporary=False)
    skip_public = run_pbb_workflow(do_public=False)
    skip_pbbdigest = run_pbb_workflow(do_pbbdigest=False)
    skip_replay = run_pbb_workflow(replay=False)
    skip_pbbid = run_pbb_workflow(use_pbbid=False)
    live = run_pbb_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_pbbid"
    ...
    assert skip_pbbdigest["ok"] is False
    assert skip_pbbdigest["error"] == "pbbdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_pbbid["ok"] is False
    assert skip_pbbid["error"] == "pbbid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_pbbdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["pbbdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["pbbid_bound"] is True
    assert row["digest"]
    assert live["pbbid"] == DEFAULT_PBBID
    assert live["pbbdigest"] == DEFAULT_PBBDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, pbbid=DEFAULT_PBBID, pbbdigest=DEFAULT_PBBDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["pbbid"] == DEFAULT_PBBID
    assert queried["pbbdigest"] == DEFAULT_PBBDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == PBB_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, pbbid=DEFAULT_PBBID, pbbdigest=DEFAULT_PBBDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["pbbid"] == DEFAULT_PBBID
    assert answered["pbbdigest"] == DEFAULT_PBBDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, pbbid=DEFAULT_PBBID, include_pbbid=False)
    )
    assert bare["has_pbbid"] is False
    assert bare["pbbid"] == EMPTY_PBBID
    publicised = serialize_pbb(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_pbb(publicised) == DEFAULT_SOURCE
    assert parse_pbb(RFC_DEST_FIELD) == DEST_HOP
    header = parse_pbb_header(encode_pbb_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_PBBID, DEFAULT_PBBDIGEST))
    ...
    assert asked["method"] == "PBB"
    assert asked["disc_kind"] == "pbb"
    assert listed["disc_kind"] == "bmac"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_PBBID).startswith("PBB")
    assert "pbbdigest=" in canonical_public(SENTINEL, DEFAULT_PBBID, DEFAULT_PBBDIGEST)


def test_builtin_proof_seals_pbb_actuation() -> None:
    report = builtin_pbb_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "pbb_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_pbb"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_pbbid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_pbbdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_pbbid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_pbbdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_pbb"]
    ...
    assert report["checks"]["leftover_text_binds_pbb"]
    assert report["checks"]["proved_pbb_consumes_leftover"]
    assert report["mission_goal"] == PBB_ACTUATION_GOAL
    assert report["done_when"] == PBB_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[PBB_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "pbb" in capability.tags
    assert "rfc7623" in capability.tags
    assert "http" in capability.tags
    assert "pbbid" in capability.tags
    assert "pbbdigest" in capability.tags
    assert "pbb" in capability.tags
    assert "bmac" in capability.tags


def test_selection_gate_rejects_ledger_only_pbb_family(tmp_path: Path) -> None:
    ...
    assert "pbb" in family.split("/")
    assert "rfc7623" in set(semantic_tokens(PBB_ACTUATION_GOAL))
    assert "pbbid" in set(semantic_tokens(PBB_ACTUATION_GOAL))
    assert "pbbdigest" in family
