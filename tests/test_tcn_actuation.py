...
from blackhole_agent.tcn_actuation import (
    ...
    TCN_FIRST,
    CHOICE_POLICY,
    RFC_ALTERNATES_FIELD,
    RFC_CHOICE_FIELD,
    SENTINEL,
    ALTERNATES_HEADER,
    builtin_tcn_actuation_proof,
    canonical_choice,
...
def test_goal_binds_tcn_actuation_plane() -> None:
    assert leftover_marker_ids(TCN_ACTUATION_GOAL) == (TCN_ACTUATION_ID,)
    assert leftover_marker_ids(TCN_LEFTOVER) == (TCN_ACTUATION_ID,)
    assert leftover_marker_ids(HITMETER_ACTUATION_GOAL) == (HITMETER_ACTUATION_ID,)
    assert leftover_marker_ids(HTTPAUTH_ACTUATION_GOAL) == (HTTPAUTH_ACTUATION_ID,)
    assert HITMETER_ACTUATION_ID in LOCAL_DENYLIST
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert TCN_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(TCN_ACTUATION_GOAL)
    tcn_signature = semantic_signature(TCN_ACTUATION_GOAL)
    for neighbor in NEIGHBORS:
        assert semantic_similarity(tcn_signature, semantic_signature(neighbor)) < 0.82


def test_opted_in_tcn_tool_completes_alternates_choice_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("tcn",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, TCN_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["tcn"]

    missing = run_tcn_workflow(with_variantid=False)
    skip_bind = run_tcn_workflow(skip_bind=True)
    skip_alternates = run_tcn_workflow(do_alternates=False)
    skip_choice = run_tcn_workflow(do_choice=False)
    skip_choicedigest = run_tcn_workflow(do_choicedigest=False)
    skip_replay = run_tcn_workflow(replay=False)
    skip_variantid = run_tcn_workflow(use_variantid=False)
    live = run_tcn_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_variantid"
    assert skip_bind["ok"] is False
    assert skip_bind["error"] == "not_bound"
    assert skip_alternates["ok"] is False
    assert skip_alternates["error"] == "alternates_required"
    assert skip_choice["ok"] is False
    assert skip_choice["error"] == "choice_required"
    assert skip_choicedigest["ok"] is False
    assert skip_choicedigest["error"] == "choicedigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_variantid["ok"] is False
    assert skip_variantid["error"] == "variantid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_tcn_digest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["alternates_frame"] is True
    assert row["choice_frame"] is True
    assert row["choicedigest_response"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["variantid_bound"] is True
    assert row["digest"]
    assert live["variantid"] == DEFAULT_VARIANTID
    assert live["choicedigest"] == DEFAULT_CHOICEDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_alternates(identity=SENTINEL, variantid=DEFAULT_VARIANTID, choicedigest=DEFAULT_CHOICEDIGEST)
    )
    assert queried["is_alternates"] is True and queried["is_response"] is False
    assert queried["identity"] == SENTINEL and queried["variantid"] == DEFAULT_VARIANTID
    assert queried["choicedigest"] == DEFAULT_CHOICEDIGEST
    assert queried["type"] == FRAME_ALTERNATES
    assert queried["first_byte"] == TCN_FIRST
    answered = parse_message(
        encode_choice(identity=SENTINEL, variantid=DEFAULT_VARIANTID, choicedigest=DEFAULT_CHOICEDIGEST)
    )
    assert answered["is_choice"] is True and answered["is_response"] is True
    assert answered["variantid"] == DEFAULT_VARIANTID
    assert answered["choicedigest"] == DEFAULT_CHOICEDIGEST
    ...
    bare = parse_message(
        encode_alternates(identity=SENTINEL, variantid=DEFAULT_VARIANTID, include_variantid=False)
    )
    assert bare["has_variantid"] is False
    assert bare["variantid"] == EMPTY_VARIANTID
    advertised = serialize_tcn(DEFAULT_ALTERNATES)
    assert advertised == RFC_ALTERNATES_FIELD
    assert parse_tcn(advertised) == DEFAULT_ALTERNATES
    assert parse_tcn(RFC_CHOICE_FIELD) == CHOICE_POLICY
    header = parse_tcn_header(encode_tcn_header(DEFAULT_ALTERNATES))
    assert header["field_value"] == RFC_ALTERNATES_FIELD
    assert header["header"] == ALTERNATES_HEADER
    asked = parse_http_request(alternates_request(SENTINEL, DEFAULT_VARIANTID))
    listed = parse_http_request(choice_request(SENTINEL, DEFAULT_VARIANTID, DEFAULT_CHOICEDIGEST))
    ...
    assert asked["method"] == "ALTERNATES"
    assert asked["tcn_kind"] == "alternates"
    assert listed["tcn_kind"] == "choice"
    assert got["status"] == 200
    assert preload_reply["status"] == 200
    assert got["policy"] == DEFAULT_ALTERNATES
    assert preload_reply["policy"] == CHOICE_POLICY
    assert canonical_alternates(SENTINEL, DEFAULT_VARIANTID).startswith("ALTERNATES")
    assert "choicedigest=" in canonical_choice(SENTINEL, DEFAULT_VARIANTID, DEFAULT_CHOICEDIGEST)


def test_builtin_proof_seals_tcn_actuation() -> None:
    report = builtin_tcn_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "tcn_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_tcn"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_variantid_is_forbidden"]
    assert report["checks"]["skip_alternates_stays_empty"]
    assert report["checks"]["skip_choice_stays_empty"]
    assert report["checks"]["skip_choicedigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_variantid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_choicedigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_tcn"]
    assert report["checks"]["exhausted_catalog_stays_unbound_without_gate_passing_successor"]
    assert report["checks"]["catalog_names_tcn"]
    assert report["checks"]["catalog_names_hitmeter"]
    assert report["checks"]["leftover_text_binds_tcn"]
    assert report["checks"]["proved_tcn_consumes_leftover"]
    assert report["mission_goal"] == TCN_ACTUATION_GOAL
    assert report["done_when"] == TCN_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[TCN_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "tcn" in capability.tags
    assert "rfc2295" in capability.tags
    assert "http" in capability.tags
    assert "variantid" in capability.tags
    assert "choicedigest" in capability.tags
    assert "alternates" in capability.tags


def test_selection_gate_rejects_ledger_only_tcn_family(tmp_path: Path) -> None:
    ...
    assert "tcn" in family
    assert "rfc2295" in family
    assert "variantid" in family
    assert "choicedigest" in family
    ...
    assert "hitmeter" not in family
    assert "rfc2227" not in family
    assert "meterid" not in family
    assert "usagedigest" not in family
