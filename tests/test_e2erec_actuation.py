...
from blackhole_agent.e2erec_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_e2erec_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_e2erec_header,
    independent_e2erecdigest,
    parse_http_comm,
    parse_http_response,
    parse_message,
    parse_e2erec,
...
def test_goal_binds_e2erec_actuation_plane() -> None:
    assert leftover_marker_ids(E2EREC_ACTUATION_GOAL) == (E2EREC_ACTUATION_ID,)
    ...
    assert E2EREC_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(E2EREC_LEFTOVER) == (E2EREC_ACTUATION_ID,)
    assert leftover_marker_ids(E2EREC_ACTUATION_GOAL) == (E2EREC_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert E2EREC_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(E2EREC_ACTUATION_GOAL)
...
def test_opted_in_e2erec_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("e2erec",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, E2EREC_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["e2erec"]

    missing = run_e2erec_workflow(with_e2erecid=False)
    skip_bind = run_e2erec_workflow(skip_bind=True)
    skip_temporary = run_e2erec_workflow(do_temporary=False)
    skip_public = run_e2erec_workflow(do_public=False)
    skip_e2erecdigest = run_e2erec_workflow(do_e2erecdigest=False)
    skip_replay = run_e2erec_workflow(replay=False)
    skip_e2erecid = run_e2erec_workflow(use_e2erecid=False)
    live = run_e2erec_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_e2erecid"
    ...
    assert skip_e2erecdigest["ok"] is False
    assert skip_e2erecdigest["error"] == "e2erecdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_e2erecid["ok"] is False
    assert skip_e2erecid["error"] == "e2erecid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_e2erecdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["e2erecdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["e2erecid_bound"] is True
    assert row["digest"]
    assert live["e2erecid"] == DEFAULT_E2ERECID
    assert live["e2erecdigest"] == DEFAULT_E2ERECDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, e2erecid=DEFAULT_E2ERECID, e2erecdigest=DEFAULT_E2ERECDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["e2erecid"] == DEFAULT_E2ERECID
    assert queried["e2erecdigest"] == DEFAULT_E2ERECDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == E2EREC_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, e2erecid=DEFAULT_E2ERECID, e2erecdigest=DEFAULT_E2ERECDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["e2erecid"] == DEFAULT_E2ERECID
    assert answered["e2erecdigest"] == DEFAULT_E2ERECDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, e2erecid=DEFAULT_E2ERECID, include_e2erecid=False)
    )
    assert bare["has_e2erecid"] is False
    assert bare["e2erecid"] == EMPTY_E2ERECID
    publicised = serialize_e2erec(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_e2erec(publicised) == DEFAULT_SOURCE
    assert parse_e2erec(RFC_DEST_FIELD) == DEST_HOP
    header = parse_e2erec_header(encode_e2erec_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_E2ERECID, DEFAULT_E2ERECDIGEST))
    ...
    assert asked["method"] == "PROTECT"
    assert asked["disc_kind"] == "protect"
    assert listed["disc_kind"] == "switch"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_E2ERECID).startswith("PROTECT")
    assert "e2erecdigest=" in canonical_public(SENTINEL, DEFAULT_E2ERECID, DEFAULT_E2ERECDIGEST)


def test_builtin_proof_seals_e2erec_actuation() -> None:
    report = builtin_e2erec_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "e2erec_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_e2erec"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_e2erecid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_e2erecdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_e2erecid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_e2erecdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_e2erec"]
    ...
    assert report["checks"]["leftover_text_binds_e2erec"]
    assert report["checks"]["proved_e2erec_consumes_leftover"]
    assert report["mission_goal"] == E2EREC_ACTUATION_GOAL
    assert report["done_when"] == E2EREC_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[E2EREC_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "e2erec" in capability.tags
    assert "rfc4872" in capability.tags
    assert "http" in capability.tags
    assert "e2erecid" in capability.tags
    assert "e2erecdigest" in capability.tags
    assert "protect" in capability.tags
    assert "switch" in capability.tags


def test_selection_gate_rejects_ledger_only_e2erec_family(tmp_path: Path) -> None:
    ...
    assert "e2erec" in family.split("/")
    assert "rfc4872" in set(semantic_tokens(E2EREC_ACTUATION_GOAL))
    assert "e2erecid" in set(semantic_tokens(E2EREC_ACTUATION_GOAL))
    assert "e2erecdigest" in family
    ...
    assert "grecid" not in family
    assert "grecdigest" not in family
