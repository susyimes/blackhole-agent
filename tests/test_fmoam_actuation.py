...
def test_goal_binds_fmoam_actuation_plane() -> None:
    assert leftover_marker_ids(FMOAM_ACTUATION_GOAL) == (FMOAM_ACTUATION_ID,)
    ...
    assert FMOAM_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(FMOAM_LEFTOVER) == (FMOAM_ACTUATION_ID,)
    assert leftover_marker_ids(FMOAM_ACTUATION_GOAL) == (FMOAM_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert FMOAM_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(FMOAM_ACTUATION_GOAL)
...
def test_opted_in_fmoam_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("fmoam",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, FMOAM_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["fmoam"]

    missing = run_fmoam_workflow(with_fmoamid=False)
    skip_bind = run_fmoam_workflow(skip_bind=True)
    skip_temporary = run_fmoam_workflow(do_temporary=False)
    skip_public = run_fmoam_workflow(do_public=False)
    skip_fmoamdigest = run_fmoam_workflow(do_fmoamdigest=False)
    skip_replay = run_fmoam_workflow(replay=False)
    skip_fmoamid = run_fmoam_workflow(use_fmoamid=False)
    live = run_fmoam_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_fmoamid"
    ...
    assert skip_fmoamdigest["ok"] is False
    assert skip_fmoamdigest["error"] == "fmoamdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_fmoamid["ok"] is False
    assert skip_fmoamid["error"] == "fmoamid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_fmoamdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["fmoamdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["fmoamid_bound"] is True
    assert row["digest"]
    assert live["fmoamid"] == DEFAULT_FMOAMID
    assert live["fmoamdigest"] == DEFAULT_FMOAMDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, fmoamid=DEFAULT_FMOAMID, fmoamdigest=DEFAULT_FMOAMDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["fmoamid"] == DEFAULT_FMOAMID
    assert queried["fmoamdigest"] == DEFAULT_FMOAMDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == FMOAM_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, fmoamid=DEFAULT_FMOAMID, fmoamdigest=DEFAULT_FMOAMDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["fmoamid"] == DEFAULT_FMOAMID
    assert answered["fmoamdigest"] == DEFAULT_FMOAMDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, fmoamid=DEFAULT_FMOAMID, include_fmoamid=False)
    )
    assert bare["has_fmoamid"] is False
    assert bare["fmoamid"] == EMPTY_FMOAMID
    publicised = serialize_fmoam(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_fmoam(publicised) == DEFAULT_SOURCE
    assert parse_fmoam(RFC_DEST_FIELD) == DEST_HOP
    header = parse_fmoam_header(encode_fmoam_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_FMOAMID, DEFAULT_FMOAMDIGEST))
    ...
    assert asked["method"] == "FM"
    assert asked["disc_kind"] == "fm"
    assert listed["disc_kind"] == "ais"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_FMOAMID).startswith("FM")
    assert "fmoamdigest=" in canonical_public(SENTINEL, DEFAULT_FMOAMID, DEFAULT_FMOAMDIGEST)


def test_builtin_proof_seals_fmoam_actuation() -> None:
    report = builtin_fmoam_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "fmoam_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_fmoam"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_fmoamid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_fmoamdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_fmoamid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_fmoamdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_binds_fmoam"]
    ...
    assert report["checks"]["catalog_names_wson"]
    assert report["checks"]["leftover_text_binds_fmoam"]
    assert report["checks"]["proved_fmoam_consumes_leftover"]
    assert report["mission_goal"] == FMOAM_ACTUATION_GOAL
    assert report["done_when"] == FMOAM_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[FMOAM_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "fmoam" in capability.tags
    assert "rfc6427" in capability.tags
    assert "http" in capability.tags
    assert "fmoamid" in capability.tags
    assert "fmoamdigest" in capability.tags
    assert "fm" in capability.tags
    assert "ais" in capability.tags


def test_selection_gate_rejects_ledger_only_fmoam_family(tmp_path: Path) -> None:
    ...
    assert "fmoam" in family.split("/")
    assert "rfc6427" in set(semantic_tokens(FMOAM_ACTUATION_GOAL))
    assert "fmoamid" in set(semantic_tokens(FMOAM_ACTUATION_GOAL))
    assert "fmoamdigest" in family
