...
from blackhole_agent.pcemon_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_pcemon_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_pcemon_header,
    independent_pcemondigest,
    parse_http_comm,
    parse_http_response,
    parse_message,
    parse_pcemon,
...
def test_goal_binds_pcemon_actuation_plane() -> None:
    assert leftover_marker_ids(PCEMON_ACTUATION_GOAL) == (PCEMON_ACTUATION_ID,)
    ...
    assert PCEMON_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(PCEMON_LEFTOVER) == (PCEMON_ACTUATION_ID,)
    assert leftover_marker_ids(PCEMON_ACTUATION_GOAL) == (PCEMON_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert PCEMON_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(PCEMON_ACTUATION_GOAL)
...
def test_opted_in_pcemon_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("pcemon",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, PCEMON_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["pcemon"]

    missing = run_pcemon_workflow(with_pcemonid=False)
    skip_bind = run_pcemon_workflow(skip_bind=True)
    skip_temporary = run_pcemon_workflow(do_temporary=False)
    skip_public = run_pcemon_workflow(do_public=False)
    skip_pcemondigest = run_pcemon_workflow(do_pcemondigest=False)
    skip_replay = run_pcemon_workflow(replay=False)
    skip_pcemonid = run_pcemon_workflow(use_pcemonid=False)
    live = run_pcemon_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_pcemonid"
    ...
    assert skip_pcemondigest["ok"] is False
    assert skip_pcemondigest["error"] == "pcemondigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_pcemonid["ok"] is False
    assert skip_pcemonid["error"] == "pcemonid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_pcemondigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["pcemondigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["pcemonid_bound"] is True
    assert row["digest"]
    assert live["pcemonid"] == DEFAULT_PCEMONID
    assert live["pcemondigest"] == DEFAULT_PCEMONDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, pcemonid=DEFAULT_PCEMONID, pcemondigest=DEFAULT_PCEMONDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["pcemonid"] == DEFAULT_PCEMONID
    assert queried["pcemondigest"] == DEFAULT_PCEMONDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == PCEMON_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, pcemonid=DEFAULT_PCEMONID, pcemondigest=DEFAULT_PCEMONDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["pcemonid"] == DEFAULT_PCEMONID
    assert answered["pcemondigest"] == DEFAULT_PCEMONDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, pcemonid=DEFAULT_PCEMONID, include_pcemonid=False)
    )
    assert bare["has_pcemonid"] is False
    assert bare["pcemonid"] == EMPTY_PCEMONID
    publicised = serialize_pcemon(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_pcemon(publicised) == DEFAULT_SOURCE
    assert parse_pcemon(RFC_DEST_FIELD) == DEST_HOP
    header = parse_pcemon_header(encode_pcemon_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_PCEMONID, DEFAULT_PCEMONDIGEST))
    ...
    assert asked["method"] == "MON"
    assert asked["disc_kind"] == "mon"
    assert listed["disc_kind"] == "pceid"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_PCEMONID).startswith("MON")
    assert "pcemondigest=" in canonical_public(SENTINEL, DEFAULT_PCEMONID, DEFAULT_PCEMONDIGEST)


def test_builtin_proof_seals_pcemon_actuation() -> None:
    report = builtin_pcemon_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "pcemon_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_pcemon"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_pcemonid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_pcemondigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_pcemonid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_pcemondigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_binds_pcemon"]
    ...
    assert report["checks"]["leftover_text_binds_pcemon"]
    assert report["checks"]["proved_pcemon_consumes_leftover"]
    assert report["mission_goal"] == PCEMON_ACTUATION_GOAL
    assert report["done_when"] == PCEMON_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[PCEMON_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "pcemon" in capability.tags
    assert "rfc5886" in capability.tags
    assert "http" in capability.tags
    assert "pcemonid" in capability.tags
    assert "pcemondigest" in capability.tags
    assert "mon" in capability.tags
    assert "pceid" in capability.tags


def test_selection_gate_rejects_ledger_only_pcemon_family(tmp_path: Path) -> None:
    ...
    assert "pcemon" in family.split("/")
    assert "rfc5886" in set(semantic_tokens(PCEMON_ACTUATION_GOAL))
    assert "pcemonid" in set(semantic_tokens(PCEMON_ACTUATION_GOAL))
    assert "pcemondigest" in family
