...
def test_goal_binds_pwe3_actuation_plane() -> None:
    assert leftover_marker_ids(PWE3_ACTUATION_GOAL) == (PWE3_ACTUATION_ID,)
    ...
    assert PWE3_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(PWE3_LEFTOVER) == (PWE3_ACTUATION_ID,)
    assert leftover_marker_ids(PWE3_ACTUATION_GOAL) == (PWE3_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert PWE3_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(PWE3_ACTUATION_GOAL)
...
def test_opted_in_pwe3_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("pwe3",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, PWE3_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["pwe3"]

    missing = run_pwe3_workflow(with_pwe3id=False)
    skip_bind = run_pwe3_workflow(skip_bind=True)
    skip_temporary = run_pwe3_workflow(do_temporary=False)
    skip_public = run_pwe3_workflow(do_public=False)
    skip_pwe3digest = run_pwe3_workflow(do_pwe3digest=False)
    skip_replay = run_pwe3_workflow(replay=False)
    skip_pwe3id = run_pwe3_workflow(use_pwe3id=False)
    live = run_pwe3_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_pwe3id"
    ...
    assert skip_pwe3digest["ok"] is False
    assert skip_pwe3digest["error"] == "pwe3digest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_pwe3id["ok"] is False
    assert skip_pwe3id["error"] == "pwe3id_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_pwe3digest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["pwe3digest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["pwe3id_bound"] is True
    assert row["digest"]
    assert live["pwe3id"] == DEFAULT_PWE3ID
    assert live["pwe3digest"] == DEFAULT_PWE3DIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, pwe3id=DEFAULT_PWE3ID, pwe3digest=DEFAULT_PWE3DIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["pwe3id"] == DEFAULT_PWE3ID
    assert queried["pwe3digest"] == DEFAULT_PWE3DIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == PWE3_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, pwe3id=DEFAULT_PWE3ID, pwe3digest=DEFAULT_PWE3DIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["pwe3id"] == DEFAULT_PWE3ID
    assert answered["pwe3digest"] == DEFAULT_PWE3DIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, pwe3id=DEFAULT_PWE3ID, include_pwe3id=False)
    )
    assert bare["has_pwe3id"] is False
    assert bare["pwe3id"] == EMPTY_PWE3ID
    publicised = serialize_pwe3(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_pwe3(publicised) == DEFAULT_SOURCE
    assert parse_pwe3(RFC_DEST_FIELD) == DEST_HOP
    header = parse_pwe3_header(encode_pwe3_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_PWE3ID, DEFAULT_PWE3DIGEST))
    ...
    assert asked["method"] == "PSN"
    assert asked["disc_kind"] == "psn"
    assert listed["disc_kind"] == "nsp"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_PWE3ID).startswith("PSN")
    assert "pwe3digest=" in canonical_public(SENTINEL, DEFAULT_PWE3ID, DEFAULT_PWE3DIGEST)


def test_builtin_proof_seals_pwe3_actuation() -> None:
    report = builtin_pwe3_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "pwe3_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_pwe3"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_pwe3id_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_pwe3digest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_pwe3id_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_pwe3digest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_pwe3"]
    ...
    assert report["checks"]["leftover_text_binds_pwe3"]
    assert report["checks"]["proved_pwe3_consumes_leftover"]
    assert report["mission_goal"] == PWE3_ACTUATION_GOAL
    assert report["done_when"] == PWE3_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[PWE3_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "pwe3" in capability.tags
    assert "rfc3985" in capability.tags
    assert "http" in capability.tags
    assert "pwe3id" in capability.tags
    assert "pwe3digest" in capability.tags
    assert "psn" in capability.tags
    assert "nsp" in capability.tags


def test_selection_gate_rejects_ledger_only_pwe3_family(tmp_path: Path) -> None:
    ...
    assert "pwe3" in family.split("/")
    assert "rfc3985" in set(semantic_tokens(PWE3_ACTUATION_GOAL))
    assert "pwe3id" in set(semantic_tokens(PWE3_ACTUATION_GOAL))
    assert "pwe3digest" in family
