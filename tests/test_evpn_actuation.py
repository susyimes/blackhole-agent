...
def test_goal_binds_evpn_actuation_plane() -> None:
    assert leftover_marker_ids(EVPN_ACTUATION_GOAL) == (EVPN_ACTUATION_ID,)
    ...
    assert EVPN_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(EVPN_LEFTOVER) == (EVPN_ACTUATION_ID,)
    assert leftover_marker_ids(EVPN_ACTUATION_GOAL) == (EVPN_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert EVPN_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(EVPN_ACTUATION_GOAL)
...
def test_opted_in_evpn_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("evpn",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, EVPN_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["evpn"]

    missing = run_evpn_workflow(with_evpnid=False)
    skip_bind = run_evpn_workflow(skip_bind=True)
    skip_temporary = run_evpn_workflow(do_temporary=False)
    skip_public = run_evpn_workflow(do_public=False)
    skip_evpndigest = run_evpn_workflow(do_evpndigest=False)
    skip_replay = run_evpn_workflow(replay=False)
    skip_evpnid = run_evpn_workflow(use_evpnid=False)
    live = run_evpn_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_evpnid"
    ...
    assert skip_evpndigest["ok"] is False
    assert skip_evpndigest["error"] == "evpndigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_evpnid["ok"] is False
    assert skip_evpnid["error"] == "evpnid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_evpndigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["evpndigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["evpnid_bound"] is True
    assert row["digest"]
    assert live["evpnid"] == DEFAULT_EVPNID
    assert live["evpndigest"] == DEFAULT_EVPNDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, evpnid=DEFAULT_EVPNID, evpndigest=DEFAULT_EVPNDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["evpnid"] == DEFAULT_EVPNID
    assert queried["evpndigest"] == DEFAULT_EVPNDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == EVPN_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, evpnid=DEFAULT_EVPNID, evpndigest=DEFAULT_EVPNDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["evpnid"] == DEFAULT_EVPNID
    assert answered["evpndigest"] == DEFAULT_EVPNDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, evpnid=DEFAULT_EVPNID, include_evpnid=False)
    )
    assert bare["has_evpnid"] is False
    assert bare["evpnid"] == EMPTY_EVPNID
    publicised = serialize_evpn(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_evpn(publicised) == DEFAULT_SOURCE
    assert parse_evpn(RFC_DEST_FIELD) == DEST_HOP
    header = parse_evpn_header(encode_evpn_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_EVPNID, DEFAULT_EVPNDIGEST))
    ...
    assert asked["method"] == "AD"
    assert asked["disc_kind"] == "evpn"
    assert listed["disc_kind"] == "vpws"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_EVPNID).startswith("AD")
    assert "evpndigest=" in canonical_public(SENTINEL, DEFAULT_EVPNID, DEFAULT_EVPNDIGEST)


def test_builtin_proof_seals_evpn_actuation() -> None:
    report = builtin_evpn_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "evpn_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_evpn"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_evpnid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_evpndigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_evpnid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_evpndigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_evpn"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_evpn"]
    assert report["checks"]["proved_evpn_consumes_leftover"]
    assert report["mission_goal"] == EVPN_ACTUATION_GOAL
    assert report["done_when"] == EVPN_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[EVPN_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "evpn" in capability.tags
    assert "rfc8214" in capability.tags
    assert "http" in capability.tags
    assert "evpnid" in capability.tags
    assert "evpndigest" in capability.tags
    assert "evpn" in capability.tags
    assert "vpws" in capability.tags


def test_selection_gate_rejects_ledger_only_evpn_family(tmp_path: Path) -> None:
    ...
    assert "evpn" in family.split("/")
    assert "rfc8214" in family
    assert "evpnid" in set(semantic_tokens(EVPN_ACTUATION_GOAL))
    assert "evpndigest" in family
