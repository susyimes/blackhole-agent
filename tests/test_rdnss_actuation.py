...
def test_goal_binds_rdnss_actuation_plane() -> None:
    ...
    assert leftover_marker_ids(RDNSS_LEFTOVER) == (RDNSS_ACTUATION_ID,)
    assert leftover_marker_ids(PREF64_ACTUATION_GOAL) == (PREF64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert RDNSS_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(RDNSS_ACTUATION_GOAL)
...
def test_opted_in_rdnss_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("rdnss",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, RDNSS_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["rdnss"]

    missing = run_rdnss_workflow(with_rdnssid=False)
    skip_bind = run_rdnss_workflow(skip_bind=True)
    skip_temporary = run_rdnss_workflow(do_temporary=False)
    skip_public = run_rdnss_workflow(do_public=False)
    skip_rdnssdigest = run_rdnss_workflow(do_rdnssdigest=False)
    skip_replay = run_rdnss_workflow(replay=False)
    skip_rdnssid = run_rdnss_workflow(use_rdnssid=False)
    live = run_rdnss_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_rdnssid"
    ...
    assert skip_rdnssdigest["ok"] is False
    assert skip_rdnssdigest["error"] == "rdnssdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_rdnssid["ok"] is False
    assert skip_rdnssid["error"] == "rdnssid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_rdnssdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["rdnssdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["rdnssid_bound"] is True
    assert row["digest"]
    assert live["rdnssid"] == DEFAULT_RDNSSID
    assert live["rdnssdigest"] == DEFAULT_RDNSSDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, rdnssid=DEFAULT_RDNSSID, rdnssdigest=DEFAULT_RDNSSDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["rdnssid"] == DEFAULT_RDNSSID
    assert queried["rdnssdigest"] == DEFAULT_RDNSSDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == RDNSS_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, rdnssid=DEFAULT_RDNSSID, rdnssdigest=DEFAULT_RDNSSDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["rdnssid"] == DEFAULT_RDNSSID
    assert answered["rdnssdigest"] == DEFAULT_RDNSSDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, rdnssid=DEFAULT_RDNSSID, include_rdnssid=False)
    )
    assert bare["has_rdnssid"] is False
    assert bare["rdnssid"] == EMPTY_RDNSSID
    publicised = serialize_rdnss(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_rdnss(publicised) == DEFAULT_SOURCE
    assert parse_rdnss(RFC_DEST_FIELD) == DEST_HOP
    header = parse_rdnss_header(encode_rdnss_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_RDNSSID, DEFAULT_RDNSSDIGEST))
    ...
    assert asked["method"] == "RDNSS"
    assert asked["rdnss_kind"] == "rdnss"
    assert listed["rdnss_kind"] == "dnssl"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_RDNSSID).startswith("RDNSS")
    assert "rdnssdigest=" in canonical_public(SENTINEL, DEFAULT_RDNSSID, DEFAULT_RDNSSDIGEST)


def test_builtin_proof_seals_rdnss_actuation() -> None:
    report = builtin_rdnss_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "rdnss_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_rdnss"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_rdnssid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_rdnssdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_rdnssid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_rdnssdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_rdnss"]
    ...
    assert report["checks"]["leftover_text_binds_rdnss"]
    assert report["checks"]["proved_rdnss_consumes_leftover"]
    assert report["mission_goal"] == RDNSS_ACTUATION_GOAL
    assert report["done_when"] == RDNSS_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[RDNSS_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "rdnss" in capability.tags
    assert "rfc8106" in capability.tags
    assert "http" in capability.tags
    assert "rdnssid" in capability.tags
    assert "rdnssdigest" in capability.tags
    assert "rdnss" in capability.tags
    assert "dnssl" in capability.tags


def test_selection_gate_rejects_ledger_only_rdnss_family(tmp_path: Path) -> None:
    ...
    assert "rdnss" in family.split("/")
    assert "rdnssid" in family
    assert "rdnssdigest" in family
