...
def test_goal_binds_ndp_actuation_plane() -> None:
    assert leftover_marker_ids(NDP_ACTUATION_GOAL) == (NDP_ACTUATION_ID,)
    assert leftover_marker_ids(NDP_LEFTOVER) == (NDP_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert NDP_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(NDP_ACTUATION_GOAL)
...
def test_opted_in_ndp_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("ndp",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, NDP_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["ndp"]

    missing = run_ndp_workflow(with_ndpid=False)
    skip_bind = run_ndp_workflow(skip_bind=True)
    skip_solicit = run_ndp_workflow(do_solicit=False)
    skip_advert = run_ndp_workflow(do_advert=False)
    skip_ndpdigest = run_ndp_workflow(do_ndpdigest=False)
    skip_replay = run_ndp_workflow(replay=False)
    skip_ndpid = run_ndp_workflow(use_ndpid=False)
    live = run_ndp_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_ndpid"
    assert skip_bind["ok"] is False
    assert skip_bind["error"] == "not_bound"
    assert skip_solicit["ok"] is False
    assert skip_solicit["error"] == "solicit_required"
    assert skip_advert["ok"] is False
    assert skip_advert["error"] == "advert_required"
    assert skip_ndpdigest["ok"] is False
    assert skip_ndpdigest["error"] == "ndpdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_ndpid["ok"] is False
    assert skip_ndpid["error"] == "ndpid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_ndpdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["solicit_frame"] is True
    assert row["advert_frame"] is True
    assert row["ndpdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["ndpid_bound"] is True
    assert row["digest"]
    assert live["ndpid"] == DEFAULT_NDPID
    assert live["ndpdigest"] == DEFAULT_NDPDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_solicit(identity=SENTINEL, ndpid=DEFAULT_NDPID, ndpdigest=DEFAULT_NDPDIGEST)
    )
    assert queried["is_solicit"] is True and queried["is_advert"] is False
    assert queried["identity"] == SENTINEL and queried["ndpid"] == DEFAULT_NDPID
    assert queried["ndpdigest"] == DEFAULT_NDPDIGEST
    assert queried["type"] == FRAME_SOLICIT
    assert queried["first_byte"] == NDP_FIRST
    answered = parse_message(
        encode_advert(identity=SENTINEL, ndpid=DEFAULT_NDPID, ndpdigest=DEFAULT_NDPDIGEST)
    )
    assert answered["is_advert"] is True and answered["is_advert"] is True
    assert answered["ndpid"] == DEFAULT_NDPID
    assert answered["ndpdigest"] == DEFAULT_NDPDIGEST
    ...
    bare = parse_message(
        encode_solicit(identity=SENTINEL, ndpid=DEFAULT_NDPID, include_ndpid=False)
    )
    assert bare["has_ndpid"] is False
    assert bare["ndpid"] == EMPTY_NDPID
    advertised = serialize_ndp(DEFAULT_SOLICIT)
    assert advertised == RFC_SOLICIT_FIELD
    assert parse_ndp(advertised) == DEFAULT_SOLICIT
    assert parse_ndp(RFC_ADVERT_FIELD) == ADVERT_POLICY
    header = parse_ndp_header(encode_ndp_header(DEFAULT_SOLICIT))
    assert header["field_value"] == RFC_SOLICIT_FIELD
    assert header["header"] == SOLICIT_HEADER
    asked = parse_http_request(solicit_request(SENTINEL, DEFAULT_NDPID))
    listed = parse_http_request(advert_request(SENTINEL, DEFAULT_NDPID, DEFAULT_NDPDIGEST))
    ...
    assert asked["method"] == "SOLICIT"
    assert asked["ndp_kind"] == "solicit"
    assert listed["ndp_kind"] == "advert"
    assert got["status"] == 200
    assert preload_advert["status"] == 200
    assert got["policy"] == DEFAULT_SOLICIT
    assert preload_advert["policy"] == ADVERT_POLICY
    assert canonical_solicit(SENTINEL, DEFAULT_NDPID).startswith("SOLICIT")
    assert "ndpdigest=" in canonical_advert(SENTINEL, DEFAULT_NDPID, DEFAULT_NDPDIGEST)


def test_builtin_proof_seals_ndp_actuation() -> None:
    report = builtin_ndp_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "ndp_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_ndp"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_ndpid_is_forbidden"]
    assert report["checks"]["skip_solicit_stays_empty"]
    assert report["checks"]["skip_advert_stays_empty"]
    assert report["checks"]["skip_ndpdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_ndpid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_ndpdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_ndp"]
    assert report["checks"]["exhausted_catalog_stays_unbound_without_gate_passing_successor"]
    assert report["checks"]["catalog_names_ndp"]
    assert report["checks"]["catalog_names_slaac"]
    assert report["checks"]["leftover_text_binds_ndp"]
    assert report["checks"]["proved_ndp_consumes_leftover"]
    assert report["mission_goal"] == NDP_ACTUATION_GOAL
    assert report["done_when"] == NDP_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[NDP_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "ndp" in capability.tags
    assert "rfc4861" in capability.tags
    assert "http" in capability.tags
    assert "ndpid" in capability.tags
    assert "ndpdigest" in capability.tags
    assert "solicit" in capability.tags


def test_selection_gate_rejects_ledger_only_ndp_family(tmp_path: Path) -> None:
    ...
    assert "ndp" in family.split("/")
    assert "rfc4861" in family
    assert "ndpid" in family
    assert "ndpdigest" in family
    ...
    assert "slaacdigest" not in family
