...
def test_goal_binds_macip_actuation_plane() -> None:
    assert leftover_marker_ids(MACIP_ACTUATION_GOAL) == (MACIP_ACTUATION_ID,)
    ...
    assert MACIP_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(MACIP_LEFTOVER) == (MACIP_ACTUATION_ID,)
    assert leftover_marker_ids(MACIP_ACTUATION_GOAL) == (MACIP_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert MACIP_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(MACIP_ACTUATION_GOAL)
...
def test_opted_in_macip_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("macip",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, MACIP_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["macip"]

    missing = run_macip_workflow(with_macipid=False)
    skip_bind = run_macip_workflow(skip_bind=True)
    skip_temporary = run_macip_workflow(do_temporary=False)
    skip_public = run_macip_workflow(do_public=False)
    skip_macipdigest = run_macip_workflow(do_macipdigest=False)
    skip_replay = run_macip_workflow(replay=False)
    skip_macipid = run_macip_workflow(use_macipid=False)
    live = run_macip_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_macipid"
    ...
    assert skip_macipdigest["ok"] is False
    assert skip_macipdigest["error"] == "macipdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_macipid["ok"] is False
    assert skip_macipid["error"] == "macipid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_macipdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["macipdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["macipid_bound"] is True
    assert row["digest"]
    assert live["macipid"] == DEFAULT_MACIPID
    assert live["macipdigest"] == DEFAULT_MACIPDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, macipid=DEFAULT_MACIPID, macipdigest=DEFAULT_MACIPDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["macipid"] == DEFAULT_MACIPID
    assert queried["macipdigest"] == DEFAULT_MACIPDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == MACIP_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, macipid=DEFAULT_MACIPID, macipdigest=DEFAULT_MACIPDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["macipid"] == DEFAULT_MACIPID
    assert answered["macipdigest"] == DEFAULT_MACIPDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, macipid=DEFAULT_MACIPID, include_macipid=False)
    )
    assert bare["has_macipid"] is False
    assert bare["macipid"] == EMPTY_MACIPID
    publicised = serialize_macip(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_macip(publicised) == DEFAULT_SOURCE
    assert parse_macip(RFC_DEST_FIELD) == DEST_HOP
    header = parse_macip_header(encode_macip_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_MACIPID, DEFAULT_MACIPDIGEST))
    ...
    assert asked["method"] == "MAC"
    assert asked["disc_kind"] == "mac"
    assert listed["disc_kind"] == "ip"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_MACIPID).startswith("MAC")
    assert "macipdigest=" in canonical_public(SENTINEL, DEFAULT_MACIPID, DEFAULT_MACIPDIGEST)


def test_builtin_proof_seals_macip_actuation() -> None:
    report = builtin_macip_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "macip_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_macip"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_macipid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_macipdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_macipid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_macipdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_macip"]
    ...
    assert report["checks"]["leftover_text_binds_macip"]
    assert report["checks"]["proved_macip_consumes_leftover"]
    assert report["mission_goal"] == MACIP_ACTUATION_GOAL
    assert report["done_when"] == MACIP_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[MACIP_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "macip" in capability.tags
    assert "rfc7432" in capability.tags
    assert "http" in capability.tags
    assert "macipid" in capability.tags
    assert "macipdigest" in capability.tags
    assert "macip" in capability.tags
    assert "ip" in capability.tags


def test_selection_gate_rejects_ledger_only_macip_family(tmp_path: Path) -> None:
    ...
    assert "macip" in family.split("/")
    assert "rfc7432" in set(semantic_tokens(MACIP_ACTUATION_GOAL))
    assert "macipid" in set(semantic_tokens(MACIP_ACTUATION_GOAL))
    assert "macipdigest" in family
