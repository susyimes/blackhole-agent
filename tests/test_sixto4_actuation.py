...
from blackhole_agent.sixto4_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_sixto4_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_sixto4_header,
    independent_sixto4digest,
    parse_http_request,
    parse_http_response,
    parse_message,
    parse_sixto4,
...
def test_goal_binds_sixto4_actuation_plane() -> None:
    assert leftover_marker_ids(SIXTO4_ACTUATION_GOAL) == (SIXTO4_ACTUATION_ID,)
    ...
    assert SIXTO4_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(SIXTO4_LEFTOVER) == (SIXTO4_ACTUATION_ID,)
    assert leftover_marker_ids(SIXTO4_ACTUATION_GOAL) == (SIXTO4_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert SIXTO4_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(SIXTO4_ACTUATION_GOAL)
...
def test_opted_in_sixto4_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("sixto4",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, SIXTO4_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["sixto4"]

    missing = run_sixto4_workflow(with_sixto4id=False)
    skip_bind = run_sixto4_workflow(skip_bind=True)
    skip_temporary = run_sixto4_workflow(do_temporary=False)
    skip_public = run_sixto4_workflow(do_public=False)
    skip_sixto4digest = run_sixto4_workflow(do_sixto4digest=False)
    skip_replay = run_sixto4_workflow(replay=False)
    skip_sixto4id = run_sixto4_workflow(use_sixto4id=False)
    live = run_sixto4_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_sixto4id"
    ...
    assert skip_sixto4digest["ok"] is False
    assert skip_sixto4digest["error"] == "sixto4digest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_sixto4id["ok"] is False
    assert skip_sixto4id["error"] == "sixto4id_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_sixto4digest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["sixto4digest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["sixto4id_bound"] is True
    assert row["digest"]
    assert live["sixto4id"] == DEFAULT_SIXTO4ID
    assert live["sixto4digest"] == DEFAULT_SIXTO4DIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, sixto4id=DEFAULT_SIXTO4ID, sixto4digest=DEFAULT_SIXTO4DIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["sixto4id"] == DEFAULT_SIXTO4ID
    assert queried["sixto4digest"] == DEFAULT_SIXTO4DIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == SIXTO4_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, sixto4id=DEFAULT_SIXTO4ID, sixto4digest=DEFAULT_SIXTO4DIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["sixto4id"] == DEFAULT_SIXTO4ID
    assert answered["sixto4digest"] == DEFAULT_SIXTO4DIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, sixto4id=DEFAULT_SIXTO4ID, include_sixto4id=False)
    )
    assert bare["has_sixto4id"] is False
    assert bare["sixto4id"] == EMPTY_SIXTO4ID
    publicised = serialize_sixto4(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_sixto4(publicised) == DEFAULT_SOURCE
    assert parse_sixto4(RFC_DEST_FIELD) == DEST_HOP
    header = parse_sixto4_header(encode_sixto4_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_SIXTO4ID, DEFAULT_SIXTO4DIGEST))
    ...
    assert asked["method"] == "6TO4"
    assert asked["disc_kind"] == "6to4"
    assert listed["disc_kind"] == "border"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_SIXTO4ID).startswith("6TO4")
    assert "sixto4digest=" in canonical_public(SENTINEL, DEFAULT_SIXTO4ID, DEFAULT_SIXTO4DIGEST)


def test_builtin_proof_seals_sixto4_actuation() -> None:
    report = builtin_sixto4_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "sixto4_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_sixto4"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_sixto4id_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_sixto4digest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_sixto4id_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_sixto4digest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_sixto4"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_sixto4"]
    assert report["checks"]["proved_sixto4_consumes_leftover"]
    assert report["mission_goal"] == SIXTO4_ACTUATION_GOAL
    assert report["done_when"] == SIXTO4_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[SIXTO4_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "sixto4" in capability.tags
    assert "rfc3056" in capability.tags
    assert "http" in capability.tags
    assert "sixto4id" in capability.tags
    assert "sixto4digest" in capability.tags
    assert "6to4" in capability.tags
    assert "border" in capability.tags


def test_selection_gate_rejects_ledger_only_sixto4_family(tmp_path: Path) -> None:
    ...
    assert "sixto4" in family.split("/")
    assert "rfc3056" in family
    assert "sixto4id" in set(semantic_tokens(SIXTO4_ACTUATION_GOAL))
    assert "sixto4digest" in family
