...
def test_goal_binds_ippfx_actuation_plane() -> None:
    assert leftover_marker_ids(IPPFX_ACTUATION_GOAL) == (IPPFX_ACTUATION_ID,)
    ...
    assert IPPFX_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(IPPFX_LEFTOVER) == (IPPFX_ACTUATION_ID,)
    assert leftover_marker_ids(IPPFX_ACTUATION_GOAL) == (IPPFX_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert IPPFX_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(IPPFX_ACTUATION_GOAL)
...
def test_opted_in_ippfx_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("ippfx",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, IPPFX_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["ippfx"]

    missing = run_ippfx_workflow(with_ippfxid=False)
    skip_bind = run_ippfx_workflow(skip_bind=True)
    skip_temporary = run_ippfx_workflow(do_temporary=False)
    skip_public = run_ippfx_workflow(do_public=False)
    skip_ippfxdigest = run_ippfx_workflow(do_ippfxdigest=False)
    skip_replay = run_ippfx_workflow(replay=False)
    skip_ippfxid = run_ippfx_workflow(use_ippfxid=False)
    live = run_ippfx_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_ippfxid"
    ...
    assert skip_ippfxdigest["ok"] is False
    assert skip_ippfxdigest["error"] == "ippfxdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_ippfxid["ok"] is False
    assert skip_ippfxid["error"] == "ippfxid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_ippfxdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["ippfxdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["ippfxid_bound"] is True
    assert row["digest"]
    assert live["ippfxid"] == DEFAULT_IPPFXID
    assert live["ippfxdigest"] == DEFAULT_IPPFXDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, ippfxid=DEFAULT_IPPFXID, ippfxdigest=DEFAULT_IPPFXDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["ippfxid"] == DEFAULT_IPPFXID
    assert queried["ippfxdigest"] == DEFAULT_IPPFXDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == IPPFX_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, ippfxid=DEFAULT_IPPFXID, ippfxdigest=DEFAULT_IPPFXDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["ippfxid"] == DEFAULT_IPPFXID
    assert answered["ippfxdigest"] == DEFAULT_IPPFXDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, ippfxid=DEFAULT_IPPFXID, include_ippfxid=False)
    )
    assert bare["has_ippfxid"] is False
    assert bare["ippfxid"] == EMPTY_IPPFXID
    publicised = serialize_ippfx(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_ippfx(publicised) == DEFAULT_SOURCE
    assert parse_ippfx(RFC_DEST_FIELD) == DEST_HOP
    header = parse_ippfx_header(encode_ippfx_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_IPPFXID, DEFAULT_IPPFXDIGEST))
    ...
    assert asked["method"] == "PREFIX"
    assert asked["disc_kind"] == "ippfx"
    ...
    assert canonical_temporary(SENTINEL, DEFAULT_IPPFXID).startswith("PREFIX")
    assert "ippfxdigest=" in canonical_public(SENTINEL, DEFAULT_IPPFXID, DEFAULT_IPPFXDIGEST)


def test_builtin_proof_seals_ippfx_actuation() -> None:
    report = builtin_ippfx_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "ippfx_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_ippfx"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_ippfxid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_ippfxdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_ippfxid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_ippfxdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_ippfx"]
    ...
    assert report["checks"]["catalog_names_proxynd"]
    assert report["checks"]["leftover_text_binds_ippfx"]
    assert report["checks"]["proved_ippfx_consumes_leftover"]
    assert report["mission_goal"] == IPPFX_ACTUATION_GOAL
    assert report["done_when"] == IPPFX_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[IPPFX_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "ippfx" in capability.tags
    assert "rfc9136" in capability.tags
    assert "http" in capability.tags
    assert "ippfxid" in capability.tags
    assert "ippfxdigest" in capability.tags
    assert "ippfx" in capability.tags
    assert "ip" in capability.tags


def test_selection_gate_rejects_ledger_only_ippfx_family(tmp_path: Path) -> None:
    ...
    assert "ippfx" in family.split("/")
    assert "ippfxid" in set(semantic_tokens(IPPFX_ACTUATION_GOAL))
    assert "ippfxdigest" in family
