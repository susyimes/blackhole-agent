...
def test_goal_binds_mldp_actuation_plane() -> None:
    assert leftover_marker_ids(MLDP_ACTUATION_GOAL) == (MLDP_ACTUATION_ID,)
    ...
    assert MLDP_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(MLDP_LEFTOVER) == (MLDP_ACTUATION_ID,)
    assert leftover_marker_ids(MLDP_ACTUATION_GOAL) == (MLDP_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert MLDP_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(MLDP_ACTUATION_GOAL)
...
def test_opted_in_mldp_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("mldp",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, MLDP_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["mldp"]

    missing = run_mldp_workflow(with_mldpid=False)
    skip_bind = run_mldp_workflow(skip_bind=True)
    skip_temporary = run_mldp_workflow(do_temporary=False)
    skip_public = run_mldp_workflow(do_public=False)
    skip_mldpdigest = run_mldp_workflow(do_mldpdigest=False)
    skip_replay = run_mldp_workflow(replay=False)
    skip_mldpid = run_mldp_workflow(use_mldpid=False)
    live = run_mldp_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_mldpid"
    ...
    assert skip_mldpdigest["ok"] is False
    assert skip_mldpdigest["error"] == "mldpdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_mldpid["ok"] is False
    assert skip_mldpid["error"] == "mldpid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_mldpdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["mldpdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["mldpid_bound"] is True
    assert row["digest"]
    assert live["mldpid"] == DEFAULT_MLDPID
    assert live["mldpdigest"] == DEFAULT_MLDPDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, mldpid=DEFAULT_MLDPID, mldpdigest=DEFAULT_MLDPDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["mldpid"] == DEFAULT_MLDPID
    assert queried["mldpdigest"] == DEFAULT_MLDPDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == MLDP_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, mldpid=DEFAULT_MLDPID, mldpdigest=DEFAULT_MLDPDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["mldpid"] == DEFAULT_MLDPID
    assert answered["mldpdigest"] == DEFAULT_MLDPDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, mldpid=DEFAULT_MLDPID, include_mldpid=False)
    )
    assert bare["has_mldpid"] is False
    assert bare["mldpid"] == EMPTY_MLDPID
    publicised = serialize_mldp(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_mldp(publicised) == DEFAULT_SOURCE
    assert parse_mldp(RFC_DEST_FIELD) == DEST_HOP
    header = parse_mldp_header(encode_mldp_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_MLDPID, DEFAULT_MLDPDIGEST))
    ...
    assert asked["method"] == "MLDP"
    assert asked["disc_kind"] == "mldp"
    assert listed["disc_kind"] == "root"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_MLDPID).startswith("MLDP")
    assert "mldpdigest=" in canonical_public(SENTINEL, DEFAULT_MLDPID, DEFAULT_MLDPDIGEST)


def test_builtin_proof_seals_mldp_actuation() -> None:
    report = builtin_mldp_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "mldp_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_mldp"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_mldpid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_mldpdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_mldpid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_mldpdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_binds_mldp"]
    ...
    assert report["checks"]["catalog_names_wson"]
    assert report["checks"]["leftover_text_binds_mldp"]
    assert report["checks"]["proved_mldp_consumes_leftover"]
    assert report["mission_goal"] == MLDP_ACTUATION_GOAL
    assert report["done_when"] == MLDP_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[MLDP_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "mldp" in capability.tags
    assert "rfc6512" in capability.tags
    assert "http" in capability.tags
    assert "mldpid" in capability.tags
    assert "mldpdigest" in capability.tags
    assert "root" in capability.tags


def test_selection_gate_rejects_ledger_only_mldp_family(tmp_path: Path) -> None:
    ...
    assert "mldp" in family.split("/")
    assert "rfc6512" in set(semantic_tokens(MLDP_ACTUATION_GOAL))
    assert "mldpid" in set(semantic_tokens(MLDP_ACTUATION_GOAL))
    assert "mldpdigest" in family
