...
def test_goal_binds_guni_actuation_plane() -> None:
    assert leftover_marker_ids(GUNI_ACTUATION_GOAL) == (GUNI_ACTUATION_ID,)
    ...
    assert GUNI_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(GUNI_LEFTOVER) == (GUNI_ACTUATION_ID,)
    assert leftover_marker_ids(GUNI_ACTUATION_GOAL) == (GUNI_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert GUNI_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(GUNI_ACTUATION_GOAL)
...
def test_opted_in_guni_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("guni",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, GUNI_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["guni"]

    missing = run_guni_workflow(with_guniid=False)
    skip_bind = run_guni_workflow(skip_bind=True)
    skip_temporary = run_guni_workflow(do_temporary=False)
    skip_public = run_guni_workflow(do_public=False)
    skip_gunidigest = run_guni_workflow(do_gunidigest=False)
    skip_replay = run_guni_workflow(replay=False)
    skip_guniid = run_guni_workflow(use_guniid=False)
    live = run_guni_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_guniid"
    ...
    assert skip_gunidigest["ok"] is False
    assert skip_gunidigest["error"] == "gunidigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_guniid["ok"] is False
    assert skip_guniid["error"] == "guniid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_gunidigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["gunidigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["guniid_bound"] is True
    assert row["digest"]
    assert live["guniid"] == DEFAULT_GUNIID
    assert live["gunidigest"] == DEFAULT_GUNIDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, guniid=DEFAULT_GUNIID, gunidigest=DEFAULT_GUNIDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["guniid"] == DEFAULT_GUNIID
    assert queried["gunidigest"] == DEFAULT_GUNIDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == GUNI_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, guniid=DEFAULT_GUNIID, gunidigest=DEFAULT_GUNIDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["guniid"] == DEFAULT_GUNIID
    assert answered["gunidigest"] == DEFAULT_GUNIDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, guniid=DEFAULT_GUNIID, include_guniid=False)
    )
    assert bare["has_guniid"] is False
    assert bare["guniid"] == EMPTY_GUNIID
    publicised = serialize_guni(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_guni(publicised) == DEFAULT_SOURCE
    assert parse_guni(RFC_DEST_FIELD) == DEST_HOP
    header = parse_guni_header(encode_guni_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_GUNIID, DEFAULT_GUNIDIGEST))
    ...
    assert asked["method"] == "UNIC"
    assert asked["disc_kind"] == "unic"
    assert listed["disc_kind"] == "unin"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_GUNIID).startswith("UNIC")
    assert "gunidigest=" in canonical_public(SENTINEL, DEFAULT_GUNIID, DEFAULT_GUNIDIGEST)


def test_builtin_proof_seals_guni_actuation() -> None:
    report = builtin_guni_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "guni_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_guni"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_guniid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_gunidigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_guniid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_gunidigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_guni"]
    ...
    assert report["checks"]["leftover_text_binds_guni"]
    assert report["checks"]["proved_guni_consumes_leftover"]
    assert report["mission_goal"] == GUNI_ACTUATION_GOAL
    assert report["done_when"] == GUNI_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[GUNI_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "guni" in capability.tags
    assert "rfc4208" in capability.tags
    assert "http" in capability.tags
    assert "guniid" in capability.tags
    assert "gunidigest" in capability.tags
    assert "unic" in capability.tags
    assert "unin" in capability.tags


def test_selection_gate_rejects_ledger_only_guni_family(tmp_path: Path) -> None:
    ...
    assert "guni" in family.split("/")
    assert "rfc4208" in set(semantic_tokens(GUNI_ACTUATION_GOAL))
    assert "guniid" in set(semantic_tokens(GUNI_ACTUATION_GOAL))
    assert "gunidigest" in family
