"""Tests for growing unplannable goals from five-level nested-namespace Python class-instance sdists."""
...
from blackhole_agent.capability_application_growth import (
    ...
    load_python_quintuple_nested_namespace_class_instance_apply_catalog,
    run_application_python_quintuple_nested_namespace_class_instance_growth_plane,
    verify_application_python_quintuple_nested_namespace_class_instance_growth_plane,
)
...
def test_python_quintuple_nested_namespace_class_instance_catalog_is_live_fetched_sdist() -> None:
    catalog = load_python_quintuple_nested_namespace_class_instance_apply_catalog()
    assert catalog["query"] == query_from_goal(PYTHON_QUINTUPLE_NESTED_NAMESPACE_CLASS_INSTANCE_GROW_TASK.goal)
    ...
    winner = next(
        ...
        if item["slug"] == PYTHON_QUINTUPLE_NESTED_NAMESPACE_CLASS_INSTANCE_WINNER_SLUG
    )
...
def test_python_quintuple_nested_namespace_class_instance_match_selects_oauthlib() -> None:
    catalog = load_python_quintuple_nested_namespace_class_instance_apply_catalog()
    absorbed = sorted(APPLY_ABSORBED_SLUGS)
    trend = rank_catalog(strip_declared_provides(catalog["items"]), absorbed=absorbed)
    assert trend["winner"]["slug"] == PYTHON_QUINTUPLE_NESTED_NAMESPACE_CLASS_INSTANCE_NPM_DECOY_SLUG
    ...
    assert matched["winner"]["slug"] == PYTHON_QUINTUPLE_NESTED_NAMESPACE_CLASS_INSTANCE_WINNER_SLUG
    covering = matched["covering"] or {}
    assert PYTHON_QUINTUPLE_NESTED_NAMESPACE_CLASS_INSTANCE_GOAL_KEY in covering["inferred_provides"]
    assert covering.get("python_quintuple_nested_namespace_class_instance") is True
...
def test_grow_from_python_quintuple_nested_namespace_class_instance_catalog_forages_oauthlib() -> None:
    catalog = load_python_quintuple_nested_namespace_class_instance_apply_catalog()
    result = grow_application_task(
        PYTHON_QUINTUPLE_NESTED_NAMESPACE_CLASS_INSTANCE_GROW_TASK,
        catalog=catalog,
        forage=True,
        hide_before=[PYTHON_QUINTUPLE_NESTED_NAMESPACE_CLASS_INSTANCE_WINNER_CAPABILITY_ID],
        live_fetch=True,
    )
    assert result["ok"], result
    assert result["grew"] is True
    assert result["winner_slug"] == PYTHON_QUINTUPLE_NESTED_NAMESPACE_CLASS_INSTANCE_WINNER_SLUG
    assert (result.get("forage") or {}).get("origin", {}).get("kind") == "pypi-live"


def test_python_quintuple_nested_namespace_class_instance_plane_grows_and_rejects_tamper(
    tmp_path: Path,
) -> None:
    report_dir = tmp_path / "report"
    plane = run_application_python_quintuple_nested_namespace_class_instance_growth_plane(report_dir)
    assert plane["ok"], plane
    assert plane["winner"] == PYTHON_QUINTUPLE_NESTED_NAMESPACE_CLASS_INSTANCE_WINNER_SLUG
    assert plane["grade"]["python_quintuple_nested_namespace_class_instance_selected"]
    assert plane["grade"]["winner_is_python_quintuple_nested_namespace_class_instance"]
    ...
    hidden = build_application_registry(
        ledger,
        hide=[PYTHON_QUINTUPLE_NESTED_NAMESPACE_CLASS_INSTANCE_WINNER_CAPABILITY_ID],
        include_synthesized=True,
        include_absorbed=True,
    )
    assert plan_application_task(PYTHON_QUINTUPLE_NESTED_NAMESPACE_CLASS_INSTANCE_GROW_TASK, hidden) is None
    grown = build_application_registry(ledger, include_synthesized=True, include_absorbed=True)
    solved = run_application_task(PYTHON_QUINTUPLE_NESTED_NAMESPACE_CLASS_INSTANCE_GROW_TASK, grown)
    assert solved["ok"], solved
    verification = verify_application_python_quintuple_nested_namespace_class_instance_growth_plane(report_dir)
    assert verification["ok"], verification
    report_path = report_dir / "plane-report.json"
    report = json.loads(report_path.read_text(encoding="utf-8"))
    report["grade"]["grow_winner_is_oauthlib"] = False
    report_path.write_text(json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
    assert not verify_application_python_quintuple_nested_namespace_class_instance_growth_plane(report_dir)["ok"]


def test_builtin_application_python_quintuple_nested_namespace_class_instance_growth_plane_proof() -> None:
    result = builtin_application_python_quintuple_nested_namespace_class_instance_growth_plane_proof()
    assert result["ok"], result
    assert result["action"] == "application_python_quintuple_nested_namespace_class_instance_growth_plane"
    assert result["grow_winner_is_oauthlib"]
    assert result["python_quintuple_nested_namespace_class_instance_selected"]
    assert result["winner_is_python_quintuple_nested_namespace_class_instance"]
    leftover = (
