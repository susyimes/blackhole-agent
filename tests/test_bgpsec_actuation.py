...
from blackhole_agent.bgpsec_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_bgpsec_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_bgpsec_header,
    independent_bgpsecdigest,
    parse_http_comm,
    parse_http_response,
    parse_message,
    parse_bgpsec,
...
def test_goal_binds_bgpsec_actuation_plane() -> None:
    assert leftover_marker_ids(BGPSEC_ACTUATION_GOAL) == (BGPSEC_ACTUATION_ID,)
    ...
    assert BGPSEC_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(BGPSEC_LEFTOVER) == (BGPSEC_ACTUATION_ID,)
    assert leftover_marker_ids(BGPSEC_ACTUATION_GOAL) == (BGPSEC_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert BGPSEC_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(BGPSEC_ACTUATION_GOAL)
...
def test_opted_in_bgpsec_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("bgpsec",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, BGPSEC_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["bgpsec"]

    missing = run_bgpsec_workflow(with_bgpsecid=False)
    skip_bind = run_bgpsec_workflow(skip_bind=True)
    skip_temporary = run_bgpsec_workflow(do_temporary=False)
    skip_public = run_bgpsec_workflow(do_public=False)
    skip_bgpsecdigest = run_bgpsec_workflow(do_bgpsecdigest=False)
    skip_replay = run_bgpsec_workflow(replay=False)
    skip_bgpsecid = run_bgpsec_workflow(use_bgpsecid=False)
    live = run_bgpsec_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_bgpsecid"
    ...
    assert skip_bgpsecdigest["ok"] is False
    assert skip_bgpsecdigest["error"] == "bgpsecdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_bgpsecid["ok"] is False
    assert skip_bgpsecid["error"] == "bgpsecid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_bgpsecdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["bgpsecdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["bgpsecid_bound"] is True
    assert row["digest"]
    assert live["bgpsecid"] == DEFAULT_BGPSECID
    assert live["bgpsecdigest"] == DEFAULT_BGPSECDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, bgpsecid=DEFAULT_BGPSECID, bgpsecdigest=DEFAULT_BGPSECDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["bgpsecid"] == DEFAULT_BGPSECID
    assert queried["bgpsecdigest"] == DEFAULT_BGPSECDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == BGPSEC_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, bgpsecid=DEFAULT_BGPSECID, bgpsecdigest=DEFAULT_BGPSECDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["bgpsecid"] == DEFAULT_BGPSECID
    assert answered["bgpsecdigest"] == DEFAULT_BGPSECDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, bgpsecid=DEFAULT_BGPSECID, include_bgpsecid=False)
    )
    assert bare["has_bgpsecid"] is False
    assert bare["bgpsecid"] == EMPTY_BGPSECID
    publicised = serialize_bgpsec(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_bgpsec(publicised) == DEFAULT_SOURCE
    assert parse_bgpsec(RFC_DEST_FIELD) == DEST_HOP
    header = parse_bgpsec_header(encode_bgpsec_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_BGPSECID, DEFAULT_BGPSECDIGEST))
    ...
    assert asked["method"] == "SIGN"
    assert asked["disc_kind"] == "bgpsec"
    assert listed["disc_kind"] == "path"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_BGPSECID).startswith("SIGN")
    assert "bgpsecdigest=" in canonical_public(SENTINEL, DEFAULT_BGPSECID, DEFAULT_BGPSECDIGEST)


def test_builtin_proof_seals_bgpsec_actuation() -> None:
    report = builtin_bgpsec_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "bgpsec_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_bgpsec"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_bgpsecid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_bgpsecdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_bgpsecid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_bgpsecdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_bgpsec"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_bgpsec"]
    assert report["checks"]["proved_bgpsec_consumes_leftover"]
    assert report["mission_goal"] == BGPSEC_ACTUATION_GOAL
    assert report["done_when"] == BGPSEC_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[BGPSEC_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "bgpsec" in capability.tags
    assert "rfc8205" in capability.tags
    assert "http" in capability.tags
    assert "bgpsecid" in capability.tags
    assert "bgpsecdigest" in capability.tags
    assert "bgpsec" in capability.tags
    assert "path" in capability.tags


def test_selection_gate_rejects_ledger_only_bgpsec_family(tmp_path: Path) -> None:
    ...
    assert "bgpsec" in family.split("/")
    assert "rfc8205" in family
    assert "bgpsecid" in set(semantic_tokens(BGPSEC_ACTUATION_GOAL))
    assert "bgpsecdigest" in family
