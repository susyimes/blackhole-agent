...
from blackhole_agent.httptls_actuation import (
    ...
    SENTINEL,
    UPGRADE_HEADER,
    builtin_httptls_actuation_proof,
    canonical_tls,
    canonical_upgrade,
    crc32c,
    encode_tls,
    encode_upgrade,
    encode_httptls_header,
    independent_httptls_digest,
    parse_http_request,
    parse_http_response,
    parse_message,
    parse_httptls,
    parse_httptls_header,
    tls_request,
    tls_response,
    run_httptls_workflow,
    serialize_httptls,
    upgrade_request,
    upgrade_response,
)
...
def test_goal_binds_httptls_actuation_plane() -> None:
    assert leftover_marker_ids(HTTPTLS_ACTUATION_GOAL) == (HTTPTLS_ACTUATION_ID,)
    assert leftover_marker_ids(HTTPTLS_LEFTOVER) == (HTTPTLS_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert HTTPTLS_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(HTTPTLS_ACTUATION_GOAL)
    httptls_signature = semantic_signature(HTTPTLS_ACTUATION_GOAL)
    for neighbor in NEIGHBORS:
        assert semantic_similarity(httptls_signature, semantic_signature(neighbor)) < 0.82


def test_opted_in_httptls_tool_completes_upgrade_lock_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("httptls",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, HTTPTLS_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["httptls"]

    missing = run_httptls_workflow(with_upgradeid=False)
    skip_bind = run_httptls_workflow(skip_bind=True)
    skip_upgrade = run_httptls_workflow(do_upgrade=False)
    skip_tls = run_httptls_workflow(do_tls=False)
    skip_upgradetlsdigest = run_httptls_workflow(do_upgradetlsdigest=False)
    skip_replay = run_httptls_workflow(replay=False)
    skip_upgradeid = run_httptls_workflow(use_upgradeid=False)
    live = run_httptls_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_upgradeid"
    assert skip_bind["ok"] is False
    assert skip_bind["error"] == "not_bound"
    assert skip_upgrade["ok"] is False
    assert skip_upgrade["error"] == "upgrade_required"
    assert skip_tls["ok"] is False
    assert skip_tls["error"] == "tls_required"
    assert skip_upgradetlsdigest["ok"] is False
    assert skip_upgradetlsdigest["error"] == "upgradetlsdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_upgradeid["ok"] is False
    assert skip_upgradeid["error"] == "upgradeid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_httptls_digest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["upgrade_frame"] is True
    assert row["tls_frame"] is True
    assert row["upgradetlsdigest_response"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["upgradeid_bound"] is True
    assert row["digest"]
    assert live["upgradeid"] == DEFAULT_UPGRADEID
    assert live["upgradetlsdigest"] == DEFAULT_UPGRADETLSDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_upgrade(identity=SENTINEL, upgradeid=DEFAULT_UPGRADEID, upgradetlsdigest=DEFAULT_UPGRADETLSDIGEST)
    )
    assert queried["is_upgrade"] is True and queried["is_response"] is False
    assert queried["identity"] == SENTINEL and queried["upgradeid"] == DEFAULT_UPGRADEID
    assert queried["upgradetlsdigest"] == DEFAULT_UPGRADETLSDIGEST
    assert queried["type"] == FRAME_UPGRADE
    assert queried["first_byte"] == UPGRADE_FIRST
    answered = parse_message(
        encode_tls(identity=SENTINEL, upgradeid=DEFAULT_UPGRADEID, upgradetlsdigest=DEFAULT_UPGRADETLSDIGEST)
    )
    assert answered["is_tls"] is True and answered["is_response"] is True
    assert answered["upgradeid"] == DEFAULT_UPGRADEID
    assert answered["upgradetlsdigest"] == DEFAULT_UPGRADETLSDIGEST
    ...
    bare = parse_message(
        encode_upgrade(identity=SENTINEL, upgradeid=DEFAULT_UPGRADEID, include_upgradeid=False)
    )
    assert bare["has_upgradeid"] is False
    assert bare["upgradeid"] == EMPTY_UPGRADEID
    advertised = serialize_httptls(DEFAULT_UPGRADE)
    assert advertised == RFC_UPGRADE_FIELD
    assert parse_httptls(advertised) == DEFAULT_UPGRADE
    assert parse_httptls(RFC_TLS_FIELD) == TLS_POLICY
    header = parse_httptls_header(encode_httptls_header(DEFAULT_UPGRADE))
    assert header["field_value"] == RFC_UPGRADE_FIELD
    assert header["header"] == UPGRADE_HEADER
    asked = parse_http_request(upgrade_request(SENTINEL, DEFAULT_UPGRADEID))
    listed = parse_http_request(tls_request(SENTINEL, DEFAULT_UPGRADEID, DEFAULT_UPGRADETLSDIGEST))
    ...
    assert asked["method"] == "UPGRADE"
    assert asked["httptls_kind"] == "upgrade"
    assert listed["httptls_kind"] == "tls"
    assert got["status"] == 200
    assert preload_reply["status"] == 200
    assert got["policy"] == DEFAULT_UPGRADE
    assert preload_reply["policy"] == TLS_POLICY
    assert canonical_upgrade(SENTINEL, DEFAULT_UPGRADEID).startswith("UPGRADE")
    assert "upgradetlsdigest=" in canonical_tls(SENTINEL, DEFAULT_UPGRADEID, DEFAULT_UPGRADETLSDIGEST)


def test_builtin_proof_seals_httptls_actuation() -> None:
    report = builtin_httptls_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "httptls_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_httptls"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_upgradeid_is_forbidden"]
    assert report["checks"]["skip_upgrade_stays_empty"]
    assert report["checks"]["skip_tls_stays_empty"]
    assert report["checks"]["skip_upgradetlsdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_upgradeid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_upgradetlsdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_httptls"]
    ...
    assert report["checks"]["catalog_names_httpauth"]
    assert report["checks"]["leftover_text_binds_httptls"]
    assert report["checks"]["proved_httptls_consumes_leftover"]
    assert report["mission_goal"] == HTTPTLS_ACTUATION_GOAL
    assert report["done_when"] == HTTPTLS_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[HTTPTLS_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "httptls" in capability.tags
    assert "rfc2817" in capability.tags
    assert "http" in capability.tags
    assert "upgradeid" in capability.tags
    assert "upgradetlsdigest" in capability.tags
    assert "upgrade" in capability.tags


def test_selection_gate_rejects_ledger_only_httptls_family(tmp_path: Path) -> None:
    ...
    assert "httptl" in family
    assert "rfc2817" in family
    assert "upgradeid" in family
    assert "upgradetlsdigest" in family
