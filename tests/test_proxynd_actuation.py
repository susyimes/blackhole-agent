...
from blackhole_agent.proxynd_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_proxynd_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_proxynd_header,
    independent_proxynddigest,
    parse_http_comm,
    parse_http_response,
    parse_message,
    parse_proxynd,
    parse_proxynd_header,
    public_comm,
    public_response,
    run_proxynd_workflow,
    serialize_proxynd,
    temporary_comm,
    temporary_response,
)
...
def test_goal_binds_proxynd_actuation_plane() -> None:
    assert leftover_marker_ids(PROXYND_ACTUATION_GOAL) == (PROXYND_ACTUATION_ID,)
    ...
    assert PROXYND_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(PROXYND_LEFTOVER) == (PROXYND_ACTUATION_ID,)
    assert leftover_marker_ids(PROXYND_ACTUATION_GOAL) == (PROXYND_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert PROXYND_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(PROXYND_ACTUATION_GOAL)
...
def test_opted_in_proxynd_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("proxynd",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, PROXYND_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["proxynd"]

    missing = run_proxynd_workflow(with_proxyndid=False)
    skip_bind = run_proxynd_workflow(skip_bind=True)
    skip_temporary = run_proxynd_workflow(do_temporary=False)
    skip_public = run_proxynd_workflow(do_public=False)
    skip_proxynddigest = run_proxynd_workflow(do_proxynddigest=False)
    skip_replay = run_proxynd_workflow(replay=False)
    skip_proxyndid = run_proxynd_workflow(use_proxyndid=False)
    live = run_proxynd_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_proxyndid"
    ...
    assert skip_proxynddigest["ok"] is False
    assert skip_proxynddigest["error"] == "proxynddigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_proxyndid["ok"] is False
    assert skip_proxyndid["error"] == "proxyndid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_proxynddigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["proxynddigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["proxyndid_bound"] is True
    assert row["digest"]
    assert live["proxyndid"] == DEFAULT_PROXYNDID
    assert live["proxynddigest"] == DEFAULT_PROXYNDDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, proxyndid=DEFAULT_PROXYNDID, proxynddigest=DEFAULT_PROXYNDDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["proxyndid"] == DEFAULT_PROXYNDID
    assert queried["proxynddigest"] == DEFAULT_PROXYNDDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == PROXYND_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, proxyndid=DEFAULT_PROXYNDID, proxynddigest=DEFAULT_PROXYNDDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["proxyndid"] == DEFAULT_PROXYNDID
    assert answered["proxynddigest"] == DEFAULT_PROXYNDDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, proxyndid=DEFAULT_PROXYNDID, include_proxyndid=False)
    )
    assert bare["has_proxyndid"] is False
    assert bare["proxyndid"] == EMPTY_PROXYNDID
    publicised = serialize_proxynd(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_proxynd(publicised) == DEFAULT_SOURCE
    assert parse_proxynd(RFC_DEST_FIELD) == DEST_HOP
    header = parse_proxynd_header(encode_proxynd_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_PROXYNDID, DEFAULT_PROXYNDDIGEST))
    ...
    assert asked["method"] == "PROXY"
    assert asked["disc_kind"] == "proxynd"
    assert listed["disc_kind"] == "nd"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_PROXYNDID).startswith("PROXY")
    assert "proxynddigest=" in canonical_public(SENTINEL, DEFAULT_PROXYNDID, DEFAULT_PROXYNDDIGEST)


def test_builtin_proof_seals_proxynd_actuation() -> None:
    report = builtin_proxynd_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "proxynd_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_proxynd"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_proxyndid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_proxynddigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_proxyndid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_proxynddigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_proxynd"]
    ...
    assert report["checks"]["leftover_text_binds_proxynd"]
    assert report["checks"]["proved_proxynd_consumes_leftover"]
    assert report["mission_goal"] == PROXYND_ACTUATION_GOAL
    assert report["done_when"] == PROXYND_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[PROXYND_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "proxynd" in capability.tags
    assert "rfc9161" in capability.tags
    assert "http" in capability.tags
    assert "proxyndid" in capability.tags
    assert "proxynddigest" in capability.tags
    assert "proxynd" in capability.tags
    assert "nd" in capability.tags


def test_selection_gate_rejects_ledger_only_proxynd_family(tmp_path: Path) -> None:
    ...
    assert "proxynd" in family.split("/")
    assert "proxyndid" in set(semantic_tokens(PROXYND_ACTUATION_GOAL))
    assert "proxynddigest" in family
