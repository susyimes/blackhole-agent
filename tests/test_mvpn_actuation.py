...
def test_goal_binds_mvpn_actuation_plane() -> None:
    assert leftover_marker_ids(MVPN_ACTUATION_GOAL) == (MVPN_ACTUATION_ID,)
    ...
    assert MVPN_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(MVPN_LEFTOVER) == (MVPN_ACTUATION_ID,)
    assert leftover_marker_ids(MVPN_ACTUATION_GOAL) == (MVPN_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert MVPN_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(MVPN_ACTUATION_GOAL)
...
def test_opted_in_mvpn_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("mvpn",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, MVPN_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["mvpn"]

    missing = run_mvpn_workflow(with_mvpnid=False)
    skip_bind = run_mvpn_workflow(skip_bind=True)
    skip_temporary = run_mvpn_workflow(do_temporary=False)
    skip_public = run_mvpn_workflow(do_public=False)
    skip_mvpndigest = run_mvpn_workflow(do_mvpndigest=False)
    skip_replay = run_mvpn_workflow(replay=False)
    skip_mvpnid = run_mvpn_workflow(use_mvpnid=False)
    live = run_mvpn_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_mvpnid"
    ...
    assert skip_mvpndigest["ok"] is False
    assert skip_mvpndigest["error"] == "mvpndigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_mvpnid["ok"] is False
    assert skip_mvpnid["error"] == "mvpnid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_mvpndigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["mvpndigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["mvpnid_bound"] is True
    assert row["digest"]
    assert live["mvpnid"] == DEFAULT_MVPNID
    assert live["mvpndigest"] == DEFAULT_MVPNDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, mvpnid=DEFAULT_MVPNID, mvpndigest=DEFAULT_MVPNDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["mvpnid"] == DEFAULT_MVPNID
    assert queried["mvpndigest"] == DEFAULT_MVPNDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == MVPN_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, mvpnid=DEFAULT_MVPNID, mvpndigest=DEFAULT_MVPNDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["mvpnid"] == DEFAULT_MVPNID
    assert answered["mvpndigest"] == DEFAULT_MVPNDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, mvpnid=DEFAULT_MVPNID, include_mvpnid=False)
    )
    assert bare["has_mvpnid"] is False
    assert bare["mvpnid"] == EMPTY_MVPNID
    publicised = serialize_mvpn(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_mvpn(publicised) == DEFAULT_SOURCE
    assert parse_mvpn(RFC_DEST_FIELD) == DEST_HOP
    header = parse_mvpn_header(encode_mvpn_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_MVPNID, DEFAULT_MVPNDIGEST))
    ...
    assert asked["method"] == "MVPN"
    assert asked["disc_kind"] == "mvpn"
    assert listed["disc_kind"] == "cmcast"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_MVPNID).startswith("MVPN")
    assert "mvpndigest=" in canonical_public(SENTINEL, DEFAULT_MVPNID, DEFAULT_MVPNDIGEST)


def test_builtin_proof_seals_mvpn_actuation() -> None:
    report = builtin_mvpn_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "mvpn_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_mvpn"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_mvpnid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_mvpndigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_mvpnid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_mvpndigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_binds_mvpn"]
    ...
    assert report["checks"]["catalog_names_wson"]
    assert report["checks"]["leftover_text_binds_mvpn"]
    assert report["checks"]["proved_mvpn_consumes_leftover"]
    assert report["mission_goal"] == MVPN_ACTUATION_GOAL
    assert report["done_when"] == MVPN_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[MVPN_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "mvpn" in capability.tags
    assert "rfc6513" in capability.tags
    assert "http" in capability.tags
    assert "mvpnid" in capability.tags
    assert "mvpndigest" in capability.tags
    assert "cmcast" in capability.tags


def test_selection_gate_rejects_ledger_only_mvpn_family(tmp_path: Path) -> None:
    ...
    assert "mvpn" in family.split("/")
    assert "rfc6513" in set(semantic_tokens(MVPN_ACTUATION_GOAL))
    assert "mvpnid" in set(semantic_tokens(MVPN_ACTUATION_GOAL))
    assert "mvpndigest" in family
