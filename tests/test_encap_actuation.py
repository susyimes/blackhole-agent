...
def test_goal_binds_encap_actuation_plane() -> None:
    assert leftover_marker_ids(ENCAP_ACTUATION_GOAL) == (ENCAP_ACTUATION_ID,)
    ...
    assert ENCAP_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(ENCAP_LEFTOVER) == (ENCAP_ACTUATION_ID,)
    assert leftover_marker_ids(ENCAP_ACTUATION_GOAL) == (ENCAP_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert ENCAP_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(ENCAP_ACTUATION_GOAL)
...
def test_opted_in_encap_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("encap",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, ENCAP_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["encap"]

    missing = run_encap_workflow(with_encapid=False)
    skip_bind = run_encap_workflow(skip_bind=True)
    skip_temporary = run_encap_workflow(do_temporary=False)
    skip_public = run_encap_workflow(do_public=False)
    skip_encapdigest = run_encap_workflow(do_encapdigest=False)
    skip_replay = run_encap_workflow(replay=False)
    skip_encapid = run_encap_workflow(use_encapid=False)
    live = run_encap_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_encapid"
    ...
    assert skip_encapdigest["ok"] is False
    assert skip_encapdigest["error"] == "encapdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_encapid["ok"] is False
    assert skip_encapid["error"] == "encapid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_encapdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["encapdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["encapid_bound"] is True
    assert row["digest"]
    assert live["encapid"] == DEFAULT_ENCAPID
    assert live["encapdigest"] == DEFAULT_ENCAPDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, encapid=DEFAULT_ENCAPID, encapdigest=DEFAULT_ENCAPDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["encapid"] == DEFAULT_ENCAPID
    assert queried["encapdigest"] == DEFAULT_ENCAPDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == ENCAP_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, encapid=DEFAULT_ENCAPID, encapdigest=DEFAULT_ENCAPDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["encapid"] == DEFAULT_ENCAPID
    assert answered["encapdigest"] == DEFAULT_ENCAPDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, encapid=DEFAULT_ENCAPID, include_encapid=False)
    )
    assert bare["has_encapid"] is False
    assert bare["encapid"] == EMPTY_ENCAPID
    publicised = serialize_encap(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_encap(publicised) == DEFAULT_SOURCE
    assert parse_encap(RFC_DEST_FIELD) == DEST_HOP
    header = parse_encap_header(encode_encap_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_ENCAPID, DEFAULT_ENCAPDIGEST))
    ...
    assert asked["method"] == "ENCAP"
    assert asked["disc_kind"] == "encap"
    assert listed["disc_kind"] == "safi"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_ENCAPID).startswith("ENCAP")
    assert "encapdigest=" in canonical_public(SENTINEL, DEFAULT_ENCAPID, DEFAULT_ENCAPDIGEST)


def test_builtin_proof_seals_encap_actuation() -> None:
    report = builtin_encap_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "encap_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_encap"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_encapid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_encapdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_encapid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_encapdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_encap"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_encap"]
    assert report["checks"]["proved_encap_consumes_leftover"]
    assert report["mission_goal"] == ENCAP_ACTUATION_GOAL
    assert report["done_when"] == ENCAP_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[ENCAP_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "encap" in capability.tags
    assert "rfc5512" in capability.tags
    assert "http" in capability.tags
    assert "encapid" in capability.tags
    assert "encapdigest" in capability.tags
    assert "encap" in capability.tags
    assert "safi" in capability.tags


def test_selection_gate_rejects_ledger_only_encap_family(tmp_path: Path) -> None:
    ...
    assert "encap" in family.split("/")
    assert "rfc5512" in family
    assert "encapid" in set(semantic_tokens(ENCAP_ACTUATION_GOAL))
    assert "encapdigest" in family
