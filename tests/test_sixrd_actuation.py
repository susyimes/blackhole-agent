...
def test_goal_binds_sixrd_actuation_plane() -> None:
    assert leftover_marker_ids(SIXRD_ACTUATION_GOAL) == (SIXRD_ACTUATION_ID,)
    ...
    assert SIXRD_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(SIXRD_LEFTOVER) == (SIXRD_ACTUATION_ID,)
    assert leftover_marker_ids(SIXRD_ACTUATION_GOAL) == (SIXRD_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert SIXRD_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(SIXRD_ACTUATION_GOAL)
...
def test_opted_in_sixrd_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("sixrd",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, SIXRD_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["sixrd"]

    missing = run_sixrd_workflow(with_sixrdid=False)
    skip_bind = run_sixrd_workflow(skip_bind=True)
    skip_temporary = run_sixrd_workflow(do_temporary=False)
    skip_public = run_sixrd_workflow(do_public=False)
    skip_sixrddigest = run_sixrd_workflow(do_sixrddigest=False)
    skip_replay = run_sixrd_workflow(replay=False)
    skip_sixrdid = run_sixrd_workflow(use_sixrdid=False)
    live = run_sixrd_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_sixrdid"
    ...
    assert skip_sixrddigest["ok"] is False
    assert skip_sixrddigest["error"] == "sixrddigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_sixrdid["ok"] is False
    assert skip_sixrdid["error"] == "sixrdid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_sixrddigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["sixrddigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["sixrdid_bound"] is True
    assert row["digest"]
    assert live["sixrdid"] == DEFAULT_SIXRDID
    assert live["sixrddigest"] == DEFAULT_SIXRDDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, sixrdid=DEFAULT_SIXRDID, sixrddigest=DEFAULT_SIXRDDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["sixrdid"] == DEFAULT_SIXRDID
    assert queried["sixrddigest"] == DEFAULT_SIXRDDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == SIXRD_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, sixrdid=DEFAULT_SIXRDID, sixrddigest=DEFAULT_SIXRDDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["sixrdid"] == DEFAULT_SIXRDID
    assert answered["sixrddigest"] == DEFAULT_SIXRDDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, sixrdid=DEFAULT_SIXRDID, include_sixrdid=False)
    )
    assert bare["has_sixrdid"] is False
    assert bare["sixrdid"] == EMPTY_SIXRDID
    publicised = serialize_sixrd(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_sixrd(publicised) == DEFAULT_SOURCE
    assert parse_sixrd(RFC_DEST_FIELD) == DEST_HOP
    header = parse_sixrd_header(encode_sixrd_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_SIXRDID, DEFAULT_SIXRDDIGEST))
    ...
    assert asked["method"] == "6RD"
    assert asked["disc_kind"] == "6rd"
    assert listed["disc_kind"] == "deleg"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_SIXRDID).startswith("6RD")
    assert "sixrddigest=" in canonical_public(SENTINEL, DEFAULT_SIXRDID, DEFAULT_SIXRDDIGEST)


def test_builtin_proof_seals_sixrd_actuation() -> None:
    report = builtin_sixrd_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "sixrd_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_sixrd"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_sixrdid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_sixrddigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_sixrdid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_sixrddigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_sixrd"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_sixrd"]
    assert report["checks"]["proved_sixrd_consumes_leftover"]
    assert report["mission_goal"] == SIXRD_ACTUATION_GOAL
    assert report["done_when"] == SIXRD_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[SIXRD_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "sixrd" in capability.tags
    assert "rfc5969" in capability.tags
    assert "http" in capability.tags
    assert "sixrdid" in capability.tags
    assert "sixrddigest" in capability.tags
    assert "6rd" in capability.tags
    assert "deleg" in capability.tags


def test_selection_gate_rejects_ledger_only_sixrd_family(tmp_path: Path) -> None:
    ...
    assert "sixrd" in family.split("/")
    assert "rfc5969" in family
    assert "sixrdid" in set(semantic_tokens(SIXRD_ACTUATION_GOAL))
    assert "sixrddigest" in family
    ...
    assert "sixto4id" not in family
    assert "sixto4digest" not in family
