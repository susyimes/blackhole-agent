...
def test_goal_binds_ason_actuation_plane() -> None:
    assert leftover_marker_ids(ASON_ACTUATION_GOAL) == (ASON_ACTUATION_ID,)
    ...
    assert ASON_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(ASON_LEFTOVER) == (ASON_ACTUATION_ID,)
    assert leftover_marker_ids(ASON_ACTUATION_GOAL) == (ASON_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert ASON_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(ASON_ACTUATION_GOAL)
...
def test_opted_in_ason_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("ason",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, ASON_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["ason"]

    missing = run_ason_workflow(with_asonid=False)
    skip_bind = run_ason_workflow(skip_bind=True)
    skip_temporary = run_ason_workflow(do_temporary=False)
    skip_public = run_ason_workflow(do_public=False)
    skip_asondigest = run_ason_workflow(do_asondigest=False)
    skip_replay = run_ason_workflow(replay=False)
    skip_asonid = run_ason_workflow(use_asonid=False)
    live = run_ason_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_asonid"
    ...
    assert skip_asondigest["ok"] is False
    assert skip_asondigest["error"] == "asondigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_asonid["ok"] is False
    assert skip_asonid["error"] == "asonid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_asondigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["asondigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["asonid_bound"] is True
    assert row["digest"]
    assert live["asonid"] == DEFAULT_ASONID
    assert live["asondigest"] == DEFAULT_ASONDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, asonid=DEFAULT_ASONID, asondigest=DEFAULT_ASONDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["asonid"] == DEFAULT_ASONID
    assert queried["asondigest"] == DEFAULT_ASONDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == ASON_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, asonid=DEFAULT_ASONID, asondigest=DEFAULT_ASONDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["asonid"] == DEFAULT_ASONID
    assert answered["asondigest"] == DEFAULT_ASONDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, asonid=DEFAULT_ASONID, include_asonid=False)
    )
    assert bare["has_asonid"] is False
    assert bare["asonid"] == EMPTY_ASONID
    publicised = serialize_ason(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_ason(publicised) == DEFAULT_SOURCE
    assert parse_ason(RFC_DEST_FIELD) == DEST_HOP
    header = parse_ason_header(encode_ason_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_ASONID, DEFAULT_ASONDIGEST))
    ...
    assert asked["method"] == "CALL"
    assert asked["disc_kind"] == "call"
    assert listed["disc_kind"] == "conn"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_ASONID).startswith("CALL")
    assert "asondigest=" in canonical_public(SENTINEL, DEFAULT_ASONID, DEFAULT_ASONDIGEST)


def test_builtin_proof_seals_ason_actuation() -> None:
    report = builtin_ason_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "ason_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_ason"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_asonid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_asondigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_asonid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_asondigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_ason"]
    ...
    assert report["checks"]["leftover_text_binds_ason"]
    assert report["checks"]["proved_ason_consumes_leftover"]
    assert report["mission_goal"] == ASON_ACTUATION_GOAL
    assert report["done_when"] == ASON_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[ASON_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "ason" in capability.tags
    assert "rfc4397" in capability.tags
    assert "http" in capability.tags
    assert "asonid" in capability.tags
    assert "asondigest" in capability.tags
    assert "call" in capability.tags
    assert "conn" in capability.tags


def test_selection_gate_rejects_ledger_only_ason_family(tmp_path: Path) -> None:
    ...
    assert "ason" in family.split("/")
    assert "rfc4397" in set(semantic_tokens(ASON_ACTUATION_GOAL))
    assert "asonid" in set(semantic_tokens(ASON_ACTUATION_GOAL))
    assert "asondigest" in family
