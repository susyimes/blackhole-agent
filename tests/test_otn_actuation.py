...
def test_goal_binds_otn_actuation_plane() -> None:
    assert leftover_marker_ids(OTN_ACTUATION_GOAL) == (OTN_ACTUATION_ID,)
    ...
    assert OTN_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(OTN_LEFTOVER) == (OTN_ACTUATION_ID,)
    assert leftover_marker_ids(OTN_ACTUATION_GOAL) == (OTN_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert OTN_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(OTN_ACTUATION_GOAL)
...
def test_opted_in_otn_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("otn",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, OTN_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["otn"]

    missing = run_otn_workflow(with_otnid=False)
    skip_bind = run_otn_workflow(skip_bind=True)
    skip_temporary = run_otn_workflow(do_temporary=False)
    skip_public = run_otn_workflow(do_public=False)
    skip_otndigest = run_otn_workflow(do_otndigest=False)
    skip_replay = run_otn_workflow(replay=False)
    skip_otnid = run_otn_workflow(use_otnid=False)
    live = run_otn_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_otnid"
    ...
    assert skip_otndigest["ok"] is False
    assert skip_otndigest["error"] == "otndigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_otnid["ok"] is False
    assert skip_otnid["error"] == "otnid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_otndigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["otndigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["otnid_bound"] is True
    assert row["digest"]
    assert live["otnid"] == DEFAULT_OTNID
    assert live["otndigest"] == DEFAULT_OTNDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, otnid=DEFAULT_OTNID, otndigest=DEFAULT_OTNDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["otnid"] == DEFAULT_OTNID
    assert queried["otndigest"] == DEFAULT_OTNDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == OTN_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, otnid=DEFAULT_OTNID, otndigest=DEFAULT_OTNDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["otnid"] == DEFAULT_OTNID
    assert answered["otndigest"] == DEFAULT_OTNDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, otnid=DEFAULT_OTNID, include_otnid=False)
    )
    assert bare["has_otnid"] is False
    assert bare["otnid"] == EMPTY_OTNID
    publicised = serialize_otn(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_otn(publicised) == DEFAULT_SOURCE
    assert parse_otn(RFC_DEST_FIELD) == DEST_HOP
    header = parse_otn_header(encode_otn_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_OTNID, DEFAULT_OTNDIGEST))
    ...
    assert asked["method"] == "OTU"
    assert asked["disc_kind"] == "otu"
    assert listed["disc_kind"] == "odu"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_OTNID).startswith("OTU")
    assert "otndigest=" in canonical_public(SENTINEL, DEFAULT_OTNID, DEFAULT_OTNDIGEST)


def test_builtin_proof_seals_otn_actuation() -> None:
    report = builtin_otn_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "otn_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_otn"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_otnid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_otndigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_otnid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_otndigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_otn"]
    ...
    assert report["checks"]["leftover_text_binds_otn"]
    assert report["checks"]["proved_otn_consumes_leftover"]
    assert report["mission_goal"] == OTN_ACTUATION_GOAL
    assert report["done_when"] == OTN_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[OTN_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "otn" in capability.tags
    assert "rfc4328" in capability.tags
    assert "http" in capability.tags
    assert "otnid" in capability.tags
    assert "otndigest" in capability.tags
    assert "otu" in capability.tags
    assert "odu" in capability.tags


def test_selection_gate_rejects_ledger_only_otn_family(tmp_path: Path) -> None:
    ...
    assert "otn" in family.split("/")
    assert "rfc4328" in set(semantic_tokens(OTN_ACTUATION_GOAL))
    assert "otnid" in set(semantic_tokens(OTN_ACTUATION_GOAL))
    assert "otndigest" in family
