...
def test_goal_binds_ucpe_actuation_plane() -> None:
    assert leftover_marker_ids(UCPE_ACTUATION_GOAL) == (UCPE_ACTUATION_ID,)
    ...
    assert UCPE_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(UCPE_LEFTOVER) == (UCPE_ACTUATION_ID,)
    assert leftover_marker_ids(UCPE_ACTUATION_GOAL) == (UCPE_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert UCPE_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(UCPE_ACTUATION_GOAL)
...
def test_opted_in_ucpe_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("ucpe",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, UCPE_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["ucpe"]

    missing = run_ucpe_workflow(with_ucpeid=False)
    skip_bind = run_ucpe_workflow(skip_bind=True)
    skip_temporary = run_ucpe_workflow(do_temporary=False)
    skip_public = run_ucpe_workflow(do_public=False)
    skip_ucpedigest = run_ucpe_workflow(do_ucpedigest=False)
    skip_replay = run_ucpe_workflow(replay=False)
    skip_ucpeid = run_ucpe_workflow(use_ucpeid=False)
    live = run_ucpe_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_ucpeid"
    ...
    assert skip_ucpedigest["ok"] is False
    assert skip_ucpedigest["error"] == "ucpedigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_ucpeid["ok"] is False
    assert skip_ucpeid["error"] == "ucpeid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_ucpedigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["ucpedigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["ucpeid_bound"] is True
    assert row["digest"]
    assert live["ucpeid"] == DEFAULT_UCPEID
    assert live["ucpedigest"] == DEFAULT_UCPEDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, ucpeid=DEFAULT_UCPEID, ucpedigest=DEFAULT_UCPEDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["ucpeid"] == DEFAULT_UCPEID
    assert queried["ucpedigest"] == DEFAULT_UCPEDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == UCPE_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, ucpeid=DEFAULT_UCPEID, ucpedigest=DEFAULT_UCPEDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["ucpeid"] == DEFAULT_UCPEID
    assert answered["ucpedigest"] == DEFAULT_UCPEDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, ucpeid=DEFAULT_UCPEID, include_ucpeid=False)
    )
    assert bare["has_ucpeid"] is False
    assert bare["ucpeid"] == EMPTY_UCPEID
    publicised = serialize_ucpe(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_ucpe(publicised) == DEFAULT_SOURCE
    assert parse_ucpe(RFC_DEST_FIELD) == DEST_HOP
    header = parse_ucpe_header(encode_ucpe_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_UCPEID, DEFAULT_UCPEDIGEST))
    ...
    assert asked["method"] == "CONTAINER"
    assert asked["disc_kind"] == "container"
    assert listed["disc_kind"] == "provision"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_UCPEID).startswith("CONTAINER")
    assert "ucpedigest=" in canonical_public(SENTINEL, DEFAULT_UCPEID, DEFAULT_UCPEDIGEST)


def test_builtin_proof_seals_ucpe_actuation() -> None:
    report = builtin_ucpe_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "ucpe_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_ucpe"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_ucpeid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_ucpedigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_ucpeid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_ucpedigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_ucpe"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_ucpe"]
    assert report["checks"]["proved_ucpe_consumes_leftover"]
    assert report["mission_goal"] == UCPE_ACTUATION_GOAL
    assert report["done_when"] == UCPE_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[UCPE_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "ucpe" in capability.tags
    assert "rfc8026" in capability.tags
    assert "http" in capability.tags
    assert "ucpeid" in capability.tags
    assert "ucpedigest" in capability.tags
    assert "container" in capability.tags
    assert "provision" in capability.tags


def test_selection_gate_rejects_ledger_only_ucpe_family(tmp_path: Path) -> None:
    ...
    assert "ucpe" in family.split("/")
    assert "rfc8026" in family
    assert "ucpeid" in family
    assert "ucpedigest" in family
