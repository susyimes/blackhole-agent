...
from blackhole_agent.p2mpte_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_p2mpte_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_p2mpte_header,
    independent_p2mptedigest,
    parse_http_comm,
    parse_http_response,
    parse_message,
    parse_p2mpte,
...
def test_goal_binds_p2mpte_actuation_plane() -> None:
    assert leftover_marker_ids(P2MPTE_ACTUATION_GOAL) == (P2MPTE_ACTUATION_ID,)
    ...
    assert P2MPTE_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(P2MPTE_LEFTOVER) == (P2MPTE_ACTUATION_ID,)
    assert leftover_marker_ids(P2MPTE_ACTUATION_GOAL) == (P2MPTE_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert P2MPTE_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(P2MPTE_ACTUATION_GOAL)
...
def test_opted_in_p2mpte_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("p2mpte",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, P2MPTE_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["p2mpte"]

    missing = run_p2mpte_workflow(with_p2mpteid=False)
    skip_bind = run_p2mpte_workflow(skip_bind=True)
    skip_temporary = run_p2mpte_workflow(do_temporary=False)
    skip_public = run_p2mpte_workflow(do_public=False)
    skip_p2mptedigest = run_p2mpte_workflow(do_p2mptedigest=False)
    skip_replay = run_p2mpte_workflow(replay=False)
    skip_p2mpteid = run_p2mpte_workflow(use_p2mpteid=False)
    live = run_p2mpte_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_p2mpteid"
    ...
    assert skip_p2mptedigest["ok"] is False
    assert skip_p2mptedigest["error"] == "p2mptedigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_p2mpteid["ok"] is False
    assert skip_p2mpteid["error"] == "p2mpteid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_p2mptedigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["p2mptedigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["p2mpteid_bound"] is True
    assert row["digest"]
    assert live["p2mpteid"] == DEFAULT_P2MPTEID
    assert live["p2mptedigest"] == DEFAULT_P2MPTEDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, p2mpteid=DEFAULT_P2MPTEID, p2mptedigest=DEFAULT_P2MPTEDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["p2mpteid"] == DEFAULT_P2MPTEID
    assert queried["p2mptedigest"] == DEFAULT_P2MPTEDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == P2MPTE_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, p2mpteid=DEFAULT_P2MPTEID, p2mptedigest=DEFAULT_P2MPTEDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["p2mpteid"] == DEFAULT_P2MPTEID
    assert answered["p2mptedigest"] == DEFAULT_P2MPTEDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, p2mpteid=DEFAULT_P2MPTEID, include_p2mpteid=False)
    )
    assert bare["has_p2mpteid"] is False
    assert bare["p2mpteid"] == EMPTY_P2MPTEID
    publicised = serialize_p2mpte(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_p2mpte(publicised) == DEFAULT_SOURCE
    assert parse_p2mpte(RFC_DEST_FIELD) == DEST_HOP
    header = parse_p2mpte_header(encode_p2mpte_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_P2MPTEID, DEFAULT_P2MPTEDIGEST))
    ...
    assert listed["disc_kind"] == "s2l"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_P2MPTEID).startswith("P2MP")
    assert "p2mptedigest=" in canonical_public(SENTINEL, DEFAULT_P2MPTEID, DEFAULT_P2MPTEDIGEST)


def test_builtin_proof_seals_p2mpte_actuation() -> None:
    report = builtin_p2mpte_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "p2mpte_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_p2mpte"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_p2mpteid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_p2mptedigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_p2mpteid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_p2mptedigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_p2mpte"]
    ...
    assert report["checks"]["leftover_text_binds_p2mpte"]
    assert report["checks"]["proved_p2mpte_consumes_leftover"]
    assert report["mission_goal"] == P2MPTE_ACTUATION_GOAL
    assert report["done_when"] == P2MPTE_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[P2MPTE_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "p2mpte" in capability.tags
    assert "rfc4875" in capability.tags
    assert "http" in capability.tags
    assert "p2mpteid" in capability.tags
    assert "p2mptedigest" in capability.tags
    assert "p2mp" in capability.tags
    assert "s2l" in capability.tags


def test_selection_gate_rejects_ledger_only_p2mpte_family(tmp_path: Path) -> None:
    ...
    assert "p2mpte" in family.split("/")
    assert "rfc4875" in set(semantic_tokens(P2MPTE_ACTUATION_GOAL))
    assert "p2mpteid" in set(semantic_tokens(P2MPTE_ACTUATION_GOAL))
    assert "p2mptedigest" in family
