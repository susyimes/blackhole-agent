...
def test_goal_binds_tcp_actuation_plane() -> None:
    assert leftover_marker_ids(TCP_ACTUATION_GOAL) == (TCP_ACTUATION_ID,)
    assert leftover_marker_ids(TCP_LEFTOVER) == (TCP_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert TCP_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(TCP_ACTUATION_GOAL)
...
def test_opted_in_tcp_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("tcp",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, TCP_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["tcp"]

    missing = run_tcp_workflow(with_tcpid=False)
    skip_bind = run_tcp_workflow(skip_bind=True)
    skip_syn = run_tcp_workflow(do_syn=False)
    skip_ack = run_tcp_workflow(do_ack=False)
    skip_tcpdigest = run_tcp_workflow(do_tcpdigest=False)
    skip_replay = run_tcp_workflow(replay=False)
    skip_tcpid = run_tcp_workflow(use_tcpid=False)
    live = run_tcp_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_tcpid"
    assert skip_bind["ok"] is False
    assert skip_bind["error"] == "not_bound"
    assert skip_syn["ok"] is False
    assert skip_syn["error"] == "syn_required"
    assert skip_ack["ok"] is False
    assert skip_ack["error"] == "ack_required"
    assert skip_tcpdigest["ok"] is False
    assert skip_tcpdigest["error"] == "tcpdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_tcpid["ok"] is False
    assert skip_tcpid["error"] == "tcpid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_tcpdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["syn_frame"] is True
    assert row["ack_frame"] is True
    assert row["tcpdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["tcpid_bound"] is True
    assert row["digest"]
    assert live["tcpid"] == DEFAULT_TCPID
    assert live["tcpdigest"] == DEFAULT_TCPDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_syn(identity=SENTINEL, tcpid=DEFAULT_TCPID, tcpdigest=DEFAULT_TCPDIGEST)
    )
    assert queried["is_syn"] is True and queried["is_ack"] is False
    assert queried["identity"] == SENTINEL and queried["tcpid"] == DEFAULT_TCPID
    assert queried["tcpdigest"] == DEFAULT_TCPDIGEST
    assert queried["type"] == FRAME_SYN
    assert queried["first_byte"] == TCP_FIRST
    answered = parse_message(
        encode_ack(identity=SENTINEL, tcpid=DEFAULT_TCPID, tcpdigest=DEFAULT_TCPDIGEST)
    )
    assert answered["is_ack"] is True and answered["is_ack"] is True
    assert answered["tcpid"] == DEFAULT_TCPID
    assert answered["tcpdigest"] == DEFAULT_TCPDIGEST
    ...
    bare = parse_message(
        encode_syn(identity=SENTINEL, tcpid=DEFAULT_TCPID, include_tcpid=False)
    )
    assert bare["has_tcpid"] is False
    assert bare["tcpid"] == EMPTY_TCPID
    advertised = serialize_tcp(DEFAULT_SYN)
    assert advertised == RFC_SYN_FIELD
    assert parse_tcp(advertised) == DEFAULT_SYN
    assert parse_tcp(RFC_ACK_FIELD) == ACK_POLICY
    header = parse_tcp_header(encode_tcp_header(DEFAULT_SYN))
    assert header["field_value"] == RFC_SYN_FIELD
    assert header["header"] == SYN_HEADER
    asked = parse_http_request(syn_request(SENTINEL, DEFAULT_TCPID))
    listed = parse_http_request(ack_request(SENTINEL, DEFAULT_TCPID, DEFAULT_TCPDIGEST))
    ...
    assert asked["method"] == "SYN"
    assert asked["tcp_kind"] == "syn"
    assert listed["tcp_kind"] == "ack"
    assert got["status"] == 200
    assert preload_reply["status"] == 200
    assert got["policy"] == DEFAULT_SYN
    assert preload_reply["policy"] == ACK_POLICY
    assert canonical_syn(SENTINEL, DEFAULT_TCPID).startswith("SYN")
    assert "tcpdigest=" in canonical_ack(SENTINEL, DEFAULT_TCPID, DEFAULT_TCPDIGEST)


def test_builtin_proof_seals_tcp_actuation() -> None:
    report = builtin_tcp_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "tcp_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_tcp"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_tcpid_is_forbidden"]
    assert report["checks"]["skip_syn_stays_empty"]
    assert report["checks"]["skip_ack_stays_empty"]
    assert report["checks"]["skip_tcpdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_tcpid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_tcpdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_tcp"]
    ...
    assert report["checks"]["leftover_text_binds_tcp"]
    assert report["checks"]["proved_tcp_consumes_leftover"]
    assert report["mission_goal"] == TCP_ACTUATION_GOAL
    assert report["done_when"] == TCP_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[TCP_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "tcp" in capability.tags
    assert "rfc793" in capability.tags
    assert "http" in capability.tags
    assert "tcpid" in capability.tags
    assert "tcpdigest" in capability.tags
    assert "syn" in capability.tags


def test_selection_gate_rejects_ledger_only_tcp_family(tmp_path: Path) -> None:
    ...
    assert "tcp" in family
    assert "rfc793" in family
    assert "tcpid" in family
    assert "tcpdigest" in family
