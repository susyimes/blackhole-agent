...
def test_goal_binds_ebgp_actuation_plane() -> None:
    assert leftover_marker_ids(EBGP_ACTUATION_GOAL) == (EBGP_ACTUATION_ID,)
    ...
    assert EBGP_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(EBGP_LEFTOVER) == (EBGP_ACTUATION_ID,)
    assert leftover_marker_ids(EBGP_ACTUATION_GOAL) == (EBGP_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert EBGP_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(EBGP_ACTUATION_GOAL)
...
def test_opted_in_ebgp_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("ebgp",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, EBGP_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["ebgp"]

    missing = run_ebgp_workflow(with_ebgpid=False)
    skip_bind = run_ebgp_workflow(skip_bind=True)
    skip_temporary = run_ebgp_workflow(do_temporary=False)
    skip_public = run_ebgp_workflow(do_public=False)
    skip_ebgpdigest = run_ebgp_workflow(do_ebgpdigest=False)
    skip_replay = run_ebgp_workflow(replay=False)
    skip_ebgpid = run_ebgp_workflow(use_ebgpid=False)
    live = run_ebgp_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_ebgpid"
    ...
    assert skip_ebgpdigest["ok"] is False
    assert skip_ebgpdigest["error"] == "ebgpdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_ebgpid["ok"] is False
    assert skip_ebgpid["error"] == "ebgpid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_ebgpdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["ebgpdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["ebgpid_bound"] is True
    assert row["digest"]
    assert live["ebgpid"] == DEFAULT_EBGPID
    assert live["ebgpdigest"] == DEFAULT_EBGPDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, ebgpid=DEFAULT_EBGPID, ebgpdigest=DEFAULT_EBGPDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["ebgpid"] == DEFAULT_EBGPID
    assert queried["ebgpdigest"] == DEFAULT_EBGPDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == EBGP_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, ebgpid=DEFAULT_EBGPID, ebgpdigest=DEFAULT_EBGPDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["ebgpid"] == DEFAULT_EBGPID
    assert answered["ebgpdigest"] == DEFAULT_EBGPDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, ebgpid=DEFAULT_EBGPID, include_ebgpid=False)
    )
    assert bare["has_ebgpid"] is False
    assert bare["ebgpid"] == EMPTY_EBGPID
    publicised = serialize_ebgp(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_ebgp(publicised) == DEFAULT_SOURCE
    assert parse_ebgp(RFC_DEST_FIELD) == DEST_HOP
    header = parse_ebgp_header(encode_ebgp_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_EBGPID, DEFAULT_EBGPDIGEST))
    ...
    assert asked["method"] == "DEF"
    assert asked["disc_kind"] == "ebgp"
    assert listed["disc_kind"] == "prop"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_EBGPID).startswith("DEF")
    assert "ebgpdigest=" in canonical_public(SENTINEL, DEFAULT_EBGPID, DEFAULT_EBGPDIGEST)


def test_builtin_proof_seals_ebgp_actuation() -> None:
    report = builtin_ebgp_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "ebgp_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_ebgp"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_ebgpid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_ebgpdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_ebgpid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_ebgpdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_ebgp"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_ebgp"]
    assert report["checks"]["proved_ebgp_consumes_leftover"]
    assert report["mission_goal"] == EBGP_ACTUATION_GOAL
    assert report["done_when"] == EBGP_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[EBGP_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "ebgp" in capability.tags
    assert "rfc8212" in capability.tags
    assert "http" in capability.tags
    assert "ebgpid" in capability.tags
    assert "ebgpdigest" in capability.tags
    assert "ebgp" in capability.tags
    assert "prop" in capability.tags


def test_selection_gate_rejects_ledger_only_ebgp_family(tmp_path: Path) -> None:
    ...
    assert "ebgp" in family.split("/")
    assert "rfc8212" in family
    assert "ebgpid" in set(semantic_tokens(EBGP_ACTUATION_GOAL))
    assert "ebgpdigest" in family
