...
def test_goal_binds_pwst_actuation_plane() -> None:
    assert leftover_marker_ids(PWST_ACTUATION_GOAL) == (PWST_ACTUATION_ID,)
    ...
    assert PWST_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(PWST_LEFTOVER) == (PWST_ACTUATION_ID,)
    assert leftover_marker_ids(PWST_ACTUATION_GOAL) == (PWST_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert PWST_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(PWST_ACTUATION_GOAL)
...
def test_opted_in_pwst_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("pwst",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, PWST_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["pwst"]

    missing = run_pwst_workflow(with_pwstid=False)
    skip_bind = run_pwst_workflow(skip_bind=True)
    skip_temporary = run_pwst_workflow(do_temporary=False)
    skip_public = run_pwst_workflow(do_public=False)
    skip_pwstdigest = run_pwst_workflow(do_pwstdigest=False)
    skip_replay = run_pwst_workflow(replay=False)
    skip_pwstid = run_pwst_workflow(use_pwstid=False)
    live = run_pwst_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_pwstid"
    ...
    assert skip_pwstdigest["ok"] is False
    assert skip_pwstdigest["error"] == "pwstdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_pwstid["ok"] is False
    assert skip_pwstid["error"] == "pwstid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_pwstdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["pwstdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["pwstid_bound"] is True
    assert row["digest"]
    assert live["pwstid"] == DEFAULT_PWSTID
    assert live["pwstdigest"] == DEFAULT_PWSTDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, pwstid=DEFAULT_PWSTID, pwstdigest=DEFAULT_PWSTDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["pwstid"] == DEFAULT_PWSTID
    assert queried["pwstdigest"] == DEFAULT_PWSTDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == PWST_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, pwstid=DEFAULT_PWSTID, pwstdigest=DEFAULT_PWSTDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["pwstid"] == DEFAULT_PWSTID
    assert answered["pwstdigest"] == DEFAULT_PWSTDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, pwstid=DEFAULT_PWSTID, include_pwstid=False)
    )
    assert bare["has_pwstid"] is False
    assert bare["pwstid"] == EMPTY_PWSTID
    publicised = serialize_pwst(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_pwst(publicised) == DEFAULT_SOURCE
    assert parse_pwst(RFC_DEST_FIELD) == DEST_HOP
    header = parse_pwst_header(encode_pwst_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_PWSTID, DEFAULT_PWSTDIGEST))
    ...
    assert asked["method"] == "STATUS"
    assert asked["disc_kind"] == "status"
    assert listed["disc_kind"] == "ack"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_PWSTID).startswith("STATUS")
    assert "pwstdigest=" in canonical_public(SENTINEL, DEFAULT_PWSTID, DEFAULT_PWSTDIGEST)


def test_builtin_proof_seals_pwst_actuation() -> None:
    report = builtin_pwst_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "pwst_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_pwst"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_pwstid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_pwstdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_pwstid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_pwstdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_binds_pwst"]
    ...
    assert report["checks"]["catalog_names_pwst"]
    assert report["checks"]["catalog_names_mldp"]
    assert report["checks"]["catalog_names_wson"]
    assert report["checks"]["leftover_text_binds_pwst"]
    assert report["checks"]["proved_pwst_consumes_leftover"]
    assert report["mission_goal"] == PWST_ACTUATION_GOAL
    assert report["done_when"] == PWST_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[PWST_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "pwst" in capability.tags
    assert "rfc6478" in capability.tags
    assert "http" in capability.tags
    assert "pwstid" in capability.tags
    assert "pwstdigest" in capability.tags
    assert "status" in capability.tags
    assert "ack" in capability.tags


def test_selection_gate_rejects_ledger_only_pwst_family(tmp_path: Path) -> None:
    ...
    assert "pwst" in family.split("/")
    assert "rfc6478" in set(semantic_tokens(PWST_ACTUATION_GOAL))
    assert "pwstid" in set(semantic_tokens(PWST_ACTUATION_GOAL))
    assert "pwstdigest" in family
