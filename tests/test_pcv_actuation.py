...
def test_goal_binds_pcv_actuation_plane() -> None:
    assert leftover_marker_ids(PCV_ACTUATION_GOAL) == (PCV_ACTUATION_ID,)
    ...
    assert PCV_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(PCV_LEFTOVER) == (PCV_ACTUATION_ID,)
    assert leftover_marker_ids(PCV_ACTUATION_GOAL) == (PCV_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert PCV_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(PCV_ACTUATION_GOAL)
    icp_signature = semantic_signature(PCV_ACTUATION_GOAL)
    for neighbor in NEIGHBORS:
        assert semantic_similarity(icp_signature, semantic_signature(neighbor)) < 0.82


def test_opted_in_pcv_tool_completes_body_transfer_poll() -> None:
    descriptor = pcv_tool_descriptor()
    naive = route_tool_descriptor(descriptor)
    opted = route_tool_descriptor(
        descriptor,
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, PCV_TOOL_PROVIDER),
    )
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("pcv",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, PCV_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["pcv"]

    missing = run_pcv_workflow(with_pcvid=False)
    skip_bind = run_pcv_workflow(skip_bind=True)
    skip_temporary = run_pcv_workflow(do_temporary=False)
    skip_public = run_pcv_workflow(do_public=False)
    skip_pcvdigest = run_pcv_workflow(do_pcvdigest=False)
    skip_replay = run_pcv_workflow(replay=False)
    skip_pcvid = run_pcv_workflow(use_pcvid=False)
    live = run_pcv_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_pcvid"
    ...
    assert skip_pcvdigest["ok"] is False
    assert skip_pcvdigest["error"] == "pcvdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_pcvid["ok"] is False
    assert skip_pcvid["error"] == "pcvid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_pcvdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["pcvdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["pcvid_bound"] is True
    assert row["digest"]
    assert live["pcvid"] == DEFAULT_PCVID
    assert live["pcvdigest"] == DEFAULT_PCVDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, pcvid=DEFAULT_PCVID, pcvdigest=DEFAULT_PCVDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["pcvid"] == DEFAULT_PCVID
    assert queried["pcvdigest"] == DEFAULT_PCVDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == PCV_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, pcvid=DEFAULT_PCVID, pcvdigest=DEFAULT_PCVDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["pcvid"] == DEFAULT_PCVID
    assert answered["pcvdigest"] == DEFAULT_PCVDIGEST
    packed = encode_temporary(identity=SENTINEL, pcvid=DEFAULT_PCVID, pcvdigest=DEFAULT_PCVDIGEST)
    zeroed = packed[:-4] + (0).to_bytes(4, "big")
    assert crc32c(zeroed) == int.from_bytes(packed[-4:], "big")
    bare = parse_message(
        encode_temporary(identity=SENTINEL, pcvid=DEFAULT_PCVID, include_pcvid=False)
    )
    assert bare["has_pcvid"] is False
    assert bare["pcvid"] == EMPTY_PCVID
    publicised = serialize_pcv(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_pcv(publicised) == DEFAULT_SOURCE
    assert parse_pcv(RFC_DEST_FIELD) == DEST_HOP
    header = parse_pcv_header(encode_pcv_header(DEFAULT_SOURCE))
    assert header["field_value"] == RFC_SOURCE_FIELD
    assert header["header"] == SOURCE_HEADER
    asked = parse_http_comm(temporary_comm(SENTINEL, DEFAULT_PCVID))
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_PCVID, DEFAULT_PCVDIGEST))
    got = parse_http_response(temporary_response(SENTINEL, DEFAULT_PCVID, DEFAULT_PCVDIGEST))
    preload_public = parse_http_response(
        public_response(SENTINEL, DEFAULT_PCVID, DEFAULT_PCVDIGEST)
    )
    assert asked["method"] == "CC"
    assert asked["disc_kind"] == "cc"
    assert listed["disc_kind"] == "cv"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_PCVID).startswith("CC")
    assert "pcvdigest=" in canonical_public(SENTINEL, DEFAULT_PCVID, DEFAULT_PCVDIGEST)


def test_builtin_proof_seals_pcv_actuation() -> None:
    report = builtin_pcv_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "pcv_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_pcv"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_pcvid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_pcvdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_pcvid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_pcvdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_binds_pcv"]
    ...
    assert report["checks"]["catalog_names_wson"]
    assert report["checks"]["leftover_text_binds_pcv"]
    assert report["checks"]["proved_pcv_consumes_leftover"]
    assert report["mission_goal"] == PCV_ACTUATION_GOAL
    assert report["done_when"] == PCV_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[PCV_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "pcv" in capability.tags
    assert "rfc6428" in capability.tags
    assert "http" in capability.tags
    assert "pcvid" in capability.tags
    assert "pcvdigest" in capability.tags
    assert "cc" in capability.tags
    assert "cv" in capability.tags


def test_selection_gate_rejects_ledger_only_pcv_family(tmp_path: Path) -> None:
    ...
    family = capability_family(PCV_ACTUATION_GOAL)
    family_tokens = set(family.split("/"))
    assert "pcv" in family.split("/")
    assert "rfc6428" in set(semantic_tokens(PCV_ACTUATION_GOAL))
    assert "pcvid" in set(semantic_tokens(PCV_ACTUATION_GOAL))
    assert "pcvdigest" in family
    ...
    assert "lilb" not in family.split("/")
    assert "rfc6435" not in family
    assert "lilbid" not in family
    assert "lilbdigest" not in family
