"""Acceptance probe: reaped orphan starts once; a live owner is not doubled.
...
observed: dict[str, object] = {"family": "loop-reboot-restore"}
passed = False
...
with continuous_loop_lock(continuous_loop_lock_path(repo)):
    save_continuous_loop_state(continuous_loop_state_path(repo), {
        'loop_id': 'restored-controller', 'pid': os.getpid(),
...
def _wait_ready(repo: Path, child: subprocess.Popen[str], timeout: float = 15) -> int:
    ...
    while not (repo / "ready").exists():
        if child.poll() is not None or time.monotonic() >= deadline:
            raise RuntimeError("restored controller did not become ready")
        time.sleep(0.02)
...
try:
    ...
    with tempfile.TemporaryDirectory(prefix="accept-restore-orphan-", ignore_cleanup_errors=True) as tmp:
        repo = Path(tmp)
    ...
    with tempfile.TemporaryDirectory(prefix="accept-restore-live-", ignore_cleanup_errors=True) as tmp:
        repo = Path(tmp)
        ...
        result = restore_orphaned_loop_on_startup(repo, controller_starter=_starter_for(src))
        live_ok = (
    ...
    observed.update(
        {
            ...
            "orphaned_restored_once": orphan_ok,
            "live_owner_not_doubled": live_ok,
            "controller_count": len(children),
            "sentinel": "BH-LOOP-REBOOT-RESTORE-OK" if orphan_ok and live_ok else "",
            "error": "",
        }
    )
