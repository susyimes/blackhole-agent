...
def test_goal_binds_gmpls_actuation_plane() -> None:
    assert leftover_marker_ids(GMPLS_ACTUATION_GOAL) == (GMPLS_ACTUATION_ID,)
    ...
    assert GMPLS_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(GMPLS_LEFTOVER) == (GMPLS_ACTUATION_ID,)
    assert leftover_marker_ids(GMPLS_ACTUATION_GOAL) == (GMPLS_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert GMPLS_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(GMPLS_ACTUATION_GOAL)
...
def test_opted_in_gmpls_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("gmpls",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, GMPLS_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["gmpls"]

    missing = run_gmpls_workflow(with_gmplsid=False)
    skip_bind = run_gmpls_workflow(skip_bind=True)
    skip_temporary = run_gmpls_workflow(do_temporary=False)
    skip_public = run_gmpls_workflow(do_public=False)
    skip_gmplsdigest = run_gmpls_workflow(do_gmplsdigest=False)
    skip_replay = run_gmpls_workflow(replay=False)
    skip_gmplsid = run_gmpls_workflow(use_gmplsid=False)
    live = run_gmpls_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_gmplsid"
    ...
    assert skip_gmplsdigest["ok"] is False
    assert skip_gmplsdigest["error"] == "gmplsdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_gmplsid["ok"] is False
    assert skip_gmplsid["error"] == "gmplsid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_gmplsdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["gmplsdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["gmplsid_bound"] is True
    assert row["digest"]
    assert live["gmplsid"] == DEFAULT_GMPLSID
    assert live["gmplsdigest"] == DEFAULT_GMPLSDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, gmplsid=DEFAULT_GMPLSID, gmplsdigest=DEFAULT_GMPLSDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["gmplsid"] == DEFAULT_GMPLSID
    assert queried["gmplsdigest"] == DEFAULT_GMPLSDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == GMPLS_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, gmplsid=DEFAULT_GMPLSID, gmplsdigest=DEFAULT_GMPLSDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["gmplsid"] == DEFAULT_GMPLSID
    assert answered["gmplsdigest"] == DEFAULT_GMPLSDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, gmplsid=DEFAULT_GMPLSID, include_gmplsid=False)
    )
    assert bare["has_gmplsid"] is False
    assert bare["gmplsid"] == EMPTY_GMPLSID
    publicised = serialize_gmpls(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_gmpls(publicised) == DEFAULT_SOURCE
    assert parse_gmpls(RFC_DEST_FIELD) == DEST_HOP
    header = parse_gmpls_header(encode_gmpls_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_GMPLSID, DEFAULT_GMPLSDIGEST))
    ...
    assert listed["disc_kind"] == "resvconf"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_GMPLSID).startswith("NOTIFY")
    assert "gmplsdigest=" in canonical_public(SENTINEL, DEFAULT_GMPLSID, DEFAULT_GMPLSDIGEST)


def test_builtin_proof_seals_gmpls_actuation() -> None:
    report = builtin_gmpls_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "gmpls_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_gmpls"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_gmplsid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_gmplsdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_gmplsid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_gmplsdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_gmpls"]
    ...
    assert report["checks"]["catalog_names_gmpls"]
    assert report["checks"]["leftover_text_binds_gmpls"]
    assert report["checks"]["proved_gmpls_consumes_leftover"]
    assert report["mission_goal"] == GMPLS_ACTUATION_GOAL
    assert report["done_when"] == GMPLS_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[GMPLS_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "gmpls" in capability.tags
    assert "rfc3473" in capability.tags
    assert "http" in capability.tags
    assert "gmplsid" in capability.tags
    assert "gmplsdigest" in capability.tags
    assert "notify" in capability.tags
    assert "resvconf" in capability.tags


def test_selection_gate_rejects_ledger_only_gmpls_family(tmp_path: Path) -> None:
    ...
    assert "gmpl" in family.split("/")
    assert "rfc3473" in set(semantic_tokens(GMPLS_ACTUATION_GOAL))
    assert "gmplsid" in set(semantic_tokens(GMPLS_ACTUATION_GOAL))
    assert "gmplsdigest" in family
