...
def test_goal_binds_siit_actuation_plane() -> None:
    assert leftover_marker_ids(SIIT_ACTUATION_GOAL) == (SIIT_ACTUATION_ID,)
    ...
    assert SIIT_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(SIIT_LEFTOVER) == (SIIT_ACTUATION_ID,)
    assert leftover_marker_ids(SIIT_ACTUATION_GOAL) == (SIIT_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert SIIT_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(SIIT_ACTUATION_GOAL)
...
def test_opted_in_siit_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("siit",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, SIIT_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["siit"]

    missing = run_siit_workflow(with_siitid=False)
    skip_bind = run_siit_workflow(skip_bind=True)
    skip_temporary = run_siit_workflow(do_temporary=False)
    skip_public = run_siit_workflow(do_public=False)
    skip_siitdigest = run_siit_workflow(do_siitdigest=False)
    skip_replay = run_siit_workflow(replay=False)
    skip_siitid = run_siit_workflow(use_siitid=False)
    live = run_siit_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_siitid"
    ...
    assert skip_siitdigest["ok"] is False
    assert skip_siitdigest["error"] == "siitdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_siitid["ok"] is False
    assert skip_siitid["error"] == "siitid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_siitdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["siitdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["siitid_bound"] is True
    assert row["digest"]
    assert live["siitid"] == DEFAULT_SIITID
    assert live["siitdigest"] == DEFAULT_SIITDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, siitid=DEFAULT_SIITID, siitdigest=DEFAULT_SIITDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["siitid"] == DEFAULT_SIITID
    assert queried["siitdigest"] == DEFAULT_SIITDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == SIIT_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, siitid=DEFAULT_SIITID, siitdigest=DEFAULT_SIITDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["siitid"] == DEFAULT_SIITID
    assert answered["siitdigest"] == DEFAULT_SIITDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, siitid=DEFAULT_SIITID, include_siitid=False)
    )
    assert bare["has_siitid"] is False
    assert bare["siitid"] == EMPTY_SIITID
    publicised = serialize_siit(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_siit(publicised) == DEFAULT_SOURCE
    assert parse_siit(RFC_DEST_FIELD) == DEST_HOP
    header = parse_siit_header(encode_siit_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_SIITID, DEFAULT_SIITDIGEST))
    ...
    assert asked["method"] == "TRANSLATE"
    assert asked["disc_kind"] == "translate"
    assert listed["disc_kind"] == "icmp"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_SIITID).startswith("TRANSLATE")
    assert "siitdigest=" in canonical_public(SENTINEL, DEFAULT_SIITID, DEFAULT_SIITDIGEST)


def test_builtin_proof_seals_siit_actuation() -> None:
    report = builtin_siit_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "siit_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_siit"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_siitid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_siitdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_siitid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_siitdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_siit"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_siit"]
    assert report["checks"]["proved_siit_consumes_leftover"]
    assert report["mission_goal"] == SIIT_ACTUATION_GOAL
    assert report["done_when"] == SIIT_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[SIIT_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "siit" in capability.tags
    assert "rfc7915" in capability.tags
    assert "http" in capability.tags
    assert "siitid" in capability.tags
    assert "siitdigest" in capability.tags
    assert "translate" in capability.tags
...
def test_selection_gate_rejects_ledger_only_siit_family(tmp_path: Path) -> None:
    ...
    assert "eamid" not in family
    assert "eamdigest" not in family
