...
def test_goal_binds_icp_actuation_plane() -> None:
    assert leftover_marker_ids(ICP_ACTUATION_GOAL) == (ICP_ACTUATION_ID,)
    assert leftover_marker_ids(ICP_LEFTOVER) == (ICP_ACTUATION_ID,)
    assert leftover_marker_ids(ICP_ACTUATION_GOAL) == (ICP_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert ICP_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(ICP_ACTUATION_GOAL)
    icp_signature = semantic_signature(ICP_ACTUATION_GOAL)
    for neighbor in NEIGHBORS:
        assert semantic_similarity(icp_signature, semantic_signature(neighbor)) < 0.82


def test_opted_in_icp_tool_completes_query_hit_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("icp",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, ICP_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["icp"]

    missing = run_icp_workflow(with_queryid=False)
    skip_bind = run_icp_workflow(skip_bind=True)
    skip_query = run_icp_workflow(do_query=False)
    skip_hit = run_icp_workflow(do_hit=False)
    skip_icpdigest = run_icp_workflow(do_icpdigest=False)
    skip_replay = run_icp_workflow(replay=False)
    skip_queryid = run_icp_workflow(use_queryid=False)
    live = run_icp_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_queryid"
    assert skip_bind["ok"] is False
    assert skip_bind["error"] == "not_bound"
    assert skip_query["ok"] is False
    assert skip_query["error"] == "query_required"
    assert skip_hit["ok"] is False
    assert skip_hit["error"] == "hit_required"
    assert skip_icpdigest["ok"] is False
    assert skip_icpdigest["error"] == "icpdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_queryid["ok"] is False
    assert skip_queryid["error"] == "queryid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_icp_digest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["query_frame"] is True
    assert row["hit_frame"] is True
    assert row["icpdigest_response"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["queryid_bound"] is True
    assert row["digest"]
    assert live["queryid"] == DEFAULT_QUERYID
    assert live["icpdigest"] == DEFAULT_ICPDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_query(identity=SENTINEL, queryid=DEFAULT_QUERYID, icpdigest=DEFAULT_ICPDIGEST)
    )
    assert queried["is_query"] is True and queried["is_response"] is False
    assert queried["identity"] == SENTINEL and queried["queryid"] == DEFAULT_QUERYID
    assert queried["icpdigest"] == DEFAULT_ICPDIGEST
    assert queried["type"] == FRAME_QUERY
    assert queried["first_byte"] == ICP_FIRST
    answered = parse_message(
        encode_hit(identity=SENTINEL, queryid=DEFAULT_QUERYID, icpdigest=DEFAULT_ICPDIGEST)
    )
    assert answered["is_hit"] is True and answered["is_response"] is True
    assert answered["queryid"] == DEFAULT_QUERYID
    assert answered["icpdigest"] == DEFAULT_ICPDIGEST
    ...
    bare = parse_message(
        encode_query(identity=SENTINEL, queryid=DEFAULT_QUERYID, include_queryid=False)
    )
    assert bare["has_queryid"] is False
    assert bare["queryid"] == EMPTY_QUERYID
    advertised = serialize_icp(DEFAULT_QUERY)
    assert advertised == RFC_QUERY_FIELD
    assert parse_icp(advertised) == DEFAULT_QUERY
    assert parse_icp(RFC_HIT_FIELD) == HIT_POLICY
    header = parse_icp_header(encode_icp_header(DEFAULT_QUERY))
    ...
    listed = parse_http_request(hit_request(SENTINEL, DEFAULT_QUERYID, DEFAULT_ICPDIGEST))
    ...
    assert asked["icp_kind"] == "query"
    assert listed["icp_kind"] == "hit"
    assert got["status"] == 200
    assert preload_reply["status"] == 200
    assert got["policy"] == DEFAULT_QUERY
    assert preload_reply["policy"] == HIT_POLICY
    assert canonical_query(SENTINEL, DEFAULT_QUERYID).startswith("QUERY")
    assert "icpdigest=" in canonical_hit(SENTINEL, DEFAULT_QUERYID, DEFAULT_ICPDIGEST)


def test_builtin_proof_seals_icp_actuation() -> None:
    report = builtin_icp_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "icp_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_icp"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_queryid_is_forbidden"]
    assert report["checks"]["skip_query_stays_empty"]
    assert report["checks"]["skip_hit_stays_empty"]
    assert report["checks"]["skip_icpdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_queryid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_icpdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_icp"]
    ...
    assert report["checks"]["leftover_text_binds_icp"]
    assert report["checks"]["proved_icp_consumes_leftover"]
    assert report["mission_goal"] == ICP_ACTUATION_GOAL
    assert report["done_when"] == ICP_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[ICP_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "icp" in capability.tags
    assert "rfc2186" in capability.tags
    assert "http" in capability.tags
    assert "queryid" in capability.tags
    assert "icpdigest" in capability.tags
...
def test_selection_gate_rejects_ledger_only_icp_family(tmp_path: Path) -> None:
    ...
    assert "icp" in family
    assert "rfc2186" in family
    assert "queryid" in family
    assert "icpdigest" in family
