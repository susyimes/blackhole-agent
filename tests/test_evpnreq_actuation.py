...
from blackhole_agent.evpnreq_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_evpnreq_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_evpnreq_header,
    independent_evpnreqdigest,
    parse_http_comm,
    parse_http_response,
    parse_message,
    parse_evpnreq,
    parse_evpnreq_header,
    public_comm,
    public_response,
    run_evpnreq_workflow,
    serialize_evpnreq,
    temporary_comm,
    temporary_response,
)
...
def test_goal_binds_evpnreq_actuation_plane() -> None:
    assert leftover_marker_ids(EVPNREQ_ACTUATION_GOAL) == (EVPNREQ_ACTUATION_ID,)
    ...
    assert EVPNREQ_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(EVPNREQ_LEFTOVER) == (EVPNREQ_ACTUATION_ID,)
    assert leftover_marker_ids(EVPNREQ_ACTUATION_GOAL) == (EVPNREQ_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert EVPNREQ_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(EVPNREQ_ACTUATION_GOAL)
...
def test_opted_in_evpnreq_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("evpnreq",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, EVPNREQ_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["evpnreq"]

    missing = run_evpnreq_workflow(with_evpnreqid=False)
    skip_bind = run_evpnreq_workflow(skip_bind=True)
    skip_temporary = run_evpnreq_workflow(do_temporary=False)
    skip_public = run_evpnreq_workflow(do_public=False)
    skip_evpnreqdigest = run_evpnreq_workflow(do_evpnreqdigest=False)
    skip_replay = run_evpnreq_workflow(replay=False)
    skip_evpnreqid = run_evpnreq_workflow(use_evpnreqid=False)
    live = run_evpnreq_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_evpnreqid"
    ...
    assert skip_evpnreqdigest["ok"] is False
    assert skip_evpnreqdigest["error"] == "evpnreqdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_evpnreqid["ok"] is False
    assert skip_evpnreqid["error"] == "evpnreqid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_evpnreqdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["evpnreqdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["evpnreqid_bound"] is True
    assert row["digest"]
    assert live["evpnreqid"] == DEFAULT_EVPNREQID
    assert live["evpnreqdigest"] == DEFAULT_EVPNREQDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, evpnreqid=DEFAULT_EVPNREQID, evpnreqdigest=DEFAULT_EVPNREQDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["evpnreqid"] == DEFAULT_EVPNREQID
    assert queried["evpnreqdigest"] == DEFAULT_EVPNREQDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == EVPNREQ_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, evpnreqid=DEFAULT_EVPNREQID, evpnreqdigest=DEFAULT_EVPNREQDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["evpnreqid"] == DEFAULT_EVPNREQID
    assert answered["evpnreqdigest"] == DEFAULT_EVPNREQDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, evpnreqid=DEFAULT_EVPNREQID, include_evpnreqid=False)
    )
    assert bare["has_evpnreqid"] is False
    assert bare["evpnreqid"] == EMPTY_EVPNREQID
    publicised = serialize_evpnreq(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_evpnreq(publicised) == DEFAULT_SOURCE
    assert parse_evpnreq(RFC_DEST_FIELD) == DEST_HOP
    header = parse_evpnreq_header(encode_evpnreq_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_EVPNREQID, DEFAULT_EVPNREQDIGEST))
    ...
    assert asked["method"] == "REQ"
    assert asked["disc_kind"] == "req"
    assert listed["disc_kind"] == "aa"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_EVPNREQID).startswith("REQ")
    assert "evpnreqdigest=" in canonical_public(SENTINEL, DEFAULT_EVPNREQID, DEFAULT_EVPNREQDIGEST)


def test_builtin_proof_seals_evpnreq_actuation() -> None:
    report = builtin_evpnreq_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "evpnreq_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_evpnreq"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_evpnreqid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_evpnreqdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_evpnreqid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_evpnreqdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_evpnreq"]
    ...
    assert report["checks"]["leftover_text_binds_evpnreq"]
    assert report["checks"]["proved_evpnreq_consumes_leftover"]
    assert report["mission_goal"] == EVPNREQ_ACTUATION_GOAL
    assert report["done_when"] == EVPNREQ_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[EVPNREQ_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "evpnreq" in capability.tags
    assert "rfc7209" in capability.tags
    assert "http" in capability.tags
    assert "evpnreqid" in capability.tags
    assert "evpnreqdigest" in capability.tags
    assert "evpnreq" in capability.tags
    assert "aa" in capability.tags


def test_selection_gate_rejects_ledger_only_evpnreq_family(tmp_path: Path) -> None:
    ...
    assert "evpnreq" in family.split("/")
    assert "rfc7209" in set(semantic_tokens(EVPNREQ_ACTUATION_GOAL))
    assert "evpnreqid" in set(semantic_tokens(EVPNREQ_ACTUATION_GOAL))
    assert "evpnreqdigest" in family
