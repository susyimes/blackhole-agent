...
from blackhole_agent.httpauth_actuation import (
    ...
    SENTINEL,
    AUTH_HEADER,
    builtin_httpauth_actuation_proof,
    canonical_digest,
    canonical_auth,
    crc32c,
    encode_digest,
    encode_auth,
    encode_httpauth_header,
    independent_httpauth_digest,
    parse_http_request,
    parse_http_response,
    parse_message,
    parse_httpauth,
    parse_httpauth_header,
    digest_request,
    digest_response,
    run_httpauth_workflow,
    serialize_httpauth,
    auth_request,
    auth_response,
)
...
def test_goal_binds_httpauth_actuation_plane() -> None:
    assert leftover_marker_ids(HTTPAUTH_ACTUATION_GOAL) == (HTTPAUTH_ACTUATION_ID,)
    assert leftover_marker_ids(HTTPAUTH_LEFTOVER) == (HTTPAUTH_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert HTTPAUTH_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(HTTPAUTH_ACTUATION_GOAL)
    httpauth_signature = semantic_signature(HTTPAUTH_ACTUATION_GOAL)
    for neighbor in NEIGHBORS:
        assert semantic_similarity(httpauth_signature, semantic_signature(neighbor)) < 0.82


def test_opted_in_httpauth_tool_completes_upgrade_lock_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("httpauth",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, HTTPAUTH_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["httpauth"]

    missing = run_httpauth_workflow(with_nonceid=False)
    skip_bind = run_httpauth_workflow(skip_bind=True)
    skip_auth = run_httpauth_workflow(do_auth=False)
    skip_digest = run_httpauth_workflow(do_digest=False)
    skip_authdigest = run_httpauth_workflow(do_authdigest=False)
    skip_replay = run_httpauth_workflow(replay=False)
    skip_nonceid = run_httpauth_workflow(use_nonceid=False)
    live = run_httpauth_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_nonceid"
    assert skip_bind["ok"] is False
    assert skip_bind["error"] == "not_bound"
    assert skip_auth["ok"] is False
    assert skip_auth["error"] == "auth_required"
    assert skip_digest["ok"] is False
    assert skip_digest["error"] == "digest_required"
    assert skip_authdigest["ok"] is False
    assert skip_authdigest["error"] == "authdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_nonceid["ok"] is False
    assert skip_nonceid["error"] == "nonceid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_httpauth_digest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["auth_frame"] is True
    assert row["digest_frame"] is True
    assert row["authdigest_response"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["nonceid_bound"] is True
    assert row["digest"]
    assert live["nonceid"] == DEFAULT_NONCEID
    assert live["authdigest"] == DEFAULT_AUTHDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_auth(identity=SENTINEL, nonceid=DEFAULT_NONCEID, authdigest=DEFAULT_AUTHDIGEST)
    )
    assert queried["is_auth"] is True and queried["is_response"] is False
    assert queried["identity"] == SENTINEL and queried["nonceid"] == DEFAULT_NONCEID
    assert queried["authdigest"] == DEFAULT_AUTHDIGEST
    assert queried["type"] == FRAME_AUTH
    assert queried["first_byte"] == AUTH_FIRST
    answered = parse_message(
        encode_digest(identity=SENTINEL, nonceid=DEFAULT_NONCEID, authdigest=DEFAULT_AUTHDIGEST)
    )
    assert answered["is_digest"] is True and answered["is_response"] is True
    assert answered["nonceid"] == DEFAULT_NONCEID
    assert answered["authdigest"] == DEFAULT_AUTHDIGEST
    ...
    bare = parse_message(
        encode_auth(identity=SENTINEL, nonceid=DEFAULT_NONCEID, include_nonceid=False)
    )
    assert bare["has_nonceid"] is False
    assert bare["nonceid"] == EMPTY_NONCEID
    advertised = serialize_httpauth(DEFAULT_AUTH)
    assert advertised == RFC_AUTH_FIELD
    assert parse_httpauth(advertised) == DEFAULT_AUTH
    assert parse_httpauth(RFC_DIGEST_FIELD) == DIGEST_POLICY
    header = parse_httpauth_header(encode_httpauth_header(DEFAULT_AUTH))
    assert header["field_value"] == RFC_AUTH_FIELD
    assert header["header"] == AUTH_HEADER
    asked = parse_http_request(auth_request(SENTINEL, DEFAULT_NONCEID))
    listed = parse_http_request(digest_request(SENTINEL, DEFAULT_NONCEID, DEFAULT_AUTHDIGEST))
    ...
    assert asked["method"] == "AUTH"
    assert asked["httpauth_kind"] == "auth"
    assert listed["httpauth_kind"] == "digest"
    assert got["status"] == 200
    assert preload_reply["status"] == 200
    assert got["policy"] == DEFAULT_AUTH
    assert preload_reply["policy"] == DIGEST_POLICY
    assert canonical_auth(SENTINEL, DEFAULT_NONCEID).startswith("AUTH")
    assert "authdigest=" in canonical_digest(SENTINEL, DEFAULT_NONCEID, DEFAULT_AUTHDIGEST)


def test_builtin_proof_seals_httpauth_actuation() -> None:
    report = builtin_httpauth_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "httpauth_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_httpauth"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_nonceid_is_forbidden"]
    assert report["checks"]["skip_auth_stays_empty"]
    assert report["checks"]["skip_digest_stays_empty"]
    assert report["checks"]["skip_authdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_nonceid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_authdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_httpauth"]
    ...
    assert report["checks"]["leftover_text_binds_httpauth"]
    assert report["checks"]["proved_httpauth_consumes_leftover"]
    assert report["mission_goal"] == HTTPAUTH_ACTUATION_GOAL
    assert report["done_when"] == HTTPAUTH_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[HTTPAUTH_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "httpauth" in capability.tags
    assert "rfc2617" in capability.tags
    assert "http" in capability.tags
    assert "nonceid" in capability.tags
    assert "authdigest" in capability.tags
    assert "auth" in capability.tags


def test_selection_gate_rejects_ledger_only_httpauth_family(tmp_path: Path) -> None:
    ...
    assert "httpauth" in family
    assert "nonceid" in family
    assert "authdigest" in family
