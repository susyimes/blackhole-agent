...
from blackhole_agent.ldpsig_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_ldpsig_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_ldpsig_header,
    independent_ldpsigdigest,
    parse_http_comm,
    parse_http_response,
    parse_message,
    parse_ldpsig,
...
def test_goal_binds_ldpsig_actuation_plane() -> None:
    assert leftover_marker_ids(LDPSIG_ACTUATION_GOAL) == (LDPSIG_ACTUATION_ID,)
    ...
    assert LDPSIG_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(LDPSIG_LEFTOVER) == (LDPSIG_ACTUATION_ID,)
    assert leftover_marker_ids(LDPSIG_ACTUATION_GOAL) == (LDPSIG_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert LDPSIG_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(LDPSIG_ACTUATION_GOAL)
...
def test_opted_in_ldpsig_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("ldpsig",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, LDPSIG_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["ldpsig"]

    missing = run_ldpsig_workflow(with_ldpsigid=False)
    skip_bind = run_ldpsig_workflow(skip_bind=True)
    skip_temporary = run_ldpsig_workflow(do_temporary=False)
    skip_public = run_ldpsig_workflow(do_public=False)
    skip_ldpsigdigest = run_ldpsig_workflow(do_ldpsigdigest=False)
    skip_replay = run_ldpsig_workflow(replay=False)
    skip_ldpsigid = run_ldpsig_workflow(use_ldpsigid=False)
    live = run_ldpsig_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_ldpsigid"
    ...
    assert skip_ldpsigdigest["ok"] is False
    assert skip_ldpsigdigest["error"] == "ldpsigdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_ldpsigid["ok"] is False
    assert skip_ldpsigid["error"] == "ldpsigid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_ldpsigdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["ldpsigdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["ldpsigid_bound"] is True
    assert row["digest"]
    assert live["ldpsigid"] == DEFAULT_LDPSIGID
    assert live["ldpsigdigest"] == DEFAULT_LDPSIGDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, ldpsigid=DEFAULT_LDPSIGID, ldpsigdigest=DEFAULT_LDPSIGDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["ldpsigid"] == DEFAULT_LDPSIGID
    assert queried["ldpsigdigest"] == DEFAULT_LDPSIGDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == LDPSIG_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, ldpsigid=DEFAULT_LDPSIGID, ldpsigdigest=DEFAULT_LDPSIGDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["ldpsigid"] == DEFAULT_LDPSIGID
    assert answered["ldpsigdigest"] == DEFAULT_LDPSIGDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, ldpsigid=DEFAULT_LDPSIGID, include_ldpsigid=False)
    )
    assert bare["has_ldpsigid"] is False
    assert bare["ldpsigid"] == EMPTY_LDPSIGID
    publicised = serialize_ldpsig(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_ldpsig(publicised) == DEFAULT_SOURCE
    assert parse_ldpsig(RFC_DEST_FIELD) == DEST_HOP
    header = parse_ldpsig_header(encode_ldpsig_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_LDPSIGID, DEFAULT_LDPSIGDIGEST))
    ...
    assert asked["method"] == "PW"
    assert asked["disc_kind"] == "pw"
    assert listed["disc_kind"] == "fec"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_LDPSIGID).startswith("PW")
    assert "ldpsigdigest=" in canonical_public(SENTINEL, DEFAULT_LDPSIGID, DEFAULT_LDPSIGDIGEST)


def test_builtin_proof_seals_ldpsig_actuation() -> None:
    report = builtin_ldpsig_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "ldpsig_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_ldpsig"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_ldpsigid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_ldpsigdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_ldpsigid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_ldpsigdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_ldpsig"]
    ...
    assert report["checks"]["leftover_text_binds_ldpsig"]
    assert report["checks"]["proved_ldpsig_consumes_leftover"]
    assert report["mission_goal"] == LDPSIG_ACTUATION_GOAL
    assert report["done_when"] == LDPSIG_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[LDPSIG_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "ldpsig" in capability.tags
    assert "rfc4762" in capability.tags
    assert "http" in capability.tags
    assert "ldpsigid" in capability.tags
    assert "ldpsigdigest" in capability.tags
    assert "ldpsig" in capability.tags
    assert "fec" in capability.tags


def test_selection_gate_rejects_ledger_only_ldpsig_family(tmp_path: Path) -> None:
    ...
    assert "ldpsig" in family.split("/")
    assert "rfc4762" in set(semantic_tokens(LDPSIG_ACTUATION_GOAL))
    assert "ldpsigid" in set(semantic_tokens(LDPSIG_ACTUATION_GOAL))
    assert "ldpsigdigest" in family
