...
def test_goal_binds_dsct_actuation_plane() -> None:
    assert leftover_marker_ids(DSCT_ACTUATION_GOAL) == (DSCT_ACTUATION_ID,)
    ...
    assert DSCT_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(DSCT_LEFTOVER) == (DSCT_ACTUATION_ID,)
    assert leftover_marker_ids(DSCT_ACTUATION_GOAL) == (DSCT_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert DSCT_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(DSCT_ACTUATION_GOAL)
...
def test_opted_in_dsct_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("dsct",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, DSCT_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["dsct"]

    missing = run_dsct_workflow(with_dsctid=False)
    skip_bind = run_dsct_workflow(skip_bind=True)
    skip_temporary = run_dsct_workflow(do_temporary=False)
    skip_public = run_dsct_workflow(do_public=False)
    skip_dsctdigest = run_dsct_workflow(do_dsctdigest=False)
    skip_replay = run_dsct_workflow(replay=False)
    skip_dsctid = run_dsct_workflow(use_dsctid=False)
    live = run_dsct_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_dsctid"
    ...
    assert skip_dsctdigest["ok"] is False
    assert skip_dsctdigest["error"] == "dsctdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_dsctid["ok"] is False
    assert skip_dsctid["error"] == "dsctid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_dsctdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["dsctdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["dsctid_bound"] is True
    assert row["digest"]
    assert live["dsctid"] == DEFAULT_DSCTID
    assert live["dsctdigest"] == DEFAULT_DSCTDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, dsctid=DEFAULT_DSCTID, dsctdigest=DEFAULT_DSCTDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["dsctid"] == DEFAULT_DSCTID
    assert queried["dsctdigest"] == DEFAULT_DSCTDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == DSCT_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, dsctid=DEFAULT_DSCTID, dsctdigest=DEFAULT_DSCTDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["dsctid"] == DEFAULT_DSCTID
    assert answered["dsctdigest"] == DEFAULT_DSCTDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, dsctid=DEFAULT_DSCTID, include_dsctid=False)
    )
    assert bare["has_dsctid"] is False
    assert bare["dsctid"] == EMPTY_DSCTID
    publicised = serialize_dsct(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_dsct(publicised) == DEFAULT_SOURCE
    assert parse_dsct(RFC_DEST_FIELD) == DEST_HOP
    header = parse_dsct_header(encode_dsct_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_DSCTID, DEFAULT_DSCTDIGEST))
    ...
    assert asked["method"] == "CLASS"
    assert asked["disc_kind"] == "class"
    assert listed["disc_kind"] == "type"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_DSCTID).startswith("CLASS")
    assert "dsctdigest=" in canonical_public(SENTINEL, DEFAULT_DSCTID, DEFAULT_DSCTDIGEST)


def test_builtin_proof_seals_dsct_actuation() -> None:
    report = builtin_dsct_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "dsct_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_dsct"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_dsctid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_dsctdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_dsctid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_dsctdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_dsct"]
    ...
    assert report["checks"]["leftover_text_binds_dsct"]
    assert report["checks"]["proved_dsct_consumes_leftover"]
    assert report["mission_goal"] == DSCT_ACTUATION_GOAL
    assert report["done_when"] == DSCT_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[DSCT_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "dsct" in capability.tags
    assert "rfc5455" in capability.tags
    assert "http" in capability.tags
    assert "dsctid" in capability.tags
    assert "dsctdigest" in capability.tags
    assert "class" in capability.tags
    assert "type" in capability.tags


def test_selection_gate_rejects_ledger_only_dsct_family(tmp_path: Path) -> None:
    ...
    assert "dsct" in family.split("/")
    assert "rfc5455" in set(semantic_tokens(DSCT_ACTUATION_GOAL))
    assert "dsctid" in set(semantic_tokens(DSCT_ACTUATION_GOAL))
    assert "dsctdigest" in family
