...
def test_goal_binds_slaac_actuation_plane() -> None:
    assert leftover_marker_ids(SLAAC_ACTUATION_GOAL) == (SLAAC_ACTUATION_ID,)
    assert leftover_marker_ids(SLAAC_LEFTOVER) == (SLAAC_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert SLAAC_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(SLAAC_ACTUATION_GOAL)
...
def test_opted_in_slaac_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("slaac",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, SLAAC_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["slaac"]

    missing = run_slaac_workflow(with_slaacid=False)
    skip_bind = run_slaac_workflow(skip_bind=True)
    skip_router = run_slaac_workflow(do_router=False)
    skip_prefix = run_slaac_workflow(do_prefix=False)
    skip_slaacdigest = run_slaac_workflow(do_slaacdigest=False)
    skip_replay = run_slaac_workflow(replay=False)
    skip_slaacid = run_slaac_workflow(use_slaacid=False)
    live = run_slaac_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_slaacid"
    assert skip_bind["ok"] is False
    assert skip_bind["error"] == "not_bound"
    assert skip_router["ok"] is False
    assert skip_router["error"] == "router_required"
    assert skip_prefix["ok"] is False
    assert skip_prefix["error"] == "prefix_required"
    assert skip_slaacdigest["ok"] is False
    assert skip_slaacdigest["error"] == "slaacdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_slaacid["ok"] is False
    assert skip_slaacid["error"] == "slaacid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_slaacdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["router_frame"] is True
    assert row["prefix_frame"] is True
    assert row["slaacdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["slaacid_bound"] is True
    assert row["digest"]
    assert live["slaacid"] == DEFAULT_SLAACID
    assert live["slaacdigest"] == DEFAULT_SLAACDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_router(identity=SENTINEL, slaacid=DEFAULT_SLAACID, slaacdigest=DEFAULT_SLAACDIGEST)
    )
    assert queried["is_router"] is True and queried["is_prefix"] is False
    assert queried["identity"] == SENTINEL and queried["slaacid"] == DEFAULT_SLAACID
    assert queried["slaacdigest"] == DEFAULT_SLAACDIGEST
    assert queried["type"] == FRAME_ROUTER
    assert queried["first_byte"] == SLAAC_FIRST
    answered = parse_message(
        encode_prefix(identity=SENTINEL, slaacid=DEFAULT_SLAACID, slaacdigest=DEFAULT_SLAACDIGEST)
    )
    assert answered["is_prefix"] is True and answered["is_prefix"] is True
    assert answered["slaacid"] == DEFAULT_SLAACID
    assert answered["slaacdigest"] == DEFAULT_SLAACDIGEST
    ...
    bare = parse_message(
        encode_router(identity=SENTINEL, slaacid=DEFAULT_SLAACID, include_slaacid=False)
    )
    assert bare["has_slaacid"] is False
    assert bare["slaacid"] == EMPTY_SLAACID
    prefixised = serialize_slaac(DEFAULT_ROUTER)
    assert prefixised == RFC_ROUTER_FIELD
    assert parse_slaac(prefixised) == DEFAULT_ROUTER
    assert parse_slaac(RFC_PREFIX_FIELD) == PREFIX_POLICY
    header = parse_slaac_header(encode_slaac_header(DEFAULT_ROUTER))
    assert header["field_value"] == RFC_ROUTER_FIELD
    assert header["header"] == ROUTER_HEADER
    asked = parse_http_request(router_request(SENTINEL, DEFAULT_SLAACID))
    listed = parse_http_request(prefix_request(SENTINEL, DEFAULT_SLAACID, DEFAULT_SLAACDIGEST))
    ...
    assert asked["method"] == "ROUTER"
    assert asked["slaac_kind"] == "router"
    assert listed["slaac_kind"] == "prefix"
    assert got["status"] == 200
    assert preload_prefix["status"] == 200
    assert got["policy"] == DEFAULT_ROUTER
    assert preload_prefix["policy"] == PREFIX_POLICY
    assert canonical_router(SENTINEL, DEFAULT_SLAACID).startswith("ROUTER")
    assert "slaacdigest=" in canonical_prefix(SENTINEL, DEFAULT_SLAACID, DEFAULT_SLAACDIGEST)


def test_builtin_proof_seals_slaac_actuation() -> None:
    report = builtin_slaac_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "slaac_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_slaac"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_slaacid_is_forbidden"]
    assert report["checks"]["skip_router_stays_empty"]
    assert report["checks"]["skip_prefix_stays_empty"]
    assert report["checks"]["skip_slaacdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_slaacid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_slaacdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_slaac"]
    assert report["checks"]["exhausted_catalog_stays_unbound_without_gate_passing_successor"]
    assert report["checks"]["catalog_names_slaac"]
    assert report["checks"]["catalog_names_tempaddr"]
    assert report["checks"]["leftover_text_binds_slaac"]
    assert report["checks"]["proved_slaac_consumes_leftover"]
    assert report["mission_goal"] == SLAAC_ACTUATION_GOAL
    assert report["done_when"] == SLAAC_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[SLAAC_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "slaac" in capability.tags
    assert "rfc4862" in capability.tags
    assert "http" in capability.tags
    assert "slaacid" in capability.tags
    assert "slaacdigest" in capability.tags
    assert "router" in capability.tags


def test_selection_gate_rejects_ledger_only_slaac_family(tmp_path: Path) -> None:
    ...
    assert "slaac" in family.split("/")
    assert "rfc4862" in family
    assert "slaacid" in family
    assert "slaacdigest" in family
