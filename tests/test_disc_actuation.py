...
def test_goal_binds_disc_actuation_plane() -> None:
    ...
    assert leftover_marker_ids(DISC_LEFTOVER) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DSLITE_ACTUATION_GOAL) == (DSLITE_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    assert leftover_marker_ids(RDNSS_ACTUATION_GOAL) == (RDNSS_ACTUATION_ID,)
    ...
    assert DISC_ACTUATION_ID in LOCAL_DENYLIST
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert DISC_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(DISC_ACTUATION_GOAL)
...
def test_opted_in_disc_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("disc",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, DISC_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["disc"]

    missing = run_disc_workflow(with_discid=False)
    skip_bind = run_disc_workflow(skip_bind=True)
    skip_temporary = run_disc_workflow(do_temporary=False)
    skip_public = run_disc_workflow(do_public=False)
    skip_discdigest = run_disc_workflow(do_discdigest=False)
    skip_replay = run_disc_workflow(replay=False)
    skip_discid = run_disc_workflow(use_discid=False)
    live = run_disc_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_discid"
    ...
    assert skip_discdigest["ok"] is False
    assert skip_discdigest["error"] == "discdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_discid["ok"] is False
    assert skip_discid["error"] == "discid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_discdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["discdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["discid_bound"] is True
    assert row["digest"]
    assert live["discid"] == DEFAULT_DISCID
    assert live["discdigest"] == DEFAULT_DISCDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, discid=DEFAULT_DISCID, discdigest=DEFAULT_DISCDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["discid"] == DEFAULT_DISCID
    assert queried["discdigest"] == DEFAULT_DISCDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == DISC_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, discid=DEFAULT_DISCID, discdigest=DEFAULT_DISCDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["discid"] == DEFAULT_DISCID
    assert answered["discdigest"] == DEFAULT_DISCDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, discid=DEFAULT_DISCID, include_discid=False)
    )
    assert bare["has_discid"] is False
    assert bare["discid"] == EMPTY_DISCID
    publicised = serialize_disc(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_disc(publicised) == DEFAULT_SOURCE
    assert parse_disc(RFC_DEST_FIELD) == DEST_HOP
    header = parse_disc_header(encode_disc_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_DISCID, DEFAULT_DISCDIGEST))
    ...
    assert asked["method"] == "IPV4ONLY"
    assert asked["disc_kind"] == "ipv4only"
    assert listed["disc_kind"] == "aaaa"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_DISCID).startswith("IPV4ONLY")
    assert "discdigest=" in canonical_public(SENTINEL, DEFAULT_DISCID, DEFAULT_DISCDIGEST)


def test_builtin_proof_seals_disc_actuation() -> None:
    report = builtin_disc_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "disc_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_disc"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_discid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_discdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_discid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_discdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_disc"]
    ...
    assert report["checks"]["leftover_text_binds_disc"]
    assert report["checks"]["proved_disc_consumes_leftover"]
    assert report["mission_goal"] == DISC_ACTUATION_GOAL
    assert report["done_when"] == DISC_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[DISC_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "disc" in capability.tags
    assert "rfc7050" in capability.tags
    assert "http" in capability.tags
    assert "discid" in capability.tags
    assert "discdigest" in capability.tags
    assert "ipv4only" in capability.tags
    assert "aaaa" in capability.tags


def test_selection_gate_rejects_ledger_only_disc_family(tmp_path: Path) -> None:
    ...
    assert "disc" in family.split("/")
    assert "discid" in family
    assert "discdigest" in family
