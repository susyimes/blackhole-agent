...
def test_goal_binds_tsp_actuation_plane() -> None:
    assert leftover_marker_ids(TSP_ACTUATION_GOAL) == (TSP_ACTUATION_ID,)
    ...
    assert TSP_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(TSP_LEFTOVER) == (TSP_ACTUATION_ID,)
    assert leftover_marker_ids(TSP_ACTUATION_GOAL) == (TSP_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert TSP_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(TSP_ACTUATION_GOAL)
...
def test_opted_in_tsp_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("tsp",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, TSP_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["tsp"]

    missing = run_tsp_workflow(with_tspid=False)
    skip_bind = run_tsp_workflow(skip_bind=True)
    skip_temporary = run_tsp_workflow(do_temporary=False)
    skip_public = run_tsp_workflow(do_public=False)
    skip_tspdigest = run_tsp_workflow(do_tspdigest=False)
    skip_replay = run_tsp_workflow(replay=False)
    skip_tspid = run_tsp_workflow(use_tspid=False)
    live = run_tsp_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_tspid"
    ...
    assert skip_tspdigest["ok"] is False
    assert skip_tspdigest["error"] == "tspdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_tspid["ok"] is False
    assert skip_tspid["error"] == "tspid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_tspdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["tspdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["tspid_bound"] is True
    assert row["digest"]
    assert live["tspid"] == DEFAULT_TSPID
    assert live["tspdigest"] == DEFAULT_TSPDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, tspid=DEFAULT_TSPID, tspdigest=DEFAULT_TSPDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["tspid"] == DEFAULT_TSPID
    assert queried["tspdigest"] == DEFAULT_TSPDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == TSP_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, tspid=DEFAULT_TSPID, tspdigest=DEFAULT_TSPDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["tspid"] == DEFAULT_TSPID
    assert answered["tspdigest"] == DEFAULT_TSPDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, tspid=DEFAULT_TSPID, include_tspid=False)
    )
    assert bare["has_tspid"] is False
    assert bare["tspid"] == EMPTY_TSPID
    publicised = serialize_tsp(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_tsp(publicised) == DEFAULT_SOURCE
    assert parse_tsp(RFC_DEST_FIELD) == DEST_HOP
    header = parse_tsp_header(encode_tsp_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_TSPID, DEFAULT_TSPDIGEST))
    ...
    assert asked["method"] == "TSP"
    assert asked["disc_kind"] == "tsp"
    assert listed["disc_kind"] == "setup"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_TSPID).startswith("TSP")
    assert "tspdigest=" in canonical_public(SENTINEL, DEFAULT_TSPID, DEFAULT_TSPDIGEST)


def test_builtin_proof_seals_tsp_actuation() -> None:
    report = builtin_tsp_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "tsp_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_tsp"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_tspid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_tspdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_tspid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_tspdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_tsp"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_tsp"]
    assert report["checks"]["proved_tsp_consumes_leftover"]
    assert report["mission_goal"] == TSP_ACTUATION_GOAL
    assert report["done_when"] == TSP_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[TSP_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "tsp" in capability.tags
    assert "rfc5572" in capability.tags
    assert "http" in capability.tags
    assert "tspid" in capability.tags
    assert "tspdigest" in capability.tags
    assert "tsp" in capability.tags
...
def test_selection_gate_rejects_ledger_only_tsp_family(tmp_path: Path) -> None:
    ...
    assert "tsp" in family.split("/")
    assert "rfc5572" in family
    assert "tspid" in set(semantic_tokens(TSP_ACTUATION_GOAL))
    assert "tspdigest" in family
