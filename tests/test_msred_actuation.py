...
def test_goal_binds_msred_actuation_plane() -> None:
    assert leftover_marker_ids(MSRED_ACTUATION_GOAL) == (MSRED_ACTUATION_ID,)
    ...
    assert MSRED_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(MSRED_LEFTOVER) == (MSRED_ACTUATION_ID,)
    assert leftover_marker_ids(MSRED_ACTUATION_GOAL) == (MSRED_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert MSRED_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(MSRED_ACTUATION_GOAL)
...
def test_opted_in_msred_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("msred",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, MSRED_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["msred"]

    missing = run_msred_workflow(with_msredid=False)
    skip_bind = run_msred_workflow(skip_bind=True)
    skip_temporary = run_msred_workflow(do_temporary=False)
    skip_public = run_msred_workflow(do_public=False)
    skip_msreddigest = run_msred_workflow(do_msreddigest=False)
    skip_replay = run_msred_workflow(replay=False)
    skip_msredid = run_msred_workflow(use_msredid=False)
    live = run_msred_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_msredid"
    ...
    assert skip_msreddigest["ok"] is False
    assert skip_msreddigest["error"] == "msreddigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_msredid["ok"] is False
    assert skip_msredid["error"] == "msredid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_msreddigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["msreddigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["msredid_bound"] is True
    assert row["digest"]
    assert live["msredid"] == DEFAULT_MSREDID
    assert live["msreddigest"] == DEFAULT_MSREDDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, msredid=DEFAULT_MSREDID, msreddigest=DEFAULT_MSREDDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["msredid"] == DEFAULT_MSREDID
    assert queried["msreddigest"] == DEFAULT_MSREDDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == MSRED_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, msredid=DEFAULT_MSREDID, msreddigest=DEFAULT_MSREDDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["msredid"] == DEFAULT_MSREDID
    assert answered["msreddigest"] == DEFAULT_MSREDDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, msredid=DEFAULT_MSREDID, include_msredid=False)
    )
    assert bare["has_msredid"] is False
    assert bare["msredid"] == EMPTY_MSREDID
    publicised = serialize_msred(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_msred(publicised) == DEFAULT_SOURCE
    assert parse_msred(RFC_DEST_FIELD) == DEST_HOP
    header = parse_msred_header(encode_msred_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_MSREDID, DEFAULT_MSREDDIGEST))
    ...
    assert asked["method"] == "WARM"
    assert asked["disc_kind"] == "msred"
    assert listed["disc_kind"] == "hot"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_MSREDID).startswith("WARM")
    assert "msreddigest=" in canonical_public(SENTINEL, DEFAULT_MSREDID, DEFAULT_MSREDDIGEST)


def test_builtin_proof_seals_msred_actuation() -> None:
    report = builtin_msred_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "msred_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_msred"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_msredid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_msreddigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_msredid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_msreddigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_msred"]
    ...
    assert report["checks"]["leftover_text_binds_msred"]
    assert report["checks"]["proved_msred_consumes_leftover"]
    assert report["mission_goal"] == MSRED_ACTUATION_GOAL
    assert report["done_when"] == MSRED_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[MSRED_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "msred" in capability.tags
    assert "rfc9856" in capability.tags
    assert "http" in capability.tags
    assert "msredid" in capability.tags
    assert "msreddigest" in capability.tags
    assert "msred" in capability.tags
    assert "hot" in capability.tags


def test_selection_gate_rejects_ledger_only_msred_family(tmp_path: Path) -> None:
    ...
    assert "msred" in family.split("/")
    assert "rfc9856" in set(semantic_tokens(MSRED_ACTUATION_GOAL))
    assert "msredid" in set(semantic_tokens(MSRED_ACTUATION_GOAL))
    assert "msreddigest" in family
