...
def test_goal_binds_lilb_actuation_plane() -> None:
    assert leftover_marker_ids(LILB_ACTUATION_GOAL) == (LILB_ACTUATION_ID,)
    ...
    assert LILB_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(LILB_LEFTOVER) == (LILB_ACTUATION_ID,)
    assert leftover_marker_ids(LILB_ACTUATION_GOAL) == (LILB_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert LILB_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(LILB_ACTUATION_GOAL)
...
def test_opted_in_lilb_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("lilb",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, LILB_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["lilb"]

    missing = run_lilb_workflow(with_lilbid=False)
    skip_bind = run_lilb_workflow(skip_bind=True)
    skip_temporary = run_lilb_workflow(do_temporary=False)
    skip_public = run_lilb_workflow(do_public=False)
    skip_lilbdigest = run_lilb_workflow(do_lilbdigest=False)
    skip_replay = run_lilb_workflow(replay=False)
    skip_lilbid = run_lilb_workflow(use_lilbid=False)
    live = run_lilb_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_lilbid"
    ...
    assert skip_lilbdigest["ok"] is False
    assert skip_lilbdigest["error"] == "lilbdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_lilbid["ok"] is False
    assert skip_lilbid["error"] == "lilbid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_lilbdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["lilbdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["lilbid_bound"] is True
    assert row["digest"]
    assert live["lilbid"] == DEFAULT_LILBID
    assert live["lilbdigest"] == DEFAULT_LILBDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, lilbid=DEFAULT_LILBID, lilbdigest=DEFAULT_LILBDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["lilbid"] == DEFAULT_LILBID
    assert queried["lilbdigest"] == DEFAULT_LILBDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == LILB_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, lilbid=DEFAULT_LILBID, lilbdigest=DEFAULT_LILBDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["lilbid"] == DEFAULT_LILBID
    assert answered["lilbdigest"] == DEFAULT_LILBDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, lilbid=DEFAULT_LILBID, include_lilbid=False)
    )
    assert bare["has_lilbid"] is False
    assert bare["lilbid"] == EMPTY_LILBID
    publicised = serialize_lilb(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_lilb(publicised) == DEFAULT_SOURCE
    assert parse_lilb(RFC_DEST_FIELD) == DEST_HOP
    header = parse_lilb_header(encode_lilb_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_LILBID, DEFAULT_LILBDIGEST))
    ...
    assert asked["method"] == "LI"
    assert asked["disc_kind"] == "li"
    assert listed["disc_kind"] == "lb"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_LILBID).startswith("LI")
    assert "lilbdigest=" in canonical_public(SENTINEL, DEFAULT_LILBID, DEFAULT_LILBDIGEST)


def test_builtin_proof_seals_lilb_actuation() -> None:
    report = builtin_lilb_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "lilb_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_lilb"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_lilbid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_lilbdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_lilbid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_lilbdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_binds_lilb"]
    ...
    assert report["checks"]["catalog_names_wson"]
    assert report["checks"]["leftover_text_binds_lilb"]
    assert report["checks"]["proved_lilb_consumes_leftover"]
    assert report["mission_goal"] == LILB_ACTUATION_GOAL
    assert report["done_when"] == LILB_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[LILB_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "lilb" in capability.tags
    assert "rfc6435" in capability.tags
    assert "http" in capability.tags
    assert "lilbid" in capability.tags
    assert "lilbdigest" in capability.tags
    assert "li" in capability.tags
    assert "lb" in capability.tags


def test_selection_gate_rejects_ledger_only_lilb_family(tmp_path: Path) -> None:
    ...
    assert "lilb" in family.split("/")
    assert "rfc6435" in set(semantic_tokens(LILB_ACTUATION_GOAL))
    assert "lilbid" in set(semantic_tokens(LILB_ACTUATION_GOAL))
    assert "lilbdigest" in family
    ...
    assert "pwst" not in family.split("/")
    assert "rfc6478" not in family
    assert "pwstid" not in family
    assert "pwstdigest" not in family
