...
def test_goal_binds_nntp_actuation_plane() -> None:
    assert leftover_marker_ids(NNTP_ACTUATION_GOAL) == (NNTP_ACTUATION_ID,)
    assert leftover_marker_ids(NNTP_LEFTOVER) == (NNTP_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert NNTP_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(NNTP_ACTUATION_GOAL)
...
def test_opted_in_nntp_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("nntp",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, NNTP_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["nntp"]

    missing = run_nntp_workflow(with_nntpid=False)
    skip_bind = run_nntp_workflow(skip_bind=True)
    skip_article = run_nntp_workflow(do_article=False)
    skip_group = run_nntp_workflow(do_group=False)
    skip_nntpdigest = run_nntp_workflow(do_nntpdigest=False)
    skip_replay = run_nntp_workflow(replay=False)
    skip_nntpid = run_nntp_workflow(use_nntpid=False)
    live = run_nntp_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_nntpid"
    assert skip_bind["ok"] is False
    assert skip_bind["error"] == "not_bound"
    assert skip_article["ok"] is False
    assert skip_article["error"] == "article_required"
    assert skip_group["ok"] is False
    assert skip_group["error"] == "group_required"
    assert skip_nntpdigest["ok"] is False
    assert skip_nntpdigest["error"] == "nntpdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_nntpid["ok"] is False
    assert skip_nntpid["error"] == "nntpid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_nntpdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["article_frame"] is True
    assert row["group_frame"] is True
    assert row["nntpdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["nntpid_bound"] is True
    assert row["digest"]
    assert live["nntpid"] == DEFAULT_NNTPID
    assert live["nntpdigest"] == DEFAULT_NNTPDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_article(identity=SENTINEL, nntpid=DEFAULT_NNTPID, nntpdigest=DEFAULT_NNTPDIGEST)
    )
    assert queried["is_article"] is True and queried["is_group"] is False
    assert queried["identity"] == SENTINEL and queried["nntpid"] == DEFAULT_NNTPID
    assert queried["nntpdigest"] == DEFAULT_NNTPDIGEST
    assert queried["type"] == FRAME_ARTICLE
    assert queried["first_byte"] == NNTP_FIRST
    answered = parse_message(
        encode_group(identity=SENTINEL, nntpid=DEFAULT_NNTPID, nntpdigest=DEFAULT_NNTPDIGEST)
    )
    assert answered["is_group"] is True and answered["is_group"] is True
    assert answered["nntpid"] == DEFAULT_NNTPID
    assert answered["nntpdigest"] == DEFAULT_NNTPDIGEST
    ...
    bare = parse_message(
        encode_article(identity=SENTINEL, nntpid=DEFAULT_NNTPID, include_nntpid=False)
    )
    assert bare["has_nntpid"] is False
    assert bare["nntpid"] == EMPTY_NNTPID
    advertised = serialize_nntp(DEFAULT_ARTICLE)
    assert advertised == RFC_ARTICLE_FIELD
    assert parse_nntp(advertised) == DEFAULT_ARTICLE
    assert parse_nntp(RFC_GROUP_FIELD) == GROUP_POLICY
    header = parse_nntp_header(encode_nntp_header(DEFAULT_ARTICLE))
    assert header["field_value"] == RFC_ARTICLE_FIELD
    assert header["header"] == ARTICLE_HEADER
    asked = parse_http_request(article_request(SENTINEL, DEFAULT_NNTPID))
    listed = parse_http_request(group_request(SENTINEL, DEFAULT_NNTPID, DEFAULT_NNTPDIGEST))
    ...
    assert asked["method"] == "ARTICLE"
    assert asked["nntp_kind"] == "article"
    assert listed["nntp_kind"] == "group"
    assert got["status"] == 200
    assert preload_reply["status"] == 200
    assert got["policy"] == DEFAULT_ARTICLE
    assert preload_reply["policy"] == GROUP_POLICY
    assert canonical_article(SENTINEL, DEFAULT_NNTPID).startswith("ARTICLE")
    assert "nntpdigest=" in canonical_group(SENTINEL, DEFAULT_NNTPID, DEFAULT_NNTPDIGEST)


def test_builtin_proof_seals_nntp_actuation() -> None:
    report = builtin_nntp_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "nntp_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_nntp"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_nntpid_is_forbidden"]
    assert report["checks"]["skip_article_stays_empty"]
    assert report["checks"]["skip_group_stays_empty"]
    assert report["checks"]["skip_nntpdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_nntpid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_nntpdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_nntp"]
    assert report["checks"]["exhausted_catalog_stays_unbound_without_gate_passing_successor"]
    assert report["checks"]["catalog_names_nntp"]
    assert report["checks"]["catalog_names_telnet"]
    assert report["checks"]["leftover_text_binds_nntp"]
    assert report["checks"]["proved_nntp_consumes_leftover"]
    assert report["mission_goal"] == NNTP_ACTUATION_GOAL
    assert report["done_when"] == NNTP_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[NNTP_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "nntp" in capability.tags
    assert "rfc977" in capability.tags
    assert "http" in capability.tags
    assert "nntpid" in capability.tags
    assert "nntpdigest" in capability.tags
    assert "article" in capability.tags


def test_selection_gate_rejects_ledger_only_nntp_family(tmp_path: Path) -> None:
    ...
    assert "nntp" in family
    assert "rfc977" in family
    assert "nntpid" in family
    assert "nntpdigest" in family
