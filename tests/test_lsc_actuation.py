...
def test_goal_binds_lsc_actuation_plane() -> None:
    assert leftover_marker_ids(LSC_ACTUATION_GOAL) == (LSC_ACTUATION_ID,)
    ...
    assert LSC_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(LSC_LEFTOVER) == (LSC_ACTUATION_ID,)
    assert leftover_marker_ids(LSC_ACTUATION_GOAL) == (LSC_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert LSC_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(LSC_ACTUATION_GOAL)
...
def test_opted_in_lsc_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("lsc",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, LSC_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["lsc"]

    missing = run_lsc_workflow(with_lscid=False)
    skip_bind = run_lsc_workflow(skip_bind=True)
    skip_temporary = run_lsc_workflow(do_temporary=False)
    skip_public = run_lsc_workflow(do_public=False)
    skip_lscdigest = run_lsc_workflow(do_lscdigest=False)
    skip_replay = run_lsc_workflow(replay=False)
    skip_lscid = run_lsc_workflow(use_lscid=False)
    live = run_lsc_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_lscid"
    ...
    assert skip_lscdigest["ok"] is False
    assert skip_lscdigest["error"] == "lscdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_lscid["ok"] is False
    assert skip_lscid["error"] == "lscid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_lscdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["lscdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["lscid_bound"] is True
    assert row["digest"]
    assert live["lscid"] == DEFAULT_LSCID
    assert live["lscdigest"] == DEFAULT_LSCDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, lscid=DEFAULT_LSCID, lscdigest=DEFAULT_LSCDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["lscid"] == DEFAULT_LSCID
    assert queried["lscdigest"] == DEFAULT_LSCDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == LSC_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, lscid=DEFAULT_LSCID, lscdigest=DEFAULT_LSCDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["lscid"] == DEFAULT_LSCID
    assert answered["lscdigest"] == DEFAULT_LSCDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, lscid=DEFAULT_LSCID, include_lscid=False)
    )
    assert bare["has_lscid"] is False
    assert bare["lscid"] == EMPTY_LSCID
    publicised = serialize_lsc(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_lsc(publicised) == DEFAULT_SOURCE
    assert parse_lsc(RFC_DEST_FIELD) == DEST_HOP
    header = parse_lsc_header(encode_lsc_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_LSCID, DEFAULT_LSCDIGEST))
    ...
    assert asked["method"] == "LSC"
    assert asked["disc_kind"] == "lsc"
    assert listed["disc_kind"] == "label"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_LSCID).startswith("LSC")
    assert "lscdigest=" in canonical_public(SENTINEL, DEFAULT_LSCID, DEFAULT_LSCDIGEST)


def test_builtin_proof_seals_lsc_actuation() -> None:
    report = builtin_lsc_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "lsc_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_lsc"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_lscid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_lscdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_lscid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_lscdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_binds_lsc"]
    ...
    assert report["checks"]["catalog_names_lsc"]
    assert report["checks"]["catalog_names_asbw"]
    assert report["checks"]["catalog_names_wson"]
    assert report["checks"]["leftover_text_binds_lsc"]
    assert report["checks"]["proved_lsc_consumes_leftover"]
    assert report["mission_goal"] == LSC_ACTUATION_GOAL
    assert report["done_when"] == LSC_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[LSC_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "lsc" in capability.tags
    assert "rfc6205" in capability.tags
    assert "http" in capability.tags
    assert "lscid" in capability.tags
    assert "lscdigest" in capability.tags
    assert "lsc" in capability.tags
    assert "label" in capability.tags


def test_selection_gate_rejects_ledger_only_lsc_family(tmp_path: Path) -> None:
    ...
    assert "lsc" in family.split("/")
    assert "rfc6205" in set(semantic_tokens(LSC_ACTUATION_GOAL))
    assert "lscid" in set(semantic_tokens(LSC_ACTUATION_GOAL))
    assert "lscdigest" in family
