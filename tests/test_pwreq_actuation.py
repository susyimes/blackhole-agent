...
def test_goal_binds_pwreq_actuation_plane() -> None:
    assert leftover_marker_ids(PWREQ_ACTUATION_GOAL) == (PWREQ_ACTUATION_ID,)
    ...
    assert PWREQ_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(PWREQ_LEFTOVER) == (PWREQ_ACTUATION_ID,)
    assert leftover_marker_ids(PWREQ_ACTUATION_GOAL) == (PWREQ_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert PWREQ_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(PWREQ_ACTUATION_GOAL)
...
def test_opted_in_pwreq_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("pwreq",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, PWREQ_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["pwreq"]

    missing = run_pwreq_workflow(with_pwreqid=False)
    skip_bind = run_pwreq_workflow(skip_bind=True)
    skip_temporary = run_pwreq_workflow(do_temporary=False)
    skip_public = run_pwreq_workflow(do_public=False)
    skip_pwreqdigest = run_pwreq_workflow(do_pwreqdigest=False)
    skip_replay = run_pwreq_workflow(replay=False)
    skip_pwreqid = run_pwreq_workflow(use_pwreqid=False)
    live = run_pwreq_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_pwreqid"
    ...
    assert skip_pwreqdigest["ok"] is False
    assert skip_pwreqdigest["error"] == "pwreqdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_pwreqid["ok"] is False
    assert skip_pwreqid["error"] == "pwreqid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_pwreqdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["pwreqdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["pwreqid_bound"] is True
    assert row["digest"]
    assert live["pwreqid"] == DEFAULT_PWREQID
    assert live["pwreqdigest"] == DEFAULT_PWREQDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, pwreqid=DEFAULT_PWREQID, pwreqdigest=DEFAULT_PWREQDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["pwreqid"] == DEFAULT_PWREQID
    assert queried["pwreqdigest"] == DEFAULT_PWREQDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == PWREQ_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, pwreqid=DEFAULT_PWREQID, pwreqdigest=DEFAULT_PWREQDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["pwreqid"] == DEFAULT_PWREQID
    assert answered["pwreqdigest"] == DEFAULT_PWREQDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, pwreqid=DEFAULT_PWREQID, include_pwreqid=False)
    )
    assert bare["has_pwreqid"] is False
    assert bare["pwreqid"] == EMPTY_PWREQID
    publicised = serialize_pwreq(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_pwreq(publicised) == DEFAULT_SOURCE
    assert parse_pwreq(RFC_DEST_FIELD) == DEST_HOP
    header = parse_pwreq_header(encode_pwreq_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_PWREQID, DEFAULT_PWREQDIGEST))
    ...
    assert asked["method"] == "NATIVE"
    assert asked["disc_kind"] == "native"
    assert listed["disc_kind"] == "pw"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_PWREQID).startswith("NATIVE")
    assert "pwreqdigest=" in canonical_public(SENTINEL, DEFAULT_PWREQID, DEFAULT_PWREQDIGEST)


def test_builtin_proof_seals_pwreq_actuation() -> None:
    report = builtin_pwreq_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "pwreq_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_pwreq"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_pwreqid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_pwreqdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_pwreqid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_pwreqdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_pwreq"]
    ...
    assert report["checks"]["leftover_text_binds_pwreq"]
    assert report["checks"]["proved_pwreq_consumes_leftover"]
    assert report["mission_goal"] == PWREQ_ACTUATION_GOAL
    assert report["done_when"] == PWREQ_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[PWREQ_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "pwreq" in capability.tags
    assert "rfc3916" in capability.tags
    assert "http" in capability.tags
    assert "pwreqid" in capability.tags
    assert "pwreqdigest" in capability.tags
    assert "native" in capability.tags
    assert "pw" in capability.tags


def test_selection_gate_rejects_ledger_only_pwreq_family(tmp_path: Path) -> None:
    ...
    assert "pwreq" in family.split("/")
    assert "rfc3916" in set(semantic_tokens(PWREQ_ACTUATION_GOAL))
    assert "pwreqid" in set(semantic_tokens(PWREQ_ACTUATION_GOAL))
    assert "pwreqdigest" in family
