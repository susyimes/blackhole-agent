...
from blackhole_agent.tempaddr_actuation import (
    ...
    SENTINEL,
    TEMPORARY_HEADER,
    builtin_tempaddr_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_tempaddr_header,
    independent_tempaddrdigest,
    parse_http_request,
    parse_http_response,
    parse_message,
    parse_tempaddr,
    parse_tempaddr_header,
    public_request,
    public_response,
    run_tempaddr_workflow,
    serialize_tempaddr,
    temporary_request,
    temporary_response,
)
...
def test_goal_binds_tempaddr_actuation_plane() -> None:
    assert leftover_marker_ids(TEMPADDR_ACTUATION_GOAL) == (TEMPADDR_ACTUATION_ID,)
    assert leftover_marker_ids(TEMPADDR_LEFTOVER) == (TEMPADDR_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert TEMPADDR_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(TEMPADDR_ACTUATION_GOAL)
...
def test_opted_in_tempaddr_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("tempaddr",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, TEMPADDR_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["tempaddr"]

    missing = run_tempaddr_workflow(with_tempaddrid=False)
    skip_bind = run_tempaddr_workflow(skip_bind=True)
    skip_temporary = run_tempaddr_workflow(do_temporary=False)
    skip_public = run_tempaddr_workflow(do_public=False)
    skip_tempaddrdigest = run_tempaddr_workflow(do_tempaddrdigest=False)
    skip_replay = run_tempaddr_workflow(replay=False)
    skip_tempaddrid = run_tempaddr_workflow(use_tempaddrid=False)
    live = run_tempaddr_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_tempaddrid"
    ...
    assert skip_tempaddrdigest["ok"] is False
    assert skip_tempaddrdigest["error"] == "tempaddrdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_tempaddrid["ok"] is False
    assert skip_tempaddrid["error"] == "tempaddrid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_tempaddrdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["tempaddrdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["tempaddrid_bound"] is True
    assert row["digest"]
    assert live["tempaddrid"] == DEFAULT_TEMPADDRID
    assert live["tempaddrdigest"] == DEFAULT_TEMPADDRDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, tempaddrid=DEFAULT_TEMPADDRID, tempaddrdigest=DEFAULT_TEMPADDRDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["tempaddrid"] == DEFAULT_TEMPADDRID
    assert queried["tempaddrdigest"] == DEFAULT_TEMPADDRDIGEST
    assert queried["type"] == FRAME_TEMPORARY
    assert queried["first_byte"] == TEMPADDR_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, tempaddrid=DEFAULT_TEMPADDRID, tempaddrdigest=DEFAULT_TEMPADDRDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["tempaddrid"] == DEFAULT_TEMPADDRID
    assert answered["tempaddrdigest"] == DEFAULT_TEMPADDRDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, tempaddrid=DEFAULT_TEMPADDRID, include_tempaddrid=False)
    )
    assert bare["has_tempaddrid"] is False
    assert bare["tempaddrid"] == EMPTY_TEMPADDRID
    publicised = serialize_tempaddr(DEFAULT_TEMPORARY)
    assert publicised == RFC_TEMPORARY_FIELD
    assert parse_tempaddr(publicised) == DEFAULT_TEMPORARY
    assert parse_tempaddr(RFC_PUBLIC_FIELD) == PUBLIC_POLICY
    header = parse_tempaddr_header(encode_tempaddr_header(DEFAULT_TEMPORARY))
    assert header["field_value"] == RFC_TEMPORARY_FIELD
    assert header["header"] == TEMPORARY_HEADER
    asked = parse_http_request(temporary_request(SENTINEL, DEFAULT_TEMPADDRID))
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_TEMPADDRID, DEFAULT_TEMPADDRDIGEST))
    ...
    assert asked["method"] == "TEMPORARY"
    assert asked["tempaddr_kind"] == "temporary"
    assert listed["tempaddr_kind"] == "public"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_TEMPORARY
    assert preload_public["policy"] == PUBLIC_POLICY
    assert canonical_temporary(SENTINEL, DEFAULT_TEMPADDRID).startswith("TEMPORARY")
    assert "tempaddrdigest=" in canonical_public(SENTINEL, DEFAULT_TEMPADDRID, DEFAULT_TEMPADDRDIGEST)


def test_builtin_proof_seals_tempaddr_actuation() -> None:
    report = builtin_tempaddr_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "tempaddr_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_tempaddr"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_tempaddrid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_tempaddrdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_tempaddrid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_tempaddrdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_tempaddr"]
    ...
    assert report["checks"]["catalog_names_opaqueiid"]
    assert report["checks"]["leftover_text_binds_tempaddr"]
    assert report["checks"]["proved_tempaddr_consumes_leftover"]
    assert report["mission_goal"] == TEMPADDR_ACTUATION_GOAL
    assert report["done_when"] == TEMPADDR_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[TEMPADDR_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "tempaddr" in capability.tags
    assert "rfc4941" in capability.tags
    assert "http" in capability.tags
    assert "tempaddrid" in capability.tags
    assert "tempaddrdigest" in capability.tags
    assert "temporary" in capability.tags


def test_selection_gate_rejects_ledger_only_tempaddr_family(tmp_path: Path) -> None:
    ...
    assert "tempaddr" in family.split("/")
    assert "rfc4941" in family
    assert "tempaddrid" in family
    assert "tempaddrdigest" in family
