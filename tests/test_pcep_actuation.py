...
def test_goal_binds_pcep_actuation_plane() -> None:
    assert leftover_marker_ids(PCEP_ACTUATION_GOAL) == (PCEP_ACTUATION_ID,)
    ...
    assert PCEP_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(PCEP_LEFTOVER) == (PCEP_ACTUATION_ID,)
    assert leftover_marker_ids(PCEP_ACTUATION_GOAL) == (PCEP_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert PCEP_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(PCEP_ACTUATION_GOAL)
...
def test_opted_in_pcep_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("pcep",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, PCEP_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["pcep"]

    missing = run_pcep_workflow(with_pcepid=False)
    skip_bind = run_pcep_workflow(skip_bind=True)
    skip_temporary = run_pcep_workflow(do_temporary=False)
    skip_public = run_pcep_workflow(do_public=False)
    skip_pcepdigest = run_pcep_workflow(do_pcepdigest=False)
    skip_replay = run_pcep_workflow(replay=False)
    skip_pcepid = run_pcep_workflow(use_pcepid=False)
    live = run_pcep_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_pcepid"
    ...
    assert skip_pcepdigest["ok"] is False
    assert skip_pcepdigest["error"] == "pcepdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_pcepid["ok"] is False
    assert skip_pcepid["error"] == "pcepid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_pcepdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["pcepdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["pcepid_bound"] is True
    assert row["digest"]
    assert live["pcepid"] == DEFAULT_PCEPID
    assert live["pcepdigest"] == DEFAULT_PCEPDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, pcepid=DEFAULT_PCEPID, pcepdigest=DEFAULT_PCEPDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["pcepid"] == DEFAULT_PCEPID
    assert queried["pcepdigest"] == DEFAULT_PCEPDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == PCEP_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, pcepid=DEFAULT_PCEPID, pcepdigest=DEFAULT_PCEPDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["pcepid"] == DEFAULT_PCEPID
    assert answered["pcepdigest"] == DEFAULT_PCEPDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, pcepid=DEFAULT_PCEPID, include_pcepid=False)
    )
    assert bare["has_pcepid"] is False
    assert bare["pcepid"] == EMPTY_PCEPID
    publicised = serialize_pcep(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_pcep(publicised) == DEFAULT_SOURCE
    assert parse_pcep(RFC_DEST_FIELD) == DEST_HOP
    header = parse_pcep_header(encode_pcep_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_PCEPID, DEFAULT_PCEPDIGEST))
    ...
    assert asked["disc_kind"] == "open"
    assert listed["disc_kind"] == "pcreq"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_PCEPID).startswith("OPEN")
    assert "pcepdigest=" in canonical_public(SENTINEL, DEFAULT_PCEPID, DEFAULT_PCEPDIGEST)


def test_builtin_proof_seals_pcep_actuation() -> None:
    report = builtin_pcep_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "pcep_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_pcep"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_pcepid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_pcepdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_pcepid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_pcepdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_pcep"]
    ...
    assert report["checks"]["leftover_text_binds_pcep"]
    assert report["checks"]["proved_pcep_consumes_leftover"]
    assert report["mission_goal"] == PCEP_ACTUATION_GOAL
    assert report["done_when"] == PCEP_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[PCEP_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "pcep" in capability.tags
    assert "rfc5440" in capability.tags
    assert "http" in capability.tags
    assert "pcepid" in capability.tags
    assert "pcepdigest" in capability.tags
    assert "open" in capability.tags
...
def test_selection_gate_rejects_ledger_only_pcep_family(tmp_path: Path) -> None:
    ...
    assert "pcep" in family.split("/")
    assert "rfc5440" in set(semantic_tokens(PCEP_ACTUATION_GOAL))
    assert "pcepid" in set(semantic_tokens(PCEP_ACTUATION_GOAL))
    assert "pcepdigest" in family
