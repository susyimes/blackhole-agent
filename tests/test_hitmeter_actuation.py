...
from blackhole_agent.hitmeter_actuation import (
    ...
    SENTINEL,
    METER_HEADER,
    builtin_hitmeter_actuation_proof,
    canonical_usage,
    canonical_meter,
    crc32c,
    encode_usage,
    encode_meter,
    encode_hitmeter_header,
    independent_hitmeter_digest,
    parse_http_request,
    parse_http_response,
    parse_message,
    parse_hitmeter,
    parse_hitmeter_header,
    usage_request,
    usage_response,
    run_hitmeter_workflow,
    serialize_hitmeter,
    meter_request,
    meter_response,
)
...
def test_goal_binds_hitmeter_actuation_plane() -> None:
    assert leftover_marker_ids(HITMETER_ACTUATION_GOAL) == (HITMETER_ACTUATION_ID,)
    assert leftover_marker_ids(HITMETER_LEFTOVER) == (HITMETER_ACTUATION_ID,)
    assert leftover_marker_ids(HITMETER_ACTUATION_GOAL) == (HITMETER_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert HITMETER_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(HITMETER_ACTUATION_GOAL)
    hitmeter_signature = semantic_signature(HITMETER_ACTUATION_GOAL)
    for neighbor in NEIGHBORS:
        assert semantic_similarity(hitmeter_signature, semantic_signature(neighbor)) < 0.82


def test_opted_in_hitmeter_tool_completes_meter_usage_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("hitmeter",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, HITMETER_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["hitmeter"]

    missing = run_hitmeter_workflow(with_meterid=False)
    skip_bind = run_hitmeter_workflow(skip_bind=True)
    skip_meter = run_hitmeter_workflow(do_meter=False)
    skip_usage = run_hitmeter_workflow(do_usage=False)
    skip_usagedigest = run_hitmeter_workflow(do_usagedigest=False)
    skip_replay = run_hitmeter_workflow(replay=False)
    skip_meterid = run_hitmeter_workflow(use_meterid=False)
    live = run_hitmeter_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_meterid"
    assert skip_bind["ok"] is False
    assert skip_bind["error"] == "not_bound"
    assert skip_meter["ok"] is False
    assert skip_meter["error"] == "meter_required"
    assert skip_usage["ok"] is False
    assert skip_usage["error"] == "usage_required"
    assert skip_usagedigest["ok"] is False
    assert skip_usagedigest["error"] == "usagedigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_meterid["ok"] is False
    assert skip_meterid["error"] == "meterid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_hitmeter_digest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["meter_frame"] is True
    assert row["usage_frame"] is True
    assert row["usagedigest_response"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["meterid_bound"] is True
    assert row["digest"]
    assert live["meterid"] == DEFAULT_METERID
    assert live["usagedigest"] == DEFAULT_USAGEDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_meter(identity=SENTINEL, meterid=DEFAULT_METERID, usagedigest=DEFAULT_USAGEDIGEST)
    )
    assert queried["is_meter"] is True and queried["is_response"] is False
    assert queried["identity"] == SENTINEL and queried["meterid"] == DEFAULT_METERID
    assert queried["usagedigest"] == DEFAULT_USAGEDIGEST
    assert queried["type"] == FRAME_METER
    assert queried["first_byte"] == HITMETER_FIRST
    answered = parse_message(
        encode_usage(identity=SENTINEL, meterid=DEFAULT_METERID, usagedigest=DEFAULT_USAGEDIGEST)
    )
    assert answered["is_usage"] is True and answered["is_response"] is True
    assert answered["meterid"] == DEFAULT_METERID
    assert answered["usagedigest"] == DEFAULT_USAGEDIGEST
    ...
    bare = parse_message(
        encode_meter(identity=SENTINEL, meterid=DEFAULT_METERID, include_meterid=False)
    )
    assert bare["has_meterid"] is False
    assert bare["meterid"] == EMPTY_METERID
    advertised = serialize_hitmeter(DEFAULT_METER)
    assert advertised == RFC_METER_FIELD
    assert parse_hitmeter(advertised) == DEFAULT_METER
    assert parse_hitmeter(RFC_USAGE_FIELD) == USAGE_POLICY
    header = parse_hitmeter_header(encode_hitmeter_header(DEFAULT_METER))
    assert header["field_value"] == RFC_METER_FIELD
    assert header["header"] == METER_HEADER
    asked = parse_http_request(meter_request(SENTINEL, DEFAULT_METERID))
    listed = parse_http_request(usage_request(SENTINEL, DEFAULT_METERID, DEFAULT_USAGEDIGEST))
    ...
    assert asked["method"] == "METER"
    assert asked["hitmeter_kind"] == "meter"
    assert listed["hitmeter_kind"] == "usage"
    assert got["status"] == 200
    assert preload_reply["status"] == 200
    assert got["policy"] == DEFAULT_METER
    assert preload_reply["policy"] == USAGE_POLICY
    assert canonical_meter(SENTINEL, DEFAULT_METERID).startswith("METER")
    assert "usagedigest=" in canonical_usage(SENTINEL, DEFAULT_METERID, DEFAULT_USAGEDIGEST)


def test_builtin_proof_seals_hitmeter_actuation() -> None:
    report = builtin_hitmeter_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "hitmeter_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_hitmeter"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_meterid_is_forbidden"]
    assert report["checks"]["skip_meter_stays_empty"]
    assert report["checks"]["skip_usage_stays_empty"]
    assert report["checks"]["skip_usagedigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_meterid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_usagedigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_hitmeter"]
    ...
    assert report["checks"]["catalog_names_icp"]
    assert report["checks"]["leftover_text_binds_hitmeter"]
    assert report["checks"]["proved_hitmeter_consumes_leftover"]
    assert report["mission_goal"] == HITMETER_ACTUATION_GOAL
    assert report["done_when"] == HITMETER_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[HITMETER_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "hitmeter" in capability.tags
    assert "rfc2227" in capability.tags
    assert "http" in capability.tags
    assert "meterid" in capability.tags
    assert "usagedigest" in capability.tags
    assert "meter" in capability.tags


def test_selection_gate_rejects_ledger_only_hitmeter_family(tmp_path: Path) -> None:
    ...
    assert "hitmeter" in family
    assert "rfc2227" in family
    assert "meterid" in family
    assert "usagedigest" in family
