...
def test_goal_binds_nvo_actuation_plane() -> None:
    assert leftover_marker_ids(NVO_ACTUATION_GOAL) == (NVO_ACTUATION_ID,)
    ...
    assert NVO_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(NVO_LEFTOVER) == (NVO_ACTUATION_ID,)
    assert leftover_marker_ids(NVO_ACTUATION_GOAL) == (NVO_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert NVO_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(NVO_ACTUATION_GOAL)
...
def test_opted_in_nvo_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("nvo",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, NVO_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["nvo"]

    missing = run_nvo_workflow(with_nvoid=False)
    skip_bind = run_nvo_workflow(skip_bind=True)
    skip_temporary = run_nvo_workflow(do_temporary=False)
    skip_public = run_nvo_workflow(do_public=False)
    skip_nvodigest = run_nvo_workflow(do_nvodigest=False)
    skip_replay = run_nvo_workflow(replay=False)
    skip_nvoid = run_nvo_workflow(use_nvoid=False)
    live = run_nvo_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_nvoid"
    ...
    assert skip_nvodigest["ok"] is False
    assert skip_nvodigest["error"] == "nvodigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_nvoid["ok"] is False
    assert skip_nvoid["error"] == "nvoid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_nvodigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["nvodigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["nvoid_bound"] is True
    assert row["digest"]
    assert live["nvoid"] == DEFAULT_NVOID
    assert live["nvodigest"] == DEFAULT_NVODIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, nvoid=DEFAULT_NVOID, nvodigest=DEFAULT_NVODIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["nvoid"] == DEFAULT_NVOID
    assert queried["nvodigest"] == DEFAULT_NVODIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == NVO_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, nvoid=DEFAULT_NVOID, nvodigest=DEFAULT_NVODIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["nvoid"] == DEFAULT_NVOID
    assert answered["nvodigest"] == DEFAULT_NVODIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, nvoid=DEFAULT_NVOID, include_nvoid=False)
    )
    assert bare["has_nvoid"] is False
    assert bare["nvoid"] == EMPTY_NVOID
    publicised = serialize_nvo(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_nvo(publicised) == DEFAULT_SOURCE
    assert parse_nvo(RFC_DEST_FIELD) == DEST_HOP
    header = parse_nvo_header(encode_nvo_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_NVOID, DEFAULT_NVODIGEST))
    ...
    assert asked["method"] == "NVE"
    assert asked["disc_kind"] == "nvo"
    assert listed["disc_kind"] == "vni"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_NVOID).startswith("NVE")
    assert "nvodigest=" in canonical_public(SENTINEL, DEFAULT_NVOID, DEFAULT_NVODIGEST)


def test_builtin_proof_seals_nvo_actuation() -> None:
    report = builtin_nvo_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "nvo_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_nvo"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_nvoid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_nvodigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_nvoid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_nvodigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_nvo"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_nvo"]
    assert report["checks"]["proved_nvo_consumes_leftover"]
    assert report["mission_goal"] == NVO_ACTUATION_GOAL
    assert report["done_when"] == NVO_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[NVO_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "nvo" in capability.tags
    assert "rfc8365" in capability.tags
    assert "http" in capability.tags
    assert "nvoid" in capability.tags
    assert "nvodigest" in capability.tags
    assert "nvo" in capability.tags
    assert "vni" in capability.tags


def test_selection_gate_rejects_ledger_only_nvo_family(tmp_path: Path) -> None:
    ...
    assert "nvo" in family.split("/")
    assert "nvoid" in set(semantic_tokens(NVO_ACTUATION_GOAL))
    assert "nvodigest" in family
