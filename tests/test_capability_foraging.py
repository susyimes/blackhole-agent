...
def test_python_introspection_reflects_triple_nested_namespace_class_static(tmp_path: Path) -> None:
    ...
    assert result["record"].get("python_quadruple_nested_namespace_class_static") is not True


def test_python_introspection_reflects_quadruple_nested_namespace_class_static(tmp_path: Path) -> None:
    pkg = tmp_path / "pkg"
    ns = pkg / "forage_ns" / "codec" / "text" / "safe"
    ns.mkdir(parents=True)
    (pkg / "forage_ns" / "__init__.py").write_text("", encoding="utf-8")
    (pkg / "forage_ns" / "codec" / "__init__.py").write_text("", encoding="utf-8")
    (pkg / "forage_ns" / "codec" / "text" / "__init__.py").write_text("", encoding="utf-8")
    (pkg / "forage_ns" / "codec" / "text" / "safe" / "__init__.py").write_text("", encoding="utf-8")
    (pkg / "forage_ns" / "codec" / "text" / "safe" / "inner.py").write_text(
        "class Codec:\n"
        "    def __init__(self, *args, **kwargs):\n"
        "        raise TypeError('Codec cannot be constructed')\n"
        "    @staticmethod\n"
    ...
    assert reflected["ok"], reflected
    names = [candidate["name"] for candidate in reflected["candidates"]]
    assert "codec.text.safe.inner.Codec.encode" in names
    assert "codec.text.safe.Codec.encode" not in names
    encoded = next(
        candidate
        for candidate in reflected["candidates"]
        if candidate["name"] == "codec.text.safe.inner.Codec.encode"
    )
    assert encoded["python_quadruple_nested_namespace_class_static"] is True
    assert encoded.get("python_triple_nested_namespace_class_static") is not True
    assert encoded.get("python_deep_nested_namespace_class_static") is not True
    assert encoded.get("python_nested_namespace_class_static") is not True
    assert encoded.get("python_class_static") is not True
    result = infer_acquisition_spec(
        slug="forage-ns-quadruple-codec",
        name="forage-ns-quadruple-codec",
        source=pkg,
        staging_root=tmp_path / "infer",
        hint="forage_ns",
        close_deps=False,
    ...
    assert result["ok"], result
    assert result["record"]["winner"] == "codec.text.safe.inner.Codec.encode"
    assert result["record"]["python_quadruple_nested_namespace_class_static"] is True
    assert result["record"]["python_triple_nested_namespace_class_static"] is False
    assert result["record"]["python_deep_nested_namespace_class_static"] is False
    assert result["record"]["python_nested_namespace_class_static"] is False
    assert result["record"]["python_class_static"] is False
...
def test_python_quadruple_nested_namespace_class_static_sdist_forages_django(tmp_path: Path) -> None:
    from blackhole_agent.capability_forage_targets import live_registry_archive
