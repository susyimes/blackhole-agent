...
from blackhole_agent.interas_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_interas_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_interas_header,
    independent_interasdigest,
    parse_http_comm,
    parse_http_response,
    parse_message,
    parse_interas,
    parse_interas_header,
    public_comm,
    public_response,
    run_interas_workflow,
    serialize_interas,
    temporary_comm,
    temporary_response,
)
...
def test_goal_binds_interas_actuation_plane() -> None:
    assert leftover_marker_ids(INTERAS_ACTUATION_GOAL) == (INTERAS_ACTUATION_ID,)
    ...
    assert INTERAS_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(INTERAS_LEFTOVER) == (INTERAS_ACTUATION_ID,)
    assert leftover_marker_ids(INTERAS_ACTUATION_GOAL) == (INTERAS_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert INTERAS_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(INTERAS_ACTUATION_GOAL)
...
def test_opted_in_interas_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("interas",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, INTERAS_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["interas"]

    missing = run_interas_workflow(with_interasid=False)
    skip_bind = run_interas_workflow(skip_bind=True)
    skip_temporary = run_interas_workflow(do_temporary=False)
    skip_public = run_interas_workflow(do_public=False)
    skip_interasdigest = run_interas_workflow(do_interasdigest=False)
    skip_replay = run_interas_workflow(replay=False)
    skip_interasid = run_interas_workflow(use_interasid=False)
    live = run_interas_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_interasid"
    ...
    assert skip_interasdigest["ok"] is False
    assert skip_interasdigest["error"] == "interasdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_interasid["ok"] is False
    assert skip_interasid["error"] == "interasid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_interasdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["interasdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["interasid_bound"] is True
    assert row["digest"]
    assert live["interasid"] == DEFAULT_INTERASID
    assert live["interasdigest"] == DEFAULT_INTERASDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, interasid=DEFAULT_INTERASID, interasdigest=DEFAULT_INTERASDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["interasid"] == DEFAULT_INTERASID
    assert queried["interasdigest"] == DEFAULT_INTERASDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == INTERAS_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, interasid=DEFAULT_INTERASID, interasdigest=DEFAULT_INTERASDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["interasid"] == DEFAULT_INTERASID
    assert answered["interasdigest"] == DEFAULT_INTERASDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, interasid=DEFAULT_INTERASID, include_interasid=False)
    )
    assert bare["has_interasid"] is False
    assert bare["interasid"] == EMPTY_INTERASID
    publicised = serialize_interas(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_interas(publicised) == DEFAULT_SOURCE
    assert parse_interas(RFC_DEST_FIELD) == DEST_HOP
    header = parse_interas_header(encode_interas_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_INTERASID, DEFAULT_INTERASDIGEST))
    ...
    assert asked["method"] == "CONTIG"
    assert asked["disc_kind"] == "contig"
    assert listed["disc_kind"] == "nest"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_INTERASID).startswith("CONTIG")
    assert "interasdigest=" in canonical_public(SENTINEL, DEFAULT_INTERASID, DEFAULT_INTERASDIGEST)


def test_builtin_proof_seals_interas_actuation() -> None:
    report = builtin_interas_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "interas_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_interas"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_interasid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_interasdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_interasid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_interasdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_interas"]
    ...
    assert report["checks"]["leftover_text_binds_interas"]
    assert report["checks"]["proved_interas_consumes_leftover"]
    assert report["mission_goal"] == INTERAS_ACTUATION_GOAL
    assert report["done_when"] == INTERAS_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[INTERAS_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "interas" in capability.tags
    assert "rfc5151" in capability.tags
    assert "http" in capability.tags
    assert "interasid" in capability.tags
    assert "interasdigest" in capability.tags
    assert "contig" in capability.tags
    assert "nest" in capability.tags


def test_selection_gate_rejects_ledger_only_interas_family(tmp_path: Path) -> None:
    ...
    assert "intera" in family.split("/")
    assert "rfc5151" in set(semantic_tokens(INTERAS_ACTUATION_GOAL))
    assert "interasid" in set(semantic_tokens(INTERAS_ACTUATION_GOAL))
    assert "interasdigest" in family
