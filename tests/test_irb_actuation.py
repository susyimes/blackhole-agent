...
def test_goal_binds_irb_actuation_plane() -> None:
    assert leftover_marker_ids(IRB_ACTUATION_GOAL) == (IRB_ACTUATION_ID,)
    ...
    assert IRB_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(IRB_LEFTOVER) == (IRB_ACTUATION_ID,)
    assert leftover_marker_ids(IRB_ACTUATION_GOAL) == (IRB_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert IRB_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(IRB_ACTUATION_GOAL)
...
def test_opted_in_irb_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("irb",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, IRB_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["irb"]

    missing = run_irb_workflow(with_irbid=False)
    skip_bind = run_irb_workflow(skip_bind=True)
    skip_temporary = run_irb_workflow(do_temporary=False)
    skip_public = run_irb_workflow(do_public=False)
    skip_irbdigest = run_irb_workflow(do_irbdigest=False)
    skip_replay = run_irb_workflow(replay=False)
    skip_irbid = run_irb_workflow(use_irbid=False)
    live = run_irb_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_irbid"
    ...
    assert skip_irbdigest["ok"] is False
    assert skip_irbdigest["error"] == "irbdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_irbid["ok"] is False
    assert skip_irbid["error"] == "irbid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_irbdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["irbdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["irbid_bound"] is True
    assert row["digest"]
    assert live["irbid"] == DEFAULT_IRBID
    assert live["irbdigest"] == DEFAULT_IRBDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, irbid=DEFAULT_IRBID, irbdigest=DEFAULT_IRBDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["irbid"] == DEFAULT_IRBID
    assert queried["irbdigest"] == DEFAULT_IRBDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == IRB_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, irbid=DEFAULT_IRBID, irbdigest=DEFAULT_IRBDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["irbid"] == DEFAULT_IRBID
    assert answered["irbdigest"] == DEFAULT_IRBDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, irbid=DEFAULT_IRBID, include_irbid=False)
    )
    assert bare["has_irbid"] is False
    assert bare["irbid"] == EMPTY_IRBID
    publicised = serialize_irb(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_irb(publicised) == DEFAULT_SOURCE
    assert parse_irb(RFC_DEST_FIELD) == DEST_HOP
    header = parse_irb_header(encode_irb_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_IRBID, DEFAULT_IRBDIGEST))
    ...
    assert asked["method"] == "IRB"
    assert asked["disc_kind"] == "irb"
    assert listed["disc_kind"] == "l3"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_IRBID).startswith("IRB")
    assert "irbdigest=" in canonical_public(SENTINEL, DEFAULT_IRBID, DEFAULT_IRBDIGEST)


def test_builtin_proof_seals_irb_actuation() -> None:
    report = builtin_irb_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "irb_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_irb"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_irbid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_irbdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_irbid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_irbdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_irb"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_irb"]
    assert report["checks"]["proved_irb_consumes_leftover"]
    assert report["mission_goal"] == IRB_ACTUATION_GOAL
    assert report["done_when"] == IRB_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[IRB_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "irb" in capability.tags
    assert "rfc9135" in capability.tags
    assert "http" in capability.tags
    assert "irbid" in capability.tags
    assert "irbdigest" in capability.tags
    assert "irb" in capability.tags
    assert "l3" in capability.tags


def test_selection_gate_rejects_ledger_only_irb_family(tmp_path: Path) -> None:
    ...
    assert "irb" in family.split("/")
    assert "rfc9135" in family
    assert "irbid" in set(semantic_tokens(IRB_ACTUATION_GOAL))
    assert "irbdigest" in family
