...
def test_goal_binds_eam_actuation_plane() -> None:
    assert leftover_marker_ids(EAM_ACTUATION_GOAL) == (EAM_ACTUATION_ID,)
    ...
    assert EAM_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(EAM_LEFTOVER) == (EAM_ACTUATION_ID,)
    assert leftover_marker_ids(EAM_ACTUATION_GOAL) == (EAM_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert EAM_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(EAM_ACTUATION_GOAL)
...
def test_opted_in_eam_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("eam",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, EAM_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["eam"]

    missing = run_eam_workflow(with_eamid=False)
    skip_bind = run_eam_workflow(skip_bind=True)
    skip_temporary = run_eam_workflow(do_temporary=False)
    skip_public = run_eam_workflow(do_public=False)
    skip_eamdigest = run_eam_workflow(do_eamdigest=False)
    skip_replay = run_eam_workflow(replay=False)
    skip_eamid = run_eam_workflow(use_eamid=False)
    live = run_eam_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_eamid"
    ...
    assert skip_eamdigest["ok"] is False
    assert skip_eamdigest["error"] == "eamdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_eamid["ok"] is False
    assert skip_eamid["error"] == "eamid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_eamdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["eamdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["eamid_bound"] is True
    assert row["digest"]
    assert live["eamid"] == DEFAULT_EAMID
    assert live["eamdigest"] == DEFAULT_EAMDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, eamid=DEFAULT_EAMID, eamdigest=DEFAULT_EAMDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["eamid"] == DEFAULT_EAMID
    assert queried["eamdigest"] == DEFAULT_EAMDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == EAM_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, eamid=DEFAULT_EAMID, eamdigest=DEFAULT_EAMDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["eamid"] == DEFAULT_EAMID
    assert answered["eamdigest"] == DEFAULT_EAMDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, eamid=DEFAULT_EAMID, include_eamid=False)
    )
    assert bare["has_eamid"] is False
    assert bare["eamid"] == EMPTY_EAMID
    publicised = serialize_eam(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_eam(publicised) == DEFAULT_SOURCE
    assert parse_eam(RFC_DEST_FIELD) == DEST_HOP
    header = parse_eam_header(encode_eam_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_EAMID, DEFAULT_EAMDIGEST))
    ...
    assert asked["method"] == "EXPLICIT"
    assert asked["disc_kind"] == "explicit"
    assert listed["disc_kind"] == "mapping"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_EAMID).startswith("EXPLICIT")
    assert "eamdigest=" in canonical_public(SENTINEL, DEFAULT_EAMID, DEFAULT_EAMDIGEST)


def test_builtin_proof_seals_eam_actuation() -> None:
    report = builtin_eam_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "eam_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_eam"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_eamid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_eamdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_eamid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_eamdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_eam"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_eam"]
    assert report["checks"]["proved_eam_consumes_leftover"]
    assert report["mission_goal"] == EAM_ACTUATION_GOAL
    assert report["done_when"] == EAM_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[EAM_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "eam" in capability.tags
    assert "rfc7757" in capability.tags
    assert "http" in capability.tags
    assert "eamid" in capability.tags
    assert "eamdigest" in capability.tags
    assert "explicit" in capability.tags
    assert "mapping" in capability.tags


def test_selection_gate_rejects_ledger_only_eam_family(tmp_path: Path) -> None:
    ...
    assert "eam" in family.split("/")
    assert "rfc7757" in family
    assert "eamid" in set(semantic_tokens(EAM_ACTUATION_GOAL))
    assert "eamdigest" in family
    ...
    assert "siitdcid" not in family
    assert "siitdcdigest" not in family
