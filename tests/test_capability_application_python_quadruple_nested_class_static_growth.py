"""Tests for growing unplannable goals from four-level nested-namespace Python class-static sdists."""
...
from blackhole_agent.capability_application_growth import (
    ...
    load_python_quadruple_nested_namespace_class_static_apply_catalog,
    run_application_python_quadruple_nested_namespace_class_static_growth_plane,
    verify_application_python_quadruple_nested_namespace_class_static_growth_plane,
)
...
def test_python_quadruple_nested_namespace_class_static_catalog_is_live_fetched_sdist() -> None:
    catalog = load_python_quadruple_nested_namespace_class_static_apply_catalog()
    assert catalog["query"] == query_from_goal(PYTHON_QUADRUPLE_NESTED_NAMESPACE_CLASS_STATIC_GROW_TASK.goal)
    ...
    winner = next(
        ...
        if item["slug"] == PYTHON_QUADRUPLE_NESTED_NAMESPACE_CLASS_STATIC_WINNER_SLUG
    )
...
def test_python_quadruple_nested_namespace_class_static_match_selects_django() -> None:
    catalog = load_python_quadruple_nested_namespace_class_static_apply_catalog()
    absorbed = sorted(APPLY_ABSORBED_SLUGS)
    trend = rank_catalog(strip_declared_provides(catalog["items"]), absorbed=absorbed)
    assert trend["winner"]["slug"] == PYTHON_QUADRUPLE_NESTED_NAMESPACE_CLASS_STATIC_NPM_DECOY_SLUG
    ...
    assert matched["winner"]["slug"] == PYTHON_QUADRUPLE_NESTED_NAMESPACE_CLASS_STATIC_WINNER_SLUG
    covering = matched["covering"] or {}
    assert PYTHON_QUADRUPLE_NESTED_NAMESPACE_CLASS_STATIC_GOAL_KEY in covering["inferred_provides"]
    assert covering.get("python_quadruple_nested_namespace_class_static") is True
...
def test_grow_from_python_quadruple_nested_namespace_class_static_catalog_forages_django() -> None:
    catalog = load_python_quadruple_nested_namespace_class_static_apply_catalog()
    result = grow_application_task(
        PYTHON_QUADRUPLE_NESTED_NAMESPACE_CLASS_STATIC_GROW_TASK,
        catalog=catalog,
        forage=True,
        hide_before=[PYTHON_QUADRUPLE_NESTED_NAMESPACE_CLASS_STATIC_WINNER_CAPABILITY_ID],
        live_fetch=True,
    )
    assert result["ok"], result
    assert result["grew"] is True
    assert result["winner_slug"] == PYTHON_QUADRUPLE_NESTED_NAMESPACE_CLASS_STATIC_WINNER_SLUG
    assert (result.get("forage") or {}).get("origin", {}).get("kind") == "pypi-live"


def test_python_quadruple_nested_namespace_class_static_plane_grows_and_rejects_tamper(
    tmp_path: Path,
) -> None:
    report_dir = tmp_path / "report"
    plane = run_application_python_quadruple_nested_namespace_class_static_growth_plane(report_dir)
    assert plane["ok"], plane
    assert plane["winner"] == PYTHON_QUADRUPLE_NESTED_NAMESPACE_CLASS_STATIC_WINNER_SLUG
    assert plane["grade"]["python_quadruple_nested_namespace_class_static_selected"]
    assert plane["grade"]["winner_is_python_quadruple_nested_namespace_class_static"]
    assert plane["grade"]["winner_is_not_python_triple_nested_namespace_class_static"]
    ledger = load_ledger(default_ledger_path(REPO_ROOT))
    hidden = build_application_registry(
        ledger,
        hide=[PYTHON_QUADRUPLE_NESTED_NAMESPACE_CLASS_STATIC_WINNER_CAPABILITY_ID],
        include_synthesized=True,
        include_absorbed=True,
    )
    assert plan_application_task(PYTHON_QUADRUPLE_NESTED_NAMESPACE_CLASS_STATIC_GROW_TASK, hidden) is None
    grown = build_application_registry(ledger, include_synthesized=True, include_absorbed=True)
    solved = run_application_task(PYTHON_QUADRUPLE_NESTED_NAMESPACE_CLASS_STATIC_GROW_TASK, grown)
    assert solved["ok"], solved
    verification = verify_application_python_quadruple_nested_namespace_class_static_growth_plane(report_dir)
    assert verification["ok"], verification
    report_path = report_dir / "plane-report.json"
    report = json.loads(report_path.read_text(encoding="utf-8"))
    report["grade"]["grow_winner_is_django"] = False
    report_path.write_text(json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
    assert not verify_application_python_quadruple_nested_namespace_class_static_growth_plane(report_dir)["ok"]


def test_builtin_application_python_quadruple_nested_namespace_class_static_growth_plane_proof() -> None:
    result = builtin_application_python_quadruple_nested_namespace_class_static_growth_plane_proof()
    assert result["ok"], result
    assert result["action"] == "application_python_quadruple_nested_namespace_class_static_growth_plane"
    assert result["grow_winner_is_django"]
    assert result["python_quadruple_nested_namespace_class_static_selected"]
    assert result["winner_is_python_quadruple_nested_namespace_class_static"]
    leftover = (
