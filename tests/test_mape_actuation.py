...
def test_goal_binds_mape_actuation_plane() -> None:
    assert leftover_marker_ids(MAPE_ACTUATION_GOAL) == (MAPE_ACTUATION_ID,)
    ...
    assert leftover_marker_ids(MAPE_LEFTOVER) == (MAPE_ACTUATION_ID,)
    assert leftover_marker_ids(MAPT_ACTUATION_GOAL) == (MAPT_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    assert MAPE_ACTUATION_ID in LOCAL_DENYLIST
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert MAPE_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(MAPE_ACTUATION_GOAL)
...
def test_opted_in_mape_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("mape",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, MAPE_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["mape"]

    missing = run_mape_workflow(with_mapeid=False)
    skip_bind = run_mape_workflow(skip_bind=True)
    skip_temporary = run_mape_workflow(do_temporary=False)
    skip_public = run_mape_workflow(do_public=False)
    skip_mapedigest = run_mape_workflow(do_mapedigest=False)
    skip_replay = run_mape_workflow(replay=False)
    skip_mapeid = run_mape_workflow(use_mapeid=False)
    live = run_mape_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_mapeid"
    ...
    assert skip_mapedigest["ok"] is False
    assert skip_mapedigest["error"] == "mapedigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_mapeid["ok"] is False
    assert skip_mapeid["error"] == "mapeid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_mapedigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["mapedigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["mapeid_bound"] is True
    assert row["digest"]
    assert live["mapeid"] == DEFAULT_MAPEID
    assert live["mapedigest"] == DEFAULT_MAPEDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, mapeid=DEFAULT_MAPEID, mapedigest=DEFAULT_MAPEDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["mapeid"] == DEFAULT_MAPEID
    assert queried["mapedigest"] == DEFAULT_MAPEDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == MAPE_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, mapeid=DEFAULT_MAPEID, mapedigest=DEFAULT_MAPEDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["mapeid"] == DEFAULT_MAPEID
    assert answered["mapedigest"] == DEFAULT_MAPEDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, mapeid=DEFAULT_MAPEID, include_mapeid=False)
    )
    assert bare["has_mapeid"] is False
    assert bare["mapeid"] == EMPTY_MAPEID
    publicised = serialize_mape(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_mape(publicised) == DEFAULT_SOURCE
    assert parse_mape(RFC_DEST_FIELD) == DEST_HOP
    header = parse_mape_header(encode_mape_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_MAPEID, DEFAULT_MAPEDIGEST))
    ...
    assert asked["method"] == "CE"
    assert asked["disc_kind"] == "ce"
    assert listed["disc_kind"] == "br"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_MAPEID).startswith("CE")
    assert "mapedigest=" in canonical_public(SENTINEL, DEFAULT_MAPEID, DEFAULT_MAPEDIGEST)


def test_builtin_proof_seals_mape_actuation() -> None:
    report = builtin_mape_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "mape_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_mape"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_mapeid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_mapedigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_mapeid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_mapedigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_mape"]
    ...
    assert report["checks"]["leftover_text_binds_mape"]
    assert report["checks"]["proved_mape_consumes_leftover"]
    assert report["mission_goal"] == MAPE_ACTUATION_GOAL
    assert report["done_when"] == MAPE_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[MAPE_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "mape" in capability.tags
    assert "rfc7597" in capability.tags
    assert "http" in capability.tags
    assert "mapeid" in capability.tags
    assert "mapedigest" in capability.tags
    assert "ce" in capability.tags
    assert "br" in capability.tags


def test_selection_gate_rejects_ledger_only_mape_family(tmp_path: Path) -> None:
    ...
    assert "mape" in family.split("/")
    assert "rfc7597" in family
    assert "mapeid" in family
    assert "mapedigest" in family
