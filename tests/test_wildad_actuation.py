...
from blackhole_agent.wildad_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_wildad_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_wildad_header,
    independent_wilddigest,
    parse_http_comm,
    parse_http_response,
    parse_message,
    parse_wildad,
...
def test_goal_binds_wildad_actuation_plane() -> None:
    assert leftover_marker_ids(WILDAD_ACTUATION_GOAL) == (WILDAD_ACTUATION_ID,)
    ...
    assert WILDAD_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(WILDAD_LEFTOVER) == (WILDAD_ACTUATION_ID,)
    assert leftover_marker_ids(WILDAD_ACTUATION_GOAL) == (WILDAD_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert WILDAD_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(WILDAD_ACTUATION_GOAL)
...
def test_opted_in_wildad_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("wildad",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, WILDAD_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["wildad"]

    missing = run_wildad_workflow(with_wildid=False)
    skip_bind = run_wildad_workflow(skip_bind=True)
    skip_temporary = run_wildad_workflow(do_temporary=False)
    skip_public = run_wildad_workflow(do_public=False)
    skip_wilddigest = run_wildad_workflow(do_wilddigest=False)
    skip_replay = run_wildad_workflow(replay=False)
    skip_wildid = run_wildad_workflow(use_wildid=False)
    live = run_wildad_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_wildid"
    ...
    assert skip_wilddigest["ok"] is False
    assert skip_wilddigest["error"] == "wilddigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_wildid["ok"] is False
    assert skip_wildid["error"] == "wildid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_wilddigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["wilddigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["wildid_bound"] is True
    assert row["digest"]
    assert live["wildid"] == DEFAULT_WILDID
    assert live["wilddigest"] == DEFAULT_WILDDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, wildid=DEFAULT_WILDID, wilddigest=DEFAULT_WILDDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["wildid"] == DEFAULT_WILDID
    assert queried["wilddigest"] == DEFAULT_WILDDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == WILDAD_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, wildid=DEFAULT_WILDID, wilddigest=DEFAULT_WILDDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["wildid"] == DEFAULT_WILDID
    assert answered["wilddigest"] == DEFAULT_WILDDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, wildid=DEFAULT_WILDID, include_wildid=False)
    )
    assert bare["has_wildid"] is False
    assert bare["wildid"] == EMPTY_WILDID
    publicised = serialize_wildad(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_wildad(publicised) == DEFAULT_SOURCE
    assert parse_wildad(RFC_DEST_FIELD) == DEST_HOP
    header = parse_wildad_header(encode_wildad_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_WILDID, DEFAULT_WILDDIGEST))
    ...
    assert asked["method"] == "WILD"
    assert asked["disc_kind"] == "wildad"
    assert listed["disc_kind"] == "ad"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_WILDID).startswith("WILD")
    assert "wilddigest=" in canonical_public(SENTINEL, DEFAULT_WILDID, DEFAULT_WILDDIGEST)


def test_builtin_proof_seals_wildad_actuation() -> None:
    report = builtin_wildad_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "wildad_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_wildad"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_wildid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_wilddigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_wildid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_wilddigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_binds_wildad"]
    ...
    assert report["checks"]["catalog_names_pref64"]
    assert report["checks"]["catalog_names_nat64"]
    assert report["checks"]["catalog_names_dns64"]
    assert report["checks"]["catalog_names_xlat"]
    assert report["checks"]["catalog_names_disc"]
    assert report["checks"]["catalog_names_dslite"]
    assert report["checks"]["catalog_names_lw4o6"]
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["catalog_names_mapt"]
    assert report["checks"]["catalog_names_s46"]
    assert report["checks"]["catalog_names_ucpe"]
    assert report["checks"]["catalog_names_m46"]
    assert report["checks"]["catalog_names_prefix64"]
    assert report["checks"]["catalog_names_siit"]
    assert report["checks"]["catalog_names_eam"]
    assert report["checks"]["catalog_names_siitdc"]
    assert report["checks"]["catalog_names_siitdtm"]
    assert report["checks"]["catalog_names_v4embed"]
    assert report["checks"]["catalog_names_luprefix"]
    assert report["checks"]["catalog_names_sixrd"]
    assert report["checks"]["catalog_names_sixto4"]
    assert report["checks"]["catalog_names_teredo"]
    assert report["checks"]["catalog_names_isatap"]
    assert report["checks"]["catalog_names_sixover4"]
    assert report["checks"]["catalog_names_sixin4"]
    assert report["checks"]["catalog_names_tsp"]
    assert report["checks"]["catalog_names_l2tp"]
    assert report["checks"]["catalog_names_mesh"]
    assert report["checks"]["catalog_names_mpbgp"]
    ...
    assert report["checks"]["catalog_names_extcomm"]
    assert report["checks"]["catalog_names_largecomm"]
    assert report["checks"]["catalog_names_bgpsec"]
    assert report["checks"]["catalog_names_rtr"]
    assert report["checks"]["catalog_names_ebgp"]
    assert report["checks"]["catalog_names_evpn"]
    assert report["checks"]["catalog_names_etree"]
    assert report["checks"]["catalog_names_nvo"]
    assert report["checks"]["catalog_names_dfe"]
    assert report["checks"]["catalog_names_irb"]
    assert report["checks"]["catalog_names_ippfx"]
    assert report["checks"]["catalog_names_imlproxy"]
    assert report["checks"]["catalog_names_evpnbum"]
    assert report["checks"]["catalog_names_fxc"]
    assert report["checks"]["catalog_names_dfrec"]
    assert report["checks"]["catalog_names_msred"]
    assert report["checks"]["catalog_names_p2mpir"]
    assert report["checks"]["catalog_names_oir"]
    assert report["checks"]["catalog_names_iesi"]
    assert report["checks"]["catalog_names_pbb"]
    assert report["checks"]["catalog_names_macip"]
    assert report["checks"]["catalog_names_evpnreq"]
    assert report["checks"]["catalog_names_vpls"]
    assert report["checks"]["catalog_names_ldpsig"]
    assert report["checks"]["catalog_names_pwldp"]
    assert report["checks"]["catalog_names_pwe3"]
    assert report["checks"]["catalog_names_pwreq"]
    assert report["checks"]["catalog_names_mplsarch"]
    assert report["checks"]["catalog_names_mplslse"]
    assert report["checks"]["catalog_names_gmpls"]
    assert report["checks"]["catalog_names_lmp"]
    assert report["checks"]["catalog_names_lsphier"]
    assert report["checks"]["catalog_names_guni"]
    assert report["checks"]["catalog_names_lwdm"]
    assert report["checks"]["catalog_names_otn"]
    assert report["checks"]["catalog_names_ason"]
    assert report["checks"]["catalog_names_grec"]
    assert report["checks"]["catalog_names_e2erec"]
    assert report["checks"]["catalog_names_segrec"]
    assert report["checks"]["catalog_names_exroute"]
    assert report["checks"]["catalog_names_p2mpte"]
    assert report["checks"]["catalog_names_crankback"]
    assert report["checks"]["catalog_names_lspstitch"]
    assert report["checks"]["catalog_names_interas"]
    assert report["checks"]["catalog_names_perdom"]
    assert report["checks"]["catalog_names_pcep"]
    assert report["checks"]["catalog_names_brpc"]
    assert report["checks"]["catalog_names_dsct"]
    assert report["checks"]["catalog_names_pathkey"]
    assert report["checks"]["catalog_names_pcexcl"]
    assert report["checks"]["catalog_names_objfun"]
    assert report["checks"]["catalog_names_gco"]
    assert report["checks"]["catalog_names_ilpce"]
    assert report["checks"]["catalog_names_pcemon"]
    assert report["checks"]["catalog_names_wson"]
    assert report["checks"]["catalog_names_asbw"]
    assert report["checks"]["catalog_names_smp"]
    assert report["checks"]["catalog_names_fmoam"]
    assert report["checks"]["catalog_names_pcv"]
    assert report["checks"]["catalog_names_lilb"]
    assert report["checks"]["catalog_names_mldp"]
    assert report["checks"]["catalog_names_mvpn"]
    assert report["checks"]["catalog_names_pmsi"]
    assert report["checks"]["catalog_names_wildad"]
    assert report["checks"]["catalog_names_twfec"]
    assert report["checks"]["catalog_names_wson"]
    assert report["checks"]["leftover_text_binds_wildad"]
    assert report["checks"]["proved_wildad_consumes_leftover"]
    assert report["mission_goal"] == WILDAD_ACTUATION_GOAL
    assert report["done_when"] == WILDAD_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[WILDAD_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "wildad" in capability.tags
    assert "rfc6625" in capability.tags
    assert "http" in capability.tags
    assert "wildid" in capability.tags
    assert "wilddigest" in capability.tags
...
def test_selection_gate_rejects_ledger_only_wildad_outcome(tmp_path: Path) -> None:
    ...
    assert "wildad" in family.split("/")
    assert "rfc6625" in set(semantic_tokens(WILDAD_ACTUATION_GOAL))
    assert "wildid" in set(semantic_tokens(WILDAD_ACTUATION_GOAL))
    assert "wilddigest" in family
    assert "gmpl" not in family.split("/")
    assert "rfc3473" not in family
    assert "gmplsid" not in family
    assert "gmplsdigest" not in family
    assert "lmp" not in family.split("/")
    assert "rfc4204" not in family
    assert "lmpid" not in family
    assert "lmpdigest" not in family
    assert "dns64" not in family.split("/")
    assert "rfc6147" not in family
    assert "dns64id" not in family
    assert "dns64digest" not in family
    ...
    assert "nat64" not in family.split("/")
    assert "rfc6146" not in family
    assert "nat64id" not in family
    assert "nat64digest" not in family
    assert "siitdc" not in family.split("/")
    assert "rfc7755" not in family
    assert "siitdcid" not in family.split("/")
    assert "siitdcdigest" not in family.split("/")
    assert "siitdtm" not in family.split("/")
    assert "rfc7756" not in family
    assert "siitdtmid" not in family.split("/")
    assert "siitdtmdigest" not in family.split("/")
    assert "v4embed" not in family.split("/")
    assert "rfc6052" not in family
    assert "v4embedid" not in family.split("/")
    assert "v4embeddigest" not in family.split("/")
    assert "luprefix" not in family.split("/")
    assert "rfc8215" not in family
    assert "luprefixid" not in family.split("/")
    assert "luprefixdigest" not in family.split("/")
    assert "sixrd" not in family.split("/")
    assert "rfc5969" not in family
    assert "sixrdid" not in family.split("/")
    assert "sixrddigest" not in family.split("/")
    assert "sixto4" not in family.split("/")
    assert "rfc3056" not in family
    assert "sixto4id" not in family.split("/")
    assert "sixto4digest" not in family.split("/")
    assert "teredo" not in family.split("/")
    assert "rfc4380" not in family
    assert "teredoid" not in family
    assert "teredodigest" not in family
    assert "sixover4" not in family.split("/")
    assert "rfc2529" not in family
    assert "sixover4id" not in family
    assert "sixover4digest" not in family
    assert "sixin4" not in family.split("/")
    assert "rfc4213" not in family
    assert "sixin4id" not in family
    assert "sixin4digest" not in family
    assert "tsp" not in family.split("/")
    ...
    assert "l2tpdigest" not in family
    assert "mesh" not in family.split("/")
    assert "rfc5565" not in family
    assert "meshid" not in family.split("/")
    assert "meshdigest" not in family.split("/")
    assert "encap" not in family.split("/")
    assert "rfc5512" not in family
    assert "encapid" not in family
    assert "encapdigest" not in family
    assert "mpbgp" not in family.split("/")
    ...
    assert "extcomm" not in family.split("/")
    assert "rfc4360" not in family
    assert "extcommid" not in family
    assert "extcommdigest" not in family
    assert "largecomm" not in family.split("/")
    assert "rfc8092" not in family
    assert "largecommid" not in family
    assert "largecommdigest" not in family
    assert "rtr" not in family.split("/")
    assert "rfc8210" not in family
    assert "rtrid" not in family
    assert "rtrdigest" not in family
    assert "ebgp" not in family.split("/")
    assert "rfc8212" not in family
    assert "ebgpid" not in family
    assert "ebgpdigest" not in family
    assert "rfc8214" not in family
    assert "evpnid" not in family
    assert "evpndigest" not in family
    assert "ad/vpws" not in family
    assert "etree" not in family.split("/")
    assert "rfc8317" not in family
    assert "etreeid" not in family
    assert "etreedigest" not in family
    assert "nvo" not in family.split("/")
    assert "rfc8365" not in family
    assert "nvoid" not in family
    assert "nvodigest" not in family
    assert "dfe" not in family.split("/")
    assert "rfc8584" not in family
    assert "dfeid" not in family
    assert "dfedigest" not in family
    assert "irb" not in family.split("/")
    assert "rfc9135" not in family
    assert "irbid" not in family
    assert "irbdigest" not in family
    assert "ippfx" not in family.split("/")
    assert "rfc9136" not in family
    assert "ippfxid" not in family
    assert "ippfxdigest" not in family
    assert "proxynd" not in family.split("/")
    assert "rfc9161" not in family
    assert "proxyndid" not in family
    assert "proxynddigest" not in family
    assert "imlproxy" not in family.split("/")
    assert "rfc9251" not in family
    assert "imlproxyid" not in family
    assert "imlproxydigest" not in family
    assert "evpnbum" not in family.split("/")
    assert "rfc9572" not in family
    assert "evpnbumid" not in family
    assert "evpnbumdigest" not in family
    assert "fxc" not in family.split("/")
    assert "rfc9625" not in family
    assert "fxcid" not in family
    assert "fxcdigest" not in family
    assert "dfrec" not in family.split("/")
    assert "rfc9722" not in family
    assert "dfrecid" not in family
    assert "dfrecdigest" not in family
    assert "msred" not in family.split("/")
    assert "rfc9856" not in family
    assert "msredid" not in family
    assert "msreddigest" not in family
    assert "p2mpir" not in family.split("/")
    assert "rfc10018" not in family
    assert "p2mpirid" not in family
    assert "p2mpirdigest" not in family
    assert "oir" not in family.split("/")
    assert "rfc9574" not in family
    assert "oirid" not in family
    assert "oirdigest" not in family
    assert "iesi" not in family.split("/")
    assert "rfc9014" not in family
    assert "iesiid" not in family
    assert "iesidigest" not in family
    assert "mplsarch" not in family.split("/")
    assert "rfc3031" not in family
    assert "mplsid" not in family.split("/")
    assert "mplsdigest" not in family.split("/")
    assert "mplslse" not in family.split("/")
    assert "rfc3032" not in family
    assert "lseid" not in family
    assert "lsedigest" not in family
    assert "lsphier" not in family.split("/")
    assert "rfc4206" not in family
    assert "lsphierid" not in family
    assert "lsphierdigest" not in family
    assert "guni" not in family.split("/")
    assert "rfc4208" not in family
    assert "guniid" not in family
    assert "gunidigest" not in family
    assert "lwdm" not in family.split("/")
    assert "rfc4209" not in family
    assert "lwdmid" not in family
    assert "lwdmdigest" not in family
    assert "otn" not in family.split("/")
    assert "rfc4328" not in family
    assert "otnid" not in family
    assert "otndigest" not in family
    assert "ason" not in family.split("/")
    assert "rfc4397" not in family
    assert "asonid" not in family
    assert "asondigest" not in family
    assert "grec" not in family.split("/")
    assert "rfc4426" not in family
    assert "grecid" not in family.split("/")
    assert "grecdigest" not in family.split("/")
    assert "e2erec" not in family.split("/")
    assert "rfc4872" not in family
    assert "e2erecid" not in family
    assert "e2erecdigest" not in family
    assert "segrec" not in family.split("/")
    assert "rfc4873" not in family
    assert "segrecid" not in family
    assert "segrecdigest" not in family
    assert "p2mpte" not in family.split("/")
    assert "rfc4875" not in family
    assert "p2mpteid" not in family
    assert "p2mptedigest" not in family
    assert "crankback" not in family.split("/")
    assert "rfc4920" not in family
    assert "crankbackid" not in family
    assert "crankbackdigest" not in family
    assert "lspstitch" not in family.split("/")
    assert "rfc5150" not in family
    assert "lspstitchid" not in family
    assert "lspstitchdigest" not in family
    assert "intera" not in family.split("/")
    assert "rfc5151" not in family
    assert "interasid" not in family
    assert "interasdigest" not in family
    assert "perdom" not in family.split("/")
    assert "rfc5152" not in family
    assert "perdomid" not in family
    assert "perdomdigest" not in family
    assert "pcep" not in family.split("/")
    assert "rfc5440" not in family
    assert "pcepid" not in family
    assert "pcepdigest" not in family
    assert "brpc" not in family.split("/")
    assert "rfc5441" not in family
    assert "brpcid" not in family
    assert "brpcdigest" not in family
    assert "dsct" not in family.split("/")
    assert "rfc5455" not in family
    assert "dsctid" not in family
    assert "dsctdigest" not in family
    assert "pcexcl" not in family.split("/")
    assert "pathkey" not in family.split("/")
    assert "rfc5520" not in family
    assert "pathkeyid" not in family
    assert "pathkeydigest" not in family
    assert "pcexcl" not in family.split("/")
    assert "rfc5521" not in family
    assert "pcexclid" not in family
    assert "pcexcldigest" not in family
    assert "objfun" not in family.split("/")
    assert "rfc5541" not in family
    assert "objfunid" not in family
    assert "objfundigest" not in family
    assert "gco" not in family.split("/")
    assert "rfc5557" not in family
    assert "gcoid" not in family
    assert "gcodigest" not in family
    assert "ilpce" not in family.split("/")
    assert "rfc5623" not in family
    assert "ilpceid" not in family
    assert "ilpcedigest" not in family
    assert "pcemon" not in family.split("/")
    assert "rfc5886" not in family
    assert "pcemonid" not in family
    assert "pcemondigest" not in family
    assert "pcepmp" not in family.split("/")
    assert "rfc6006" not in family
    assert "pcepmpid" not in family
    assert "pcepmpdigest" not in family
    assert "twfec" not in family.split("/")
    assert "rfc6667" not in family
    assert "twfecid" not in family
    assert "twfecdigest" not in family
    assert "pmsi" not in family.split("/")
    assert "rfc6514" not in family
    assert "pmsiid" not in family
    assert "pmsidigest" not in family
    assert "mvpn" not in family.split("/")
    assert "rfc6513" not in family
    assert "mvpnid" not in family
    assert "mvpndigest" not in family
    assert "mldp" not in family.split("/")
    assert "rfc6512" not in family
    assert "mldpid" not in family
    assert "mldpdigest" not in family
    assert "pcv" not in family.split("/")
    assert "rfc6428" not in family
    assert "pcvid" not in family
    assert "pcvdigest" not in family
    assert "fmoam" not in family.split("/")
    assert "rfc6427" not in family
    assert "fmoamid" not in family
    assert "fmoamdigest" not in family
    assert "smp" not in family.split("/")
    assert "rfc6388" not in family
    assert "smpid" not in family
    assert "smpdigest" not in family
    assert "asbw" not in family.split("/")
    assert "rfc6387" not in family
    assert "asbwid" not in family
    assert "asbwdigest" not in family
    assert "lsc" not in family.split("/")
    assert "rfc6205" not in family
    assert "lscid" not in family
    assert "lscdigest" not in family
    assert "wson" not in family.split("/")
    assert "rfc6163" not in family
    assert "wsonid" not in family
    assert "wsondigest" not in family
    assert "exroute" not in family.split("/")
    assert "rfc4874" not in family
    assert "exrouteid" not in family
    assert "exroutedigest" not in family
    assert "rsvpte" not in family.split("/")
    assert "rfc3209" not in family
    assert "rsvpteid" not in family
    assert "rsvptedigest" not in family
    assert "pwreq" not in family.split("/")
    assert "rfc3916" not in family
    assert "pwreqid" not in family
    assert "pwreqdigest" not in family
    assert "pwe3" not in family.split("/")
    assert "rfc3985" not in family
    assert "pwe3id" not in family
    assert "pwe3digest" not in family
    assert "pwldp" not in family.split("/")
    assert "rfc4447" not in family
    assert "pwldpid" not in family
    assert "pwldpdigest" not in family
    assert "ldpsig" not in family.split("/")
    assert "rfc4762" not in family
    assert "ldpsigid" not in family
    assert "ldpsigdigest" not in family
    assert "vpls" not in family.split("/")
    assert "rfc4761" not in family
    assert "vplsid" not in family
    assert "vplsdigest" not in family
    assert "evpnreq" not in family.split("/")
    assert "rfc7209" not in family
    assert "evpnreqid" not in family
    assert "evpnreqdigest" not in family
    assert "macip" not in family.split("/")
    assert "rfc7432" not in family
    assert "macipid" not in family
    assert "macipdigest" not in family
    assert "pbb" not in family.split("/")
    assert "rfc7623" not in family
    assert "pbbid" not in family
    assert "pbbdigest" not in family
    assert "bgpsec" not in family.split("/")
    assert "rfc8205" not in family
    assert "bgpsecid" not in family
    assert "bgpsecdigest" not in family
    assert "isatap" not in family.split("/")
    assert "rfc5214" not in family
    assert "isatapid" not in family
    assert "isatapdigest" not in family
    assert "eam" not in family.split("/")
    assert "rfc7757" not in family
    assert "eamid" not in family.split("/")
    assert "eamdigest" not in family.split("/")
    assert "siit" not in family.split("/")
    assert "rfc7915" not in family
    assert "siitid" not in family
    assert "siitdigest" not in family
    assert "m46" not in family.split("/")
    assert "rfc8114" not in family
    assert "m46id" not in family
    assert "m46digest" not in family
    assert "ucpe" not in family.split("/")
    assert "rfc8026" not in family
    assert "ucpeid" not in family
    assert "ucpedigest" not in family
    assert "s46" not in family.split("/")
    assert "rfc7598" not in family
    assert "s46id" not in family
    assert "s46digest" not in family
    assert "mapt" not in family.split("/")
    assert "rfc7599" not in family
    assert "maptid" not in family
    assert "maptdigest" not in family
    assert "mape" not in family.split("/")
    assert "rfc7597" not in family
    assert "mapeid" not in family
    assert "mapedigest" not in family
    assert "lw4o6" not in family.split("/")
    assert "rfc7596" not in family
    assert "lw4o6id" not in family
    assert "lw4o6digest" not in family
    assert "dslite" not in family.split("/")
    assert "rfc6333" not in family
    assert "dsliteid" not in family
    assert "dslitedigest" not in family
    assert "disc" not in family.split("/")
    assert "rfc7050" not in family
    assert "discid" not in family
    assert "discdigest" not in family
    assert "xlat" not in family.split("/")
    assert "rfc6877" not in family
    assert "clatid" not in family
    assert "clatdigest" not in family
    assert "pref64" not in family.split("/")
    assert "rfc8781" not in family
    assert "pref64id" not in family
    assert "pref64digest" not in family
    assert "rdnss" not in family.split("/")
    assert "rfc8106" not in family
    assert "rdnssid" not in family
    assert "rdnssdigest" not in family
    assert "firsthop" not in family.split("/")
    assert "rfc8028" not in family
    assert "hopid" not in family
    assert "hopdigest" not in family
    ...
    assert "ipid" not in family.split("/")
    ...
    assert "rfc826" not in family
    assert "rfc791" not in family.split("/")
    ...
    assert "ipdigest" not in family.split("/")
