...
from blackhole_agent.httpver_actuation import (
    ...
    SENTINEL,
    VERSION_HEADER,
    builtin_httpver_actuation_proof,
    canonical_interpret,
    canonical_version,
    crc32c,
    encode_interpret,
    encode_version,
    encode_httpver_header,
    independent_httpver_digest,
    parse_http_request,
    parse_http_response,
    parse_message,
    parse_httpver,
    parse_httpver_header,
    interpret_request,
    interpret_response,
    run_httpver_workflow,
    serialize_httpver,
    version_request,
    version_response,
)
...
def test_goal_binds_httpver_actuation_plane() -> None:
    assert leftover_marker_ids(HTTPVER_ACTUATION_GOAL) == (HTTPVER_ACTUATION_ID,)
    assert leftover_marker_ids(HTTPVER_LEFTOVER) == (HTTPVER_ACTUATION_ID,)
    assert leftover_marker_ids(HTTPVER_ACTUATION_GOAL) == (HTTPVER_ACTUATION_ID,)
    ...
    assert HTTPVER_ACTUATION_ID in LOCAL_DENYLIST
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert HTTPVER_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(HTTPVER_ACTUATION_GOAL)
...
def test_opted_in_httpver_tool_completes_query_hit_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("httpver",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, HTTPVER_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["httpver"]

    missing = run_httpver_workflow(with_versionid=False)
    skip_bind = run_httpver_workflow(skip_bind=True)
    skip_version = run_httpver_workflow(do_version=False)
    skip_interpret = run_httpver_workflow(do_interpret=False)
    skip_versiondigest = run_httpver_workflow(do_versiondigest=False)
    skip_replay = run_httpver_workflow(replay=False)
    skip_versionid = run_httpver_workflow(use_versionid=False)
    live = run_httpver_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_versionid"
    assert skip_bind["ok"] is False
    assert skip_bind["error"] == "not_bound"
    assert skip_version["ok"] is False
    assert skip_version["error"] == "version_required"
    assert skip_interpret["ok"] is False
    assert skip_interpret["error"] == "interpret_required"
    assert skip_versiondigest["ok"] is False
    assert skip_versiondigest["error"] == "versiondigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_versionid["ok"] is False
    assert skip_versionid["error"] == "versionid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_httpver_digest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["version_frame"] is True
    assert row["interpret_frame"] is True
    assert row["versiondigest_response"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["versionid_bound"] is True
    assert row["digest"]
    assert live["versionid"] == DEFAULT_VERSIONID
    assert live["versiondigest"] == DEFAULT_VERSIONDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_version(identity=SENTINEL, versionid=DEFAULT_VERSIONID, versiondigest=DEFAULT_VERSIONDIGEST)
    )
    assert queried["is_version"] is True and queried["is_response"] is False
    assert queried["identity"] == SENTINEL and queried["versionid"] == DEFAULT_VERSIONID
    assert queried["versiondigest"] == DEFAULT_VERSIONDIGEST
    assert queried["type"] == FRAME_VERSION
    assert queried["first_byte"] == HTTPVER_FIRST
    answered = parse_message(
        encode_interpret(identity=SENTINEL, versionid=DEFAULT_VERSIONID, versiondigest=DEFAULT_VERSIONDIGEST)
    )
    assert answered["is_interpret"] is True and answered["is_response"] is True
    assert answered["versionid"] == DEFAULT_VERSIONID
    assert answered["versiondigest"] == DEFAULT_VERSIONDIGEST
    ...
    bare = parse_message(
        encode_version(identity=SENTINEL, versionid=DEFAULT_VERSIONID, include_versionid=False)
    )
    assert bare["has_versionid"] is False
    assert bare["versionid"] == EMPTY_VERSIONID
    advertised = serialize_httpver(DEFAULT_VERSION)
    assert advertised == RFC_VERSION_FIELD
    assert parse_httpver(advertised) == DEFAULT_VERSION
    assert parse_httpver(RFC_INTERPRET_FIELD) == INTERPRET_POLICY
    header = parse_httpver_header(encode_httpver_header(DEFAULT_VERSION))
    assert header["field_value"] == RFC_VERSION_FIELD
    assert header["header"] == VERSION_HEADER
    asked = parse_http_request(version_request(SENTINEL, DEFAULT_VERSIONID))
    listed = parse_http_request(interpret_request(SENTINEL, DEFAULT_VERSIONID, DEFAULT_VERSIONDIGEST))
    ...
    assert asked["method"] == "VERSION"
    assert asked["httpver_kind"] == "version"
    assert listed["httpver_kind"] == "interpret"
    assert got["status"] == 200
    assert preload_reply["status"] == 200
    assert got["policy"] == DEFAULT_VERSION
    assert preload_reply["policy"] == INTERPRET_POLICY
    assert canonical_version(SENTINEL, DEFAULT_VERSIONID).startswith("VERSION")
    assert "versiondigest=" in canonical_interpret(SENTINEL, DEFAULT_VERSIONID, DEFAULT_VERSIONDIGEST)


def test_builtin_proof_seals_httpver_actuation() -> None:
    report = builtin_httpver_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "httpver_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_httpver"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_versionid_is_forbidden"]
    assert report["checks"]["skip_version_stays_empty"]
    assert report["checks"]["skip_interpret_stays_empty"]
    assert report["checks"]["skip_versiondigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_versionid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_versiondigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_httpver"]
    assert report["checks"]["exhausted_catalog_stays_unbound_without_gate_passing_successor"]
    assert report["checks"]["catalog_names_httpver"]
    assert report["checks"]["catalog_names_httpstate"]
    assert report["checks"]["leftover_text_binds_httpver"]
    assert report["checks"]["proved_httpver_consumes_leftover"]
    assert report["mission_goal"] == HTTPVER_ACTUATION_GOAL
    assert report["done_when"] == HTTPVER_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[HTTPVER_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "httpver" in capability.tags
    assert "rfc2145" in capability.tags
    assert "http" in capability.tags
    assert "versionid" in capability.tags
    assert "versiondigest" in capability.tags
    assert "version" in capability.tags


def test_selection_gate_rejects_ledger_only_icp_family(tmp_path: Path) -> None:
    ...
    assert "httpver" in family
    assert "rfc2145" in family
    assert "versionid" in family
    assert "versiondigest" in family
