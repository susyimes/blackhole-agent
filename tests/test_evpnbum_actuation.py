...
from blackhole_agent.evpnbum_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_evpnbum_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_evpnbum_header,
    independent_evpnbumdigest,
    parse_http_comm,
    parse_http_response,
    parse_message,
    parse_evpnbum,
    parse_evpnbum_header,
    public_comm,
    public_response,
    run_evpnbum_workflow,
    serialize_evpnbum,
    temporary_comm,
    temporary_response,
)
...
def test_goal_binds_evpnbum_actuation_plane() -> None:
    assert leftover_marker_ids(EVPNBUM_ACTUATION_GOAL) == (EVPNBUM_ACTUATION_ID,)
    ...
    assert EVPNBUM_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(EVPNBUM_LEFTOVER) == (EVPNBUM_ACTUATION_ID,)
    assert leftover_marker_ids(EVPNBUM_ACTUATION_GOAL) == (EVPNBUM_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert EVPNBUM_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(EVPNBUM_ACTUATION_GOAL)
...
def test_opted_in_evpnbum_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("evpnbum",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, EVPNBUM_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["evpnbum"]

    missing = run_evpnbum_workflow(with_evpnbumid=False)
    skip_bind = run_evpnbum_workflow(skip_bind=True)
    skip_temporary = run_evpnbum_workflow(do_temporary=False)
    skip_public = run_evpnbum_workflow(do_public=False)
    skip_evpnbumdigest = run_evpnbum_workflow(do_evpnbumdigest=False)
    skip_replay = run_evpnbum_workflow(replay=False)
    skip_evpnbumid = run_evpnbum_workflow(use_evpnbumid=False)
    live = run_evpnbum_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_evpnbumid"
    ...
    assert skip_evpnbumdigest["ok"] is False
    assert skip_evpnbumdigest["error"] == "evpnbumdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_evpnbumid["ok"] is False
    assert skip_evpnbumid["error"] == "evpnbumid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_evpnbumdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["evpnbumdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["evpnbumid_bound"] is True
    assert row["digest"]
    assert live["evpnbumid"] == DEFAULT_EVPNBUMID
    assert live["evpnbumdigest"] == DEFAULT_EVPNBUMDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, evpnbumid=DEFAULT_EVPNBUMID, evpnbumdigest=DEFAULT_EVPNBUMDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["evpnbumid"] == DEFAULT_EVPNBUMID
    assert queried["evpnbumdigest"] == DEFAULT_EVPNBUMDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == EVPNBUM_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, evpnbumid=DEFAULT_EVPNBUMID, evpnbumdigest=DEFAULT_EVPNBUMDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["evpnbumid"] == DEFAULT_EVPNBUMID
    assert answered["evpnbumdigest"] == DEFAULT_EVPNBUMDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, evpnbumid=DEFAULT_EVPNBUMID, include_evpnbumid=False)
    )
    assert bare["has_evpnbumid"] is False
    assert bare["evpnbumid"] == EMPTY_EVPNBUMID
    publicised = serialize_evpnbum(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_evpnbum(publicised) == DEFAULT_SOURCE
    assert parse_evpnbum(RFC_DEST_FIELD) == DEST_HOP
    header = parse_evpnbum_header(encode_evpnbum_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_EVPNBUMID, DEFAULT_EVPNBUMDIGEST))
    ...
    assert asked["method"] == "SMET"
    assert asked["disc_kind"] == "evpnbum"
    assert listed["disc_kind"] == "imet"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_EVPNBUMID).startswith("SMET")
    assert "evpnbumdigest=" in canonical_public(SENTINEL, DEFAULT_EVPNBUMID, DEFAULT_EVPNBUMDIGEST)


def test_builtin_proof_seals_evpnbum_actuation() -> None:
    report = builtin_evpnbum_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "evpnbum_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_evpnbum"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_evpnbumid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_evpnbumdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_evpnbumid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_evpnbumdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_evpnbum"]
    ...
    assert report["checks"]["leftover_text_binds_evpnbum"]
    assert report["checks"]["proved_evpnbum_consumes_leftover"]
    assert report["mission_goal"] == EVPNBUM_ACTUATION_GOAL
    assert report["done_when"] == EVPNBUM_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[EVPNBUM_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "evpnbum" in capability.tags
    assert "rfc9572" in capability.tags
    assert "http" in capability.tags
    assert "evpnbumid" in capability.tags
    assert "evpnbumdigest" in capability.tags
    assert "evpnbum" in capability.tags
    assert "imet" in capability.tags


def test_selection_gate_rejects_ledger_only_evpnbum_family(tmp_path: Path) -> None:
    ...
    assert "evpnbum" in family.split("/")
    assert "rfc9572" in set(semantic_tokens(EVPNBUM_ACTUATION_GOAL))
    assert "evpnbumid" in set(semantic_tokens(EVPNBUM_ACTUATION_GOAL))
    assert "evpnbumdigest" in family
