...
def test_goal_binds_etree_actuation_plane() -> None:
    assert leftover_marker_ids(ETREE_ACTUATION_GOAL) == (ETREE_ACTUATION_ID,)
    ...
    assert ETREE_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(ETREE_LEFTOVER) == (ETREE_ACTUATION_ID,)
    assert leftover_marker_ids(ETREE_ACTUATION_GOAL) == (ETREE_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert ETREE_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(ETREE_ACTUATION_GOAL)
...
def test_opted_in_etree_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("etree",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, ETREE_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["etree"]

    missing = run_etree_workflow(with_etreeid=False)
    skip_bind = run_etree_workflow(skip_bind=True)
    skip_temporary = run_etree_workflow(do_temporary=False)
    skip_public = run_etree_workflow(do_public=False)
    skip_etreedigest = run_etree_workflow(do_etreedigest=False)
    skip_replay = run_etree_workflow(replay=False)
    skip_etreeid = run_etree_workflow(use_etreeid=False)
    live = run_etree_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_etreeid"
    ...
    assert skip_etreedigest["ok"] is False
    assert skip_etreedigest["error"] == "etreedigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_etreeid["ok"] is False
    assert skip_etreeid["error"] == "etreeid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_etreedigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["etreedigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["etreeid_bound"] is True
    assert row["digest"]
    assert live["etreeid"] == DEFAULT_ETREEID
    assert live["etreedigest"] == DEFAULT_ETREEDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, etreeid=DEFAULT_ETREEID, etreedigest=DEFAULT_ETREEDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["etreeid"] == DEFAULT_ETREEID
    assert queried["etreedigest"] == DEFAULT_ETREEDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == ETREE_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, etreeid=DEFAULT_ETREEID, etreedigest=DEFAULT_ETREEDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["etreeid"] == DEFAULT_ETREEID
    assert answered["etreedigest"] == DEFAULT_ETREEDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, etreeid=DEFAULT_ETREEID, include_etreeid=False)
    )
    assert bare["has_etreeid"] is False
    assert bare["etreeid"] == EMPTY_ETREEID
    publicised = serialize_etree(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_etree(publicised) == DEFAULT_SOURCE
    assert parse_etree(RFC_DEST_FIELD) == DEST_HOP
    header = parse_etree_header(encode_etree_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_ETREEID, DEFAULT_ETREEDIGEST))
    ...
    assert asked["method"] == "ROOT"
    assert asked["disc_kind"] == "etree"
    assert listed["disc_kind"] == "leaf"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_ETREEID).startswith("ROOT")
    assert "etreedigest=" in canonical_public(SENTINEL, DEFAULT_ETREEID, DEFAULT_ETREEDIGEST)


def test_builtin_proof_seals_etree_actuation() -> None:
    report = builtin_etree_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "etree_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_etree"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_etreeid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_etreedigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_etreeid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_etreedigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_etree"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_etree"]
    assert report["checks"]["proved_etree_consumes_leftover"]
    assert report["mission_goal"] == ETREE_ACTUATION_GOAL
    assert report["done_when"] == ETREE_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[ETREE_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "etree" in capability.tags
    assert "rfc8317" in capability.tags
    assert "http" in capability.tags
    assert "etreeid" in capability.tags
    assert "etreedigest" in capability.tags
    assert "etree" in capability.tags
    assert "leaf" in capability.tags


def test_selection_gate_rejects_ledger_only_etree_family(tmp_path: Path) -> None:
    ...
    assert "etree" in family.split("/")
    assert "rfc8317" in family
    assert "etreeid" in set(semantic_tokens(ETREE_ACTUATION_GOAL))
    assert "etreedigest" in family
