...
from blackhole_agent.spnego_actuation import (
    ...
    AUTHENTICATE_POLICY,
    RFC_NEGOTIATE_FIELD,
    RFC_AUTHENTICATE_FIELD,
    SENTINEL,
    NEGOTIATE_HEADER,
    builtin_spnego_actuation_proof,
    canonical_authenticate,
    canonical_negotiate,
    crc32c,
    encode_authenticate,
    encode_negotiate,
    encode_spnego_header,
    independent_spnego_digest,
    parse_http_request,
    parse_http_response,
    parse_message,
    parse_spnego,
...
def test_goal_binds_spnego_actuation_plane() -> None:
    assert leftover_marker_ids(SPNEGO_ACTUATION_GOAL) == (SPNEGO_ACTUATION_ID,)
    assert leftover_marker_ids(SPNEGO_LEFTOVER) == (SPNEGO_ACTUATION_ID,)
    assert leftover_marker_ids(HTTPTLS_ACTUATION_GOAL) == (HTTPTLS_ACTUATION_ID,)
    ...
    assert HTTPTLS_ACTUATION_ID in LOCAL_DENYLIST
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert SPNEGO_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(SPNEGO_ACTUATION_GOAL)
    spnego_signature = semantic_signature(SPNEGO_ACTUATION_GOAL)
    for neighbor in NEIGHBORS:
        assert semantic_similarity(spnego_signature, semantic_signature(neighbor)) < 0.82


def test_opted_in_spnego_tool_completes_negotiate_lock_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("spnego",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, SPNEGO_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["spnego"]

    missing = run_spnego_workflow(with_negotiateid=False)
    skip_bind = run_spnego_workflow(skip_bind=True)
    skip_negotiate = run_spnego_workflow(do_negotiate=False)
    skip_authenticate = run_spnego_workflow(do_authenticate=False)
    skip_negotiatedigest = run_spnego_workflow(do_negotiatedigest=False)
    skip_replay = run_spnego_workflow(replay=False)
    skip_negotiateid = run_spnego_workflow(use_negotiateid=False)
    live = run_spnego_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_negotiateid"
    assert skip_bind["ok"] is False
    assert skip_bind["error"] == "not_bound"
    assert skip_negotiate["ok"] is False
    assert skip_negotiate["error"] == "negotiate_required"
    assert skip_authenticate["ok"] is False
    assert skip_authenticate["error"] == "authenticate_required"
    assert skip_negotiatedigest["ok"] is False
    assert skip_negotiatedigest["error"] == "negotiatedigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_negotiateid["ok"] is False
    assert skip_negotiateid["error"] == "negotiateid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_spnego_digest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["negotiate_frame"] is True
    assert row["authenticate_frame"] is True
    assert row["negotiatedigest_response"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["negotiateid_bound"] is True
    assert row["digest"]
    assert live["negotiateid"] == DEFAULT_NEGOTIATEID
    assert live["negotiatedigest"] == DEFAULT_NEGOTIATEDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_negotiate(identity=SENTINEL, negotiateid=DEFAULT_NEGOTIATEID, negotiatedigest=DEFAULT_NEGOTIATEDIGEST)
    )
    assert queried["is_negotiate"] is True and queried["is_response"] is False
    assert queried["identity"] == SENTINEL and queried["negotiateid"] == DEFAULT_NEGOTIATEID
    assert queried["negotiatedigest"] == DEFAULT_NEGOTIATEDIGEST
    assert queried["type"] == FRAME_NEGOTIATE
    assert queried["first_byte"] == NEGOTIATE_FIRST
    answered = parse_message(
        encode_authenticate(identity=SENTINEL, negotiateid=DEFAULT_NEGOTIATEID, negotiatedigest=DEFAULT_NEGOTIATEDIGEST)
    )
    assert answered["is_authenticate"] is True and answered["is_response"] is True
    assert answered["negotiateid"] == DEFAULT_NEGOTIATEID
    assert answered["negotiatedigest"] == DEFAULT_NEGOTIATEDIGEST
    ...
    bare = parse_message(
        encode_negotiate(identity=SENTINEL, negotiateid=DEFAULT_NEGOTIATEID, include_negotiateid=False)
    )
    assert bare["has_negotiateid"] is False
    assert bare["negotiateid"] == EMPTY_NEGOTIATEID
    advertised = serialize_spnego(DEFAULT_NEGOTIATE)
    assert advertised == RFC_NEGOTIATE_FIELD
    assert parse_spnego(advertised) == DEFAULT_NEGOTIATE
    assert parse_spnego(RFC_AUTHENTICATE_FIELD) == AUTHENTICATE_POLICY
    header = parse_spnego_header(encode_spnego_header(DEFAULT_NEGOTIATE))
    assert header["field_value"] == RFC_NEGOTIATE_FIELD
    assert header["header"] == NEGOTIATE_HEADER
    asked = parse_http_request(negotiate_request(SENTINEL, DEFAULT_NEGOTIATEID))
    listed = parse_http_request(authenticate_request(SENTINEL, DEFAULT_NEGOTIATEID, DEFAULT_NEGOTIATEDIGEST))
    ...
    assert asked["method"] == "NEGOTIATE"
    assert asked["spnego_kind"] == "negotiate"
    assert listed["spnego_kind"] == "authenticate"
    assert got["status"] == 200
    assert preload_reply["status"] == 200
    assert got["policy"] == DEFAULT_NEGOTIATE
    assert preload_reply["policy"] == AUTHENTICATE_POLICY
    assert canonical_negotiate(SENTINEL, DEFAULT_NEGOTIATEID).startswith("NEGOTIATE")
    assert "negotiatedigest=" in canonical_authenticate(SENTINEL, DEFAULT_NEGOTIATEID, DEFAULT_NEGOTIATEDIGEST)


def test_builtin_proof_seals_spnego_actuation() -> None:
    report = builtin_spnego_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "spnego_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_spnego"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_negotiateid_is_forbidden"]
    assert report["checks"]["skip_negotiate_stays_empty"]
    assert report["checks"]["skip_authenticate_stays_empty"]
    assert report["checks"]["skip_negotiatedigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_negotiateid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_negotiatedigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_spnego"]
    ...
    assert report["checks"]["catalog_names_httptls"]
    assert report["checks"]["leftover_text_binds_spnego"]
    assert report["checks"]["proved_spnego_consumes_leftover"]
    assert report["mission_goal"] == SPNEGO_ACTUATION_GOAL
    assert report["done_when"] == SPNEGO_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[SPNEGO_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "spnego" in capability.tags
    assert "rfc4559" in capability.tags
    assert "http" in capability.tags
    assert "negotiateid" in capability.tags
    assert "negotiatedigest" in capability.tags
    assert "negotiate" in capability.tags


def test_selection_gate_rejects_ledger_only_spnego_family(tmp_path: Path) -> None:
    ...
    assert "spnego" in family
    assert "rfc4559" in family
    assert "negotiateid" in family
    assert "negotiatedigest" in family
    ...
    assert "httptls" not in family
    assert "rfc2817" not in family
    assert "upgradeid" not in family
    assert "upgradetlsdigest" not in family
