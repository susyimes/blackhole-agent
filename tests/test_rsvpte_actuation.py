...
from blackhole_agent.rsvpte_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_rsvpte_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_rsvpte_header,
    independent_rsvptedigest,
    parse_http_comm,
    parse_http_response,
    parse_message,
    parse_rsvpte,
...
def test_goal_binds_rsvpte_actuation_plane() -> None:
    assert leftover_marker_ids(RSVPTE_ACTUATION_GOAL) == (RSVPTE_ACTUATION_ID,)
    ...
    assert RSVPTE_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(RSVPTE_LEFTOVER) == (RSVPTE_ACTUATION_ID,)
    assert leftover_marker_ids(RSVPTE_ACTUATION_GOAL) == (RSVPTE_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert RSVPTE_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(RSVPTE_ACTUATION_GOAL)
...
def test_opted_in_rsvpte_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("rsvpte",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, RSVPTE_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["rsvpte"]

    missing = run_rsvpte_workflow(with_rsvpteid=False)
    skip_bind = run_rsvpte_workflow(skip_bind=True)
    skip_temporary = run_rsvpte_workflow(do_temporary=False)
    skip_public = run_rsvpte_workflow(do_public=False)
    skip_rsvptedigest = run_rsvpte_workflow(do_rsvptedigest=False)
    skip_replay = run_rsvpte_workflow(replay=False)
    skip_rsvpteid = run_rsvpte_workflow(use_rsvpteid=False)
    live = run_rsvpte_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_rsvpteid"
    ...
    assert skip_rsvptedigest["ok"] is False
    assert skip_rsvptedigest["error"] == "rsvptedigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_rsvpteid["ok"] is False
    assert skip_rsvpteid["error"] == "rsvpteid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_rsvptedigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["rsvptedigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["rsvpteid_bound"] is True
    assert row["digest"]
    assert live["rsvpteid"] == DEFAULT_RSVPTEID
    assert live["rsvptedigest"] == DEFAULT_RSVPTEDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, rsvpteid=DEFAULT_RSVPTEID, rsvptedigest=DEFAULT_RSVPTEDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["rsvpteid"] == DEFAULT_RSVPTEID
    assert queried["rsvptedigest"] == DEFAULT_RSVPTEDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == RSVPTE_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, rsvpteid=DEFAULT_RSVPTEID, rsvptedigest=DEFAULT_RSVPTEDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["rsvpteid"] == DEFAULT_RSVPTEID
    assert answered["rsvptedigest"] == DEFAULT_RSVPTEDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, rsvpteid=DEFAULT_RSVPTEID, include_rsvpteid=False)
    )
    assert bare["has_rsvpteid"] is False
    assert bare["rsvpteid"] == EMPTY_RSVPTEID
    publicised = serialize_rsvpte(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_rsvpte(publicised) == DEFAULT_SOURCE
    assert parse_rsvpte(RFC_DEST_FIELD) == DEST_HOP
    header = parse_rsvpte_header(encode_rsvpte_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_RSVPTEID, DEFAULT_RSVPTEDIGEST))
    ...
    assert listed["disc_kind"] == "resv"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_RSVPTEID).startswith("PATH")
    assert "rsvptedigest=" in canonical_public(SENTINEL, DEFAULT_RSVPTEID, DEFAULT_RSVPTEDIGEST)


def test_builtin_proof_seals_rsvpte_actuation() -> None:
    report = builtin_rsvpte_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "rsvpte_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_rsvpte"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_rsvpteid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_rsvptedigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_rsvpteid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_rsvptedigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_rsvpte"]
    ...
    assert report["checks"]["catalog_names_rsvpte"]
    assert report["checks"]["catalog_names_gmpls"]
    assert report["checks"]["leftover_text_binds_rsvpte"]
    assert report["checks"]["proved_rsvpte_consumes_leftover"]
    assert report["mission_goal"] == RSVPTE_ACTUATION_GOAL
    assert report["done_when"] == RSVPTE_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[RSVPTE_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "rsvpte" in capability.tags
    assert "rfc3209" in capability.tags
    assert "http" in capability.tags
    assert "rsvpteid" in capability.tags
    assert "rsvptedigest" in capability.tags
    assert "path" in capability.tags
    assert "resv" in capability.tags


def test_selection_gate_rejects_ledger_only_rsvpte_family(tmp_path: Path) -> None:
    ...
    assert "rsvpte" in family.split("/")
    assert "rfc3209" in set(semantic_tokens(RSVPTE_ACTUATION_GOAL))
    assert "rsvpteid" in set(semantic_tokens(RSVPTE_ACTUATION_GOAL))
    assert "rsvptedigest" in family
