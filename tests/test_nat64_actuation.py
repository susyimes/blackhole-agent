...
def test_goal_binds_nat64_actuation_plane() -> None:
    ...
    assert leftover_marker_ids(NAT64_LEFTOVER) == (NAT64_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(RDNSS_ACTUATION_GOAL) == (RDNSS_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert NAT64_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(NAT64_ACTUATION_GOAL)
...
def test_opted_in_nat64_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("nat64",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, NAT64_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["nat64"]

    missing = run_nat64_workflow(with_nat64id=False)
    skip_bind = run_nat64_workflow(skip_bind=True)
    skip_temporary = run_nat64_workflow(do_temporary=False)
    skip_public = run_nat64_workflow(do_public=False)
    skip_nat64digest = run_nat64_workflow(do_nat64digest=False)
    skip_replay = run_nat64_workflow(replay=False)
    skip_nat64id = run_nat64_workflow(use_nat64id=False)
    live = run_nat64_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_nat64id"
    ...
    assert skip_nat64digest["ok"] is False
    assert skip_nat64digest["error"] == "nat64digest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_nat64id["ok"] is False
    assert skip_nat64id["error"] == "nat64id_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_nat64digest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["nat64digest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["nat64id_bound"] is True
    assert row["digest"]
    assert live["nat64id"] == DEFAULT_NAT64ID
    assert live["nat64digest"] == DEFAULT_NAT64DIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, nat64id=DEFAULT_NAT64ID, nat64digest=DEFAULT_NAT64DIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["nat64id"] == DEFAULT_NAT64ID
    assert queried["nat64digest"] == DEFAULT_NAT64DIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == NAT64_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, nat64id=DEFAULT_NAT64ID, nat64digest=DEFAULT_NAT64DIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["nat64id"] == DEFAULT_NAT64ID
    assert answered["nat64digest"] == DEFAULT_NAT64DIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, nat64id=DEFAULT_NAT64ID, include_nat64id=False)
    )
    assert bare["has_nat64id"] is False
    assert bare["nat64id"] == EMPTY_NAT64ID
    publicised = serialize_nat64(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_nat64(publicised) == DEFAULT_SOURCE
    assert parse_nat64(RFC_DEST_FIELD) == DEST_HOP
    header = parse_nat64_header(encode_nat64_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_NAT64ID, DEFAULT_NAT64DIGEST))
    ...
    assert asked["method"] == "NAT64"
    assert asked["nat64_kind"] == "nat64"
    assert listed["nat64_kind"] == "session"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_NAT64ID).startswith("NAT64")
    assert "nat64digest=" in canonical_public(SENTINEL, DEFAULT_NAT64ID, DEFAULT_NAT64DIGEST)


def test_builtin_proof_seals_nat64_actuation() -> None:
    report = builtin_nat64_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "nat64_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_nat64"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_nat64id_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_nat64digest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_nat64id_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_nat64digest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_nat64"]
    ...
    assert report["checks"]["leftover_text_binds_nat64"]
    assert report["checks"]["proved_nat64_consumes_leftover"]
    assert report["mission_goal"] == NAT64_ACTUATION_GOAL
    assert report["done_when"] == NAT64_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[NAT64_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "nat64" in capability.tags
    assert "rfc6146" in capability.tags
    assert "http" in capability.tags
    assert "nat64id" in capability.tags
    assert "nat64digest" in capability.tags
    assert "nat64" in capability.tags
    assert "session" in capability.tags


def test_selection_gate_rejects_ledger_only_nat64_family(tmp_path: Path) -> None:
    ...
    assert "nat64" in family.split("/")
    assert "rfc6146" in family
    assert "nat64id" in family
    assert "nat64digest" in family
