...
from blackhole_agent.sixover4_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_sixover4_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_sixover4_header,
    independent_sixover4digest,
    parse_http_request,
    parse_http_response,
    parse_message,
    parse_sixover4,
    parse_sixover4_header,
    public_request,
    public_response,
    run_sixover4_workflow,
    serialize_sixover4,
    temporary_request,
    temporary_response,
)
...
def test_goal_binds_sixover4_actuation_plane() -> None:
    assert leftover_marker_ids(SIXOVER4_ACTUATION_GOAL) == (SIXOVER4_ACTUATION_ID,)
    ...
    assert SIXOVER4_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(SIXOVER4_LEFTOVER) == (SIXOVER4_ACTUATION_ID,)
    assert leftover_marker_ids(SIXOVER4_ACTUATION_GOAL) == (SIXOVER4_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert SIXOVER4_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(SIXOVER4_ACTUATION_GOAL)
...
def test_opted_in_sixover4_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("sixover4",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, SIXOVER4_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["sixover4"]

    missing = run_sixover4_workflow(with_sixover4id=False)
    skip_bind = run_sixover4_workflow(skip_bind=True)
    skip_temporary = run_sixover4_workflow(do_temporary=False)
    skip_public = run_sixover4_workflow(do_public=False)
    skip_sixover4digest = run_sixover4_workflow(do_sixover4digest=False)
    skip_replay = run_sixover4_workflow(replay=False)
    skip_sixover4id = run_sixover4_workflow(use_sixover4id=False)
    live = run_sixover4_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_sixover4id"
    ...
    assert skip_sixover4digest["ok"] is False
    assert skip_sixover4digest["error"] == "sixover4digest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_sixover4id["ok"] is False
    assert skip_sixover4id["error"] == "sixover4id_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_sixover4digest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["sixover4digest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["sixover4id_bound"] is True
    assert row["digest"]
    assert live["sixover4id"] == DEFAULT_SIXOVER4ID
    assert live["sixover4digest"] == DEFAULT_SIXOVER4DIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, sixover4id=DEFAULT_SIXOVER4ID, sixover4digest=DEFAULT_SIXOVER4DIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["sixover4id"] == DEFAULT_SIXOVER4ID
    assert queried["sixover4digest"] == DEFAULT_SIXOVER4DIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == SIXOVER4_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, sixover4id=DEFAULT_SIXOVER4ID, sixover4digest=DEFAULT_SIXOVER4DIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["sixover4id"] == DEFAULT_SIXOVER4ID
    assert answered["sixover4digest"] == DEFAULT_SIXOVER4DIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, sixover4id=DEFAULT_SIXOVER4ID, include_sixover4id=False)
    )
    assert bare["has_sixover4id"] is False
    assert bare["sixover4id"] == EMPTY_SIXOVER4ID
    publicised = serialize_sixover4(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_sixover4(publicised) == DEFAULT_SOURCE
    assert parse_sixover4(RFC_DEST_FIELD) == DEST_HOP
    header = parse_sixover4_header(encode_sixover4_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_SIXOVER4ID, DEFAULT_SIXOVER4DIGEST))
    ...
    assert asked["method"] == "6OVER4"
    assert asked["disc_kind"] == "sixover4"
    assert listed["disc_kind"] == "mcast"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_SIXOVER4ID).startswith("6OVER4")
    assert "sixover4digest=" in canonical_public(SENTINEL, DEFAULT_SIXOVER4ID, DEFAULT_SIXOVER4DIGEST)


def test_builtin_proof_seals_sixover4_actuation() -> None:
    report = builtin_sixover4_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "sixover4_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_sixover4"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_sixover4id_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_sixover4digest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_sixover4id_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_sixover4digest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_sixover4"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_sixover4"]
    assert report["checks"]["proved_sixover4_consumes_leftover"]
    assert report["mission_goal"] == SIXOVER4_ACTUATION_GOAL
    assert report["done_when"] == SIXOVER4_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[SIXOVER4_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "sixover4" in capability.tags
    assert "rfc2529" in capability.tags
    assert "http" in capability.tags
    assert "sixover4id" in capability.tags
    assert "sixover4digest" in capability.tags
    assert "sixover4" in capability.tags
