...
def test_goal_binds_mesh_actuation_plane() -> None:
    assert leftover_marker_ids(MESH_ACTUATION_GOAL) == (MESH_ACTUATION_ID,)
    ...
    assert MESH_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(MESH_LEFTOVER) == (MESH_ACTUATION_ID,)
    assert leftover_marker_ids(MESH_ACTUATION_GOAL) == (MESH_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert MESH_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(MESH_ACTUATION_GOAL)
...
def test_opted_in_mesh_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("mesh",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, MESH_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["mesh"]

    missing = run_mesh_workflow(with_meshid=False)
    skip_bind = run_mesh_workflow(skip_bind=True)
    skip_temporary = run_mesh_workflow(do_temporary=False)
    skip_public = run_mesh_workflow(do_public=False)
    skip_meshdigest = run_mesh_workflow(do_meshdigest=False)
    skip_replay = run_mesh_workflow(replay=False)
    skip_meshid = run_mesh_workflow(use_meshid=False)
    live = run_mesh_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_meshid"
    ...
    assert skip_meshdigest["ok"] is False
    assert skip_meshdigest["error"] == "meshdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_meshid["ok"] is False
    assert skip_meshid["error"] == "meshid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_meshdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["meshdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["meshid_bound"] is True
    assert row["digest"]
    assert live["meshid"] == DEFAULT_MESHID
    assert live["meshdigest"] == DEFAULT_MESHDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, meshid=DEFAULT_MESHID, meshdigest=DEFAULT_MESHDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["meshid"] == DEFAULT_MESHID
    assert queried["meshdigest"] == DEFAULT_MESHDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == MESH_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, meshid=DEFAULT_MESHID, meshdigest=DEFAULT_MESHDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["meshid"] == DEFAULT_MESHID
    assert answered["meshdigest"] == DEFAULT_MESHDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, meshid=DEFAULT_MESHID, include_meshid=False)
    )
    assert bare["has_meshid"] is False
    assert bare["meshid"] == EMPTY_MESHID
    publicised = serialize_mesh(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_mesh(publicised) == DEFAULT_SOURCE
    assert parse_mesh(RFC_DEST_FIELD) == DEST_HOP
    header = parse_mesh_header(encode_mesh_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_MESHID, DEFAULT_MESHDIGEST))
    ...
    assert asked["method"] == "MESH"
    assert asked["disc_kind"] == "mesh"
    assert listed["disc_kind"] == "peer"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_MESHID).startswith("MESH")
    assert "meshdigest=" in canonical_public(SENTINEL, DEFAULT_MESHID, DEFAULT_MESHDIGEST)


def test_builtin_proof_seals_mesh_actuation() -> None:
    report = builtin_mesh_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "mesh_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_mesh"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_meshid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_meshdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_meshid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_meshdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_mesh"]
    ...
    assert report["checks"]["catalog_names_encap"]
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_mesh"]
    assert report["checks"]["proved_mesh_consumes_leftover"]
    assert report["mission_goal"] == MESH_ACTUATION_GOAL
    assert report["done_when"] == MESH_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[MESH_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "mesh" in capability.tags
    assert "rfc5565" in capability.tags
    assert "http" in capability.tags
    assert "meshid" in capability.tags
    assert "meshdigest" in capability.tags
    assert "mesh" in capability.tags
    assert "peer" in capability.tags


def test_selection_gate_rejects_ledger_only_mesh_family(tmp_path: Path) -> None:
    ...
    assert "mesh" in family.split("/")
    assert "meshid" in set(semantic_tokens(MESH_ACTUATION_GOAL))
    assert "meshdigest" in family
