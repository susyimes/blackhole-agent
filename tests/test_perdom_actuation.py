...
from blackhole_agent.perdom_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_perdom_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_perdom_header,
    independent_perdomdigest,
    parse_http_comm,
    parse_http_response,
    parse_message,
    parse_perdom,
...
def test_goal_binds_perdom_actuation_plane() -> None:
    assert leftover_marker_ids(PERDOM_ACTUATION_GOAL) == (PERDOM_ACTUATION_ID,)
    ...
    assert PERDOM_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(PERDOM_LEFTOVER) == (PERDOM_ACTUATION_ID,)
    assert leftover_marker_ids(PERDOM_ACTUATION_GOAL) == (PERDOM_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert PERDOM_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(PERDOM_ACTUATION_GOAL)
...
def test_opted_in_perdom_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("perdom",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, PERDOM_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["perdom"]

    missing = run_perdom_workflow(with_perdomid=False)
    skip_bind = run_perdom_workflow(skip_bind=True)
    skip_temporary = run_perdom_workflow(do_temporary=False)
    skip_public = run_perdom_workflow(do_public=False)
    skip_perdomdigest = run_perdom_workflow(do_perdomdigest=False)
    skip_replay = run_perdom_workflow(replay=False)
    skip_perdomid = run_perdom_workflow(use_perdomid=False)
    live = run_perdom_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_perdomid"
    ...
    assert skip_perdomdigest["ok"] is False
    assert skip_perdomdigest["error"] == "perdomdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_perdomid["ok"] is False
    assert skip_perdomid["error"] == "perdomid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_perdomdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["perdomdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["perdomid_bound"] is True
    assert row["digest"]
    assert live["perdomid"] == DEFAULT_PERDOMID
    assert live["perdomdigest"] == DEFAULT_PERDOMDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, perdomid=DEFAULT_PERDOMID, perdomdigest=DEFAULT_PERDOMDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["perdomid"] == DEFAULT_PERDOMID
    assert queried["perdomdigest"] == DEFAULT_PERDOMDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == PERDOM_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, perdomid=DEFAULT_PERDOMID, perdomdigest=DEFAULT_PERDOMDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["perdomid"] == DEFAULT_PERDOMID
    assert answered["perdomdigest"] == DEFAULT_PERDOMDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, perdomid=DEFAULT_PERDOMID, include_perdomid=False)
    )
    assert bare["has_perdomid"] is False
    assert bare["perdomid"] == EMPTY_PERDOMID
    publicised = serialize_perdom(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_perdom(publicised) == DEFAULT_SOURCE
    assert parse_perdom(RFC_DEST_FIELD) == DEST_HOP
    header = parse_perdom_header(encode_perdom_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_PERDOMID, DEFAULT_PERDOMDIGEST))
    ...
    assert asked["method"] == "COMPUTE"
    assert asked["disc_kind"] == "compute"
    assert listed["disc_kind"] == "domain"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_PERDOMID).startswith("COMPUTE")
    assert "perdomdigest=" in canonical_public(SENTINEL, DEFAULT_PERDOMID, DEFAULT_PERDOMDIGEST)


def test_builtin_proof_seals_perdom_actuation() -> None:
    report = builtin_perdom_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "perdom_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_perdom"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_perdomid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_perdomdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_perdomid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_perdomdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_perdom"]
    ...
    assert report["checks"]["leftover_text_binds_perdom"]
    assert report["checks"]["proved_perdom_consumes_leftover"]
    assert report["mission_goal"] == PERDOM_ACTUATION_GOAL
    assert report["done_when"] == PERDOM_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[PERDOM_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "perdom" in capability.tags
    assert "rfc5152" in capability.tags
    assert "http" in capability.tags
    assert "perdomid" in capability.tags
    assert "perdomdigest" in capability.tags
    assert "compute" in capability.tags
    assert "domain" in capability.tags


def test_selection_gate_rejects_ledger_only_perdom_family(tmp_path: Path) -> None:
    ...
    assert "perdom" in family.split("/")
    assert "rfc5152" in set(semantic_tokens(PERDOM_ACTUATION_GOAL))
    assert "perdomid" in set(semantic_tokens(PERDOM_ACTUATION_GOAL))
    assert "perdomdigest" in family
