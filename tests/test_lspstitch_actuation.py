...
from blackhole_agent.lspstitch_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_lspstitch_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_lspstitch_header,
    independent_lspstitchdigest,
    parse_http_comm,
    parse_http_response,
    parse_message,
    parse_lspstitch,
    parse_lspstitch_header,
    public_comm,
    public_response,
    run_lspstitch_workflow,
    serialize_lspstitch,
    temporary_comm,
    temporary_response,
)
...
def test_goal_binds_lspstitch_actuation_plane() -> None:
    assert leftover_marker_ids(LSPSTITCH_ACTUATION_GOAL) == (LSPSTITCH_ACTUATION_ID,)
    ...
    assert LSPSTITCH_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(LSPSTITCH_LEFTOVER) == (LSPSTITCH_ACTUATION_ID,)
    assert leftover_marker_ids(LSPSTITCH_ACTUATION_GOAL) == (LSPSTITCH_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert LSPSTITCH_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(LSPSTITCH_ACTUATION_GOAL)
...
def test_opted_in_lspstitch_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("lspstitch",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, LSPSTITCH_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["lspstitch"]

    missing = run_lspstitch_workflow(with_lspstitchid=False)
    skip_bind = run_lspstitch_workflow(skip_bind=True)
    skip_temporary = run_lspstitch_workflow(do_temporary=False)
    skip_public = run_lspstitch_workflow(do_public=False)
    skip_lspstitchdigest = run_lspstitch_workflow(do_lspstitchdigest=False)
    skip_replay = run_lspstitch_workflow(replay=False)
    skip_lspstitchid = run_lspstitch_workflow(use_lspstitchid=False)
    live = run_lspstitch_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_lspstitchid"
    ...
    assert skip_lspstitchdigest["ok"] is False
    assert skip_lspstitchdigest["error"] == "lspstitchdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_lspstitchid["ok"] is False
    assert skip_lspstitchid["error"] == "lspstitchid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_lspstitchdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["lspstitchdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["lspstitchid_bound"] is True
    assert row["digest"]
    assert live["lspstitchid"] == DEFAULT_LSPSTITCHID
    assert live["lspstitchdigest"] == DEFAULT_LSPSTITCHDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, lspstitchid=DEFAULT_LSPSTITCHID, lspstitchdigest=DEFAULT_LSPSTITCHDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["lspstitchid"] == DEFAULT_LSPSTITCHID
    assert queried["lspstitchdigest"] == DEFAULT_LSPSTITCHDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == LSPSTITCH_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, lspstitchid=DEFAULT_LSPSTITCHID, lspstitchdigest=DEFAULT_LSPSTITCHDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["lspstitchid"] == DEFAULT_LSPSTITCHID
    assert answered["lspstitchdigest"] == DEFAULT_LSPSTITCHDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, lspstitchid=DEFAULT_LSPSTITCHID, include_lspstitchid=False)
    )
    assert bare["has_lspstitchid"] is False
    assert bare["lspstitchid"] == EMPTY_LSPSTITCHID
    publicised = serialize_lspstitch(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_lspstitch(publicised) == DEFAULT_SOURCE
    assert parse_lspstitch(RFC_DEST_FIELD) == DEST_HOP
    header = parse_lspstitch_header(encode_lspstitch_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_LSPSTITCHID, DEFAULT_LSPSTITCHDIGEST))
    ...
    assert asked["method"] == "STITCH"
    assert asked["disc_kind"] == "stitch"
    assert listed["disc_kind"] == "join"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_LSPSTITCHID).startswith("STITCH")
    assert "lspstitchdigest=" in canonical_public(SENTINEL, DEFAULT_LSPSTITCHID, DEFAULT_LSPSTITCHDIGEST)


def test_builtin_proof_seals_lspstitch_actuation() -> None:
    report = builtin_lspstitch_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "lspstitch_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_lspstitch"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_lspstitchid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_lspstitchdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_lspstitchid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_lspstitchdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_lspstitch"]
    ...
    assert report["checks"]["leftover_text_binds_lspstitch"]
    assert report["checks"]["proved_lspstitch_consumes_leftover"]
    assert report["mission_goal"] == LSPSTITCH_ACTUATION_GOAL
    assert report["done_when"] == LSPSTITCH_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[LSPSTITCH_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "lspstitch" in capability.tags
    assert "rfc5150" in capability.tags
    assert "http" in capability.tags
    assert "lspstitchid" in capability.tags
    assert "lspstitchdigest" in capability.tags
    assert "stitch" in capability.tags
    assert "join" in capability.tags


def test_selection_gate_rejects_ledger_only_lspstitch_family(tmp_path: Path) -> None:
    ...
    assert "lspstitch" in family.split("/")
    assert "rfc5150" in set(semantic_tokens(LSPSTITCH_ACTUATION_GOAL))
    assert "lspstitchid" in set(semantic_tokens(LSPSTITCH_ACTUATION_GOAL))
    assert "lspstitchdigest" in family
    ...
    assert "interas" not in family.split("/")
