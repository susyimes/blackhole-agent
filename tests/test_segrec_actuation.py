...
from blackhole_agent.segrec_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_segrec_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_segrec_header,
    independent_segrecdigest,
    parse_http_comm,
    parse_http_response,
    parse_message,
    parse_segrec,
...
def test_goal_binds_segrec_actuation_plane() -> None:
    assert leftover_marker_ids(SEGREC_ACTUATION_GOAL) == (SEGREC_ACTUATION_ID,)
    ...
    assert SEGREC_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(SEGREC_LEFTOVER) == (SEGREC_ACTUATION_ID,)
    assert leftover_marker_ids(SEGREC_ACTUATION_GOAL) == (SEGREC_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert SEGREC_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(SEGREC_ACTUATION_GOAL)
...
def test_opted_in_segrec_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("segrec",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, SEGREC_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["segrec"]

    missing = run_segrec_workflow(with_segrecid=False)
    skip_bind = run_segrec_workflow(skip_bind=True)
    skip_temporary = run_segrec_workflow(do_temporary=False)
    skip_public = run_segrec_workflow(do_public=False)
    skip_segrecdigest = run_segrec_workflow(do_segrecdigest=False)
    skip_replay = run_segrec_workflow(replay=False)
    skip_segrecid = run_segrec_workflow(use_segrecid=False)
    live = run_segrec_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_segrecid"
    ...
    assert skip_segrecdigest["ok"] is False
    assert skip_segrecdigest["error"] == "segrecdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_segrecid["ok"] is False
    assert skip_segrecid["error"] == "segrecid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_segrecdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["segrecdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["segrecid_bound"] is True
    assert row["digest"]
    assert live["segrecid"] == DEFAULT_SEGRECID
    assert live["segrecdigest"] == DEFAULT_SEGRECDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, segrecid=DEFAULT_SEGRECID, segrecdigest=DEFAULT_SEGRECDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["segrecid"] == DEFAULT_SEGRECID
    assert queried["segrecdigest"] == DEFAULT_SEGRECDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == SEGREC_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, segrecid=DEFAULT_SEGRECID, segrecdigest=DEFAULT_SEGRECDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["segrecid"] == DEFAULT_SEGRECID
    assert answered["segrecdigest"] == DEFAULT_SEGRECDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, segrecid=DEFAULT_SEGRECID, include_segrecid=False)
    )
    assert bare["has_segrecid"] is False
    assert bare["segrecid"] == EMPTY_SEGRECID
    publicised = serialize_segrec(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_segrec(publicised) == DEFAULT_SOURCE
    assert parse_segrec(RFC_DEST_FIELD) == DEST_HOP
    header = parse_segrec_header(encode_segrec_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_SEGRECID, DEFAULT_SEGRECDIGEST))
    ...
    assert asked["method"] == "SEGMENT"
    assert asked["disc_kind"] == "segment"
    assert listed["disc_kind"] == "recover"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_SEGRECID).startswith("SEGMENT")
    assert "segrecdigest=" in canonical_public(SENTINEL, DEFAULT_SEGRECID, DEFAULT_SEGRECDIGEST)


def test_builtin_proof_seals_segrec_actuation() -> None:
    report = builtin_segrec_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "segrec_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_segrec"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_segrecid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_segrecdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_segrecid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_segrecdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_segrec"]
    ...
    assert report["checks"]["leftover_text_binds_segrec"]
    assert report["checks"]["proved_segrec_consumes_leftover"]
    assert report["mission_goal"] == SEGREC_ACTUATION_GOAL
    assert report["done_when"] == SEGREC_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[SEGREC_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "segrec" in capability.tags
    assert "rfc4873" in capability.tags
    assert "http" in capability.tags
    assert "segrecid" in capability.tags
    assert "segrecdigest" in capability.tags
    assert "segment" in capability.tags
    assert "recover" in capability.tags


def test_selection_gate_rejects_ledger_only_segrec_family(tmp_path: Path) -> None:
    ...
    assert "segrec" in family.split("/")
    assert "rfc4873" in set(semantic_tokens(SEGREC_ACTUATION_GOAL))
    assert "segrecid" in set(semantic_tokens(SEGREC_ACTUATION_GOAL))
    assert "segrecdigest" in family
