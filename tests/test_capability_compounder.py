...
def test_mandate_plane_orders_and_adversarial():
    """Mandate plane posts multi-privilege orders and falsifies wrong-privilege binds."""
    ...
    from blackhole_agent.capability_compounder import (
        ...
        parse_outcome_contract,
        run_mandate_plane,
        verify_mandate_bundle_integrity,
    )

    repo = Path(__file__).resolve().parents[1]
    path, ledger = ensure_seeded_ledger(repo)
    assert "capability.mandate-plane" in ledger.capabilities
    assert "capability.privilege-plane" in ledger.capabilities

    parsed = parse_outcome_contract(
        "no_skill_route; mandate_ok; mandated_ok; min_mandates:2; "
        "mandate_root_valid; privilege_ok; privileged_ok; min_privileges:2; "
        "privilege_root_valid; chain_valid"
    )
    kinds = {item["kind"] for item in parsed["predicates"]}
    assert "mandate_ok" in kinds
    assert "mandated_ok" in kinds
    assert "min_mandates" in kinds
    assert "mandate_root_valid" in kinds

    privilege_path = repo / "artifacts" / "privilege-bundles" / "proof-privilege.json"
    assert privilege_path.is_file(), "requires existing privilege proof bundle"
    mandate_path = repo / "artifacts" / "mandate-bundles" / "test-mandate-plane.json"
    if mandate_path.exists():
        mandate_path.unlink()

    plane = run_mandate_plane(
        repo,
        "mandate over privilege",
        "min_capabilities:5; capability_exists:repo.import-health; no_skill_route",
        max_steps=3,
        run_privilege=False,
    ...
    assert plane["ok"] is True, plane
    assert plane["action"] == "mandate_plane"
    assert plane["mandated"] is True
    assert int(plane["mandate_count"]) >= 2
    assert int(plane["tip_height"]) >= 2
    assert plane.get("mandate_plan_digest")
    assert plane["integrity"]["ok"] is True
    assert plane["integrity"]["multi_mandate"] is True
    assert plane["integrity"]["mandate_ok"] is True
    assert plane["rehydrate"]["ok"] is True
    assert plane["prove"]["ok"] is True
    assert int(plane["prove"]["proved_count"]) >= 1
    assert plane["chain"]["valid"] is True
    assert plane["mandate_certificate"]["valid"] is True
    assert plane["adversarial"]["ok"] is True
    assert plane["adversarial"]["wrong_privilege_fails_as_expected"] is True
    assert plane["adversarial"]["reorder_fails_as_expected"] is True
    assert plane["adversarial"]["digest_tamper_fails_as_expected"] is True
    assert plane["adversarial"]["single_mandate_fails_as_expected"] is True
    assert plane["adversarial"]["duplicate_apply_fails_as_expected"] is True
    assert plane["adversarial"]["replay_matches_tip"] is True
    assert plane["used_skill_route_discovery"] is False
    assert mandate_path.is_file()

    loaded = load_mandate_bundle(mandate_path)
    assert verify_mandate_bundle_integrity(loaded)["ok"] is True
    assert loaded.get("mandate_hash")
    assert int(loaded.get("mandate_count") or 0) >= 2
    assert int(loaded.get("tip_height") or 0) >= 2
...
def test_union_plane_orders_and_adversarial():
    """Union plane posts multi-confederation orders and falsifies wrong-confederation binds."""
    ...
    from blackhole_agent.capability_compounder import (
        ...
        parse_outcome_contract,
        run_union_plane,
        verify_union_bundle_integrity,
    )

    repo = Path(__file__).resolve().parents[1]
    path, ledger = ensure_seeded_ledger(repo)
    assert "capability.union-plane" in ledger.capabilities
    assert "capability.confederation-plane" in ledger.capabilities

    parsed = parse_outcome_contract(
        "no_skill_route; union_ok; united_ok; min_unions:2; "
        "union_root_valid; confederation_ok; confederated_ok; min_confederations:2; "
        "confederation_root_valid; chain_valid"
    )
    kinds = {item["kind"] for item in parsed["predicates"]}
    assert "union_ok" in kinds
    assert "united_ok" in kinds
    assert "min_unions" in kinds
    assert "union_root_valid" in kinds

    confederation_path = (
        repo / "artifacts" / "confederation-bundles" / "proof-confederation.json"
    )
    assert confederation_path.is_file(), "requires existing confederation proof bundle"
    union_path = repo / "artifacts" / "union-bundles" / "test-union-plane.json"
    if union_path.exists():
        union_path.unlink()

    plane = run_union_plane(
        repo,
        "union over confederation",
        "min_capabilities:5; capability_exists:repo.import-health; no_skill_route",
        max_steps=3,
        run_confederation=False,
        confederation_path=confederation_path,
    ...
    assert plane["ok"] is True, plane
    assert plane["action"] == "union_plane"
    assert plane["united"] is True
    assert int(plane["union_count"]) >= 2
    assert int(plane["tip_height"]) >= 2
    assert plane.get("union_plan_digest")
    assert plane["integrity"]["ok"] is True
    assert plane["integrity"]["multi_union"] is True
    assert plane["rehydrate"]["ok"] is True
    assert plane["prove"]["ok"] is True
    assert int(plane["prove"]["proved_count"]) >= 1
    assert plane["chain"]["valid"] is True
    assert plane["union_certificate"]["valid"] is True
    assert plane["adversarial"]["ok"] is True
    assert plane["adversarial"]["wrong_confederation_fails_as_expected"] is True
    assert plane["adversarial"]["reorder_fails_as_expected"] is True
    assert plane["adversarial"]["digest_tamper_fails_as_expected"] is True
    assert plane["adversarial"]["single_union_fails_as_expected"] is True
    assert plane["adversarial"]["duplicate_apply_fails_as_expected"] is True
    assert plane["adversarial"]["replay_matches_tip"] is True
    assert plane["used_skill_route_discovery"] is False
    assert union_path.is_file()

    loaded = load_union_bundle(union_path)
    assert verify_union_bundle_integrity(loaded)["ok"] is True
    assert loaded.get("union_hash")
    assert int(loaded.get("union_count") or 0) >= 2
    assert int(loaded.get("tip_height") or 0) >= 2
    assert loaded.get("union_plan_digest")
    assert path.name == "ledger.json"



def test_commonwealth_plane_orders_and_adversarial():
    """Commonwealth plane posts multi-union orders and falsifies wrong-union binds."""

    from blackhole_agent.capability_compounder import (
        ensure_seeded_ledger,
        load_commonwealth_bundle,
        parse_outcome_contract,
        run_commonwealth_plane,
        verify_commonwealth_bundle_integrity,
    )
    ...
    parsed = parse_outcome_contract(
        "no_skill_route; commonwealth_ok; commonwealthed_ok; min_commonwealths:2; "
        "commonwealth_root_valid; union_ok; united_ok; min_unions:2; "
        "union_root_valid; chain_valid"
    )
    kinds = {item["kind"] for item in parsed["predicates"]}
    assert "commonwealth_ok" in kinds
    assert "commonwealthed_ok" in kinds
    assert "min_commonwealths" in kinds
    assert "commonwealth_root_valid" in kinds

    union_path = (
        repo / "artifacts" / "union-bundles" / "proof-union.json"
    )
    assert union_path.is_file(), "requires existing union proof bundle"
    commonwealth_path = (
        repo / "artifacts" / "commonwealth-bundles" / "test-commonwealth-plane.json"
    )
    if commonwealth_path.exists():
        commonwealth_path.unlink()

    plane = run_commonwealth_plane(
        repo,
        "commonwealth over union",
        "min_capabilities:5; capability_exists:repo.import-health; no_skill_route",
        max_steps=3,
        run_union=False,
        union_path=union_path,
    ...
    assert plane["ok"] is True, plane
    assert plane["action"] == "commonwealth_plane"
    assert plane["commonwealthed"] is True
    assert int(plane["commonwealth_count"]) >= 2
    assert int(plane["tip_height"]) >= 2
    assert plane.get("commonwealth_plan_digest")
    assert plane["integrity"]["ok"] is True
    assert plane["integrity"]["multi_commonwealth"] is True
    assert plane["rehydrate"]["ok"] is True
    assert plane["prove"]["ok"] is True
    assert int(plane["prove"]["proved_count"]) >= 1
    assert plane["chain"]["valid"] is True
    assert plane["commonwealth_certificate"]["valid"] is True
    assert plane["adversarial"]["ok"] is True
    assert plane["adversarial"]["wrong_union_fails_as_expected"] is True
    assert plane["adversarial"]["reorder_fails_as_expected"] is True
    assert plane["adversarial"]["digest_tamper_fails_as_expected"] is True
    assert plane["adversarial"]["single_commonwealth_fails_as_expected"] is True
    assert plane["adversarial"]["duplicate_apply_fails_as_expected"] is True
    assert plane["adversarial"]["replay_matches_tip"] is True
    assert plane["used_skill_route_discovery"] is False
    assert commonwealth_path.is_file()

    loaded = load_commonwealth_bundle(commonwealth_path)
    assert verify_commonwealth_bundle_integrity(loaded)["ok"] is True
    assert loaded.get("commonwealth_hash")
    assert int(loaded.get("commonwealth_count") or 0) >= 2
    assert int(loaded.get("tip_height") or 0) >= 2
...
def test_dominion_plane_orders_and_adversarial():
    """Dominion plane posts multi-empire orders and falsifies wrong-empire binds."""

    from blackhole_agent.capability_compounder import (
        ensure_seeded_ledger,
        load_dominion_bundle,
        parse_outcome_contract,
        run_dominion_plane,
        verify_dominion_bundle_integrity,
    )
    ...
    kinds = {item["kind"] for item in parsed["predicates"]}
    assert "dominion_ok" in kinds
    assert "dominioned_ok" in kinds
    assert "min_dominions" in kinds
    assert "dominion_root_valid" in kinds

    empire_path = (
        repo / "artifacts" / "empire-bundles" / "proof-empire.json"
    )
    assert empire_path.is_file(), "requires existing empire proof bundle"
    dominion_path = (
        repo / "artifacts" / "dominion-bundles" / "test-dominion-plane.json"
    )
    if dominion_path.exists():
        dominion_path.unlink()

    plane = run_dominion_plane(
        repo,
        "dominion over empire",
        "min_capabilities:5; capability_exists:repo.import-health; no_skill_route",
        max_steps=3,
        run_empire=False,
        empire_path=empire_path,
    ...
    assert plane["ok"] is True, plane
    assert plane["action"] == "dominion_plane"
    assert plane["dominioned"] is True
    assert int(plane["dominion_count"]) >= 2
    assert int(plane["tip_height"]) >= 2
    assert plane.get("dominion_plan_digest")
    assert plane["integrity"]["ok"] is True
    assert plane["integrity"]["multi_dominion"] is True
    assert plane["rehydrate"]["ok"] is True
    assert plane["prove"]["ok"] is True
    assert int(plane["prove"]["proved_count"]) >= 1
    assert plane["chain"]["valid"] is True
    assert plane["dominion_certificate"]["valid"] is True
    assert plane["adversarial"]["ok"] is True
    assert plane["adversarial"]["wrong_empire_fails_as_expected"] is True
    assert plane["adversarial"]["reorder_fails_as_expected"] is True
    assert plane["adversarial"]["digest_tamper_fails_as_expected"] is True
    assert plane["adversarial"]["single_dominion_fails_as_expected"] is True
    assert plane["adversarial"]["duplicate_apply_fails_as_expected"] is True
    assert plane["adversarial"]["replay_matches_tip"] is True
    assert plane["used_skill_route_discovery"] is False
    assert dominion_path.is_file()

    loaded = load_dominion_bundle(dominion_path)
    assert verify_dominion_bundle_integrity(loaded)["ok"] is True
    assert loaded.get("dominion_hash")
    assert int(loaded.get("dominion_count") or 0) >= 2
    assert int(loaded.get("tip_height") or 0) >= 2
