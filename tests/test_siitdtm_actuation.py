...
from blackhole_agent.siitdtm_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_siitdtm_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_siitdtm_header,
    independent_siitdtmdigest,
    parse_http_request,
    parse_http_response,
    parse_message,
    parse_siitdtm,
    parse_siitdtm_header,
    public_request,
    public_response,
    run_siitdtm_workflow,
    serialize_siitdtm,
    temporary_request,
    temporary_response,
)
...
def test_goal_binds_siitdtm_actuation_plane() -> None:
    assert leftover_marker_ids(SIITDTM_ACTUATION_GOAL) == (SIITDTM_ACTUATION_ID,)
    ...
    assert SIITDTM_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(SIITDTM_LEFTOVER) == (SIITDTM_ACTUATION_ID,)
    assert leftover_marker_ids(SIITDTM_ACTUATION_GOAL) == (SIITDTM_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert SIITDTM_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(SIITDTM_ACTUATION_GOAL)
...
def test_opted_in_siitdtm_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("siitdtm",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, SIITDTM_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["siitdtm"]

    missing = run_siitdtm_workflow(with_siitdtmid=False)
    skip_bind = run_siitdtm_workflow(skip_bind=True)
    skip_temporary = run_siitdtm_workflow(do_temporary=False)
    skip_public = run_siitdtm_workflow(do_public=False)
    skip_siitdtmdigest = run_siitdtm_workflow(do_siitdtmdigest=False)
    skip_replay = run_siitdtm_workflow(replay=False)
    skip_siitdtmid = run_siitdtm_workflow(use_siitdtmid=False)
    live = run_siitdtm_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_siitdtmid"
    ...
    assert skip_siitdtmdigest["ok"] is False
    assert skip_siitdtmdigest["error"] == "siitdtmdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_siitdtmid["ok"] is False
    assert skip_siitdtmid["error"] == "siitdtmid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_siitdtmdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["siitdtmdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["siitdtmid_bound"] is True
    assert row["digest"]
    assert live["siitdtmid"] == DEFAULT_SIITDTMID
    assert live["siitdtmdigest"] == DEFAULT_SIITDTMDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, siitdtmid=DEFAULT_SIITDTMID, siitdtmdigest=DEFAULT_SIITDTMDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["siitdtmid"] == DEFAULT_SIITDTMID
    assert queried["siitdtmdigest"] == DEFAULT_SIITDTMDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == SIITDTM_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, siitdtmid=DEFAULT_SIITDTMID, siitdtmdigest=DEFAULT_SIITDTMDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["siitdtmid"] == DEFAULT_SIITDTMID
    assert answered["siitdtmdigest"] == DEFAULT_SIITDTMDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, siitdtmid=DEFAULT_SIITDTMID, include_siitdtmid=False)
    )
    assert bare["has_siitdtmid"] is False
    assert bare["siitdtmid"] == EMPTY_SIITDTMID
    publicised = serialize_siitdtm(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_siitdtm(publicised) == DEFAULT_SOURCE
    assert parse_siitdtm(RFC_DEST_FIELD) == DEST_HOP
    header = parse_siitdtm_header(encode_siitdtm_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_SIITDTMID, DEFAULT_SIITDTMDIGEST))
    ...
    assert asked["method"] == "DTM"
    assert asked["disc_kind"] == "dtm"
    assert listed["disc_kind"] == "mode"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_SIITDTMID).startswith("DTM")
    assert "siitdtmdigest=" in canonical_public(SENTINEL, DEFAULT_SIITDTMID, DEFAULT_SIITDTMDIGEST)


def test_builtin_proof_seals_siitdtm_actuation() -> None:
    report = builtin_siitdtm_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "siitdtm_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_siitdtm"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_siitdtmid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_siitdtmdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_siitdtmid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_siitdtmdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_siitdtm"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_siitdtm"]
    assert report["checks"]["proved_siitdtm_consumes_leftover"]
    assert report["mission_goal"] == SIITDTM_ACTUATION_GOAL
    assert report["done_when"] == SIITDTM_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[SIITDTM_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "siitdtm" in capability.tags
    assert "rfc7756" in capability.tags
    assert "http" in capability.tags
    assert "siitdtmid" in capability.tags
    assert "siitdtmdigest" in capability.tags
    assert "dtm" in capability.tags
    assert "mode" in capability.tags


def test_selection_gate_rejects_ledger_only_siitdtm_family(tmp_path: Path) -> None:
    ...
    assert "siitdtm" in family.split("/")
    assert "rfc7756" in family
    assert "siitdtmid" in set(semantic_tokens(SIITDTM_ACTUATION_GOAL))
    assert "siitdtmdigest" in family
    ...
    assert "v4embedid" not in family
    assert "v4embeddigest" not in family
