...
def test_goal_binds_gco_actuation_plane() -> None:
    assert leftover_marker_ids(GCO_ACTUATION_GOAL) == (GCO_ACTUATION_ID,)
    ...
    assert GCO_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(GCO_LEFTOVER) == (GCO_ACTUATION_ID,)
    assert leftover_marker_ids(GCO_ACTUATION_GOAL) == (GCO_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert GCO_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(GCO_ACTUATION_GOAL)
...
def test_opted_in_gco_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("gco",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, GCO_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["gco"]

    missing = run_gco_workflow(with_gcoid=False)
    skip_bind = run_gco_workflow(skip_bind=True)
    skip_temporary = run_gco_workflow(do_temporary=False)
    skip_public = run_gco_workflow(do_public=False)
    skip_gcodigest = run_gco_workflow(do_gcodigest=False)
    skip_replay = run_gco_workflow(replay=False)
    skip_gcoid = run_gco_workflow(use_gcoid=False)
    live = run_gco_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_gcoid"
    ...
    assert skip_gcodigest["ok"] is False
    assert skip_gcodigest["error"] == "gcodigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_gcoid["ok"] is False
    assert skip_gcoid["error"] == "gcoid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_gcodigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["gcodigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["gcoid_bound"] is True
    assert row["digest"]
    assert live["gcoid"] == DEFAULT_GCOID
    assert live["gcodigest"] == DEFAULT_GCODIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, gcoid=DEFAULT_GCOID, gcodigest=DEFAULT_GCODIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["gcoid"] == DEFAULT_GCOID
    assert queried["gcodigest"] == DEFAULT_GCODIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == GCO_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, gcoid=DEFAULT_GCOID, gcodigest=DEFAULT_GCODIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["gcoid"] == DEFAULT_GCOID
    assert answered["gcodigest"] == DEFAULT_GCODIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, gcoid=DEFAULT_GCOID, include_gcoid=False)
    )
    assert bare["has_gcoid"] is False
    assert bare["gcoid"] == EMPTY_GCOID
    publicised = serialize_gco(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_gco(publicised) == DEFAULT_SOURCE
    assert parse_gco(RFC_DEST_FIELD) == DEST_HOP
    header = parse_gco_header(encode_gco_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_GCOID, DEFAULT_GCODIGEST))
    ...
    assert asked["method"] == "GCO"
    assert asked["disc_kind"] == "gco"
    assert listed["disc_kind"] == "svec"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_GCOID).startswith("GCO")
    assert "gcodigest=" in canonical_public(SENTINEL, DEFAULT_GCOID, DEFAULT_GCODIGEST)


def test_builtin_proof_seals_gco_actuation() -> None:
    report = builtin_gco_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "gco_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_gco"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_gcoid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_gcodigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_gcoid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_gcodigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_binds_gco"]
    ...
    assert report["checks"]["leftover_text_binds_gco"]
    assert report["checks"]["proved_gco_consumes_leftover"]
    assert report["mission_goal"] == GCO_ACTUATION_GOAL
    assert report["done_when"] == GCO_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[GCO_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "gco" in capability.tags
    assert "rfc5557" in capability.tags
    assert "http" in capability.tags
    assert "gcoid" in capability.tags
    assert "gcodigest" in capability.tags
    assert "gco" in capability.tags
    assert "svec" in capability.tags


def test_selection_gate_rejects_ledger_only_gco_family(tmp_path: Path) -> None:
    ...
    assert "gco" in family.split("/")
    assert "rfc5557" in set(semantic_tokens(GCO_ACTUATION_GOAL))
    assert "gcoid" in set(semantic_tokens(GCO_ACTUATION_GOAL))
    assert "gcodigest" in family
