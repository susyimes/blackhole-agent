...
from blackhole_agent.crankback_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_crankback_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_crankback_header,
    independent_crankbackdigest,
    parse_http_comm,
    parse_http_response,
    parse_message,
    parse_crankback,
    parse_crankback_header,
    public_comm,
    public_response,
    run_crankback_workflow,
    serialize_crankback,
    temporary_comm,
    temporary_response,
)
...
def test_goal_binds_crankback_actuation_plane() -> None:
    assert leftover_marker_ids(CRANKBACK_ACTUATION_GOAL) == (CRANKBACK_ACTUATION_ID,)
    ...
    assert CRANKBACK_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(CRANKBACK_LEFTOVER) == (CRANKBACK_ACTUATION_ID,)
    assert leftover_marker_ids(CRANKBACK_ACTUATION_GOAL) == (CRANKBACK_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert CRANKBACK_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(CRANKBACK_ACTUATION_GOAL)
...
def test_opted_in_crankback_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("crankback",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, CRANKBACK_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["crankback"]

    missing = run_crankback_workflow(with_crankbackid=False)
    skip_bind = run_crankback_workflow(skip_bind=True)
    skip_temporary = run_crankback_workflow(do_temporary=False)
    skip_public = run_crankback_workflow(do_public=False)
    skip_crankbackdigest = run_crankback_workflow(do_crankbackdigest=False)
    skip_replay = run_crankback_workflow(replay=False)
    skip_crankbackid = run_crankback_workflow(use_crankbackid=False)
    live = run_crankback_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_crankbackid"
    ...
    assert skip_crankbackdigest["ok"] is False
    assert skip_crankbackdigest["error"] == "crankbackdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_crankbackid["ok"] is False
    assert skip_crankbackid["error"] == "crankbackid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_crankbackdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["crankbackdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["crankbackid_bound"] is True
    assert row["digest"]
    assert live["crankbackid"] == DEFAULT_CRANKBACKID
    assert live["crankbackdigest"] == DEFAULT_CRANKBACKDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, crankbackid=DEFAULT_CRANKBACKID, crankbackdigest=DEFAULT_CRANKBACKDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["crankbackid"] == DEFAULT_CRANKBACKID
    assert queried["crankbackdigest"] == DEFAULT_CRANKBACKDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == CRANKBACK_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, crankbackid=DEFAULT_CRANKBACKID, crankbackdigest=DEFAULT_CRANKBACKDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["crankbackid"] == DEFAULT_CRANKBACKID
    assert answered["crankbackdigest"] == DEFAULT_CRANKBACKDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, crankbackid=DEFAULT_CRANKBACKID, include_crankbackid=False)
    )
    assert bare["has_crankbackid"] is False
    assert bare["crankbackid"] == EMPTY_CRANKBACKID
    publicised = serialize_crankback(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_crankback(publicised) == DEFAULT_SOURCE
    assert parse_crankback(RFC_DEST_FIELD) == DEST_HOP
    header = parse_crankback_header(encode_crankback_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_CRANKBACKID, DEFAULT_CRANKBACKDIGEST))
    ...
    assert asked["method"] == "CRANK"
    assert asked["disc_kind"] == "crank"
    assert listed["disc_kind"] == "retry"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_CRANKBACKID).startswith("CRANK")
    assert "crankbackdigest=" in canonical_public(SENTINEL, DEFAULT_CRANKBACKID, DEFAULT_CRANKBACKDIGEST)


def test_builtin_proof_seals_crankback_actuation() -> None:
    report = builtin_crankback_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "crankback_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_crankback"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_crankbackid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_crankbackdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_crankbackid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_crankbackdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_crankback"]
    ...
    assert report["checks"]["leftover_text_binds_crankback"]
    assert report["checks"]["proved_crankback_consumes_leftover"]
    assert report["mission_goal"] == CRANKBACK_ACTUATION_GOAL
    assert report["done_when"] == CRANKBACK_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[CRANKBACK_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "crankback" in capability.tags
    assert "rfc4920" in capability.tags
    assert "http" in capability.tags
    assert "crankbackid" in capability.tags
    assert "crankbackdigest" in capability.tags
    assert "crank" in capability.tags
    assert "retry" in capability.tags


def test_selection_gate_rejects_ledger_only_crankback_family(tmp_path: Path) -> None:
    ...
    assert "crankback" in family.split("/")
    assert "rfc4920" in set(semantic_tokens(CRANKBACK_ACTUATION_GOAL))
    assert "crankbackid" in set(semantic_tokens(CRANKBACK_ACTUATION_GOAL))
    assert "crankbackdigest" in family
