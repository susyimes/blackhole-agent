...
from blackhole_agent.luprefix_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_luprefix_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_luprefix_header,
    independent_luprefixdigest,
    parse_http_request,
    parse_http_response,
    parse_message,
    parse_luprefix,
    parse_luprefix_header,
    public_request,
    public_response,
    run_luprefix_workflow,
    serialize_luprefix,
    temporary_request,
    temporary_response,
)
...
def test_goal_binds_luprefix_actuation_plane() -> None:
    assert leftover_marker_ids(LUPREFIX_ACTUATION_GOAL) == (LUPREFIX_ACTUATION_ID,)
    ...
    assert LUPREFIX_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(LUPREFIX_LEFTOVER) == (LUPREFIX_ACTUATION_ID,)
    assert leftover_marker_ids(LUPREFIX_ACTUATION_GOAL) == (LUPREFIX_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert LUPREFIX_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(LUPREFIX_ACTUATION_GOAL)
...
def test_opted_in_luprefix_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("luprefix",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, LUPREFIX_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["luprefix"]

    missing = run_luprefix_workflow(with_luprefixid=False)
    skip_bind = run_luprefix_workflow(skip_bind=True)
    skip_temporary = run_luprefix_workflow(do_temporary=False)
    skip_public = run_luprefix_workflow(do_public=False)
    skip_luprefixdigest = run_luprefix_workflow(do_luprefixdigest=False)
    skip_replay = run_luprefix_workflow(replay=False)
    skip_luprefixid = run_luprefix_workflow(use_luprefixid=False)
    live = run_luprefix_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_luprefixid"
    ...
    assert skip_luprefixdigest["ok"] is False
    assert skip_luprefixdigest["error"] == "luprefixdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_luprefixid["ok"] is False
    assert skip_luprefixid["error"] == "luprefixid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_luprefixdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["luprefixdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["luprefixid_bound"] is True
    assert row["digest"]
    assert live["luprefixid"] == DEFAULT_LUPREFIXID
    assert live["luprefixdigest"] == DEFAULT_LUPREFIXDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, luprefixid=DEFAULT_LUPREFIXID, luprefixdigest=DEFAULT_LUPREFIXDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["luprefixid"] == DEFAULT_LUPREFIXID
    assert queried["luprefixdigest"] == DEFAULT_LUPREFIXDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == LUPREFIX_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, luprefixid=DEFAULT_LUPREFIXID, luprefixdigest=DEFAULT_LUPREFIXDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["luprefixid"] == DEFAULT_LUPREFIXID
    assert answered["luprefixdigest"] == DEFAULT_LUPREFIXDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, luprefixid=DEFAULT_LUPREFIXID, include_luprefixid=False)
    )
    assert bare["has_luprefixid"] is False
    assert bare["luprefixid"] == EMPTY_LUPREFIXID
    publicised = serialize_luprefix(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_luprefix(publicised) == DEFAULT_SOURCE
    assert parse_luprefix(RFC_DEST_FIELD) == DEST_HOP
    header = parse_luprefix_header(encode_luprefix_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_LUPREFIXID, DEFAULT_LUPREFIXDIGEST))
    ...
    assert asked["method"] == "LUP"
    assert asked["disc_kind"] == "lup"
    assert listed["disc_kind"] == "nsl"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_LUPREFIXID).startswith("LUP")
    assert "luprefixdigest=" in canonical_public(SENTINEL, DEFAULT_LUPREFIXID, DEFAULT_LUPREFIXDIGEST)


def test_builtin_proof_seals_luprefix_actuation() -> None:
    report = builtin_luprefix_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "luprefix_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_luprefix"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_luprefixid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_luprefixdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_luprefixid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_luprefixdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_luprefix"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_luprefix"]
    assert report["checks"]["proved_luprefix_consumes_leftover"]
    assert report["mission_goal"] == LUPREFIX_ACTUATION_GOAL
    assert report["done_when"] == LUPREFIX_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[LUPREFIX_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "luprefix" in capability.tags
    assert "rfc8215" in capability.tags
    assert "http" in capability.tags
    assert "luprefixid" in capability.tags
    assert "luprefixdigest" in capability.tags
    assert "lup" in capability.tags
    assert "nsl" in capability.tags


def test_selection_gate_rejects_ledger_only_luprefix_family(tmp_path: Path) -> None:
    ...
    assert "luprefix" in family.split("/")
    assert "rfc8215" in family
    assert "luprefixid" in set(semantic_tokens(LUPREFIX_ACTUATION_GOAL))
    assert "luprefixdigest" in family
    ...
    assert "sixrdid" not in family
    assert "sixrddigest" not in family
