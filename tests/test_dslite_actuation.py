...
from blackhole_agent.dslite_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_dslite_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_dslite_header,
    independent_dslitedigest,
    parse_http_request,
    parse_http_response,
    parse_message,
    parse_dslite,
...
def test_goal_binds_dslite_actuation_plane() -> None:
    assert leftover_marker_ids(DSLITE_ACTUATION_GOAL) == (DSLITE_ACTUATION_ID,)
    ...
    assert leftover_marker_ids(DSLITE_LEFTOVER) == (DSLITE_ACTUATION_ID,)
    assert leftover_marker_ids(LW4O6_ACTUATION_GOAL) == (LW4O6_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    assert DSLITE_ACTUATION_ID in LOCAL_DENYLIST
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert DSLITE_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(DSLITE_ACTUATION_GOAL)
...
def test_opted_in_dslite_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("dslite",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, DSLITE_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["dslite"]

    missing = run_dslite_workflow(with_dsliteid=False)
    skip_bind = run_dslite_workflow(skip_bind=True)
    skip_temporary = run_dslite_workflow(do_temporary=False)
    skip_public = run_dslite_workflow(do_public=False)
    skip_dslitedigest = run_dslite_workflow(do_dslitedigest=False)
    skip_replay = run_dslite_workflow(replay=False)
    skip_dsliteid = run_dslite_workflow(use_dsliteid=False)
    live = run_dslite_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_dsliteid"
    ...
    assert skip_dslitedigest["ok"] is False
    assert skip_dslitedigest["error"] == "dslitedigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_dsliteid["ok"] is False
    assert skip_dsliteid["error"] == "dsliteid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_dslitedigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["dslitedigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["dsliteid_bound"] is True
    assert row["digest"]
    assert live["dsliteid"] == DEFAULT_DSLITEID
    assert live["dslitedigest"] == DEFAULT_DSLITEDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, dsliteid=DEFAULT_DSLITEID, dslitedigest=DEFAULT_DSLITEDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["dsliteid"] == DEFAULT_DSLITEID
    assert queried["dslitedigest"] == DEFAULT_DSLITEDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == DSLITE_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, dsliteid=DEFAULT_DSLITEID, dslitedigest=DEFAULT_DSLITEDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["dsliteid"] == DEFAULT_DSLITEID
    assert answered["dslitedigest"] == DEFAULT_DSLITEDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, dsliteid=DEFAULT_DSLITEID, include_dsliteid=False)
    )
    assert bare["has_dsliteid"] is False
    assert bare["dsliteid"] == EMPTY_DSLITEID
    publicised = serialize_dslite(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_dslite(publicised) == DEFAULT_SOURCE
    assert parse_dslite(RFC_DEST_FIELD) == DEST_HOP
    header = parse_dslite_header(encode_dslite_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_DSLITEID, DEFAULT_DSLITEDIGEST))
    ...
    assert asked["method"] == "B4"
    assert asked["disc_kind"] == "b4"
    assert listed["disc_kind"] == "aftr"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_DSLITEID).startswith("B4")
    assert "dslitedigest=" in canonical_public(SENTINEL, DEFAULT_DSLITEID, DEFAULT_DSLITEDIGEST)


def test_builtin_proof_seals_dslite_actuation() -> None:
    report = builtin_dslite_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "dslite_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_dslite"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_dsliteid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_dslitedigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_dsliteid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_dslitedigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_dslite"]
    ...
    assert report["checks"]["leftover_text_binds_dslite"]
    assert report["checks"]["proved_dslite_consumes_leftover"]
    assert report["mission_goal"] == DSLITE_ACTUATION_GOAL
    assert report["done_when"] == DSLITE_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[DSLITE_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "dslite" in capability.tags
    assert "rfc6333" in capability.tags
    assert "http" in capability.tags
    assert "dsliteid" in capability.tags
    assert "dslitedigest" in capability.tags
    assert "b4" in capability.tags
    assert "aftr" in capability.tags


def test_selection_gate_rejects_ledger_only_dslite_family(tmp_path: Path) -> None:
    ...
    assert "dslite" in family.split("/")
    assert "rfc6333" in family
    assert "dsliteid" in family
    assert "dslitedigest" in family
