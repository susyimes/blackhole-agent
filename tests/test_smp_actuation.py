...
def test_goal_binds_smp_actuation_plane() -> None:
    assert leftover_marker_ids(SMP_ACTUATION_GOAL) == (SMP_ACTUATION_ID,)
    ...
    assert SMP_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(SMP_LEFTOVER) == (SMP_ACTUATION_ID,)
    assert leftover_marker_ids(SMP_ACTUATION_GOAL) == (SMP_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert SMP_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(SMP_ACTUATION_GOAL)
    icp_signature = semantic_signature(SMP_ACTUATION_GOAL)
    for neighbor in NEIGHBORS:
        assert semantic_similarity(icp_signature, semantic_signature(neighbor)) < 0.82


def test_opted_in_smp_tool_completes_body_transfer_poll() -> None:
    descriptor = smp_tool_descriptor()
    naive = route_tool_descriptor(descriptor)
    opted = route_tool_descriptor(
        descriptor,
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, SMP_TOOL_PROVIDER),
    )
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("smp",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, SMP_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["smp"]

    missing = run_smp_workflow(with_smpid=False)
    skip_bind = run_smp_workflow(skip_bind=True)
    skip_temporary = run_smp_workflow(do_temporary=False)
    skip_public = run_smp_workflow(do_public=False)
    skip_smpdigest = run_smp_workflow(do_smpdigest=False)
    skip_replay = run_smp_workflow(replay=False)
    skip_smpid = run_smp_workflow(use_smpid=False)
    live = run_smp_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_smpid"
    ...
    assert skip_smpdigest["ok"] is False
    assert skip_smpdigest["error"] == "smpdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_smpid["ok"] is False
    assert skip_smpid["error"] == "smpid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_smpdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["smpdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["smpid_bound"] is True
    assert row["digest"]
    assert live["smpid"] == DEFAULT_SMPID
    assert live["smpdigest"] == DEFAULT_SMPDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, smpid=DEFAULT_SMPID, smpdigest=DEFAULT_SMPDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["smpid"] == DEFAULT_SMPID
    assert queried["smpdigest"] == DEFAULT_SMPDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == SMP_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, smpid=DEFAULT_SMPID, smpdigest=DEFAULT_SMPDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["smpid"] == DEFAULT_SMPID
    assert answered["smpdigest"] == DEFAULT_SMPDIGEST
    packed = encode_temporary(identity=SENTINEL, smpid=DEFAULT_SMPID, smpdigest=DEFAULT_SMPDIGEST)
    zeroed = packed[:-4] + (0).to_bytes(4, "big")
    assert crc32c(zeroed) == int.from_bytes(packed[-4:], "big")
    bare = parse_message(
        encode_temporary(identity=SENTINEL, smpid=DEFAULT_SMPID, include_smpid=False)
    )
    assert bare["has_smpid"] is False
    assert bare["smpid"] == EMPTY_SMPID
    publicised = serialize_smp(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_smp(publicised) == DEFAULT_SOURCE
    assert parse_smp(RFC_DEST_FIELD) == DEST_HOP
    header = parse_smp_header(encode_smp_header(DEFAULT_SOURCE))
    assert header["field_value"] == RFC_SOURCE_FIELD
    assert header["header"] == SOURCE_HEADER
    asked = parse_http_comm(temporary_comm(SENTINEL, DEFAULT_SMPID))
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_SMPID, DEFAULT_SMPDIGEST))
    got = parse_http_response(temporary_response(SENTINEL, DEFAULT_SMPID, DEFAULT_SMPDIGEST))
    preload_public = parse_http_response(
        public_response(SENTINEL, DEFAULT_SMPID, DEFAULT_SMPDIGEST)
    )
    assert asked["method"] == "SHARE"
    assert asked["disc_kind"] == "share"
    assert listed["disc_kind"] == "protect"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_SMPID).startswith("SHARE")
    assert "smpdigest=" in canonical_public(SENTINEL, DEFAULT_SMPID, DEFAULT_SMPDIGEST)


def test_builtin_proof_seals_smp_actuation() -> None:
    report = builtin_smp_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "smp_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_smp"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_smpid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_smpdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_smpid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_smpdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_binds_smp"]
    ...
    assert report["checks"]["catalog_names_wson"]
    assert report["checks"]["leftover_text_binds_smp"]
    assert report["checks"]["proved_smp_consumes_leftover"]
    assert report["mission_goal"] == SMP_ACTUATION_GOAL
    assert report["done_when"] == SMP_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[SMP_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "smp" in capability.tags
    assert "rfc6388" in capability.tags
    assert "http" in capability.tags
    assert "smpid" in capability.tags
    assert "smpdigest" in capability.tags
    assert "share" in capability.tags
    assert "protect" in capability.tags


def test_selection_gate_rejects_ledger_only_smp_family(tmp_path: Path) -> None:
    ...
    family = capability_family(SMP_ACTUATION_GOAL)
    family_tokens = set(family.split("/"))
    assert "smp" in family.split("/")
    assert "rfc6388" in set(semantic_tokens(SMP_ACTUATION_GOAL))
    assert "smpid" in set(semantic_tokens(SMP_ACTUATION_GOAL))
    assert "smpdigest" in family
