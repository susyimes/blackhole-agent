...
def test_goal_binds_mld_actuation_plane() -> None:
    assert leftover_marker_ids(MLD_ACTUATION_GOAL) == (MLD_ACTUATION_ID,)
    assert leftover_marker_ids(MLD_LEFTOVER) == (MLD_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert MLD_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(MLD_ACTUATION_GOAL)
...
def test_opted_in_mld_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("mld",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, MLD_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["mld"]

    missing = run_mld_workflow(with_mldid=False)
    skip_bind = run_mld_workflow(skip_bind=True)
    skip_listener = run_mld_workflow(do_listener=False)
    skip_done = run_mld_workflow(do_done=False)
    skip_mlddigest = run_mld_workflow(do_mlddigest=False)
    skip_replay = run_mld_workflow(replay=False)
    skip_mldid = run_mld_workflow(use_mldid=False)
    live = run_mld_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_mldid"
    assert skip_bind["ok"] is False
    assert skip_bind["error"] == "not_bound"
    assert skip_listener["ok"] is False
    assert skip_listener["error"] == "listener_required"
    assert skip_done["ok"] is False
    assert skip_done["error"] == "done_required"
    assert skip_mlddigest["ok"] is False
    assert skip_mlddigest["error"] == "mlddigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_mldid["ok"] is False
    assert skip_mldid["error"] == "mldid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_mlddigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["listener_frame"] is True
    assert row["done_frame"] is True
    assert row["mlddigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["mldid_bound"] is True
    assert row["digest"]
    assert live["mldid"] == DEFAULT_MLDID
    assert live["mlddigest"] == DEFAULT_MLDDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_listener(identity=SENTINEL, mldid=DEFAULT_MLDID, mlddigest=DEFAULT_MLDDIGEST)
    )
    assert queried["is_listener"] is True and queried["is_done"] is False
    assert queried["identity"] == SENTINEL and queried["mldid"] == DEFAULT_MLDID
    assert queried["mlddigest"] == DEFAULT_MLDDIGEST
    assert queried["type"] == FRAME_LISTENER
    assert queried["first_byte"] == MLD_FIRST
    answered = parse_message(
        encode_done(identity=SENTINEL, mldid=DEFAULT_MLDID, mlddigest=DEFAULT_MLDDIGEST)
    )
    assert answered["is_done"] is True and answered["is_done"] is True
    assert answered["mldid"] == DEFAULT_MLDID
    assert answered["mlddigest"] == DEFAULT_MLDDIGEST
    ...
    bare = parse_message(
        encode_listener(identity=SENTINEL, mldid=DEFAULT_MLDID, include_mldid=False)
    )
    assert bare["has_mldid"] is False
    assert bare["mldid"] == EMPTY_MLDID
    advertised = serialize_mld(DEFAULT_LISTENER)
    assert advertised == RFC_LISTENER_FIELD
    assert parse_mld(advertised) == DEFAULT_LISTENER
    assert parse_mld(RFC_DONE_FIELD) == DONE_POLICY
    header = parse_mld_header(encode_mld_header(DEFAULT_LISTENER))
    assert header["field_value"] == RFC_LISTENER_FIELD
    assert header["header"] == LISTENER_HEADER
    asked = parse_http_request(listener_request(SENTINEL, DEFAULT_MLDID))
    listed = parse_http_request(done_request(SENTINEL, DEFAULT_MLDID, DEFAULT_MLDDIGEST))
    ...
    assert asked["method"] == "LISTENER"
    assert asked["mld_kind"] == "listener"
    assert listed["mld_kind"] == "done"
    assert got["status"] == 200
    assert preload_done["status"] == 200
    assert got["policy"] == DEFAULT_LISTENER
    assert preload_done["policy"] == DONE_POLICY
    assert canonical_listener(SENTINEL, DEFAULT_MLDID).startswith("LISTENER")
    assert "mlddigest=" in canonical_done(SENTINEL, DEFAULT_MLDID, DEFAULT_MLDDIGEST)


def test_builtin_proof_seals_mld_actuation() -> None:
    done = builtin_mld_actuation_proof()
    assert done["ok"] is True, done.get("failed") or done.get("checks")
    assert done["action"] == "mld_actuation"
    assert done["used_skill_route_discovery"] is False
    assert done["passed_count"] == len(done["checks"])
    assert done["passed_count"] >= 12
    assert done["checks"]["naive_preflight_missing_mld"]
    assert done["checks"]["opted_in_preflight_ok"]
    assert done["checks"]["naive_without_mldid_is_forbidden"]
    assert done["checks"]["skip_listener_stays_empty"]
    assert done["checks"]["skip_done_stays_empty"]
    assert done["checks"]["skip_mlddigest_stays_empty"]
    assert done["checks"]["skip_replay_stays_empty"]
    assert done["checks"]["skip_mldid_stays_empty"]
    assert done["checks"]["workflow_extracts_sentinel"]
    assert done["checks"]["workflow_commits_independent_digest"]
    assert done["checks"]["workflow_writes_sealed_file"]
    assert done["checks"]["workflow_records_mlddigest"]
    assert done["checks"]["sealed_trace_verifies"]
    assert done["checks"]["tampered_trace_fails"]
    assert done["checks"]["exhausted_catalog_binds_mld"]
    assert done["checks"]["catalog_names_mld"]
    assert done["checks"]["catalog_names_ndp"]
    assert done["checks"]["leftover_text_binds_mld"]
    assert done["checks"]["proved_mld_consumes_leftover"]
    assert done["mission_goal"] == MLD_ACTUATION_GOAL
    assert done["done_when"] == MLD_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[MLD_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "mld" in capability.tags
    assert "rfc2710" in capability.tags
    assert "http" in capability.tags
    assert "mldid" in capability.tags
    assert "mlddigest" in capability.tags
    assert "listener" in capability.tags


def test_selection_gate_rejects_ledger_only_mld_family(tmp_path: Path) -> None:
    ...
    assert "mld" in family.split("/")
    assert "rfc2710" in family
    assert "mldid" in family
    assert "mlddigest" in family
