...
from blackhole_agent.imlproxy_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_imlproxy_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_imlproxy_header,
    independent_imlproxydigest,
    parse_http_comm,
    parse_http_response,
    parse_message,
    parse_imlproxy,
    parse_imlproxy_header,
    public_comm,
    public_response,
    run_imlproxy_workflow,
    serialize_imlproxy,
    temporary_comm,
    temporary_response,
)
...
def test_goal_binds_imlproxy_actuation_plane() -> None:
    assert leftover_marker_ids(IMLPROXY_ACTUATION_GOAL) == (IMLPROXY_ACTUATION_ID,)
    ...
    assert IMLPROXY_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(IMLPROXY_LEFTOVER) == (IMLPROXY_ACTUATION_ID,)
    assert leftover_marker_ids(IMLPROXY_ACTUATION_GOAL) == (IMLPROXY_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert IMLPROXY_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(IMLPROXY_ACTUATION_GOAL)
...
def test_opted_in_imlproxy_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("imlproxy",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, IMLPROXY_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["imlproxy"]

    missing = run_imlproxy_workflow(with_imlproxyid=False)
    skip_bind = run_imlproxy_workflow(skip_bind=True)
    skip_temporary = run_imlproxy_workflow(do_temporary=False)
    skip_public = run_imlproxy_workflow(do_public=False)
    skip_imlproxydigest = run_imlproxy_workflow(do_imlproxydigest=False)
    skip_replay = run_imlproxy_workflow(replay=False)
    skip_imlproxyid = run_imlproxy_workflow(use_imlproxyid=False)
    live = run_imlproxy_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_imlproxyid"
    ...
    assert skip_imlproxydigest["ok"] is False
    assert skip_imlproxydigest["error"] == "imlproxydigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_imlproxyid["ok"] is False
    assert skip_imlproxyid["error"] == "imlproxyid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_imlproxydigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["imlproxydigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["imlproxyid_bound"] is True
    assert row["digest"]
    assert live["imlproxyid"] == DEFAULT_IMLPROXYID
    assert live["imlproxydigest"] == DEFAULT_IMLPROXYDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, imlproxyid=DEFAULT_IMLPROXYID, imlproxydigest=DEFAULT_IMLPROXYDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["imlproxyid"] == DEFAULT_IMLPROXYID
    assert queried["imlproxydigest"] == DEFAULT_IMLPROXYDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == IMLPROXY_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, imlproxyid=DEFAULT_IMLPROXYID, imlproxydigest=DEFAULT_IMLPROXYDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["imlproxyid"] == DEFAULT_IMLPROXYID
    assert answered["imlproxydigest"] == DEFAULT_IMLPROXYDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, imlproxyid=DEFAULT_IMLPROXYID, include_imlproxyid=False)
    )
    assert bare["has_imlproxyid"] is False
    assert bare["imlproxyid"] == EMPTY_IMLPROXYID
    publicised = serialize_imlproxy(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_imlproxy(publicised) == DEFAULT_SOURCE
    assert parse_imlproxy(RFC_DEST_FIELD) == DEST_HOP
    header = parse_imlproxy_header(encode_imlproxy_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_IMLPROXYID, DEFAULT_IMLPROXYDIGEST))
    ...
    assert asked["method"] == "IGMP"
    assert asked["disc_kind"] == "imlproxy"
    assert listed["disc_kind"] == "mld"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_IMLPROXYID).startswith("IGMP")
    assert "imlproxydigest=" in canonical_public(SENTINEL, DEFAULT_IMLPROXYID, DEFAULT_IMLPROXYDIGEST)


def test_builtin_proof_seals_imlproxy_actuation() -> None:
    report = builtin_imlproxy_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "imlproxy_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_imlproxy"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_imlproxyid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_imlproxydigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_imlproxyid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_imlproxydigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_imlproxy"]
    ...
    assert report["checks"]["leftover_text_binds_imlproxy"]
    assert report["checks"]["proved_imlproxy_consumes_leftover"]
    assert report["mission_goal"] == IMLPROXY_ACTUATION_GOAL
    assert report["done_when"] == IMLPROXY_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[IMLPROXY_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "imlproxy" in capability.tags
    assert "rfc9251" in capability.tags
    assert "http" in capability.tags
    assert "imlproxyid" in capability.tags
    assert "imlproxydigest" in capability.tags
    assert "imlproxy" in capability.tags
...
def test_selection_gate_rejects_ledger_only_imlproxy_family(tmp_path: Path) -> None:
    ...
    assert "imlproxy" in family.split("/")
    assert "rfc9251" in set(semantic_tokens(IMLPROXY_ACTUATION_GOAL))
    assert "imlproxyid" in set(semantic_tokens(IMLPROXY_ACTUATION_GOAL))
    assert "imlproxydigest" in family
    ...
    assert "evpn" not in family.split("/")
