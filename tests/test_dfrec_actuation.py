...
def test_goal_binds_dfrec_actuation_plane() -> None:
    assert leftover_marker_ids(DFREC_ACTUATION_GOAL) == (DFREC_ACTUATION_ID,)
    ...
    assert DFREC_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(DFREC_LEFTOVER) == (DFREC_ACTUATION_ID,)
    assert leftover_marker_ids(DFREC_ACTUATION_GOAL) == (DFREC_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert DFREC_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(DFREC_ACTUATION_GOAL)
...
def test_opted_in_dfrec_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("dfrec",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, DFREC_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["dfrec"]

    missing = run_dfrec_workflow(with_dfrecid=False)
    skip_bind = run_dfrec_workflow(skip_bind=True)
    skip_temporary = run_dfrec_workflow(do_temporary=False)
    skip_public = run_dfrec_workflow(do_public=False)
    skip_dfrecdigest = run_dfrec_workflow(do_dfrecdigest=False)
    skip_replay = run_dfrec_workflow(replay=False)
    skip_dfrecid = run_dfrec_workflow(use_dfrecid=False)
    live = run_dfrec_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_dfrecid"
    ...
    assert skip_dfrecdigest["ok"] is False
    assert skip_dfrecdigest["error"] == "dfrecdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_dfrecid["ok"] is False
    assert skip_dfrecid["error"] == "dfrecid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_dfrecdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["dfrecdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["dfrecid_bound"] is True
    assert row["digest"]
    assert live["dfrecid"] == DEFAULT_DFRECID
    assert live["dfrecdigest"] == DEFAULT_DFRECDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, dfrecid=DEFAULT_DFRECID, dfrecdigest=DEFAULT_DFRECDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["dfrecid"] == DEFAULT_DFRECID
    assert queried["dfrecdigest"] == DEFAULT_DFRECDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == DFREC_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, dfrecid=DEFAULT_DFRECID, dfrecdigest=DEFAULT_DFRECDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["dfrecid"] == DEFAULT_DFRECID
    assert answered["dfrecdigest"] == DEFAULT_DFRECDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, dfrecid=DEFAULT_DFRECID, include_dfrecid=False)
    )
    assert bare["has_dfrecid"] is False
    assert bare["dfrecid"] == EMPTY_DFRECID
    publicised = serialize_dfrec(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_dfrec(publicised) == DEFAULT_SOURCE
    assert parse_dfrec(RFC_DEST_FIELD) == DEST_HOP
    header = parse_dfrec_header(encode_dfrec_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_DFRECID, DEFAULT_DFRECDIGEST))
    ...
    assert asked["method"] == "DFREC"
    assert asked["disc_kind"] == "dfrec"
    assert listed["disc_kind"] == "fast"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_DFRECID).startswith("DFREC")
    assert "dfrecdigest=" in canonical_public(SENTINEL, DEFAULT_DFRECID, DEFAULT_DFRECDIGEST)


def test_builtin_proof_seals_dfrec_actuation() -> None:
    report = builtin_dfrec_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "dfrec_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_dfrec"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_dfrecid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_dfrecdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_dfrecid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_dfrecdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_dfrec"]
    ...
    assert report["checks"]["leftover_text_binds_dfrec"]
    assert report["checks"]["proved_dfrec_consumes_leftover"]
    assert report["mission_goal"] == DFREC_ACTUATION_GOAL
    assert report["done_when"] == DFREC_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[DFREC_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "dfrec" in capability.tags
    assert "rfc9722" in capability.tags
    assert "http" in capability.tags
    assert "dfrecid" in capability.tags
    assert "dfrecdigest" in capability.tags
    assert "dfrec" in capability.tags
    assert "fast" in capability.tags


def test_selection_gate_rejects_ledger_only_dfrec_family(tmp_path: Path) -> None:
    ...
    assert "dfrec" in family.split("/")
    assert "rfc9722" in set(semantic_tokens(DFREC_ACTUATION_GOAL))
    assert "dfrecid" in set(semantic_tokens(DFREC_ACTUATION_GOAL))
    assert "dfrecdigest" in family
