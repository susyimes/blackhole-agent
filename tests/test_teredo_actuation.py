...
from blackhole_agent.teredo_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_teredo_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_teredo_header,
    independent_teredodigest,
    parse_http_request,
    parse_http_response,
    parse_message,
    parse_teredo,
...
def test_goal_binds_teredo_actuation_plane() -> None:
    assert leftover_marker_ids(TEREDO_ACTUATION_GOAL) == (TEREDO_ACTUATION_ID,)
    ...
    assert TEREDO_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(TEREDO_LEFTOVER) == (TEREDO_ACTUATION_ID,)
    assert leftover_marker_ids(TEREDO_ACTUATION_GOAL) == (TEREDO_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert TEREDO_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(TEREDO_ACTUATION_GOAL)
...
def test_opted_in_teredo_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("teredo",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, TEREDO_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["teredo"]

    missing = run_teredo_workflow(with_teredoid=False)
    skip_bind = run_teredo_workflow(skip_bind=True)
    skip_temporary = run_teredo_workflow(do_temporary=False)
    skip_public = run_teredo_workflow(do_public=False)
    skip_teredodigest = run_teredo_workflow(do_teredodigest=False)
    skip_replay = run_teredo_workflow(replay=False)
    skip_teredoid = run_teredo_workflow(use_teredoid=False)
    live = run_teredo_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_teredoid"
    ...
    assert skip_teredodigest["ok"] is False
    assert skip_teredodigest["error"] == "teredodigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_teredoid["ok"] is False
    assert skip_teredoid["error"] == "teredoid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_teredodigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["teredodigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["teredoid_bound"] is True
    assert row["digest"]
    assert live["teredoid"] == DEFAULT_TEREDOID
    assert live["teredodigest"] == DEFAULT_TEREDODIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, teredoid=DEFAULT_TEREDOID, teredodigest=DEFAULT_TEREDODIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["teredoid"] == DEFAULT_TEREDOID
    assert queried["teredodigest"] == DEFAULT_TEREDODIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == TEREDO_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, teredoid=DEFAULT_TEREDOID, teredodigest=DEFAULT_TEREDODIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["teredoid"] == DEFAULT_TEREDOID
    assert answered["teredodigest"] == DEFAULT_TEREDODIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, teredoid=DEFAULT_TEREDOID, include_teredoid=False)
    )
    assert bare["has_teredoid"] is False
    assert bare["teredoid"] == EMPTY_TEREDOID
    publicised = serialize_teredo(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_teredo(publicised) == DEFAULT_SOURCE
    assert parse_teredo(RFC_DEST_FIELD) == DEST_HOP
    header = parse_teredo_header(encode_teredo_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_TEREDOID, DEFAULT_TEREDODIGEST))
    ...
    assert asked["method"] == "BUBBLE"
    assert asked["disc_kind"] == "bubble"
    assert listed["disc_kind"] == "qual"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_TEREDOID).startswith("BUBBLE")
    assert "teredodigest=" in canonical_public(SENTINEL, DEFAULT_TEREDOID, DEFAULT_TEREDODIGEST)


def test_builtin_proof_seals_teredo_actuation() -> None:
    report = builtin_teredo_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "teredo_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_teredo"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_teredoid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_teredodigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_teredoid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_teredodigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_teredo"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_teredo"]
    assert report["checks"]["proved_teredo_consumes_leftover"]
    assert report["mission_goal"] == TEREDO_ACTUATION_GOAL
    assert report["done_when"] == TEREDO_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[TEREDO_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "teredo" in capability.tags
    assert "rfc4380" in capability.tags
    assert "http" in capability.tags
    assert "teredoid" in capability.tags
    assert "teredodigest" in capability.tags
    assert "bubble" in capability.tags
