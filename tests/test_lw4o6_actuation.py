...
def test_goal_binds_lw4o6_actuation_plane() -> None:
    ...
    assert leftover_marker_ids(LW4O6_LEFTOVER) == (LW4O6_ACTUATION_ID,)
    assert leftover_marker_ids(MAPE_ACTUATION_GOAL) == (MAPE_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert LW4O6_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(LW4O6_ACTUATION_GOAL)
...
def test_opted_in_lw4o6_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("lw4o6",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, LW4O6_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["lw4o6"]

    missing = run_lw4o6_workflow(with_lw4o6id=False)
    skip_bind = run_lw4o6_workflow(skip_bind=True)
    skip_temporary = run_lw4o6_workflow(do_temporary=False)
    skip_public = run_lw4o6_workflow(do_public=False)
    skip_lw4o6digest = run_lw4o6_workflow(do_lw4o6digest=False)
    skip_replay = run_lw4o6_workflow(replay=False)
    skip_lw4o6id = run_lw4o6_workflow(use_lw4o6id=False)
    live = run_lw4o6_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_lw4o6id"
    ...
    assert skip_lw4o6digest["ok"] is False
    assert skip_lw4o6digest["error"] == "lw4o6digest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_lw4o6id["ok"] is False
    assert skip_lw4o6id["error"] == "lw4o6id_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_lw4o6digest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["lw4o6digest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["lw4o6id_bound"] is True
    assert row["digest"]
    assert live["lw4o6id"] == DEFAULT_LW4O6ID
    assert live["lw4o6digest"] == DEFAULT_LW4O6DIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, lw4o6id=DEFAULT_LW4O6ID, lw4o6digest=DEFAULT_LW4O6DIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["lw4o6id"] == DEFAULT_LW4O6ID
    assert queried["lw4o6digest"] == DEFAULT_LW4O6DIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == LW4O6_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, lw4o6id=DEFAULT_LW4O6ID, lw4o6digest=DEFAULT_LW4O6DIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["lw4o6id"] == DEFAULT_LW4O6ID
    assert answered["lw4o6digest"] == DEFAULT_LW4O6DIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, lw4o6id=DEFAULT_LW4O6ID, include_lw4o6id=False)
    )
    assert bare["has_lw4o6id"] is False
    assert bare["lw4o6id"] == EMPTY_LW4O6ID
    publicised = serialize_lw4o6(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_lw4o6(publicised) == DEFAULT_SOURCE
    assert parse_lw4o6(RFC_DEST_FIELD) == DEST_HOP
    header = parse_lw4o6_header(encode_lw4o6_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_LW4O6ID, DEFAULT_LW4O6DIGEST))
    ...
    assert asked["method"] == "BINDING"
    assert asked["disc_kind"] == "binding"
    assert listed["disc_kind"] == "portset"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_LW4O6ID).startswith("BINDING")
    assert "lw4o6digest=" in canonical_public(SENTINEL, DEFAULT_LW4O6ID, DEFAULT_LW4O6DIGEST)


def test_builtin_proof_seals_lw4o6_actuation() -> None:
    report = builtin_lw4o6_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "lw4o6_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_lw4o6"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_lw4o6id_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_lw4o6digest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_lw4o6id_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_lw4o6digest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_lw4o6"]
    ...
    assert report["checks"]["leftover_text_binds_lw4o6"]
    assert report["checks"]["proved_lw4o6_consumes_leftover"]
    assert report["mission_goal"] == LW4O6_ACTUATION_GOAL
    assert report["done_when"] == LW4O6_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[LW4O6_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "lw4o6" in capability.tags
    assert "rfc7596" in capability.tags
    assert "http" in capability.tags
    assert "lw4o6id" in capability.tags
    assert "lw4o6digest" in capability.tags
    assert "binding" in capability.tags
    assert "portset" in capability.tags


def test_selection_gate_rejects_ledger_only_lw4o6_family(tmp_path: Path) -> None:
    ...
    assert "lw4o6" in family.split("/")
    assert "rfc7596" in family
    assert "lw4o6id" in family
    assert "lw4o6digest" in family
