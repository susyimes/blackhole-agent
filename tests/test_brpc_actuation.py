...
def test_goal_binds_brpc_actuation_plane() -> None:
    assert leftover_marker_ids(BRPC_ACTUATION_GOAL) == (BRPC_ACTUATION_ID,)
    ...
    assert BRPC_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(BRPC_LEFTOVER) == (BRPC_ACTUATION_ID,)
    assert leftover_marker_ids(BRPC_ACTUATION_GOAL) == (BRPC_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert BRPC_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(BRPC_ACTUATION_GOAL)
...
def test_opted_in_brpc_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("brpc",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, BRPC_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["brpc"]

    missing = run_brpc_workflow(with_brpcid=False)
    skip_bind = run_brpc_workflow(skip_bind=True)
    skip_temporary = run_brpc_workflow(do_temporary=False)
    skip_public = run_brpc_workflow(do_public=False)
    skip_brpcdigest = run_brpc_workflow(do_brpcdigest=False)
    skip_replay = run_brpc_workflow(replay=False)
    skip_brpcid = run_brpc_workflow(use_brpcid=False)
    live = run_brpc_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_brpcid"
    ...
    assert skip_brpcdigest["ok"] is False
    assert skip_brpcdigest["error"] == "brpcdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_brpcid["ok"] is False
    assert skip_brpcid["error"] == "brpcid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_brpcdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["brpcdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["brpcid_bound"] is True
    assert row["digest"]
    assert live["brpcid"] == DEFAULT_BRPCID
    assert live["brpcdigest"] == DEFAULT_BRPCDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, brpcid=DEFAULT_BRPCID, brpcdigest=DEFAULT_BRPCDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["brpcid"] == DEFAULT_BRPCID
    assert queried["brpcdigest"] == DEFAULT_BRPCDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == BRPC_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, brpcid=DEFAULT_BRPCID, brpcdigest=DEFAULT_BRPCDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["brpcid"] == DEFAULT_BRPCID
    assert answered["brpcdigest"] == DEFAULT_BRPCDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, brpcid=DEFAULT_BRPCID, include_brpcid=False)
    )
    assert bare["has_brpcid"] is False
    assert bare["brpcid"] == EMPTY_BRPCID
    publicised = serialize_brpc(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_brpc(publicised) == DEFAULT_SOURCE
    assert parse_brpc(RFC_DEST_FIELD) == DEST_HOP
    header = parse_brpc_header(encode_brpc_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_BRPCID, DEFAULT_BRPCDIGEST))
    ...
    assert asked["method"] == "BRPC"
    assert asked["disc_kind"] == "brpc"
    assert listed["disc_kind"] == "reply"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_BRPCID).startswith("BRPC")
    assert "brpcdigest=" in canonical_public(SENTINEL, DEFAULT_BRPCID, DEFAULT_BRPCDIGEST)


def test_builtin_proof_seals_brpc_actuation() -> None:
    report = builtin_brpc_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "brpc_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_brpc"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_brpcid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_brpcdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_brpcid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_brpcdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_brpc"]
    ...
    assert report["checks"]["leftover_text_binds_brpc"]
    assert report["checks"]["proved_brpc_consumes_leftover"]
    assert report["mission_goal"] == BRPC_ACTUATION_GOAL
    assert report["done_when"] == BRPC_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[BRPC_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "brpc" in capability.tags
    assert "rfc5441" in capability.tags
    assert "http" in capability.tags
    assert "brpcid" in capability.tags
    assert "brpcdigest" in capability.tags
    assert "brpc" in capability.tags
    assert "reply" in capability.tags


def test_selection_gate_rejects_ledger_only_brpc_family(tmp_path: Path) -> None:
    ...
    assert "brpc" in family.split("/")
    assert "rfc5441" in set(semantic_tokens(BRPC_ACTUATION_GOAL))
    assert "brpcid" in set(semantic_tokens(BRPC_ACTUATION_GOAL))
    assert "brpcdigest" in family
