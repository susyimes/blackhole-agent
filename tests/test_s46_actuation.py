...
def test_goal_binds_s46_actuation_plane() -> None:
    assert leftover_marker_ids(S46_ACTUATION_GOAL) == (S46_ACTUATION_ID,)
    ...
    assert S46_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(S46_LEFTOVER) == (S46_ACTUATION_ID,)
    assert leftover_marker_ids(S46_ACTUATION_GOAL) == (S46_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert S46_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(S46_ACTUATION_GOAL)
...
def test_opted_in_s46_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("s46",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, S46_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["s46"]

    missing = run_s46_workflow(with_s46id=False)
    skip_bind = run_s46_workflow(skip_bind=True)
    skip_temporary = run_s46_workflow(do_temporary=False)
    skip_public = run_s46_workflow(do_public=False)
    skip_s46digest = run_s46_workflow(do_s46digest=False)
    skip_replay = run_s46_workflow(replay=False)
    skip_s46id = run_s46_workflow(use_s46id=False)
    live = run_s46_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_s46id"
    ...
    assert skip_s46digest["ok"] is False
    assert skip_s46digest["error"] == "s46digest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_s46id["ok"] is False
    assert skip_s46id["error"] == "s46id_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_s46digest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["s46digest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["s46id_bound"] is True
    assert row["digest"]
    assert live["s46id"] == DEFAULT_S46ID
    assert live["s46digest"] == DEFAULT_S46DIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, s46id=DEFAULT_S46ID, s46digest=DEFAULT_S46DIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["s46id"] == DEFAULT_S46ID
    assert queried["s46digest"] == DEFAULT_S46DIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == S46_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, s46id=DEFAULT_S46ID, s46digest=DEFAULT_S46DIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["s46id"] == DEFAULT_S46ID
    assert answered["s46digest"] == DEFAULT_S46DIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, s46id=DEFAULT_S46ID, include_s46id=False)
    )
    assert bare["has_s46id"] is False
    assert bare["s46id"] == EMPTY_S46ID
    publicised = serialize_s46(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_s46(publicised) == DEFAULT_SOURCE
    assert parse_s46(RFC_DEST_FIELD) == DEST_HOP
    header = parse_s46_header(encode_s46_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_S46ID, DEFAULT_S46DIGEST))
    ...
    assert asked["method"] == "RULE"
    assert asked["disc_kind"] == "rule"
    assert listed["disc_kind"] == "portparams"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_S46ID).startswith("RULE")
    assert "s46digest=" in canonical_public(SENTINEL, DEFAULT_S46ID, DEFAULT_S46DIGEST)


def test_builtin_proof_seals_s46_actuation() -> None:
    report = builtin_s46_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "s46_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_s46"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_s46id_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_s46digest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_s46id_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_s46digest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_s46"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_s46"]
    assert report["checks"]["proved_s46_consumes_leftover"]
    assert report["mission_goal"] == S46_ACTUATION_GOAL
    assert report["done_when"] == S46_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[S46_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "s46" in capability.tags
    assert "rfc7598" in capability.tags
    assert "http" in capability.tags
    assert "s46id" in capability.tags
    assert "s46digest" in capability.tags
    assert "rule" in capability.tags
    assert "portparams" in capability.tags


def test_selection_gate_rejects_ledger_only_s46_family(tmp_path: Path) -> None:
    ...
    assert "s46" in family.split("/")
    assert "rfc7598" in family
    assert "s46id" in family
    assert "s46digest" in family
