...
from blackhole_agent.opaqueiid_actuation import (
    ...
    SENTINEL,
    STABLE_HEADER,
    builtin_opaqueiid_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_opaqueiid_header,
    independent_opaquedigest,
    parse_http_request,
    parse_http_response,
    parse_message,
    parse_opaqueiid,
    parse_opaqueiid_header,
    public_request,
    public_response,
    run_opaqueiid_workflow,
    serialize_opaqueiid,
    temporary_request,
    temporary_response,
)
...
def test_goal_binds_opaqueiid_actuation_plane() -> None:
    assert leftover_marker_ids(OPAQUEIID_ACTUATION_GOAL) == (OPAQUEIID_ACTUATION_ID,)
    assert leftover_marker_ids(OPAQUEIID_LEFTOVER) == (OPAQUEIID_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert OPAQUEIID_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(OPAQUEIID_ACTUATION_GOAL)
...
def test_opted_in_opaqueiid_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("opaqueiid",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, OPAQUEIID_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["opaqueiid"]

    missing = run_opaqueiid_workflow(with_opaqueid=False)
    skip_bind = run_opaqueiid_workflow(skip_bind=True)
    skip_temporary = run_opaqueiid_workflow(do_temporary=False)
    skip_public = run_opaqueiid_workflow(do_public=False)
    skip_opaquedigest = run_opaqueiid_workflow(do_opaquedigest=False)
    skip_replay = run_opaqueiid_workflow(replay=False)
    skip_opaqueid = run_opaqueiid_workflow(use_opaqueid=False)
    live = run_opaqueiid_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_opaqueid"
    ...
    assert skip_opaquedigest["ok"] is False
    assert skip_opaquedigest["error"] == "opaquedigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_opaqueid["ok"] is False
    assert skip_opaqueid["error"] == "opaqueid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_opaquedigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["opaquedigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["opaqueid_bound"] is True
    assert row["digest"]
    assert live["opaqueid"] == DEFAULT_OPAQUEID
    assert live["opaquedigest"] == DEFAULT_OPAQUEDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, opaqueid=DEFAULT_OPAQUEID, opaquedigest=DEFAULT_OPAQUEDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["opaqueid"] == DEFAULT_OPAQUEID
    assert queried["opaquedigest"] == DEFAULT_OPAQUEDIGEST
    assert queried["type"] == FRAME_STABLE
    assert queried["first_byte"] == OPAQUEIID_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, opaqueid=DEFAULT_OPAQUEID, opaquedigest=DEFAULT_OPAQUEDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["opaqueid"] == DEFAULT_OPAQUEID
    assert answered["opaquedigest"] == DEFAULT_OPAQUEDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, opaqueid=DEFAULT_OPAQUEID, include_opaqueid=False)
    )
    assert bare["has_opaqueid"] is False
    assert bare["opaqueid"] == EMPTY_OPAQUEID
    publicised = serialize_opaqueiid(DEFAULT_STABLE)
    assert publicised == RFC_STABLE_FIELD
    assert parse_opaqueiid(publicised) == DEFAULT_STABLE
    assert parse_opaqueiid(RFC_OPAQUE_FIELD) == OPAQUE_POLICY
    header = parse_opaqueiid_header(encode_opaqueiid_header(DEFAULT_STABLE))
    assert header["field_value"] == RFC_STABLE_FIELD
    assert header["header"] == STABLE_HEADER
    asked = parse_http_request(temporary_request(SENTINEL, DEFAULT_OPAQUEID))
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_OPAQUEID, DEFAULT_OPAQUEDIGEST))
    ...
    assert asked["method"] == "STABLE"
    assert asked["opaqueiid_kind"] == "stable"
    assert listed["opaqueiid_kind"] == "opaque"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_STABLE
    assert preload_public["policy"] == OPAQUE_POLICY
    assert canonical_temporary(SENTINEL, DEFAULT_OPAQUEID).startswith("STABLE")
    assert "opaquedigest=" in canonical_public(SENTINEL, DEFAULT_OPAQUEID, DEFAULT_OPAQUEDIGEST)


def test_builtin_proof_seals_opaqueiid_actuation() -> None:
    report = builtin_opaqueiid_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "opaqueiid_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_opaqueiid"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_opaqueid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_opaquedigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_opaqueid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_opaquedigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_opaqueiid"]
    ...
    assert report["checks"]["catalog_names_cga"]
    assert report["checks"]["leftover_text_binds_opaqueiid"]
    assert report["checks"]["proved_opaqueiid_consumes_leftover"]
    assert report["mission_goal"] == OPAQUEIID_ACTUATION_GOAL
    assert report["done_when"] == OPAQUEIID_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[OPAQUEIID_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "opaqueiid" in capability.tags
    assert "rfc7217" in capability.tags
    assert "http" in capability.tags
    assert "opaqueid" in capability.tags
    assert "opaquedigest" in capability.tags
    assert "stable" in capability.tags


def test_selection_gate_rejects_ledger_only_opaqueiid_family(tmp_path: Path) -> None:
    ...
    assert "opaqueiid" in family.split("/")
    assert "rfc7217" in family
    assert "opaqueid" in family
    assert "opaquedigest" in family
