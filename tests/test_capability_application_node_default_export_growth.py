"""Tests for growing unplannable goals from default-export-only live-fetched npm tarballs."""
...
from blackhole_agent.capability_application_growth import (
    ...
    NODE_DEFAULT_EXPORT_DEP_NAME,
    NODE_DEFAULT_EXPORT_GOAL_KEY,
    NODE_DEFAULT_EXPORT_GROW_TASK,
    NODE_DEFAULT_EXPORT_NPM_DECOY_SLUG,
    NODE_DEFAULT_EXPORT_WINNER_CAPABILITY_ID,
    NODE_DEFAULT_EXPORT_WINNER_SLUG,
    REPO_ROOT,
    builtin_application_node_default_export_growth_plane_proof,
    grow_application_task,
    load_node_default_export_apply_catalog,
    run_application_node_default_export_growth_plane,
    verify_application_node_default_export_growth_plane,
)
...
def test_node_default_export_catalog_is_live_fetched_default_only_tarball() -> None:
    catalog = load_node_default_export_apply_catalog()
    assert catalog["query"] == query_from_goal(NODE_DEFAULT_EXPORT_GROW_TASK.goal)
    ...
    winner = next(item for item in catalog["items"] if item["slug"] == NODE_DEFAULT_EXPORT_WINNER_SLUG)
    request = forage_request_for(winner, live_fetch=True)
    assert request["origin"]["kind"] == "npm-live"
    assert Path(request["source"]).is_file()


def test_node_default_export_match_selects_humanize_string_and_closes_deps() -> None:
    catalog = load_node_default_export_apply_catalog()
    absorbed = sorted(APPLY_ABSORBED_SLUGS)
    trend = rank_catalog(strip_declared_provides(catalog["items"]), absorbed=absorbed)
    assert trend["winner"]["slug"] == NODE_DEFAULT_EXPORT_NPM_DECOY_SLUG
    matched = match_forage_goal(
        (NODE_DEFAULT_EXPORT_GOAL_KEY,), catalog=catalog, absorbed=absorbed, forage=False, live_fetch=True
    )
    assert matched["ok"], matched
    assert matched["winner"]["slug"] == NODE_DEFAULT_EXPORT_WINNER_SLUG
    covering = matched["covering"] or {}
    assert NODE_DEFAULT_EXPORT_GOAL_KEY in covering["inferred_provides"]
    assert covering.get("default_export") is True
    assert any(item.get("name") == NODE_DEFAULT_EXPORT_DEP_NAME for item in covering.get("runtime_deps") or [])
    assert covering.get("extra_paths")


def test_grow_from_node_default_export_catalog_forages_humanize_string() -> None:
    catalog = load_node_default_export_apply_catalog()
    result = grow_application_task(
        NODE_DEFAULT_EXPORT_GROW_TASK,
        catalog=catalog,
        forage=True,
        hide_before=[NODE_DEFAULT_EXPORT_WINNER_CAPABILITY_ID],
        live_fetch=True,
    )
    assert result["ok"], result
    assert result["grew"] is True
    assert result["winner_slug"] == NODE_DEFAULT_EXPORT_WINNER_SLUG
    assert (result.get("forage") or {}).get("origin", {}).get("kind") == "npm-live"
    assert any(
        item.get("name") == NODE_DEFAULT_EXPORT_DEP_NAME
        for item in (result.get("forage") or {}).get("runtime_deps") or []
    )


def test_node_default_export_plane_grows_and_rejects_tamper(tmp_path: Path) -> None:
    report_dir = tmp_path / "report"
    plane = run_application_node_default_export_growth_plane(report_dir)
    assert plane["ok"], plane
    assert plane["winner"] == NODE_DEFAULT_EXPORT_WINNER_SLUG
    ...
    hidden = build_application_registry(
        ledger,
        hide=[NODE_DEFAULT_EXPORT_WINNER_CAPABILITY_ID],
        include_synthesized=True,
        include_absorbed=True,
    )
    assert plan_application_task(NODE_DEFAULT_EXPORT_GROW_TASK, hidden) is None
    grown = build_application_registry(ledger, include_synthesized=True, include_absorbed=True)
    solved = run_application_task(NODE_DEFAULT_EXPORT_GROW_TASK, grown)
    assert solved["ok"], solved
    verification = verify_application_node_default_export_growth_plane(report_dir)
    assert verification["ok"], verification
    report_path = report_dir / "plane-report.json"
    report = json.loads(report_path.read_text(encoding="utf-8"))
    report["grade"]["grow_winner_is_humanize_string"] = False
    report_path.write_text(json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
    assert not verify_application_node_default_export_growth_plane(report_dir)["ok"]


def test_builtin_application_node_default_export_growth_plane_proof() -> None:
    result = builtin_application_node_default_export_growth_plane_proof()
    assert result["ok"], result
    assert result["action"] == "application_node_default_export_growth_plane"
    assert result["grow_winner_is_humanize_string"]
