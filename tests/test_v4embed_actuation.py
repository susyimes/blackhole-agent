...
from blackhole_agent.v4embed_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_v4embed_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_v4embed_header,
    independent_v4embeddigest,
    parse_http_request,
    parse_http_response,
    parse_message,
    parse_v4embed,
    parse_v4embed_header,
    public_request,
    public_response,
    run_v4embed_workflow,
    serialize_v4embed,
    temporary_request,
    temporary_response,
)
...
def test_goal_binds_v4embed_actuation_plane() -> None:
    assert leftover_marker_ids(V4EMBED_ACTUATION_GOAL) == (V4EMBED_ACTUATION_ID,)
    ...
    assert V4EMBED_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(V4EMBED_LEFTOVER) == (V4EMBED_ACTUATION_ID,)
    assert leftover_marker_ids(V4EMBED_ACTUATION_GOAL) == (V4EMBED_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert V4EMBED_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(V4EMBED_ACTUATION_GOAL)
...
def test_opted_in_v4embed_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("v4embed",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, V4EMBED_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["v4embed"]

    missing = run_v4embed_workflow(with_v4embedid=False)
    skip_bind = run_v4embed_workflow(skip_bind=True)
    skip_temporary = run_v4embed_workflow(do_temporary=False)
    skip_public = run_v4embed_workflow(do_public=False)
    skip_v4embeddigest = run_v4embed_workflow(do_v4embeddigest=False)
    skip_replay = run_v4embed_workflow(replay=False)
    skip_v4embedid = run_v4embed_workflow(use_v4embedid=False)
    live = run_v4embed_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_v4embedid"
    ...
    assert skip_v4embeddigest["ok"] is False
    assert skip_v4embeddigest["error"] == "v4embeddigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_v4embedid["ok"] is False
    assert skip_v4embedid["error"] == "v4embedid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_v4embeddigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["v4embeddigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["v4embedid_bound"] is True
    assert row["digest"]
    assert live["v4embedid"] == DEFAULT_V4EMBEDID
    assert live["v4embeddigest"] == DEFAULT_V4EMBEDDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, v4embedid=DEFAULT_V4EMBEDID, v4embeddigest=DEFAULT_V4EMBEDDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["v4embedid"] == DEFAULT_V4EMBEDID
    assert queried["v4embeddigest"] == DEFAULT_V4EMBEDDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == V4EMBED_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, v4embedid=DEFAULT_V4EMBEDID, v4embeddigest=DEFAULT_V4EMBEDDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["v4embedid"] == DEFAULT_V4EMBEDID
    assert answered["v4embeddigest"] == DEFAULT_V4EMBEDDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, v4embedid=DEFAULT_V4EMBEDID, include_v4embedid=False)
    )
    assert bare["has_v4embedid"] is False
    assert bare["v4embedid"] == EMPTY_V4EMBEDID
    publicised = serialize_v4embed(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_v4embed(publicised) == DEFAULT_SOURCE
    assert parse_v4embed(RFC_DEST_FIELD) == DEST_HOP
    header = parse_v4embed_header(encode_v4embed_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_V4EMBEDID, DEFAULT_V4EMBEDDIGEST))
    ...
    assert asked["method"] == "WKP"
    assert asked["disc_kind"] == "wkp"
    ...
    assert canonical_temporary(SENTINEL, DEFAULT_V4EMBEDID).startswith("WKP")
    assert "v4embeddigest=" in canonical_public(SENTINEL, DEFAULT_V4EMBEDID, DEFAULT_V4EMBEDDIGEST)


def test_builtin_proof_seals_v4embed_actuation() -> None:
    report = builtin_v4embed_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "v4embed_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_v4embed"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_v4embedid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_v4embeddigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_v4embedid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_v4embeddigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_v4embed"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_v4embed"]
    assert report["checks"]["proved_v4embed_consumes_leftover"]
    assert report["mission_goal"] == V4EMBED_ACTUATION_GOAL
    assert report["done_when"] == V4EMBED_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[V4EMBED_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "v4embed" in capability.tags
    assert "rfc6052" in capability.tags
    assert "http" in capability.tags
    assert "v4embedid" in capability.tags
    assert "v4embeddigest" in capability.tags
    assert "wkp" in capability.tags
    assert "nsp" in capability.tags


def test_selection_gate_rejects_ledger_only_v4embed_family(tmp_path: Path) -> None:
    ...
    assert "v4embed" in family.split("/")
    assert "rfc6052" in family
    assert "v4embedid" in set(semantic_tokens(V4EMBED_ACTUATION_GOAL))
    assert "v4embeddigest" in family
    ...
    assert "luprefixid" not in family
    assert "luprefixdigest" not in family
