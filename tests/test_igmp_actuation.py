...
def test_goal_binds_igmp_actuation_plane() -> None:
    assert leftover_marker_ids(IGMP_ACTUATION_GOAL) == (IGMP_ACTUATION_ID,)
    assert leftover_marker_ids(IGMP_LEFTOVER) == (IGMP_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert IGMP_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(IGMP_ACTUATION_GOAL)
...
def test_opted_in_igmp_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("igmp",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, IGMP_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["igmp"]

    missing = run_igmp_workflow(with_igmpid=False)
    skip_bind = run_igmp_workflow(skip_bind=True)
    skip_query = run_igmp_workflow(do_query=False)
    skip_report = run_igmp_workflow(do_report=False)
    skip_igmpdigest = run_igmp_workflow(do_igmpdigest=False)
    skip_replay = run_igmp_workflow(replay=False)
    skip_igmpid = run_igmp_workflow(use_igmpid=False)
    live = run_igmp_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_igmpid"
    ...
    assert skip_igmpdigest["ok"] is False
    assert skip_igmpdigest["error"] == "igmpdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_igmpid["ok"] is False
    assert skip_igmpid["error"] == "igmpid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_igmpdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["query_frame"] is True
    assert row["report_frame"] is True
    assert row["igmpdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["igmpid_bound"] is True
    assert row["digest"]
    assert live["igmpid"] == DEFAULT_IGMPID
    assert live["igmpdigest"] == DEFAULT_IGMPDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_query(identity=SENTINEL, igmpid=DEFAULT_IGMPID, igmpdigest=DEFAULT_IGMPDIGEST)
    )
    assert queried["is_query"] is True and queried["is_report"] is False
    assert queried["identity"] == SENTINEL and queried["igmpid"] == DEFAULT_IGMPID
    assert queried["igmpdigest"] == DEFAULT_IGMPDIGEST
    assert queried["type"] == FRAME_QUERY
    assert queried["first_byte"] == IGMP_FIRST
    answered = parse_message(
        encode_report(identity=SENTINEL, igmpid=DEFAULT_IGMPID, igmpdigest=DEFAULT_IGMPDIGEST)
    )
    assert answered["is_report"] is True and answered["is_report"] is True
    assert answered["igmpid"] == DEFAULT_IGMPID
    assert answered["igmpdigest"] == DEFAULT_IGMPDIGEST
    ...
    bare = parse_message(
        encode_query(identity=SENTINEL, igmpid=DEFAULT_IGMPID, include_igmpid=False)
    )
    assert bare["has_igmpid"] is False
    assert bare["igmpid"] == EMPTY_IGMPID
    advertised = serialize_igmp(DEFAULT_QUERY)
    assert advertised == RFC_QUERY_FIELD
    assert parse_igmp(advertised) == DEFAULT_QUERY
    assert parse_igmp(RFC_REPORT_FIELD) == REPORT_POLICY
    header = parse_igmp_header(encode_igmp_header(DEFAULT_QUERY))
    ...
    listed = parse_http_request(report_request(SENTINEL, DEFAULT_IGMPID, DEFAULT_IGMPDIGEST))
    ...
    assert asked["igmp_kind"] == "query"
    assert listed["igmp_kind"] == "report"
    assert got["status"] == 200
    assert preload_report["status"] == 200
    assert got["policy"] == DEFAULT_QUERY
    assert preload_report["policy"] == REPORT_POLICY
    assert canonical_query(SENTINEL, DEFAULT_IGMPID).startswith("QUERY")
    assert "igmpdigest=" in canonical_report(SENTINEL, DEFAULT_IGMPID, DEFAULT_IGMPDIGEST)


def test_builtin_proof_seals_igmp_actuation() -> None:
    report = builtin_igmp_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "igmp_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_igmp"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_igmpid_is_forbidden"]
    assert report["checks"]["skip_query_stays_empty"]
    assert report["checks"]["skip_report_stays_empty"]
    assert report["checks"]["skip_igmpdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_igmpid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_igmpdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_igmp"]
    ...
    assert report["checks"]["catalog_names_mld"]
    assert report["checks"]["leftover_text_binds_igmp"]
    assert report["checks"]["proved_igmp_consumes_leftover"]
    assert report["mission_goal"] == IGMP_ACTUATION_GOAL
    assert report["done_when"] == IGMP_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[IGMP_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "igmp" in capability.tags
    assert "rfc1112" in capability.tags
    assert "http" in capability.tags
    assert "igmpid" in capability.tags
    assert "igmpdigest" in capability.tags
    assert "query" in capability.tags


def test_selection_gate_rejects_ledger_only_igmp_family(tmp_path: Path) -> None:
    ...
    assert "igmp" in family.split("/")
    assert "rfc1112" in family
    assert "igmpid" in family
    assert "igmpdigest" in family
