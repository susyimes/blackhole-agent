...
def test_goal_binds_rarp_actuation_plane() -> None:
    assert leftover_marker_ids(RARP_ACTUATION_GOAL) == (RARP_ACTUATION_ID,)
    assert leftover_marker_ids(RARP_LEFTOVER) == (RARP_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert RARP_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(RARP_ACTUATION_GOAL)
...
def test_opted_in_rarp_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("rarp",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, RARP_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["rarp"]

    missing = run_rarp_workflow(with_rarpid=False)
    skip_bind = run_rarp_workflow(skip_bind=True)
    skip_reverse = run_rarp_workflow(do_reverse=False)
    skip_reply = run_rarp_workflow(do_reply=False)
    skip_rarpdigest = run_rarp_workflow(do_rarpdigest=False)
    skip_replay = run_rarp_workflow(replay=False)
    skip_rarpid = run_rarp_workflow(use_rarpid=False)
    live = run_rarp_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_rarpid"
    assert skip_bind["ok"] is False
    assert skip_bind["error"] == "not_bound"
    assert skip_reverse["ok"] is False
    assert skip_reverse["error"] == "reverse_required"
    assert skip_reply["ok"] is False
    assert skip_reply["error"] == "reply_required"
    assert skip_rarpdigest["ok"] is False
    assert skip_rarpdigest["error"] == "rarpdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_rarpid["ok"] is False
    assert skip_rarpid["error"] == "rarpid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_rarpdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["reverse_frame"] is True
    assert row["reply_frame"] is True
    assert row["rarpdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["rarpid_bound"] is True
    assert row["digest"]
    assert live["rarpid"] == DEFAULT_RARPID
    assert live["rarpdigest"] == DEFAULT_RARPDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_reverse(identity=SENTINEL, rarpid=DEFAULT_RARPID, rarpdigest=DEFAULT_RARPDIGEST)
    )
    assert queried["is_reverse"] is True and queried["is_reply"] is False
    assert queried["identity"] == SENTINEL and queried["rarpid"] == DEFAULT_RARPID
    assert queried["rarpdigest"] == DEFAULT_RARPDIGEST
    assert queried["type"] == FRAME_REVERSE
    assert queried["first_byte"] == RARP_FIRST
    answered = parse_message(
        encode_reply(identity=SENTINEL, rarpid=DEFAULT_RARPID, rarpdigest=DEFAULT_RARPDIGEST)
    )
    assert answered["is_reply"] is True and answered["is_reply"] is True
    assert answered["rarpid"] == DEFAULT_RARPID
    assert answered["rarpdigest"] == DEFAULT_RARPDIGEST
    ...
    bare = parse_message(
        encode_reverse(identity=SENTINEL, rarpid=DEFAULT_RARPID, include_rarpid=False)
    )
    assert bare["has_rarpid"] is False
    assert bare["rarpid"] == EMPTY_RARPID
    advertised = serialize_rarp(DEFAULT_REVERSE)
    assert advertised == RFC_REVERSE_FIELD
    assert parse_rarp(advertised) == DEFAULT_REVERSE
    assert parse_rarp(RFC_REPLY_FIELD) == REPLY_POLICY
    header = parse_rarp_header(encode_rarp_header(DEFAULT_REVERSE))
    assert header["field_value"] == RFC_REVERSE_FIELD
    assert header["header"] == REVERSE_HEADER
    asked = parse_http_request(reverse_request(SENTINEL, DEFAULT_RARPID))
    listed = parse_http_request(reply_request(SENTINEL, DEFAULT_RARPID, DEFAULT_RARPDIGEST))
    ...
    assert asked["method"] == "REVERSE"
    assert asked["rarp_kind"] == "reverse"
    assert listed["rarp_kind"] == "reply"
    assert got["status"] == 200
    assert preload_reply["status"] == 200
    assert got["policy"] == DEFAULT_REVERSE
    assert preload_reply["policy"] == REPLY_POLICY
    assert canonical_reverse(SENTINEL, DEFAULT_RARPID).startswith("REVERSE")
    assert "rarpdigest=" in canonical_reply(SENTINEL, DEFAULT_RARPID, DEFAULT_RARPDIGEST)


def test_builtin_proof_seals_rarp_actuation() -> None:
    report = builtin_rarp_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "rarp_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_rarp"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_rarpid_is_forbidden"]
    assert report["checks"]["skip_reverse_stays_empty"]
    assert report["checks"]["skip_reply_stays_empty"]
    assert report["checks"]["skip_rarpdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_rarpid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_rarpdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_rarp"]
    ...
    assert report["checks"]["catalog_names_igmp"]
    assert report["checks"]["leftover_text_binds_rarp"]
    assert report["checks"]["proved_rarp_consumes_leftover"]
    assert report["mission_goal"] == RARP_ACTUATION_GOAL
    assert report["done_when"] == RARP_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[RARP_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "rarp" in capability.tags
    assert "rfc903" in capability.tags
    assert "http" in capability.tags
    assert "rarpid" in capability.tags
    assert "rarpdigest" in capability.tags
    assert "reverse" in capability.tags


def test_selection_gate_rejects_ledger_only_rarp_family(tmp_path: Path) -> None:
    ...
    assert "rarp" in family.split("/")
    assert "rfc903" in family
    assert "rarpid" in family
    assert "rarpdigest" in family
