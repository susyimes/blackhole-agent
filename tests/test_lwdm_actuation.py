...
def test_goal_binds_lwdm_actuation_plane() -> None:
    assert leftover_marker_ids(LWDM_ACTUATION_GOAL) == (LWDM_ACTUATION_ID,)
    ...
    assert LWDM_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(LWDM_LEFTOVER) == (LWDM_ACTUATION_ID,)
    assert leftover_marker_ids(LWDM_ACTUATION_GOAL) == (LWDM_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert LWDM_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(LWDM_ACTUATION_GOAL)
...
def test_opted_in_lwdm_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("lwdm",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, LWDM_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["lwdm"]

    missing = run_lwdm_workflow(with_lwdmid=False)
    skip_bind = run_lwdm_workflow(skip_bind=True)
    skip_temporary = run_lwdm_workflow(do_temporary=False)
    skip_public = run_lwdm_workflow(do_public=False)
    skip_lwdmdigest = run_lwdm_workflow(do_lwdmdigest=False)
    skip_replay = run_lwdm_workflow(replay=False)
    skip_lwdmid = run_lwdm_workflow(use_lwdmid=False)
    live = run_lwdm_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_lwdmid"
    ...
    assert skip_lwdmdigest["ok"] is False
    assert skip_lwdmdigest["error"] == "lwdmdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_lwdmid["ok"] is False
    assert skip_lwdmid["error"] == "lwdmid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_lwdmdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["lwdmdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["lwdmid_bound"] is True
    assert row["digest"]
    assert live["lwdmid"] == DEFAULT_LWDMID
    assert live["lwdmdigest"] == DEFAULT_LWDMDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, lwdmid=DEFAULT_LWDMID, lwdmdigest=DEFAULT_LWDMDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["lwdmid"] == DEFAULT_LWDMID
    assert queried["lwdmdigest"] == DEFAULT_LWDMDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == LWDM_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, lwdmid=DEFAULT_LWDMID, lwdmdigest=DEFAULT_LWDMDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["lwdmid"] == DEFAULT_LWDMID
    assert answered["lwdmdigest"] == DEFAULT_LWDMDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, lwdmid=DEFAULT_LWDMID, include_lwdmid=False)
    )
    assert bare["has_lwdmid"] is False
    assert bare["lwdmid"] == EMPTY_LWDMID
    publicised = serialize_lwdm(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_lwdm(publicised) == DEFAULT_SOURCE
    assert parse_lwdm(RFC_DEST_FIELD) == DEST_HOP
    header = parse_lwdm_header(encode_lwdm_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_LWDMID, DEFAULT_LWDMDIGEST))
    ...
    assert asked["method"] == "VERIFY"
    assert asked["disc_kind"] == "verify"
    assert listed["disc_kind"] == "trace"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_LWDMID).startswith("VERIFY")
    assert "lwdmdigest=" in canonical_public(SENTINEL, DEFAULT_LWDMID, DEFAULT_LWDMDIGEST)


def test_builtin_proof_seals_lwdm_actuation() -> None:
    report = builtin_lwdm_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "lwdm_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_lwdm"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_lwdmid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_lwdmdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_lwdmid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_lwdmdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_lwdm"]
    ...
    assert report["checks"]["leftover_text_binds_lwdm"]
    assert report["checks"]["proved_lwdm_consumes_leftover"]
    assert report["mission_goal"] == LWDM_ACTUATION_GOAL
    assert report["done_when"] == LWDM_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[LWDM_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "lwdm" in capability.tags
    assert "rfc4209" in capability.tags
    assert "http" in capability.tags
    assert "lwdmid" in capability.tags
    assert "lwdmdigest" in capability.tags
    assert "verify" in capability.tags
    assert "trace" in capability.tags


def test_selection_gate_rejects_ledger_only_lwdm_family(tmp_path: Path) -> None:
    ...
    assert "lwdm" in family.split("/")
    assert "rfc4209" in set(semantic_tokens(LWDM_ACTUATION_GOAL))
    assert "lwdmid" in set(semantic_tokens(LWDM_ACTUATION_GOAL))
    assert "lwdmdigest" in family
