...
from blackhole_agent.siitdc_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_siitdc_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_siitdc_header,
    independent_siitdcdigest,
    parse_http_request,
    parse_http_response,
    parse_message,
    parse_siitdc,
...
def test_goal_binds_siitdc_actuation_plane() -> None:
    assert leftover_marker_ids(SIITDC_ACTUATION_GOAL) == (SIITDC_ACTUATION_ID,)
    ...
    assert SIITDC_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(SIITDC_LEFTOVER) == (SIITDC_ACTUATION_ID,)
    assert leftover_marker_ids(SIITDC_ACTUATION_GOAL) == (SIITDC_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert SIITDC_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(SIITDC_ACTUATION_GOAL)
...
def test_opted_in_siitdc_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("siitdc",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, SIITDC_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["siitdc"]

    missing = run_siitdc_workflow(with_siitdcid=False)
    skip_bind = run_siitdc_workflow(skip_bind=True)
    skip_temporary = run_siitdc_workflow(do_temporary=False)
    skip_public = run_siitdc_workflow(do_public=False)
    skip_siitdcdigest = run_siitdc_workflow(do_siitdcdigest=False)
    skip_replay = run_siitdc_workflow(replay=False)
    skip_siitdcid = run_siitdc_workflow(use_siitdcid=False)
    live = run_siitdc_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_siitdcid"
    ...
    assert skip_siitdcdigest["ok"] is False
    assert skip_siitdcdigest["error"] == "siitdcdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_siitdcid["ok"] is False
    assert skip_siitdcid["error"] == "siitdcid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_siitdcdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["siitdcdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["siitdcid_bound"] is True
    assert row["digest"]
    assert live["siitdcid"] == DEFAULT_SIITDCID
    assert live["siitdcdigest"] == DEFAULT_SIITDCDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, siitdcid=DEFAULT_SIITDCID, siitdcdigest=DEFAULT_SIITDCDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["siitdcid"] == DEFAULT_SIITDCID
    assert queried["siitdcdigest"] == DEFAULT_SIITDCDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == SIITDC_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, siitdcid=DEFAULT_SIITDCID, siitdcdigest=DEFAULT_SIITDCDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["siitdcid"] == DEFAULT_SIITDCID
    assert answered["siitdcdigest"] == DEFAULT_SIITDCDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, siitdcid=DEFAULT_SIITDCID, include_siitdcid=False)
    )
    assert bare["has_siitdcid"] is False
    assert bare["siitdcid"] == EMPTY_SIITDCID
    publicised = serialize_siitdc(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_siitdc(publicised) == DEFAULT_SOURCE
    assert parse_siitdc(RFC_DEST_FIELD) == DEST_HOP
    header = parse_siitdc_header(encode_siitdc_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_SIITDCID, DEFAULT_SIITDCDIGEST))
    ...
    assert asked["disc_kind"] == "prefix"
    assert listed["disc_kind"] == "dc"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_SIITDCID).startswith("PREFIX")
    assert "siitdcdigest=" in canonical_public(SENTINEL, DEFAULT_SIITDCID, DEFAULT_SIITDCDIGEST)


def test_builtin_proof_seals_siitdc_actuation() -> None:
    report = builtin_siitdc_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "siitdc_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_siitdc"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_siitdcid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_siitdcdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_siitdcid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_siitdcdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_siitdc"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_siitdc"]
    assert report["checks"]["proved_siitdc_consumes_leftover"]
    assert report["mission_goal"] == SIITDC_ACTUATION_GOAL
    assert report["done_when"] == SIITDC_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[SIITDC_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "siitdc" in capability.tags
    assert "rfc7755" in capability.tags
    assert "http" in capability.tags
    assert "siitdcid" in capability.tags
    assert "siitdcdigest" in capability.tags
    assert "prefix" in capability.tags
    assert "dc" in capability.tags


def test_selection_gate_rejects_ledger_only_siitdc_family(tmp_path: Path) -> None:
    ...
    assert "siitdc" in family.split("/")
    assert "rfc7755" in family
    assert "siitdcid" in set(semantic_tokens(SIITDC_ACTUATION_GOAL))
    assert "siitdcdigest" in family
    ...
    assert "siitdtmid" not in family
    assert "siitdtmdigest" not in family
