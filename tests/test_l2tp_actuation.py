...
def test_goal_binds_l2tp_actuation_plane() -> None:
    assert leftover_marker_ids(L2TP_ACTUATION_GOAL) == (L2TP_ACTUATION_ID,)
    ...
    assert L2TP_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(L2TP_LEFTOVER) == (L2TP_ACTUATION_ID,)
    assert leftover_marker_ids(L2TP_ACTUATION_GOAL) == (L2TP_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert L2TP_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(L2TP_ACTUATION_GOAL)
...
def test_opted_in_l2tp_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("l2tp",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, L2TP_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["l2tp"]

    missing = run_l2tp_workflow(with_l2tpid=False)
    skip_bind = run_l2tp_workflow(skip_bind=True)
    skip_temporary = run_l2tp_workflow(do_temporary=False)
    skip_public = run_l2tp_workflow(do_public=False)
    skip_l2tpdigest = run_l2tp_workflow(do_l2tpdigest=False)
    skip_replay = run_l2tp_workflow(replay=False)
    skip_l2tpid = run_l2tp_workflow(use_l2tpid=False)
    live = run_l2tp_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_l2tpid"
    ...
    assert skip_l2tpdigest["ok"] is False
    assert skip_l2tpdigest["error"] == "l2tpdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_l2tpid["ok"] is False
    assert skip_l2tpid["error"] == "l2tpid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_l2tpdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["l2tpdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["l2tpid_bound"] is True
    assert row["digest"]
    assert live["l2tpid"] == DEFAULT_L2TPID
    assert live["l2tpdigest"] == DEFAULT_L2TPDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, l2tpid=DEFAULT_L2TPID, l2tpdigest=DEFAULT_L2TPDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["l2tpid"] == DEFAULT_L2TPID
    assert queried["l2tpdigest"] == DEFAULT_L2TPDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == L2TP_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, l2tpid=DEFAULT_L2TPID, l2tpdigest=DEFAULT_L2TPDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["l2tpid"] == DEFAULT_L2TPID
    assert answered["l2tpdigest"] == DEFAULT_L2TPDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, l2tpid=DEFAULT_L2TPID, include_l2tpid=False)
    )
    assert bare["has_l2tpid"] is False
    assert bare["l2tpid"] == EMPTY_L2TPID
    publicised = serialize_l2tp(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_l2tp(publicised) == DEFAULT_SOURCE
    assert parse_l2tp(RFC_DEST_FIELD) == DEST_HOP
    header = parse_l2tp_header(encode_l2tp_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_L2TPID, DEFAULT_L2TPDIGEST))
    ...
    assert asked["method"] == "L2TP"
    assert asked["disc_kind"] == "l2tp"
    assert listed["disc_kind"] == "spoke"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_L2TPID).startswith("L2TP")
    assert "l2tpdigest=" in canonical_public(SENTINEL, DEFAULT_L2TPID, DEFAULT_L2TPDIGEST)


def test_builtin_proof_seals_l2tp_actuation() -> None:
    report = builtin_l2tp_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "l2tp_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_l2tp"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_l2tpid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_l2tpdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_l2tpid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_l2tpdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_l2tp"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_l2tp"]
    assert report["checks"]["proved_l2tp_consumes_leftover"]
    assert report["mission_goal"] == L2TP_ACTUATION_GOAL
    assert report["done_when"] == L2TP_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[L2TP_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "l2tp" in capability.tags
    assert "rfc5571" in capability.tags
    assert "http" in capability.tags
    assert "l2tpid" in capability.tags
    assert "l2tpdigest" in capability.tags
    assert "l2tp" in capability.tags
    assert "spoke" in capability.tags


def test_selection_gate_rejects_ledger_only_l2tp_family(tmp_path: Path) -> None:
    ...
    assert "l2tp" in family.split("/")
    assert "rfc5571" in family
    assert "l2tpid" in set(semantic_tokens(L2TP_ACTUATION_GOAL))
    assert "l2tpdigest" in family
    ...
    assert "meshid" not in family
    assert "meshdigest" not in family
