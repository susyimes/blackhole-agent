...
from blackhole_agent.mplslse_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_mplslse_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_mplslse_header,
    independent_lsedigest,
    parse_http_comm,
    parse_http_response,
    parse_message,
    parse_mplslse,
    parse_mplslse_header,
    public_comm,
    public_response,
    run_mplslse_workflow,
    serialize_mplslse,
    temporary_comm,
    temporary_response,
)
...
def test_goal_binds_mplslse_actuation_plane() -> None:
    assert leftover_marker_ids(MPLSLSE_ACTUATION_GOAL) == (MPLSLSE_ACTUATION_ID,)
    ...
    assert MPLSLSE_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(MPLSLSE_LEFTOVER) == (MPLSLSE_ACTUATION_ID,)
    assert leftover_marker_ids(MPLSLSE_ACTUATION_GOAL) == (MPLSLSE_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert MPLSLSE_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(MPLSLSE_ACTUATION_GOAL)
...
def test_opted_in_mplslse_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("mplslse",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, MPLSLSE_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["mplslse"]

    missing = run_mplslse_workflow(with_lseid=False)
    skip_bind = run_mplslse_workflow(skip_bind=True)
    skip_temporary = run_mplslse_workflow(do_temporary=False)
    skip_public = run_mplslse_workflow(do_public=False)
    skip_lsedigest = run_mplslse_workflow(do_lsedigest=False)
    skip_replay = run_mplslse_workflow(replay=False)
    skip_lseid = run_mplslse_workflow(use_lseid=False)
    live = run_mplslse_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_lseid"
    ...
    assert skip_lsedigest["ok"] is False
    assert skip_lsedigest["error"] == "lsedigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_lseid["ok"] is False
    assert skip_lseid["error"] == "lseid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_lsedigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["lsedigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["lseid_bound"] is True
    assert row["digest"]
    assert live["lseid"] == DEFAULT_LSEID
    assert live["lsedigest"] == DEFAULT_LSEDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, lseid=DEFAULT_LSEID, lsedigest=DEFAULT_LSEDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["lseid"] == DEFAULT_LSEID
    assert queried["lsedigest"] == DEFAULT_LSEDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == MPLSLSE_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, lseid=DEFAULT_LSEID, lsedigest=DEFAULT_LSEDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["lseid"] == DEFAULT_LSEID
    assert answered["lsedigest"] == DEFAULT_LSEDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, lseid=DEFAULT_LSEID, include_lseid=False)
    )
    assert bare["has_lseid"] is False
    assert bare["lseid"] == EMPTY_LSEID
    publicised = serialize_mplslse(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_mplslse(publicised) == DEFAULT_SOURCE
    assert parse_mplslse(RFC_DEST_FIELD) == DEST_HOP
    header = parse_mplslse_header(encode_mplslse_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_LSEID, DEFAULT_LSEDIGEST))
    ...
    assert asked["method"] == "LABEL"
    assert asked["disc_kind"] == "label"
    assert listed["disc_kind"] == "stack"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_LSEID).startswith("LABEL")
    assert "lsedigest=" in canonical_public(SENTINEL, DEFAULT_LSEID, DEFAULT_LSEDIGEST)


def test_builtin_proof_seals_mplslse_actuation() -> None:
    report = builtin_mplslse_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "mplslse_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_mplslse"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_lseid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_lsedigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_lseid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_lsedigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_mplslse"]
    ...
    assert report["checks"]["leftover_text_binds_mplslse"]
    assert report["checks"]["proved_mplslse_consumes_leftover"]
    assert report["mission_goal"] == MPLSLSE_ACTUATION_GOAL
    assert report["done_when"] == MPLSLSE_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[MPLSLSE_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "mplslse" in capability.tags
    assert "rfc3032" in capability.tags
    assert "http" in capability.tags
    assert "lseid" in capability.tags
    assert "lsedigest" in capability.tags
    assert "label" in capability.tags
    assert "stack" in capability.tags


def test_selection_gate_rejects_ledger_only_mplslse_family(tmp_path: Path) -> None:
    ...
    assert "mplslse" in family.split("/")
    assert "rfc3032" in set(semantic_tokens(MPLSLSE_ACTUATION_GOAL))
    assert "lseid" in set(semantic_tokens(MPLSLSE_ACTUATION_GOAL))
    assert "lsedigest" in family
