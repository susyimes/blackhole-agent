...
def test_goal_binds_mapt_actuation_plane() -> None:
    assert leftover_marker_ids(MAPT_ACTUATION_GOAL) == (MAPT_ACTUATION_ID,)
    ...
    assert MAPT_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(MAPT_LEFTOVER) == (MAPT_ACTUATION_ID,)
    assert leftover_marker_ids(MAPT_ACTUATION_GOAL) == (MAPT_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert MAPT_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(MAPT_ACTUATION_GOAL)
...
def test_opted_in_mapt_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("mapt",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, MAPT_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["mapt"]

    missing = run_mapt_workflow(with_maptid=False)
    skip_bind = run_mapt_workflow(skip_bind=True)
    skip_temporary = run_mapt_workflow(do_temporary=False)
    skip_public = run_mapt_workflow(do_public=False)
    skip_maptdigest = run_mapt_workflow(do_maptdigest=False)
    skip_replay = run_mapt_workflow(replay=False)
    skip_maptid = run_mapt_workflow(use_maptid=False)
    live = run_mapt_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_maptid"
    ...
    assert skip_maptdigest["ok"] is False
    assert skip_maptdigest["error"] == "maptdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_maptid["ok"] is False
    assert skip_maptid["error"] == "maptid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_maptdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["maptdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["maptid_bound"] is True
    assert row["digest"]
    assert live["maptid"] == DEFAULT_MAPTID
    assert live["maptdigest"] == DEFAULT_MAPTDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, maptid=DEFAULT_MAPTID, maptdigest=DEFAULT_MAPTDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["maptid"] == DEFAULT_MAPTID
    assert queried["maptdigest"] == DEFAULT_MAPTDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == MAPT_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, maptid=DEFAULT_MAPTID, maptdigest=DEFAULT_MAPTDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["maptid"] == DEFAULT_MAPTID
    assert answered["maptdigest"] == DEFAULT_MAPTDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, maptid=DEFAULT_MAPTID, include_maptid=False)
    )
    assert bare["has_maptid"] is False
    assert bare["maptid"] == EMPTY_MAPTID
    publicised = serialize_mapt(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_mapt(publicised) == DEFAULT_SOURCE
    assert parse_mapt(RFC_DEST_FIELD) == DEST_HOP
    header = parse_mapt_header(encode_mapt_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_MAPTID, DEFAULT_MAPTDIGEST))
    ...
    assert asked["method"] == "DMR"
    assert asked["disc_kind"] == "dmr"
    assert listed["disc_kind"] == "ea"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_MAPTID).startswith("DMR")
    assert "maptdigest=" in canonical_public(SENTINEL, DEFAULT_MAPTID, DEFAULT_MAPTDIGEST)


def test_builtin_proof_seals_mapt_actuation() -> None:
    report = builtin_mapt_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "mapt_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_mapt"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_maptid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_maptdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_maptid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_maptdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_mapt"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_mapt"]
    assert report["checks"]["proved_mapt_consumes_leftover"]
    assert report["mission_goal"] == MAPT_ACTUATION_GOAL
    assert report["done_when"] == MAPT_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[MAPT_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "mapt" in capability.tags
    assert "rfc7599" in capability.tags
    assert "http" in capability.tags
    assert "maptid" in capability.tags
    assert "maptdigest" in capability.tags
    assert "dmr" in capability.tags
    assert "ea" in capability.tags


def test_selection_gate_rejects_ledger_only_mapt_family(tmp_path: Path) -> None:
    ...
    assert "mapt" in family.split("/")
    assert "rfc7599" in family
    assert "maptid" in family
    assert "maptdigest" in family
