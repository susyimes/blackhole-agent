...
from blackhole_agent.sixin4_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_sixin4_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_sixin4_header,
    independent_sixin4digest,
    parse_http_request,
    parse_http_response,
    parse_message,
    parse_sixin4,
...
def test_goal_binds_sixin4_actuation_plane() -> None:
    assert leftover_marker_ids(SIXIN4_ACTUATION_GOAL) == (SIXIN4_ACTUATION_ID,)
    ...
    assert SIXIN4_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(SIXIN4_LEFTOVER) == (SIXIN4_ACTUATION_ID,)
    assert leftover_marker_ids(SIXIN4_ACTUATION_GOAL) == (SIXIN4_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert SIXIN4_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(SIXIN4_ACTUATION_GOAL)
...
def test_opted_in_sixin4_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("sixin4",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, SIXIN4_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["sixin4"]

    missing = run_sixin4_workflow(with_sixin4id=False)
    skip_bind = run_sixin4_workflow(skip_bind=True)
    skip_temporary = run_sixin4_workflow(do_temporary=False)
    skip_public = run_sixin4_workflow(do_public=False)
    skip_sixin4digest = run_sixin4_workflow(do_sixin4digest=False)
    skip_replay = run_sixin4_workflow(replay=False)
    skip_sixin4id = run_sixin4_workflow(use_sixin4id=False)
    live = run_sixin4_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_sixin4id"
    ...
    assert skip_sixin4digest["ok"] is False
    assert skip_sixin4digest["error"] == "sixin4digest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_sixin4id["ok"] is False
    assert skip_sixin4id["error"] == "sixin4id_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_sixin4digest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["sixin4digest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["sixin4id_bound"] is True
    assert row["digest"]
    assert live["sixin4id"] == DEFAULT_SIXIN4ID
    assert live["sixin4digest"] == DEFAULT_SIXIN4DIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, sixin4id=DEFAULT_SIXIN4ID, sixin4digest=DEFAULT_SIXIN4DIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["sixin4id"] == DEFAULT_SIXIN4ID
    assert queried["sixin4digest"] == DEFAULT_SIXIN4DIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == SIXIN4_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, sixin4id=DEFAULT_SIXIN4ID, sixin4digest=DEFAULT_SIXIN4DIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["sixin4id"] == DEFAULT_SIXIN4ID
    assert answered["sixin4digest"] == DEFAULT_SIXIN4DIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, sixin4id=DEFAULT_SIXIN4ID, include_sixin4id=False)
    )
    assert bare["has_sixin4id"] is False
    assert bare["sixin4id"] == EMPTY_SIXIN4ID
    publicised = serialize_sixin4(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_sixin4(publicised) == DEFAULT_SOURCE
    assert parse_sixin4(RFC_DEST_FIELD) == DEST_HOP
    header = parse_sixin4_header(encode_sixin4_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_SIXIN4ID, DEFAULT_SIXIN4DIGEST))
    ...
    assert asked["method"] == "6IN4"
    assert asked["disc_kind"] == "sixin4"
    assert listed["disc_kind"] == "config"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_SIXIN4ID).startswith("6IN4")
    assert "sixin4digest=" in canonical_public(SENTINEL, DEFAULT_SIXIN4ID, DEFAULT_SIXIN4DIGEST)


def test_builtin_proof_seals_sixin4_actuation() -> None:
    report = builtin_sixin4_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "sixin4_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_sixin4"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_sixin4id_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_sixin4digest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_sixin4id_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_sixin4digest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_sixin4"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_sixin4"]
    assert report["checks"]["proved_sixin4_consumes_leftover"]
    assert report["mission_goal"] == SIXIN4_ACTUATION_GOAL
    assert report["done_when"] == SIXIN4_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[SIXIN4_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "sixin4" in capability.tags
    assert "rfc4213" in capability.tags
    assert "http" in capability.tags
    assert "sixin4id" in capability.tags
    assert "sixin4digest" in capability.tags
    assert "sixin4" in capability.tags
    assert "config" in capability.tags


def test_selection_gate_rejects_ledger_only_sixin4_family(tmp_path: Path) -> None:
    ...
    assert "sixin4" in family.split("/")
    assert "rfc4213" in family
    assert "sixin4id" in set(semantic_tokens(SIXIN4_ACTUATION_GOAL))
    assert "sixin4digest" in family
