...
from blackhole_agent.isatap_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_isatap_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_isatap_header,
    independent_isatapdigest,
    parse_http_request,
    parse_http_response,
    parse_message,
    parse_isatap,
...
def test_goal_binds_isatap_actuation_plane() -> None:
    assert leftover_marker_ids(ISATAP_ACTUATION_GOAL) == (ISATAP_ACTUATION_ID,)
    ...
    assert ISATAP_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(ISATAP_LEFTOVER) == (ISATAP_ACTUATION_ID,)
    assert leftover_marker_ids(ISATAP_ACTUATION_GOAL) == (ISATAP_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert ISATAP_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(ISATAP_ACTUATION_GOAL)
...
def test_opted_in_isatap_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("isatap",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, ISATAP_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["isatap"]

    missing = run_isatap_workflow(with_isatapid=False)
    skip_bind = run_isatap_workflow(skip_bind=True)
    skip_temporary = run_isatap_workflow(do_temporary=False)
    skip_public = run_isatap_workflow(do_public=False)
    skip_isatapdigest = run_isatap_workflow(do_isatapdigest=False)
    skip_replay = run_isatap_workflow(replay=False)
    skip_isatapid = run_isatap_workflow(use_isatapid=False)
    live = run_isatap_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_isatapid"
    ...
    assert skip_isatapdigest["ok"] is False
    assert skip_isatapdigest["error"] == "isatapdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_isatapid["ok"] is False
    assert skip_isatapid["error"] == "isatapid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_isatapdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["isatapdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["isatapid_bound"] is True
    assert row["digest"]
    assert live["isatapid"] == DEFAULT_ISATAPID
    assert live["isatapdigest"] == DEFAULT_ISATAPDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, isatapid=DEFAULT_ISATAPID, isatapdigest=DEFAULT_ISATAPDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["isatapid"] == DEFAULT_ISATAPID
    assert queried["isatapdigest"] == DEFAULT_ISATAPDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == ISATAP_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, isatapid=DEFAULT_ISATAPID, isatapdigest=DEFAULT_ISATAPDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["isatapid"] == DEFAULT_ISATAPID
    assert answered["isatapdigest"] == DEFAULT_ISATAPDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, isatapid=DEFAULT_ISATAPID, include_isatapid=False)
    )
    assert bare["has_isatapid"] is False
    assert bare["isatapid"] == EMPTY_ISATAPID
    publicised = serialize_isatap(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_isatap(publicised) == DEFAULT_SOURCE
    assert parse_isatap(RFC_DEST_FIELD) == DEST_HOP
    header = parse_isatap_header(encode_isatap_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_ISATAPID, DEFAULT_ISATAPDIGEST))
    ...
    assert asked["method"] == "ISATAP"
    assert asked["disc_kind"] == "isatap"
    assert listed["disc_kind"] == "prl"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_ISATAPID).startswith("ISATAP")
    assert "isatapdigest=" in canonical_public(SENTINEL, DEFAULT_ISATAPID, DEFAULT_ISATAPDIGEST)


def test_builtin_proof_seals_isatap_actuation() -> None:
    report = builtin_isatap_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "isatap_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_isatap"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_isatapid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_isatapdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_isatapid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_isatapdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_isatap"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_isatap"]
    assert report["checks"]["proved_isatap_consumes_leftover"]
    assert report["mission_goal"] == ISATAP_ACTUATION_GOAL
    assert report["done_when"] == ISATAP_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[ISATAP_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "isatap" in capability.tags
    assert "rfc5214" in capability.tags
    assert "http" in capability.tags
    assert "isatapid" in capability.tags
    assert "isatapdigest" in capability.tags
    assert "isatap" in capability.tags
