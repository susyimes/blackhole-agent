...
def test_goal_binds_oir_actuation_plane() -> None:
    assert leftover_marker_ids(OIR_ACTUATION_GOAL) == (OIR_ACTUATION_ID,)
    ...
    assert OIR_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(OIR_LEFTOVER) == (OIR_ACTUATION_ID,)
    assert leftover_marker_ids(OIR_ACTUATION_GOAL) == (OIR_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert OIR_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(OIR_ACTUATION_GOAL)
...
def test_opted_in_oir_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("oir",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, OIR_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["oir"]

    missing = run_oir_workflow(with_oirid=False)
    skip_bind = run_oir_workflow(skip_bind=True)
    skip_temporary = run_oir_workflow(do_temporary=False)
    skip_public = run_oir_workflow(do_public=False)
    skip_oirdigest = run_oir_workflow(do_oirdigest=False)
    skip_replay = run_oir_workflow(replay=False)
    skip_oirid = run_oir_workflow(use_oirid=False)
    live = run_oir_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_oirid"
    ...
    assert skip_oirdigest["ok"] is False
    assert skip_oirdigest["error"] == "oirdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_oirid["ok"] is False
    assert skip_oirid["error"] == "oirid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_oirdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["oirdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["oirid_bound"] is True
    assert row["digest"]
    assert live["oirid"] == DEFAULT_OIRID
    assert live["oirdigest"] == DEFAULT_OIRDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, oirid=DEFAULT_OIRID, oirdigest=DEFAULT_OIRDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["oirid"] == DEFAULT_OIRID
    assert queried["oirdigest"] == DEFAULT_OIRDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == OIR_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, oirid=DEFAULT_OIRID, oirdigest=DEFAULT_OIRDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["oirid"] == DEFAULT_OIRID
    assert answered["oirdigest"] == DEFAULT_OIRDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, oirid=DEFAULT_OIRID, include_oirid=False)
    )
    assert bare["has_oirid"] is False
    assert bare["oirid"] == EMPTY_OIRID
    publicised = serialize_oir(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_oir(publicised) == DEFAULT_SOURCE
    assert parse_oir(RFC_DEST_FIELD) == DEST_HOP
    header = parse_oir_header(encode_oir_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_OIRID, DEFAULT_OIRDIGEST))
    ...
    assert asked["method"] == "OIR"
    assert asked["disc_kind"] == "oir"
    assert listed["disc_kind"] == "bum"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_OIRID).startswith("OIR")
    assert "oirdigest=" in canonical_public(SENTINEL, DEFAULT_OIRID, DEFAULT_OIRDIGEST)


def test_builtin_proof_seals_oir_actuation() -> None:
    report = builtin_oir_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "oir_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_oir"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_oirid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_oirdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_oirid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_oirdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_oir"]
    ...
    assert report["checks"]["leftover_text_binds_oir"]
    assert report["checks"]["proved_oir_consumes_leftover"]
    assert report["mission_goal"] == OIR_ACTUATION_GOAL
    assert report["done_when"] == OIR_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[OIR_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "oir" in capability.tags
    assert "rfc9574" in capability.tags
    assert "http" in capability.tags
    assert "oirid" in capability.tags
    assert "oirdigest" in capability.tags
    assert "oir" in capability.tags
    assert "bum" in capability.tags


def test_selection_gate_rejects_ledger_only_oir_family(tmp_path: Path) -> None:
    ...
    assert "oir" in family.split("/")
    assert "rfc9574" in set(semantic_tokens(OIR_ACTUATION_GOAL))
    assert "oirid" in set(semantic_tokens(OIR_ACTUATION_GOAL))
    assert "oirdigest" in family
