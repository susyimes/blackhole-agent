...
def test_goal_binds_rtr_actuation_plane() -> None:
    assert leftover_marker_ids(RTR_ACTUATION_GOAL) == (RTR_ACTUATION_ID,)
    ...
    assert RTR_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(RTR_LEFTOVER) == (RTR_ACTUATION_ID,)
    assert leftover_marker_ids(RTR_ACTUATION_GOAL) == (RTR_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert RTR_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(RTR_ACTUATION_GOAL)
...
def test_opted_in_rtr_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("rtr",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, RTR_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["rtr"]

    missing = run_rtr_workflow(with_rtrid=False)
    skip_bind = run_rtr_workflow(skip_bind=True)
    skip_temporary = run_rtr_workflow(do_temporary=False)
    skip_public = run_rtr_workflow(do_public=False)
    skip_rtrdigest = run_rtr_workflow(do_rtrdigest=False)
    skip_replay = run_rtr_workflow(replay=False)
    skip_rtrid = run_rtr_workflow(use_rtrid=False)
    live = run_rtr_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_rtrid"
    ...
    assert skip_rtrdigest["ok"] is False
    assert skip_rtrdigest["error"] == "rtrdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_rtrid["ok"] is False
    assert skip_rtrid["error"] == "rtrid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_rtrdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["rtrdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["rtrid_bound"] is True
    assert row["digest"]
    assert live["rtrid"] == DEFAULT_RTRID
    assert live["rtrdigest"] == DEFAULT_RTRDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, rtrid=DEFAULT_RTRID, rtrdigest=DEFAULT_RTRDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["rtrid"] == DEFAULT_RTRID
    assert queried["rtrdigest"] == DEFAULT_RTRDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == RTR_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, rtrid=DEFAULT_RTRID, rtrdigest=DEFAULT_RTRDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["rtrid"] == DEFAULT_RTRID
    assert answered["rtrdigest"] == DEFAULT_RTRDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, rtrid=DEFAULT_RTRID, include_rtrid=False)
    )
    assert bare["has_rtrid"] is False
    assert bare["rtrid"] == EMPTY_RTRID
    publicised = serialize_rtr(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_rtr(publicised) == DEFAULT_SOURCE
    assert parse_rtr(RFC_DEST_FIELD) == DEST_HOP
    header = parse_rtr_header(encode_rtr_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_RTRID, DEFAULT_RTRDIGEST))
    ...
    assert asked["method"] == "SERIAL"
    assert asked["disc_kind"] == "rtr"
    assert listed["disc_kind"] == "reset"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_RTRID).startswith("SERIAL")
    assert "rtrdigest=" in canonical_public(SENTINEL, DEFAULT_RTRID, DEFAULT_RTRDIGEST)


def test_builtin_proof_seals_rtr_actuation() -> None:
    report = builtin_rtr_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "rtr_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_rtr"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_rtrid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_rtrdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_rtrid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_rtrdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_rtr"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_rtr"]
    assert report["checks"]["proved_rtr_consumes_leftover"]
    assert report["mission_goal"] == RTR_ACTUATION_GOAL
    assert report["done_when"] == RTR_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[RTR_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "rtr" in capability.tags
    assert "rfc8210" in capability.tags
    assert "http" in capability.tags
    assert "rtrid" in capability.tags
    assert "rtrdigest" in capability.tags
    assert "rtr" in capability.tags
    assert "reset" in capability.tags


def test_selection_gate_rejects_ledger_only_rtr_family(tmp_path: Path) -> None:
    ...
    assert "rtr" in family.split("/")
    assert "rfc8210" in family
    assert "rtrid" in set(semantic_tokens(RTR_ACTUATION_GOAL))
    assert "rtrdigest" in family
