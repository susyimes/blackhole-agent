...
from blackhole_agent.lsphier_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_lsphier_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_lsphier_header,
    independent_lsphierdigest,
    parse_http_comm,
    parse_http_response,
    parse_message,
    parse_lsphier,
    parse_lsphier_header,
    public_comm,
    public_response,
    run_lsphier_workflow,
    serialize_lsphier,
    temporary_comm,
    temporary_response,
)
...
def test_goal_binds_lsphier_actuation_plane() -> None:
    assert leftover_marker_ids(LSPHIER_ACTUATION_GOAL) == (LSPHIER_ACTUATION_ID,)
    ...
    assert LSPHIER_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(LSPHIER_LEFTOVER) == (LSPHIER_ACTUATION_ID,)
    assert leftover_marker_ids(LSPHIER_ACTUATION_GOAL) == (LSPHIER_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert LSPHIER_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(LSPHIER_ACTUATION_GOAL)
...
def test_opted_in_lsphier_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("lsphier",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, LSPHIER_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["lsphier"]

    missing = run_lsphier_workflow(with_lsphierid=False)
    skip_bind = run_lsphier_workflow(skip_bind=True)
    skip_temporary = run_lsphier_workflow(do_temporary=False)
    skip_public = run_lsphier_workflow(do_public=False)
    skip_lsphierdigest = run_lsphier_workflow(do_lsphierdigest=False)
    skip_replay = run_lsphier_workflow(replay=False)
    skip_lsphierid = run_lsphier_workflow(use_lsphierid=False)
    live = run_lsphier_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_lsphierid"
    ...
    assert skip_lsphierdigest["ok"] is False
    assert skip_lsphierdigest["error"] == "lsphierdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_lsphierid["ok"] is False
    assert skip_lsphierid["error"] == "lsphierid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_lsphierdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["lsphierdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["lsphierid_bound"] is True
    assert row["digest"]
    assert live["lsphierid"] == DEFAULT_LSPHIERID
    assert live["lsphierdigest"] == DEFAULT_LSPHIERDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, lsphierid=DEFAULT_LSPHIERID, lsphierdigest=DEFAULT_LSPHIERDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["lsphierid"] == DEFAULT_LSPHIERID
    assert queried["lsphierdigest"] == DEFAULT_LSPHIERDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == LSPHIER_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, lsphierid=DEFAULT_LSPHIERID, lsphierdigest=DEFAULT_LSPHIERDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["lsphierid"] == DEFAULT_LSPHIERID
    assert answered["lsphierdigest"] == DEFAULT_LSPHIERDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, lsphierid=DEFAULT_LSPHIERID, include_lsphierid=False)
    )
    assert bare["has_lsphierid"] is False
    assert bare["lsphierid"] == EMPTY_LSPHIERID
    publicised = serialize_lsphier(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_lsphier(publicised) == DEFAULT_SOURCE
    assert parse_lsphier(RFC_DEST_FIELD) == DEST_HOP
    header = parse_lsphier_header(encode_lsphier_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_LSPHIERID, DEFAULT_LSPHIERDIGEST))
    ...
    assert asked["method"] == "FA"
    assert asked["disc_kind"] == "fa"
    assert listed["disc_kind"] == "hierarchy"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_LSPHIERID).startswith("FA")
    assert "lsphierdigest=" in canonical_public(SENTINEL, DEFAULT_LSPHIERID, DEFAULT_LSPHIERDIGEST)


def test_builtin_proof_seals_lsphier_actuation() -> None:
    report = builtin_lsphier_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "lsphier_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_lsphier"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_lsphierid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_lsphierdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_lsphierid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_lsphierdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_lsphier"]
    ...
    assert report["checks"]["leftover_text_binds_lsphier"]
    assert report["checks"]["proved_lsphier_consumes_leftover"]
    assert report["mission_goal"] == LSPHIER_ACTUATION_GOAL
    assert report["done_when"] == LSPHIER_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[LSPHIER_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "lsphier" in capability.tags
    assert "rfc4206" in capability.tags
    assert "http" in capability.tags
    assert "lsphierid" in capability.tags
    assert "lsphierdigest" in capability.tags
    assert "fa" in capability.tags
    assert "hierarchy" in capability.tags


def test_selection_gate_rejects_ledger_only_lsphier_family(tmp_path: Path) -> None:
    ...
    assert "lsphier" in family.split("/")
    assert "rfc4206" in set(semantic_tokens(LSPHIER_ACTUATION_GOAL))
    assert "lsphierid" in set(semantic_tokens(LSPHIER_ACTUATION_GOAL))
    assert "lsphierdigest" in family
