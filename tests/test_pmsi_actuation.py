...
def test_goal_binds_pmsi_actuation_plane() -> None:
    assert leftover_marker_ids(PMSI_ACTUATION_GOAL) == (PMSI_ACTUATION_ID,)
    ...
    assert PMSI_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(PMSI_LEFTOVER) == (PMSI_ACTUATION_ID,)
    assert leftover_marker_ids(PMSI_ACTUATION_GOAL) == (PMSI_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert PMSI_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(PMSI_ACTUATION_GOAL)
...
def test_opted_in_pmsi_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("pmsi",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, PMSI_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["pmsi"]

    missing = run_pmsi_workflow(with_pmsiid=False)
    skip_bind = run_pmsi_workflow(skip_bind=True)
    skip_temporary = run_pmsi_workflow(do_temporary=False)
    skip_public = run_pmsi_workflow(do_public=False)
    skip_pmsidigest = run_pmsi_workflow(do_pmsidigest=False)
    skip_replay = run_pmsi_workflow(replay=False)
    skip_pmsiid = run_pmsi_workflow(use_pmsiid=False)
    live = run_pmsi_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_pmsiid"
    ...
    assert skip_pmsidigest["ok"] is False
    assert skip_pmsidigest["error"] == "pmsidigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_pmsiid["ok"] is False
    assert skip_pmsiid["error"] == "pmsiid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_pmsidigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["pmsidigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["pmsiid_bound"] is True
    assert row["digest"]
    assert live["pmsiid"] == DEFAULT_PMSIID
    assert live["pmsidigest"] == DEFAULT_PMSIDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, pmsiid=DEFAULT_PMSIID, pmsidigest=DEFAULT_PMSIDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["pmsiid"] == DEFAULT_PMSIID
    assert queried["pmsidigest"] == DEFAULT_PMSIDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == PMSI_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, pmsiid=DEFAULT_PMSIID, pmsidigest=DEFAULT_PMSIDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["pmsiid"] == DEFAULT_PMSIID
    assert answered["pmsidigest"] == DEFAULT_PMSIDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, pmsiid=DEFAULT_PMSIID, include_pmsiid=False)
    )
    assert bare["has_pmsiid"] is False
    assert bare["pmsiid"] == EMPTY_PMSIID
    publicised = serialize_pmsi(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_pmsi(publicised) == DEFAULT_SOURCE
    assert parse_pmsi(RFC_DEST_FIELD) == DEST_HOP
    header = parse_pmsi_header(encode_pmsi_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_PMSIID, DEFAULT_PMSIDIGEST))
    ...
    assert asked["method"] == "PMSI"
    assert asked["disc_kind"] == "pmsi"
    assert listed["disc_kind"] == "nlri"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_PMSIID).startswith("PMSI")
    assert "pmsidigest=" in canonical_public(SENTINEL, DEFAULT_PMSIID, DEFAULT_PMSIDIGEST)


def test_builtin_proof_seals_pmsi_actuation() -> None:
    report = builtin_pmsi_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "pmsi_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_pmsi"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_pmsiid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_pmsidigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_pmsiid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_pmsidigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_binds_pmsi"]
    ...
    assert report["checks"]["catalog_names_wson"]
    assert report["checks"]["leftover_text_binds_pmsi"]
    assert report["checks"]["proved_pmsi_consumes_leftover"]
    assert report["mission_goal"] == PMSI_ACTUATION_GOAL
    assert report["done_when"] == PMSI_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[PMSI_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "pmsi" in capability.tags
    assert "rfc6514" in capability.tags
    assert "http" in capability.tags
    assert "pmsiid" in capability.tags
    assert "pmsidigest" in capability.tags
    assert "nlri" in capability.tags


def test_selection_gate_rejects_ledger_only_pmsi_family(tmp_path: Path) -> None:
    ...
    assert "pmsi" in family.split("/")
    assert "rfc6514" in set(semantic_tokens(PMSI_ACTUATION_GOAL))
    assert "pmsiid" in set(semantic_tokens(PMSI_ACTUATION_GOAL))
    assert "pmsidigest" in family
    ...
    assert "wildad" not in family.split("/")
    assert "rfc6625" not in family
    assert "wildid" not in family
    assert "wilddigest" not in family
