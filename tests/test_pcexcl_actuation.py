...
from blackhole_agent.pcexcl_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_pcexcl_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_pcexcl_header,
    independent_pcexcldigest,
    parse_http_comm,
    parse_http_response,
    parse_message,
    parse_pcexcl,
...
def test_goal_binds_pcexcl_actuation_plane() -> None:
    assert leftover_marker_ids(PCEXCL_ACTUATION_GOAL) == (PCEXCL_ACTUATION_ID,)
    ...
    assert PCEXCL_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(PCEXCL_LEFTOVER) == (PCEXCL_ACTUATION_ID,)
    assert leftover_marker_ids(PCEXCL_ACTUATION_GOAL) == (PCEXCL_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert PCEXCL_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(PCEXCL_ACTUATION_GOAL)
...
def test_opted_in_pcexcl_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("pcexcl",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, PCEXCL_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["pcexcl"]

    missing = run_pcexcl_workflow(with_pcexclid=False)
    skip_bind = run_pcexcl_workflow(skip_bind=True)
    skip_temporary = run_pcexcl_workflow(do_temporary=False)
    skip_public = run_pcexcl_workflow(do_public=False)
    skip_pcexcldigest = run_pcexcl_workflow(do_pcexcldigest=False)
    skip_replay = run_pcexcl_workflow(replay=False)
    skip_pcexclid = run_pcexcl_workflow(use_pcexclid=False)
    live = run_pcexcl_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_pcexclid"
    ...
    assert skip_pcexcldigest["ok"] is False
    assert skip_pcexcldigest["error"] == "pcexcldigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_pcexclid["ok"] is False
    assert skip_pcexclid["error"] == "pcexclid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_pcexcldigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["pcexcldigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["pcexclid_bound"] is True
    assert row["digest"]
    assert live["pcexclid"] == DEFAULT_PCEXCLID
    assert live["pcexcldigest"] == DEFAULT_PCEXCLDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, pcexclid=DEFAULT_PCEXCLID, pcexcldigest=DEFAULT_PCEXCLDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["pcexclid"] == DEFAULT_PCEXCLID
    assert queried["pcexcldigest"] == DEFAULT_PCEXCLDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == PCEXCL_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, pcexclid=DEFAULT_PCEXCLID, pcexcldigest=DEFAULT_PCEXCLDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["pcexclid"] == DEFAULT_PCEXCLID
    assert answered["pcexcldigest"] == DEFAULT_PCEXCLDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, pcexclid=DEFAULT_PCEXCLID, include_pcexclid=False)
    )
    assert bare["has_pcexclid"] is False
    assert bare["pcexclid"] == EMPTY_PCEXCLID
    publicised = serialize_pcexcl(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_pcexcl(publicised) == DEFAULT_SOURCE
    assert parse_pcexcl(RFC_DEST_FIELD) == DEST_HOP
    header = parse_pcexcl_header(encode_pcexcl_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_PCEXCLID, DEFAULT_PCEXCLDIGEST))
    ...
    assert asked["method"] == "EXCLUDE"
    assert asked["disc_kind"] == "exclude"
    assert listed["disc_kind"] == "xro"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_PCEXCLID).startswith("EXCLUDE")
    assert "pcexcldigest=" in canonical_public(SENTINEL, DEFAULT_PCEXCLID, DEFAULT_PCEXCLDIGEST)


def test_builtin_proof_seals_pcexcl_actuation() -> None:
    report = builtin_pcexcl_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "pcexcl_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_pcexcl"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_pcexclid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_pcexcldigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_pcexclid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_pcexcldigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_binds_pcexcl"]
    ...
    assert report["checks"]["leftover_text_binds_pcexcl"]
    assert report["checks"]["proved_pcexcl_consumes_leftover"]
    assert report["mission_goal"] == PCEXCL_ACTUATION_GOAL
    assert report["done_when"] == PCEXCL_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[PCEXCL_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "pcexcl" in capability.tags
    assert "rfc5521" in capability.tags
    assert "http" in capability.tags
    assert "pcexclid" in capability.tags
    assert "pcexcldigest" in capability.tags
    assert "exclude" in capability.tags
    assert "xro" in capability.tags


def test_selection_gate_rejects_ledger_only_pcexcl_family(tmp_path: Path) -> None:
    ...
    assert "pcexcl" in family.split("/")
    assert "rfc5521" in set(semantic_tokens(PCEXCL_ACTUATION_GOAL))
    assert "pcexclid" in set(semantic_tokens(PCEXCL_ACTUATION_GOAL))
    assert "pcexcldigest" in family
