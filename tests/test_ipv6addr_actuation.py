...
from blackhole_agent.ipv6addr_actuation import (
    ...
    SENTINEL,
    GLOBAL_HEADER,
    builtin_ipv6addr_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_ipv6addr_header,
    independent_ipv6addrdigest,
    parse_http_request,
    parse_http_response,
    parse_message,
    parse_ipv6addr,
    parse_ipv6addr_header,
    public_request,
    public_response,
    run_ipv6addr_workflow,
    serialize_ipv6addr,
    temporary_request,
    temporary_response,
)
...
def test_goal_binds_ipv6addr_actuation_plane() -> None:
    assert leftover_marker_ids(IPV6ADDR_ACTUATION_GOAL) == (IPV6ADDR_ACTUATION_ID,)
    assert leftover_marker_ids(IPV6ADDR_LEFTOVER) == (IPV6ADDR_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert IPV6ADDR_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(IPV6ADDR_ACTUATION_GOAL)
...
def test_opted_in_ipv6addr_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("ipv6addr",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, IPV6ADDR_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["ipv6addr"]

    missing = run_ipv6addr_workflow(with_ipv6addrid=False)
    skip_bind = run_ipv6addr_workflow(skip_bind=True)
    skip_temporary = run_ipv6addr_workflow(do_temporary=False)
    skip_public = run_ipv6addr_workflow(do_public=False)
    skip_ipv6addrdigest = run_ipv6addr_workflow(do_ipv6addrdigest=False)
    skip_replay = run_ipv6addr_workflow(replay=False)
    skip_ipv6addrid = run_ipv6addr_workflow(use_ipv6addrid=False)
    live = run_ipv6addr_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_ipv6addrid"
    ...
    assert skip_ipv6addrdigest["ok"] is False
    assert skip_ipv6addrdigest["error"] == "ipv6addrdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_ipv6addrid["ok"] is False
    assert skip_ipv6addrid["error"] == "ipv6addrid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_ipv6addrdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["ipv6addrdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["ipv6addrid_bound"] is True
    assert row["digest"]
    assert live["ipv6addrid"] == DEFAULT_IPV6ADDRID
    assert live["ipv6addrdigest"] == DEFAULT_IPV6ADDRDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, ipv6addrid=DEFAULT_IPV6ADDRID, ipv6addrdigest=DEFAULT_IPV6ADDRDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["ipv6addrid"] == DEFAULT_IPV6ADDRID
    assert queried["ipv6addrdigest"] == DEFAULT_IPV6ADDRDIGEST
    assert queried["type"] == FRAME_GLOBAL
    assert queried["first_byte"] == IPV6ADDR_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, ipv6addrid=DEFAULT_IPV6ADDRID, ipv6addrdigest=DEFAULT_IPV6ADDRDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["ipv6addrid"] == DEFAULT_IPV6ADDRID
    assert answered["ipv6addrdigest"] == DEFAULT_IPV6ADDRDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, ipv6addrid=DEFAULT_IPV6ADDRID, include_ipv6addrid=False)
    )
    assert bare["has_ipv6addrid"] is False
    assert bare["ipv6addrid"] == EMPTY_IPV6ADDRID
    publicised = serialize_ipv6addr(DEFAULT_GLOBAL)
    assert publicised == RFC_GLOBAL_FIELD
    assert parse_ipv6addr(publicised) == DEFAULT_GLOBAL
    assert parse_ipv6addr(RFC_UNICAST_FIELD) == UNICAST_POLICY
    header = parse_ipv6addr_header(encode_ipv6addr_header(DEFAULT_GLOBAL))
    assert header["field_value"] == RFC_GLOBAL_FIELD
    assert header["header"] == GLOBAL_HEADER
    asked = parse_http_request(temporary_request(SENTINEL, DEFAULT_IPV6ADDRID))
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_IPV6ADDRID, DEFAULT_IPV6ADDRDIGEST))
    ...
    assert asked["method"] == "GLOBAL"
    assert asked["ipv6addr_kind"] == "global"
    assert listed["ipv6addr_kind"] == "unicast"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_GLOBAL
    assert preload_public["policy"] == UNICAST_POLICY
    assert canonical_temporary(SENTINEL, DEFAULT_IPV6ADDRID).startswith("GLOBAL")
    assert "ipv6addrdigest=" in canonical_public(SENTINEL, DEFAULT_IPV6ADDRID, DEFAULT_IPV6ADDRDIGEST)


def test_builtin_proof_seals_ipv6addr_actuation() -> None:
    report = builtin_ipv6addr_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "ipv6addr_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_ipv6addr"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_ipv6addrid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_ipv6addrdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_ipv6addrid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_ipv6addrdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_ipv6addr"]
    ...
    assert report["checks"]["leftover_text_binds_ipv6addr"]
    assert report["checks"]["proved_ipv6addr_consumes_leftover"]
    assert report["mission_goal"] == IPV6ADDR_ACTUATION_GOAL
    assert report["done_when"] == IPV6ADDR_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[IPV6ADDR_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "ipv6addr" in capability.tags
    assert "rfc4291" in capability.tags
    assert "http" in capability.tags
    assert "ipv6addrid" in capability.tags
    assert "ipv6addrdigest" in capability.tags
    assert "global" in capability.tags
    assert "unicast" in capability.tags


def test_selection_gate_rejects_ledger_only_ipv6addr_family(tmp_path: Path) -> None:
    ...
    assert "ipv6addr" in family.split("/")
    assert "rfc4291" in family
    assert "ipv6addrid" in family
    assert "ipv6addrdigest" in family
