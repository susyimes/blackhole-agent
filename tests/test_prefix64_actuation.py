...
from blackhole_agent.prefix64_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_prefix64_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_prefix64_header,
    independent_prefix64digest,
    parse_http_request,
    parse_http_response,
    parse_message,
    parse_prefix64,
    parse_prefix64_header,
    public_request,
    public_response,
    run_prefix64_workflow,
    serialize_prefix64,
    temporary_request,
    temporary_response,
)
...
def test_goal_binds_prefix64_actuation_plane() -> None:
    assert leftover_marker_ids(PREFIX64_ACTUATION_GOAL) == (PREFIX64_ACTUATION_ID,)
    ...
    assert PREFIX64_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(PREFIX64_LEFTOVER) == (PREFIX64_ACTUATION_ID,)
    assert leftover_marker_ids(PREFIX64_ACTUATION_GOAL) == (PREFIX64_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert PREFIX64_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(PREFIX64_ACTUATION_GOAL)
...
def test_opted_in_prefix64_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("prefix64",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, PREFIX64_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["prefix64"]

    missing = run_prefix64_workflow(with_prefix64id=False)
    skip_bind = run_prefix64_workflow(skip_bind=True)
    skip_temporary = run_prefix64_workflow(do_temporary=False)
    skip_public = run_prefix64_workflow(do_public=False)
    skip_prefix64digest = run_prefix64_workflow(do_prefix64digest=False)
    skip_replay = run_prefix64_workflow(replay=False)
    skip_prefix64id = run_prefix64_workflow(use_prefix64id=False)
    live = run_prefix64_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_prefix64id"
    ...
    assert skip_prefix64digest["ok"] is False
    assert skip_prefix64digest["error"] == "prefix64digest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_prefix64id["ok"] is False
    assert skip_prefix64id["error"] == "prefix64id_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_prefix64digest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["prefix64digest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["prefix64id_bound"] is True
    assert row["digest"]
    assert live["prefix64id"] == DEFAULT_PREFIX64ID
    assert live["prefix64digest"] == DEFAULT_PREFIX64DIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, prefix64id=DEFAULT_PREFIX64ID, prefix64digest=DEFAULT_PREFIX64DIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["prefix64id"] == DEFAULT_PREFIX64ID
    assert queried["prefix64digest"] == DEFAULT_PREFIX64DIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == PREFIX64_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, prefix64id=DEFAULT_PREFIX64ID, prefix64digest=DEFAULT_PREFIX64DIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["prefix64id"] == DEFAULT_PREFIX64ID
    assert answered["prefix64digest"] == DEFAULT_PREFIX64DIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, prefix64id=DEFAULT_PREFIX64ID, include_prefix64id=False)
    )
    assert bare["has_prefix64id"] is False
    assert bare["prefix64id"] == EMPTY_PREFIX64ID
    publicised = serialize_prefix64(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_prefix64(publicised) == DEFAULT_SOURCE
    assert parse_prefix64(RFC_DEST_FIELD) == DEST_HOP
    header = parse_prefix64_header(encode_prefix64_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_request(public_request(SENTINEL, DEFAULT_PREFIX64ID, DEFAULT_PREFIX64DIGEST))
    ...
    assert listed["disc_kind"] == "embed"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_PREFIX64ID).startswith("PREFIX")
    assert "prefix64digest=" in canonical_public(SENTINEL, DEFAULT_PREFIX64ID, DEFAULT_PREFIX64DIGEST)


def test_builtin_proof_seals_prefix64_actuation() -> None:
    report = builtin_prefix64_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "prefix64_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_prefix64"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_prefix64id_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_prefix64digest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_prefix64id_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_prefix64digest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_prefix64"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_prefix64"]
    assert report["checks"]["proved_prefix64_consumes_leftover"]
    assert report["mission_goal"] == PREFIX64_ACTUATION_GOAL
    assert report["done_when"] == PREFIX64_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[PREFIX64_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "prefix64" in capability.tags
    assert "rfc8115" in capability.tags
    assert "http" in capability.tags
    assert "prefix64id" in capability.tags
    assert "prefix64digest" in capability.tags
    assert "prefix" in capability.tags
    assert "embed" in capability.tags


def test_selection_gate_rejects_ledger_only_prefix64_family(tmp_path: Path) -> None:
    ...
    assert "prefix64" in family.split("/")
    assert "rfc8115" in family
    assert "prefix64id" in family
    assert "prefix64digest" in family
