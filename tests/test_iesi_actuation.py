...
def test_goal_binds_iesi_actuation_plane() -> None:
    assert leftover_marker_ids(IESI_ACTUATION_GOAL) == (IESI_ACTUATION_ID,)
    ...
    assert IESI_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(IESI_LEFTOVER) == (IESI_ACTUATION_ID,)
    assert leftover_marker_ids(IESI_ACTUATION_GOAL) == (IESI_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert IESI_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(IESI_ACTUATION_GOAL)
...
def test_opted_in_iesi_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("iesi",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, IESI_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["iesi"]

    missing = run_iesi_workflow(with_iesiid=False)
    skip_bind = run_iesi_workflow(skip_bind=True)
    skip_temporary = run_iesi_workflow(do_temporary=False)
    skip_public = run_iesi_workflow(do_public=False)
    skip_iesidigest = run_iesi_workflow(do_iesidigest=False)
    skip_replay = run_iesi_workflow(replay=False)
    skip_iesiid = run_iesi_workflow(use_iesiid=False)
    live = run_iesi_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_iesiid"
    ...
    assert skip_iesidigest["ok"] is False
    assert skip_iesidigest["error"] == "iesidigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_iesiid["ok"] is False
    assert skip_iesiid["error"] == "iesiid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_iesidigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["iesidigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["iesiid_bound"] is True
    assert row["digest"]
    assert live["iesiid"] == DEFAULT_IESIID
    assert live["iesidigest"] == DEFAULT_IESIDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, iesiid=DEFAULT_IESIID, iesidigest=DEFAULT_IESIDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["iesiid"] == DEFAULT_IESIID
    assert queried["iesidigest"] == DEFAULT_IESIDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == IESI_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, iesiid=DEFAULT_IESIID, iesidigest=DEFAULT_IESIDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["iesiid"] == DEFAULT_IESIID
    assert answered["iesidigest"] == DEFAULT_IESIDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, iesiid=DEFAULT_IESIID, include_iesiid=False)
    )
    assert bare["has_iesiid"] is False
    assert bare["iesiid"] == EMPTY_IESIID
    publicised = serialize_iesi(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_iesi(publicised) == DEFAULT_SOURCE
    assert parse_iesi(RFC_DEST_FIELD) == DEST_HOP
    header = parse_iesi_header(encode_iesi_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_IESIID, DEFAULT_IESIDIGEST))
    ...
    assert asked["method"] == "IESI"
    assert asked["disc_kind"] == "iesi"
    assert listed["disc_kind"] == "gw"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_IESIID).startswith("IESI")
    assert "iesidigest=" in canonical_public(SENTINEL, DEFAULT_IESIID, DEFAULT_IESIDIGEST)


def test_builtin_proof_seals_iesi_actuation() -> None:
    report = builtin_iesi_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "iesi_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_iesi"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_iesiid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_iesidigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_iesiid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_iesidigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_iesi"]
    ...
    assert report["checks"]["leftover_text_binds_iesi"]
    assert report["checks"]["proved_iesi_consumes_leftover"]
    assert report["mission_goal"] == IESI_ACTUATION_GOAL
    assert report["done_when"] == IESI_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[IESI_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "iesi" in capability.tags
    assert "rfc9014" in capability.tags
    assert "http" in capability.tags
    assert "iesiid" in capability.tags
    assert "iesidigest" in capability.tags
    assert "iesi" in capability.tags
    assert "gw" in capability.tags


def test_selection_gate_rejects_ledger_only_iesi_family(tmp_path: Path) -> None:
    ...
    assert "iesi" in family.split("/")
    assert "rfc9014" in set(semantic_tokens(IESI_ACTUATION_GOAL))
    assert "iesiid" in set(semantic_tokens(IESI_ACTUATION_GOAL))
    assert "iesidigest" in family
