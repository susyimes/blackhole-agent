...
from blackhole_agent.github_growth import (
    ...
    build_skill_route_discovery_config_gate_boundary,
    build_skill_route_discovery_local_apply,
...
def _skill_route_discovery_pipeline_fixture_proposals() -> list[dict]:
...
def test_skill_route_discovery_capability_pipeline_classifies_reverse_flow_and_rnskill():
    ...
    fortress_url = "https://github.com/tiliondev/fortress"
    chief_url = "https://github.com/SmileLikeYe/agent-chief"
    proposals = _skill_route_discovery_pipeline_fixture_proposals()

    # Defer unlock to show the pass-1 ready_for_local_comparison gate surface.
...
def test_skill_route_discovery_capability_pipeline_pass2_reverse_flow_local_comparison():
    """Pass 2: reverse-flow test lane unlocks only after pipeline-stage local comparison."""

    reverse_url = "https://github.com/lingbol088-spec/reverse-flow-skill"
    rnskill_url = "https://github.com/Pluviobyte/rnskill"
    proposals = [
        {
            "proposal_id": "prop-skill-pipeline-reverse-flow-test",
            "kind": "test",
            "summary": (
                "Lock reverse-flow-skill codex_workflow_gate and skill_route_discovery_first "
                "into a bounded local test validation lane before any apply unlock."
            ),
            "evidence_urls": [reverse_url],
            "risk_flags": [],
            "implementation_scope": "local_validation_candidate",
            "validation_gate": "focused-evidence-review",
            "validation_task": (
                "Locally compare reverse-flow skill-workflow probe outputs to "
                "classifier/route_profiles/bounded_local_apply_lanes stages."
            ),
            "requires_approval": False,
        },
        {
            "proposal_id": "prop-skill-pipeline-rnskill-docs",
            "kind": "documentation",
            "summary": (
                "Fold rnskill generic_skill_workflow into the shared skill_route_discovery "
                "capability pipeline via bounded documentation."
            ),
            "evidence_urls": [rnskill_url],
            "risk_flags": [],
            "implementation_scope": "local_validation_candidate",
            "validation_gate": "focused-evidence-review",
            "validation_task": "Document rnskill generic_skill_workflow documentation lane.",
            "requires_approval": False,
        },
        {
            "proposal_id": "prop-skill-pipeline-config-gates",
            "kind": "config",
            "summary": (
                "Validate config wiring for skill_route_discovery route_profiles "
                "codex_workflow_gate vs generic_skill_workflow without reverse-flow markers."
            ),
            "evidence_urls": [rnskill_url],
            "risk_flags": [],
            "implementation_scope": "local_validation_candidate",
            "validation_gate": "focused-evidence-review",
            "validation_task": "Keep general_agent_project items out of skill apply unlocks.",
            "requires_approval": False,
        },
        {
            "proposal_id": "prop-fortress-agent-harness-eval-local-candidate",
            "kind": "test",
            "summary": "tiliondev/fortress general_agent_project harness-eval candidate.",
            "evidence_urls": ["https://github.com/tiliondev/fortress"],
            "risk_flags": [],
            "implementation_scope": "local_validation_candidate",
            "validation_gate": "focused-evidence-review",
            "validation_task": "Keep fortress behind agent_harness_eval_required.",
        ...
        },
        {
            ...
            "evidence_urls": ["https://github.com/SmileLikeYe/agent-chief"],
            "risk_flags": ["privacy-leakage"],
            "implementation_scope": "reviewable_proposal_only",
            "validation_gate": "privacy-leakage-human-review",
            "validation_task": "Do not select agent-chief for local apply.",
        ...
        },
        {
            ...
            "kind": "test",
            "summary": "Retained agent harness eval cluster boundary.",
            "evidence_urls": [],
            "risk_flags": [],
            "implementation_scope": "reviewable_proposal_only",
            "validation_gate": "focused-evidence-review",
            "validation_task": "Keep agent harness eval boundary review-only for this pass.",
            "requires_approval": False,
        },
    ]
    ...
    assert selected["proposal_id"] == "prop-skill-pipeline-reverse-flow-test"
    assert selected["route_class"] == "skill_route_discovery"
    assert "codex_workflow_gate" in selected["route_profiles"]
    assert selected["skill_route_discovery_first"] is True
    assert selected["selected_local_lane"] == "test"
    assert set(selected["allowed_local_lanes"]).issubset(
        {"documentation", "config", "test", "code_patch"}
    )
    assert selected["allowed_local_lanes"] == [
        "documentation",
        "config",
    ...
    assert pipeline["runtime_action"] == "none"
    assert pipeline["bounded_local_apply"]["runtime_action"] == "none"
    assert pipeline["bounded_local_apply"]["external_skill_execution_allowed"] is False
    assert pipeline["bounded_local_apply"]["provider_launch_allowed"] is False
    assert pipeline["bounded_local_apply"]["remote_apply_allowed"] is False
    assert "external_skill_execution" in pipeline["denied_actions"]
    assert "provider_launch" in pipeline["denied_actions"]
    assert "remote_apply" in pipeline["denied_actions"]

    assert comparison["controller_surface"] == "skill_route_discovery_local_comparison"
    assert comparison["status"] == "passed"
    assert comparison["local_comparison_passed"] is True
    assert comparison["failed_criteria"] == []
    assert comparison["runtime_action"] == "none"
    assert comparison["external_skill_execution_allowed"] is False
    assert comparison["provider_launch_allowed"] is False
    assert comparison["remote_apply_allowed"] is False
    criterion_ids = {row["criterion_id"] for row in comparison["criteria_results"]}
    assert "skill_route_discovery_first_for_codex_workflow_gate" in criterion_ids
    assert "allowed_lanes_subset_of_bounded_local_apply" in criterion_ids
    assert "runtime_action_is_none" in criterion_ids
    assert all(row["passed"] is True for row in comparison["criteria_results"])

    assert reverse_lane["controller_surface"] == (
        "skill_route_discovery_reverse_flow_test_validation_lane"
    )
    assert reverse_lane["proposal_track"] == "prop-skill-pipeline-reverse-flow-test"
    assert reverse_lane["status"] == "ready"
    assert reverse_lane["selected_local_lane"] == "test"
    assert reverse_lane["skill_route_discovery_first"] is True
    assert reverse_lane["unlocked_local_lanes"] == ["test"]
    assert reverse_lane["runtime_action"] == "none"
    assert reverse_lane["external_skill_execution_allowed"] is False
    assert reverse_lane["companion_rnskill_documentation_profiles"]
    assert (
        reverse_lane["companion_rnskill_documentation_profiles"][0]["preferred_local_lane"]
        == "documentation"
    )

    # Deferred apply keeps the gate visible without unlocking lanes.
    assert deferred["status"] == "ready_for_local_comparison"
    assert deferred["selected_step"]["unlocked_local_lanes"] == []
    assert deferred["selected_step"]["local_comparison_status"] == "required_before_unlock"
    assert deferred["local_comparison"]["local_comparison_passed"] is True
    assert deferred["reverse_flow_test_validation_lane"]["status"] == (
        "comparison_passed_awaiting_lane_unlock"
    )
    assert deferred["reverse_flow_test_validation_lane"]["unlocked_local_lanes"] == []

    # Boundary isolation: fortress and agent-chief never inherit skill unlocks.
    rows_by_id = {row["proposal_id"]: row for row in pipeline["classifier"]["candidate_rows"]}
    assert rows_by_id["prop-fortress-agent-harness-eval-local-candidate"]["route_class"] == (
        "agent_harness_eval_required"
    )
    ...
    assert all(
        ...
        for row in pipeline["adjacent_general_agent_rows"]
    )
    assert reverse_url not in json.dumps(pipeline)
    assert rnskill_url not in json.dumps(pipeline)

    # Direct evaluator contract for a reverse-flow probe row.
    probe = evaluate_skill_route_discovery_local_comparison(
        rows_by_id["prop-skill-pipeline-reverse-flow-test"],
        adjacent_general_agent_rows=pipeline["adjacent_general_agent_rows"],
        retained_boundaries=pipeline["retained_boundaries"],
    )
    assert probe["local_comparison_passed"] is True
    assert probe["unlocked_local_lanes"] == ["test"]
    lane = build_skill_route_discovery_reverse_flow_test_validation_lane(
        selected_step=selected,
        local_comparison=comparison,
        route_profile_rows=pipeline["route_profiles"]["rows"],
        theme_window={"theme_id": "skill-route-discovery", "planned_passes": 2, "target_passes": 4},
    )
    assert lane["status"] == "ready"


def test_skill_route_discovery_capability_pipeline_pass3_local_apply_handoff():
    """Pass 3: reverse-flow local apply with rnskill docs companion and config gates."""
    ...
    reverse_lane = pipeline["reverse_flow_test_validation_lane"]
    rnskill_lane = pipeline["rnskill_docs_validation_lane"]
    ...
    assert rnskill_lane["controller_surface"] == (
        "skill_route_discovery_rnskill_docs_validation_lane"
    )
    assert rnskill_lane["proposal_track"] == "prop-skill-pipeline-rnskill-docs"
    assert rnskill_lane["status"] == "ready"
    assert rnskill_lane["preferred_local_lane"] == "documentation"
    assert rnskill_lane["unlocked_local_lanes"] == ["documentation"]
    assert rnskill_lane["body_free"] is True
    assert rnskill_lane["runtime_action"] == "none"
    assert rnskill_lane["external_skill_execution_allowed"] is False
    assert rnskill_lane["companion_count"] >= 1
    assert rnskill_lane["route_profiles"] == ["generic_skill_workflow"]

    assert config_gates["controller_surface"] == "skill_route_discovery_config_gate_boundary"
    assert config_gates["proposal_track"] == "prop-skill-pipeline-config-gates"
    ...
    assert config_gates["general_agent_isolation_passed"] is True
    assert config_gates["privacy_isolation_passed"] is True
    assert config_gates["route_profiles_separated"] is True
    assert config_gates["adjacent_general_agent_count"] >= 1
    assert config_gates["retained_boundary_count"] >= 1
    assert config_gates["skill_unlocked_lanes"] == ["test"]
    assert config_gates["runtime_action"] == "none"

    assert local_apply["controller_surface"] == "skill_route_discovery_local_apply"
    assert local_apply["proposal_track"] == "prop-skill-pipeline-reverse-flow-test"
    assert "prop-skill-pipeline-rnskill-docs" in local_apply["companion_tracks"]
    assert "prop-skill-pipeline-config-gates" in local_apply["companion_tracks"]
    assert local_apply["status"] == "ready"
    assert local_apply["decision"] == (
        "apply_one_local_skill_route_validation_candidate_after_lane_unlock"
    )
    assert local_apply["capability_action"] == "apply_one_local_skill_route_validation_candidate"
    assert local_apply["selected_local_lane"] == "test"
    assert local_apply["unlocked_local_lanes"] == ["test"]
    assert local_apply["local_comparison_passed"] is True
    assert local_apply["reverse_flow_test_validation_lane_status"] == "ready"
    assert local_apply["rnskill_docs_validation_lane_status"] == "ready"
    assert local_apply["config_gate_boundary_status"] == "ready"
    assert local_apply["rnskill_docs_companion"]["body_free"] is True
    assert local_apply["rnskill_docs_companion"]["preferred_local_lane"] == "documentation"
    assert local_apply["config_gates"]["general_agent_inherits_skill_unlock"] is False
    assert local_apply["config_gates"]["privacy_rows_selectable_for_local_apply"] is False
    assert local_apply["runtime_action"] == "none"
    assert local_apply["external_skill_execution_allowed"] is False
    assert local_apply["provider_launch_allowed"] is False
    assert local_apply["remote_apply_allowed"] is False
    assert local_apply["push_or_promotion_allowed"] is False
    assert local_apply["kernel_restart_allowed"] is False
    assert local_apply["supervisor_next_action"] == (
        "replay_skill_route_discovery_local_apply_then_continue_to_pass4"
    )

    # Deferred comparison keeps reverse-flow unlock and local apply gated.
    assert deferred["status"] == "ready_for_local_comparison"
    assert deferred["reverse_flow_test_validation_lane"]["status"] == (
        "comparison_passed_awaiting_lane_unlock"
    )
    assert deferred["rnskill_docs_validation_lane"]["status"] == (
        "comparison_passed_awaiting_reverse_flow_lane"
    )
    assert deferred["local_apply"]["status"] == "blocked_until_reverse_flow_lane_ready"
    assert deferred["local_apply"]["unlocked_local_lanes"] == []

    # Direct builder contracts stay body-free and deny remote/runtime paths.
    docs_lane = build_skill_route_discovery_rnskill_docs_validation_lane(
        route_profile_rows=pipeline["route_profiles"]["rows"],
        local_comparison=pipeline["local_comparison"],
        reverse_flow_test_validation_lane=reverse_lane,
        theme_window=theme,
    )
    assert docs_lane["status"] == "ready"
    gates = build_skill_route_discovery_config_gate_boundary(
        selected_step=pipeline["selected_step"],
        route_profile_rows=pipeline["route_profiles"]["rows"],
        adjacent_general_agent_rows=pipeline["adjacent_general_agent_rows"],
        retained_boundaries=pipeline["retained_boundaries"],
        local_comparison=pipeline["local_comparison"],
    ...
    apply_packet = build_skill_route_discovery_local_apply(
        selected_step=pipeline["selected_step"],
        local_comparison=pipeline["local_comparison"],
        reverse_flow_test_validation_lane=reverse_lane,
        rnskill_docs_validation_lane=docs_lane,
        config_gate_boundary=gates,
        theme_window=theme,
    )
...
def test_skill_route_discovery_capability_pipeline_pass4_local_apply_completion():
    """Pass 4: complete reverse-flow skill-route pipeline after local apply unlock."""
    ...
    proposals = [
        {
            ...
            "kind": "test",
            "summary": (
                "Complete pass-4 reverse-flow skill_route_discovery local test "
                "validation lane via criteria-driven comparison against "
            ...
            "validation_gate": "focused-evidence-review",
            "validation_task": (
                "Run skill_route_discovery_local_comparison for reverse-flow-skill only; "
                "unlock skill_route_discovery_reverse_flow_test_validation_lane when probe "
                "rows match classifier, route_profiles=[codex_workflow_gate], "
                "evaluation_lane=skill_route_discovery_first, allowed_lanes include test, "
                "and runtime_action remains none."
            ),
            "requires_approval": False,
        },
        {
    ...
    assert reverse_lane["unlocked_local_lanes"] == ["test"]
    assert reverse_lane["skill_route_discovery_first"] is True
    assert "codex_workflow_gate" in reverse_lane["route_profiles"]
    ...
    assert completion["controller_surface"] == "skill_route_discovery_local_apply_completion"
    assert completion["proposal_track"] == "prop-skill-pipeline-reverse-flow-test"
    assert "prop-skill-pipeline-rnskill-docs" in completion["companion_tracks"]
    assert "prop-skill-pipeline-config-gates" in completion["companion_tracks"]
    ...
    assert completion["capability_action"] == (
        "apply_one_local_skill_route_validation_candidate"
    )
    assert completion["selected_local_lane"] == "test"
    assert completion["unlocked_local_lanes"] == ["test"]
    assert completion["local_apply_status"] == "ready"
    assert completion["local_comparison_passed"] is True
    assert completion["reverse_flow_test_validation_lane_status"] == "ready"
    assert completion["rnskill_docs_validation_lane_status"] == "ready"
    assert completion["config_gate_boundary_status"] == "ready"
    assert completion["skill_route_discovery_first"] is True
    assert "codex_workflow_gate" in completion["route_profiles"]
    assert completion["general_agent_inherits_skill_unlock"] is False
    assert completion["privacy_rows_selectable_for_local_apply"] is False
    assert completion["general_agent_isolation_passed"] is True
    assert completion["privacy_isolation_passed"] is True
    assert completion["runtime_action"] == "none"
    assert completion["external_skill_execution_allowed"] is False
    assert completion["provider_launch_allowed"] is False
    assert completion["remote_apply_allowed"] is False
    ...
    assert completion["body_free"] is True
    ...
    assert "skill_route_discovery_local_apply_completion" in completion["capability_pipeline"]
    assert "skill_route_discovery_local_apply" in completion["capability_pipeline"]
    assert "skill_route_discovery_local_comparison" in completion["capability_pipeline"]
    assert completion["retained_boundary_proposal_ids"]
    assert "prop-agent-chief-privacy-review-only" in completion["retained_boundary_proposal_ids"]
    assert completion["adjacent_general_agent_proposal_ids"]
    assert (
        "prop-fortress-agent-harness-eval-local-candidate"
        in completion["adjacent_general_agent_proposal_ids"]
    )

    # Final pass closes the operator-visible pipeline status.
    assert pipeline["status"] == "complete"
    assert pipeline["theme_pass"]["status"] == "complete"
    assert pipeline["runtime_action"] == "none"

    # Deferred comparison keeps theme completion blocked.
    assert deferred["status"] == "ready_for_local_comparison"
    assert deferred["local_apply"]["status"] == "blocked_until_reverse_flow_lane_ready"
    assert deferred["local_apply_completion"]["status"] == "blocked"
    assert deferred["local_apply_completion"]["theme_complete"] is False
    assert deferred["local_apply_completion"]["unlocked_local_lanes"] == []
    assert deferred["local_apply_completion"]["supervisor_next_action"] == (
        "repair_skill_route_discovery_local_apply_before_theme_completion"
    )

    # Direct builder contract: completion only after reverse-flow apply unlocks.
    direct = build_skill_route_discovery_local_apply_completion(
        local_apply=local_apply,
        reverse_flow_test_validation_lane=reverse_lane,
        rnskill_docs_validation_lane=rnskill_lane,
        config_gate_boundary=config_gates,
        local_comparison=pipeline["local_comparison"],
        retained_boundaries=pipeline["retained_boundaries"],
        adjacent_general_agent_rows=pipeline["adjacent_general_agent_rows"],
        theme_window=theme,
        source_digest="github-growth-20260712T195308.158137Z",
    )
    assert direct["status"] == "complete"
    assert direct["source_digest"] == "github-growth-20260712T195308.158137Z"
    assert direct["theme_complete"] is True

    # rnskill companion may be not_applicable and still complete the theme.
    rnskill_na = dict(rnskill_lane)
    rnskill_na["status"] = "not_applicable"
    rnskill_na["unlocked_local_lanes"] = []
    complete_with_na = build_skill_route_discovery_local_apply_completion(
        local_apply=local_apply,
        reverse_flow_test_validation_lane=reverse_lane,
        rnskill_docs_validation_lane=rnskill_na,
        config_gate_boundary=config_gates,
        local_comparison=pipeline["local_comparison"],
        retained_boundaries=pipeline["retained_boundaries"],
        adjacent_general_agent_rows=pipeline["adjacent_general_agent_rows"],
...
def test_skill_route_discovery_focused_local_test_validation_after_unlocked_apply():
    ...
    assert pipeline["reverse_flow_focused_validation_recorded_command_hashes"] == []
    assert pipeline["reverse_flow_focused_validation_recorded_command_hash_count"] == 0
    ...
    assert pipeline["reverse_flow_focused_validation_pending_work_unit_count"] == len(
        focused["focused_validation"]["commands"]
    )
    assert pipeline["reverse_flow_focused_validation_pending_work_units"] == [
        {
            "command": command,
            "command_hash": command_hash,
            "inventory_index": index,
        }
        for index, (command, command_hash) in enumerate(
            zip(
                focused["focused_validation"]["commands"],
                focused["focused_validation"]["command_hashes"],
            )
    ...
    assert pipeline[
        "reverse_flow_focused_validation_continue_finish_receipt_helper"
    ] == "package_reverse_flow_focused_validation_continue_finish_receipt"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_finish_receipt_helper"
    ] == "package_reverse_flow_focused_validation_continue_finish_receipt"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_finished"
    ] is False
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_queue_ready"
    ] is False
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_finish_line"
    ].startswith("finish complete=false ")
    ...
    assert pipeline[
        "reverse_flow_focused_validation_continue_residual_entry_helper"
    ] == "package_reverse_flow_focused_validation_continue_residual_entry"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_entry_helper"
    ] == "package_reverse_flow_focused_validation_continue_residual_entry"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_entry_ready"
    ] is False
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_selected_residual_proposal_id"
    ] == ""
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_entry_line"
    ].startswith("residual_entry ready=false ")
    assert "selected=none" in pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_entry_line"
    ]
    assert "residual_export=false" in pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_entry_line"
    ...
    assert pipeline[
        "reverse_flow_focused_validation_continue_residual_follow_helper"
    ] == "package_reverse_flow_focused_validation_continue_residual_follow"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_follow_helper"
    ] == "package_reverse_flow_focused_validation_continue_residual_follow"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_follow_ready"
    ] is False
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_follow_action"
    ] == "wait_for_reverse_flow"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_call_residual_comparison"
    ] is False
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_follow_line"
    ].startswith("residual_follow ready=false ")
    assert "action=wait_for_reverse_flow" in pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_follow_line"
    ]
    assert "call_comparison=false" in pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_follow_line"
    ]
    assert "residual_export=false" in pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_follow_line"
    ...
    assert pipeline[
        "reverse_flow_focused_validation_continue_residual_unlocked_apply_helper"
    ] == "package_reverse_flow_focused_validation_continue_residual_unlocked_apply"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_unlocked_apply_helper"
    ] == "package_reverse_flow_focused_validation_continue_residual_unlocked_apply"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_unlocked_apply_ready"
    ] is False
    assert pipeline["operator_state"][
    ...
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_unlocked_apply_line"
    ].startswith("residual_unlocked_apply ready=false ")
    assert "action=wait_for_reverse_flow" in pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_unlocked_apply_line"
    ]
    assert "call_focused_validation=false" in pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_unlocked_apply_line"
    ]
    assert "residual_export=false" in pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_unlocked_apply_line"
    ...
    assert pipeline[
        "reverse_flow_focused_validation_continue_residual_focused_validation_helper"
    ] == "package_reverse_flow_focused_validation_continue_residual_focused_validation"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_focused_validation_helper"
    ] == "package_reverse_flow_focused_validation_continue_residual_focused_validation"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_focused_validation_ready"
    ] is False
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_focused_validation_action"
    ] == "wait_for_reverse_flow"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_call_residual_handoff"
    ] is False
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_focused_validation_line"
    ].startswith("residual_focused_validation ready=false ")
    assert "action=wait_for_reverse_flow" in pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_focused_validation_line"
    ]
    assert "call_handoff=false" in pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_focused_validation_line"
    ]
    assert "residual_export=false" in pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_focused_validation_line"
    ...
    assert pipeline[
        "reverse_flow_focused_validation_continue_residual_handoff_helper"
    ] == "package_reverse_flow_focused_validation_continue_residual_handoff"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_handoff_helper"
    ] == "package_reverse_flow_focused_validation_continue_residual_handoff"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_handoff_ready"
    ] is False
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_handoff_action"
    ] == "wait_for_reverse_flow"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_call_residual_acceptance"
    ] is False
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_handoff_line"
    ].startswith("residual_handoff ready=false ")
    assert "action=wait_for_reverse_flow" in pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_residual_handoff_line"
    ]
    ...
    assert pipeline[
        "reverse_flow_focused_validation_continue_cascade_helper"
    ] == "package_reverse_flow_focused_validation_continue_cascade"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_helper"
    ] == "package_reverse_flow_focused_validation_continue_cascade"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_ready"
    ] is False
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_action"
    ] == "execute_now"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_reverse_progress_label"
    ] == "0/3"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_residual_progress_label"
    ] == "0/8"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_residual_blocked_at"
    ] == "open"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_line"
    ].startswith("continue_cascade ready=false ")
    assert "reverse_progress=0/3" in pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_line"
    ]
    assert "residual_progress=0/8" in pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_line"
    ]
    assert "residual_blocked_at=open" in pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_line"
    ]
    assert "action=execute_now" in pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_line"
    ]
    assert "residual_export=false" in pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_line"
    ...
    # Body-free continue cascade wake route maps wake_outcome → route_action
    # without residual_export so supervisors pin one next-helper policy.
    assert pipeline[
        "reverse_flow_focused_validation_continue_cascade_wake_route_helper"
    ] == "package_reverse_flow_focused_validation_continue_cascade_wake_route"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_helper"
    ] == "package_reverse_flow_focused_validation_continue_cascade_wake_route"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_action"
    ] in {"execute_now", "inventory_only"}
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_line"
    ].startswith("continue_cascade_wake_route outcome=")
    assert "residual_export=false" in pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_line"
    ]
    assert "route_helper=package_reverse_flow_focused_validation_continue_cascade_wake_route" in (
        pipeline["operator_state"][
            "reverse_flow_focused_validation_continue_cascade_wake_route_line"
        ]
    )
    execute_route = package_reverse_flow_focused_validation_continue_cascade_wake_route(
        cascade_wake={
            ...
            "reverse_progress_transition": "0/3→0/3",
            "residual_progress_transition": "0/8→0/8",
            "residual_open_ready": False,
        }
    )
    assert execute_route["controller_surface"] == (
        "reverse_flow_focused_validation_continue_cascade_wake_route"
    )
    assert execute_route["route_action"] == "execute_now"
    assert execute_route["call_follow_with_execute"] is True
    assert execute_route["residual_route_ready"] is False
    assert execute_route["residual_export_allowed"] is False
    assert execute_route["preferred_helper"] == (
        "follow_reverse_flow_focused_validation_continue_dispatch"
    )
    assert execute_route["continue_cascade_wake_route_line"].startswith(
        "continue_cascade_wake_route outcome=execute_recommended "
    )
    assert "action=execute_now" in execute_route["continue_cascade_wake_route_line"]
    assert "call_execute=true" in execute_route["continue_cascade_wake_route_line"]
    assert "residual_export=false" in execute_route["continue_cascade_wake_route_line"]
    finished_route = package_reverse_flow_focused_validation_continue_cascade_wake_route(
        cascade_wake=cascade_wake,
    )
    assert finished_route["route_action"] == "keep_activation_external"
    assert finished_route["call_follow_with_execute"] is False
    assert finished_route["preferred_helper"] == (
        "package_reverse_flow_focused_validation_continue_finish_receipt"
    )
    residual_open_route = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route(
            cascade_wake=residual_open_wake,
        )
    )
    assert residual_open_route["route_action"] == "open_residual_entry"
    assert residual_open_route["residual_route_ready"] is True
    assert residual_open_route["call_follow_with_execute"] is False
    assert residual_open_route["residual_export_allowed"] is False
    assert residual_open_route["preferred_helper"] == (
        "package_reverse_flow_focused_validation_continue_residual_entry"
    )
    assert "action=open_residual_entry" in residual_open_route[
    ...
    # Body-free continue cascade wake route apply follow maps applied route
    # action → preferred helper so supervisors pin one follow receipt.
    assert pipeline[
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_helper"
    ] == "package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_helper"
    ] == "package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_line"
    ].startswith("continue_cascade_wake_route_apply_follow applied=")
    assert "residual_export=false" in pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_line"
    ]
    assert "apply_helper=package_reverse_flow_focused_validation_continue_cascade_wake_route_apply" in (
        pipeline["operator_state"][
            "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_line"
        ]
    )
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_action"
    ] in {"execute_now", "inventory_only", "record_remaining", "keep_activation_external"}
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_call_execute"
    ] is True or pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_action"
    ] != "execute_now"
    identity_follow = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow(
            route_apply=identity_apply,
            post_route=execute_route,
        )
    )
    assert identity_follow["controller_surface"] == (
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow"
    )
    assert identity_follow["applied_route_action"] == "execute_now"
    assert identity_follow["follow_action"] == "execute_now"
    assert identity_follow["call_follow_with_execute"] is True
    assert identity_follow["preferred_helper"] == (
        "follow_reverse_flow_focused_validation_continue_dispatch"
    )
    assert identity_follow["residual_export_allowed"] is False
    assert identity_follow["continue_cascade_wake_route_apply_follow_line"].startswith(
        "continue_cascade_wake_route_apply_follow applied=execute_now "
    )
    assert "call_execute=true" in identity_follow[
        "continue_cascade_wake_route_apply_follow_line"
    ]
    assert "residual_export=false" in identity_follow[
        "continue_cascade_wake_route_apply_follow_line"
    ]
    advanced_follow = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow(
            route_apply=advanced_apply,
            post_route=finished_route,
        )
    )
    assert advanced_follow["applied_route_action"] == "keep_activation_external"
    assert advanced_follow["follow_action"] == "keep_activation_external"
    assert advanced_follow["route_advanced"] is True
    assert advanced_follow["executed"] is True
    assert advanced_follow["call_follow_with_execute"] is False
    assert advanced_follow["preferred_helper"] == (
        "package_reverse_flow_focused_validation_continue_finish_receipt"
    )
    assert advanced_follow["residual_export_allowed"] is False
    assert "applied=keep_activation_external" in advanced_follow[
        "continue_cascade_wake_route_apply_follow_line"
    ]
    assert "advanced=true" in advanced_follow[
        "continue_cascade_wake_route_apply_follow_line"
    ]
    assert "executed=true" in advanced_follow[
        "continue_cascade_wake_route_apply_follow_line"
    ]
    residual_follow = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow(
            route_apply=residual_apply,
            post_route=residual_open_route,
        )
    )
    assert residual_follow["applied_route_action"] == "open_residual_entry"
    assert residual_follow["follow_action"] == "open_residual_entry"
    assert residual_follow["residual_route_ready"] is True
    assert residual_follow["call_follow_with_execute"] is False
    assert residual_follow["preferred_helper"] == (
        "package_reverse_flow_focused_validation_continue_residual_entry"
    )
    assert residual_follow["residual_export_allowed"] is False
    assert "applied=open_residual_entry" in residual_follow[
        "continue_cascade_wake_route_apply_follow_line"
    ]
    ...
    # Body-free continue cascade wake route apply follow pin call collapses
    # pre/post pin recipes into one call receipt (action/mode transitions).
    assert pipeline[
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_helper"
    ] == (
        "package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call"
    )
    assert pipeline["operator_state"][
    ...
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_line"
    ].startswith("continue_cascade_wake_route_apply_follow_pin_call pre_action=")
    assert "residual_export=false" in pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_line"
    ]
    assert "pin_helper=package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin" in (
        pipeline["operator_state"][
            "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_line"
        ]
    )
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_action_transition"
    ].count("→") == 1
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_mode_transition"
    ].count("→") == 1
    identity_pin_call = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call(
            pre_pin=identity_pin,
            post_pin=identity_pin,
            executed=False,
            recorded=False,
        )
    )
    assert identity_pin_call["controller_surface"] == (
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call"
    )
    assert identity_pin_call["pin_action_transition"] == "execute_now→execute_now"
    assert identity_pin_call["pin_mode_transition"] == "execute_helper→execute_helper"
    assert identity_pin_call["pin_advanced"] is False
    assert identity_pin_call["executed"] is False
    assert identity_pin_call["pre_call_pin_with_execute"] is True
    assert identity_pin_call["post_call_pin_with_execute"] is True
    assert identity_pin_call["preferred_helper"] == (
        "follow_reverse_flow_focused_validation_continue_dispatch"
    )
    assert identity_pin_call["residual_export_allowed"] is False
    assert identity_pin_call["runtime_action"] == "none"
    assert identity_pin_call[
    ...
    assert "mode=execute_helper→execute_helper" in identity_pin_call[
        "continue_cascade_wake_route_apply_follow_pin_call_line"
    ]
    assert "pin_advanced=false" in identity_pin_call[
        "continue_cascade_wake_route_apply_follow_pin_call_line"
    ]
    assert "residual_export=false" in identity_pin_call[
        "continue_cascade_wake_route_apply_follow_pin_call_line"
    ]
    advanced_pin_call = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call(
            pre_pin=identity_pin,
            post_pin=advanced_pin,
            executed=True,
            recorded=True,
        )
    )
    assert advanced_pin_call["pin_action_transition"] == (
        "execute_now→keep_activation_external"
    )
    assert advanced_pin_call["pin_mode_transition"] == (
        "execute_helper→package_helper"
    )
    assert advanced_pin_call["pin_advanced"] is True
    assert advanced_pin_call["executed"] is True
    assert advanced_pin_call["recorded"] is True
    assert advanced_pin_call["pre_call_pin_with_execute"] is True
    assert advanced_pin_call["post_call_pin_with_execute"] is False
    assert advanced_pin_call["applied_pin_action"] == "keep_activation_external"
    assert advanced_pin_call["applied_pin_mode"] == "package_helper"
    assert advanced_pin_call["residual_export_allowed"] is False
    assert "action=execute_now→keep_activation_external" in advanced_pin_call[
        "continue_cascade_wake_route_apply_follow_pin_call_line"
    ]
    assert "mode=execute_helper→package_helper" in advanced_pin_call[
        "continue_cascade_wake_route_apply_follow_pin_call_line"
    ]
    assert "pin_advanced=true" in advanced_pin_call[
        "continue_cascade_wake_route_apply_follow_pin_call_line"
    ]
    assert "call_execute=true→false" in advanced_pin_call[
        "continue_cascade_wake_route_apply_follow_pin_call_line"
    ]
    residual_pin_call = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call(
            pre_pin=advanced_pin,
            post_pin=residual_pin,
            executed=True,
            recorded=True,
        )
    )
    assert residual_pin_call["pin_action_transition"] == (
        "keep_activation_external→open_residual_entry"
    )
    assert residual_pin_call["pin_mode_transition"] == "package_helper→package_helper"
    assert residual_pin_call["pin_advanced"] is True
    assert residual_pin_call["post_residual_route_ready"] is True
    assert residual_pin_call["post_call_pin_with_execute"] is False
    assert residual_pin_call["residual_export_allowed"] is False
    assert "action=keep_activation_external→open_residual_entry" in residual_pin_call[
        "continue_cascade_wake_route_apply_follow_pin_call_line"
    ]
    assert "residual_route=false→true" in residual_pin_call[
        "continue_cascade_wake_route_apply_follow_pin_call_line"
    ]
    # Body-free continue cascade wake route apply follow pin call next seals
    # pin_call into one next-invoke recipe (execute_helper/package_helper/inventory).
    assert pipeline[
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_helper"
    ] == (
        "package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next"
    )
    assert pipeline["operator_state"][
    ...
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_line"
    ].startswith("continue_cascade_wake_route_apply_follow_pin_call_next action=")
    assert "residual_export=false" in pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_line"
    ]
    assert "pin_call_helper=package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call" in (
        pipeline["operator_state"][
            "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_line"
        ]
    )
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_invoke"
    ] in {"execute_helper", "package_helper", "inventory_only"}
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_ready"
    ] is True
    identity_pin_call_next = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next(
            pin_call=identity_pin_call,
        )
    )
    assert identity_pin_call_next["controller_surface"] == (
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next"
    )
    assert identity_pin_call_next["next_action"] == "execute_now"
    assert identity_pin_call_next["next_invoke"] == "execute_helper"
    assert identity_pin_call_next["call_next_with_execute"] is True
    assert identity_pin_call_next["next_ready"] is True
    assert identity_pin_call_next["pin_advanced"] is False
    assert identity_pin_call_next["preferred_helper"] == (
        "follow_reverse_flow_focused_validation_continue_dispatch"
    )
    assert identity_pin_call_next["residual_export_allowed"] is False
    assert identity_pin_call_next["runtime_action"] == "none"
    assert identity_pin_call_next[
        "continue_cascade_wake_route_apply_follow_pin_call_next_line"
    ].startswith(
        "continue_cascade_wake_route_apply_follow_pin_call_next action=execute_now "
    )
    assert "invoke=execute_helper" in identity_pin_call_next[
        "continue_cascade_wake_route_apply_follow_pin_call_next_line"
    ]
    assert "call_execute=true" in identity_pin_call_next[
        "continue_cascade_wake_route_apply_follow_pin_call_next_line"
    ]
    assert "advanced=false" in identity_pin_call_next[
        "continue_cascade_wake_route_apply_follow_pin_call_next_line"
    ]
    assert "residual_export=false" in identity_pin_call_next[
        "continue_cascade_wake_route_apply_follow_pin_call_next_line"
    ]
    advanced_pin_call_next = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next(
            pin_call=advanced_pin_call,
        )
    )
    assert advanced_pin_call_next["next_action"] == "keep_activation_external"
    assert advanced_pin_call_next["next_invoke"] == "package_helper"
    assert advanced_pin_call_next["call_next_with_execute"] is False
    assert advanced_pin_call_next["next_ready"] is True
    assert advanced_pin_call_next["pin_advanced"] is True
    assert advanced_pin_call_next["applied_pin_mode"] == "package_helper"
    assert advanced_pin_call_next["residual_export_allowed"] is False
    assert "action=keep_activation_external" in advanced_pin_call_next[
        "continue_cascade_wake_route_apply_follow_pin_call_next_line"
    ]
    assert "invoke=package_helper" in advanced_pin_call_next[
        "continue_cascade_wake_route_apply_follow_pin_call_next_line"
    ]
    assert "advanced=true" in advanced_pin_call_next[
        "continue_cascade_wake_route_apply_follow_pin_call_next_line"
    ]
    assert "call_execute=false" in advanced_pin_call_next[
        "continue_cascade_wake_route_apply_follow_pin_call_next_line"
    ]
    residual_pin_call_next = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next(
            pin_call=residual_pin_call,
        )
    )
    assert residual_pin_call_next["next_action"] == "open_residual_entry"
    assert residual_pin_call_next["next_invoke"] == "package_helper"
    assert residual_pin_call_next["call_next_with_execute"] is False
    assert residual_pin_call_next["post_residual_route_ready"] is True
    assert residual_pin_call_next["residual_export_allowed"] is False
    assert "action=open_residual_entry" in residual_pin_call_next[
        "continue_cascade_wake_route_apply_follow_pin_call_next_line"
    ]
    assert "residual_route=true" in residual_pin_call_next[
        "continue_cascade_wake_route_apply_follow_pin_call_next_line"
    ]
    # Body-free continue cascade wake route apply follow pin call next call
    # collapses pre/post next-invoke recipes into one next-call receipt.
    assert pipeline[
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_helper"
    ] == (
        "package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call"
    )
    assert pipeline["operator_state"][
    ...
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_line"
    ].startswith("continue_cascade_wake_route_apply_follow_pin_call_next_call pre_action=")
    assert "residual_export=false" in pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_line"
    ]
    assert "pin_call_next_helper=package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next" in (
        pipeline["operator_state"][
            "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_line"
        ]
    )
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_advanced"
    ] is False
    identity_pin_call_next_call = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call(
            pre_next=identity_pin_call_next,
            post_next=identity_pin_call_next,
            executed=False,
            recorded=False,
        )
    )
    assert identity_pin_call_next_call["controller_surface"] == (
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call"
    )
    assert identity_pin_call_next_call["next_action_transition"] == "execute_now→execute_now"
    assert identity_pin_call_next_call["next_invoke_transition"] == (
        "execute_helper→execute_helper"
    )
    assert identity_pin_call_next_call["next_advanced"] is False
    assert identity_pin_call_next_call["applied_next_action"] == "execute_now"
    assert identity_pin_call_next_call["applied_next_invoke"] == "execute_helper"
    assert identity_pin_call_next_call["residual_export_allowed"] is False
    assert identity_pin_call_next_call["runtime_action"] == "none"
    assert identity_pin_call_next_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_line"
    ].startswith(
        "continue_cascade_wake_route_apply_follow_pin_call_next_call pre_action=execute_now "
    )
    assert "invoke=execute_helper→execute_helper" in identity_pin_call_next_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_line"
    ]
    assert "next_advanced=false" in identity_pin_call_next_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_line"
    ]
    assert "residual_export=false" in identity_pin_call_next_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_line"
    ]
    advanced_pin_call_next_call = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call(
            pre_next=identity_pin_call_next,
            post_next=advanced_pin_call_next,
            executed=True,
            recorded=True,
        )
    )
    assert advanced_pin_call_next_call["next_action_transition"] == (
        "execute_now→keep_activation_external"
    )
    assert advanced_pin_call_next_call["next_invoke_transition"] == (
        "execute_helper→package_helper"
    )
    assert advanced_pin_call_next_call["next_advanced"] is True
    assert advanced_pin_call_next_call["applied_next_action"] == "keep_activation_external"
    assert advanced_pin_call_next_call["applied_next_invoke"] == "package_helper"
    assert advanced_pin_call_next_call["post_call_next_with_execute"] is False
    assert advanced_pin_call_next_call["residual_export_allowed"] is False
    assert "action=execute_now→keep_activation_external" in advanced_pin_call_next_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_line"
    ]
    assert "invoke=execute_helper→package_helper" in advanced_pin_call_next_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_line"
    ]
    assert "next_advanced=true" in advanced_pin_call_next_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_line"
    ]
    assert "call_execute=true→false" in advanced_pin_call_next_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_line"
    ]
    residual_pin_call_next_call = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call(
            pre_next=advanced_pin_call_next,
            post_next=residual_pin_call_next,
            executed=True,
            recorded=True,
        )
    )
    assert residual_pin_call_next_call["next_action_transition"] == (
        "keep_activation_external→open_residual_entry"
    )
    assert residual_pin_call_next_call["next_invoke_transition"] == (
        "package_helper→package_helper"
    )
    assert residual_pin_call_next_call["next_advanced"] is True
    assert residual_pin_call_next_call["post_residual_route_ready"] is True
    assert residual_pin_call_next_call["residual_export_allowed"] is False
    assert "action=keep_activation_external→open_residual_entry" in residual_pin_call_next_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_line"
    ]
    assert "residual_route=false→true" in residual_pin_call_next_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_line"
    ]
    # Body-free continue cascade wake route apply follow pin call next call
    # follow maps next-call receipts into one durable follow policy.
    assert pipeline[
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_helper"
    ] == (
        "package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow"
    )
    assert pipeline["operator_state"][
    ...
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_line"
    ].startswith(
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow applied="
    )
    assert "residual_export=false" in pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_line"
    ]
    assert "next_call_helper=package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call" in (
        pipeline["operator_state"][
            "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_line"
        ]
    )
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_action"
    ] == "execute_now"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_mode"
    ] == "execute_helper"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_call_execute"
    ] is True
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_ready"
    ] is True
    identity_pin_call_next_call_follow = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow(
            next_call=identity_pin_call_next_call,
        )
    )
    assert identity_pin_call_next_call_follow["controller_surface"] == (
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow"
    )
    assert identity_pin_call_next_call_follow["follow_action"] == "execute_now"
    assert identity_pin_call_next_call_follow["follow_mode"] == "execute_helper"
    assert identity_pin_call_next_call_follow["call_follow_with_execute"] is True
    assert identity_pin_call_next_call_follow["follow_ready"] is True
    assert identity_pin_call_next_call_follow["next_advanced"] is False
    assert identity_pin_call_next_call_follow["residual_export_allowed"] is False
    assert identity_pin_call_next_call_follow["runtime_action"] == "none"
    assert identity_pin_call_next_call_follow[
    ...
    assert "mode=execute_helper" in identity_pin_call_next_call_follow[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_line"
    ]
    assert "call_execute=true" in identity_pin_call_next_call_follow[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_line"
    ]
    assert "follow_ready=true" in identity_pin_call_next_call_follow[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_line"
    ]
    assert "residual_export=false" in identity_pin_call_next_call_follow[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_line"
    ]
    advanced_pin_call_next_call_follow = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow(
            next_call=advanced_pin_call_next_call,
        )
    )
    assert advanced_pin_call_next_call_follow["follow_action"] == (
        "keep_activation_external"
    )
    assert advanced_pin_call_next_call_follow["follow_mode"] == "package_helper"
    assert advanced_pin_call_next_call_follow["call_follow_with_execute"] is False
    assert advanced_pin_call_next_call_follow["follow_ready"] is True
    assert advanced_pin_call_next_call_follow["next_advanced"] is True
    assert advanced_pin_call_next_call_follow["applied_next_action"] == (
        "keep_activation_external"
    )
    assert advanced_pin_call_next_call_follow["applied_next_invoke"] == "package_helper"
    assert advanced_pin_call_next_call_follow["residual_export_allowed"] is False
    assert "applied=keep_activation_external" in advanced_pin_call_next_call_follow[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_line"
    ]
    assert "mode=package_helper" in advanced_pin_call_next_call_follow[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_line"
    ]
    assert "advanced=true" in advanced_pin_call_next_call_follow[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_line"
    ]
    assert "call_execute=false" in advanced_pin_call_next_call_follow[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_line"
    ]
    residual_pin_call_next_call_follow = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow(
            next_call=residual_pin_call_next_call,
        )
    )
    assert residual_pin_call_next_call_follow["follow_action"] == "open_residual_entry"
    assert residual_pin_call_next_call_follow["follow_mode"] == "package_helper"
    assert residual_pin_call_next_call_follow["call_follow_with_execute"] is False
    assert residual_pin_call_next_call_follow["post_residual_route_ready"] is True
    assert residual_pin_call_next_call_follow["residual_export_allowed"] is False
    assert "applied=open_residual_entry" in residual_pin_call_next_call_follow[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_line"
    ]
    assert "residual_route=true" in residual_pin_call_next_call_follow[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_line"
    ]
    # Body-free continue cascade wake route apply follow pin call next call
    # follow pin maps next-call-follow receipts into one durable pin recipe.
    assert pipeline[
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_helper"
    ] == (
        "package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin"
    )
    assert pipeline["operator_state"][
    ...
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_line"
    ].startswith(
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin action="
    )
    assert "residual_export=false" in pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_line"
    ]
    assert "follow_helper=package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow" in (
        pipeline["operator_state"][
            "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_line"
        ]
    )
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_action"
    ] == "execute_now"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_mode"
    ] == "execute_helper"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_execute"
    ] is True
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_ready"
    ] is True
    identity_pin_call_next_call_follow_pin = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin(
            next_call_follow=identity_pin_call_next_call_follow,
        )
    )
    assert identity_pin_call_next_call_follow_pin["controller_surface"] == (
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin"
    )
    assert identity_pin_call_next_call_follow_pin["pin_action"] == "execute_now"
    assert identity_pin_call_next_call_follow_pin["pin_mode"] == "execute_helper"
    assert identity_pin_call_next_call_follow_pin["call_pin_with_execute"] is True
    assert identity_pin_call_next_call_follow_pin["pin_ready"] is True
    assert identity_pin_call_next_call_follow_pin["next_advanced"] is False
    assert identity_pin_call_next_call_follow_pin["residual_export_allowed"] is False
    assert identity_pin_call_next_call_follow_pin["runtime_action"] == "none"
    assert identity_pin_call_next_call_follow_pin[
    ...
    assert "mode=execute_helper" in identity_pin_call_next_call_follow_pin[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_line"
    ]
    assert "call_execute=true" in identity_pin_call_next_call_follow_pin[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_line"
    ]
    assert "pin_ready=true" in identity_pin_call_next_call_follow_pin[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_line"
    ]
    assert "residual_export=false" in identity_pin_call_next_call_follow_pin[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_line"
    ]
    advanced_pin_call_next_call_follow_pin = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin(
            next_call_follow=advanced_pin_call_next_call_follow,
        )
    )
    assert advanced_pin_call_next_call_follow_pin["pin_action"] == (
        "keep_activation_external"
    )
    assert advanced_pin_call_next_call_follow_pin["pin_mode"] == "package_helper"
    assert advanced_pin_call_next_call_follow_pin["call_pin_with_execute"] is False
    assert advanced_pin_call_next_call_follow_pin["pin_ready"] is True
    assert advanced_pin_call_next_call_follow_pin["next_advanced"] is True
    assert advanced_pin_call_next_call_follow_pin["follow_action"] == (
        "keep_activation_external"
    )
    assert advanced_pin_call_next_call_follow_pin["follow_mode"] == "package_helper"
    assert advanced_pin_call_next_call_follow_pin["residual_export_allowed"] is False
    assert "action=keep_activation_external" in advanced_pin_call_next_call_follow_pin[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_line"
    ]
    assert "mode=package_helper" in advanced_pin_call_next_call_follow_pin[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_line"
    ]
    assert "advanced=true" in advanced_pin_call_next_call_follow_pin[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_line"
    ]
    assert "call_execute=false" in advanced_pin_call_next_call_follow_pin[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_line"
    ]
    residual_pin_call_next_call_follow_pin = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin(
            next_call_follow=residual_pin_call_next_call_follow,
        )
    )
    assert residual_pin_call_next_call_follow_pin["pin_action"] == "open_residual_entry"
    assert residual_pin_call_next_call_follow_pin["pin_mode"] == "package_helper"
    assert residual_pin_call_next_call_follow_pin["call_pin_with_execute"] is False
    assert residual_pin_call_next_call_follow_pin["post_residual_route_ready"] is True
    assert residual_pin_call_next_call_follow_pin["residual_export_allowed"] is False
    assert "action=open_residual_entry" in residual_pin_call_next_call_follow_pin[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_line"
    ]
    assert "residual_route=true" in residual_pin_call_next_call_follow_pin[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_line"
    ]
    # Body-free continue cascade wake route apply follow pin call next call
    # follow pin call collapses pre/post pin recipes into one call receipt.
    assert pipeline[
    ...
    assert "pin_helper=package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin" in (
        pipeline["operator_state"][
            "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_line"
        ]
    )
    ...
    identity_pin_call_next_call_follow_pin_call = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call(
            pre_pin=identity_pin_call_next_call_follow_pin,
            post_pin=identity_pin_call_next_call_follow_pin,
            executed=False,
            recorded=False,
        )
    )
    assert identity_pin_call_next_call_follow_pin_call["controller_surface"] == (
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call"
    )
    assert identity_pin_call_next_call_follow_pin_call["pin_action_transition"] == (
        "execute_now→execute_now"
    )
    assert identity_pin_call_next_call_follow_pin_call["pin_mode_transition"] == (
        "execute_helper→execute_helper"
    )
    assert identity_pin_call_next_call_follow_pin_call["pin_advanced"] is False
    assert identity_pin_call_next_call_follow_pin_call["applied_pin_action"] == (
        "execute_now"
    )
    assert identity_pin_call_next_call_follow_pin_call["applied_pin_mode"] == (
        "execute_helper"
    )
    assert identity_pin_call_next_call_follow_pin_call["residual_export_allowed"] is False
    assert identity_pin_call_next_call_follow_pin_call["runtime_action"] == "none"
    assert identity_pin_call_next_call_follow_pin_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_line"
    ].startswith(
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call "
        "pre_action=execute_now "
    )
    assert "pin_advanced=false" in identity_pin_call_next_call_follow_pin_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_line"
    ]
    assert "residual_export=false" in identity_pin_call_next_call_follow_pin_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_line"
    ]
    advanced_pin_call_next_call_follow_pin_call = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call(
            pre_pin=identity_pin_call_next_call_follow_pin,
            post_pin=advanced_pin_call_next_call_follow_pin,
            executed=True,
            recorded=True,
        )
    )
    assert advanced_pin_call_next_call_follow_pin_call["pin_action_transition"] == (
        "execute_now→keep_activation_external"
    )
    assert advanced_pin_call_next_call_follow_pin_call["pin_mode_transition"] == (
        "execute_helper→package_helper"
    )
    assert advanced_pin_call_next_call_follow_pin_call["pin_advanced"] is True
    assert advanced_pin_call_next_call_follow_pin_call["applied_pin_action"] == (
        "keep_activation_external"
    )
    assert advanced_pin_call_next_call_follow_pin_call["applied_pin_mode"] == (
        "package_helper"
    )
    assert advanced_pin_call_next_call_follow_pin_call["executed"] is True
    assert advanced_pin_call_next_call_follow_pin_call["recorded"] is True
    assert advanced_pin_call_next_call_follow_pin_call["residual_export_allowed"] is False
    assert "action=execute_now→keep_activation_external" in (
        advanced_pin_call_next_call_follow_pin_call[
            "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_line"
        ]
    )
    assert "mode=execute_helper→package_helper" in advanced_pin_call_next_call_follow_pin_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_line"
    ]
    assert "pin_advanced=true" in advanced_pin_call_next_call_follow_pin_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_line"
    ]
    assert "call_execute=true→false" in advanced_pin_call_next_call_follow_pin_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_line"
    ]
    residual_pin_call_next_call_follow_pin_call = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call(
            pre_pin=advanced_pin_call_next_call_follow_pin,
            post_pin=residual_pin_call_next_call_follow_pin,
            executed=True,
            recorded=True,
        )
    )
    assert residual_pin_call_next_call_follow_pin_call["pin_action_transition"] == (
        "keep_activation_external→open_residual_entry"
    )
    assert residual_pin_call_next_call_follow_pin_call["pin_mode_transition"] == (
        "package_helper→package_helper"
    )
    assert residual_pin_call_next_call_follow_pin_call["pin_advanced"] is True
    assert residual_pin_call_next_call_follow_pin_call["post_residual_route_ready"] is True
    assert residual_pin_call_next_call_follow_pin_call["residual_export_allowed"] is False
    assert "action=keep_activation_external→open_residual_entry" in (
        residual_pin_call_next_call_follow_pin_call[
            "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_line"
        ]
    )
    assert "residual_route=false→true" in residual_pin_call_next_call_follow_pin_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_line"
    ]
    identity_pin_call_next_call_follow_pin_call_next = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next(
            pin_call=identity_pin_call_next_call_follow_pin_call,
        )
    )
    assert identity_pin_call_next_call_follow_pin_call_next["controller_surface"] == (
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next"
    )
    assert identity_pin_call_next_call_follow_pin_call_next["next_action"] == "execute_now"
    assert identity_pin_call_next_call_follow_pin_call_next["next_invoke"] == (
        "execute_helper"
    )
    assert identity_pin_call_next_call_follow_pin_call_next["next_ready"] is True
    assert identity_pin_call_next_call_follow_pin_call_next["call_next_with_execute"] is True
    assert identity_pin_call_next_call_follow_pin_call_next["pin_advanced"] is False
    assert identity_pin_call_next_call_follow_pin_call_next["residual_export_allowed"] is False
    assert identity_pin_call_next_call_follow_pin_call_next["runtime_action"] == "none"
    assert identity_pin_call_next_call_follow_pin_call_next[
    ...
    assert "invoke=execute_helper" in identity_pin_call_next_call_follow_pin_call_next[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_line"
    ]
    assert "advanced=false" in identity_pin_call_next_call_follow_pin_call_next[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_line"
    ]
    assert "residual_export=false" in identity_pin_call_next_call_follow_pin_call_next[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_line"
    ]
    advanced_pin_call_next_call_follow_pin_call_next = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next(
            pin_call=advanced_pin_call_next_call_follow_pin_call,
        )
    )
    assert advanced_pin_call_next_call_follow_pin_call_next["next_action"] == (
        "keep_activation_external"
    )
    assert advanced_pin_call_next_call_follow_pin_call_next["next_invoke"] == (
        "package_helper"
    )
    assert advanced_pin_call_next_call_follow_pin_call_next["next_ready"] is True
    assert advanced_pin_call_next_call_follow_pin_call_next["call_next_with_execute"] is False
    assert advanced_pin_call_next_call_follow_pin_call_next["pin_advanced"] is True
    assert advanced_pin_call_next_call_follow_pin_call_next["executed"] is True
    assert advanced_pin_call_next_call_follow_pin_call_next["recorded"] is True
    assert advanced_pin_call_next_call_follow_pin_call_next["residual_export_allowed"] is False
    assert "action=keep_activation_external" in advanced_pin_call_next_call_follow_pin_call_next[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_line"
    ]
    assert "invoke=package_helper" in advanced_pin_call_next_call_follow_pin_call_next[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_line"
    ]
    assert "advanced=true" in advanced_pin_call_next_call_follow_pin_call_next[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_line"
    ]
    residual_pin_call_next_call_follow_pin_call_next = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next(
            pin_call=residual_pin_call_next_call_follow_pin_call,
        )
    )
    assert residual_pin_call_next_call_follow_pin_call_next["next_action"] == (
        "open_residual_entry"
    )
    assert residual_pin_call_next_call_follow_pin_call_next["next_invoke"] == (
        "package_helper"
    )
    assert residual_pin_call_next_call_follow_pin_call_next["next_ready"] is True
    assert residual_pin_call_next_call_follow_pin_call_next["post_residual_route_ready"] is True
    assert residual_pin_call_next_call_follow_pin_call_next["residual_export_allowed"] is False
    assert "action=open_residual_entry" in residual_pin_call_next_call_follow_pin_call_next[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_line"
    ]
    assert "residual_route=true" in residual_pin_call_next_call_follow_pin_call_next[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_line"
    ]
    identity_pin_call_next_call_follow_pin_call_next_call = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call(
            pre_next=identity_pin_call_next_call_follow_pin_call_next,
            post_next=identity_pin_call_next_call_follow_pin_call_next,
            executed=False,
            recorded=False,
        )
    )
    assert identity_pin_call_next_call_follow_pin_call_next_call["controller_surface"] == (
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call"
    )
    assert identity_pin_call_next_call_follow_pin_call_next_call["next_action_transition"] == (
        "execute_now→execute_now"
    )
    assert identity_pin_call_next_call_follow_pin_call_next_call["next_invoke_transition"] == (
        "execute_helper→execute_helper"
    )
    assert identity_pin_call_next_call_follow_pin_call_next_call["next_advanced"] is False
    assert identity_pin_call_next_call_follow_pin_call_next_call["residual_export_allowed"] is False
    assert identity_pin_call_next_call_follow_pin_call_next_call["runtime_action"] == "none"
    assert identity_pin_call_next_call_follow_pin_call_next_call[
    ...
    assert "next_advanced=false" in identity_pin_call_next_call_follow_pin_call_next_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_line"
    ]
    assert "residual_export=false" in identity_pin_call_next_call_follow_pin_call_next_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_line"
    ]
    advanced_pin_call_next_call_follow_pin_call_next_call = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call(
            pre_next=identity_pin_call_next_call_follow_pin_call_next,
            post_next=advanced_pin_call_next_call_follow_pin_call_next,
            executed=True,
            recorded=True,
        )
    )
    assert advanced_pin_call_next_call_follow_pin_call_next_call["next_action_transition"] == (
        "execute_now→keep_activation_external"
    )
    assert advanced_pin_call_next_call_follow_pin_call_next_call["next_invoke_transition"] == (
        "execute_helper→package_helper"
    )
    assert advanced_pin_call_next_call_follow_pin_call_next_call["next_advanced"] is True
    assert advanced_pin_call_next_call_follow_pin_call_next_call["applied_next_action"] == (
        "keep_activation_external"
    )
    assert advanced_pin_call_next_call_follow_pin_call_next_call["applied_next_invoke"] == (
        "package_helper"
    )
    assert advanced_pin_call_next_call_follow_pin_call_next_call["executed"] is True
    assert advanced_pin_call_next_call_follow_pin_call_next_call["recorded"] is True
    assert advanced_pin_call_next_call_follow_pin_call_next_call["residual_export_allowed"] is False
    assert "action=execute_now→keep_activation_external" in (
        advanced_pin_call_next_call_follow_pin_call_next_call[
            "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_line"
        ]
    )
    assert "invoke=execute_helper→package_helper" in advanced_pin_call_next_call_follow_pin_call_next_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_line"
    ]
    assert "next_advanced=true" in advanced_pin_call_next_call_follow_pin_call_next_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_line"
    ]
    assert "call_execute=true→false" in advanced_pin_call_next_call_follow_pin_call_next_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_line"
    ]
    residual_pin_call_next_call_follow_pin_call_next_call = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call(
            pre_next=advanced_pin_call_next_call_follow_pin_call_next,
            post_next=residual_pin_call_next_call_follow_pin_call_next,
            executed=True,
            recorded=True,
        )
    )
    assert residual_pin_call_next_call_follow_pin_call_next_call["next_action_transition"] == (
        "keep_activation_external→open_residual_entry"
    )
    assert residual_pin_call_next_call_follow_pin_call_next_call["next_invoke_transition"] == (
        "package_helper→package_helper"
    )
    assert residual_pin_call_next_call_follow_pin_call_next_call["next_advanced"] is True
    assert residual_pin_call_next_call_follow_pin_call_next_call["post_residual_route_ready"] is True
    assert residual_pin_call_next_call_follow_pin_call_next_call["residual_export_allowed"] is False
    assert "action=keep_activation_external→open_residual_entry" in (
        residual_pin_call_next_call_follow_pin_call_next_call[
            "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_line"
        ]
    )
    assert "residual_route=false→true" in residual_pin_call_next_call_follow_pin_call_next_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_line"
    ]
    identity_pin_call_next_call_follow_pin_call_next_call_follow = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow(
            next_call=identity_pin_call_next_call_follow_pin_call_next_call,
        )
    )
    assert identity_pin_call_next_call_follow_pin_call_next_call_follow[
        "controller_surface"
    ] == (
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow"
    )
    assert identity_pin_call_next_call_follow_pin_call_next_call_follow[
        "follow_action"
    ] == "execute_now"
    ...
    assert "mode=execute_helper" in identity_pin_call_next_call_follow_pin_call_next_call_follow[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_line"
    ]
    assert "call_execute=true" in identity_pin_call_next_call_follow_pin_call_next_call_follow[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_line"
    ]
    assert "residual_export=false" in identity_pin_call_next_call_follow_pin_call_next_call_follow[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_line"
    ]
    advanced_pin_call_next_call_follow_pin_call_next_call_follow = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow(
            next_call=advanced_pin_call_next_call_follow_pin_call_next_call,
        )
    )
    assert advanced_pin_call_next_call_follow_pin_call_next_call_follow[
        "follow_action"
    ] == "keep_activation_external"
    assert advanced_pin_call_next_call_follow_pin_call_next_call_follow[
        "follow_mode"
    ...
    assert "applied=keep_activation_external" in (
        advanced_pin_call_next_call_follow_pin_call_next_call_follow[
            "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_line"
        ]
    )
    assert "mode=package_helper" in advanced_pin_call_next_call_follow_pin_call_next_call_follow[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_line"
    ]
    assert "advanced=true" in advanced_pin_call_next_call_follow_pin_call_next_call_follow[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_line"
    ]
    residual_pin_call_next_call_follow_pin_call_next_call_follow = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow(
            next_call=residual_pin_call_next_call_follow_pin_call_next_call,
        )
    )
    assert residual_pin_call_next_call_follow_pin_call_next_call_follow[
        "follow_action"
    ] == "open_residual_entry"
    assert residual_pin_call_next_call_follow_pin_call_next_call_follow[
        "follow_mode"
    ...
    assert "applied=open_residual_entry" in (
        residual_pin_call_next_call_follow_pin_call_next_call_follow[
            "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_line"
        ]
    )
    ...
    identity_pin_call_next_call_follow_pin_call_next_call_follow_pin_call = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call(
            pre_pin=identity_pin_call_next_call_follow_pin_call_next_call_follow_pin,
            post_pin=identity_pin_call_next_call_follow_pin_call_next_call_follow_pin,
            executed=False,
            recorded=False,
        )
    )
    assert identity_pin_call_next_call_follow_pin_call_next_call_follow_pin_call[
        "controller_surface"
    ] == (
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call"
    )
    assert identity_pin_call_next_call_follow_pin_call_next_call_follow_pin_call[
    ...
    assert "pin_advanced=false" in identity_pin_call_next_call_follow_pin_call_next_call_follow_pin_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_line"
    ]
    assert "residual_export=false" in identity_pin_call_next_call_follow_pin_call_next_call_follow_pin_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_line"
    ]
    advanced_pin_call_next_call_follow_pin_call_next_call_follow_pin_call = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call(
            pre_pin=identity_pin_call_next_call_follow_pin_call_next_call_follow_pin,
            post_pin=advanced_pin_call_next_call_follow_pin_call_next_call_follow_pin,
            executed=True,
            recorded=True,
        )
    )
    assert advanced_pin_call_next_call_follow_pin_call_next_call_follow_pin_call[
        "pin_action_transition"
    ] == "execute_now→keep_activation_external"
    assert advanced_pin_call_next_call_follow_pin_call_next_call_follow_pin_call[
        "pin_mode_transition"
    ...
    assert "mode=execute_helper→package_helper" in advanced_pin_call_next_call_follow_pin_call_next_call_follow_pin_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_line"
    ]
    assert "pin_advanced=true" in advanced_pin_call_next_call_follow_pin_call_next_call_follow_pin_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_line"
    ]
    assert "call_execute=true→false" in advanced_pin_call_next_call_follow_pin_call_next_call_follow_pin_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_line"
    ]
    residual_pin_call_next_call_follow_pin_call_next_call_follow_pin_call = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call(
            pre_pin=advanced_pin_call_next_call_follow_pin_call_next_call_follow_pin,
            post_pin=residual_pin_call_next_call_follow_pin_call_next_call_follow_pin,
            executed=True,
            recorded=True,
        )
    )
    assert residual_pin_call_next_call_follow_pin_call_next_call_follow_pin_call[
        "pin_action_transition"
    ] == "keep_activation_external→open_residual_entry"
    assert residual_pin_call_next_call_follow_pin_call_next_call_follow_pin_call[
        "pin_mode_transition"
    ...
    assert "residual_route=false→true" in residual_pin_call_next_call_follow_pin_call_next_call_follow_pin_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_line"
    ]
    identity_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next(
            pin_call=identity_pin_call_next_call_follow_pin_call_next_call_follow_pin_call,
        )
    )
    assert identity_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next[
        "controller_surface"
    ] == (
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next"
    )
    assert identity_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next[
        "next_action"
    ] == "execute_now"
    ...
    assert "invoke=execute_helper" in identity_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_line"
    ]
    assert "residual_export=false" in identity_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_line"
    ]
    advanced_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next(
            pin_call=advanced_pin_call_next_call_follow_pin_call_next_call_follow_pin_call,
        )
    )
    assert advanced_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next[
        "next_action"
    ] == "keep_activation_external"
    assert advanced_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next[
        "next_invoke"
    ...
    assert "action=keep_activation_external" in advanced_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_line"
    ]
    assert "invoke=package_helper" in advanced_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_line"
    ]
    assert "advanced=true" in advanced_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_line"
    ]
    residual_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next(
            pin_call=residual_pin_call_next_call_follow_pin_call_next_call_follow_pin_call,
        )
    )
    assert residual_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next[
        "next_action"
    ] == "open_residual_entry"
    assert residual_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next[
        "next_invoke"
    ...
    assert "action=open_residual_entry" in residual_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_line"
    ]
    assert "residual_route=true" in residual_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_line"
    ]
    identity_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call(
            pre_next=identity_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next,
            post_next=identity_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next,
            executed=False,
            recorded=False,
        )
    )
    assert identity_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call[
        "controller_surface"
    ] == (
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call"
    )
    assert identity_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call[
        "next_action_transition"
    ] == "execute_now→execute_now"
    assert identity_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call[
        "next_invoke_transition"
    ] == "execute_helper→execute_helper"
    assert identity_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call[
        "next_advanced"
    ] is False
    ...
    assert "residual_export=false" in identity_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_line"
    ]
    advanced_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call(
            pre_next=identity_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next,
            post_next=advanced_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next,
            executed=True,
            recorded=True,
        )
    )
    assert advanced_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call[
        "next_action_transition"
    ] == "execute_now→keep_activation_external"
    assert advanced_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call[
        "next_invoke_transition"
    ...
    assert "action=execute_now→keep_activation_external" in advanced_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_line"
    ]
    assert "invoke=execute_helper→package_helper" in advanced_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_line"
    ]
    assert "next_advanced=true" in advanced_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_line"
    ]
    residual_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call(
            pre_next=advanced_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next,
            post_next=residual_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next,
            executed=True,
            recorded=True,
        )
    )
    assert residual_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call[
        "next_action_transition"
    ] == "keep_activation_external→open_residual_entry"
    assert residual_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call[
        "next_advanced"
    ...
    assert "action=keep_activation_external→open_residual_entry" in residual_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_line"
    ]
    assert "residual_route=false→true" in residual_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_line"
    ]
    identity_deep_next_call_follow = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow(
            next_call=identity_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call,
        )
    )
    assert identity_deep_next_call_follow["controller_surface"] == (
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow"
    )
    assert identity_deep_next_call_follow["follow_action"] == "execute_now"
    assert identity_deep_next_call_follow["follow_mode"] == "execute_helper"
    assert identity_deep_next_call_follow["call_follow_with_execute"] is True
    assert identity_deep_next_call_follow["follow_ready"] is True
    assert identity_deep_next_call_follow["next_advanced"] is False
    assert identity_deep_next_call_follow["residual_export_allowed"] is False
    assert identity_deep_next_call_follow["runtime_action"] == "none"
    assert identity_deep_next_call_follow[
        ...
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow applied=execute_now "
    )
    assert "residual_export=false" in identity_deep_next_call_follow[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_line"
    ]
    advanced_deep_next_call_follow = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow(
            next_call=advanced_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call,
        )
    )
    assert advanced_deep_next_call_follow["follow_action"] == "keep_activation_external"
    assert advanced_deep_next_call_follow["follow_mode"] == "package_helper"
    assert advanced_deep_next_call_follow["call_follow_with_execute"] is False
    assert advanced_deep_next_call_follow["follow_ready"] is True
    assert advanced_deep_next_call_follow["next_advanced"] is True
    assert advanced_deep_next_call_follow["residual_export_allowed"] is False
    assert "applied=keep_activation_external" in advanced_deep_next_call_follow[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_line"
    ]
    assert "mode=package_helper" in advanced_deep_next_call_follow[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_line"
    ]
    assert "advanced=true" in advanced_deep_next_call_follow[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_line"
    ]
    residual_deep_next_call_follow = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow(
            next_call=residual_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call,
        )
    )
    assert residual_deep_next_call_follow["follow_action"] == "open_residual_entry"
    assert residual_deep_next_call_follow["follow_mode"] == "package_helper"
    assert residual_deep_next_call_follow["post_residual_route_ready"] is True
    assert residual_deep_next_call_follow["residual_export_allowed"] is False
    assert "applied=open_residual_entry" in residual_deep_next_call_follow[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_line"
    ]
    assert "residual_route=true" in residual_deep_next_call_follow[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_line"
    ]
    identity_deep_next_call_follow_pin = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin(
            next_call_follow=identity_deep_next_call_follow,
        )
    )
    assert identity_deep_next_call_follow_pin["controller_surface"] == (
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin"
    )
    assert identity_deep_next_call_follow_pin["pin_action"] == "execute_now"
    assert identity_deep_next_call_follow_pin["pin_mode"] == "execute_helper"
    assert identity_deep_next_call_follow_pin["call_pin_with_execute"] is True
    assert identity_deep_next_call_follow_pin["pin_ready"] is True
    assert identity_deep_next_call_follow_pin["residual_export_allowed"] is False
    assert identity_deep_next_call_follow_pin["runtime_action"] == "none"
    assert identity_deep_next_call_follow_pin[
        ...
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin action=execute_now "
    )
    assert "residual_export=false" in identity_deep_next_call_follow_pin[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_line"
    ]
    advanced_deep_next_call_follow_pin = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin(
            next_call_follow=advanced_deep_next_call_follow,
        )
    )
    assert advanced_deep_next_call_follow_pin["pin_action"] == "keep_activation_external"
    assert advanced_deep_next_call_follow_pin["pin_mode"] == "package_helper"
    assert advanced_deep_next_call_follow_pin["call_pin_with_execute"] is False
    assert advanced_deep_next_call_follow_pin["pin_ready"] is True
    assert advanced_deep_next_call_follow_pin["residual_export_allowed"] is False
    assert "action=keep_activation_external" in advanced_deep_next_call_follow_pin[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_line"
    ]
    assert "mode=package_helper" in advanced_deep_next_call_follow_pin[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_line"
    ]
    residual_deep_next_call_follow_pin = (
        package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin(
            next_call_follow=residual_deep_next_call_follow,
        )
    )
    assert residual_deep_next_call_follow_pin["pin_action"] == "open_residual_entry"
    assert residual_deep_next_call_follow_pin["pin_mode"] == "package_helper"
    assert residual_deep_next_call_follow_pin["post_residual_route_ready"] is True
    assert residual_deep_next_call_follow_pin["residual_export_allowed"] is False
    assert "action=open_residual_entry" in residual_deep_next_call_follow_pin[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_line"
    ]
    assert "residual_route=true" in residual_deep_next_call_follow_pin[
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_line"
    ]
    assert pipeline["operator_state"][
    ...
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_action"
    ] == "execute_now"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_invoke"
    ] == "execute_helper"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_ready"
    ] is True
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_line"
    ].startswith(
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next "
    )
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_helper"
    ] == (
        "package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call"
    )
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_action_transition"
    ] == "execute_now→execute_now"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_invoke_transition"
    ] == "execute_helper→execute_helper"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_advanced"
    ] is False
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_line"
    ].startswith(
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call "
    )
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_helper"
    ] == (
        "package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow"
    )
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_action"
    ] == "execute_now"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_mode"
    ] == "execute_helper"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_call_execute"
    ] is True
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_ready"
    ] is True
    assert pipeline["operator_state"][
    ...
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_helper"
    ] == (
        "package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call"
    )
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_action_transition"
    ] == "execute_now→execute_now"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_mode_transition"
    ] == "execute_helper→execute_helper"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_advanced"
    ] is False
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_line"
    ].startswith(
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call "
    )
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_helper"
    ] == (
        "package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next"
    )
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_action"
    ] == "execute_now"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_invoke"
    ] == "execute_helper"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_execute"
    ] is True
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_ready"
    ] is True
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_line"
    ].startswith(
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next "
    )
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_helper"
    ] == (
        "package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call"
    )
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_action_transition"
    ] == "execute_now→execute_now"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_invoke_transition"
    ] == "execute_helper→execute_helper"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_advanced"
    ] is False
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_line"
    ].startswith(
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call "
    )
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_helper"
    ] == (
        "package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow"
    )
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_action"
    ] == "execute_now"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_mode"
    ] == "execute_helper"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_call_execute"
    ] is True
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_ready"
    ] is True
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_line"
    ].startswith(
        "continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow "
    )
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_helper"
    ] == (
        "package_reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin"
    )
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_action"
    ] == "execute_now"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_mode"
    ] == "execute_helper"
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_execute"
    ] is True
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_ready"
    ] is True
    assert pipeline["operator_state"][
        "reverse_flow_focused_validation_continue_cascade_wake_route_apply_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_call_next_call_follow_pin_line"
    ].startswith(
    ...
    ] == pipeline["reverse_flow_focused_validation_pending_work_units"]
    ...
    assert focused["focused_validation"]["recorded_command_hashes"] == []
    assert focused["focused_validation"]["recorded_command_hash_count"] == 0
    assert focused["focused_validation"]["pending_commands"] == list(
        focused["focused_validation"]["commands"]
    )
    ...
    assert focused["continue_plan"]["pending_work_units"] == [
        {
            "command": command,
            "command_hash": command_hash,
            "inventory_index": index,
    ...
    assert focused["continue_plan"]["pending_work_unit_count"] == len(
        focused["focused_validation"]["commands"]
    )
    assert focused["focused_validation"]["pending_work_units"] == focused["continue_plan"][
        "pending_work_units"
    ]
    ...
    assert acceptance_blocked["controller_surface"] == (
        "skill_route_discovery_focused_validation_activation_external_acceptance"
    )
    assert acceptance_blocked["proposal_track"] == (
        "prop-skill-reverse-flow-focused-test-validation"
    )
    assert acceptance_blocked["status"] == (
        "blocked_until_activation_external_handoff_ready"
    )
    assert acceptance_blocked["decision"] == (
        "hold_activation_external_acceptance_until_handoff_ready"
    )
    assert acceptance_blocked["supervisor_next_action"] == (
        "run_focused_local_test_validation_then_keep_activation_external"
    )
    assert acceptance_blocked["activation_external_only"] is True
    assert acceptance_blocked["supervisor_activation_allowed"] is False
    assert acceptance_blocked["runtime_action"] == "none"
    assert acceptance_blocked["push_or_promotion_allowed"] is False
    assert acceptance_blocked["kernel_restart_allowed"] is False
    ...
    assert residual_blocked["controller_surface"] == (
        "skill_route_discovery_focused_validation_residual_adjacent_queue"
    )
    residual_apply_blocked = pipeline["residual_adjacent_harness_eval_local_apply"]
    assert residual_apply_blocked["controller_surface"] == (
        "skill_route_discovery_residual_adjacent_harness_eval_local_apply"
    )
    assert residual_apply_blocked["status"] == (
        "blocked_until_residual_adjacent_queue_ready"
    )
    assert residual_apply_blocked["decision"] == (
        "hold_residual_adjacent_harness_eval_local_apply_until_queue_ready"
    )
    assert residual_apply_blocked["activation_external_only"] is True
    ...
    assert residual_comparison_blocked["controller_surface"] == (
        "skill_route_discovery_residual_adjacent_harness_eval_local_comparison"
    )
    assert residual_comparison_blocked["status"] == (
        "blocked_until_residual_adjacent_harness_eval_local_apply_ready"
    )
    assert residual_comparison_blocked["decision"] == (
        "hold_residual_adjacent_harness_eval_local_comparison_until_local_apply_ready"
    )
    assert residual_comparison_blocked["unlocked_local_lanes"] == []
    assert residual_comparison_blocked["skill_route_unlocked_local_lanes"] == []
    assert residual_comparison_blocked["skill_route_discovery_inherited"] is False
    assert residual_comparison_blocked["local_comparison_passed"] is False
    assert residual_comparison_blocked["activation_external_only"] is True
    assert residual_comparison_blocked["supervisor_activation_allowed"] is False
    residual_unlocked_lane_blocked = pipeline[
        "residual_adjacent_unlocked_local_lane_apply"
    ]
    assert residual_unlocked_lane_blocked["controller_surface"] == (
        "skill_route_discovery_residual_adjacent_unlocked_local_lane_apply"
    )
    assert residual_unlocked_lane_blocked["status"] == (
        "blocked_until_residual_adjacent_harness_local_comparison_ready"
    )
    assert residual_unlocked_lane_blocked["decision"] == (
        "hold_residual_adjacent_unlocked_local_lane_apply_until_harness_local_comparison_ready"
    )
    assert residual_unlocked_lane_blocked["unlocked_local_lanes"] == []
    assert residual_unlocked_lane_blocked["skill_route_unlocked_local_lanes"] == []
    assert residual_unlocked_lane_blocked["skill_route_discovery_inherited"] is False
    assert residual_unlocked_lane_blocked["selected_local_lane"] == "none"
    assert residual_unlocked_lane_blocked["activation_external_only"] is True
    assert residual_unlocked_lane_blocked["supervisor_activation_allowed"] is False
    assert residual_blocked["proposal_track"] == (
    ...
    assert residual_blocked["decision"] == (
        "hold_residual_adjacent_queue_until_activation_external_acceptance"
    )
    assert residual_blocked["supervisor_next_action"] == (
        "run_focused_local_test_validation_then_keep_activation_external"
    )
    assert residual_blocked["activation_external_only"] is True
    assert residual_blocked["supervisor_activation_allowed"] is False
    assert residual_blocked["runtime_action"] == "none"
    assert residual_blocked["push_or_promotion_allowed"] is False
    assert residual_blocked["kernel_restart_allowed"] is False
    assert residual_blocked["external_skill_execution_allowed"] is False
    assert residual_blocked["provider_launch_allowed"] is False
    assert residual_blocked["remote_apply_allowed"] is False
    ...
    # Residual fortress selection is computed for residual-active readiness but not
    # exported while residual stages are only reverse-flow-waiting.
    assert residual_apply_blocked["selected_residual_proposal_id"] == ""
    assert residual_apply_blocked["proposal_id"] == ""
    assert residual_apply_blocked["residual_selection_held_until_residual_active"] is True
    assert residual_comparison_blocked["selected_residual_proposal_id"] == ""
    assert residual_unlocked_lane_blocked["selected_residual_proposal_id"] == ""
    residual_focused_blocked = pipeline["residual_adjacent_focused_local_validation"]
    assert residual_focused_blocked["selected_residual_proposal_id"] == ""
    assert residual_focused_blocked["proposal_id"] == ""
    assert residual_handoff_blocked["selected_residual_proposal_id"] == ""
    assert residual_handoff_blocked["proposal_id"] == ""
    assert residual_accept_blocked["selected_residual_proposal_id"] == ""
    assert residual_accept_blocked["proposal_id"] == ""
    pipeline_rendered = "\n".join(
        github_growth.render_skill_route_discovery_capability_pipeline_lines(pipeline)
    )
    assert (
    ...
    # Residual packets themselves must not pre-advertise fortress while reverse-flow waits.
    assert "prop-harness-fortress-local-eval" not in pipeline_rendered.split(
        "Residual adjacent selected proposal:"
    )[0]
    assert (
        "While reverse-flow focused validation is ready/unrecorded or failed, residual fortress/Hy3 stages stay held"
        in pipeline_rendered
    )
    assert (
    ...
    assert deferred["focused_validation_activation_external_acceptance"]["status"] in {
        "blocked_until_activation_external_handoff_ready",
        "not_applicable",
    }
    assert deferred["focused_validation_residual_adjacent_queue"]["status"] in {
        "blocked_until_activation_external_acceptance",
        "not_applicable",
    }
    ...
    # supervisor_next promotes to record-remaining (not full re-run) while partial.
    ...
    assert partial["reverse_flow_focused_validation_recorded_command_hashes"] == [
        command_hashes[0]
    ]
    assert partial["reverse_flow_focused_validation_recorded_command_hash_count"] == 1
    ...
    assert partial["reverse_flow_focused_validation_pending_work_unit_count"] == (
        len(command_hashes) - 1
    )
    assert partial["reverse_flow_focused_validation_pending_work_units"] == partial[
        "focused_local_test_validation"
    ]["continue_plan"]["pending_work_units"]
    assert partial["focused_local_test_validation"]["continue_plan"][
        "supervisor_next_action"
    ] == (
        "record_remaining_reverse_flow_focused_validation_command_hashes_then_keep_activation_external"
    ...
    assert partial["operator_state"]["supervisor_next_action"] == (
        partial["supervisor_next_action"]
    )
    ...
    # Blocked reverse-flow-waiting handoff/acceptance also promote record-remaining.
    assert partial["focused_validation_activation_external_handoff"][
        "supervisor_next_action"
    ] == (
        "record_remaining_reverse_flow_focused_validation_command_hashes_then_keep_activation_external"
    )
    assert partial["focused_validation_activation_external_acceptance"][
        "supervisor_next_action"
    ] == (
        "record_remaining_reverse_flow_focused_validation_command_hashes_then_keep_activation_external"
    )
    assert partial["focused_validation_residual_adjacent_queue"][
        "supervisor_next_action"
    ] == (
        "record_remaining_reverse_flow_focused_validation_command_hashes_then_keep_activation_external"
    )
    ...
    partial_rendered = "\n".join(
        github_growth.render_skill_route_discovery_capability_pipeline_lines(partial)
    )
    assert (
        "Supervisor next action: `record_remaining_reverse_flow_focused_validation_command_hashes_then_keep_activation_external`"
        in partial_rendered
    )
    assert (
    ...
    assert merged_partial["focused_local_test_validation"]["decision"] == (
        "record_remaining_focused_validation_command_hashes_before_activation_external"
    )
    assert merged_partial["supervisor_next_action"] == (
        "record_remaining_reverse_flow_focused_validation_command_hashes_then_keep_activation_external"
    )
    ...
    assert merged_partial["focused_local_test_validation"]["focused_validation"][
        "pending_commands"
    ] == [focused["focused_validation"]["commands"][2]]
    assert merged_partial["reverse_flow_focused_validation_missing_command_hash_count"] == 1
    assert merged_partial["reverse_flow_focused_validation_pending_command_count"] == 1
    ...
    assert recorded_skill_route_discovery_focused_validation_command_hashes(
        expected_command_hashes=command_hashes,
        command_results=[{"command_hash": command_hashes[0], "passed": True}],
    ) == [command_hashes[0]]
    assert pending_skill_route_discovery_focused_validation_commands(
        commands=focused["focused_validation"]["commands"],
        command_hashes=command_hashes,
        missing_command_hashes=command_hashes[1:],
    ) == focused["focused_validation"]["commands"][1:]
    assert pending_skill_route_discovery_focused_validation_work_units(
        commands=focused["focused_validation"]["commands"],
        command_hashes=command_hashes,
        missing_command_hashes=command_hashes[1:],
    ) == [
        {
            "command": focused["focused_validation"]["commands"][1],
            "command_hash": command_hashes[1],
            "inventory_index": 1,
        },
        {
            "command": focused["focused_validation"]["commands"][2],
            "command_hash": command_hashes[2],
            "inventory_index": 2,
        },
    ]
    # Continue-plan materializer builds body-free rows for pending units only.
    assert materialize_reverse_flow_focused_validation_continue_record_rows(
        continue_plan=partial["focused_local_test_validation"]["continue_plan"],
        outcomes={command_hashes[1]: True, command_hashes[2]: False},
    ) == [
        {
            "command_hash": command_hashes[1],
            "passed": True,
    ...
    # Integration seam: record continue outcomes from pending work units only.
    continue_partial = record_reverse_flow_focused_validation_continue_outcomes(
        pipeline,
        {command_hashes[0]: True},
        source_digest="github-growth-20260713T053123.445910Z",
    )
    assert continue_partial["focused_local_test_validation"]["status"] == "ready"
    assert continue_partial["focused_local_test_validation"]["focused_validation"][
        "recorded_command_hashes"
    ] == [command_hashes[0]]
    assert continue_partial["reverse_flow_focused_validation_continue_plan_mode"] == (
        "record_remaining"
    )
    assert continue_partial["residual_export_allowed"] is False
    ...
    assert merged_pass["focused_local_test_validation"]["continue_plan"][
        "pending_work_unit_count"
    ] == 0
    assert merged_pass["reverse_flow_focused_validation_pending_work_units"] == []
    assert merged_pass["reverse_flow_focused_validation_continue_plan_mode"] == (
        "keep_activation_external"
    )
    # Closing remaining pending units through continue-outcomes helper also passes.
    continue_pass = record_reverse_flow_focused_validation_continue_outcomes(
        continue_partial,
        {hash_item: True for hash_item in command_hashes[1:]},
        source_digest="github-growth-20260713T053123.445910Z",
    )
    assert continue_pass["focused_local_test_validation"]["status"] == "passed"
    assert continue_pass["focused_local_test_validation"]["continue_plan"]["mode"] == (
        "keep_activation_external"
    )
    assert continue_pass["reverse_flow_focused_validation_pending_work_unit_count"] == 0
    ...
    # Durable follow-through collapses action + execute_recommended.
    ...
    assert follow_through["controller_surface"] == (
        "reverse_flow_focused_validation_continue_dispatch_follow_through"
    )
    assert follow_through["follow_through_action"] == "execute_now"
    assert follow_through["call_dispatch_with_execute"] is True
    assert follow_through["inventory_action"] == "inventory_only"
    assert follow_through["execute_recommended"] is True
    assert follow_through["residual_export_allowed"] is False
    assert follow_through["supervisor_activation_allowed"] is False
    assert follow_through["kernel_restart_allowed"] is False
    assert follow_through["raw_command_stdout_exported"] is False
    assert follow_through["follow_through_helper"] == (
        "follow_reverse_flow_focused_validation_continue_dispatch"
    )
    assert follow_through["resolve_helper"] == (
        "resolve_reverse_flow_focused_validation_continue_dispatch_follow_through"
    )
    ...
    assert inventory_dispatch["follow_through_action"] == "execute_now"
    ...
    # Inventory-only finish receipt stays incomplete until reverse-flow pass.
    assert inventory_dispatch["finish_receipt"]["controller_surface"] == (
        "reverse_flow_focused_validation_continue_finish_receipt"
    )
    assert inventory_dispatch["continue_finished"] is False
    assert inventory_dispatch["residual_queue_ready"] is False
    assert inventory_dispatch["finish_line"].startswith("finish complete=false ")
    ...
    # Inventory-only residual entry stays blocked; selected residual held empty.
    assert inventory_dispatch["residual_entry"]["controller_surface"] == (
        "reverse_flow_focused_validation_continue_residual_entry"
    )
    assert inventory_dispatch["residual_entry_ready"] is False
    assert inventory_dispatch["selected_residual_proposal_id"] == ""
    assert inventory_dispatch["residual_entry_line"].startswith(
        "residual_entry ready=false "
    )
    assert "selected=none" in inventory_dispatch["residual_entry_line"]
    assert "residual_export=false" in inventory_dispatch["residual_entry_line"]
    assert inventory_dispatch["residual_entry"]["residual_export_allowed"] is False
    assert inventory_dispatch["residual_entry"]["supervisor_activation_allowed"] is False
    assert inventory_dispatch["residual_entry"]["kernel_restart_allowed"] is False
    # Inventory-only residual follow stays wait_for_reverse_flow; comparison denied.
    assert inventory_dispatch["residual_follow"]["controller_surface"] == (
        "reverse_flow_focused_validation_continue_residual_follow"
    )
    assert inventory_dispatch["residual_follow_ready"] is False
    assert inventory_dispatch["residual_follow_action"] == "wait_for_reverse_flow"
    assert inventory_dispatch["call_residual_comparison"] is False
    assert inventory_dispatch["residual_follow_line"].startswith(
        "residual_follow ready=false "
    )
    assert "action=wait_for_reverse_flow" in inventory_dispatch["residual_follow_line"]
    assert "call_comparison=false" in inventory_dispatch["residual_follow_line"]
    assert "residual_export=false" in inventory_dispatch["residual_follow_line"]
    ...
    # Inventory-only residual unlocked apply stays wait_for_reverse_flow; focused
    # validation call denied.
    assert inventory_dispatch["residual_unlocked_apply"]["controller_surface"] == (
        "reverse_flow_focused_validation_continue_residual_unlocked_apply"
    )
    assert inventory_dispatch["residual_unlocked_apply_ready"] is False
    assert inventory_dispatch["residual_unlocked_apply_action"] == (
        "wait_for_reverse_flow"
    )
    assert inventory_dispatch["call_residual_focused_validation"] is False
    assert inventory_dispatch["residual_unlocked_apply_line"].startswith(
        "residual_unlocked_apply ready=false "
    )
    assert "action=wait_for_reverse_flow" in inventory_dispatch[
        "residual_unlocked_apply_line"
    ]
    assert "call_focused_validation=false" in inventory_dispatch[
        "residual_unlocked_apply_line"
    ]
    assert "residual_export=false" in inventory_dispatch["residual_unlocked_apply_line"]
    assert inventory_dispatch["residual_unlocked_apply"][
        "residual_export_allowed"
    ] is False
    assert inventory_dispatch["residual_unlocked_apply"][
        "supervisor_activation_allowed"
    ...
    # Inventory-only residual focused validation stays wait_for_reverse_flow;
    # residual handoff call denied.
    assert inventory_dispatch["residual_focused_validation"]["controller_surface"] == (
        "reverse_flow_focused_validation_continue_residual_focused_validation"
    )
    assert inventory_dispatch["residual_focused_validation_ready"] is False
    assert inventory_dispatch["residual_focused_validation_action"] == (
        "wait_for_reverse_flow"
    )
    assert inventory_dispatch["call_residual_handoff"] is False
    assert inventory_dispatch["residual_focused_validation_line"].startswith(
        "residual_focused_validation ready=false "
    )
    assert "action=wait_for_reverse_flow" in inventory_dispatch[
        "residual_focused_validation_line"
    ]
    assert "call_handoff=false" in inventory_dispatch[
        "residual_focused_validation_line"
    ]
    assert "residual_export=false" in inventory_dispatch[
        "residual_focused_validation_line"
    ]
    assert inventory_dispatch["residual_focused_validation"][
        "residual_export_allowed"
    ] is False
    assert inventory_dispatch["residual_focused_validation"][
        "supervisor_activation_allowed"
    ...
    # Inventory-only residual handoff stays wait_for_reverse_flow;
    # residual acceptance call denied.
    assert inventory_dispatch["residual_handoff"]["controller_surface"] == (
        "reverse_flow_focused_validation_continue_residual_handoff"
    )
    assert inventory_dispatch["residual_handoff_ready"] is False
    assert inventory_dispatch["residual_handoff_action"] == (
        "wait_for_reverse_flow"
    )
    assert inventory_dispatch["call_residual_acceptance"] is False
    assert inventory_dispatch["residual_handoff_line"].startswith(
        "residual_handoff ready=false "
    )
    assert "action=wait_for_reverse_flow" in inventory_dispatch[
        "residual_handoff_line"
    ]
    ...
    # Inventory-only continue cascade unifies reverse 0/3 + residual 0/8;
    # reverse-flow-first action is execute_now; residual export denied.
    assert inventory_dispatch["continue_cascade"]["controller_surface"] == (
        "reverse_flow_focused_validation_continue_cascade"
    )
    assert inventory_dispatch["continue_cascade_ready"] is False
    assert inventory_dispatch["continue_cascade_action"] == "execute_now"
    assert inventory_dispatch["continue_cascade_reverse_progress_label"] == "0/3"
    assert inventory_dispatch["continue_cascade_residual_progress_label"] == "0/8"
    assert inventory_dispatch["continue_cascade_residual_blocked_at"] == "open"
    assert inventory_dispatch["continue_cascade_line"].startswith(
        "continue_cascade ready=false "
    )
    assert "reverse_progress=0/3" in inventory_dispatch["continue_cascade_line"]
    assert "residual_progress=0/8" in inventory_dispatch["continue_cascade_line"]
    assert "residual_blocked_at=open" in inventory_dispatch["continue_cascade_line"]
    assert "action=execute_now" in inventory_dispatch["continue_cascade_line"]
    assert "residual_export=false" in inventory_dispatch["continue_cascade_line"]
    assert inventory_dispatch["continue_cascade"][
        "residual_export_allowed"
    ] is False
    assert inventory_dispatch["continue_cascade"][
        "supervisor_activation_allowed"
    ...
    assert "package_reverse_flow_focused_validation_continue_cascade" in (
    ...
    assert "package_reverse_flow_focused_validation_continue_residual_follow" in (
        inventory_dispatch["record_helpers"]
    )
    assert "package_reverse_flow_focused_validation_continue_residual_entry" in (
        inventory_dispatch["record_helpers"]
    )
    ...
    assert "resolve_reverse_flow_focused_validation_continue_dispatch_follow_through" in (
        inventory_dispatch["record_helpers"]
    )
    ...
    assert full_dispatch["follow_through_action"] == "execute_now"
    assert full_dispatch["call_dispatch_with_execute"] is True
    ...
    # Finish receipt: reverse-flow continue complete + residual queue ready signal.
    assert full_dispatch["finish_receipt"]["controller_surface"] == (
        "reverse_flow_focused_validation_continue_finish_receipt"
    )
    assert full_dispatch["continue_finished"] is True
    assert full_dispatch["residual_queue_ready"] is True
    assert full_dispatch["finish_receipt"]["progress_complete"] is True
    assert full_dispatch["finish_receipt"]["focused_validation_status"] == "passed"
    assert full_dispatch["finish_receipt"][
        "activation_external_handoff_status"
    ] == "ready"
    assert full_dispatch["finish_receipt"][
        "activation_external_acceptance_status"
    ] == "accepted"
    assert full_dispatch["finish_receipt"]["residual_export_allowed"] is False
    assert full_dispatch["finish_receipt"]["supervisor_activation_allowed"] is False
    assert full_dispatch["finish_receipt"]["kernel_restart_allowed"] is False
    assert full_dispatch["finish_receipt"]["raw_command_stdout_exported"] is False
    assert full_dispatch["finish_line"].startswith("finish complete=true ")
    assert f"progress={n}/{n}" in full_dispatch["finish_line"]
    assert "status=passed" in full_dispatch["finish_line"]
    assert "handoff=ready" in full_dispatch["finish_line"]
    ...
    # Residual entry collapses residual open + residual apply selection.
    assert full_dispatch["residual_entry"]["controller_surface"] == (
        "reverse_flow_focused_validation_continue_residual_entry"
    )
    assert full_dispatch["residual_entry_ready"] is True
    assert full_dispatch["selected_residual_proposal_id"]
    assert "fortress" in full_dispatch["selected_residual_proposal_id"].lower() or (
        "prop-harness-fortress-local-eval"
        in full_dispatch["selected_residual_proposal_id"]
    )
    assert full_dispatch["residual_entry"]["residual_apply_status"] == "ready"
    assert full_dispatch["residual_entry"]["residual_export_allowed"] is False
    assert full_dispatch["residual_entry"]["supervisor_activation_allowed"] is False
    assert full_dispatch["residual_entry"]["kernel_restart_allowed"] is False
    assert full_dispatch["residual_entry"]["raw_command_stdout_exported"] is False
    assert full_dispatch["residual_entry_line"].startswith("residual_entry ready=true ")
    assert "residual_export=false" in full_dispatch["residual_entry_line"]
    assert (
        "run_agent_harness_eval_local_comparison_for_residual_adjacent_row"
        in full_dispatch["residual_entry_line"]
        or "build_skill_route_discovery_residual_adjacent_harness_eval_local_apply"
        in full_dispatch["residual_entry_line"]
    )
    # Residual follow collapses residual entry into residual comparison policy.
    assert full_dispatch["residual_follow"]["controller_surface"] == (
        "reverse_flow_focused_validation_continue_residual_follow"
    )
    assert full_dispatch["residual_follow_ready"] is True
    assert full_dispatch["residual_follow_action"] == (
        "open_residual_harness_eval_local_comparison"
    )
    assert full_dispatch["call_residual_comparison"] is True
    assert full_dispatch["residual_follow"]["selected_residual_proposal_id"] == (
        full_dispatch["selected_residual_proposal_id"]
    )
    assert full_dispatch["residual_follow"]["residual_export_allowed"] is False
    assert full_dispatch["residual_follow"]["supervisor_activation_allowed"] is False
    assert full_dispatch["residual_follow"]["kernel_restart_allowed"] is False
    assert full_dispatch["residual_follow"]["raw_command_stdout_exported"] is False
    assert full_dispatch["residual_follow_line"].startswith(
        "residual_follow ready=true "
    )
    assert "action=open_residual_harness_eval_local_comparison" in (
        full_dispatch["residual_follow_line"]
    )
    assert "call_comparison=true" in full_dispatch["residual_follow_line"]
    assert "residual_export=false" in full_dispatch["residual_follow_line"]
    assert (
        "build_skill_route_discovery_residual_adjacent_harness_eval_local_comparison"
    ...
    assert full_dispatch["residual_unlocked_apply"]["controller_surface"] == (
        "reverse_flow_focused_validation_continue_residual_unlocked_apply"
    )
    assert full_dispatch["residual_unlocked_apply_ready"] is True
    assert full_dispatch["residual_unlocked_apply_action"] == (
        "open_residual_focused_local_validation"
    )
    assert full_dispatch["call_residual_focused_validation"] is True
    assert full_dispatch["residual_unlocked_apply"][
        "selected_residual_proposal_id"
    ] == full_dispatch["selected_residual_proposal_id"]
    assert full_dispatch["residual_unlocked_apply"]["selected_local_lane"] in {
        "test",
        "documentation",
        "code_patch",
    }
    assert full_dispatch["residual_unlocked_apply"]["unlocked_local_lanes"]
    assert full_dispatch["residual_unlocked_apply"]["residual_export_allowed"] is False
    assert full_dispatch["residual_unlocked_apply"][
        "supervisor_activation_allowed"
    ] is False
    assert full_dispatch["residual_unlocked_apply"]["kernel_restart_allowed"] is False
    assert full_dispatch["residual_unlocked_apply"][
        "raw_command_stdout_exported"
    ] is False
    assert full_dispatch["residual_unlocked_apply_line"].startswith(
        "residual_unlocked_apply ready=true "
    )
    assert "action=open_residual_focused_local_validation" in (
        full_dispatch["residual_unlocked_apply_line"]
    )
    assert "call_focused_validation=true" in full_dispatch[
        "residual_unlocked_apply_line"
    ]
    assert "comparison_ready=true" in full_dispatch["residual_unlocked_apply_line"]
    assert "residual_export=false" in full_dispatch["residual_unlocked_apply_line"]
    assert (
        "build_skill_route_discovery_residual_adjacent_unlocked_local_lane_apply"
        in full_dispatch["residual_unlocked_apply_line"]
        or "run_focused_local_validation_for_residual_adjacent_unlocked_lane"
        in full_dispatch["residual_unlocked_apply_line"]
    )
    ...
    # Standalone residual unlocked apply package matches dispatch attachment.
    standalone_residual_unlocked_apply = (
        package_reverse_flow_focused_validation_continue_residual_unlocked_apply(
            pipeline=full_dispatch["pipeline"],
    ...
    assert standalone_residual_unlocked_apply["controller_surface"] == (
        "reverse_flow_focused_validation_continue_residual_unlocked_apply"
    )
    assert standalone_residual_unlocked_apply["residual_unlocked_apply"] is True
    assert standalone_residual_unlocked_apply[
        "call_residual_focused_validation"
    ] is True
    assert standalone_residual_unlocked_apply["residual_export_allowed"] is False
    # Residual focused validation collapses residual unlocked apply into
    # body-free command-hash progress + handoff policy without residual_export.
    assert full_dispatch["residual_focused_validation"]["controller_surface"] == (
        "reverse_flow_focused_validation_continue_residual_focused_validation"
    )
    assert full_dispatch["residual_focused_validation_ready"] is True
    assert full_dispatch["residual_focused_validation_action"] == (
        "run_residual_focused_validation"
    )
    assert full_dispatch["call_residual_handoff"] is False
    assert full_dispatch["residual_focused_validation"][
        "selected_residual_proposal_id"
    ] == full_dispatch["selected_residual_proposal_id"]
    assert full_dispatch["residual_focused_validation"]["selected_local_lane"] in {
        "test",
        "documentation",
        "code_patch",
    }
    ...
    assert full_dispatch["residual_focused_validation_line"].startswith(
        "residual_focused_validation ready=true "
    )
    assert "action=run_residual_focused_validation" in (
        full_dispatch["residual_focused_validation_line"]
    )
    assert "call_handoff=false" in full_dispatch["residual_focused_validation_line"]
    assert "unlocked_apply_ready=true" in full_dispatch[
        "residual_focused_validation_line"
    ]
    assert "residual_export=false" in full_dispatch[
        "residual_focused_validation_line"
    ]
    assert (
        "build_skill_route_discovery_residual_adjacent_focused_local_validation"
        in full_dispatch["residual_focused_validation_line"]
        or "run_focused_local_validation_for_residual_adjacent_unlocked_lane"
        in full_dispatch["residual_focused_validation_line"]
    )
    # Standalone residual focused validation package matches dispatch attachment.
    standalone_residual_focused_validation = (
        package_reverse_flow_focused_validation_continue_residual_focused_validation(
            pipeline=full_dispatch["pipeline"],
    ...
    assert standalone_residual_focused_validation["controller_surface"] == (
        "reverse_flow_focused_validation_continue_residual_focused_validation"
    )
    assert standalone_residual_focused_validation[
        "residual_focused_validation"
    ] is True
    assert standalone_residual_focused_validation["call_residual_handoff"] is False
    assert standalone_residual_focused_validation[
        "residual_export_allowed"
    ] is False
    # Residual handoff collapses residual focused validation into activation-
    # external handoff readiness + acceptance policy without residual_export.
    # While residual focused validation is ready/unrecorded, handoff waits.
    assert full_dispatch["residual_handoff"]["controller_surface"] == (
        "reverse_flow_focused_validation_continue_residual_handoff"
    )
    assert full_dispatch["residual_handoff_ready"] is False
    assert full_dispatch["residual_handoff_action"] == (
        "wait_for_residual_focused_validation"
    )
    assert full_dispatch["call_residual_acceptance"] is False
    assert full_dispatch["residual_handoff"][
        "residual_focused_validation_ready"
    ] is True
    assert full_dispatch["residual_handoff"][
        "residual_export_allowed"
    ] is False
    ...
    assert full_dispatch["residual_handoff_line"].startswith(
        "residual_handoff ready=false "
    )
    assert "action=wait_for_residual_focused_validation" in (
        full_dispatch["residual_handoff_line"]
    )
    assert "call_acceptance=false" in full_dispatch["residual_handoff_line"]
    assert "focused_ready=true" in full_dispatch["residual_handoff_line"]
    assert "residual_export=false" in full_dispatch["residual_handoff_line"]
    # Standalone residual handoff package matches dispatch attachment.
    standalone_residual_handoff = (
        package_reverse_flow_focused_validation_continue_residual_handoff(
            pipeline=full_dispatch["pipeline"],
    ...
    assert standalone_residual_handoff["controller_surface"] == (
        "reverse_flow_focused_validation_continue_residual_handoff"
    )
    ...
    # Continue cascade unifies reverse progress (complete) with residual
    # cascade progress (blocked at handoff) without residual_export.
    assert full_dispatch["continue_cascade"]["controller_surface"] == (
        "reverse_flow_focused_validation_continue_cascade"
    )
    assert full_dispatch["continue_cascade_ready"] is False
    assert full_dispatch["continue_cascade_action"] == (
        "wait_for_residual_handoff"
    )
    assert full_dispatch["continue_cascade_reverse_progress_label"] == f"{n}/{n}"
    assert full_dispatch["continue_cascade_residual_blocked_at"] == "handoff"
    assert full_dispatch["continue_cascade"][
        "residual_export_allowed"
    ] is False
    assert full_dispatch["continue_cascade"][
        "supervisor_activation_allowed"
    ...
    assert full_dispatch["continue_cascade_line"].startswith(
        "continue_cascade ready=false "
    )
    assert f"reverse_progress={n}/{n}" in full_dispatch["continue_cascade_line"]
    assert "residual_blocked_at=handoff" in full_dispatch["continue_cascade_line"]
    assert "action=wait_for_residual_handoff" in (
        full_dispatch["continue_cascade_line"]
    )
    assert "residual_export=false" in full_dispatch["continue_cascade_line"]
    standalone_continue_cascade = (
        package_reverse_flow_focused_validation_continue_cascade(
            pipeline=full_dispatch["pipeline"],
            operator_card=full_dispatch["post_operator_card"],
            residual_cascade=full_dispatch["residual_cascade"],
            follow_through=full_dispatch["post_follow_through"],
            finish_receipt=full_dispatch["finish_receipt"],
        )
    )
    assert standalone_continue_cascade["controller_surface"] == (
        "reverse_flow_focused_validation_continue_cascade"
    )
    assert standalone_continue_cascade["continue_cascade"] is False
    assert standalone_continue_cascade["continue_cascade_action"] == (
        "wait_for_residual_handoff"
    )
    assert standalone_continue_cascade["residual_cascade_blocked_at"] == "handoff"
    assert standalone_continue_cascade[
        "residual_export_allowed"
    ] is False
    ...
    standalone_finish = package_reverse_flow_focused_validation_continue_finish_receipt(
        pipeline=full_dispatch["pipeline"],
        post_operator_card=full_dispatch["post_operator_card"],
        progress_transition=full_dispatch["progress_transition"],
        exec_receipt=full_dispatch["exec_receipt"],
        supervisor_wake=full_dispatch["supervisor_wake"],
        executed=True,
        recorded=True,
    )
    assert standalone_finish["controller_surface"] == (
        "reverse_flow_focused_validation_continue_finish_receipt"
    )
    assert standalone_finish["continue_finished"] is True
    assert standalone_finish["residual_queue_ready"] is True
    assert standalone_finish["residual_export_allowed"] is False
    ...
    standalone_residual_entry = (
        package_reverse_flow_focused_validation_continue_residual_entry(
            pipeline=full_dispatch["pipeline"],
            residual_open=standalone_residual_open,
        )
    )
    assert standalone_residual_entry["controller_surface"] == (
        "reverse_flow_focused_validation_continue_residual_entry"
    )
    assert standalone_residual_entry["residual_entry"] is True
    assert standalone_residual_entry["selected_residual_proposal_id"]
    assert standalone_residual_entry["residual_export_allowed"] is False
    assert standalone_residual_entry["supervisor_activation_allowed"] is False
    assert standalone_residual_entry["kernel_restart_allowed"] is False
    assert standalone_residual_entry["residual_entry_line"].startswith(
        "residual_entry ready=true "
    )
    standalone_residual_follow = (
        package_reverse_flow_focused_validation_continue_residual_follow(
            pipeline=full_dispatch["pipeline"],
            residual_entry=standalone_residual_entry,
            residual_open=standalone_residual_open,
        )
    )
    assert standalone_residual_follow["controller_surface"] == (
        "reverse_flow_focused_validation_continue_residual_follow"
    )
    assert standalone_residual_follow["residual_follow"] is True
    assert standalone_residual_follow["residual_follow_action"] == (
        "open_residual_harness_eval_local_comparison"
    )
    assert standalone_residual_follow["call_residual_comparison"] is True
    assert standalone_residual_follow["selected_residual_proposal_id"]
    assert standalone_residual_follow["residual_export_allowed"] is False
    assert standalone_residual_follow["supervisor_activation_allowed"] is False
    assert standalone_residual_follow["kernel_restart_allowed"] is False
    assert standalone_residual_follow["residual_follow_line"].startswith(
        "residual_follow ready=true "
    )
    ...
    follow_packet = follow_reverse_flow_focused_validation_continue_dispatch(
        pipeline,
        command_runner=_fake_runner,
        cwd=".",
        source_digest="github-growth-20260713T071123.677935Z",
    )
    assert follow_packet["controller_surface"] == (
        "reverse_flow_focused_validation_continue_dispatch_follow_through"
    )
    assert follow_packet["followed_recommendation"] is True
    assert follow_packet["follow_through_action"] == "execute_now"
    assert follow_packet["call_dispatch_with_execute"] is True
    assert follow_packet["action"] == "run_and_record"
    assert follow_packet["executed"] is True
    assert follow_packet["recorded"] is True
    assert follow_packet["pipeline"]["focused_local_test_validation"]["status"] == "passed"
    assert follow_packet["supervisor_next_action"] == (
        "keep_activation_external_after_focused_local_test_validation"
    )
    ...
    assert follow_packet["finish_receipt"]["continue_finished"] is True
    assert follow_packet["continue_finished"] is True
    assert follow_packet["residual_queue_ready"] is True
    ...
    assert follow_packet["residual_entry_ready"] is True
    assert follow_packet["selected_residual_proposal_id"]
    assert follow_packet["residual_entry_line"].startswith("residual_entry ready=true ")
    assert "residual_export=false" in follow_packet["residual_entry_line"]
    assert follow_packet["residual_entry"]["residual_export_allowed"] is False
    assert follow_packet["residual_follow_ready"] is True
    assert follow_packet["residual_follow_action"] == (
        "open_residual_harness_eval_local_comparison"
    )
    assert follow_packet["call_residual_comparison"] is True
    assert follow_packet["residual_follow_line"].startswith(
        "residual_follow ready=true "
    )
    assert "action=open_residual_harness_eval_local_comparison" in (
        follow_packet["residual_follow_line"]
    )
    assert "call_comparison=true" in follow_packet["residual_follow_line"]
    assert "residual_export=false" in follow_packet["residual_follow_line"]
    assert follow_packet["residual_follow"]["residual_export_allowed"] is False
    ...
    assert follow_packet["raw_command_stdout_exported"] is False
    # Inventory-only follow (execute=False) keeps execute_now recommendation legible.
    follow_inventory = follow_reverse_flow_focused_validation_continue_dispatch(
        pipeline,
        execute=False,
    )
    assert follow_inventory["executed"] is False
    assert follow_inventory["follow_through_action"] == "execute_now"
    assert follow_inventory["call_dispatch_with_execute"] is False
    assert follow_inventory["action"] == "inventory_only"
    assert follow_inventory["followed_recommendation"] is False
    assert follow_inventory["residual_export_allowed"] is False
    ...
    assert partial_dispatch["follow_through_action"] == "execute_now"
    assert partial_dispatch["post_follow_through_action"] == "keep_activation_external"
    # Policy-aware follow-through on partial also runs remaining only.
    partial_follow = follow_reverse_flow_focused_validation_continue_dispatch(
        continue_partial,
        command_runner=_fake_runner,
        cwd=".",
        source_digest="github-growth-20260713T071123.677935Z",
    )
    assert partial_follow["action"] == "run_and_record"
    assert partial_follow["run_plan"]["mode"] == "record_remaining"
    assert partial_follow["run_result"]["ran_count"] == len(command_hashes) - 1
    assert partial_follow["post_follow_through_action"] == "keep_activation_external"
    assert partial_follow["residual_export_allowed"] is False
    ...
    assert post_pass_dispatch["follow_through_action"] == "keep_activation_external"
    assert post_pass_dispatch["call_dispatch_with_execute"] is False
    ...
    # Policy-aware follow after pass does not re-run units.
    post_pass_follow = follow_reverse_flow_focused_validation_continue_dispatch(
        full_dispatch["pipeline"],
    )
    assert post_pass_follow["executed"] is False
    assert post_pass_follow["follow_through_action"] == "keep_activation_external"
    assert post_pass_follow["call_dispatch_with_execute"] is False
    assert post_pass_follow["action"] == "keep_activation_external"
    assert post_pass_follow["residual_export_allowed"] is False
    assert post_pass_follow["continue_finished"] is True
    ...
    assert post_pass_follow["residual_entry_ready"] is True
    assert post_pass_follow["selected_residual_proposal_id"]
    assert post_pass_follow["residual_entry_line"].startswith(
        "residual_entry ready=true "
    )
    assert post_pass_follow["residual_entry"]["residual_export_allowed"] is False
    ...
    failed = build_skill_route_discovery_focused_local_test_validation(
        unlocked_local_test_lane_apply=unlocked_apply,
        command_results=[
            {"command_hash": command_hash, "passed": False} for command_hash in command_hashes
        ],
        theme_window=theme,
        source_digest="github-growth-20260713T031123.591532Z",
    )
    assert failed["status"] == "failed"
    assert failed["residual_adjacent_hold_until_recorded"] is False
    assert failed["residual_adjacent_hold_active"] is True
    ...
    acceptance_ready = build_skill_route_discovery_focused_validation_activation_external_acceptance(
        focused_validation_activation_external_handoff=activation_ready,
        focused_local_test_validation=passed,
        unlocked_local_test_lane_apply=unlocked_apply,
        adjacent_general_agent_rows=pipeline["adjacent_general_agent_rows"],
        retained_boundaries=pipeline["retained_boundaries"],
        theme_window=theme,
        source_digest="github-growth-20260712T215308.239488Z",
    )
    assert acceptance_ready["status"] == "accepted"
    assert acceptance_ready["source_digest"] == "github-growth-20260712T215308.239488Z"
    assert acceptance_ready["decision"] == (
        "accept_activation_external_package_after_focused_validation_pass"
    )
    assert acceptance_ready["supervisor_next_action"] == (
        "keep_activation_external_and_queue_residual_adjacent_harness_eval"
    )
    assert acceptance_ready["focused_local_test_validation_recorded"] is True
    assert acceptance_ready["focused_validation_results_cover_expected"] is True
    assert acceptance_ready["focused_validation_all_expected_passed"] is True
    assert acceptance_ready["activation_external_only"] is True
    assert acceptance_ready["supervisor_activation_allowed"] is False
    assert acceptance_ready["runtime_action"] == "none"
    assert acceptance_ready["external_skill_execution_allowed"] is False
    assert acceptance_ready["provider_launch_allowed"] is False
    assert acceptance_ready["remote_apply_allowed"] is False
    assert acceptance_ready["push_or_promotion_allowed"] is False
    assert acceptance_ready["kernel_restart_allowed"] is False
    assert acceptance_ready["body_free"] is True
    assert acceptance_ready["raw_command_text_exported"] is False
    assert acceptance_ready["focused_validation"]["commands_exported"] is False
    assert "commands" not in acceptance_ready["focused_validation"]
    for row in acceptance_ready["focused_validation"]["command_results"]:
        assert set(row.keys()) <= {"command_hash", "passed", "in_expected_set"}
    assert acceptance_ready["residual_adjacent_harness_eval_available"] is True
    assert acceptance_ready["residual_adjacent_handoff_surface"] == (
        "agent_harness_eval_cluster_local_apply"
    )
    assert acceptance_ready["general_agent_inherits_skill_unlock"] is False

    # Accepted package queues residual fortress-style rows without skill unlocks.
    residual_ready = build_skill_route_discovery_focused_validation_residual_adjacent_queue(
        focused_validation_activation_external_acceptance=acceptance_ready,
        focused_validation_activation_external_handoff=activation_ready,
        focused_local_test_validation=passed,
        adjacent_general_agent_rows=pipeline["adjacent_general_agent_rows"],
        retained_boundaries=pipeline["retained_boundaries"],
        theme_window=theme,
        source_digest="github-growth-20260712T221308.618244Z",
    )
    assert residual_ready["status"] == "ready"
    assert residual_ready["source_digest"] == "github-growth-20260712T221308.618244Z"
    assert residual_ready["decision"] == (
        "queue_residual_adjacent_harness_eval_after_focused_validation_acceptance"
    )
    assert residual_ready["supervisor_next_action"] == (
        "hand_off_residual_adjacent_rows_to_agent_harness_eval_cluster_local_apply"
    )
    assert residual_ready["handoff_controller_surface"] == (
        "agent_harness_eval_cluster_local_apply"
    )
    assert residual_ready["proposal_track"] == (
        "prop-skill-reverse-flow-continue-local-validation"
    )
    assert residual_ready["adjacent_general_agent_proposal_ids"]
    assert residual_ready["unlocked_local_lanes"] == []
    assert residual_ready["skill_route_discovery_inherited"] is False
    assert residual_ready["skill_route_unlocks_closed_for_residual"] is True
    assert residual_ready["activation_external_only"] is True
    assert residual_ready["supervisor_activation_allowed"] is False
    assert residual_ready["runtime_action"] == "none"
    assert residual_ready["external_skill_execution_allowed"] is False
    assert residual_ready["provider_launch_allowed"] is False
    assert residual_ready["remote_apply_allowed"] is False
    assert residual_ready["push_or_promotion_allowed"] is False
    assert residual_ready["kernel_restart_allowed"] is False
    assert residual_ready["body_free"] is True
    assert residual_ready["raw_evidence_urls_exported"] is False
    assert residual_ready["general_agent_inherits_skill_unlock"] is False
    assert residual_ready["direct_allowed_lanes_before_eval"] == []
    assert "skill_route_discovery_focused_validation_residual_adjacent_queue" in residual_ready[
        "capability_pipeline"
    ]
    assert "skill_route_discovery_residual_adjacent_harness_eval_local_apply" in residual_ready[
        "capability_pipeline"
    ]

    # Ready residual queue hands one fortress/Hy3 row to harness-eval local apply.
    residual_apply_ready = build_skill_route_discovery_residual_adjacent_harness_eval_local_apply(
        focused_validation_residual_adjacent_queue=residual_ready,
        adjacent_general_agent_rows=pipeline["adjacent_general_agent_rows"],
        retained_boundaries=pipeline["retained_boundaries"],
        theme_window=theme,
        source_digest="github-growth-20260712T223308.255959Z",
    )
    assert residual_apply_ready["status"] == "ready"
    assert residual_apply_ready["source_digest"] == "github-growth-20260712T223308.255959Z"
    assert residual_apply_ready["controller_surface"] == (
        "skill_route_discovery_residual_adjacent_harness_eval_local_apply"
    )
    assert residual_apply_ready["proposal_track"] == (
        "prop-residual-adjacent-fortress-harness-eval"
    )
    assert residual_apply_ready["decision"] == (
        "hand_off_selected_residual_adjacent_row_to_agent_harness_eval_cluster_local_apply"
    )
    assert residual_apply_ready["supervisor_next_action"] == (
        "run_agent_harness_eval_local_comparison_for_residual_adjacent_row"
    )
    assert residual_apply_ready["handoff_controller_surface"] == (
        "agent_harness_eval_cluster_local_apply"
    )
    assert residual_apply_ready["selected_residual_proposal_id"]
    assert "fortress" in residual_apply_ready["selected_residual_proposal_id"]
    assert residual_apply_ready["unlocked_local_lanes"] == []
    assert residual_apply_ready["skill_route_discovery_inherited"] is False
    assert residual_apply_ready["skill_route_unlocks_closed_for_residual"] is True
    assert residual_apply_ready["local_comparison_required"] is True
    assert residual_apply_ready["local_comparison_status"] == (
        "ready_for_agent_harness_eval_local_comparison"
    )
    assert residual_apply_ready["allowed_local_lanes_after_local_comparison"] == [
        "documentation",
        "test",
        "code_patch",
    ]
    assert residual_apply_ready["direct_allowed_lanes_before_eval"] == []
    assert residual_apply_ready["activation_external_only"] is True
    assert residual_apply_ready["supervisor_activation_allowed"] is False
    assert residual_apply_ready["runtime_action"] == "none"
    assert residual_apply_ready["external_skill_execution_allowed"] is False
    assert residual_apply_ready["provider_launch_allowed"] is False
    assert residual_apply_ready["remote_apply_allowed"] is False
    assert residual_apply_ready["push_or_promotion_allowed"] is False
    assert residual_apply_ready["kernel_restart_allowed"] is False
    assert residual_apply_ready["body_free"] is True
    assert residual_apply_ready["raw_evidence_urls_exported"] is False
    assert residual_apply_ready["general_agent_inherits_skill_unlock"] is False
    assert residual_apply_ready["local_comparison_notes"]
    for note in residual_apply_ready["local_comparison_notes"]:
        assert set(note.keys()) <= {"criterion_id", "required", "passed"}
        assert note["passed"] is True

    # Ready residual local apply runs harness local comparison and unlocks
    # documentation/test/code_patch while skill unlocks stay closed.
    residual_comparison_ready = (
        build_skill_route_discovery_residual_adjacent_harness_eval_local_comparison(
            residual_adjacent_harness_eval_local_apply=residual_apply_ready,
            adjacent_general_agent_rows=pipeline["adjacent_general_agent_rows"],
            retained_boundaries=pipeline["retained_boundaries"],
            theme_window=theme,
            source_digest="github-growth-20260712T225308.154547Z",
        )
    )
    assert residual_comparison_ready["status"] == "ready"
    assert residual_comparison_ready["source_digest"] == (
        "github-growth-20260712T225308.154547Z"
    )
    assert residual_comparison_ready["controller_surface"] == (
        "skill_route_discovery_residual_adjacent_harness_eval_local_comparison"
    )
    assert residual_comparison_ready["proposal_track"] == (
        "prop-residual-adjacent-fortress-harness-eval"
    )
    assert residual_comparison_ready["decision"] == (
        "unlock_documentation_test_or_code_patch_after_residual_adjacent_harness_local_comparison"
    )
    assert residual_comparison_ready["supervisor_next_action"] == (
        "apply_unlocked_documentation_test_or_code_patch_with_focused_validation_and_keep_activation_external"
    )
    assert residual_comparison_ready["local_comparison_passed"] is True
    assert residual_comparison_ready["local_comparison_status"] == (
        "passed_local_comparison"
    )
    assert residual_comparison_ready["unlocked_local_lanes"] == [
        "documentation",
        "test",
        "code_patch",
    ]
    assert residual_comparison_ready["skill_route_unlocked_local_lanes"] == []
    assert residual_comparison_ready["skill_route_discovery_inherited"] is False
    assert residual_comparison_ready["skill_route_unlocks_closed_for_residual"] is True
    assert residual_comparison_ready["direct_allowed_lanes_before_eval"] == []
    assert residual_comparison_ready["handoff_controller_surface"] == (
        "agent_harness_eval_cluster_local_apply"
    )
    assert "fortress" in residual_comparison_ready["selected_residual_proposal_id"]
    assert residual_comparison_ready["activation_external_only"] is True
    assert residual_comparison_ready["supervisor_activation_allowed"] is False
    assert residual_comparison_ready["runtime_action"] == "none"
    assert residual_comparison_ready["external_skill_execution_allowed"] is False
    assert residual_comparison_ready["provider_launch_allowed"] is False
    assert residual_comparison_ready["remote_apply_allowed"] is False
    assert residual_comparison_ready["push_or_promotion_allowed"] is False
    assert residual_comparison_ready["kernel_restart_allowed"] is False
    assert residual_comparison_ready["body_free"] is True
    assert residual_comparison_ready["raw_evidence_urls_exported"] is False
    assert residual_comparison_ready["general_agent_inherits_skill_unlock"] is False
    assert residual_comparison_ready["local_comparison_notes"]
    for note in residual_comparison_ready["local_comparison_notes"]:
        assert set(note.keys()) <= {"criterion_id", "required", "passed"}
        assert note["passed"] is True
    assert residual_comparison_ready["unlocked_lane_routes"]
    for route in residual_comparison_ready["unlocked_lane_routes"]:
        assert route["lane"] in {"documentation", "test", "code_patch"}
        assert route["status"] == (
            "unlocked_after_residual_adjacent_harness_local_comparison"
        )
        assert route["runtime_action"] == "none"
        assert route["skill_route_discovery_inherited"] is False
    assert "skill_route_discovery_residual_adjacent_harness_eval_local_comparison" in (
        residual_comparison_ready["capability_pipeline"]
    )
    assert "skill_route_discovery_residual_adjacent_unlocked_local_lane_apply" in (
        residual_comparison_ready["capability_pipeline"]
    )

    # Ready residual harness comparison packages unlocked lane apply with
    # preferred test-first focused validation while skill unlocks stay closed.
    residual_unlocked_lane_ready = (
        build_skill_route_discovery_residual_adjacent_unlocked_local_lane_apply(
            residual_adjacent_harness_eval_local_comparison=residual_comparison_ready,
            adjacent_general_agent_rows=pipeline["adjacent_general_agent_rows"],
            retained_boundaries=pipeline["retained_boundaries"],
            theme_window=theme,
            source_digest="github-growth-20260712T231308.528323Z",
        )
    )
    assert residual_unlocked_lane_ready["status"] == "ready"
    assert residual_unlocked_lane_ready["source_digest"] == (
        "github-growth-20260712T231308.528323Z"
    )
    assert residual_unlocked_lane_ready["controller_surface"] == (
        "skill_route_discovery_residual_adjacent_unlocked_local_lane_apply"
    )
    assert residual_unlocked_lane_ready["proposal_track"] == (
        "prop-residual-adjacent-fortress-harness-eval"
    )
    assert residual_unlocked_lane_ready["decision"] == (
        "apply_unlocked_documentation_test_or_code_patch_with_focused_validation_and_keep_activation_external"
    )
    assert residual_unlocked_lane_ready["supervisor_next_action"] == (
        "run_focused_local_validation_for_residual_adjacent_unlocked_lane_and_keep_activation_external"
    )
    assert residual_unlocked_lane_ready["selected_local_lane"] == "test"
    assert residual_unlocked_lane_ready["preferred_local_lane"] == "test"
    assert residual_unlocked_lane_ready["unlocked_local_lanes"] == [
        "documentation",
        "test",
        "code_patch",
    ]
    assert residual_unlocked_lane_ready["skill_route_unlocked_local_lanes"] == []
    assert residual_unlocked_lane_ready["skill_route_discovery_inherited"] is False
    assert residual_unlocked_lane_ready["skill_route_unlocks_closed_for_residual"] is True
    assert residual_unlocked_lane_ready["direct_allowed_lanes_before_eval"] == []
    assert residual_unlocked_lane_ready["local_comparison_passed"] is True
    assert residual_unlocked_lane_ready["focused_validation"]["status"] == "ready"
    assert residual_unlocked_lane_ready["focused_validation"]["lane"] == "test"
    assert residual_unlocked_lane_ready["focused_validation"]["commands"]
    assert residual_unlocked_lane_ready["focused_validation"]["command_hashes"]
    assert residual_unlocked_lane_ready["focused_validation"]["commands_exported"] is False
    assert residual_unlocked_lane_ready["focused_validation"]["unit_test_signal"] is True
    assert "fortress" in residual_unlocked_lane_ready["selected_residual_proposal_id"]
    assert residual_unlocked_lane_ready["activation_external_only"] is True
    assert residual_unlocked_lane_ready["supervisor_activation_allowed"] is False
    assert residual_unlocked_lane_ready["runtime_action"] == "none"
    assert residual_unlocked_lane_ready["external_skill_execution_allowed"] is False
    assert residual_unlocked_lane_ready["provider_launch_allowed"] is False
    assert residual_unlocked_lane_ready["remote_apply_allowed"] is False
    assert residual_unlocked_lane_ready["push_or_promotion_allowed"] is False
    assert residual_unlocked_lane_ready["kernel_restart_allowed"] is False
    assert residual_unlocked_lane_ready["body_free"] is True
    assert residual_unlocked_lane_ready["raw_evidence_urls_exported"] is False
    assert residual_unlocked_lane_ready["general_agent_inherits_skill_unlock"] is False
    assert residual_unlocked_lane_ready["local_comparison_notes"]
    for note in residual_unlocked_lane_ready["local_comparison_notes"]:
        assert set(note.keys()) <= {"criterion_id", "required", "passed"}
        assert note["passed"] is True
    assert residual_unlocked_lane_ready["unlocked_lane_routes"]
    selected_routes = [
        route
        for route in residual_unlocked_lane_ready["unlocked_lane_routes"]
        if route.get("selected") is True
    ]
    assert len(selected_routes) == 1
    assert selected_routes[0]["lane"] == "test"
    assert selected_routes[0]["status"] == (
        "selected_for_residual_adjacent_focused_validation"
    )
    assert selected_routes[0]["runtime_action"] == "none"
    assert selected_routes[0]["skill_route_discovery_inherited"] is False
    assert "skill_route_discovery_residual_adjacent_unlocked_local_lane_apply" in (
    ...
    # Unrecorded residual focused validation blocks residual activation-external handoff.
    residual_activation_blocked = pipeline[
        "residual_adjacent_focused_validation_activation_external_handoff"
    ]
    assert residual_activation_blocked["controller_surface"] == (
        "skill_route_discovery_residual_adjacent_focused_validation_activation_external_handoff"
    )
    assert residual_activation_blocked["status"] in {
        "blocked_until_residual_adjacent_focused_validation_recorded",
        "blocked_until_residual_adjacent_focused_validation_ready",
        "not_applicable",
    }
    assert residual_activation_blocked["activation_external_only"] is True
    assert residual_activation_blocked["supervisor_activation_allowed"] is False
    assert residual_activation_blocked["skill_route_unlocked_local_lanes"] == []
    assert residual_activation_blocked["skill_route_discovery_inherited"] is False
    assert residual_activation_blocked["runtime_action"] == "none"
    assert residual_activation_blocked["push_or_promotion_allowed"] is False
    assert residual_activation_blocked["kernel_restart_allowed"] is False

    # Recorded residual focused pass packages residual activation-external handoff.
    residual_activation_ready = (
        build_skill_route_discovery_residual_adjacent_focused_validation_activation_external_handoff(
            ...
            retained_boundaries=pipeline["retained_boundaries"],
            theme_window=theme,
            source_digest="github-growth-20260713T010202.728081Z",
        )
    )
    assert residual_activation_ready["status"] == "ready"
    assert residual_activation_ready["source_digest"] == (
        "github-growth-20260713T010202.728081Z"
    )
    assert residual_activation_ready["controller_surface"] == (
        "skill_route_discovery_residual_adjacent_focused_validation_activation_external_handoff"
    )
    assert residual_activation_ready["proposal_track"] == (
        "prop-residual-adjacent-fortress-harness-eval"
    )
    assert residual_activation_ready["decision"] == (
        "package_activation_external_handoff_after_residual_adjacent_focused_validation_pass"
    )
    assert residual_activation_ready["supervisor_next_action"] in {
        "keep_activation_external_after_residual_adjacent_focused_local_validation",
        "keep_activation_external_and_note_remaining_residual_adjacent_rows",
    }
    if residual_activation_ready["remaining_residual_adjacent_proposal_ids"]:
        assert residual_activation_ready["supervisor_next_action"] == (
            "keep_activation_external_and_note_remaining_residual_adjacent_rows"
        )
        assert residual_activation_ready["remaining_residual_adjacent_available"] is True
        assert residual_activation_ready["selected_residual_proposal_id"] not in (
            residual_activation_ready["remaining_residual_adjacent_proposal_ids"]
        )
    else:
        assert residual_activation_ready["supervisor_next_action"] == (
            "keep_activation_external_after_residual_adjacent_focused_local_validation"
        )
        assert residual_activation_ready["remaining_residual_adjacent_available"] is False
    assert residual_activation_ready[
    ...
    assert residual_activation_ready["selected_local_lane"] == "test"
    assert residual_activation_ready["unlocked_local_lanes"] == [
        "documentation",
        "test",
        "code_patch",
    ]
    assert residual_activation_ready["skill_route_unlocked_local_lanes"] == []
    assert residual_activation_ready["skill_route_discovery_inherited"] is False
    assert residual_activation_ready["skill_route_unlocks_closed_for_residual"] is True
    assert residual_activation_ready["direct_allowed_lanes_before_eval"] == []
    assert "fortress" in residual_activation_ready["selected_residual_proposal_id"]
    assert residual_activation_ready["activation_external_only"] is True
    assert residual_activation_ready["supervisor_activation_allowed"] is False
    assert residual_activation_ready["runtime_action"] == "none"
    assert residual_activation_ready["external_skill_execution_allowed"] is False
    assert residual_activation_ready["provider_launch_allowed"] is False
    assert residual_activation_ready["remote_apply_allowed"] is False
    assert residual_activation_ready["push_or_promotion_allowed"] is False
    assert residual_activation_ready["kernel_restart_allowed"] is False
    assert residual_activation_ready["body_free"] is True
    assert residual_activation_ready["raw_command_text_exported"] is False
    assert residual_activation_ready["focused_validation"]["commands_exported"] is False
    assert "commands" not in residual_activation_ready["focused_validation"]
    for row in residual_activation_ready["focused_validation"]["command_results"]:
        assert set(row.keys()) <= {"command_hash", "passed", "in_expected_set"}
    assert residual_activation_ready["general_agent_inherits_skill_unlock"] is False
    assert "skill_route_discovery_residual_adjacent_focused_validation_activation_external_handoff" in (
        residual_activation_ready["capability_pipeline"]
    )
    ...
    assert len(materialized) == len(command_hashes)
    assert {row["command_hash"] for row in materialized} == set(command_hashes)
    for row in materialized:
        assert set(row.keys()) <= {"command_hash", "passed", "in_expected_set"}
        assert row["passed"] is True
        assert row["in_expected_set"] is True
        assert "command" not in row
        assert "stdout" not in row

    closed = close_skill_route_discovery_focused_local_test_validation_with_outcome(
        pipeline,
        passed=True,
        source_digest="github-growth-20260712T215308.239488Z",
    )
    ...
    assert closed["focused_validation_activation_external_acceptance"]["status"] == "accepted"
    assert closed["focused_validation_activation_external_acceptance"]["decision"] == (
        "accept_activation_external_package_after_focused_validation_pass"
    )
    assert closed["focused_validation_activation_external_acceptance"][
        "supervisor_next_action"
    ] == ("keep_activation_external_and_queue_residual_adjacent_harness_eval")
    assert closed["focused_validation_activation_external_acceptance"][
        "supervisor_activation_allowed"
    ] is False
    assert closed["focused_validation_residual_adjacent_queue"]["status"] == "ready"
    assert closed["focused_validation_residual_adjacent_queue"]["decision"] == (
        "queue_residual_adjacent_harness_eval_after_focused_validation_acceptance"
    )
    assert closed["focused_validation_residual_adjacent_queue"][
        "supervisor_next_action"
    ] == ("hand_off_residual_adjacent_rows_to_agent_harness_eval_cluster_local_apply")
    assert closed["focused_validation_residual_adjacent_queue"][
        "supervisor_activation_allowed"
    ...
    assert closed["residual_adjacent_harness_eval_local_apply"]["status"] == "ready"
    assert closed["residual_adjacent_harness_eval_local_apply"]["decision"] == (
        "hand_off_selected_residual_adjacent_row_to_agent_harness_eval_cluster_local_apply"
    )
    assert closed["residual_adjacent_harness_eval_local_apply"][
        "supervisor_next_action"
    ] == ("run_agent_harness_eval_local_comparison_for_residual_adjacent_row")
    assert closed["residual_adjacent_harness_eval_local_apply"][
        "supervisor_activation_allowed"
    ...
    assert "fortress" in closed["residual_adjacent_harness_eval_local_apply"][
        "selected_residual_proposal_id"
    ]
    assert closed["residual_adjacent_harness_eval_local_comparison"]["status"] == "ready"
    assert closed["residual_adjacent_harness_eval_local_comparison"]["decision"] == (
        "unlock_documentation_test_or_code_patch_after_residual_adjacent_harness_local_comparison"
    )
    assert closed["residual_adjacent_harness_eval_local_comparison"][
        "supervisor_next_action"
    ] == (
        "apply_unlocked_documentation_test_or_code_patch_with_focused_validation_and_keep_activation_external"
    )
    ...
    assert "fortress" in closed["residual_adjacent_harness_eval_local_comparison"][
        "selected_residual_proposal_id"
    ]
    assert closed["residual_adjacent_unlocked_local_lane_apply"]["status"] == "ready"
    assert closed["residual_adjacent_unlocked_local_lane_apply"]["decision"] == (
        "apply_unlocked_documentation_test_or_code_patch_with_focused_validation_and_keep_activation_external"
    )
    assert closed["residual_adjacent_unlocked_local_lane_apply"][
        "supervisor_next_action"
    ] == (
        "run_focused_local_validation_for_residual_adjacent_unlocked_lane_and_keep_activation_external"
    )
    ...
    assert pipeline_passed["focused_validation_activation_external_acceptance"][
        "status"
    ] == "accepted"
    assert pipeline_passed["focused_validation_activation_external_acceptance"][
        "decision"
    ...
    assert pipeline_passed["focused_validation_residual_adjacent_queue"][
        "status"
    ] == "ready"
    assert pipeline_passed["focused_validation_residual_adjacent_queue"][
        "decision"
    ...
    assert pipeline_passed["residual_adjacent_harness_eval_local_apply"][
        "status"
    ] == "ready"
    assert pipeline_passed["residual_adjacent_harness_eval_local_apply"][
        "decision"
    ...
    assert pipeline_passed["residual_adjacent_harness_eval_local_comparison"][
        "status"
    ] == "ready"
    assert pipeline_passed["residual_adjacent_harness_eval_local_comparison"][
        "decision"
    ...
    assert recorded["focused_validation_activation_external_acceptance"]["status"] == "accepted"
    assert recorded["focused_validation_activation_external_acceptance"]["decision"] == (
        "accept_activation_external_package_after_focused_validation_pass"
    )
    assert recorded["focused_validation_activation_external_acceptance"][
        "supervisor_next_action"
    ] == ("keep_activation_external_and_queue_residual_adjacent_harness_eval")
    assert recorded["focused_validation_residual_adjacent_queue"]["status"] == "ready"
    assert recorded["focused_validation_residual_adjacent_queue"]["decision"] == (
        "queue_residual_adjacent_harness_eval_after_focused_validation_acceptance"
    )
    assert recorded["focused_validation_residual_adjacent_queue"][
        "supervisor_next_action"
    ] == ("hand_off_residual_adjacent_rows_to_agent_harness_eval_cluster_local_apply")
    assert recorded["residual_adjacent_harness_eval_local_apply"]["status"] == "ready"
    assert recorded["residual_adjacent_harness_eval_local_apply"]["decision"] == (
        "hand_off_selected_residual_adjacent_row_to_agent_harness_eval_cluster_local_apply"
    )
    assert recorded["residual_adjacent_harness_eval_local_apply"][
        "supervisor_next_action"
    ] == ("run_agent_harness_eval_local_comparison_for_residual_adjacent_row")
    assert recorded["residual_adjacent_harness_eval_local_apply"][
        "unlocked_local_lanes"
    ] == []
    assert recorded["residual_adjacent_harness_eval_local_comparison"]["status"] == "ready"
    assert recorded["residual_adjacent_harness_eval_local_comparison"]["decision"] == (
        "unlock_documentation_test_or_code_patch_after_residual_adjacent_harness_local_comparison"
    )
    assert recorded["residual_adjacent_harness_eval_local_comparison"][
        "supervisor_next_action"
    ] == (
        "apply_unlocked_documentation_test_or_code_patch_with_focused_validation_and_keep_activation_external"
    )
    ...
    assert recorded["residual_adjacent_unlocked_local_lane_apply"]["status"] == "ready"
    assert recorded["residual_adjacent_unlocked_local_lane_apply"]["decision"] == (
        "apply_unlocked_documentation_test_or_code_patch_with_focused_validation_and_keep_activation_external"
    )
    ...
    assert "Focused validation activation-external acceptance: `blocked_until_activation_external_handoff_ready`" in rendered
    assert "Focused validation residual adjacent queue: `blocked_until_activation_external_acceptance`" in rendered
    ...
    assert "Focused validation activation-external acceptance: `accepted`" in rendered_passed
    assert "Focused validation residual adjacent queue: `ready`" in rendered_passed
    assert "Residual adjacent harness-eval local apply: `ready`" in rendered_passed
    assert "Residual adjacent harness-eval local comparison: `ready`" in rendered_passed
    assert "Residual adjacent unlocked local lane apply: `ready`" in rendered_passed
    assert "Residual adjacent unlocked selected lane: `test`" in rendered_passed
    assert (
        "package_activation_external_handoff_after_focused_validation_pass"
        in rendered_passed
    )
    ...
    assert "Residual adjacent harness unlocked lanes: `documentation, test, code_patch`" in (
        rendered_passed
    )
    assert (
        "record_skill_route_discovery_focused_local_test_validation_results"
    ...
    # Failed focused validation keeps activation-external handoff/acceptance blocked.
    ...
    failed_acceptance = build_skill_route_discovery_focused_validation_activation_external_acceptance(
        focused_validation_activation_external_handoff=failed_activation,
        focused_local_test_validation=failed,
        unlocked_local_test_lane_apply=unlocked_apply,
        theme_window=theme,
    )
    assert failed_acceptance["status"] == (
        "blocked_until_activation_external_handoff_ready"
    )
    assert failed_acceptance["decision"] == (
        "hold_activation_external_acceptance_until_handoff_ready"
    )
    assert failed_acceptance["supervisor_next_action"] == (
        "repair_failed_focused_local_test_validation_commands"
    )
    assert failed_acceptance["supervisor_activation_allowed"] is False

    closed_failed = close_skill_route_discovery_focused_local_test_validation_with_outcome(
        pipeline,
        passed=False,
        source_digest="github-growth-20260712T215308.239488Z",
    )
    assert closed_failed["focused_local_test_validation"]["status"] == "failed"
    assert closed_failed["focused_validation_activation_external_handoff"]["status"] == (
        "blocked_until_focused_validation_repaired"
    )
    assert closed_failed["focused_validation_activation_external_acceptance"]["status"] == (
        "blocked_until_activation_external_handoff_ready"
    )
    assert closed_failed["focused_validation_activation_external_acceptance"][
        "supervisor_activation_allowed"
    ] is False
    assert closed_failed["focused_validation_residual_adjacent_queue"]["status"] == (
        "blocked_until_activation_external_acceptance"
    )
    assert closed_failed["focused_validation_residual_adjacent_queue"][
        "supervisor_activation_allowed"
    ] is False
    assert closed_failed["residual_adjacent_harness_eval_local_apply"]["status"] == (
        "blocked_until_residual_adjacent_queue_ready"
    )
    assert closed_failed["residual_adjacent_harness_eval_local_apply"][
        "supervisor_activation_allowed"
    ] is False

    # No residual rows after acceptance keeps residual queue not_applicable.
    residual_none = build_skill_route_discovery_focused_validation_residual_adjacent_queue(
        focused_validation_activation_external_acceptance={
            **acceptance_ready,
            "residual_adjacent_harness_eval_available": False,
            "adjacent_general_agent_proposal_ids": [],
        },
    ...
    assert residual_none["status"] == "not_applicable"
    assert residual_none["decision"] == (
        "no_residual_adjacent_rows_after_focused_validation_acceptance"
    )
    assert residual_none["supervisor_next_action"] == (
        "keep_activation_external_after_focused_local_test_validation"
    )
    assert residual_none["supervisor_activation_allowed"] is False
    residual_apply_none = build_skill_route_discovery_residual_adjacent_harness_eval_local_apply(
        focused_validation_residual_adjacent_queue=residual_none,
        adjacent_general_agent_rows=[],
        retained_boundaries=pipeline["retained_boundaries"],
        theme_window=theme,
    )
    assert residual_apply_none["status"] == "not_applicable"
    assert residual_apply_none["decision"] == (
        "no_residual_adjacent_queue_for_harness_eval_local_apply"
    )
    assert residual_apply_none["supervisor_activation_allowed"] is False
    assert residual_apply_none["unlocked_local_lanes"] == []
    ...
    dumped_acceptance = json.dumps(acceptance_ready)
    assert reverse_url not in dumped_acceptance
    assert fortress_url not in dumped_acceptance
    assert rnskill_url not in dumped_acceptance
    dumped_closed = json.dumps(closed)
    assert reverse_url not in dumped_closed
    assert fortress_url not in dumped_closed
    dumped_residual_apply = json.dumps(residual_apply_ready)
    assert reverse_url not in dumped_residual_apply
    assert fortress_url not in dumped_residual_apply
    assert rnskill_url not in dumped_residual_apply
...
def test_skill_route_discovery_residual_adjacent_harness_eval_local_apply_prefers_fortress():
    """Ready residual queue selects fortress before Hy3 for harness-eval local apply."""

    residual_queue = {
        "status": "ready",
        "decision": "queue_residual_adjacent_harness_eval_after_focused_validation_acceptance",
        "activation_external_only": True,
        "supervisor_activation_allowed": False,
        "runtime_action": "none",
        ...
        "retained_boundary_proposal_ids": ["prop-agent-chief-privacy-review-only"],
        "source_digest": "github-growth-20260712T223308.255959Z",
        "theme_id": "skill-route-discovery",
        "theme_pass": {"planned_passes": 4, "target_passes": 4, "status": "complete"},
    }
    adjacent_rows = [
        {
            "proposal_id": "prop-harness-hy3-adjacent-eval",
            "route_class": "agent_harness_eval_required",
            "evaluation_lane": "agent_harness_eval_required",
            "skill_route_discovery_inherited": False,
            "direct_allowed_lanes_before_eval": [],
            "reason": "general_agent_project_requires_agent_harness_eval",
        },
        {
            "proposal_id": "prop-harness-fortress-local-eval",
            "route_class": "agent_harness_eval_required",
    ...
    retained = [
        {
            ...
            "route_class": "privacy_boundary_review_only",
            "validation_gate": "privacy-leakage-human-review",
            "reason": "privacy_leakage_or_agent_chief_remains_review_only",
        }
    ]

    packet = build_skill_route_discovery_residual_adjacent_harness_eval_local_apply(
    ...
    assert packet["selected_residual_proposal_id"] == "prop-harness-fortress-local-eval"
    assert packet["proposal_id"] == "prop-harness-fortress-local-eval"
    assert packet["decision"] == (
        "hand_off_selected_residual_adjacent_row_to_agent_harness_eval_cluster_local_apply"
    )
    assert packet["supervisor_next_action"] == (
        "run_agent_harness_eval_local_comparison_for_residual_adjacent_row"
    )
    assert packet["handoff_controller_surface"] == "agent_harness_eval_cluster_local_apply"
    assert packet["unlocked_local_lanes"] == []
    assert packet["skill_route_discovery_inherited"] is False
    assert packet["activation_external_only"] is True
    assert packet["supervisor_activation_allowed"] is False
    assert packet["runtime_action"] == "none"
    assert packet["body_free"] is True
    dumped = json.dumps(packet)
    assert "https://github.com/" not in dumped
    assert "tiliondev" not in dumped
...
def test_skill_route_discovery_local_apply_completion_blocks_when_apply_not_ready():
    ...
    completion = build_skill_route_discovery_local_apply_completion(
        local_apply={
            "status": "blocked_until_reverse_flow_lane_ready",
            "selected_proposal_id": "prop-skill-pipeline-reverse-flow-test",
        ...
        rnskill_docs_validation_lane={"status": "ready", "body_free": True},
        config_gate_boundary={
            "status": "ready",
            "general_agent_inherits_skill_unlock": False,
            "privacy_rows_selectable_for_local_apply": False,
            "route_profiles_separated": True,
        },
        local_comparison={"local_comparison_passed": True, "status": "passed"},
        theme_window={
            "theme_id": "skill-route-discovery",
            "planned_passes": 4,
            "target_passes": 4,
...
def test_skill_route_discovery_local_comparison_blocks_when_skill_first_missing():
    """Local comparison fails if codex_workflow_gate loses skill_route_discovery_first."""
    ...
    comparison = evaluate_skill_route_discovery_local_comparison(selected)
    assert comparison["local_comparison_passed"] is False
    assert "skill_route_discovery_first_for_codex_workflow_gate" in comparison["failed_criteria"]
    assert comparison["unlocked_local_lanes"] == []
    assert comparison["runtime_action"] == "none"
...
def test_self_evolution_plan_carries_skill_route_discovery_capability_pipeline(tmp_path: Path):
    ...
    # Pass 2/3 criteria-driven comparison unlocks reverse-flow and local-apply handoff.
    assert pipeline["status"] == "ready"
    ...
    assert pipeline["selected_step"]["unlocked_local_lanes"] == ["test"]
    assert pipeline["selected_step"]["local_comparison_status"] == "passed"
    assert pipeline["local_comparison"]["local_comparison_passed"] is True
    assert pipeline["reverse_flow_test_validation_lane"]["status"] == "ready"
    assert pipeline["rnskill_docs_validation_lane"]["status"] == "ready"
    assert pipeline["config_gate_boundary"]["status"] == "ready"
    assert pipeline["local_apply"]["status"] == "ready"
    ...
    assert "Reverse-flow test validation lane" in plan.task
    assert "Local apply handoff" in plan.task
    assert "Rnskill docs validation lane" in plan.task
    assert "Config gate boundary" in plan.task
