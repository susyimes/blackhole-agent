...
def test_build_digest_redirects_skill_route_pipeline_when_compounder_is_ready():
    ...
    assert route["selected_step"]["route_class"] == "capability_compounder"
    assert route["skill_route_pin_cascade_frozen"] is True
    assert "## Evolution Route" in markdown
    assert "https://github.com/lingbol088-spec/reverse-flow-skill" not in json.dumps(route)
