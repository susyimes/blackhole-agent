...
def test_goal_binds_lmp_actuation_plane() -> None:
    assert leftover_marker_ids(LMP_ACTUATION_GOAL) == (LMP_ACTUATION_ID,)
    ...
    assert LMP_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(LMP_LEFTOVER) == (LMP_ACTUATION_ID,)
    assert leftover_marker_ids(LMP_ACTUATION_GOAL) == (LMP_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert LMP_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(LMP_ACTUATION_GOAL)
...
def test_opted_in_lmp_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("lmp",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, LMP_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["lmp"]

    missing = run_lmp_workflow(with_lmpid=False)
    skip_bind = run_lmp_workflow(skip_bind=True)
    skip_temporary = run_lmp_workflow(do_temporary=False)
    skip_public = run_lmp_workflow(do_public=False)
    skip_lmpdigest = run_lmp_workflow(do_lmpdigest=False)
    skip_replay = run_lmp_workflow(replay=False)
    skip_lmpid = run_lmp_workflow(use_lmpid=False)
    live = run_lmp_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_lmpid"
    ...
    assert skip_lmpdigest["ok"] is False
    assert skip_lmpdigest["error"] == "lmpdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_lmpid["ok"] is False
    assert skip_lmpid["error"] == "lmpid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_lmpdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["lmpdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["lmpid_bound"] is True
    assert row["digest"]
    assert live["lmpid"] == DEFAULT_LMPID
    assert live["lmpdigest"] == DEFAULT_LMPDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, lmpid=DEFAULT_LMPID, lmpdigest=DEFAULT_LMPDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["lmpid"] == DEFAULT_LMPID
    assert queried["lmpdigest"] == DEFAULT_LMPDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == LMP_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, lmpid=DEFAULT_LMPID, lmpdigest=DEFAULT_LMPDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["lmpid"] == DEFAULT_LMPID
    assert answered["lmpdigest"] == DEFAULT_LMPDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, lmpid=DEFAULT_LMPID, include_lmpid=False)
    )
    assert bare["has_lmpid"] is False
    assert bare["lmpid"] == EMPTY_LMPID
    publicised = serialize_lmp(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_lmp(publicised) == DEFAULT_SOURCE
    assert parse_lmp(RFC_DEST_FIELD) == DEST_HOP
    header = parse_lmp_header(encode_lmp_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_LMPID, DEFAULT_LMPDIGEST))
    ...
    assert asked["method"] == "CONFIG"
    assert asked["disc_kind"] == "config"
    assert listed["disc_kind"] == "hello"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_LMPID).startswith("CONFIG")
    assert "lmpdigest=" in canonical_public(SENTINEL, DEFAULT_LMPID, DEFAULT_LMPDIGEST)


def test_builtin_proof_seals_lmp_actuation() -> None:
    report = builtin_lmp_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "lmp_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_lmp"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_lmpid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_lmpdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_lmpid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_lmpdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_lmp"]
    ...
    assert report["checks"]["leftover_text_binds_lmp"]
    assert report["checks"]["proved_lmp_consumes_leftover"]
    assert report["mission_goal"] == LMP_ACTUATION_GOAL
    assert report["done_when"] == LMP_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[LMP_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "lmp" in capability.tags
    assert "rfc4204" in capability.tags
    assert "http" in capability.tags
    assert "lmpid" in capability.tags
    assert "lmpdigest" in capability.tags
    assert "config" in capability.tags
    assert "hello" in capability.tags


def test_selection_gate_rejects_ledger_only_lmp_family(tmp_path: Path) -> None:
    ...
    assert "lmp" in family.split("/")
    assert "rfc4204" in set(semantic_tokens(LMP_ACTUATION_GOAL))
    assert "lmpid" in set(semantic_tokens(LMP_ACTUATION_GOAL))
    assert "lmpdigest" in family
