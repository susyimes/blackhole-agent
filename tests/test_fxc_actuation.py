...
def test_goal_binds_fxc_actuation_plane() -> None:
    assert leftover_marker_ids(FXC_ACTUATION_GOAL) == (FXC_ACTUATION_ID,)
    ...
    assert FXC_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(FXC_LEFTOVER) == (FXC_ACTUATION_ID,)
    assert leftover_marker_ids(FXC_ACTUATION_GOAL) == (FXC_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert FXC_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(FXC_ACTUATION_GOAL)
...
def test_opted_in_fxc_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("fxc",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, FXC_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["fxc"]

    missing = run_fxc_workflow(with_fxcid=False)
    skip_bind = run_fxc_workflow(skip_bind=True)
    skip_temporary = run_fxc_workflow(do_temporary=False)
    skip_public = run_fxc_workflow(do_public=False)
    skip_fxcdigest = run_fxc_workflow(do_fxcdigest=False)
    skip_replay = run_fxc_workflow(replay=False)
    skip_fxcid = run_fxc_workflow(use_fxcid=False)
    live = run_fxc_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_fxcid"
    ...
    assert skip_fxcdigest["ok"] is False
    assert skip_fxcdigest["error"] == "fxcdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_fxcid["ok"] is False
    assert skip_fxcid["error"] == "fxcid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_fxcdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["fxcdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["fxcid_bound"] is True
    assert row["digest"]
    assert live["fxcid"] == DEFAULT_FXCID
    assert live["fxcdigest"] == DEFAULT_FXCDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, fxcid=DEFAULT_FXCID, fxcdigest=DEFAULT_FXCDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["fxcid"] == DEFAULT_FXCID
    assert queried["fxcdigest"] == DEFAULT_FXCDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == FXC_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, fxcid=DEFAULT_FXCID, fxcdigest=DEFAULT_FXCDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["fxcid"] == DEFAULT_FXCID
    assert answered["fxcdigest"] == DEFAULT_FXCDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, fxcid=DEFAULT_FXCID, include_fxcid=False)
    )
    assert bare["has_fxcid"] is False
    assert bare["fxcid"] == EMPTY_FXCID
    publicised = serialize_fxc(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_fxc(publicised) == DEFAULT_SOURCE
    assert parse_fxc(RFC_DEST_FIELD) == DEST_HOP
    header = parse_fxc_header(encode_fxc_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_FXCID, DEFAULT_FXCDIGEST))
    ...
    assert asked["method"] == "FXC"
    assert asked["disc_kind"] == "fxc"
    assert listed["disc_kind"] == "vlan"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_FXCID).startswith("FXC")
    assert "fxcdigest=" in canonical_public(SENTINEL, DEFAULT_FXCID, DEFAULT_FXCDIGEST)


def test_builtin_proof_seals_fxc_actuation() -> None:
    report = builtin_fxc_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "fxc_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_fxc"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_fxcid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_fxcdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_fxcid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_fxcdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_fxc"]
    ...
    assert report["checks"]["leftover_text_binds_fxc"]
    assert report["checks"]["proved_fxc_consumes_leftover"]
    assert report["mission_goal"] == FXC_ACTUATION_GOAL
    assert report["done_when"] == FXC_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[FXC_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "fxc" in capability.tags
    assert "rfc9625" in capability.tags
    assert "http" in capability.tags
    assert "fxcid" in capability.tags
    assert "fxcdigest" in capability.tags
    assert "fxc" in capability.tags
    assert "vlan" in capability.tags


def test_selection_gate_rejects_ledger_only_fxc_family(tmp_path: Path) -> None:
    ...
    assert "fxc" in family.split("/")
    assert "rfc9625" in set(semantic_tokens(FXC_ACTUATION_GOAL))
    assert "fxcid" in set(semantic_tokens(FXC_ACTUATION_GOAL))
    assert "fxcdigest" in family
