...
def test_goal_binds_wson_actuation_plane() -> None:
    assert leftover_marker_ids(WSON_ACTUATION_GOAL) == (WSON_ACTUATION_ID,)
    ...
    assert WSON_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(WSON_LEFTOVER) == (WSON_ACTUATION_ID,)
    assert leftover_marker_ids(WSON_ACTUATION_GOAL) == (WSON_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert WSON_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(WSON_ACTUATION_GOAL)
...
def test_opted_in_wson_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("wson",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, WSON_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["wson"]

    missing = run_wson_workflow(with_wsonid=False)
    skip_bind = run_wson_workflow(skip_bind=True)
    skip_temporary = run_wson_workflow(do_temporary=False)
    skip_public = run_wson_workflow(do_public=False)
    skip_wsondigest = run_wson_workflow(do_wsondigest=False)
    skip_replay = run_wson_workflow(replay=False)
    skip_wsonid = run_wson_workflow(use_wsonid=False)
    live = run_wson_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_wsonid"
    ...
    assert skip_wsondigest["ok"] is False
    assert skip_wsondigest["error"] == "wsondigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_wsonid["ok"] is False
    assert skip_wsonid["error"] == "wsonid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_wsondigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["wsondigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["wsonid_bound"] is True
    assert row["digest"]
    assert live["wsonid"] == DEFAULT_WSONID
    assert live["wsondigest"] == DEFAULT_WSONDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, wsonid=DEFAULT_WSONID, wsondigest=DEFAULT_WSONDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["wsonid"] == DEFAULT_WSONID
    assert queried["wsondigest"] == DEFAULT_WSONDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == WSON_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, wsonid=DEFAULT_WSONID, wsondigest=DEFAULT_WSONDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["wsonid"] == DEFAULT_WSONID
    assert answered["wsondigest"] == DEFAULT_WSONDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, wsonid=DEFAULT_WSONID, include_wsonid=False)
    )
    assert bare["has_wsonid"] is False
    assert bare["wsonid"] == EMPTY_WSONID
    publicised = serialize_wson(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_wson(publicised) == DEFAULT_SOURCE
    assert parse_wson(RFC_DEST_FIELD) == DEST_HOP
    header = parse_wson_header(encode_wson_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_WSONID, DEFAULT_WSONDIGEST))
    ...
    assert asked["method"] == "WSON"
    assert asked["disc_kind"] == "wson"
    assert listed["disc_kind"] == "rwa"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_WSONID).startswith("WSON")
    assert "wsondigest=" in canonical_public(SENTINEL, DEFAULT_WSONID, DEFAULT_WSONDIGEST)


def test_builtin_proof_seals_wson_actuation() -> None:
    report = builtin_wson_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "wson_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_wson"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_wsonid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_wsondigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_wsonid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_wsondigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_binds_wson"]
    ...
    assert report["checks"]["catalog_names_wson"]
    assert report["checks"]["leftover_text_binds_wson"]
    assert report["checks"]["proved_wson_consumes_leftover"]
    assert report["mission_goal"] == WSON_ACTUATION_GOAL
    assert report["done_when"] == WSON_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[WSON_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "wson" in capability.tags
    assert "rfc6163" in capability.tags
    assert "http" in capability.tags
    assert "wsonid" in capability.tags
    assert "wsondigest" in capability.tags
    assert "wson" in capability.tags
    assert "rwa" in capability.tags


def test_selection_gate_rejects_ledger_only_wson_family(tmp_path: Path) -> None:
    ...
    assert "wson" in family.split("/")
    assert "rfc6163" in set(semantic_tokens(WSON_ACTUATION_GOAL))
    assert "wsonid" in set(semantic_tokens(WSON_ACTUATION_GOAL))
    assert "wsondigest" in family
