...
from blackhole_agent.largecomm_actuation import (
    ...
    SENTINEL,
    SOURCE_HEADER,
    builtin_largecomm_actuation_proof,
    canonical_public,
    canonical_temporary,
    crc32c,
    encode_public,
    encode_temporary,
    encode_largecomm_header,
    independent_largecommdigest,
    parse_http_comm,
    parse_http_response,
    parse_message,
    parse_largecomm,
    parse_largecomm_header,
    public_comm,
    public_response,
    run_largecomm_workflow,
    serialize_largecomm,
    temporary_comm,
    temporary_response,
)
...
def test_goal_binds_largecomm_actuation_plane() -> None:
    assert leftover_marker_ids(LARGECOMM_ACTUATION_GOAL) == (LARGECOMM_ACTUATION_ID,)
    ...
    assert LARGECOMM_ACTUATION_ID in LOCAL_DENYLIST
    ...
    assert leftover_marker_ids(LARGECOMM_LEFTOVER) == (LARGECOMM_ACTUATION_ID,)
    assert leftover_marker_ids(LARGECOMM_ACTUATION_GOAL) == (LARGECOMM_ACTUATION_ID,)
    assert leftover_marker_ids(DISC_ACTUATION_GOAL) == (DISC_ACTUATION_ID,)
    assert leftover_marker_ids(DNS64_ACTUATION_GOAL) == (DNS64_ACTUATION_ID,)
    assert leftover_marker_ids(NAT64_ACTUATION_GOAL) == (NAT64_ACTUATION_ID,)
    ...
    for goal, capability_id in zip(NEIGHBORS, NEIGHBOR_IDS, strict=True):
        ...
        assert LARGECOMM_ACTUATION_ID not in leftover_marker_ids(goal)
        assert capability_id not in leftover_marker_ids(LARGECOMM_ACTUATION_GOAL)
...
def test_opted_in_largecomm_tool_completes_body_transfer_poll() -> None:
    ...
    preflight = build_tool_routing_preflight(
        [descriptor],
        required_tool_names=("largecomm",),
        executable_providers=(*DEFAULT_EXECUTABLE_TOOL_PROVIDERS, LARGECOMM_TOOL_PROVIDER),
    )
    assert preflight["ok"] is True
    assert preflight["executable_tool_names"] == ["largecomm"]

    missing = run_largecomm_workflow(with_largecommid=False)
    skip_bind = run_largecomm_workflow(skip_bind=True)
    skip_temporary = run_largecomm_workflow(do_temporary=False)
    skip_public = run_largecomm_workflow(do_public=False)
    skip_largecommdigest = run_largecomm_workflow(do_largecommdigest=False)
    skip_replay = run_largecomm_workflow(replay=False)
    skip_largecommid = run_largecomm_workflow(use_largecommid=False)
    live = run_largecomm_workflow()
    assert missing["ok"] is False
    assert missing["final_status"] == 403
    assert missing["error"] == "missing_largecommid"
    ...
    assert skip_largecommdigest["ok"] is False
    assert skip_largecommdigest["error"] == "largecommdigest_required"
    assert skip_replay["ok"] is False
    assert skip_replay["error"] == "replay_required"
    assert skip_largecommid["ok"] is False
    assert skip_largecommid["error"] == "largecommid_required"
    assert live["ok"] is True
    assert live["sentinel"] == SENTINEL
    assert live["independent_sentinel"] == SENTINEL
    assert Path(live["sealed_path"]).is_file()
    row = independent_largecommdigest(Path(live["sealed_path"]))
    assert row["sentinel"] == SENTINEL
    assert row["temporary_frame"] is True
    assert row["public_frame"] is True
    assert row["largecommdigest_locate"] is True
    assert row["stored"] is True
    assert row["retrieved"] is True
    assert row["replayed"] is True
    assert row["independent"] is True
    assert row["largecommid_bound"] is True
    assert row["digest"]
    assert live["largecommid"] == DEFAULT_LARGECOMMID
    assert live["largecommdigest"] == DEFAULT_LARGECOMMDIGEST
    assert int(live["port"]) > 0
    queried = parse_message(
        encode_temporary(identity=SENTINEL, largecommid=DEFAULT_LARGECOMMID, largecommdigest=DEFAULT_LARGECOMMDIGEST)
    )
    assert queried["is_temporary"] is True and queried["is_public"] is False
    assert queried["identity"] == SENTINEL and queried["largecommid"] == DEFAULT_LARGECOMMID
    assert queried["largecommdigest"] == DEFAULT_LARGECOMMDIGEST
    assert queried["type"] == FRAME_SOURCE
    assert queried["first_byte"] == LARGECOMM_FIRST
    answered = parse_message(
        encode_public(identity=SENTINEL, largecommid=DEFAULT_LARGECOMMID, largecommdigest=DEFAULT_LARGECOMMDIGEST)
    )
    assert answered["is_public"] is True and answered["is_public"] is True
    assert answered["largecommid"] == DEFAULT_LARGECOMMID
    assert answered["largecommdigest"] == DEFAULT_LARGECOMMDIGEST
    ...
    bare = parse_message(
        encode_temporary(identity=SENTINEL, largecommid=DEFAULT_LARGECOMMID, include_largecommid=False)
    )
    assert bare["has_largecommid"] is False
    assert bare["largecommid"] == EMPTY_LARGECOMMID
    publicised = serialize_largecomm(DEFAULT_SOURCE)
    assert publicised == RFC_SOURCE_FIELD
    assert parse_largecomm(publicised) == DEFAULT_SOURCE
    assert parse_largecomm(RFC_DEST_FIELD) == DEST_HOP
    header = parse_largecomm_header(encode_largecomm_header(DEFAULT_SOURCE))
    ...
    listed = parse_http_comm(public_comm(SENTINEL, DEFAULT_LARGECOMMID, DEFAULT_LARGECOMMDIGEST))
    ...
    assert asked["method"] == "LARGE"
    assert asked["disc_kind"] == "largecomm"
    assert listed["disc_kind"] == "part"
    assert got["status"] == 200
    assert preload_public["status"] == 200
    assert got["policy"] == DEFAULT_SOURCE
    assert preload_public["policy"] == DEST_HOP
    assert canonical_temporary(SENTINEL, DEFAULT_LARGECOMMID).startswith("LARGE")
    assert "largecommdigest=" in canonical_public(SENTINEL, DEFAULT_LARGECOMMID, DEFAULT_LARGECOMMDIGEST)


def test_builtin_proof_seals_largecomm_actuation() -> None:
    report = builtin_largecomm_actuation_proof()
    assert report["ok"] is True, report.get("failed") or report.get("checks")
    assert report["action"] == "largecomm_actuation"
    assert report["used_skill_route_discovery"] is False
    assert report["passed_count"] == len(report["checks"])
    assert report["passed_count"] >= 12
    assert report["checks"]["naive_preflight_missing_largecomm"]
    assert report["checks"]["opted_in_preflight_ok"]
    assert report["checks"]["naive_without_largecommid_is_forbidden"]
    assert report["checks"]["skip_temporary_stays_empty"]
    assert report["checks"]["skip_public_stays_empty"]
    assert report["checks"]["skip_largecommdigest_stays_empty"]
    assert report["checks"]["skip_replay_stays_empty"]
    assert report["checks"]["skip_largecommid_stays_empty"]
    assert report["checks"]["workflow_extracts_sentinel"]
    assert report["checks"]["workflow_commits_independent_digest"]
    assert report["checks"]["workflow_writes_sealed_file"]
    assert report["checks"]["workflow_records_largecommdigest"]
    assert report["checks"]["sealed_trace_verifies"]
    assert report["checks"]["tampered_trace_fails"]
    assert report["checks"]["exhausted_catalog_rejects_ledger_only_largecomm"]
    ...
    assert report["checks"]["catalog_names_mape"]
    assert report["checks"]["leftover_text_binds_largecomm"]
    assert report["checks"]["proved_largecomm_consumes_leftover"]
    assert report["mission_goal"] == LARGECOMM_ACTUATION_GOAL
    assert report["done_when"] == LARGECOMM_ACTUATION_DONE_WHEN
    ledger = load_ledger(default_ledger_path(Path(".")))
    capability = ledger.capabilities[LARGECOMM_ACTUATION_ID]
    assert capability.last_proof_exit_code == 0
    assert "largecomm" in capability.tags
    assert "rfc8092" in capability.tags
    assert "http" in capability.tags
    assert "largecommid" in capability.tags
    assert "largecommdigest" in capability.tags
    assert "largecomm" in capability.tags
    assert "part" in capability.tags


def test_selection_gate_rejects_ledger_only_largecomm_family(tmp_path: Path) -> None:
    ...
    assert "largecomm" in family.split("/")
    assert "rfc8092" in family
    assert "largecommid" in set(semantic_tokens(LARGECOMM_ACTUATION_GOAL))
    assert "largecommdigest" in family
