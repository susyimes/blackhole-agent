...
| `latest-self-evolution-manifest.json` | replay metadata for a Codex self-evolution run, including source digest, branch, target HEAD, evidence URLs, validation gates, task path, timing, proposal controls, and `replayable_validation_report` with pre-adoption risk review, startup/import health checks, runtime capability-change fields, completion requirements, and a pending adoption decision |
