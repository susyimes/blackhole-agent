...
def builtin_pcep_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="pcep-bind-") as tmp:
        ...
        gate = assess_mission_selection(pbb, PCEP_ACTUATION_GOAL, PCEP_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(pbb)
    checks["exhausted_catalog_rejects_ledger_only_pcep"] = (
        not gate.accepted
        and live_goal != PCEP_ACTUATION_GOAL
        and PCEP_ACTUATION_ID not in live_done
