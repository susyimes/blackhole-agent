...
- an :class:`AcquisitionSpec` declares only what cannot be machine-derived:
  the package source (directory or sdist tarball), the import name, the entry
  callable, the state keys, and the probe inputs;
- synthesis stages the package source into a scratch tree, writes a generic
  state-threading adapter (JSON state on stdin, JSON fragment on stdout), and
  **derives** the frozen case expectations by executing the probes against
  the real package — expectations are measured from observed behavior, never
  hand-written;
...
ADAPTER_NAME = "acquisition_adapter.py"
...
def synthesize_adapter_source(spec: AcquisitionSpec) -> str:
    ...
    return _ADAPTER_TEMPLATE.format(slug=spec.slug, config=json.dumps(config, indent=2, sort_keys=True))
...
def _run_probe(staged_dir: Path, spec: AcquisitionSpec, probe: Mapping[str, Any]) -> dict[str, Any]:
    ...
    command = [sys.executable, ADAPTER_NAME]
    env = dict(os.environ)
...
def synthesize_acquisition(spec: AcquisitionSpec, staging_root: Path) -> dict[str, Any]:
    ...
    (staged_dir / ADAPTER_NAME).write_text(synthesize_adapter_source(spec), encoding="utf-8")
...
def builtin_acquisition_plane_proof() -> dict[str, Any]:
    ...
    expectation fails re-execution. Live half: the stewardship acquisitions
    run end-to-end (unplannable before, solved after, ablation breaks) and
    the sealed report verifies.
    """
...
def register_acquisition_plane_capability(repo_root: Path = REPO_ROOT) -> dict[str, Any]:
    ...
    capability = Capability(
        ...
        description=(
            ...
            "frozen expectations are derived by executing the real package, and the result "
            "passes through the absorption plane's vendoring, digest sealing, and honesty "
            "scenario. Specs whose probes fail are refused before any ledger write."
        ),
        kind="python",
        ...
        capability_delta=(
            "External packages that ship no absorption contract are now acquirable: "
            "stewardship-staged sdists (tomli, Python-Markdown) become proved, invocable, "
            "application-plane-composable capabilities with machine-derived frozen cases."
        ),
