...
def evaluate_skill_route_discovery_lane(raw_input: dict[str, Any], *, source_path: Path) -> dict[str, Any]:
    ...
    pass1_validation_queue = skill_route_discovery_pass1_validation_queue(
        raw_input=raw_input,
        pass_validation_replay_queue=pass_validation_replay_queue,
        mixed_local_lane_probe=mixed_local_lane_probe,
        profile_lane_acceptance_contract=profile_lane_acceptance_contract,
    ...
    *,
    ...
    return {
        ...
        "operator_checkpoint_list": {
            ...
            "status": "ready" if ready else "not_applicable" if current_pass != 2 else "blocked",
            "decision": "operator_can_replay_pass2_checkpoints"
            if ready
            else "repair_pass2_checkpoints_before_replay",
...
def skill_route_discovery_pass3_handoff_packet(
    *,
    raw_input: dict[str, Any],
    current_action: dict[str, Any],
    pass_validation_replay_queue: dict[str, Any],
    ...
    for raw_row in queue_rows:
        ...
        selected_lane = optional_string(row.get("selected_local_lane")) or ""
        if selected_lane not in allowed_lanes:
            diagnostics.append("queued_lane_not_bounded")
            continue
        rows.append(
            {
                "queue_position": int(row.get("queue_position") or len(rows) + 1),
    ...
    if pass_validation_replay_queue.get("status") != "ready":
        diagnostics.append("pass_validation_replay_queue_not_ready")
    mixed_probe_status = str(mixed_local_lane_probe.get("status") or "")
    mixed_probe_primary_route = optional_string(mixed_local_lane_probe.get("primary_route")) or "skill_route_discovery"
    mixed_probe_secondary_status = (
        optional_string(mixed_local_lane_probe.get("secondary_lane_status"))
        or "blocked_until_local_corroboration"
    )
    mixed_probe_bounded_review = (
        mixed_probe_status == "review"
        and mixed_probe_primary_route == "skill_route_discovery"
        and mixed_probe_secondary_status == "blocked_until_local_corroboration"
    )
    if mixed_probe_status not in {"ready", "blocked"} and not mixed_probe_bounded_review:
    ...
    selected_rows = [row for row in rows if row["queue_role"] == "selected_current_pass_lane"]
    queued_rows = [row for row in rows if row["queue_role"] == "queued_bounded_lane"]
    ready = current_pass == 3 and total_passes >= 4 and bool(selected_rows) and not diagnostics
    route_profiles = sorted(
        {
            profile
            for row in rows
    ...
    local_validation_probe = skill_route_discovery_pass3_local_validation_probe(
        ready=ready,
        profile_activation_gates=profile_activation_gates,
        profile_validation_proof=profile_validation_proof,
    ...
    return {
        ...
        "status": "ready" if ready else "not_applicable" if current_pass != 3 else "blocked",
        "decision": "continue_active_and_queued_bounded_lanes_to_final_pass"
        if ready
        else "repair_pass3_handoff_before_final_pass",
        "theme": optional_string(window.get("theme")) or optional_string(current_action.get("theme")) or "",
        "current_pass": current_pass,
        "next_pass": int(current_action.get("next_pass") or 0),
        "total_passes": total_passes,
...
def skill_route_discovery_pass3_local_validation_probe(
    *,
    ...
    """Expose one pass-3 probe that maps skill evidence to bounded replay lanes."""
    ...
    runbook_by_profile: dict[str, dict[str, Any]] = {}
    for raw_step in runbook_rows:
        step = raw_step if isinstance(raw_step, dict) else {}
        for profile in string_list(step.get("route_profiles")):
            runbook_by_profile.setdefault(profile, step)

    rows: list[dict[str, Any]] = []
    diagnostics: list[str] = []
    for raw_gate in gate_rows:
        gate = raw_gate if isinstance(raw_gate, dict) else {}
        profile = optional_string(gate.get("route_profile")) or ""
        proof = proof_by_profile.get(profile, {})
        runbook_step = runbook_by_profile.get(profile, {})
        selected_lanes = string_list(gate.get("selected_local_lanes"))
        selected_lane = (
            selected_lanes[0]
            if selected_lanes
            else optional_string(proof.get("selected_local_lane")) or ""
        )
        route_probe_decision = optional_string(gate.get("route_probe_decision")) or "skill_route_discovery"
        acceptance_ready = gate.get("acceptance_contract_ready") is True
        proof_ready = proof.get("status") == "ready"
        runbook_ready = runbook_step.get("status") == "ready"
        selected_lane_bounded = selected_lane in SKILL_ROUTE_DISCOVERY_ALLOWED_LANES
        blockers = sorted(
            dict.fromkeys(
                string_list(gate.get("activation_blockers"))
                + string_list(proof.get("blockers"))
                + string_list(runbook_step.get("blockers"))
                + ([] if selected_lane_bounded else ["unbounded_or_missing_local_lane"])
                + ([] if acceptance_ready else ["acceptance_contract_not_ready"])
                + ([] if proof_ready else ["local_artifact_proof_not_ready"])
                + ([] if runbook_ready else ["promotion_runbook_step_not_ready"])
                + (
                    []
                    if profile != "codex_workflow_gate" or route_probe_decision == "skill_route_discovery_first"
                    else ["skill_route_discovery_first_not_confirmed"]
                )
            )
        )
        diagnostics.extend(f"{profile}:{blocker}" for blocker in blockers if profile)
        rows.append(
            {
                "route_profile": profile,
                "status": "ready" if ready and not blockers else "blocked",
                "decision": "validate_local_lane_before_activation"
                if ready and not blockers
                else "repair_local_validation_probe_before_activation",
                "selected_local_lane": selected_lane,
                ...
                "queue_roles": string_list(gate.get("queue_roles")),
                "queue_fingerprint": optional_string(runbook_step.get("queue_fingerprint")) or "",
                "evidence_ref_mode": "selected_item_ids_only",
                ...
                "required_validation": string_list(gate.get("required_validation")),
                "required_validation_hashes": [
                    stable_text_hash(command) for command in string_list(gate.get("required_validation"))
                ],
                "provider_runtime_replay_commands": string_list(gate.get("provider_runtime_replay_commands")),
                "acceptance_contract_ready": acceptance_ready,
                "local_artifact_proof_present": proof.get("local_artifact_proof_present") is True,
                "promotion_runbook_step_ready": runbook_ready,
                "blockers": blockers,
                "local_validation_required": True,
                "body_free": True,
                "runtime_action": "none",
    ...
    probe_ready = (
        ...
        and promotion_runbook.get("status") == "ready"
    )
    return {
        "controller_surface": "skill_route_discovery_pass3_local_validation_probe",
        "status": "ready" if probe_ready else "blocked",
        "decision": "local_validation_probe_ready_for_activation_review"
        if probe_ready
        else "repair_local_validation_probe_before_activation_review",
...
def skill_route_discovery_pass3_profile_validation_proof(
    *,
    ready: bool,
    raw_input: dict[str, Any],
    rows: list[dict[str, Any]],
    ...
    """Prove each pass-3 route profile has a bounded local lane before activation."""

    from blackhole_agent.skill_routing import SKILL_ROUTE_DISCOVERY_ALLOWED_LANES

    local_artifact_proofs = raw_input.get("local_artifact_proofs")
    local_artifact_proofs = local_artifact_proofs if isinstance(local_artifact_proofs, list) else []
    proof_lanes = {
        str(proof.get("proposal_kind") or "")
        for proof in local_artifact_proofs
    ...
    for row in rows:
        ...
        for profile in string_list(row.get("route_profiles")):
            ...
            entry["queue_roles"].append(optional_string(row.get("queue_role")) or "queued_bounded_lane")
    ...
    proof_diagnostics = list(diagnostics)
    allowed_lanes = set(SKILL_ROUTE_DISCOVERY_ALLOWED_LANES)
    for profile in sorted(set(source_rows) | set(gate_by_profile)):
        source_row = source_rows.get(profile, {})
        gate_row = gate_by_profile.get(profile, {})
        selected_lanes = sorted(dict.fromkeys(string_list(source_row.get("selected_local_lanes"))))
        lane = selected_lanes[0] if len(selected_lanes) == 1 else ""
        row_blockers: list[str] = []
        if not lane or lane not in allowed_lanes:
            row_blockers.append("missing_bounded_selected_lane")
        if lane and lane not in proof_lanes:
            row_blockers.append("missing_local_artifact_proof_for_lane")
        if not string_list(source_row.get("evidence_item_ids")):
            row_blockers.append("missing_selected_item_ids")
        if not string_list(source_row.get("candidate_source_hashes")):
            row_blockers.append("missing_candidate_source_hashes")
        if gate_row.get("acceptance_contract_ready") is not True:
            row_blockers.append("profile_acceptance_contract_not_ready")
        if not string_list(source_row.get("required_validation")):
            row_blockers.append("missing_required_validation")

        proof_diagnostics.extend(f"{profile}:{blocker}" for blocker in row_blockers)
        proof_rows.append(
            {
                "route_profile": profile,
                "status": "ready" if ready and not row_blockers else "blocked",
                "decision": "profile_has_bounded_local_validation_proof"
                if ready and not row_blockers
                else "repair_profile_validation_proof_before_activation",
                "selected_local_lane": lane,
                "queue_roles": sorted(dict.fromkeys(string_list(source_row.get("queue_roles")))),
                "validation_gate": optional_string(gate_row.get("validation_gate")) or "",
                "evidence_ref_mode": "selected_item_ids_only",
                "evidence_item_ids": sorted(dict.fromkeys(string_list(source_row.get("evidence_item_ids")))),
                "candidate_source_hashes": sorted(
                    dict.fromkeys(string_list(source_row.get("candidate_source_hashes")))
                ),
                "local_artifact_proof_present": bool(lane and lane in proof_lanes),
                "required_validation": sorted(
                    dict.fromkeys(string_list(source_row.get("required_validation")))
                ),
                "provider_runtime_replay_commands": sorted(
                    dict.fromkeys(string_list(source_row.get("provider_runtime_replay_commands")))
                ),
                "acceptance_contract_ready": gate_row.get("acceptance_contract_ready") is True,
    ...
    blocked_rows = [row["route_profile"] for row in proof_rows if row["status"] != "ready"]
    proof_ready = bool(proof_rows) and ready and not blocked_rows
    return {
        "controller_surface": "skill_route_discovery_pass3_profile_validation_proof",
        "status": "ready" if proof_ready else "blocked",
        "decision": "profiles_have_bounded_local_validation_proof"
        if proof_ready
        else "repair_profile_validation_proof_before_final_activation",
        "validation_gate": "focused-evidence-review",
        "profile_count": len(proof_rows),
        "ready_profile_count": len(proof_rows) - len(blocked_rows),
        "blocked_profile_count": len(blocked_rows),
        "blocked_profiles": blocked_rows,
        "allowed_local_lanes": list(SKILL_ROUTE_DISCOVERY_ALLOWED_LANES),
        "local_artifact_proof_lanes": sorted(lane for lane in proof_lanes if lane in allowed_lanes),
        "rows": proof_rows,
        "diagnostics": sorted(dict.fromkeys(proof_diagnostics)),
        "local_validation_required": True,
        "body_free": True,
        "runtime_action_allowed": False,
        "external_skill_activation_allowed": False,
...
def skill_route_discovery_pass1_validation_queue(
    *,
    raw_input: dict[str, Any],
    pass_validation_replay_queue: dict[str, Any],
    mixed_local_lane_probe: dict[str, Any],
    ...
    for row in queue_rows:
        if not isinstance(row, dict):
            continue
        for item_id in string_list(row.get("evidence_item_ids")):
            rows_by_evidence_id[item_id] = row

    rows: list[dict[str, Any]] = []
    diagnostics: list[str] = []
    for proposal_id in anchoring_proposals:
        proposal_lower = proposal_id.casefold()
        if "general-agent" in proposal_lower or "omnigent" in proposal_lower:
            rows.append(
                {
        ...
        replay_row = rows_by_evidence_id.get(proposal_id)
        if not replay_row:
            diagnostics.append(f"{stable_text_hash(proposal_id)}:anchoring_proposal_not_in_replay_queue")
            rows.append(
                {
                    "proposal_id": proposal_id,
                    ...
                    "route_profiles": [],
                    "evidence_ref_mode": "selected_item_ids_only",
                    "evidence_item_ids": [proposal_id],
                    "evidence_item_id_count": 1,
                    "ready_for_local_replay": False,
                    "diagnostics": ["anchoring_proposal_not_in_replay_queue"],
        ...
        selected_lane = optional_string(replay_row.get("selected_local_lane")) or "none"
        row_diagnostics = string_list(replay_row.get("diagnostics"))
        if selected_lane not in allowed_lanes:
            row_diagnostics.append("selected_local_lane_not_bounded")
        if replay_row.get("local_validation_required") is not True:
            row_diagnostics.append("local_validation_not_required")
        if str(replay_row.get("runtime_action") or "none") != "none":
            row_diagnostics.append("runtime_action_requested")
        if replay_row.get("external_skill_activation_allowed") is not False:
            row_diagnostics.append("external_skill_activation_not_denied")
        if row_diagnostics:
            diagnostics.extend(f"{stable_text_hash(proposal_id)}:{diagnostic}" for diagnostic in row_diagnostics)

        route_profiles = skill_route_discovery_pass1_profiles_for_proposal(
            proposal_id,
        ...
        rows.append(
            {
                ...
                "provider_runtime_replay_commands": string_list(
                    replay_row.get("provider_runtime_replay_commands")
                ),
                "local_validation_required": True,
                "runtime_action": "none",
                "external_skill_activation_allowed": False,
    ...
    ready = (
        current_pass == 1
        and bool(skill_rows)
        and all(row["ready_for_local_replay"] for row in skill_rows)
    ...
    return {
        ...
        "status": "ready" if ready else "not_applicable" if current_pass != 1 else "blocked",
        "decision": "pass1_skill_route_validation_queue_ready"
        if ready
        else "repair_pass1_validation_queue_before_replay",
        "current_pass": current_pass,
        "total_passes": int(window.get("total_passes") or 0),
        ...
        "ready_skill_route_row_count": sum(1 for row in skill_rows if row["ready_for_local_replay"]),
        "selected_local_lanes": sorted(
            {row["selected_local_lane"] for row in skill_rows if row["selected_local_lane"] in allowed_lanes}
        ),
        "route_profiles": sorted(
            {
                profile
...
def skill_route_discovery_pass1_profiles_for_proposal(proposal_id: str, route_profiles: list[str]) -> list[str]:
    """Narrow grouped replay profiles for proposal-level pass-1 queue rows."""
    ...
    if "compass" in proposal_lower:
        ...
        narrowed = ["codex_workflow_gate"]
    else:
        narrowed = []

    available = set(route_profiles)
    filtered = [profile for profile in narrowed if profile in available]
    return filtered or route_profiles
...
def skill_route_discovery_capability_window_completion(
    *,
    ...
    provider_runtime_final_diagnostics = skill_route_discovery_provider_runtime_final_diagnostics(
        theme=theme,
        status=status,
        decision=decision,
        planned_window_complete=planned_window_complete,
    ...
    if theme == "provider-runtime-control":
        completion_handoff["provider_runtime_final_diagnostics"] = provider_runtime_final_diagnostics
...
def skill_route_discovery_completion_report(
    *,
    ...
    activation_handoff = skill_route_discovery_completion_activation_handoff(
...
def skill_route_discovery_completion_activation_handoff(
    *,
    ...
    """Package the final bounded-lane supervisor handoff without activation authority."""
    ...
    lane_replay_steps = sorted(
        {
            step
            for row in ready_rows
            for step in [optional_string(row.get("supervisor_replay_step")) or ""]
            if step
        }
    )
    ready = (
    ...
    decision = (
        "supervisor_may_replay_bounded_local_lanes_after_validation"
        if ready
        else "repair_completion_blockers_before_supervisor_replay"
    )
    next_action = (
        "external_supervisor_replay_bounded_local_lanes"
        if ready
        else "external_supervisor_repair_or_replay_local_validation"
    )

    return {
        "controller_surface": "skill_route_discovery_completion_activation_handoff",
        "status": "ready" if ready else "blocked",
        "decision": decision,
        "completion_next_action": completion_next_action,
        ...
        or "",
        "selected_local_lanes": sorted(dict.fromkeys(selected_local_lanes)),
        "lane_count": len(closure_rows),
        "ready_lane_count": len(ready_rows),
        "blocked_lane_count": len(closure_rows) - len(ready_rows),
        "replay_step_count": len(lane_replay_steps),
        "replay_step_hashes": [stable_text_hash(step) for step in lane_replay_steps],
        "completion_blocker_count": len(blocked_reasons),
        "completion_blocker_hashes": [stable_text_hash(reason) for reason in blocked_reasons],
        "required_validation": skill_route_discovery_preactivation_validation_commands(),
        "provider_runtime_replay_commands": [
...
def skill_route_discovery_provider_runtime_final_diagnostics(
    *,
    theme: str,
    status: str,
    decision: str,
    ...
    """Summarize final provider/runtime replay state as one supervisor packet."""

    provider_runtime_theme = theme == "provider-runtime-control"
    panel_status = optional_string(provider_runtime_diagnostic_panel.get("status")) or "blocked"
    gate_status = optional_string(provider_runtime_sample_gate.get("status")) or "blocked"
    handoff_status = optional_string(provider_runtime_completion_handoff.get("status")) or "blocked"
    sample_route_status = optional_string(provider_runtime_replay_sample.get("route_status")) or "missing"
    sample_ready_for_local_replay = provider_runtime_replay_sample.get("ready_for_local_replay") is True
    sample_ready_for_supervisor_promotion = (
        provider_runtime_replay_sample.get("ready_for_supervisor_promotion") is True
    ...
    if not provider_runtime_theme:
        ...
        diagnostic_decision = "provider_runtime_final_diagnostics_not_required_for_theme"
        supervisor_next_action = optional_string(provider_runtime_completion_handoff.get("supervisor_next_action")) or ""
    elif ready:
        diagnostic_status = "ready"
        diagnostic_decision = "provider_runtime_final_diagnostics_ready_for_supervisor_replay"
        supervisor_next_action = "supervisor_replay_provider_runtime_preflight_then_promote"
    elif gate_status != "ready" or not sample_ready_for_supervisor_promotion:
        diagnostic_status = "blocked"
        diagnostic_decision = "repair_provider_runtime_sample_before_supervisor_replay"
        supervisor_next_action = "replay_provider_runtime_preflight_and_recovery_summary"
    else:
        diagnostic_status = "blocked"
        diagnostic_decision = "repair_completion_handoff_before_provider_runtime_replay"
        supervisor_next_action = optional_string(
            provider_runtime_completion_handoff.get("supervisor_next_action")
        ) or "repair_provider_runtime_completion_handoff"

    operator_replay_workflow = skill_route_discovery_provider_runtime_operator_replay_workflow(
    ...
    return {
        "controller_surface": "provider_runtime_final_diagnostics",
        ...
        "provider_runtime_diagnostic_panel_status": panel_status,
        "provider_runtime_sample_gate_status": gate_status,
        "provider_runtime_completion_handoff_status": handoff_status,
        "provider_runtime_sample_route_status": sample_route_status,
        "provider_runtime_sample_ready_for_local_replay": sample_ready_for_local_replay,
        "provider_runtime_sample_ready_for_supervisor_promotion": sample_ready_for_supervisor_promotion,
        "success_claim_allowed": success_claim_allowed,
        ...
        "recovery_hint_codes": recovery_hint_codes,
        "recovery_hint_code_hashes": [stable_text_hash(code) for code in recovery_hint_codes],
        "operator_replay_workflow": operator_replay_workflow,
        "required_validation": skill_route_discovery_preactivation_validation_commands(),
        "provider_runtime_replay_commands": [
            PROVIDER_RUNTIME_PREFLIGHT_COMMAND,
            PROVIDER_RUNTIME_RECOVERY_SUMMARY_COMMAND,
...
def skill_route_discovery_provider_runtime_operator_replay_workflow(
    *,
    diagnostic_status: str,
    diagnostic_decision: str,
    supervisor_next_action: str,
    provider_runtime_theme: bool,
    planned_window_complete: bool,
    proposal_kinds: list[str],
    route_profiles: list[str],
    diagnostics: list[str],
    ...
    """Expose final provider-runtime replay as an ordered, body-free workflow."""
    ...
    steps = [
        {
            "step": "provider_runtime_sample_gate",
            "status": gate_status,
            "decision": optional_string(provider_runtime_sample_gate.get("decision")) or "",
            "ready": gate_status == "ready",
            "recovery_hint_code_hashes": [
                stable_text_hash(code)
                for code in string_list(provider_runtime_sample_gate.get("recovery_hint_codes"))
            ],
        },
        {
            "step": "provider_runtime_recovery_summary",
            "status": sample_route_status,
            "decision": optional_string(provider_runtime_replay_sample.get("supervisor_decision")) or "",
            "ready": sample_ready_for_local_replay and sample_ready_for_supervisor_promotion,
            "sample_provided": sample_provided,
            "success_claim_allowed": success_claim_allowed,
            "recovery_hint_code_hashes": [
                stable_text_hash(code)
                for code in string_list(provider_runtime_replay_sample.get("recovery_hint_codes"))
            ],
        },
        {
            "step": "provider_runtime_diagnostic_panel",
            "status": panel_status,
            "decision": optional_string(provider_runtime_diagnostic_panel.get("decision")) or "",
            "ready": panel_status == "ready",
            "recovery_hint_code_hashes": [
                stable_text_hash(code)
                for code in string_list(provider_runtime_diagnostic_panel.get("recovery_hint_codes"))
            ],
        },
        {
            "step": "completion_handoff",
            "status": handoff_status,
            "decision": optional_string(provider_runtime_completion_handoff.get("decision")) or "",
            "ready": handoff_status == "ready",
            "recovery_hint_code_hashes": [
                stable_text_hash(code)
                for code in string_list(provider_runtime_completion_handoff.get("recovery_hint_codes"))
            ],
        },
    ]
    ready_step_count = sum(1 for step in steps if step["ready"])
    blocked_step_count = len(steps) - ready_step_count

    return {
        "controller_surface": "provider_runtime_operator_replay_workflow",
        "status": diagnostic_status,
        "decision": diagnostic_decision,
        "supervisor_next_action": supervisor_next_action,
        "provider_runtime_theme": provider_runtime_theme,
        "planned_window_complete": planned_window_complete,
        "proposal_kind_count": len(proposal_kinds),
        "proposal_kind_hashes": [stable_text_hash(kind) for kind in proposal_kinds],
        "route_profile_count": len(route_profiles),
        "route_profile_hashes": [stable_text_hash(profile) for profile in route_profiles],
        "step_count": len(steps),
        ...
        "steps": steps,
        "completion_blocker_count": len(completion_blockers),
        "completion_blocker_hashes": [stable_text_hash(blocker) for blocker in completion_blockers],
        "recovery_hint_count": len(recovery_hint_codes),
        "recovery_hint_codes": recovery_hint_codes,
        "recovery_hint_code_hashes": [stable_text_hash(code) for code in recovery_hint_codes],
        "provider_runtime_replay_commands": [
...
def skill_route_discovery_profile_lane_acceptance_contract(
    *,
    route_profile_review: dict[str, Any],
    mixed_local_lane_probe: dict[str, Any],
) -> dict[str, Any]:
    """Summarize which bounded lanes each route profile may enter."""

    from blackhole_agent.skill_routing import (
        SKILL_ROUTE_DISCOVERY_ALLOWED_LANES,
        SKILL_ROUTE_DISCOVERY_ROUTE_PROFILE_VALIDATION_CONTRACTS,
    )
    ...
    rows: list[dict[str, Any]] = []
    diagnostics: list[str] = []
    for index, raw_row in enumerate(review_rows):
        review_row = raw_row if isinstance(raw_row, dict) else {}
        profile = optional_string(review_row.get("route_profile")) or "generic_skill_workflow"
        ...
        preferred_lanes = [
            lane
            for lane in [str(value) for value in contract.get("preferred_lanes", ())]
            if lane in proposal_kinds
        ]
        selected_first_lane = preferred_lanes[0] if preferred_lanes else ""
        validation_scope = f"local_{selected_first_lane}_lane_only" if selected_first_lane else ""
        first_route_required = profile == "codex_workflow_gate"
        first_route_confirmed = True
        if first_route_required:
            first_route_confirmed = any(
                isinstance(row, dict)
                and profile in string_list(row.get("route_profiles"))
                and str(row.get("route_probe_decision") or "") == "skill_route_discovery_first"
                and str(row.get("primary_route") or "") == "skill_route_discovery"
        ...
        acceptance_gates = {
            "profile_review_ready": route_profile_review.get("status") == "ready",
            "bounded_lanes_only": bool(proposal_kinds)
            and set(proposal_kinds) <= allowed_lanes,
            "metadata_complete": review_row.get("metadata_complete") is True,
            "local_validation_required": review_row.get("local_validation_required") is True,
            "runtime_action_none": str(review_row.get("runtime_action") or "") == "none",
            "first_route_confirmed": first_route_confirmed,
            "external_skill_activation_denied": review_row.get("external_skill_activation_allowed") is False,
            "provider_runtime_launch_denied": True,
            "remote_execution_denied": True,
            "raw_source_urls_not_exported": review_row.get("raw_source_urls_exported") is False,
            "raw_target_paths_not_exported": review_row.get("raw_target_paths_exported") is False,
            "raw_upstream_body_not_exported": review_row.get("raw_upstream_body_exported") is False,
        }
        row_diagnostics = [
            gate for gate, passed in acceptance_gates.items() if passed is not True
        ]
        diagnostics.extend(f"rows[{index}].{profile}:{diagnostic}" for diagnostic in row_diagnostics)
        rows.append(
            {
                "route_profile": profile,
                "status": "ready" if not row_diagnostics else "blocked",
                "decision": "profile_lane_ready_for_bounded_local_validation"
                if not row_diagnostics
                else "repair_profile_lane_before_local_validation",
                "validation_gate": str(contract.get("validation_gate") or ""),
                "blocked_activation_reason": str(contract.get("blocked_activation_reason") or ""),
                "allowed_local_lanes": proposal_kinds,
                "preferred_local_lanes": preferred_lanes,
                "selected_first_local_lane": selected_first_lane,
                "validation_scope": validation_scope,
                "required_metadata": [str(value) for value in contract.get("required_metadata", ())],
                "expected_metadata": string_list(review_row.get("expected_metadata")),
                "acceptance_gates": acceptance_gates,
                "diagnostics": row_diagnostics,
                "local_validation_required": True,
                "runtime_action": "none",
    ...
    ready = bool(rows) and not diagnostics
    return {
        "controller_surface": "skill_route_discovery_profile_lane_acceptance_contract",
        "status": "ready" if ready else "blocked",
        "decision": "profile_lanes_ready_for_bounded_local_validation"
        if ready
        ...
        "allowed_local_lanes": list(SKILL_ROUTE_DISCOVERY_ALLOWED_LANES),
        "route_profiles": [row["route_profile"] for row in rows],
        "selected_first_local_lanes": sorted(
            {
                row["selected_first_local_lane"]
                for row in rows
                if row["selected_first_local_lane"]
            }
        ),
        "evidence_ref_mode": "selected_item_ids_only",
        "rows": rows,
