...
def tower_unique_coverage_is_saturated(
    ledger: CapabilityLedger,
    campaign: LocalCampaign | None = None,
) -> bool:
    """True when no novelty-ranked program tower remains to promote."""
...
def saturate_program_towers(
    root: Path,
    ledger: CapabilityLedger,
    campaign: LocalCampaign,
) -> list[str]:
    """Promote every remaining novel consecutive-pair program tower."""
    ...
    while True:
        ...
        if not tower_id:
            return promoted
        proved = promote_and_prove_program_tower(Path(root), ledger, tower_id)
        if not proved:
            return promoted
