...
def builtin_ohsvcb_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="ohsvcb-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, OHSVCB_ACTUATION_GOAL, OHSVCB_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_ohsvcb"] = (
        not gate.accepted
        and live_goal != OHSVCB_ACTUATION_GOAL
        and OHSVCB_ACTUATION_ID not in live_done
