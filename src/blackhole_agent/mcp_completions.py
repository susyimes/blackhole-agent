...
def builtin_mcp_completions_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="mcp-completions-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, MCP_COMPLETIONS_GOAL, MCP_COMPLETIONS_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_completions"] = (
        not gate.accepted
        and live_goal != MCP_COMPLETIONS_GOAL
        and MCP_COMPLETIONS_ID not in live_done
