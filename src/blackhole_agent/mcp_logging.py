...
def builtin_mcp_logging_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="mcp-logging-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, MCP_LOGGING_GOAL, MCP_LOGGING_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_logging"] = (
        not gate.accepted
        and live_goal != MCP_LOGGING_GOAL
        and MCP_LOGGING_ID not in live_done
