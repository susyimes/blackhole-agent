...
def builtin_sqlite_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="sqlite-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, SQLITE_ACTUATION_GOAL, SQLITE_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_sqlite"] = (
        not gate.accepted
        and live_goal != SQLITE_ACTUATION_GOAL
        and SQLITE_ACTUATION_ID not in live_done
