...
def builtin_ssh_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="ssh-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, SSH_ACTUATION_GOAL, SSH_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_ssh"] = (
        not gate.accepted
        and live_goal != SSH_ACTUATION_GOAL
        and SSH_ACTUATION_ID not in live_done
