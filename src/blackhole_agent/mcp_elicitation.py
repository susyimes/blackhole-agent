...
def builtin_mcp_elicitation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="mcp-elicitation-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, MCP_ELICITATION_GOAL, MCP_ELICITATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_elicitation"] = (
        not gate.accepted
        and live_goal != MCP_ELICITATION_GOAL
        and MCP_ELICITATION_ID not in live_done
