...
def builtin_hitmeter_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="hitmeter-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, HITMETER_ACTUATION_GOAL, HITMETER_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_hitmeter"] = (
        not gate.accepted
        and live_goal != HITMETER_ACTUATION_GOAL
        and HITMETER_ACTUATION_ID not in live_done
