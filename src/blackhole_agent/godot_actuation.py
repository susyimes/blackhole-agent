...
def builtin_godot_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="godot-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, GODOT_ACTUATION_GOAL, GODOT_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_godot"] = (
        not gate.accepted
        and live_goal != GODOT_ACTUATION_GOAL
        and GODOT_ACTUATION_ID not in live_done
