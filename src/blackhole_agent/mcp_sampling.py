...
def builtin_mcp_sampling_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="mcp-sampling-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, MCP_SAMPLING_GOAL, MCP_SAMPLING_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_sampling"] = (
        not gate.accepted
        and live_goal != MCP_SAMPLING_GOAL
        and MCP_SAMPLING_ID not in live_done
