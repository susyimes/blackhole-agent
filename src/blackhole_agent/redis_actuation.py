...
def builtin_redis_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="redis-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, REDIS_ACTUATION_GOAL, REDIS_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_redis"] = (
        not gate.accepted
        and live_goal != REDIS_ACTUATION_GOAL
        and REDIS_ACTUATION_ID not in live_done
