"""Post-recovery resolution-versus-strategy for the absolute total spine.

Closes the restored-but-unresolved cliff: after ``recovery_total_spine``
seals atomic RvP receipts, independently confirm a second recovery, book
each restored pair into a resolution register and pair it with a strategy
(RvS), seal hash-chained atomic resolution receipts bound to the recovery
digests, refuse split / one-sided / mismatched / failed / wrong-root /
tampered resolutions, short-circuit re-resolution, and rebind the depth-28 tip
without skill-route.
"""
...
TOTAL_SPINE_RESOLUTION_IMPL = True
TOTAL_SPINE_RESOLUTION_KIND: str = "total_spine_resolution"
TOTAL_SPINE_RESOLUTION_FILENAME: str = "total-spine-resolution.json"
TOTAL_SPINE_RESOLUTION_MIN_RESOLUTIONS: int = 2
...
def _recovery_digest_of(row: Mapping[str, Any]) -> str:
    return str(
...
def _capability_list(row: Mapping[str, Any]) -> list[str]:
    ...
    if caps:
        return caps
    for leg in row.get("recoveries") or row.get("risks") or row.get("resolutions") or []:
...
def _book_signature(margin: Mapping[str, Any]) -> str:
    ...
    legs = margin.get("recoveries") or margin.get("risks") or []
    sigs: list[str] = []
...
def _rvs_pairs(capabilities: Sequence[str]) -> list[dict[str, Any]]:
        ...
        rows.append(
            {
                ...
                "resolved_ok": bool(row.get("resolved_ok", True)),
                "strategy_ok": bool(row.get("strategy_ok", True)),
...
def _assert_pairs_atomic(pairs: Sequence[Mapping[str, Any]]) -> None:
    if not pairs:
        raise StageRefused(
            "total_spine_resolution_pairs_empty",
            "stress refuses an empty LvC pair book",
        )
    for row in pairs:
        if not isinstance(row, Mapping):
            raise StageRefused(
                "total_spine_resolution_partial",
                "stress refuses a malformed LvC pair",
            )
        resolved_ok = bool(row.get("resolved_ok", True))
        strategy_ok = bool(row.get("strategy_ok", True))
        if resolved_ok != strategy_ok or not bool(row.get("atomic_ok", True)):
            raise StageRefused(
                "total_spine_resolution_partial",
                "stress refuses a split (non-atomic) risk-versus-requirement pair",
            )
        if not resolved_ok or not strategy_ok:
            raise StageRefused(
                "total_spine_resolution_partial",
...
def _resolution_certificate_material(body: Mapping[str, Any]) -> dict[str, Any]:
    """Canonical material for total-spine risk certificate digests."""
    legs = body.get("resolutions") or body.get("legs") or []
    ...
    if isinstance(legs, list):
        for row in legs:
            ...
            collateral_rows.append(
                {
                    "recovery_index": int(row.get("recovery_index") or 0),
                    "recovery_height": int(row.get("recovery_height") or 0),
                    "recovery_digest": str(row.get("recovery_digest") or ""),
                    "bound_recovery_root": str(
                        row.get("bound_recovery_root") or ""
                    ),
                    "bound_delivery_root": str(
                    ...
                    "strategic": bool(row.get("strategic", True)),
                    "atomic_ok": bool(row.get("atomic_ok", True)),
                    ...
                    "parent_resolution_root": str(
                        row.get("parent_resolution_root") or ""
                    ),
                    "recovery_root": str(row.get("recovery_root") or ""),
                    "post_resolution": bool(row.get("post_resolution", True)),
                    "deterministic": bool(row.get("deterministic", True)),
                    "rvs": bool(row.get("rvs", True)),
                }
            )
    return {
        "schema_version": int(body.get("schema_version") or SCHEMA_VERSION),
        "kind": str(body.get("kind") or TOTAL_SPINE_RESOLUTION_KIND),
        ...
        "bound_recovery_root": str(body.get("bound_recovery_root") or ""),
        "bound_custody_root": str(body.get("bound_custody_root") or ""),
        "bound_delivery_root": str(body.get("bound_delivery_root") or ""),
        "recovery_digest": str(body.get("recovery_digest") or ""),
        "delivery_digest": str(body.get("delivery_digest") or ""),
        "parent_resolution_root": str(body.get("parent_resolution_root") or ""),
        "tip_resolution_root": str(body.get("tip_resolution_root") or ""),
        "resolution_height": int(body.get("resolution_height") or 0),
        "resolution_count": int(body.get("resolution_count") or 0),
        ...
        "strategic": bool(body.get("strategic", True)),
        "atomic_ok": bool(body.get("atomic_ok", True)),
        "rvs_ok": bool(body.get("rvs_ok", True)),
        "one_sided": bool(body.get("one_sided", False)),
        "resolutions_ok": bool(body.get("resolutions_ok", True)),
        "resolutions_ok": bool(body.get("resolutions_ok", True)),
        "post_resolution": bool(body.get("post_resolution", True)),
        "deterministic": bool(body.get("deterministic", True)),
        "irreversible": bool(body.get("irreversible", True)),
        "success": bool(body.get("success", True)),
        "resolutions": collateral_rows,
    }


def compute_total_spine_resolution_root(
    ...
    if not risks:
        return "0" * 64
    last = risks[-1]
    tip = str(last.get("recovery_root") or "").strip()
    if tip:
        return tip
    parent = ""
    for idx, row in enumerate(risks):
        body = {
            "recovery_index": int(row.get("recovery_index") or idx),
            "recovery_height": int(row.get("recovery_height") or (idx + 1)),
            "recovery_digest": str(row.get("recovery_digest") or ""),
            "bound_recovery_root": str(row.get("bound_recovery_root") or ""),
...
def book_total_spine_recoveries(
    ...
    *,
    ...
    want = max(int(min_resolutions), TOTAL_SPINE_RESOLUTION_MIN_RESOLUTIONS)
    verified: list[Mapping[str, Any]] = []
    for raw in margins:
        if not isinstance(raw, Mapping):
            continue
        verify = verify_total_spine_recovery_certificate(raw)
        if not verify.get("ok"):
            raise StageRefused(
                "total_spine_resolution_margin_tampered",
                "stress refuses a margin whose digest/chain does not verify",
            )
        if raw.get("restored") is False or raw.get("success") is False:
            raise StageRefused(
                "total_spine_resolution_capital_unrestored",
                "stress refuses an unrestored risk receipt",
            )
        if raw.get("planned") is False or raw.get("rvp_ok") is False:
            raise StageRefused(
                "total_spine_resolution_capital_uncapacitated",
                "stress refuses a risk whose RvA is not complete",
            )
        if raw.get("atomic_ok") is False:
            raise StageRefused(
                "total_spine_resolution_margin_partial",
                "stress refuses a non-atomic margin receipt",
            )
        if int(raw.get("residual") or 0) != 0:
            raise StageRefused(
                "total_spine_resolution_residual",
                "stress refuses a margin with a non-zero residual",
            )
        verified.append(raw)
    if len(verified) < want:
        raise StageRefused(
            "total_spine_resolution_margins_short",
    ...
    if not book_state or not book_action or not book_actuation:
        raise StageRefused(
            "total_spine_resolution_root_missing",
    ...
    parent = str(parent_resolution_root or "")
    for idx, margin in enumerate(verified):
        ...
        if (
            ...
            raise StageRefused(
                "total_spine_resolution_root_mismatch",
        ...
        material = {
            ...
            "recovery_digest": _recovery_digest_of(margin),
            "bound_recovery_root": str(
                margin.get("tip_recovery_root") or ""
            ),
            "bound_delivery_root": str(
                margin.get("bound_delivery_root")
                or margin.get("tip_delivery_root")
        ...
        recovery_root = _sha256_json(material)
        row = dict(material)
        row["recovery_root"] = recovery_root
        row["pairs"] = pairs
...
def seal_total_spine_resolution_certificate(
    body: Mapping[str, Any],
) -> dict[str, Any]:
    """Seal post-recovery LvC log into a tamper-evident receipt."""
    sealed_body = dict(body)
    resolutions = list(sealed_body.get("resolutions") or [])
    if not str(sealed_body.get("tip_resolution_root") or "").strip():
        sealed_body["tip_resolution_root"] = compute_total_spine_resolution_root(
            resolutions
        )
    if not int(sealed_body.get("resolution_count") or 0):
        sealed_body["resolution_count"] = len(resolutions)
    if not int(sealed_body.get("resolution_height") or 0):
        sealed_body["resolution_height"] = len(resolutions)
    material = _resolution_certificate_material(sealed_body)
    material["tip_resolution_root"] = str(sealed_body.get("tip_resolution_root") or "")
    digest = _sha256_json(material)
    sealed = dict(material)
    sealed["resolution_digest"] = digest
    sealed["certificate_hash"] = digest
    sealed["total_spine_resolution"] = True
    sealed["total_spine_resolution_impl"] = TOTAL_SPINE_RESOLUTION_IMPL
    sealed["resolved_at"] = str(body.get("resolved_at") or utc_now_iso())
    sealed["used_skill_route_discovery"] = legacy_pipeline_was_used()
    if resolutions:
        sealed_pairs: list[Any] = []
        for src, dest in zip(resolutions, sealed.get("resolutions") or []):
            if isinstance(src, Mapping) and isinstance(dest, dict):
                if src.get("pairs"):
                    dest["pairs"] = list(src.get("pairs") or [])
                sealed_pairs.append(dest)
        if sealed_pairs:
            sealed["resolutions"] = sealed_pairs
    return sealed


def resolution_certificate_path(root: Path) -> Path:
    """Resolve ``total-spine-resolution.json`` under a recovery/ out root."""
    path = Path(root)
    if path.is_file():
        if path.name == TOTAL_SPINE_RESOLUTION_FILENAME or path.suffix == ".json":
            try:
                probe = json.loads(path.read_text(encoding="utf-8"))
            except (OSError, json.JSONDecodeError):
                probe = None
            if isinstance(probe, Mapping) and (
                str(probe.get("kind") or "") == TOTAL_SPINE_RESOLUTION_KIND
                or path.name == TOTAL_SPINE_RESOLUTION_FILENAME
            ):
                return path
        parent = path.parent
        sibling = parent / TOTAL_SPINE_RESOLUTION_FILENAME
        if sibling.is_file():
            return sibling
        nested = parent / "resolution" / TOTAL_SPINE_RESOLUTION_FILENAME
        if nested.is_file():
            return nested
        grand = parent.parent / "resolution" / TOTAL_SPINE_RESOLUTION_FILENAME
        if grand.is_file():
            return grand
        grand_sib = parent.parent / TOTAL_SPINE_RESOLUTION_FILENAME
        if grand_sib.is_file():
            return grand_sib
        return parent / "resolution" / TOTAL_SPINE_RESOLUTION_FILENAME
    named = path / TOTAL_SPINE_RESOLUTION_FILENAME
    if named.is_file():
        return named
    nested = path / "resolution" / TOTAL_SPINE_RESOLUTION_FILENAME
    if nested.is_file():
        return nested
    return path / "resolution" / TOTAL_SPINE_RESOLUTION_FILENAME


def write_total_spine_resolution_certificate(
    out_root: Path,
    body: Mapping[str, Any],
    *,
    ...
    sealed = seal_total_spine_resolution_certificate(body)
    path = resolution_certificate_path(Path(out_root))
    if path.is_file():
        try:
            existing = load_total_spine_resolution_certificate(path)
        except StageRefused:
            existing = None
        if existing is not None:
            existing_digest = str(
                existing.get("resolution_digest")
                or existing.get("certificate_hash")
                or ""
            )
            new_digest = str(
                sealed.get("resolution_digest")
            ...
            if (
                ...
                existing["resolution_path"] = str(path)
                existing["total_spine_resolution_idempotent"] = True
                return existing
            raise StageRefused(
                "total_spine_resolution_supersession_refused",
    ...
    sealed["resolution_path"] = str(path)
    sealed["total_spine_resolution_idempotent"] = False
    return sealed


def verify_total_spine_resolution_certificate(
    certificate: Mapping[str, Any],
) -> dict[str, Any]:
    """Recompute collateral digest and LvC roots; fail closed on tamper."""
    claimed = str(
        certificate.get("resolution_digest")
        or certificate.get("certificate_hash")
        or ""
    )
    material = _resolution_certificate_material(certificate)
    expected = _sha256_json(material)
    resolutions = list(certificate.get("resolutions") or [])
    recomputed_tip = compute_total_spine_resolution_root(resolutions)
    claimed_tip = str(certificate.get("tip_resolution_root") or "")
    height = int(certificate.get("resolution_height") or 0)
    count = int(certificate.get("resolution_count") or 0)
    ...
    cert_parent = str(certificate.get("parent_resolution_root") or "")
    chain_ok = True
    parent = cert_parent
    book_sig = ""
    for idx, row in enumerate(resolutions):
        ...
        if str(row.get("actuation_digest") or "") != actuation_digest:
            chain_ok = False
            break
        if str(row.get("parent_resolution_root") or "") != parent:
            chain_ok = False
            break
        ...
        if str(row.get("recovery_root") or "") != expected_root:
            chain_ok = False
            break
    ...
    min_ok = count >= TOTAL_SPINE_RESOLUTION_MIN_RESOLUTIONS and height >= count
    resolutions_ok = all(
        isinstance(row, Mapping)
        and bool(row.get("resolved", True))
        and bool(row.get("strategic", True))
        and bool(row.get("atomic_ok", True))
        and bool(row.get("rvs", True))
    ...
    ok = (
        ...
        and str(certificate.get("kind") or "") == TOTAL_SPINE_RESOLUTION_KIND
        and int(certificate.get("schema_version") or 0) == SCHEMA_VERSION
        and certificate.get("irreversible") is True
        and certificate.get("post_resolution") is True
        and certificate.get("deterministic") is True
        and certificate.get("resolved") is True
        and certificate.get("strategic") is True
        and certificate.get("atomic_ok") is True
        and certificate.get("rvs_ok") is True
    ...
    return {
        "ok": ok,
        "action": "verify_total_spine_resolution",
        "claimed_digest": claimed,
        "expected_digest": expected,
        "recovery_root_ok": claimed_tip == recomputed_tip and bool(claimed_tip),
        "recovery_root_ok": claimed_tip == recomputed_tip and bool(claimed_tip),
        "recomputed_tip_resolution_root": recomputed_tip,
        "chain_ok": chain_ok,
        "min_resolutions_ok": min_ok,
        "resolutions_ok": resolutions_ok,
        "rvs_ok": certificate.get("rvs_ok") is True,
        "atomic_ok": certificate.get("atomic_ok") is True,
        "kind_ok": str(certificate.get("kind") or "") == TOTAL_SPINE_RESOLUTION_KIND,
        "schema_ok": int(certificate.get("schema_version") or 0) == SCHEMA_VERSION,
        "irreversible_ok": certificate.get("irreversible") is True,
        "resolved_ok": certificate.get("resolved") is True,
        "strategic_ok": certificate.get("strategic") is True,
        "total_spine_resolution": True,
        "used_skill_route_discovery": legacy_pipeline_was_used(),
    }


def load_total_spine_resolution_certificate(
    path: Path | str,
) -> dict[str, Any]:
    """Load and integrity-check a sealed liquidity receipt."""
    file_path = resolution_certificate_path(Path(path))
    if not file_path.is_file():
        raise StageRefused(
            "total_spine_resolution_missing",
    ...
    try:
        ...
        raise StageRefused(
            "total_spine_resolution_unreadable",
            f"risk certificate unreadable at {file_path}: {exc}",
        ) from exc
    if not isinstance(payload, Mapping):
        raise StageRefused(
            "total_spine_resolution_invalid",
            "risk certificate root must be a JSON object",
        )
    if str(payload.get("kind") or "") != TOTAL_SPINE_RESOLUTION_KIND and not payload.get(
    ...
    verify = verify_total_spine_resolution_certificate(payload)
    if not verify.get("ok"):
        raise StageRefused(
            "total_spine_resolution_tampered",
    ...
    body = dict(payload)
    body["resolution_path"] = str(file_path)
    body["resolution_verify"] = verify
    body["total_spine_resolution_loaded"] = True
    return body


def seal_total_spine_resolution_chain(
    *,
    prior_tip: str,
    ...
    md = str(resolution_digest or "").strip() or ("0" * 64)
    mr = str(tip_resolution_root or "").strip() or ("0" * 64)
    cr = str(bound_recovery_root or "").strip() or ("0" * 64)
    ...
    cd = str(recovery_digest or "").strip() or ("0" * 64)
    dvd = str(delivery_digest or "").strip() or ("0" * 64)
    material = (
        f"risk|{int(bool(short_circuit))}|{int(resolution_height)}|"
    ...
    return {
        ...
        "tip_resolution_root": mr,
        "bound_recovery_root": cr,
...
def annotate_total_spine_resolution(
    body: dict[str, Any],
    *,
    certificate: Mapping[str, Any],
    prior_tip: str,
    ...
    tip_resolution_root = str(certificate.get("tip_resolution_root") or "")
    resolution_height = int(certificate.get("resolution_height") or 0)
    resolution_count = int(certificate.get("resolution_count") or 0)
    ...
    bound_recovery_root = str(certificate.get("bound_recovery_root") or "")
    bound_delivery_root = str(certificate.get("bound_delivery_root") or "")
    actuation_digest = str(certificate.get("actuation_digest") or "")
    recovery_digest = str(certificate.get("recovery_digest") or "")
    delivery_digest = str(certificate.get("delivery_digest") or "")
    chain = seal_total_spine_resolution_chain(
        prior_tip=prior_tip,
        resolution_digest=cst_digest,
        tip_resolution_root=tip_resolution_root,
        bound_recovery_root=bound_recovery_root,
    ...
    body["total_spine_resolution"] = True
    body["total_spine_resolution_impl"] = TOTAL_SPINE_RESOLUTION_IMPL
    body["total_spine_resolution_short_circuit"] = bool(short_circuit)
    body["total_spine_resolution_irreversible"] = True
    body["total_spine_resolution_post_resolution"] = True
    body["total_spine_resolution_deterministic"] = True
    body["total_spine_resolution_rvs"] = True
    body["total_spine_resolution_certificate"] = dict(certificate)
    body["total_spine_resolution_digest"] = cst_digest
    body["total_spine_resolution_chain"] = chain
    body["total_spine_resolution_tip"] = cst_tip
    body["total_spine_resolution_bound_tip"] = bound
    body["total_spine_digest_pre_risk"] = prior_tip
    body["total_spine_tip_resolution_root"] = tip_resolution_root
    body["total_spine_resolution_height"] = resolution_height
    body["total_spine_resolution_count"] = resolution_count
    body["total_spine_resolved"] = bool(certificate.get("resolved", True))
    body["total_spine_resolved_ok"] = bool(certificate.get("resolved", True))
    body["total_spine_strategic"] = bool(certificate.get("strategic", True))
    body["total_spine_strategic"] = bool(certificate.get("strategic", True))
    body["total_spine_strategic_ok"] = bool(certificate.get("strategic", True))
    body["total_spine_rvs_ok"] = bool(certificate.get("rvs_ok", True))
    body["total_spine_resolution_atomic"] = bool(certificate.get("atomic_ok", True))
    body["total_spine_resolution_one_sided"] = bool(
        certificate.get("one_sided", False)
    )
    body["total_spine_resolutions_ok"] = bool(
        certificate.get("resolutions_ok", True)
    )
    body["total_spine_resolution_root_valid"] = bool(tip_resolution_root)
    body["total_spine_resolution_residual"] = int(certificate.get("residual") or 0)
    body["total_spine_resolution_pair_count"] = int(
        certificate.get("pair_count") or 0
    )
    body["recovery_root"] = tip_resolution_root
    body["tip_resolution_root"] = tip_resolution_root
    body["resolution_count"] = resolution_count
    body["resolution_height"] = resolution_height
    body["resolved"] = bool(certificate.get("resolved", True))
    body["resolved_ok"] = bool(certificate.get("resolved", True))
    body["risk_ok"] = bool(certificate.get("resolved", True))
    body["rvs_ok"] = bool(certificate.get("rvs_ok", True))
    body["strategic"] = bool(certificate.get("strategic", True))
    if certificate.get("resolution_path"):
        body["total_spine_resolution_path"] = certificate.get("resolution_path")
    if bound_state_root:
    ...
    if bound_recovery_root:
        body["total_spine_tip_risk_root"] = bound_recovery_root
        body["recovery_root"] = bound_recovery_root
        body["tip_recovery_root"] = bound_recovery_root
        body.setdefault("total_spine_recovery", True)
        body.setdefault("total_spine_solvent", True)
        body.setdefault("total_spine_rvp_ok", True)
    ...
    if recovery_digest:
        body["total_spine_recovery_digest"] = recovery_digest
    if delivery_digest:
    ...
    body["verdict"] = (
        "total_spine_resolution_ok_short_circuit"
        if short_circuit
        else "total_spine_resolution_ok"
    )
    body["ok"] = True
    return body
...
def _as_recovery_mapping(value: Any) -> dict[str, Any] | None:
    from blackhole_agent.upstream_total_spine_recovery import (
        StageRefused as RecoveryRefused,
        load_total_spine_recovery_certificate,
    )

    if isinstance(value, Mapping):
        kind = str(value.get("kind") or "")
        if kind == TOTAL_SPINE_RESOLUTION_KIND or value.get("total_spine_resolution"):
            nested_liq = value.get("total_spine_recovery_certificate")
        ...
        ) or value.get("total_spine_recovery_loaded") or value.get(
            "tip_recovery_root"
        ):
    ...
    try:
        probe_path = path
        if path.is_dir():
            nested = path / "recovery" / "total-spine-recovery.json"
            named = path / "total-spine-recovery.json"
            probe_path = nested if nested.is_file() else named
        if probe_path.is_file():
            probe = json.loads(probe_path.read_text(encoding="utf-8"))
            if isinstance(probe, Mapping):
                kind = str(probe.get("kind") or "")
                if kind and kind != TOTAL_SPINE_RECOVERY_KIND:
                    return None
    except (OSError, json.JSONDecodeError):
        pass
    try:
        return load_total_spine_recovery_certificate(path)
...
def _as_resolution_mapping(value: Any) -> dict[str, Any] | None:
    if isinstance(value, Mapping):
        ...
        if kind == TOTAL_SPINE_RESOLUTION_KIND or value.get(
            "total_spine_resolution"
        ) or value.get("total_spine_resolution_loaded"):
    ...
    if value is None:
        return None
    try:
        return load_total_spine_resolution_certificate(Path(str(value)))
...
def _settlements_from(item: Any) -> list[dict[str, Any]]:
    ...
    if isinstance(nested, Mapping) and (
        nested.get("tip_settlement_root") or nested.get("observstions")
    ):
        found.append(dict(nested))
    if item.get("tip_settlement_root") and item.get("observstions"):
        found.append(dict(item))
...
def _confirm_recovery(
    ...
    *,
    ...
    from blackhole_agent.upstream_total_spine_recovery import recovery_total_spine
    ...
    confirm_out = None
    tip_risk = str(primary.get("tip_recovery_root") or "")
    sol_height = int(primary.get("recovery_height") or 0)
    ...
    for drop in (
        ...
        if kind in {TOTAL_SPINE_RECOVERY_KIND, TOTAL_SPINE_RESOLUTION_KIND}:
            confirm_body.pop("kind", None)
        confirm_body.pop(drop, None)
    stresses_in: list[dict[str, Any]] = []
    seen: set[str] = set()
    for src in (primary, body, confirm_body):
        for row in _stresses_from(src):
            key = str(
                row.get("risk_digest")
                or row.get("certificate_hash")
            ...
            if not key or key in seen:
                continue
            seen.add(key)
            stresses_in.append(row)
    bundle: list[Any] = list(stresses_in)
    ...
    if not stresses_in and not bundle and not confirm_body:
        raise StageRefused(
            "total_spine_resolution_confirmation_missing",
    ...
    confirmed = recovery_total_spine(
        source,
        stresses=stresses_in or None,
        ...
        prior_tip=prior_tip,
        parent_recovery_root=tip_risk,
        recovery_height=sol_height + 1 if sol_height else None,
        repo_path=repo_path or REPO_ROOT,
        confirm=True,
    )
    cert = confirmed.get("total_spine_recovery_certificate")
...
def _collect_recoveries(
    ...
    existing = _as_resolution_mapping(source)
    if existing is None and body is not None:
        existing = _as_resolution_mapping(body)
    ...
    def _push(item: Any) -> None:
        mapped = _as_recovery_mapping(item)
    ...
    if existing is None:
        if isinstance(source, Sequence) and not isinstance(
            ...
            _push(source)
    if body is not None:
        _push(body.get("total_spine_recovery_certificate"))
    ...
    seen: set[str] = set()
    for row in risks:
        digest = _recovery_digest_of(row)
        tip = str(row.get("tip_recovery_root") or "")
        key = digest or tip
        if not key or key in seen:
            continue
        seen.add(key)
...
def _strip_resolution_predicates(done_when: str) -> str:
    """Evaluate the pre-resolution contract, never stress_* (no recurse)."""
...
def resolution_total_spine(
    ...
    *,
    ...
    """Apply post-recovery atomic RvP stress on the absolute total spine."""
    ...
    if not TOTAL_SPINE_RESOLUTION_IMPL:
        raise StageRefused(
            "total_spine_resolution_disabled",
            "TOTAL_SPINE_RESOLUTION_IMPL is False",
        )

    extra_books: list[Any] = []
    extra_books.extend(list(recoveries or []))
    ...
    (
        ...
        found_settlements,
        found_actuation,
    ) = _collect_recoveries(source, body, extra_books)
    ...
    if (
        ...
        and existing.get("tip_resolution_root")
        and (
            str(existing.get("kind") or "") == TOTAL_SPINE_RESOLUTION_KIND
            or existing.get("total_spine_resolution_loaded")
            or existing.get("total_spine_resolution")
        )
    ):
        tip = str(
            prior_tip
        ...
        result = body if body is not None else {
            "ok": True,
            "action": "resolution_total_spine",
            "total_spine": True,
        }
        return annotate_total_spine_resolution(
            result,
    ...
    legs = book_total_spine_recoveries(
        collected,
        min_resolutions=want,
        parent_resolution_root=parent_resolution_root,
        resolution_height=resolution_height,
    )
    first = collected[0]
    ...
    margin_root = str(first.get("tip_recovery_root") or "")
    recovery_digest = _recovery_digest_of(first)
    root_layer = str(
    ...
    pre_liquidity = _strip_resolution_predicates(done_when)
    if pre_liquidity:
        ctx = {
            "liquidity": {
                "ok": True,
        ...
        if contract_machine and contract_met is not True:
            raise StageRefused(
                "total_spine_resolution_contract_unmet",
                f"done_when not met at collateral: {pre_liquidity!r}",
            )

    tip_resolution_root = compute_total_spine_resolution_root(legs)
    cst_height = int(legs[-1]["recovery_height"]) if legs else 0
    tip = str(
        prior_tip
        or (body or {}).get("total_spine_risk_bound_tip")
        or (body or {}).get("total_spine_digest")
    ...
    cst_body: dict[str, Any] = {
        ...
        "kind": TOTAL_SPINE_RESOLUTION_KIND,
        ...
        "bound_recovery_root": margin_root,
        ...
        "prior_tip": tip,
        "parent_resolution_root": str(
            parent_resolution_root
            or (legs[0].get("parent_resolution_root") if legs else "")
            or ""
        ),
        "resolutions": legs,
        "resolution_count": len(legs),
        "resolution_height": cst_height,
        "tip_resolution_root": tip_resolution_root,
    ...
    if write_target is not None:
        certificate = write_total_spine_resolution_certificate(write_target, cst_body)
    else:
        certificate = seal_total_spine_resolution_certificate(cst_body)
    ...
    if annotated.get("total_spine_compressed") and root_layer:
        ...
        cst_bound = str(annotated.get("total_spine_resolution_bound_tip") or tip)
    ...
    annotated["total_spine_resolution_bound_state_root"] = state_root
    annotated["total_spine_resolution_bound_action_root"] = action_root
    annotated["total_spine_resolution_bound_settlement_root"] = settlement_root
    annotated["total_spine_resolution_bound_clearing_root"] = clearing_root
    annotated["total_spine_resolution_bound_recovery_root"] = margin_root
    annotated["total_spine_resolution_actuation_digest"] = actuation_digest
    annotated["used_skill_route_discovery"] = legacy_pipeline_was_used()
    return annotated

def builtin_total_spine_resolution_proof() -> dict[str, Any]:
    """Hermetic proof: post-recovery atomic RvS on the absolute tower."""
    import shutil
    import tempfile
    ...
    from blackhole_agent.upstream_control_engine import (
        ...
        TOTAL_SPINE_RESOLUTION_IMPL as ENGINE_CAP_IMPL,
    ...
    scratch = Path(tempfile.mkdtemp(prefix="total-spine-resolution-proof-"))
    try:
        from blackhole_agent import upstream_loop_engine as le_facade

        flags_ok = (
            TOTAL_SPINE_RESOLUTION_IMPL is True
            ...
            and TOTAL_SPINE_RESOLUTION_KIND == "total_spine_resolution"
            and bool(TOTAL_SPINE_RESOLUTION_FILENAME)
            and TOTAL_SPINE_RESOLUTION_MIN_RESOLUTIONS >= 2
        )
        ...
        offline_cap = resolution_total_spine(
            [cert_rec1, cert_rec2],
        ...
        cap_path = offline_cap.get("total_spine_resolution_path")
        tip_resolution = str(offline_cap.get("total_spine_tip_resolution_root") or "")
        offline_ok = (
            bool(offline_cap.get("ok"))
            and offline_cap.get("total_spine_resolution") is True
            and offline_cap.get("total_spine_resolution_post_resolution") is True
            and offline_cap.get("total_spine_resolution_irreversible") is True
            and offline_cap.get("total_spine_resolved") is True
            and offline_cap.get("total_spine_strategic") is True
            and offline_cap.get("total_spine_rvs_ok") is True
            and offline_cap.get("total_spine_resolution_atomic") is True
            and offline_cap.get("total_spine_resolution_one_sided") is False
            and int(offline_cap.get("total_spine_resolution_count") or 0) >= 2
            and int(offline_cap.get("total_spine_resolution_height") or 0) >= 2
            and int(offline_cap.get("total_spine_resolution_residual") or 0) == 0
            and int(offline_cap.get("total_spine_resolution_pair_count") or 0) >= 1
            and len(tip_resolution) >= 32
            ...
            != str(rec1.get("total_spine_digest") or "")
            and isinstance(cap_path, str)
            and Path(cap_path).is_file()
            and not legacy_pipeline_was_used()
        )

        loaded = load_total_spine_resolution_certificate(cap_path or scratch)
        verify_ok = bool(
            loaded.get("total_spine_resolution_loaded")
            and (loaded.get("resolution_verify") or {}).get("ok")
            and (loaded.get("resolution_verify") or {}).get("recovery_root_ok")
            and (loaded.get("resolution_verify") or {}).get("chain_ok")
            and (loaded.get("resolution_verify") or {}).get("resolutions_ok")
            and (loaded.get("resolution_verify") or {}).get("rvs_ok")
        )

        tampered_path = scratch / "tampered-resolution.json"
        ...
        tampered_body["resolution_height"] = 99
        atomic_write_json(tampered_path, tampered_body)
        tamper_ok = False
        try:
            load_total_spine_resolution_certificate(tampered_path)
        except StageRefused as exc:
            tamper_ok = str(exc.verdict) == "total_spine_resolution_tampered"
        except Exception:  # noqa: BLE001
            tamper_ok = False
        ...
        try:
            ...
            supersession_ok = (
                str(exc.verdict) == "total_spine_resolution_supersession_refused"
            )
        except Exception:  # noqa: BLE001
            supersession_ok = False
        ...
        resealed = seal_total_spine_resolution_certificate(wrong_body)
        wrong_verify = verify_total_spine_resolution_certificate(resealed)
        wrong_root_ok = wrong_verify.get("ok") is False and (
            wrong_verify.get("chain_ok") is False
            or wrong_verify.get("recovery_root_ok") is False
        )

        mismatch_ok = False
        try:
            mixed = dict(cert_rec1)
        ...
        try:
            ...
            resealed_one = seal_total_spine_recovery_certificate(forged)
            book_total_spine_recoveries(
                [cert_rec1, resealed_one], min_resolutions=2
            )
        except StageRefused as exc:
            one_sided_ok = str(exc.verdict) == "total_spine_resolution_one_sided"
        except Exception:  # noqa: BLE001
            one_sided_ok = False
        ...
        try:
            ...
            partial_ok = str(exc.verdict) == "total_spine_resolution_partial"
        except Exception:  # noqa: BLE001
            partial_ok = False

        h2 = resolution_total_spine(
            [cert_rec1, cert_rec2],
            out_root=scratch / "res-h2",
            prior_tip=str(
                offline_cap.get("total_spine_resolution_bound_tip") or ""
            ),
            parent_resolution_root=tip_resolution,
            resolution_height=int(
                offline_cap.get("total_spine_resolution_height") or 0
            )
            + 1,
            repo_path=REPO_ROOT,
            confirm=False,
        ...
        multi_height_ok = (
            bool(h2.get("ok"))
            and int(h2.get("total_spine_resolution_count") or 0) >= 2
            and str(h2.get("total_spine_tip_resolution_root") or "") != tip_resolution
            and str(
                (h2.get("total_spine_resolution_certificate") or {}).get(
                    "parent_resolution_root"
                )
                or ""
            )
        ...
        recomputed = compute_total_spine_resolution_root(
            loaded.get("resolutions") or []
        )
        determinism_ok = recomputed == tip_resolution and bool(recomputed)

        live = run_total_spine(
            root_layer="quettacontinuum",
            out_root=scratch / "live-res",
            max_rounds=1,
            dispatch=True,
            dispatch_budget=2,
            max_successions=1,
        ...
        live_cap_path = live.get("total_spine_resolution_path")
        live_ok = (
            bool(live.get("ok"))
            ...
            and live.get("total_spine_resolved") is True
            and live.get("total_spine_rvs_ok") is True
            and int(live.get("total_spine_resolution_count") or 0) >= 2
            and isinstance(live.get("total_spine_tip_resolution_root"), str)
            and len(str(live.get("total_spine_tip_resolution_root"))) >= 32
        ...
        short_ok = (
            bool(shorted.get("ok"))
            and shorted.get("total_spine_resolution") is True
            and shorted.get("total_spine_resolution_short_circuit") is True
            and str(shorted.get("total_spine_tip_resolution_root") or "")
            == str(live.get("total_spine_tip_resolution_root") or "")
            and int(shorted.get("total_nest_depth") or 0) == 28
            and not legacy_pipeline_was_used()
        )

        cap_chain = live.get("total_spine_resolution_chain") or {}
        chain_integrity_ok = False
        if isinstance(cap_chain, Mapping) and cap_chain:
            re_seal = seal_total_spine_resolution_chain(
                prior_tip=str(cap_chain.get("prior_tip") or ""),
                resolution_digest=str(cap_chain.get("resolution_digest") or ""),
                tip_resolution_root=str(cap_chain.get("tip_resolution_root") or ""),
                bound_recovery_root=str(
                    cap_chain.get("bound_recovery_root") or ""
                ),
                bound_delivery_root=str(
                ...
                resolution_height=int(cap_chain.get("resolution_height") or 0),
                short_circuit=bool(cap_chain.get("short_circuit")),
            )
            chain_integrity_ok = (
                re_seal.get("digest") == cap_chain.get("digest")
                and re_seal.get("digest") == live.get("total_spine_resolution_tip")
            )

        differential_ok = (
            offline_ok
            and live_ok
            and str(rec1.get("total_spine_digest") or "")
        ...
        source_ok = (
            getattr(le_facade, "TOTAL_SPINE_RESOLUTION_IMPL", None) is TOTAL_SPINE_RESOLUTION_IMPL
            and getattr(le_facade, "builtin_total_spine_resolution_proof", None) is builtin_total_spine_resolution_proof
            and getattr(le_facade, "resolution_total_spine", None) is resolution_total_spine
            and callable(
                getattr(le_facade, "builtin_total_spine_resolution_proof", None)
    
        )
            and callable(getattr(le_facade, "resolution_total_spine", None))
            and getattr(le_facade, "TOTAL_SPINE_RESOLUTION_IMPL", False) is True
        )
        ...
        engine_source_ok = (
            "TOTAL_SPINE_RESOLUTION_IMPL" in engine_text
            and "resolution_total_spine" in engine_text
            and (
                "resolution=True" in engine_text
                or "resolution: bool = False" in engine_text
            )
            and "builtin_total_spine_resolution_proof" in engine_text
        )

        mod_path = Path(__file__).resolve()
        mod_text = mod_path.read_text(encoding="utf-8")
        mod_source_ok = (
            "def resolution_total_spine" in mod_text
            and "def builtin_total_spine_resolution_proof" in mod_text
            and "total_spine_resolution_supersession_refused" in mod_text
            and "total_spine_resolution_tampered" in mod_text
            and "total_spine_resolution_one_sided" in mod_text
            and "total_spine_resolution_rvp_failed" in mod_text
        )
        ...
        try:
            ...
            ledger_ok = (
                entry is not None
                and (
                    "upstream_total_spine_resolution" in (entry.entry or "")
                    or "upstream_control_engine" in (entry.entry or "")
                )
                and "builtin_total_spine_resolution_proof" in (entry.entry or "")
                and (
                    "resolution" in tags_blob
                    or "resolution" in name_blob
                    or "resolution" in delta_blob
                )
                and ("total" in tags_blob or "total" in name_blob)
                and (
                    "resolution_total_spine" in delta_blob
                    or "post-resolution" in delta_blob
                    or "post_resolution" in delta_blob
                    or "rvs" in delta_blob
                )
            )
        except Exception:  # noqa: BLE001
            ledger_ok = False
        ...
        return {
            "ok": ok,
            "action": "total_spine_resolution_proof",
            "flags_ok": flags_ok,
            "offline_ok": offline_ok,
            "resolution_path": cap_path,
            ...
            "state_root": state_root,
            "resolution_count": offline_cap.get("total_spine_resolution_count"),
            "pair_count": offline_cap.get("total_spine_resolution_pair_count"),
            "verify_ok": verify_ok,
            "tamper_ok": tamper_ok,
            ...
            "live_ok": live_ok,
            "live_resolution_path": live_cap_path,
            "live_tip_resolution_root": live.get("total_spine_tip_resolution_root"),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    sub.add_parser(
        "resolution-proof",
        help=(
            "Total spine resolution proof: post-recovery atomic RvS seals "
            "matching recovery books into irreversible resolution receipts"
        ),
    )
    sub.add_parser("proof", help="Alias for resolution-proof")
    args = parser.parse_args(list(argv) if argv is not None else None)
    if args.cmd in {"resolution-proof", "proof"}:
