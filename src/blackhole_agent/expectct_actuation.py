...
def builtin_expectct_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="expectct-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, EXPECTCT_ACTUATION_GOAL, EXPECTCT_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_expectct"] = (
        not gate.accepted
        and live_goal != EXPECTCT_ACTUATION_GOAL
        and EXPECTCT_ACTUATION_ID not in live_done
