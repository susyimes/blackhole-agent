...
def builtin_http10_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="http10-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, HTTP10_ACTUATION_GOAL, HTTP10_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_http10"] = (
        not gate.accepted
        and live_goal != HTTP10_ACTUATION_GOAL
        and HTTP10_ACTUATION_ID not in live_done
