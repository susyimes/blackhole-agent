"""Upstream yottacontinuum plane: multi-zettacontinuum durable stewardship yottacontinuum.

The zettacontinuum plane (``upstream_zettacontinuum``) closes multi-continuum unions *within one
zettacontinuum*. It does not:

1. chain multiple independent zettacontinuums under a durable yottacontinuum constitution;
2. allocate a shared global dispatch budget across zettacontinuums by ROI;
3. admit/retire zettacontinuum slots from an yottacontinuum charter over time
   (deferred admission under a concurrent-active cap);
4. grow the yottacontinuum charter mid-run via ``charter_expand`` (constitution
   growth beyond the initial charter, not just deferred admission of a fixed set);
5. federate multi-zettacontinuum portfolio coverage into one yottacontinuum world-model;
6. persist yottacontinuum state so a later process can resume the union;
7. seal a multi-zettacontinuum yottacontinuum chronicle linking zettacontinuum digests.

The yottacontinuum plane closes that outer multi-zettacontinuum loop:

1. **admit** — materialize zettacontinuum slots from a durable yottacontinuum charter
   (each slot owns a nested continuum charter). When ``max_active_zettacontinuums``
   is set, only that many *unmet* zettacontinuums are concurrent: further charter
   slots stay pending and are admitted as capacity frees after retirements
   (yottacontinuum constitution growth over time);
2. **schedule** — pick the next open zettacontinuum by priority and historical ROI;
3. **zettacontinuum** — call the zettacontinuum plane (injected ``zettacontinuum_runner``;
   default ``run_zettacontinuum``) with a share of the remaining global dispatch
   budget;
4. **federate** — merge per-zettacontinuum portfolios into one yottacontinuum world-model
   and re-score coverage across all stewarded keys;
5. **retire** — mark zettacontinuums met when their zettacontinuum_goal is satisfied,
   then re-admit pending charter slots up to the active capacity;
6. **expand** — optional ``charter_expand`` may append new zettacontinuum slots when
   the active charter has no pending work and all admitted zettacontinuums are met,
   so the yottacontinuum constitution can grow after start (not only defer a fixed charter);
7. **persist** — write ``yottacontinuum_state.json`` after every zettacontinuum round so a
   later ``run_yottacontinuum(..., resume_dir=...)`` continues the same union
   (including pending charter and admission history);
8. **stop** when any of:

   - ``max_rounds`` reached
   - global ``dispatch_budget`` exhausted across zettacontinuums
   - yottacontinuum goal met (``all_zettacontinuums_met``: every *admitted*
     zettacontinuum is met *and* no pending charter slots remain)
   - consecutive idle/no-progress rounds (``idle_round_limit``)
   - explicit ``stop_when`` predicate returns a reason string

9. **seal** — write an yottacontinuum receipt under
   ``artifacts/upstream-yottacontinuum/`` with sha256 digests of every
   zettacontinuum, portfolio federation, admission history, ROI history, stop
   reason, and an yottacontinuum chain digest; ``verify_yottacontinuum_receipt``
   re-checks the chain and detects tampering.

No skill-route discovery is used. The plane is yottacontinuum-level direction
over the zettacontinuum plane, not a new verifier of individual repairs.
"""
...
from blackhole_agent import upstream_zettacontinuum as zz
...
ARTIFACTS_ROOT = REPO_ROOT / "artifacts" / "upstream-yottacontinuum"

TERMINAL_SUCCESS_OUTCOMES = zz.TERMINAL_SUCCESS_OUTCOMES


class YottacontinuumRefused(Exception):
...
def normalize_yottacontinuum_charter(
    charter: Sequence[Mapping[str, Any]] | None,
) -> list[dict[str, Any]]:
    """Normalize an yottacontinuum charter into deterministic continuum slots.
    ...
    Each slot is::

        {
          ...
          "priority": int,
          "charter": [...exacontinuum slots...],  # nested zettacontinuum charter
    ...
    for raw in charter:
        if not isinstance(raw, Mapping):
            continue
        zettacontinuum_id = str(
            raw.get("zettacontinuum_id") or raw.get("id") or ""
        ).strip()
        if not zettacontinuum_id or zettacontinuum_id in seen:
            continue
        seen.add(zettacontinuum_id)

        nested = zz.normalize_zettacontinuum_charter(
            raw.get("charter")
            or raw.get("exacontinuums")
        ...
        max_active_exacontinuums = raw.get("max_active_exacontinuums")
        if max_active_exacontinuums is None:
            max_active_exacontinuums = raw.get("max_active_continuums")
        if max_active_exacontinuums is None:
            max_active_exacontinuums = raw.get("max_active_civilizations")
        if max_active_exacontinuums is not None:
            max_active_exacontinuums = max(1, int(max_active_exacontinuums))
        ...
        out.append(
            {
                ...
                "zettacontinuum_goal": str(
                    raw.get("zettacontinuum_goal") or "all_exacontinuums_met"
                ),
                "kind": str(raw.get("kind") or "stewardship_zettacontinuum"),
            }
        )
    return out


def admit_zettacontinuum_slot(
    *,
    yottacontinuum_dir: Path,
    slot: Mapping[str, Any],
) -> dict[str, Any]:
    """Materialize one institution slot under the league directory.

    Returns admission record with zettacontinuum_root and nested charter.
    Stewardship surfaces are created lazily by the institution plane on run.
    """
    zettacontinuum_id = str(slot.get("zettacontinuum_id") or "")
    if not zettacontinuum_id:
        raise YottacontinuumRefused("yottacontinuum_invalid", "slot missing zettacontinuum_id")

    zettacontinuum_root = Path(yottacontinuum_dir) / "zettacontinuums" / zettacontinuum_id
    zettacontinuum_root.mkdir(parents=True, exist_ok=True)

    nested_charter = zz.normalize_zettacontinuum_charter(slot.get("charter"))
    if not nested_charter:
        raise YottacontinuumRefused(
            "yottacontinuum_invalid",
            f"continuum slot {zettacontinuum_id!r} has empty nested charter",
        )

    max_active_exacontinuums = slot.get("max_active_exacontinuums")
    if max_active_exacontinuums is None:
        max_active_exacontinuums = slot.get("max_active_continuums")
    if max_active_exacontinuums is None:
        max_active_exacontinuums = slot.get("max_active_civilizations")

    return {
        "zettacontinuum_id": zettacontinuum_id,
        "zettacontinuum_root": str(zettacontinuum_root),
        "admitted": True,
        "charter": nested_charter,
        "max_active_exacontinuums": max_active_exacontinuums,
        "max_rounds": int(slot.get("max_rounds") or 6),
        "zettacontinuum_goal": str(slot.get("zettacontinuum_goal") or "all_exacontinuums_met"),
...
def federate_portfolios(
    ...
    portfolio: dict[str, Any] = {
        ...
        "entries": entries,
        "source": "yottacontinuum_federation",
    }
...
def yottacontinuum_terminal_coverage(
    *,
    zettacontinuum_states: Sequence[Mapping[str, Any]],
    federated_portfolio: Mapping[str, Any] | None,
) -> dict[str, Any]:
    """Coverage across zettacontinuum->exacontinuum->...->program."""
    exacontinuum_states: list[dict[str, Any]] = []
    for uc in zettacontinuum_states:
        if not isinstance(uc, Mapping):
            continue
        nested = (
            list(uc.get("exacontinuum_states") or [])
        ...
        if nested:
            for hc in nested:
                if isinstance(hc, Mapping):
                    exacontinuum_states.append(dict(hc))
            continue
        # Pre-run: zettacontinuum charter is a list of exacontinuum slots.
        for cslot in list(uc.get("charter") or []) + list(
            uc.get("exacontinuums") or []
            ...
            if isinstance(cslot, Mapping):
                exacontinuum_states.append(dict(cslot))
        if not nested and not list(uc.get("charter") or []):
            for cont in list(uc.get("continuum_states") or []) + list(
                ...
                if isinstance(cont, Mapping):
                    exacontinuum_states.append(dict(cont))
        for raw in list(uc.get("inventory_keys") or []):
            if isinstance(raw, (list, tuple)) and len(raw) >= 3:
                ...
                exacontinuum_states.append({"inventory_keys": [raw]})
    return zz.zettacontinuum_terminal_coverage(
        exacontinuum_states=exacontinuum_states,
        federated_portfolio=federated_portfolio,
    )


def zettacontinuums_all_met(zettacontinuum_states: Sequence[Mapping[str, Any]]) -> bool:
    if not zettacontinuum_states:
        return False
    return all(bool(ist.get("zettacontinuum_met")) for ist in zettacontinuum_states)


def open_unmet_count(zettacontinuum_states: Sequence[Mapping[str, Any]]) -> int:
    """Count admitted institutions that are not yet continuum_met."""
    return sum(1 for ist in zettacontinuum_states if not ist.get("zettacontinuum_met"))
...
def pending_charter_slots(
    ...
    """Charter slots not yet admitted, priority-desc then zettacontinuum_id-asc."""
    known = {str(ist.get("zettacontinuum_id") or "") for ist in zettacontinuum_states}
    pending = [
        dict(slot)
        for slot in charter
        if str(slot.get("zettacontinuum_id") or "")
        and str(slot.get("zettacontinuum_id")) not in known
    ]
    pending.sort(
        key=lambda s: (
            -int(s.get("priority") or 0),
            str(s.get("zettacontinuum_id") or ""),
        )
    )
    return pending
...
def constitution_satisfied(
    *,
    ...
    if yottacontinuum_goal == "none":
        return False
    if yottacontinuum_goal == "terminal_coverage":
        cov = yottacontinuum_terminal_coverage(
            zettacontinuum_states=zettacontinuum_states,
            federated_portfolio=federated_portfolio,
        )
        return bool(cov.get("met")) and not pending_charter_slots(
            charter, zettacontinuum_states
        )
    if yottacontinuum_goal == "all_zettacontinuums_met":
        if not zettacontinuum_states:
            return False
        if pending_charter_slots(charter, zettacontinuum_states):
            return False
        return zettacontinuums_all_met(zettacontinuum_states)
    return False


def reopen_incomplete_zettacontinuums(
    ...
    *,
    ...
    Nested zettacontinuums can retire after only partial surface work (e.g. before
    deferred program targets expand). Under a ``terminal_coverage`` yottacontinuum
    goal those children must re-run until federated inventory is fully
    terminal-success. Returns the reopened ``zettacontinuum_id`` list.
    """
    cov = yottacontinuum_terminal_coverage(
        zettacontinuum_states=zettacontinuum_states,
        federated_portfolio=federated_portfolio,
    ...
    for item in list(cov.get("open_or_missing") or []):
        ...
        if key[0] and key[2]:
            open_keys.add(key)
    reopened: list[str] = []
    for i, ist in enumerate(zettacontinuum_states):
        if not ist.get("zettacontinuum_met"):
            continue
        ...
        for mv in list(ist.get("exacontinuum_states") or []) + list(
        ...
        if not owns_open:
            continue
        updated = dict(ist)
        updated["zettacontinuum_met"] = False
        zettacontinuum_states[i] = updated
        reopened.append(str(updated.get("zettacontinuum_id") or ""))
    return [r for r in reopened if r]


def merge_yottacontinuum_charter(
    ...
    Existing ``zettacontinuum_id`` values win (additions with the same id are
    ignored). Returns a fully re-normalized charter so nested confederation
    charters stay deterministic.
    """
    base = normalize_yottacontinuum_charter(existing)
    if not additions:
        return base
    known = {str(s.get("zettacontinuum_id") or "") for s in base}
    merged: list[Mapping[str, Any]] = list(base)
    for raw in additions:
        if not isinstance(raw, Mapping):
            continue
        cid = str(raw.get("zettacontinuum_id") or raw.get("id") or "").strip()
        if not cid or cid in known:
            continue
        known.add(cid)
        merged.append(raw)
    return normalize_yottacontinuum_charter(merged)


def make_yottacontinuum_charter_expand(
    ...
    *,
    ...
    """
    pending_growth = normalize_yottacontinuum_charter(growth)
    ...
    def _runner(
        *,
        ...
        remaining = [
            ...
            if str(s.get("zettacontinuum_id") or "") not in applied_ids
            and str(s.get("zettacontinuum_id") or "")
            not in {str(x.get("zettacontinuum_id") or "") for x in active_charter}
        ]
        if not remaining:
            return {
                "expanded": False,
        ...
        for s in take:
            applied_ids.add(str(s.get("zettacontinuum_id") or ""))
        merged = merge_yottacontinuum_charter(active_charter, take)
        state["applied"] = applied_ids
        return {
            "expanded": True,
            "added": [str(s.get("zettacontinuum_id") or "") for s in take],
            "charter": merged,
            "detail": "charter_growth_applied",
            "round_index": round_index,
            "realms_met": zettacontinuums_all_met(zettacontinuum_states),
        }
...
def admit_pending_slots(
    *,
    ...
    ``max_active_zettacontinuums`` caps *unmet* concurrent realms. ``None``
    admits every remaining pending slot. Returns admission records for newly
    admitted slots (also mutates ``zettacontinuum_states``).
    """
    pending = pending_charter_slots(charter, zettacontinuum_states)
    if not pending:
        return []

    open_n = open_unmet_count(zettacontinuum_states)
    if max_active_zettacontinuums is None:
        capacity = len(pending)
    else:
        capacity = max(0, int(max_active_zettacontinuums) - open_n)
    if capacity <= 0:
        return []

    admissions: list[dict[str, Any]] = []
    for slot in pending[:capacity]:
        admission = admit_zettacontinuum_slot(yottacontinuum_dir=yottacontinuum_dir, slot=slot)
        ...
        zettacontinuum_states.append(
            {
                "zettacontinuum_id": admission["zettacontinuum_id"],
                "zettacontinuum_root": admission["zettacontinuum_root"],
                "charter": admission["charter"],
                "max_active_exacontinuums": admission.get("max_active_exacontinuums"),
                "max_rounds": admission["max_rounds"],
                "zettacontinuum_goal": admission["zettacontinuum_goal"],
                "priority": admission["priority"],
                "zettacontinuum_met": False,
                "last_zettacontinuum_dir": None,
                "last_zettacontinuum_digest": None,
                "portfolio": None,
                "omniverse_states": [],
                "multiverse_states": [],
                "cosmos_states": [],
...
def score_zettacontinuum_roi(
    *,
    ...
    return {
        ...
        "zettacontinuum_met": bool(continuum_result.get("zettacontinuum_met")),
        "continuum_digest": continuum_result.get("zettacontinuum_digest"),
        "continuums_admitted": int(
            continuum_result.get("exacontinuums_admitted")
            or continuum_result.get("megacontinuums_admitted")
            or continuum_result.get("ultracontinuums_admitted")
            or continuum_result.get("zettacontinuums_admitted")
...
def _roi_summary(roi_history: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for r in roi_history:
        iid = str(r.get("zettacontinuum_id") or "")
...
def select_next_zettacontinuum(
    zettacontinuum_states: Sequence[Mapping[str, Any]],
    roi_history: Sequence[Mapping[str, Any]],
    *,
    round_index: int,
    ...
    Tie-break: lower run_count, then stable zettacontinuum_id order. Round-robin
    among equal scores uses ``round_index`` so multi-institution progress is fair.
    """
    open_slots = [
        dict(ist) for ist in zettacontinuum_states if not ist.get("zettacontinuum_met")
    ]
    if not open_slots:
        return None
    ...
    def sort_key(ist: Mapping[str, Any]) -> tuple[Any, ...]:
        iid = str(ist.get("zettacontinuum_id") or "")
...
def allocate_zettacontinuum_budget(
    *,
    ...
    hist = (summary.get("by_continuum") or {}).get(
        str(selected.get("zettacontinuum_id") or "")
    ) or {}
    mean_eff = float(hist.get("mean_efficiency") or 0.0)
...
def _state_payload(
    *,
    ...
    roi_history: Sequence[Mapping[str, Any]],
    zettacontinuum_states: Sequence[Mapping[str, Any]],
    zettacontinuum_digests: Sequence[str],
    charter: Sequence[Mapping[str, Any]],
    ...
    return {
        ...
        "zettacontinuum_states": [dict(ist) for ist in zettacontinuum_states],
        "zettacontinuum_digests": list(zettacontinuum_digests),
        ...
        "pending_zettacontinuum_ids": [
            str(s.get("zettacontinuum_id") or "")
            for s in pending_charter_slots(charter, zettacontinuum_states)
        ],
    }


def write_yottacontinuum_state(yottacontinuum_dir: Path, state: Mapping[str, Any]) -> Path:
    path = Path(yottacontinuum_dir) / "yottacontinuum_state.json"
    atomic_write_json(path, dict(state))
    return path


def load_yottacontinuum_state(resume_dir: Path) -> dict[str, Any]:
    path = durable_read_path(Path(resume_dir) / "yottacontinuum_state.json")
    if not path.is_file():
        raise YottacontinuumRefused(
            "yottacontinuum_state_missing",
            f"no yottacontinuum_state.json under {resume_dir}",
        )
    try:
        state = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise YottacontinuumRefused("yottacontinuum_state_unreadable", str(exc)) from exc
    if not isinstance(state, dict):
        raise YottacontinuumRefused("yottacontinuum_state_invalid", "state root must be object")
    return state
...
def _zettacontinuum_round_record(
    *,
    round_index: int,
    zettacontinuum_id: str,
    continuum_result: Mapping[str, Any],
    ...
    return {
        ...
        "stop_reason": continuum_result.get("stop_reason"),
        "zettacontinuum_dir": continuum_result.get("zettacontinuum_dir"),
        "zettacontinuum_digest": continuum_result.get("zettacontinuum_digest"),
        "continuums_admitted": int(
            continuum_result.get("exacontinuums_admitted")
            or continuum_result.get("megacontinuums_admitted")
            or continuum_result.get("ultracontinuums_admitted")
...
def _yottacontinuum_digest_payload(receipt: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": receipt.get("schema_version"),
        "verdict": receipt.get("verdict"),
        "stop_reason": receipt.get("stop_reason"),
        "yottacontinuum_id": receipt.get("yottacontinuum_id"),
        "yottacontinuum_goal": receipt.get("yottacontinuum_goal"),
        "max_rounds": receipt.get("max_rounds"),
        "max_active_zettacontinuums": receipt.get("max_active_zettacontinuums"),
        ...
        "zettacontinuum_digests": list(receipt.get("zettacontinuum_digests") or []),
        "total_dispatched": receipt.get("total_dispatched"),
        "total_dispatched_ok": receipt.get("total_dispatched_ok"),
        "yottacontinuum_met": receipt.get("yottacontinuum_met"),
        "coverage_end": receipt.get("coverage_end"),
        "zettacontinuums_met_count": receipt.get("zettacontinuums_met_count"),
        "zettacontinuums_admitted": receipt.get("zettacontinuums_admitted"),
...
def verify_yottacontinuum_receipt(yottacontinuum_dir: Path) -> dict[str, Any]:
    """Re-check a sealed league receipt for digest integrity."""
    path = durable_read_path(Path(yottacontinuum_dir) / "yottacontinuum.json")
    if not path.is_file():
    ...
    expected = _sha256_json(_yottacontinuum_digest_payload(receipt))
    recorded = str(receipt.get("yottacontinuum_digest") or "")
    mismatched: list[str] = []
    if not recorded or recorded != expected:
        mismatched.append("yottacontinuum_digest")

    institutions = list(receipt.get("zettacontinuums") or receipt.get("realms") or receipt.get("leagues") or [])
    listed = list(receipt.get("zettacontinuum_digests") or [])
    if len(listed) != len(institutions):
        mismatched.append("zettacontinuum_digests_length")
    else:
        for i, (listed_d, rec) in enumerate(zip(listed, institutions)):
            if listed_d != rec.get("zettacontinuum_digest"):
                mismatched.append(f"zettacontinuum_digests[{i}]")

    nested_failures: list[str] = []
    for rec in institutions:
        idir = rec.get("zettacontinuum_dir")
        if not idir:
            continue
        ip = Path(str(idir))
        if (ip / "zettacontinuum.json").is_file():
            nested = zz.verify_zettacontinuum_receipt(ip)
    ...
    return {
        "ok": ok,
        "verdict": "yottacontinuum_sealed" if ok else "yottacontinuum_tampered",
        "yottacontinuum_digest": recorded,
...
def run_yottacontinuum(
    *,
    ...
    max_active_zettacontinuums: int | None = None,
    dispatch: bool = True,
    zettacontinuum_runner: Callable[..., dict[str, Any]] | None = None,
    ...
    yottacontinuum_id: str | None = None,
    ...
    charter:
        ...
        :func:`normalize_yottacontinuum_charter`).
    max_rounds:
    ...
    yottacontinuum_goal:
        ``all_zettacontinuums_met`` (default) stops when every admitted institution
    ...
    resume_dir:
        Load ``yottacontinuum_state.json`` from a prior league dir and continue.
        New receipt is written under ``out_root`` (or a fresh stamp).
    """
    if max_rounds < 1:
        raise YottacontinuumRefused("yottacontinuum_invalid", "max_rounds must be >= 1")
    if per_wave_dispatch_limit < 0:
        raise YottacontinuumRefused(
            "yottacontinuum_invalid", "per_wave_dispatch_limit must be >= 0"
        )
    if max_active_zettacontinuums is not None and int(max_active_zettacontinuums) < 1:
        raise YottacontinuumRefused(
            "yottacontinuum_invalid", "max_active_zettacontinuums must be >= 1 when set"
        )
    if yottacontinuum_goal not in {"all_zettacontinuums_met", "terminal_coverage", "none"}:
        raise YottacontinuumRefused(
            "yottacontinuum_invalid",
            f"unknown yottacontinuum_goal: {yottacontinuum_goal}",
        )

    runner = zettacontinuum_runner or zz.run_zettacontinuum

    prior_round_count = 0
    roi_history: list[dict[str, Any]] = []
    zettacontinuum_digests: list[str] = []
    total_dispatched = 0
    total_dispatched_ok = 0
    resumed = False
    resume_yottacontinuum_id: str | None = None
    zettacontinuum_states: list[dict[str, Any]] = []
    ...
    if resume_dir is not None:
        state = load_yottacontinuum_state(resume_dir)
        resumed = True
        resume_yottacontinuum_id = str(state.get("yottacontinuum_id") or "") or None
        ...
        zettacontinuum_digests = [str(d) for d in (state.get("zettacontinuum_digests") or [])]
        zettacontinuum_states = [
            dict(ist)
            for ist in (state.get("zettacontinuum_states") or [])
        ...
        if isinstance(state.get("charter"), list):
            active_charter = normalize_yottacontinuum_charter(
        ...
        if (
            state.get("max_active_zettacontinuums") is not None
            and max_active_zettacontinuums is None
        ):
            resumed_max_active = int(state["max_active_zettacontinuums"])
        # Resume may also merge a caller-supplied charter growth tail.
        if charter:
            active_charter = merge_yottacontinuum_charter(active_charter, charter)
    else:
        active_charter = normalize_yottacontinuum_charter(charter)
    ...
    if not active_charter and not zettacontinuum_states:
        raise YottacontinuumRefused(
            "yottacontinuum_empty",
            "continuum charter has no admitable realm slots",
        )

    lid = (
        yottacontinuum_id
        or resume_yottacontinuum_id
        or f"yottacontinuum-{utc_now_iso().replace(':', '').replace('-', '')}"
    )

    stamp = utc_now_iso().replace(":", "").replace("-", "")
    if out_root is not None:
        yottacontinuum_dir = Path(out_root)
        if (yottacontinuum_dir / "yottacontinuum.json").is_file():
            yottacontinuum_dir = yottacontinuum_dir / stamp
    else:
        yottacontinuum_dir = ARTIFACTS_ROOT / stamp
    yottacontinuum_dir.mkdir(parents=True, exist_ok=True)
    ...
    # on a flat ultra-short base (not under yottacontinuum_dir) so one extra
    ...
    if league_out_root is not None:
        ...
        lg_root = yottacontinuum_dir / "e"
    lg_root.mkdir(parents=True, exist_ok=True)
    if _os.name == "nt":
        inst_flat_root = Path("C:/t") / "q"
    else:
        inst_flat_root = yottacontinuum_dir / "x"
    inst_flat_root.mkdir(parents=True, exist_ok=True)
    # Per-run namespace so parallel/resumed ultracontinua do not collide.
    # Prefer a short random token over the human lid (often "yottacontinuum-…")
    # so Windows paths stay short and never re-enter a prior zettacontinuum_dir that
    ...
    initial_admissions = admit_pending_slots(
        yottacontinuum_dir=yottacontinuum_dir,
        charter=active_charter,
        zettacontinuum_states=zettacontinuum_states,
        max_active_zettacontinuums=active_max,
        round_index=prior_round_count,
    )
    admissions.extend(initial_admissions)

    if not zettacontinuum_states and not pending_charter_slots(
        active_charter, zettacontinuum_states
    ):
        raise YottacontinuumRefused("yottacontinuum_empty", "no realm slots admitted")
    if not zettacontinuum_states and pending_charter_slots(
        active_charter, zettacontinuum_states
    ):
        raise YottacontinuumRefused(
            "yottacontinuum_empty",
            "no realm slots admitted under max_active_zettacontinuums policy",
        )

    if federated_portfolio is None:
        federated_portfolio = federate_portfolios(
            [ist.get("portfolio") for ist in zettacontinuum_states]
        )
    ...
    max_terminal_reopens = max(2, (len(zettacontinuum_states) or 1) * 4)
    coverage_end: dict[str, Any] = yottacontinuum_terminal_coverage(
        zettacontinuum_states=zettacontinuum_states,
        federated_portfolio=federated_portfolio,
    )
    ...
    for local_index in range(max_rounds):
        ...
        coverage_before = yottacontinuum_terminal_coverage(
            zettacontinuum_states=zettacontinuum_states,
            federated_portfolio=federated_portfolio,
        )
        ...
        selected = select_next_zettacontinuum(
            zettacontinuum_states, roi_history, round_index=round_index
        )
        if selected is None:
            # Children may all report met while federated terminal coverage is
            # still incomplete (e.g. deferred surfaces). Only declare yottacontinuum
            # met when the active yottacontinuum_goal is actually satisfied.
            if constitution_satisfied(
                zettacontinuum_states=zettacontinuum_states,
                charter=active_charter,
                yottacontinuum_goal=yottacontinuum_goal,
                ...
                break
            if (
                yottacontinuum_goal == "terminal_coverage"
                and terminal_reopen_count < max_terminal_reopens
            ):
                reopened = reopen_incomplete_zettacontinuums(
                    zettacontinuum_states,
                    federated_portfolio=federated_portfolio,
                )
                if reopened:
            ...
            if selected is None:
                stop_reason = "yottacontinuum_idle"
                coverage_end = coverage_before
                break

        open_count = sum(
            1 for ist in zettacontinuum_states if not ist.get("zettacontinuum_met")
        )
        allocated = allocate_zettacontinuum_budget(
        ...
        zettacontinuum_id = str(selected["zettacontinuum_id"])
        resume_zettacontinuum_dir = selected.get("last_zettacontinuum_dir")
        continuum_resume: Path | None = None
        if (
            resume_zettacontinuum_dir
            and (Path(str(resume_zettacontinuum_dir)) / "zettacontinuum_state.json").is_file()
            and not selected.get("zettacontinuum_met")
        ):
            continuum_resume = Path(str(resume_zettacontinuum_dir))

        # Ultra-short stamp (each outer plane tightens Windows MAX_PATH).
        # Do NOT nest child zettacontinuum trees under yottacontinuum roots:
        # let the zettacontinuum plane use its own flat C:/t/{h,i} bases so the
        # deep continuum→…→wave chain still fits. Only the zettacontinuum receipt
        # itself lands on a flat unique out_root.
        safe_id = "".join(c if c.isalnum() else "" for c in zettacontinuum_id)[:2] or "i"
        ...
        continuum_kwargs: dict[str, Any] = {
            ...
            "zettacontinuum_goal": str(
                selected.get("zettacontinuum_goal") or "all_exacontinuums_met"
            ),
            "zettacontinuum_id": zettacontinuum_id,
            "out_root": out_dir,
        }
        ...
        max_active_exacontinuums = selected.get("max_active_exacontinuums")
        if max_active_exacontinuums is None:
            max_active_exacontinuums = selected.get("max_active_continuums")
        if max_active_exacontinuums is None:
            max_active_exacontinuums = selected.get("max_active_civilizations")
        if max_active_exacontinuums is not None:
            continuum_kwargs["max_active_exacontinuums"] = int(
        ...
        except zz.ZettacontinuumRefused as exc:
            if local_index == 0 and not resumed:
                raise YottacontinuumRefused(exc.verdict, exc.detail) from exc
        ...
        nested_zettacontinuum_dir = continuum_result.get("zettacontinuum_dir")
        nested_multiverse_states: list[dict[str, Any]] = []
        nested_receipt_path = (
            Path(str(nested_zettacontinuum_dir)) / "zettacontinuum.json"
            if nested_zettacontinuum_dir
            else None
        )
        if nested_receipt_path is not None and nested_receipt_path.is_file():
            ...
            for ist in list(
                receipt.get("exacontinuum_states")
        ...
        if not nested_multiverse_states:
            for ist in list(
                continuum_result.get("exacontinuum_states")
        ...
        for i, lst in enumerate(zettacontinuum_states):
            if str(lst.get("zettacontinuum_id")) != zettacontinuum_id:
                continue
            updated = dict(lst)
            updated["last_zettacontinuum_dir"] = continuum_result.get("zettacontinuum_dir")
            updated["last_zettacontinuum_digest"] = continuum_result.get("zettacontinuum_digest")
            updated["zettacontinuum_met"] = bool(continuum_result.get("zettacontinuum_met"))
            ...
            if nested_multiverse_states:
                updated["exacontinuum_states"] = nested_multiverse_states
        ...
        coverage_after = yottacontinuum_terminal_coverage(
            zettacontinuum_states=zettacontinuum_states,
            federated_portfolio=federated_portfolio,
        )

        roi = score_zettacontinuum_roi(
            round_index=round_index,
            zettacontinuum_id=zettacontinuum_id,
        ...
        idigest = str(continuum_result.get("zettacontinuum_digest") or "")
        if idigest:
            zettacontinuum_digests.append(idigest)

        rec = _zettacontinuum_round_record(
            round_index=round_index,
            zettacontinuum_id=zettacontinuum_id,
            continuum_result=continuum_result,
            coverage_before=coverage_before,
        ...
        write_yottacontinuum_state(
            yottacontinuum_dir,
            _state_payload(
                yottacontinuum_id=lid,
                ...
                roi_history=roi_history,
                zettacontinuum_states=zettacontinuum_states,
                zettacontinuum_digests=zettacontinuum_digests,
                charter=active_charter,
                stop_reason=None,
                yottacontinuum_goal=yottacontinuum_goal,
                max_active_zettacontinuums=active_max,
        ...
        if (
            ...
            and not continuum_result.get("zettacontinuum_met")
        ):
            idle_streak += 1
        else:
            idle_streak = 0
            ...
            reason = stop_when(
                {
                    ...
                    "roi_history": roi_history,
                    "zettacontinuum_states": zettacontinuum_states,
                    "last_zettacontinuum_id": zettacontinuum_id,
                    "federated_portfolio": federated_portfolio,
                    "yottacontinuum_dir": str(yottacontinuum_dir),
                    "pending_zettacontinuum_ids": [
                        str(s.get("zettacontinuum_id") or "")
        ...
        # Grow constitution before declaring yottacontinuum_met when expand remains.
        if (
            charter_expand is not None
            and not pending_charter_slots(active_charter, zettacontinuum_states)
            and zettacontinuums_all_met(zettacontinuum_states)
        ):
    ...
    coverage_end = yottacontinuum_terminal_coverage(
        zettacontinuum_states=zettacontinuum_states,
        federated_portfolio=federated_portfolio,
    )
    if constitution_satisfied(
    ...
    zettacontinuums_met_count = sum(
        1 for ist in zettacontinuum_states if ist.get("zettacontinuum_met")
    )
    pending_remaining = [
        str(s.get("zettacontinuum_id") or "")
        for s in pending_charter_slots(active_charter, zettacontinuum_states)
    ]

    if yottacontinuum_met and stop_reason in {"yottacontinuum_met", "max_rounds"}:
        verdict = "yottacontinuum_met"
        ok = True
        stop_reason = "yottacontinuum_met"
    elif stop_reason == "rank_only":
        verdict = "yottacontinuum_ranked"
        ok = True
    elif stop_reason == "yottacontinuum_idle":
        verdict = "yottacontinuum_idle"
        ok = True
    elif stop_reason == "dispatch_budget":
        verdict = "yottacontinuum_budgeted"
        ok = True
        ...
        verdict = "yottacontinuum_completed"
        ok = True
    ...
    receipt: dict[str, Any] = {
        ...
        "yottacontinuum_goal": yottacontinuum_goal,
        "yottacontinuum_met": yottacontinuum_met,
        ...
        "round_count": len(institutions),
        "zettacontinuums": institutions,
        "zettacontinuum_digests": [
            i.get("zettacontinuum_digest")
            for i in institutions
            if i.get("zettacontinuum_digest")
        ],
        "zettacontinuum_states": zettacontinuum_states,
        "zettacontinuums_admitted": len(zettacontinuum_states),
        "zettacontinuums_met_count": zettacontinuums_met_count,
        "admissions": admissions,
    ...
    receipt["zettacontinuum_digests"] = [
        str(i.get("zettacontinuum_digest") or "") for i in institutions
    ]
    receipt["yottacontinuum_digest"] = _sha256_json(_yottacontinuum_digest_payload(receipt))
    atomic_write_json(yottacontinuum_dir / "yottacontinuum.json", receipt)
    atomic_write_json(
        yottacontinuum_dir / "summary.json",
        {
            "verdict": receipt["verdict"],
            "ok": receipt["ok"],
            "stop_reason": receipt["stop_reason"],
            "yottacontinuum_id": receipt["yottacontinuum_id"],
            "round_count": receipt["round_count"],
            "total_dispatched": receipt["total_dispatched"],
            "total_dispatched_ok": receipt["total_dispatched_ok"],
            "yottacontinuum_met": receipt["yottacontinuum_met"],
            ...
            "max_active_zettacontinuums": receipt["max_active_zettacontinuums"],
            ...
            "yottacontinuum_digest": receipt["yottacontinuum_digest"],
            "resumed": resumed,
        },
    )
    ...
    write_yottacontinuum_state(
        ...
        _state_payload(
            ...
            roi_history=roi_history,
            zettacontinuum_states=zettacontinuum_states,
            zettacontinuum_digests=receipt["zettacontinuum_digests"],
            charter=active_charter,
            stop_reason=stop_reason,
            yottacontinuum_goal=yottacontinuum_goal,
            max_active_zettacontinuums=active_max,
    ...
    return {
        ...
        "yottacontinuum_id": lid,
        "round_count": len(institutions),
        "zettacontinuum_digests": list(receipt["zettacontinuum_digests"]),
        "total_dispatched": total_dispatched,
        "total_dispatched_ok": total_dispatched_ok,
        "yottacontinuum_met": yottacontinuum_met,
        "zettacontinuums_admitted": len(zettacontinuum_states),
...
def _zettacontinuum_slot(
    zettacontinuum_id: str,
    *,
    priority: int = 0,
    exacontinuums: Sequence[dict[str, Any]] | None = None,
    ...
    """Build a hermetic yottacontinuum charter zettacontinuum slot.

    Prefer ``exacontinuums=`` (nested giga slots for the zettacontinuum plane).
    ``institutions=`` wrap a single auto giga with mega nesting underneath.
    """
    nested: list[dict[str, Any]]
    if exacontinuums is not None:
        nested = list(exacontinuums)
    elif megacontinuums is not None:
        nested = [
            zz._exacontinuum_slot(
                f"{zettacontinuum_id[:1]}g",
...
def builtin_upstream_yottacontinuum_proof() -> dict[str, Any]:
    """Hermetic end-to-end proof of the multi-zettacontinuum yottacontinuum plane (no network).

    Uses an injected fast ``zettacontinuum_runner`` so the outer plane constitution
    ...
    try:
        # Per-zettacontinuum progress for budget/resume/terminal multi-call realism.
        ...
        def _fast_mega_runner(**kwargs: Any) -> dict[str, Any]:
            ...
            mid = str(kwargs.get("zettacontinuum_id") or "m")
            nested_charter = zz.normalize_zettacontinuum_charter(kwargs.get("charter"))
            if kwargs.get("resume_dir") is not None and not nested_charter:
                state_path = Path(str(kwargs["resume_dir"])) / "zettacontinuum_state.json"
                if state_path.is_file():
                    st = json.loads(state_path.read_text(encoding="utf-8"))
                    nested_charter = zz.normalize_zettacontinuum_charter(st.get("charter"))
                    mid = str(st.get("zettacontinuum_id") or mid)

            goal = str(kwargs.get("zettacontinuum_goal") or "all_exacontinuums_met")
            ...
            include_deferred = st["calls"] > 1 or goal == "all_exacontinuums_met"
            ...
            exacontinuum_states = [
                {
                    "exacontinuum_id": str(
                        s.get("exacontinuum_id") or s.get("id") or "g"
                    ),
            ...
            receipt = {
                "schema_version": 1,
                "ok": True,
                "verdict": "zettacontinuum_met" if report_met else "zettacontinuum_completed",
                "stop_reason": "zettacontinuum_met" if report_met else "dispatch_budget",
                "zettacontinuum_id": mid,
                "zettacontinuum_goal": str(
                    kwargs.get("zettacontinuum_goal") or "all_exacontinuums_met"
                ),
                "max_rounds": int(kwargs.get("max_rounds") or 6),
                "max_active_exacontinuums": kwargs.get("max_active_exacontinuums"),
                ...
                "exacontinuums_admitted": max(1, len(nested_charter) or 1),
                "exacontinuums_met_count": (
            ...
            receipt["zettacontinuum_digest"] = _sha256_json(
                zz._zettacontinuum_digest_payload(receipt)
            )
            digest = receipt["zettacontinuum_digest"]
            atomic_write_json(out / "zettacontinuum.json", receipt)
            atomic_write_json(
                out / "zettacontinuum_state.json",
                {
                    "zettacontinuum_id": mid,
                    "round_count": st["calls"],
                    "total_dispatched": st["dispatched"],
            ...
            return {
                ...
                "verdict": receipt["verdict"],
                "stop_reason": receipt["stop_reason"],
                "zettacontinuum_dir": str(out),
                "zettacontinuum_digest": digest,
                "zettacontinuum_id": mid,
                "total_dispatched": st["dispatched"],
                "total_dispatched_ok": st["dispatched_ok"],
                "zettacontinuum_met": report_met,
        ...
        confed = run_yottacontinuum(
            charter=charter,
            max_rounds=8,
            dispatch_budget=12,
            dispatch=True,
            zettacontinuum_runner=_fast_mega_runner,
            yottacontinuum_goal="all_zettacontinuums_met",
            out_root=scratch / "m",
        )
        multi_yottacontinuum_ok = (
            confed["ok"]
            and confed["yottacontinuum_met"] is True
            and confed["stop_reason"] == "yottacontinuum_met"
            and confed["zettacontinuums_admitted"] == 2
            and confed["zettacontinuums_met_count"] == 2
            and confed["round_count"] >= 2
            and confed["total_dispatched_ok"] >= 3
            and float((confed.get("coverage_end") or {}).get("coverage_ratio") or 0) == 1.0
        )
        scheduled_ids = {
            i.get("zettacontinuum_id")
            for i in (confed.get("zettacontinuums") or [])
        }
        multi_yottacontinuum_scheduled = multi_yottacontinuum_ok and scheduled_ids >= {"a", "b"}

        verified = verify_yottacontinuum_receipt(Path(confed["yottacontinuum_dir"]))
        seal_ok = bool(verified.get("ok")) and verified.get("round_count") == confed["round_count"]

        confed_path = Path(confed["yottacontinuum_dir"]) / "yottacontinuum.json"
        receipt = json.loads(confed_path.read_text(encoding="utf-8"))
        receipt["yottacontinuum_digest"] = "0" * 64
        confed_path.write_text(json.dumps(receipt, indent=2), encoding="utf-8")
        tampered = verify_yottacontinuum_receipt(Path(confed["yottacontinuum_dir"]))
        tamper_detected = (
            not tampered["ok"]
            and "yottacontinuum_digest" in (tampered.get("mismatched") or [])
        )

        # Budget stop
        _progress.clear()
        budgeted = run_yottacontinuum(
            charter=[
        ...
        def _premet_runner(**kwargs: Any) -> dict[str, Any]:
            ...
            receipt = {
                ...
                "zettacontinuum_id": kwargs.get("zettacontinuum_id"),
                "zettacontinuum_goal": "all_exacontinuums_met",
                "max_rounds": 1,
                "max_active_exacontinuums": None,
                ...
                "exacontinuum_digests": [],
                "exacontinuums_admitted": max(1, len(nested_charter) or 1),
                "exacontinuums_met_count": max(1, len(nested_charter) or 1),
                "admission_count": 0,
                "pending_remaining": [],
                "charter_expansion_count": 0,
                "charter_expanded_ids": [],
            ...
            atomic_write_json(
                out / "zettacontinuum_state.json",
                {
                    ...
                    "total_dispatched_ok": 0,
                    "federated_portfolio": portfolio,
                    "exacontinuum_states": receipt["exacontinuum_states"],
                    "stop_reason": "zettacontinuum_met",
                    "charter": nested_charter,
                },
            )
        ...
        pre_met = run_yottacontinuum(
            charter=[_slot("om", initial=[("om", "9.0.0", "omega-merged")])],
            max_rounds=3,
            dispatch=True,
            zettacontinuum_runner=_premet_runner,
            yottacontinuum_goal="all_zettacontinuums_met",
            out_root=scratch / "p",
        )
        premet_ok = (
            pre_met["ok"]
            and pre_met["yottacontinuum_met"] is True
            and pre_met["stop_reason"] == "yottacontinuum_met"
            and pre_met["zettacontinuums_met_count"] == 1
            and pre_met["total_dispatched"] == 0
        )

        ranked = run_yottacontinuum(
            charter=[
        ...
        rank_only_ok = (
            ranked["ok"]
            and ranked["verdict"] == "yottacontinuum_ranked"
        ...
        except YottacontinuumRefused as exc:
        ...
        partial = run_yottacontinuum(
            ...
            zettacontinuum_runner=_fast_mega_runner,
            yottacontinuum_goal="none",
            yottacontinuum_id="rcp",
            out_root=scratch / "a",
        )
        state_path = Path(partial["yottacontinuum_dir"]) / "yottacontinuum_state.json"
        state_exists = state_path.is_file()
        resumed = run_yottacontinuum(
            resume_dir=Path(partial["yottacontinuum_dir"]),
            max_rounds=4,
            dispatch_budget=4,
            dispatch=True,
            zettacontinuum_runner=_fast_mega_runner,
        ...
        resume_ok = (
            ...
            and resumed["yottacontinuum_id"] == "rcp"
        ...
        admit_ids = [a.get("zettacontinuum_id") for a in (deferred.get("admissions") or [])]
        deferred_ok = (
            deferred["ok"]
            and deferred["yottacontinuum_met"] is True
            and deferred["zettacontinuums_admitted"] == 3
            and deferred["zettacontinuums_met_count"] == 3
            and deferred.get("max_active_zettacontinuums") == 1
        ...
        expand_ok = (
            expanded["ok"]
            and expanded["yottacontinuum_met"] is True
            and expanded["zettacontinuums_admitted"] == 2
            and expanded["zettacontinuums_met_count"] == 2
        ...
        merged = merge_yottacontinuum_charter(
        ...
        merge_ok = [s["zettacontinuum_id"] for s in merged] == ["m1", "m2"]

        # Terminal coverage with premature-met + deferred reopen (ids start with t)
        _progress.clear()
        terminal = run_yottacontinuum(
            charter=[
        ...
        terminal_ok = (
            terminal["ok"]
            and terminal["yottacontinuum_met"] is True
            and terminal["stop_reason"] == "yottacontinuum_met"
            ...
            and terminal["zettacontinuums_admitted"] == 2
        ...
        ok = all(
            [
                multi_yottacontinuum_ok,
                multi_yottacontinuum_scheduled,
                seal_ok,
                tamper_detected,
                budget_ok,
                premet_ok,
        ...
        return {
            "ok": ok,
            "yottacontinuum_met": multi_yottacontinuum_ok,
            "multi_yottacontinuum_progressed": multi_yottacontinuum_scheduled,
            ...
            "roi_scored": roi_ok,
            "yottacontinuum_digest": confed.get("yottacontinuum_digest"),
            "round_count": confed.get("round_count"),
            "total_dispatched_ok": confed.get("total_dispatched_ok"),
            "zettacontinuums_admitted": confed.get("zettacontinuums_admitted"),
            "used_skill_route_discovery": legacy_pipeline_was_used(),
            "flags": {
                "multi_yottacontinuum_ok": multi_yottacontinuum_ok,
                "multi_yottacontinuum_scheduled": multi_yottacontinuum_scheduled,
                "seal_ok": seal_ok,
                "tamper_detected": tamper_detected,
                "budget_ok": budget_ok,
                "premet_ok": premet_ok,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.verify:
        result = verify_yottacontinuum_receipt(Path(args.verify))
        print(json.dumps(result, indent=2, sort_keys=True))
        return 0 if result.get("ok") else 1
    if args.proof:
        result = builtin_upstream_yottacontinuum_proof()
        print(json.dumps(result, indent=2, sort_keys=True))
        return 0 if result.get("ok") else 1
    parser.print_help()
    return 2
