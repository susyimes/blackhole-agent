...
def builtin_mcp_prompts_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="mcp-prompts-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, MCP_PROMPTS_GOAL, MCP_PROMPTS_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_prompts"] = (
        not gate.accepted
        and live_goal != MCP_PROMPTS_GOAL
        and MCP_PROMPTS_ID not in live_done
