...
def builtin_smtp_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="smtp-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, SMTP_ACTUATION_GOAL, SMTP_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_smtp"] = (
        not gate.accepted
        and live_goal != SMTP_ACTUATION_GOAL
        and SMTP_ACTUATION_ID not in live_done
