...
def builtin_mcp_structured_output_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="mcp-structured-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, MCP_STRUCTURED_GOAL, MCP_STRUCTURED_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_structured"] = (
        not gate.accepted
        and live_goal != MCP_STRUCTURED_GOAL
        and MCP_STRUCTURED_ID not in live_done
