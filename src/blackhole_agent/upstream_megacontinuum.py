"""Upstream megacontinuum plane: multi-ultracontinuum durable stewardship megacontinuum.

The ultracontinuum plane (``upstream_ultracontinuum``) closes multi-continuum unions *within one
ultracontinuum*. It does not:

1. chain multiple independent ultracontinuums under a durable megacontinuum constitution;
2. allocate a shared global dispatch budget across ultracontinuums by ROI;
3. admit/retire ultracontinuum slots from an megacontinuum charter over time
   (deferred admission under a concurrent-active cap);
4. grow the megacontinuum charter mid-run via ``charter_expand`` (constitution
   growth beyond the initial charter, not just deferred admission of a fixed set);
5. federate multi-ultracontinuum portfolio coverage into one megacontinuum world-model;
6. persist megacontinuum state so a later process can resume the union;
7. seal a multi-ultracontinuum megacontinuum chronicle linking ultracontinuum digests.

The megacontinuum plane closes that outer multi-ultracontinuum loop:

1. **admit** — materialize ultracontinuum slots from a durable megacontinuum charter
   (each slot owns a nested continuum charter). When ``max_active_ultracontinuums``
   is set, only that many *unmet* ultracontinuums are concurrent: further charter
   slots stay pending and are admitted as capacity frees after retirements
   (megacontinuum constitution growth over time);
2. **schedule** — pick the next open ultracontinuum by priority and historical ROI;
3. **ultracontinuum** — call the ultracontinuum plane (injected ``ultracontinuum_runner``;
   default ``run_ultracontinuum``) with a share of the remaining global dispatch
   budget;
4. **federate** — merge per-ultracontinuum portfolios into one megacontinuum world-model
   and re-score coverage across all stewarded keys;
5. **retire** — mark ultracontinuums met when their ultracontinuum_goal is satisfied,
   then re-admit pending charter slots up to the active capacity;
6. **expand** — optional ``charter_expand`` may append new ultracontinuum slots when
   the active charter has no pending work and all admitted ultracontinuums are met,
   so the megacontinuum constitution can grow after start (not only defer a fixed charter);
7. **persist** — write ``megacontinuum_state.json`` after every ultracontinuum round so a
   later ``run_megacontinuum(..., resume_dir=...)`` continues the same union
   (including pending charter and admission history);
8. **stop** when any of:

   - ``max_rounds`` reached
   - global ``dispatch_budget`` exhausted across ultracontinuums
   - megacontinuum goal met (``all_ultracontinuums_met``: every *admitted*
     ultracontinuum is met *and* no pending charter slots remain)
   - consecutive idle/no-progress rounds (``idle_round_limit``)
   - explicit ``stop_when`` predicate returns a reason string

9. **seal** — write an megacontinuum receipt under
   ``artifacts/upstream-megacontinuum/`` with sha256 digests of every
   ultracontinuum, portfolio federation, admission history, ROI history, stop
   reason, and an megacontinuum chain digest; ``verify_megacontinuum_receipt``
   re-checks the chain and detects tampering.

No skill-route discovery is used. The plane is megacontinuum-level direction
over the ultracontinuum plane, not a new verifier of individual repairs.
"""
...
ARTIFACTS_ROOT = REPO_ROOT / "artifacts" / "upstream-megacontinuum"

TERMINAL_SUCCESS_OUTCOMES = uu.TERMINAL_SUCCESS_OUTCOMES


class MegacontinuumRefused(Exception):
...
def normalize_megacontinuum_charter(
    charter: Sequence[Mapping[str, Any]] | None,
) -> list[dict[str, Any]]:
    """Normalize a megacontinuum charter into deterministic continuum slots.
    ...
    for raw in charter:
        if not isinstance(raw, Mapping):
            continue
        ultracontinuum_id = str(
            raw.get("ultracontinuum_id") or raw.get("id") or ""
        ).strip()
        if not ultracontinuum_id or ultracontinuum_id in seen:
            continue
        seen.add(ultracontinuum_id)

        nested = uu.normalize_ultracontinuum_charter(
            raw.get("charter")
            or raw.get("hypercontinuums")
        ...
        max_active_hypercontinuums = raw.get("max_active_hypercontinuums")
        if max_active_hypercontinuums is None:
            max_active_hypercontinuums = raw.get("max_active_continuums")
        if max_active_hypercontinuums is None:
            max_active_hypercontinuums = raw.get("max_active_civilizations")
        if max_active_hypercontinuums is not None:
            max_active_hypercontinuums = max(1, int(max_active_hypercontinuums))
        ...
        out.append(
            {
                ...
                "ultracontinuum_goal": str(
                    raw.get("ultracontinuum_goal") or "all_hypercontinuums_met"
                ),
                "kind": str(raw.get("kind") or "stewardship_ultracontinuum"),
            }
        )
    return out


def admit_ultracontinuum_slot(
    *,
    megacontinuum_dir: Path,
    slot: Mapping[str, Any],
) -> dict[str, Any]:
    """Materialize one institution slot under the league directory.

    Returns admission record with ultracontinuum_root and nested charter.
    Stewardship surfaces are created lazily by the institution plane on run.
    """
    ultracontinuum_id = str(slot.get("ultracontinuum_id") or "")
    if not ultracontinuum_id:
        raise MegacontinuumRefused("megacontinuum_invalid", "slot missing ultracontinuum_id")

    ultracontinuum_root = Path(megacontinuum_dir) / "ultracontinuums" / ultracontinuum_id
    ultracontinuum_root.mkdir(parents=True, exist_ok=True)

    nested_charter = uu.normalize_ultracontinuum_charter(slot.get("charter"))
    if not nested_charter:
        raise MegacontinuumRefused(
            "megacontinuum_invalid",
            f"continuum slot {ultracontinuum_id!r} has empty nested charter",
        )

    max_active_hypercontinuums = slot.get("max_active_hypercontinuums")
    if max_active_hypercontinuums is None:
        max_active_hypercontinuums = slot.get("max_active_continuums")
    if max_active_hypercontinuums is None:
        max_active_hypercontinuums = slot.get("max_active_civilizations")

    return {
        "ultracontinuum_id": ultracontinuum_id,
        "ultracontinuum_root": str(ultracontinuum_root),
        "admitted": True,
        "charter": nested_charter,
        "max_active_hypercontinuums": max_active_hypercontinuums,
        "max_rounds": int(slot.get("max_rounds") or 6),
        "ultracontinuum_goal": str(slot.get("ultracontinuum_goal") or "all_hypercontinuums_met"),
...
def federate_portfolios(
    ...
    portfolio: dict[str, Any] = {
        ...
        "entries": entries,
        "source": "megacontinuum_federation",
    }
...
def megacontinuum_terminal_coverage(
    *,
    ultracontinuum_states: Sequence[Mapping[str, Any]],
    federated_portfolio: Mapping[str, Any] | None,
) -> dict[str, Any]:
    """Coverage across ultracontinuum->hypercontinuum->continuum->...->program."""
    hypercontinuum_states: list[dict[str, Any]] = []
    for uc in ultracontinuum_states:
        if not isinstance(uc, Mapping):
            continue
        nested = list(uc.get("hypercontinuum_states") or [])
        if nested:
            for hc in nested:
                if isinstance(hc, Mapping):
                    hypercontinuum_states.append(dict(hc))
            continue
        # Pre-run: ultracontinuum charter is a list of hypercontinuum slots.
        for cslot in list(uc.get("charter") or []) + list(
            uc.get("hypercontinuums") or []
        ) + list(uc.get("continuums") or []) + list(uc.get("omniverses") or []) + list(
            uc.get("multiverses") or []
        ):
            if isinstance(cslot, Mapping):
                hypercontinuum_states.append(dict(cslot))
        if not nested and not list(uc.get("charter") or []):
            for cont in list(uc.get("continuum_states") or []) + list(
                ...
                if isinstance(cont, Mapping):
                    hypercontinuum_states.append(dict(cont))
        for raw in list(uc.get("inventory_keys") or []):
            if isinstance(raw, (list, tuple)) and len(raw) >= 3:
                ...
                hypercontinuum_states.append({"inventory_keys": [raw]})
    return uu.ultracontinuum_terminal_coverage(
        hypercontinuum_states=hypercontinuum_states,
        federated_portfolio=federated_portfolio,
    )


def ultracontinuums_all_met(ultracontinuum_states: Sequence[Mapping[str, Any]]) -> bool:
    if not ultracontinuum_states:
        return False
    return all(bool(ist.get("ultracontinuum_met")) for ist in ultracontinuum_states)


def open_unmet_count(ultracontinuum_states: Sequence[Mapping[str, Any]]) -> int:
    """Count admitted institutions that are not yet continuum_met."""
    return sum(1 for ist in ultracontinuum_states if not ist.get("ultracontinuum_met"))
...
def pending_charter_slots(
    ...
    """Charter slots not yet admitted, priority-desc then ultracontinuum_id-asc."""
    known = {str(ist.get("ultracontinuum_id") or "") for ist in ultracontinuum_states}
    pending = [
        dict(slot)
        for slot in charter
        if str(slot.get("ultracontinuum_id") or "")
        and str(slot.get("ultracontinuum_id")) not in known
    ]
    pending.sort(
        key=lambda s: (
            -int(s.get("priority") or 0),
            str(s.get("ultracontinuum_id") or ""),
        )
    )
    return pending
...
def constitution_satisfied(
    *,
    ...
    if megacontinuum_goal == "none":
        return False
    if megacontinuum_goal == "terminal_coverage":
        cov = megacontinuum_terminal_coverage(
            ultracontinuum_states=ultracontinuum_states,
            federated_portfolio=federated_portfolio,
        )
        return bool(cov.get("met")) and not pending_charter_slots(
            charter, ultracontinuum_states
        )
    if megacontinuum_goal == "all_ultracontinuums_met":
        if not ultracontinuum_states:
            return False
        if pending_charter_slots(charter, ultracontinuum_states):
            return False
        return ultracontinuums_all_met(ultracontinuum_states)
    return False


def reopen_incomplete_ultracontinuums(
    ...
    *,
    ...
    Nested ultracontinuums can retire after only partial surface work (e.g. before
    deferred program targets expand). Under a ``terminal_coverage`` megacontinuum
    goal those children must re-run until federated inventory is fully
    terminal-success. Returns the reopened ``ultracontinuum_id`` list.
    """
    cov = megacontinuum_terminal_coverage(
        ultracontinuum_states=ultracontinuum_states,
        federated_portfolio=federated_portfolio,
    ...
    for item in list(cov.get("open_or_missing") or []):
        ...
        if key[0] and key[2]:
            open_keys.add(key)
    reopened: list[str] = []
    for i, ist in enumerate(ultracontinuum_states):
        if not ist.get("ultracontinuum_met"):
            continue
        ...
        # Walk nested hypercontinuum / multiverse / program charter surfaces.
        for mv in list(ist.get("hypercontinuum_states") or []) + list(
        ...
        if not owns_open:
            continue
        updated = dict(ist)
        updated["ultracontinuum_met"] = False
        ultracontinuum_states[i] = updated
        reopened.append(str(updated.get("ultracontinuum_id") or ""))
    return [r for r in reopened if r]


def merge_megacontinuum_charter(
    ...
    Existing ``ultracontinuum_id`` values win (additions with the same id are
    ignored). Returns a fully re-normalized charter so nested confederation
    charters stay deterministic.
    """
    base = normalize_megacontinuum_charter(existing)
    if not additions:
        return base
    known = {str(s.get("ultracontinuum_id") or "") for s in base}
    merged: list[Mapping[str, Any]] = list(base)
    for raw in additions:
        if not isinstance(raw, Mapping):
            continue
        cid = str(raw.get("ultracontinuum_id") or raw.get("id") or "").strip()
        if not cid or cid in known:
            continue
        known.add(cid)
        merged.append(raw)
    return normalize_megacontinuum_charter(merged)


def make_megacontinuum_charter_expand(
    ...
    *,
    ...
    """
    pending_growth = normalize_megacontinuum_charter(growth)
    ...
    def _runner(
        *,
        ...
        remaining = [
            ...
            if str(s.get("ultracontinuum_id") or "") not in applied_ids
            and str(s.get("ultracontinuum_id") or "")
            not in {str(x.get("ultracontinuum_id") or "") for x in active_charter}
        ]
        if not remaining:
            return {
                "expanded": False,
        ...
        for s in take:
            applied_ids.add(str(s.get("ultracontinuum_id") or ""))
        merged = merge_megacontinuum_charter(active_charter, take)
        state["applied"] = applied_ids
        return {
            "expanded": True,
            "added": [str(s.get("ultracontinuum_id") or "") for s in take],
            "charter": merged,
            "detail": "charter_growth_applied",
            "round_index": round_index,
            "realms_met": ultracontinuums_all_met(ultracontinuum_states),
        }
...
def admit_pending_slots(
    *,
    ...
    ``max_active_ultracontinuums`` caps *unmet* concurrent realms. ``None``
    admits every remaining pending slot. Returns admission records for newly
    admitted slots (also mutates ``ultracontinuum_states``).
    """
    pending = pending_charter_slots(charter, ultracontinuum_states)
    if not pending:
        return []

    open_n = open_unmet_count(ultracontinuum_states)
    if max_active_ultracontinuums is None:
        capacity = len(pending)
    else:
        capacity = max(0, int(max_active_ultracontinuums) - open_n)
    if capacity <= 0:
        return []

    admissions: list[dict[str, Any]] = []
    for slot in pending[:capacity]:
        admission = admit_ultracontinuum_slot(megacontinuum_dir=megacontinuum_dir, slot=slot)
        ...
        ultracontinuum_states.append(
            {
                "ultracontinuum_id": admission["ultracontinuum_id"],
                "ultracontinuum_root": admission["ultracontinuum_root"],
                "charter": admission["charter"],
                "max_active_hypercontinuums": admission.get("max_active_hypercontinuums"),
                "max_rounds": admission["max_rounds"],
                "ultracontinuum_goal": admission["ultracontinuum_goal"],
                "priority": admission["priority"],
                "ultracontinuum_met": False,
                "last_ultracontinuum_dir": None,
                "last_ultracontinuum_digest": None,
                "portfolio": None,
                "omniverse_states": [],
                "multiverse_states": [],
                "cosmos_states": [],
...
def score_ultracontinuum_roi(
    *,
    ...
    return {
        ...
        "ultracontinuum_met": bool(continuum_result.get("ultracontinuum_met")),
        "continuum_digest": continuum_result.get("ultracontinuum_digest"),
        "continuums_admitted": int(
            continuum_result.get("hypercontinuums_admitted")
        ...
        "continuums_met_count": int(
            continuum_result.get("hypercontinuums_met_count")
...
def _roi_summary(roi_history: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for r in roi_history:
        iid = str(r.get("ultracontinuum_id") or "")
...
def select_next_ultracontinuum(
    ultracontinuum_states: Sequence[Mapping[str, Any]],
    roi_history: Sequence[Mapping[str, Any]],
    *,
    round_index: int,
    ...
    Tie-break: lower run_count, then stable ultracontinuum_id order. Round-robin
    among equal scores uses ``round_index`` so multi-institution progress is fair.
    """
    open_slots = [
        dict(ist) for ist in ultracontinuum_states if not ist.get("ultracontinuum_met")
    ]
    if not open_slots:
        return None
    ...
    def sort_key(ist: Mapping[str, Any]) -> tuple[Any, ...]:
        iid = str(ist.get("ultracontinuum_id") or "")
...
def allocate_ultracontinuum_budget(
    *,
    ...
    hist = (summary.get("by_continuum") or {}).get(
        str(selected.get("ultracontinuum_id") or "")
    ) or {}
    mean_eff = float(hist.get("mean_efficiency") or 0.0)
...
def _state_payload(
    *,
    ...
    roi_history: Sequence[Mapping[str, Any]],
    ultracontinuum_states: Sequence[Mapping[str, Any]],
    ultracontinuum_digests: Sequence[str],
    charter: Sequence[Mapping[str, Any]],
    ...
    return {
        ...
        "ultracontinuum_states": [dict(ist) for ist in ultracontinuum_states],
        "ultracontinuum_digests": list(ultracontinuum_digests),
        ...
        "pending_ultracontinuum_ids": [
            str(s.get("ultracontinuum_id") or "")
            for s in pending_charter_slots(charter, ultracontinuum_states)
        ],
    }


def write_megacontinuum_state(megacontinuum_dir: Path, state: Mapping[str, Any]) -> Path:
    path = Path(megacontinuum_dir) / "megacontinuum_state.json"
    atomic_write_json(path, dict(state))
    return path


def load_megacontinuum_state(resume_dir: Path) -> dict[str, Any]:
    path = durable_read_path(Path(resume_dir) / "megacontinuum_state.json")
    if not path.is_file():
        raise MegacontinuumRefused(
            "megacontinuum_state_missing",
            f"no megacontinuum_state.json under {resume_dir}",
        )
    try:
        state = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise MegacontinuumRefused("megacontinuum_state_unreadable", str(exc)) from exc
    if not isinstance(state, dict):
        raise MegacontinuumRefused("megacontinuum_state_invalid", "state root must be object")
    return state
...
def _ultracontinuum_round_record(
    *,
    round_index: int,
    ultracontinuum_id: str,
    continuum_result: Mapping[str, Any],
    ...
    return {
        ...
        "stop_reason": continuum_result.get("stop_reason"),
        "ultracontinuum_dir": continuum_result.get("ultracontinuum_dir"),
        "ultracontinuum_digest": continuum_result.get("ultracontinuum_digest"),
        "continuums_admitted": int(
...
def _megacontinuum_digest_payload(receipt: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": receipt.get("schema_version"),
        "verdict": receipt.get("verdict"),
        "stop_reason": receipt.get("stop_reason"),
        "megacontinuum_id": receipt.get("megacontinuum_id"),
        "megacontinuum_goal": receipt.get("megacontinuum_goal"),
        "max_rounds": receipt.get("max_rounds"),
        "max_active_ultracontinuums": receipt.get("max_active_ultracontinuums"),
        ...
        "ultracontinuum_digests": list(receipt.get("ultracontinuum_digests") or []),
        "total_dispatched": receipt.get("total_dispatched"),
        "total_dispatched_ok": receipt.get("total_dispatched_ok"),
        "megacontinuum_met": receipt.get("megacontinuum_met"),
        "coverage_end": receipt.get("coverage_end"),
        "ultracontinuums_met_count": receipt.get("ultracontinuums_met_count"),
        "ultracontinuums_admitted": receipt.get("ultracontinuums_admitted"),
...
def verify_megacontinuum_receipt(megacontinuum_dir: Path) -> dict[str, Any]:
    """Re-check a sealed league receipt for digest integrity."""
    path = durable_read_path(Path(megacontinuum_dir) / "megacontinuum.json")
    if not path.is_file():
    ...
    expected = _sha256_json(_megacontinuum_digest_payload(receipt))
    recorded = str(receipt.get("megacontinuum_digest") or "")
    mismatched: list[str] = []
    if not recorded or recorded != expected:
        mismatched.append("megacontinuum_digest")

    institutions = list(receipt.get("ultracontinuums") or receipt.get("realms") or receipt.get("leagues") or [])
    listed = list(receipt.get("ultracontinuum_digests") or [])
    if len(listed) != len(institutions):
        mismatched.append("ultracontinuum_digests_length")
    else:
        for i, (listed_d, rec) in enumerate(zip(listed, institutions)):
            if listed_d != rec.get("ultracontinuum_digest"):
                mismatched.append(f"ultracontinuum_digests[{i}]")

    nested_failures: list[str] = []
    for rec in institutions:
        idir = rec.get("ultracontinuum_dir")
        if not idir:
            continue
        ip = Path(str(idir))
        if (ip / "ultracontinuum.json").is_file():
            nested = uu.verify_ultracontinuum_receipt(ip)
    ...
    return {
        "ok": ok,
        "verdict": "megacontinuum_sealed" if ok else "megacontinuum_tampered",
        "megacontinuum_digest": recorded,
...
def run_megacontinuum(
    *,
    ...
    max_active_ultracontinuums: int | None = None,
    dispatch: bool = True,
    ultracontinuum_runner: Callable[..., dict[str, Any]] | None = None,
    ...
    megacontinuum_id: str | None = None,
    ...
    charter:
        ...
        :func:`normalize_megacontinuum_charter`).
    max_rounds:
    ...
    megacontinuum_goal:
        ``all_ultracontinuums_met`` (default) stops when every admitted institution
    ...
    resume_dir:
        Load ``megacontinuum_state.json`` from a prior league dir and continue.
        New receipt is written under ``out_root`` (or a fresh stamp).
    """
    if max_rounds < 1:
        raise MegacontinuumRefused("megacontinuum_invalid", "max_rounds must be >= 1")
    if per_wave_dispatch_limit < 0:
        raise MegacontinuumRefused(
            "megacontinuum_invalid", "per_wave_dispatch_limit must be >= 0"
        )
    if max_active_ultracontinuums is not None and int(max_active_ultracontinuums) < 1:
        raise MegacontinuumRefused(
            "megacontinuum_invalid", "max_active_ultracontinuums must be >= 1 when set"
        )
    if megacontinuum_goal not in {"all_ultracontinuums_met", "terminal_coverage", "none"}:
        raise MegacontinuumRefused(
            "megacontinuum_invalid",
            f"unknown megacontinuum_goal: {megacontinuum_goal}",
        )

    runner = ultracontinuum_runner or uu.run_ultracontinuum

    prior_round_count = 0
    roi_history: list[dict[str, Any]] = []
    ultracontinuum_digests: list[str] = []
    total_dispatched = 0
    total_dispatched_ok = 0
    resumed = False
    resume_megacontinuum_id: str | None = None
    ultracontinuum_states: list[dict[str, Any]] = []
    ...
    if resume_dir is not None:
        state = load_megacontinuum_state(resume_dir)
        resumed = True
        resume_megacontinuum_id = str(state.get("megacontinuum_id") or "") or None
        ...
        ultracontinuum_digests = [str(d) for d in (state.get("ultracontinuum_digests") or [])]
        ultracontinuum_states = [
            dict(ist)
            for ist in (state.get("ultracontinuum_states") or [])
        ...
        if isinstance(state.get("charter"), list):
            active_charter = normalize_megacontinuum_charter(
        ...
        if (
            state.get("max_active_ultracontinuums") is not None
            and max_active_ultracontinuums is None
        ):
            resumed_max_active = int(state["max_active_ultracontinuums"])
        # Resume may also merge a caller-supplied charter growth tail.
        if charter:
            active_charter = merge_megacontinuum_charter(active_charter, charter)
    else:
        active_charter = normalize_megacontinuum_charter(charter)
    ...
    if not active_charter and not ultracontinuum_states:
        raise MegacontinuumRefused(
            "megacontinuum_empty",
    ...
    lid = (
        megacontinuum_id
        or resume_megacontinuum_id
        or f"megacontinuum-{utc_now_iso().replace(':', '').replace('-', '')}"
    )

    stamp = utc_now_iso().replace(":", "").replace("-", "")
    if out_root is not None:
        megacontinuum_dir = Path(out_root)
        if (megacontinuum_dir / "megacontinuum.json").is_file():
            megacontinuum_dir = megacontinuum_dir / stamp
    else:
        megacontinuum_dir = ARTIFACTS_ROOT / stamp
    megacontinuum_dir.mkdir(parents=True, exist_ok=True)
    ...
    # on a flat ultra-short base (not under megacontinuum_dir) so one extra
    ...
    if league_out_root is not None:
        ...
        lg_root = megacontinuum_dir / "e"
    lg_root.mkdir(parents=True, exist_ok=True)
    if _os.name == "nt":
        inst_flat_root = Path("C:/t") / "j"
    else:
        inst_flat_root = megacontinuum_dir / "x"
    inst_flat_root.mkdir(parents=True, exist_ok=True)
    # Per-run namespace so parallel/resumed ultracontinua do not collide.
    # Prefer a short random token over the human lid (often "megacontinuum-…")
    # so Windows paths stay short and never re-enter a prior ultracontinuum_dir that
    ...
    initial_admissions = admit_pending_slots(
        megacontinuum_dir=megacontinuum_dir,
        charter=active_charter,
        ultracontinuum_states=ultracontinuum_states,
        max_active_ultracontinuums=active_max,
        round_index=prior_round_count,
    )
    admissions.extend(initial_admissions)

    if not ultracontinuum_states and not pending_charter_slots(
        active_charter, ultracontinuum_states
    ):
        raise MegacontinuumRefused("megacontinuum_empty", "no realm slots admitted")
    if not ultracontinuum_states and pending_charter_slots(
        active_charter, ultracontinuum_states
    ):
        raise MegacontinuumRefused(
            "megacontinuum_empty",
            "no realm slots admitted under max_active_ultracontinuums policy",
        )

    if federated_portfolio is None:
        federated_portfolio = federate_portfolios(
            [ist.get("portfolio") for ist in ultracontinuum_states]
        )
    ...
    max_terminal_reopens = max(2, (len(ultracontinuum_states) or 1) * 4)
    coverage_end: dict[str, Any] = megacontinuum_terminal_coverage(
        ultracontinuum_states=ultracontinuum_states,
        federated_portfolio=federated_portfolio,
    )
    ...
    for local_index in range(max_rounds):
        ...
        coverage_before = megacontinuum_terminal_coverage(
            ultracontinuum_states=ultracontinuum_states,
            federated_portfolio=federated_portfolio,
        )
        ...
        selected = select_next_ultracontinuum(
            ultracontinuum_states, roi_history, round_index=round_index
        )
        if selected is None:
            # Children may all report met while federated terminal coverage is
            # still incomplete (e.g. deferred surfaces). Only declare megacontinuum
            # met when the active megacontinuum_goal is actually satisfied.
            if constitution_satisfied(
                ultracontinuum_states=ultracontinuum_states,
                charter=active_charter,
                megacontinuum_goal=megacontinuum_goal,
                ...
                break
            if (
                megacontinuum_goal == "terminal_coverage"
                and terminal_reopen_count < max_terminal_reopens
            ):
                reopened = reopen_incomplete_ultracontinuums(
                    ultracontinuum_states,
                    federated_portfolio=federated_portfolio,
                )
                if reopened:
            ...
            if selected is None:
                stop_reason = "megacontinuum_idle"
                coverage_end = coverage_before
                break

        open_count = sum(
            1 for ist in ultracontinuum_states if not ist.get("ultracontinuum_met")
        )
        allocated = allocate_ultracontinuum_budget(
        ...
        ultracontinuum_id = str(selected["ultracontinuum_id"])
        resume_ultracontinuum_dir = selected.get("last_ultracontinuum_dir")
        continuum_resume: Path | None = None
        if (
            resume_ultracontinuum_dir
            and (Path(str(resume_ultracontinuum_dir)) / "ultracontinuum_state.json").is_file()
            and not selected.get("ultracontinuum_met")
        ):
            continuum_resume = Path(str(resume_ultracontinuum_dir))

        # Ultra-short stamp (each outer plane tightens Windows MAX_PATH).
        # Do NOT nest child ultracontinuum trees under megacontinuum roots:
        # let the ultracontinuum plane use its own flat C:/t/{h,i} bases so the
        # deep continuum→…→wave chain still fits. Only the ultracontinuum receipt
        # itself lands on a flat unique out_root.
        safe_id = "".join(c if c.isalnum() else "" for c in ultracontinuum_id)[:2] or "i"
        child_token = _secrets.token_hex(2)
        if _os.name == "nt":
            out_dir = Path("C:/t") / "m" / f"{child_token}{safe_id}"
            inst_out = None
        else:
        ...
        continuum_kwargs: dict[str, Any] = {
            ...
            "ultracontinuum_goal": str(
                selected.get("ultracontinuum_goal") or "all_hypercontinuums_met"
            ),
            "ultracontinuum_id": ultracontinuum_id,
            "out_root": out_dir,
        }
        ...
        max_active_hypercontinuums = selected.get("max_active_hypercontinuums")
        if max_active_hypercontinuums is None:
            max_active_hypercontinuums = selected.get("max_active_continuums")
        if max_active_hypercontinuums is None:
            max_active_hypercontinuums = selected.get("max_active_civilizations")
        if max_active_hypercontinuums is not None:
            continuum_kwargs["max_active_hypercontinuums"] = int(
        ...
        except uu.UltracontinuumRefused as exc:
            if local_index == 0 and not resumed:
                raise MegacontinuumRefused(exc.verdict, exc.detail) from exc
        ...
        nested_ultracontinuum_dir = continuum_result.get("ultracontinuum_dir")
        nested_multiverse_states: list[dict[str, Any]] = []
        nested_receipt_path = (
            Path(str(nested_ultracontinuum_dir)) / "ultracontinuum.json"
            if nested_ultracontinuum_dir
            else None
        )
        if nested_receipt_path is not None and nested_receipt_path.is_file():
            ...
            for ist in list(
                receipt.get("hypercontinuum_states")
        ...
        if not nested_multiverse_states:
            for ist in list(
                continuum_result.get("hypercontinuum_states")
        ...
        for i, lst in enumerate(ultracontinuum_states):
            if str(lst.get("ultracontinuum_id")) != ultracontinuum_id:
                continue
            updated = dict(lst)
            updated["last_ultracontinuum_dir"] = continuum_result.get("ultracontinuum_dir")
            updated["last_ultracontinuum_digest"] = continuum_result.get("ultracontinuum_digest")
            updated["ultracontinuum_met"] = bool(continuum_result.get("ultracontinuum_met"))
            ...
            if nested_multiverse_states:
                updated["hypercontinuum_states"] = nested_multiverse_states
        ...
        coverage_after = megacontinuum_terminal_coverage(
            ultracontinuum_states=ultracontinuum_states,
            federated_portfolio=federated_portfolio,
        )

        roi = score_ultracontinuum_roi(
            round_index=round_index,
            ultracontinuum_id=ultracontinuum_id,
        ...
        idigest = str(continuum_result.get("ultracontinuum_digest") or "")
        if idigest:
            ultracontinuum_digests.append(idigest)

        rec = _ultracontinuum_round_record(
            round_index=round_index,
            ultracontinuum_id=ultracontinuum_id,
            continuum_result=continuum_result,
            coverage_before=coverage_before,
        ...
        write_megacontinuum_state(
            megacontinuum_dir,
            _state_payload(
                megacontinuum_id=lid,
                ...
                roi_history=roi_history,
                ultracontinuum_states=ultracontinuum_states,
                ultracontinuum_digests=ultracontinuum_digests,
                charter=active_charter,
                stop_reason=None,
                megacontinuum_goal=megacontinuum_goal,
                max_active_ultracontinuums=active_max,
        ...
        if (
            ...
            and not continuum_result.get("ultracontinuum_met")
        ):
            idle_streak += 1
        else:
            idle_streak = 0
            ...
            reason = stop_when(
                {
                    ...
                    "roi_history": roi_history,
                    "ultracontinuum_states": ultracontinuum_states,
                    "last_ultracontinuum_id": ultracontinuum_id,
                    "federated_portfolio": federated_portfolio,
                    "megacontinuum_dir": str(megacontinuum_dir),
                    "pending_ultracontinuum_ids": [
                        str(s.get("ultracontinuum_id") or "")
        ...
        # Grow constitution before declaring megacontinuum_met when expand remains.
        if (
            charter_expand is not None
            and not pending_charter_slots(active_charter, ultracontinuum_states)
            and ultracontinuums_all_met(ultracontinuum_states)
        ):
    ...
    coverage_end = megacontinuum_terminal_coverage(
        ultracontinuum_states=ultracontinuum_states,
        federated_portfolio=federated_portfolio,
    )
    if constitution_satisfied(
    ...
    ultracontinuums_met_count = sum(
        1 for ist in ultracontinuum_states if ist.get("ultracontinuum_met")
    )
    pending_remaining = [
        str(s.get("ultracontinuum_id") or "")
        for s in pending_charter_slots(active_charter, ultracontinuum_states)
    ]

    if megacontinuum_met and stop_reason in {"megacontinuum_met", "max_rounds"}:
        verdict = "megacontinuum_met"
        ok = True
        stop_reason = "megacontinuum_met"
    elif stop_reason == "rank_only":
        verdict = "megacontinuum_ranked"
        ok = True
    elif stop_reason == "megacontinuum_idle":
        verdict = "megacontinuum_idle"
        ok = True
    elif stop_reason == "dispatch_budget":
        verdict = "megacontinuum_budgeted"
        ok = True
        ...
        verdict = "megacontinuum_completed"
        ok = True
    ...
    receipt: dict[str, Any] = {
        ...
        "megacontinuum_goal": megacontinuum_goal,
        "megacontinuum_met": megacontinuum_met,
        ...
        "round_count": len(institutions),
        "ultracontinuums": institutions,
        "ultracontinuum_digests": [
            i.get("ultracontinuum_digest")
            for i in institutions
            if i.get("ultracontinuum_digest")
        ],
        "ultracontinuum_states": ultracontinuum_states,
        "ultracontinuums_admitted": len(ultracontinuum_states),
        "ultracontinuums_met_count": ultracontinuums_met_count,
        "admissions": admissions,
    ...
    receipt["ultracontinuum_digests"] = [
        str(i.get("ultracontinuum_digest") or "") for i in institutions
    ]
    receipt["megacontinuum_digest"] = _sha256_json(_megacontinuum_digest_payload(receipt))
    atomic_write_json(megacontinuum_dir / "megacontinuum.json", receipt)
    atomic_write_json(
        megacontinuum_dir / "summary.json",
        {
            "verdict": receipt["verdict"],
            "ok": receipt["ok"],
            "stop_reason": receipt["stop_reason"],
            "megacontinuum_id": receipt["megacontinuum_id"],
            "round_count": receipt["round_count"],
            "total_dispatched": receipt["total_dispatched"],
            "total_dispatched_ok": receipt["total_dispatched_ok"],
            "megacontinuum_met": receipt["megacontinuum_met"],
            ...
            "max_active_ultracontinuums": receipt["max_active_ultracontinuums"],
            ...
            "megacontinuum_digest": receipt["megacontinuum_digest"],
            "resumed": resumed,
        },
    )
    ...
    write_megacontinuum_state(
        ...
        _state_payload(
            ...
            roi_history=roi_history,
            ultracontinuum_states=ultracontinuum_states,
            ultracontinuum_digests=receipt["ultracontinuum_digests"],
            charter=active_charter,
            stop_reason=stop_reason,
            megacontinuum_goal=megacontinuum_goal,
            max_active_ultracontinuums=active_max,
    ...
    return {
        ...
        "megacontinuum_id": lid,
        "round_count": len(institutions),
        "ultracontinuum_digests": list(receipt["ultracontinuum_digests"]),
        "total_dispatched": total_dispatched,
        "total_dispatched_ok": total_dispatched_ok,
        "megacontinuum_met": megacontinuum_met,
        "ultracontinuums_admitted": len(ultracontinuum_states),
...
def _ultracontinuum_slot(
    ultracontinuum_id: str,
    *,
    priority: int = 0,
    hypercontinuums: Sequence[dict[str, Any]] | None = None,
    ...
    """Build a hermetic megacontinuum charter continuum slot.

    Prefer ``hypercontinuums=`` (nested continuum slots for the ultracontinuum plane).
    ``institutions=`` wrap a single auto omniverse with multiverse nesting underneath.
    """
    nested: list[dict[str, Any]]
    if hypercontinuums is not None:
        nested = list(hypercontinuums)
    elif multiverses is not None:
        nested = [
            uu._hypercontinuum_slot(
                f"{ultracontinuum_id[:1]}v",
...
def builtin_upstream_megacontinuum_proof() -> dict[str, Any]:
    """Hermetic end-to-end proof of the multi-ultracontinuum megacontinuum plane (no network)."""
    ...
    try:
        ...
        multi_megacontinuum_ok = (
            confed["ok"]
            and confed["megacontinuum_met"] is True
            and confed["stop_reason"] == "megacontinuum_met"
            and confed["ultracontinuums_admitted"] == 2
            and confed["ultracontinuums_met_count"] == 2
        ...
        scheduled_ids = {
            i.get("ultracontinuum_id") for i in (confed.get("ultracontinuums") or confed.get("realms") or [])
        }
        multi_megacontinuum_scheduled = multi_megacontinuum_ok and scheduled_ids >= {"a", "b"}

        verified = verify_megacontinuum_receipt(Path(confed["megacontinuum_dir"]))
        seal_ok = bool(verified.get("ok")) and verified.get("round_count") == confed[
            "round_count"
        ]

        # Tamper detection.
        confed_path = Path(confed["megacontinuum_dir"]) / "megacontinuum.json"
        receipt = json.loads(confed_path.read_text(encoding="utf-8"))
        receipt["megacontinuum_digest"] = "0" * 64
        confed_path.write_text(json.dumps(receipt, indent=2), encoding="utf-8")
        tampered = verify_megacontinuum_receipt(Path(confed["megacontinuum_dir"]))
        tamper_detected = (
            not tampered["ok"]
            and "megacontinuum_digest" in (tampered.get("mismatched") or [])
        )

        # Budget stop across leagues.
        campaign2 = _proof_campaign_runner(scratch / "b")
        budgeted = run_megacontinuum(
            charter=[
                _ultracontinuum_slot(
                    "b1",
                    priority=1,
        ...
        # Pre-met short circuit via custom ultracontinuum_runner.
        ...
        def _premet_runner(**kwargs: Any) -> dict[str, Any]:
            ...
            nested_charter = uu.normalize_ultracontinuum_charter(kwargs.get("charter"))
            ...
            receipt = {
                ...
                "ultracontinuum_id": kwargs.get("ultracontinuum_id"),
                "ultracontinuum_met": True,
                "total_dispatched": 0,
                "total_dispatched_ok": 0,
                "hypercontinuums_admitted": len(nested_charter),
                "hypercontinuums_met_count": len(nested_charter),
                "ultracontinuum_digest": digest,
                "federated_portfolio": portfolio,
                "hypercontinuum_states": civilization_states,
            ...
            atomic_write_json(out / "ultracontinuum.json", receipt)
            atomic_write_json(
                out / "ultracontinuum_state.json",
                {
                    "ultracontinuum_id": kwargs.get("ultracontinuum_id"),
                    "round_count": 0,
                    "total_dispatched": 0,
        ...
        pre_met = run_megacontinuum(
            ...
            max_rounds=3,
            dispatch=True,
            ultracontinuum_runner=_premet_runner,
            megacontinuum_goal="all_ultracontinuums_met",
            out_root=scratch / "p",
        )
        premet_ok = (
            pre_met["ok"]
            and pre_met["megacontinuum_met"] is True
            and pre_met["stop_reason"] == "megacontinuum_met"
            and pre_met["ultracontinuums_met_count"] == 1
            and pre_met["total_dispatched"] == 0
        )

        # Rank-only domain.
        ranked = run_megacontinuum(
            charter=[
                _ultracontinuum_slot(
                    "ra",
                    institutions=[
        ...
        rank_only_ok = (
            ranked["ok"]
            and ranked["verdict"] == "megacontinuum_ranked"
        ...
        except MegacontinuumRefused as exc:
        ...
        partial = run_megacontinuum(
            ...
            megacontinuum_goal="none",
            megacontinuum_id="rcp",
            out_root=scratch / "a",
        )
        state_path = Path(partial["megacontinuum_dir"]) / "megacontinuum_state.json"
        state_exists = state_path.is_file()
        campaign5 = _proof_campaign_runner(scratch / "rb")
        resumed = run_megacontinuum(
            resume_dir=Path(partial["megacontinuum_dir"]),
            max_rounds=4,
            max_epochs_per_succession=2,
            max_waves_per_epoch=2,
            per_wave_dispatch_limit=1,
        ...
        resume_ok = (
            ...
            and resumed["megacontinuum_id"] == "rcp"
        ...
        first_cw = (confed.get("ultracontinuums") or confed.get("realms") or [{}])[0].get("ultracontinuum_id")
        priority_ok = first_cw == "a"

        # Federation: inventories across both ultracontinuums form a joint surface.
        fed_keys: set[tuple[str, str, str]] = set()
        for est in confed.get("ultracontinuum_states") or []:
        ...
        federation_ok = multi_megacontinuum_ok and (
            len(fed_keys) >= 3
        ...
        admit_ids = [a.get("ultracontinuum_id") for a in (deferred.get("admissions") or [])]
        deferred_ok = (
            deferred["ok"]
            and deferred["megacontinuum_met"] is True
            and deferred["ultracontinuums_admitted"] == 3
            and deferred["ultracontinuums_met_count"] == 3
            and deferred.get("max_active_ultracontinuums") == 1
        ...
        expand_runner = make_megacontinuum_charter_expand(
            [
                _ultracontinuum_slot(
                    "xg",
                    priority=1,
        ...
        expand_ok = (
            expanded["ok"]
            and expanded["megacontinuum_met"] is True
            and expanded["ultracontinuums_admitted"] == 2
            and expanded["ultracontinuums_met_count"] == 2
        ...
        # merge_megacontinuum_charter unit evidence (ids de-dupe, additions append).
        merged = merge_megacontinuum_charter(
            [_ultracontinuum_slot("m1", institutions=[_inst_slot("mi", programs=[_program_slot("mp", initial=[("m", "1.0.0", "m-1")])])])],
            [
                _ultracontinuum_slot("m1", institutions=[_inst_slot("mi2", programs=[_program_slot("mp2", initial=[("m2", "1.0.0", "m2-1")])])]),
                _ultracontinuum_slot("m2", institutions=[_inst_slot("mj", programs=[_program_slot("mq", initial=[("n", "1.0.0", "n-1")])])]),
            ],
        )
        merge_ok = [s["ultracontinuum_id"] for s in merged] == ["m1", "m2"]

        # Terminal-coverage constitution goal: megacontinuum_met when federated
        ...
        terminal_ok = (
            terminal["ok"]
            and terminal["megacontinuum_met"] is True
            and terminal["stop_reason"] == "megacontinuum_met"
            ...
            and terminal["ultracontinuums_admitted"] == 2
        ...
        return {
            "ok": ok,
            "megacontinuum_met": multi_megacontinuum_ok,
            "multi_megacontinuum_progressed": multi_megacontinuum_scheduled,
            ...
            "roi_scored": roi_ok,
            "megacontinuum_digest": confed.get("megacontinuum_digest"),
            "round_count": confed.get("round_count"),
            "total_dispatched_ok": confed.get("total_dispatched_ok"),
            "ultracontinuums_admitted": confed.get("ultracontinuums_admitted"),
            "used_skill_route_discovery": legacy_pipeline_was_used(),
            "flags": {
                "multi_megacontinuum_ok": multi_megacontinuum_ok,
                "multi_megacontinuum_scheduled": multi_megacontinuum_scheduled,
                "seal_ok": seal_ok,
                "tamper_detected": tamper_detected,
                "budget_ok": budget_ok,
                "premet_ok": premet_ok,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.verify:
        result = verify_megacontinuum_receipt(Path(args.verify))
        print(json.dumps(result, indent=2, sort_keys=True))
        return 0 if result.get("ok") else 1
    if args.proof:
        result = builtin_upstream_megacontinuum_proof()
        print(json.dumps(result, indent=2, sort_keys=True))
        return 0 if result.get("ok") else 1
    parser.print_help()
    return 2
