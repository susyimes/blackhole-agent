...
def _consider_class(prefix, target, nested=False, deep=False, triple=False, quadruple=False, quintuple=False, sextuple=False, septuple=False, octuple=False, nonuple=False):
    try:
    ...
    if class_name.endswith(("AsyncClient", "AsyncIOClient")):
        return
    depth = (
        9 if nonuple else 8 if octuple else 7 if septuple else 6 if sextuple else 5 if quintuple
