...
def builtin_finger_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="finger-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, FINGER_ACTUATION_GOAL, FINGER_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_finger"] = (
        not gate.accepted
        and live_goal != FINGER_ACTUATION_GOAL
        and FINGER_ACTUATION_ID not in live_done
