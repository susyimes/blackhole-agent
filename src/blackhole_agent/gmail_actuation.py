...
def builtin_gmail_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="gmail-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, GMAIL_ACTUATION_GOAL, GMAIL_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_gmail"] = (
        not gate.accepted
        and live_goal != GMAIL_ACTUATION_GOAL
        and GMAIL_ACTUATION_ID not in live_done
