...
def builtin_s3_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="s3-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, S3_ACTUATION_GOAL, S3_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_s3"] = (
        not gate.accepted
        and live_goal != S3_ACTUATION_GOAL
        and S3_ACTUATION_ID not in live_done
