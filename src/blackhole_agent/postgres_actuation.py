...
def builtin_postgres_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="postgres-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, POSTGRES_ACTUATION_GOAL, POSTGRES_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_postgres"] = (
        not gate.accepted
        and live_goal != POSTGRES_ACTUATION_GOAL
        and POSTGRES_ACTUATION_ID not in live_done
