...
def builtin_radius_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="radius-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, RADIUS_ACTUATION_GOAL, RADIUS_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_radius"] = (
        not gate.accepted
        and live_goal != RADIUS_ACTUATION_GOAL
        and RADIUS_ACTUATION_ID not in live_done
