"""Upstream exacontinuum plane: multi-petacontinuum durable stewardship exacontinuum.

The petacontinuum plane (``upstream_petacontinuum``) closes multi-continuum unions *within one
petacontinuum*. It does not:

1. chain multiple independent petacontinuums under a durable exacontinuum constitution;
2. allocate a shared global dispatch budget across petacontinuums by ROI;
3. admit/retire petacontinuum slots from an exacontinuum charter over time
   (deferred admission under a concurrent-active cap);
4. grow the exacontinuum charter mid-run via ``charter_expand`` (constitution
   growth beyond the initial charter, not just deferred admission of a fixed set);
5. federate multi-petacontinuum portfolio coverage into one exacontinuum world-model;
6. persist exacontinuum state so a later process can resume the union;
7. seal a multi-petacontinuum exacontinuum chronicle linking petacontinuum digests.

The exacontinuum plane closes that outer multi-petacontinuum loop:

1. **admit** — materialize petacontinuum slots from a durable exacontinuum charter
   (each slot owns a nested continuum charter). When ``max_active_petacontinuums``
   is set, only that many *unmet* petacontinuums are concurrent: further charter
   slots stay pending and are admitted as capacity frees after retirements
   (exacontinuum constitution growth over time);
2. **schedule** — pick the next open petacontinuum by priority and historical ROI;
3. **petacontinuum** — call the petacontinuum plane (injected ``petacontinuum_runner``;
   default ``run_petacontinuum``) with a share of the remaining global dispatch
   budget;
4. **federate** — merge per-petacontinuum portfolios into one exacontinuum world-model
   and re-score coverage across all stewarded keys;
5. **retire** — mark petacontinuums met when their petacontinuum_goal is satisfied,
   then re-admit pending charter slots up to the active capacity;
6. **expand** — optional ``charter_expand`` may append new petacontinuum slots when
   the active charter has no pending work and all admitted petacontinuums are met,
   so the exacontinuum constitution can grow after start (not only defer a fixed charter);
7. **persist** — write ``exacontinuum_state.json`` after every petacontinuum round so a
   later ``run_exacontinuum(..., resume_dir=...)`` continues the same union
   (including pending charter and admission history);
8. **stop** when any of:

   - ``max_rounds`` reached
   - global ``dispatch_budget`` exhausted across petacontinuums
   - exacontinuum goal met (``all_petacontinuums_met``: every *admitted*
     petacontinuum is met *and* no pending charter slots remain)
   - consecutive idle/no-progress rounds (``idle_round_limit``)
   - explicit ``stop_when`` predicate returns a reason string

9. **seal** — write an exacontinuum receipt under
   ``artifacts/upstream-exacontinuum/`` with sha256 digests of every
   petacontinuum, portfolio federation, admission history, ROI history, stop
   reason, and an exacontinuum chain digest; ``verify_exacontinuum_receipt``
   re-checks the chain and detects tampering.

No skill-route discovery is used. The plane is exacontinuum-level direction
over the petacontinuum plane, not a new verifier of individual repairs.
"""
...
ARTIFACTS_ROOT = REPO_ROOT / "artifacts" / "upstream-exacontinuum"

TERMINAL_SUCCESS_OUTCOMES = pp.TERMINAL_SUCCESS_OUTCOMES


class ExacontinuumRefused(Exception):
...
def normalize_exacontinuum_charter(
    charter: Sequence[Mapping[str, Any]] | None,
) -> list[dict[str, Any]]:
    """Normalize an exacontinuum charter into deterministic continuum slots.
    ...
    Each slot is::

        {
          ...
          "priority": int,
          "charter": [...teracontinuum slots...],  # nested petacontinuum charter
    ...
    for raw in charter:
        if not isinstance(raw, Mapping):
            continue
        petacontinuum_id = str(
            raw.get("petacontinuum_id") or raw.get("id") or ""
        ).strip()
        if not petacontinuum_id or petacontinuum_id in seen:
            continue
        seen.add(petacontinuum_id)

        nested = pp.normalize_petacontinuum_charter(
            raw.get("charter")
            or raw.get("teracontinuums")
        ...
        max_active_teracontinuums = raw.get("max_active_teracontinuums")
        if max_active_teracontinuums is None:
            max_active_teracontinuums = raw.get("max_active_continuums")
        if max_active_teracontinuums is None:
            max_active_teracontinuums = raw.get("max_active_civilizations")
        if max_active_teracontinuums is not None:
            max_active_teracontinuums = max(1, int(max_active_teracontinuums))
        ...
        out.append(
            {
                ...
                "petacontinuum_goal": str(
                    raw.get("petacontinuum_goal") or "all_teracontinuums_met"
                ),
                "kind": str(raw.get("kind") or "stewardship_petacontinuum"),
            }
        )
    return out


def admit_petacontinuum_slot(
    *,
    exacontinuum_dir: Path,
    slot: Mapping[str, Any],
) -> dict[str, Any]:
    """Materialize one institution slot under the league directory.

    Returns admission record with petacontinuum_root and nested charter.
    Stewardship surfaces are created lazily by the institution plane on run.
    """
    petacontinuum_id = str(slot.get("petacontinuum_id") or "")
    if not petacontinuum_id:
        raise ExacontinuumRefused("exacontinuum_invalid", "slot missing petacontinuum_id")

    petacontinuum_root = Path(exacontinuum_dir) / "petacontinuums" / petacontinuum_id
    petacontinuum_root.mkdir(parents=True, exist_ok=True)

    nested_charter = pp.normalize_petacontinuum_charter(slot.get("charter"))
    if not nested_charter:
        raise ExacontinuumRefused(
            "exacontinuum_invalid",
            f"continuum slot {petacontinuum_id!r} has empty nested charter",
        )

    max_active_teracontinuums = slot.get("max_active_teracontinuums")
    if max_active_teracontinuums is None:
        max_active_teracontinuums = slot.get("max_active_continuums")
    if max_active_teracontinuums is None:
        max_active_teracontinuums = slot.get("max_active_civilizations")

    return {
        "petacontinuum_id": petacontinuum_id,
        "petacontinuum_root": str(petacontinuum_root),
        "admitted": True,
        "charter": nested_charter,
        "max_active_teracontinuums": max_active_teracontinuums,
        "max_rounds": int(slot.get("max_rounds") or 6),
        "petacontinuum_goal": str(slot.get("petacontinuum_goal") or "all_teracontinuums_met"),
...
def federate_portfolios(
    ...
    portfolio: dict[str, Any] = {
        ...
        "entries": entries,
        "source": "exacontinuum_federation",
    }
...
def exacontinuum_terminal_coverage(
    *,
    petacontinuum_states: Sequence[Mapping[str, Any]],
    federated_portfolio: Mapping[str, Any] | None,
) -> dict[str, Any]:
    """Coverage across petacontinuum->teracontinuum->...->program."""
    teracontinuum_states: list[dict[str, Any]] = []
    for uc in petacontinuum_states:
        if not isinstance(uc, Mapping):
            continue
        nested = (
            list(uc.get("teracontinuum_states") or [])
        ...
        if nested:
            for hc in nested:
                if isinstance(hc, Mapping):
                    teracontinuum_states.append(dict(hc))
            continue
        # Pre-run: petacontinuum charter is a list of teracontinuum slots.
        for cslot in list(uc.get("charter") or []) + list(
            uc.get("teracontinuums") or []
            ...
            if isinstance(cslot, Mapping):
                teracontinuum_states.append(dict(cslot))
        if not nested and not list(uc.get("charter") or []):
            for cont in list(uc.get("continuum_states") or []) + list(
                ...
                if isinstance(cont, Mapping):
                    teracontinuum_states.append(dict(cont))
        for raw in list(uc.get("inventory_keys") or []):
            if isinstance(raw, (list, tuple)) and len(raw) >= 3:
                ...
                teracontinuum_states.append({"inventory_keys": [raw]})
    return pp.petacontinuum_terminal_coverage(
        teracontinuum_states=teracontinuum_states,
        federated_portfolio=federated_portfolio,
    )


def petacontinuums_all_met(petacontinuum_states: Sequence[Mapping[str, Any]]) -> bool:
    if not petacontinuum_states:
        return False
    return all(bool(ist.get("petacontinuum_met")) for ist in petacontinuum_states)


def open_unmet_count(petacontinuum_states: Sequence[Mapping[str, Any]]) -> int:
    """Count admitted institutions that are not yet continuum_met."""
    return sum(1 for ist in petacontinuum_states if not ist.get("petacontinuum_met"))
...
def pending_charter_slots(
    ...
    """Charter slots not yet admitted, priority-desc then petacontinuum_id-asc."""
    known = {str(ist.get("petacontinuum_id") or "") for ist in petacontinuum_states}
    pending = [
        dict(slot)
        for slot in charter
        if str(slot.get("petacontinuum_id") or "")
        and str(slot.get("petacontinuum_id")) not in known
    ]
    pending.sort(
        key=lambda s: (
            -int(s.get("priority") or 0),
            str(s.get("petacontinuum_id") or ""),
        )
    )
    return pending
...
def constitution_satisfied(
    *,
    ...
    if exacontinuum_goal == "none":
        return False
    if exacontinuum_goal == "terminal_coverage":
        cov = exacontinuum_terminal_coverage(
            petacontinuum_states=petacontinuum_states,
            federated_portfolio=federated_portfolio,
        )
        return bool(cov.get("met")) and not pending_charter_slots(
            charter, petacontinuum_states
        )
    if exacontinuum_goal == "all_petacontinuums_met":
        if not petacontinuum_states:
            return False
        if pending_charter_slots(charter, petacontinuum_states):
            return False
        return petacontinuums_all_met(petacontinuum_states)
    return False


def reopen_incomplete_petacontinuums(
    ...
    *,
    ...
    Nested petacontinuums can retire after only partial surface work (e.g. before
    deferred program targets expand). Under a ``terminal_coverage`` exacontinuum
    goal those children must re-run until federated inventory is fully
    terminal-success. Returns the reopened ``petacontinuum_id`` list.
    """
    cov = exacontinuum_terminal_coverage(
        petacontinuum_states=petacontinuum_states,
        federated_portfolio=federated_portfolio,
    ...
    for item in list(cov.get("open_or_missing") or []):
        ...
        if key[0] and key[2]:
            open_keys.add(key)
    reopened: list[str] = []
    for i, ist in enumerate(petacontinuum_states):
        if not ist.get("petacontinuum_met"):
            continue
        ...
        for mv in list(ist.get("teracontinuum_states") or []) + list(
        ...
        if not owns_open:
            continue
        updated = dict(ist)
        updated["petacontinuum_met"] = False
        petacontinuum_states[i] = updated
        reopened.append(str(updated.get("petacontinuum_id") or ""))
    return [r for r in reopened if r]


def merge_exacontinuum_charter(
    ...
    Existing ``petacontinuum_id`` values win (additions with the same id are
    ignored). Returns a fully re-normalized charter so nested confederation
    charters stay deterministic.
    """
    base = normalize_exacontinuum_charter(existing)
    if not additions:
        return base
    known = {str(s.get("petacontinuum_id") or "") for s in base}
    merged: list[Mapping[str, Any]] = list(base)
    for raw in additions:
        if not isinstance(raw, Mapping):
            continue
        cid = str(raw.get("petacontinuum_id") or raw.get("id") or "").strip()
        if not cid or cid in known:
            continue
        known.add(cid)
        merged.append(raw)
    return normalize_exacontinuum_charter(merged)


def make_exacontinuum_charter_expand(
    ...
    *,
    ...
    """
    pending_growth = normalize_exacontinuum_charter(growth)
    ...
    def _runner(
        *,
        ...
        remaining = [
            ...
            if str(s.get("petacontinuum_id") or "") not in applied_ids
            and str(s.get("petacontinuum_id") or "")
            not in {str(x.get("petacontinuum_id") or "") for x in active_charter}
        ]
        if not remaining:
            return {
                "expanded": False,
        ...
        for s in take:
            applied_ids.add(str(s.get("petacontinuum_id") or ""))
        merged = merge_exacontinuum_charter(active_charter, take)
        state["applied"] = applied_ids
        return {
            "expanded": True,
            "added": [str(s.get("petacontinuum_id") or "") for s in take],
            "charter": merged,
            "detail": "charter_growth_applied",
            "round_index": round_index,
            "realms_met": petacontinuums_all_met(petacontinuum_states),
        }
...
def admit_pending_slots(
    *,
    ...
    ``max_active_petacontinuums`` caps *unmet* concurrent realms. ``None``
    admits every remaining pending slot. Returns admission records for newly
    admitted slots (also mutates ``petacontinuum_states``).
    """
    pending = pending_charter_slots(charter, petacontinuum_states)
    if not pending:
        return []

    open_n = open_unmet_count(petacontinuum_states)
    if max_active_petacontinuums is None:
        capacity = len(pending)
    else:
        capacity = max(0, int(max_active_petacontinuums) - open_n)
    if capacity <= 0:
        return []

    admissions: list[dict[str, Any]] = []
    for slot in pending[:capacity]:
        admission = admit_petacontinuum_slot(exacontinuum_dir=exacontinuum_dir, slot=slot)
        ...
        petacontinuum_states.append(
            {
                "petacontinuum_id": admission["petacontinuum_id"],
                "petacontinuum_root": admission["petacontinuum_root"],
                "charter": admission["charter"],
                "max_active_teracontinuums": admission.get("max_active_teracontinuums"),
                "max_rounds": admission["max_rounds"],
                "petacontinuum_goal": admission["petacontinuum_goal"],
                "priority": admission["priority"],
                "petacontinuum_met": False,
                "last_petacontinuum_dir": None,
                "last_petacontinuum_digest": None,
                "portfolio": None,
                "omniverse_states": [],
                "multiverse_states": [],
                "cosmos_states": [],
...
def score_petacontinuum_roi(
    *,
    ...
    return {
        ...
        "petacontinuum_met": bool(continuum_result.get("petacontinuum_met")),
        "continuum_digest": continuum_result.get("petacontinuum_digest"),
        "continuums_admitted": int(
            continuum_result.get("teracontinuums_admitted")
            or continuum_result.get("megacontinuums_admitted")
            or continuum_result.get("ultracontinuums_admitted")
            or continuum_result.get("petacontinuums_admitted")
...
def _roi_summary(roi_history: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for r in roi_history:
        iid = str(r.get("petacontinuum_id") or "")
...
def select_next_petacontinuum(
    ...
    *,
    ...
    Tie-break: lower run_count, then stable petacontinuum_id order. Round-robin
    among equal scores uses ``round_index`` so multi-institution progress is fair.
    """
    open_slots = [
        dict(ist) for ist in petacontinuum_states if not ist.get("petacontinuum_met")
    ]
    if not open_slots:
        return None
    ...
    def sort_key(ist: Mapping[str, Any]) -> tuple[Any, ...]:
        iid = str(ist.get("petacontinuum_id") or "")
...
def allocate_petacontinuum_budget(
    *,
    ...
    hist = (summary.get("by_continuum") or {}).get(
        str(selected.get("petacontinuum_id") or "")
    ) or {}
    mean_eff = float(hist.get("mean_efficiency") or 0.0)
...
def _state_payload(
    *,
    ...
    roi_history: Sequence[Mapping[str, Any]],
    petacontinuum_states: Sequence[Mapping[str, Any]],
    petacontinuum_digests: Sequence[str],
    charter: Sequence[Mapping[str, Any]],
    ...
    return {
        ...
        "petacontinuum_states": [dict(ist) for ist in petacontinuum_states],
        "petacontinuum_digests": list(petacontinuum_digests),
        ...
        "pending_petacontinuum_ids": [
            str(s.get("petacontinuum_id") or "")
            for s in pending_charter_slots(charter, petacontinuum_states)
        ],
    }


def write_exacontinuum_state(exacontinuum_dir: Path, state: Mapping[str, Any]) -> Path:
    path = Path(exacontinuum_dir) / "exacontinuum_state.json"
    atomic_write_json(path, dict(state))
    return path


def load_exacontinuum_state(resume_dir: Path) -> dict[str, Any]:
    path = durable_read_path(Path(resume_dir) / "exacontinuum_state.json")
    if not path.is_file():
        raise ExacontinuumRefused(
            "exacontinuum_state_missing",
            f"no exacontinuum_state.json under {resume_dir}",
        )
    try:
        state = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise ExacontinuumRefused("exacontinuum_state_unreadable", str(exc)) from exc
    if not isinstance(state, dict):
        raise ExacontinuumRefused("exacontinuum_state_invalid", "state root must be object")
    return state
...
def _petacontinuum_round_record(
    *,
    round_index: int,
    petacontinuum_id: str,
    continuum_result: Mapping[str, Any],
    ...
    return {
        ...
        "stop_reason": continuum_result.get("stop_reason"),
        "petacontinuum_dir": continuum_result.get("petacontinuum_dir"),
        "petacontinuum_digest": continuum_result.get("petacontinuum_digest"),
        "continuums_admitted": int(
            continuum_result.get("teracontinuums_admitted")
            or continuum_result.get("megacontinuums_admitted")
            or continuum_result.get("ultracontinuums_admitted")
...
def _exacontinuum_digest_payload(receipt: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": receipt.get("schema_version"),
        "verdict": receipt.get("verdict"),
        "stop_reason": receipt.get("stop_reason"),
        "exacontinuum_id": receipt.get("exacontinuum_id"),
        "exacontinuum_goal": receipt.get("exacontinuum_goal"),
        "max_rounds": receipt.get("max_rounds"),
        "max_active_petacontinuums": receipt.get("max_active_petacontinuums"),
        ...
        "petacontinuum_digests": list(receipt.get("petacontinuum_digests") or []),
        "total_dispatched": receipt.get("total_dispatched"),
        "total_dispatched_ok": receipt.get("total_dispatched_ok"),
        "exacontinuum_met": receipt.get("exacontinuum_met"),
        "coverage_end": receipt.get("coverage_end"),
        "petacontinuums_met_count": receipt.get("petacontinuums_met_count"),
        "petacontinuums_admitted": receipt.get("petacontinuums_admitted"),
...
def verify_exacontinuum_receipt(exacontinuum_dir: Path) -> dict[str, Any]:
    """Re-check a sealed league receipt for digest integrity."""
    path = durable_read_path(Path(exacontinuum_dir) / "exacontinuum.json")
    if not path.is_file():
    ...
    expected = _sha256_json(_exacontinuum_digest_payload(receipt))
    recorded = str(receipt.get("exacontinuum_digest") or "")
    mismatched: list[str] = []
    if not recorded or recorded != expected:
        mismatched.append("exacontinuum_digest")

    institutions = list(receipt.get("petacontinuums") or receipt.get("realms") or receipt.get("leagues") or [])
    listed = list(receipt.get("petacontinuum_digests") or [])
    if len(listed) != len(institutions):
        mismatched.append("petacontinuum_digests_length")
    else:
        for i, (listed_d, rec) in enumerate(zip(listed, institutions)):
            if listed_d != rec.get("petacontinuum_digest"):
                mismatched.append(f"petacontinuum_digests[{i}]")

    nested_failures: list[str] = []
    for rec in institutions:
        idir = rec.get("petacontinuum_dir")
        if not idir:
            continue
        ip = Path(str(idir))
        if (ip / "petacontinuum.json").is_file():
            nested = pp.verify_petacontinuum_receipt(ip)
    ...
    return {
        "ok": ok,
        "verdict": "exacontinuum_sealed" if ok else "exacontinuum_tampered",
        "exacontinuum_digest": recorded,
...
def run_exacontinuum(
    *,
    ...
    max_active_petacontinuums: int | None = None,
    dispatch: bool = True,
    petacontinuum_runner: Callable[..., dict[str, Any]] | None = None,
    ...
    charter:
        ...
        :func:`normalize_exacontinuum_charter`).
    max_rounds:
    ...
    exacontinuum_goal:
        ``all_petacontinuums_met`` (default) stops when every admitted institution
    ...
    resume_dir:
        Load ``exacontinuum_state.json`` from a prior league dir and continue.
        New receipt is written under ``out_root`` (or a fresh stamp).
    """
    if max_rounds < 1:
        raise ExacontinuumRefused("exacontinuum_invalid", "max_rounds must be >= 1")
    if per_wave_dispatch_limit < 0:
        raise ExacontinuumRefused(
            "exacontinuum_invalid", "per_wave_dispatch_limit must be >= 0"
        )
    if max_active_petacontinuums is not None and int(max_active_petacontinuums) < 1:
        raise ExacontinuumRefused(
            "exacontinuum_invalid", "max_active_petacontinuums must be >= 1 when set"
        )
    if exacontinuum_goal not in {"all_petacontinuums_met", "terminal_coverage", "none"}:
        raise ExacontinuumRefused(
            "exacontinuum_invalid",
            f"unknown exacontinuum_goal: {exacontinuum_goal}",
        )

    runner = petacontinuum_runner or pp.run_petacontinuum

    prior_round_count = 0
    roi_history: list[dict[str, Any]] = []
    petacontinuum_digests: list[str] = []
    total_dispatched = 0
    total_dispatched_ok = 0
    resumed = False
    resume_exacontinuum_id: str | None = None
    petacontinuum_states: list[dict[str, Any]] = []
    ...
    if resume_dir is not None:
        state = load_exacontinuum_state(resume_dir)
        resumed = True
        resume_exacontinuum_id = str(state.get("exacontinuum_id") or "") or None
        ...
        petacontinuum_digests = [str(d) for d in (state.get("petacontinuum_digests") or [])]
        petacontinuum_states = [
            dict(ist)
            for ist in (state.get("petacontinuum_states") or [])
        ...
        if isinstance(state.get("charter"), list):
            active_charter = normalize_exacontinuum_charter(
        ...
        if (
            state.get("max_active_petacontinuums") is not None
            and max_active_petacontinuums is None
        ):
            resumed_max_active = int(state["max_active_petacontinuums"])
        # Resume may also merge a caller-supplied charter growth tail.
        if charter:
            active_charter = merge_exacontinuum_charter(active_charter, charter)
    else:
        active_charter = normalize_exacontinuum_charter(charter)
    ...
    if not active_charter and not petacontinuum_states:
        raise ExacontinuumRefused(
            "exacontinuum_empty",
    ...
    lid = (
        exacontinuum_id
        or resume_exacontinuum_id
        or f"exacontinuum-{utc_now_iso().replace(':', '').replace('-', '')}"
    )

    stamp = utc_now_iso().replace(":", "").replace("-", "")
    if out_root is not None:
        exacontinuum_dir = Path(out_root)
        if (exacontinuum_dir / "exacontinuum.json").is_file():
            exacontinuum_dir = exacontinuum_dir / stamp
    else:
        exacontinuum_dir = ARTIFACTS_ROOT / stamp
    exacontinuum_dir.mkdir(parents=True, exist_ok=True)
    ...
    # on a flat ultra-short base (not under exacontinuum_dir) so one extra
    ...
    if league_out_root is not None:
        ...
        lg_root = exacontinuum_dir / "e"
    lg_root.mkdir(parents=True, exist_ok=True)
    if _os.name == "nt":
        inst_flat_root = Path("C:/t") / "q"
    else:
        inst_flat_root = exacontinuum_dir / "x"
    inst_flat_root.mkdir(parents=True, exist_ok=True)
    # Per-run namespace so parallel/resumed ultracontinua do not collide.
    # Prefer a short random token over the human lid (often "exacontinuum-…")
    # so Windows paths stay short and never re-enter a prior petacontinuum_dir that
    ...
    initial_admissions = admit_pending_slots(
        exacontinuum_dir=exacontinuum_dir,
        charter=active_charter,
        petacontinuum_states=petacontinuum_states,
        max_active_petacontinuums=active_max,
        round_index=prior_round_count,
    )
    admissions.extend(initial_admissions)

    if not petacontinuum_states and not pending_charter_slots(
        active_charter, petacontinuum_states
    ):
        raise ExacontinuumRefused("exacontinuum_empty", "no realm slots admitted")
    if not petacontinuum_states and pending_charter_slots(
        active_charter, petacontinuum_states
    ):
        raise ExacontinuumRefused(
            "exacontinuum_empty",
            "no realm slots admitted under max_active_petacontinuums policy",
        )

    if federated_portfolio is None:
        federated_portfolio = federate_portfolios(
            [ist.get("portfolio") for ist in petacontinuum_states]
        )
    ...
    max_terminal_reopens = max(2, (len(petacontinuum_states) or 1) * 4)
    coverage_end: dict[str, Any] = exacontinuum_terminal_coverage(
        petacontinuum_states=petacontinuum_states,
        federated_portfolio=federated_portfolio,
    )
    ...
    for local_index in range(max_rounds):
        ...
        coverage_before = exacontinuum_terminal_coverage(
            petacontinuum_states=petacontinuum_states,
            federated_portfolio=federated_portfolio,
        )
        ...
        selected = select_next_petacontinuum(
            petacontinuum_states, roi_history, round_index=round_index
        )
        if selected is None:
            # Children may all report met while federated terminal coverage is
            # still incomplete (e.g. deferred surfaces). Only declare exacontinuum
            # met when the active exacontinuum_goal is actually satisfied.
            if constitution_satisfied(
                petacontinuum_states=petacontinuum_states,
                charter=active_charter,
                exacontinuum_goal=exacontinuum_goal,
                ...
                break
            if (
                exacontinuum_goal == "terminal_coverage"
                and terminal_reopen_count < max_terminal_reopens
            ):
                reopened = reopen_incomplete_petacontinuums(
                    petacontinuum_states,
                    federated_portfolio=federated_portfolio,
                )
                if reopened:
            ...
            if selected is None:
                stop_reason = "exacontinuum_idle"
                coverage_end = coverage_before
                break

        open_count = sum(
            1 for ist in petacontinuum_states if not ist.get("petacontinuum_met")
        )
        allocated = allocate_petacontinuum_budget(
        ...
        petacontinuum_id = str(selected["petacontinuum_id"])
        resume_petacontinuum_dir = selected.get("last_petacontinuum_dir")
        continuum_resume: Path | None = None
        if (
            resume_petacontinuum_dir
            and (Path(str(resume_petacontinuum_dir)) / "petacontinuum_state.json").is_file()
            and not selected.get("petacontinuum_met")
        ):
            continuum_resume = Path(str(resume_petacontinuum_dir))

        # Ultra-short stamp (each outer plane tightens Windows MAX_PATH).
        # Do NOT nest child petacontinuum trees under exacontinuum roots:
        # let the petacontinuum plane use its own flat C:/t/{h,i} bases so the
        # deep continuum→…→wave chain still fits. Only the petacontinuum receipt
        # itself lands on a flat unique out_root.
        safe_id = "".join(c if c.isalnum() else "" for c in petacontinuum_id)[:2] or "i"
        ...
        continuum_kwargs: dict[str, Any] = {
            ...
            "petacontinuum_goal": str(
                selected.get("petacontinuum_goal") or "all_teracontinuums_met"
            ),
            "petacontinuum_id": petacontinuum_id,
            "out_root": out_dir,
        }
        ...
        max_active_teracontinuums = selected.get("max_active_teracontinuums")
        if max_active_teracontinuums is None:
            max_active_teracontinuums = selected.get("max_active_continuums")
        if max_active_teracontinuums is None:
            max_active_teracontinuums = selected.get("max_active_civilizations")
        if max_active_teracontinuums is not None:
            continuum_kwargs["max_active_teracontinuums"] = int(
        ...
        except pp.PetacontinuumRefused as exc:
            if local_index == 0 and not resumed:
                raise ExacontinuumRefused(exc.verdict, exc.detail) from exc
        ...
        nested_petacontinuum_dir = continuum_result.get("petacontinuum_dir")
        nested_multiverse_states: list[dict[str, Any]] = []
        nested_receipt_path = (
            Path(str(nested_petacontinuum_dir)) / "petacontinuum.json"
            if nested_petacontinuum_dir
            else None
        )
        if nested_receipt_path is not None and nested_receipt_path.is_file():
            ...
            for ist in list(
                receipt.get("teracontinuum_states")
        ...
        if not nested_multiverse_states:
            for ist in list(
                continuum_result.get("teracontinuum_states")
        ...
        for i, lst in enumerate(petacontinuum_states):
            if str(lst.get("petacontinuum_id")) != petacontinuum_id:
                continue
            updated = dict(lst)
            updated["last_petacontinuum_dir"] = continuum_result.get("petacontinuum_dir")
            updated["last_petacontinuum_digest"] = continuum_result.get("petacontinuum_digest")
            updated["petacontinuum_met"] = bool(continuum_result.get("petacontinuum_met"))
            ...
            if nested_multiverse_states:
                updated["teracontinuum_states"] = nested_multiverse_states
        ...
        coverage_after = exacontinuum_terminal_coverage(
            petacontinuum_states=petacontinuum_states,
            federated_portfolio=federated_portfolio,
        )

        roi = score_petacontinuum_roi(
            round_index=round_index,
            petacontinuum_id=petacontinuum_id,
        ...
        idigest = str(continuum_result.get("petacontinuum_digest") or "")
        if idigest:
            petacontinuum_digests.append(idigest)

        rec = _petacontinuum_round_record(
            round_index=round_index,
            petacontinuum_id=petacontinuum_id,
            continuum_result=continuum_result,
            coverage_before=coverage_before,
        ...
        write_exacontinuum_state(
            ...
            _state_payload(
                ...
                roi_history=roi_history,
                petacontinuum_states=petacontinuum_states,
                petacontinuum_digests=petacontinuum_digests,
                charter=active_charter,
                stop_reason=None,
                exacontinuum_goal=exacontinuum_goal,
                max_active_petacontinuums=active_max,
        ...
        if (
            ...
            and not continuum_result.get("petacontinuum_met")
        ):
            idle_streak += 1
        else:
            idle_streak = 0
            ...
            reason = stop_when(
                {
                    ...
                    "roi_history": roi_history,
                    "petacontinuum_states": petacontinuum_states,
                    "last_petacontinuum_id": petacontinuum_id,
                    "federated_portfolio": federated_portfolio,
                    "exacontinuum_dir": str(exacontinuum_dir),
                    "pending_petacontinuum_ids": [
                        str(s.get("petacontinuum_id") or "")
        ...
        # Grow constitution before declaring exacontinuum_met when expand remains.
        if (
            charter_expand is not None
            and not pending_charter_slots(active_charter, petacontinuum_states)
            and petacontinuums_all_met(petacontinuum_states)
        ):
    ...
    coverage_end = exacontinuum_terminal_coverage(
        petacontinuum_states=petacontinuum_states,
        federated_portfolio=federated_portfolio,
    )
    if constitution_satisfied(
    ...
    petacontinuums_met_count = sum(
        1 for ist in petacontinuum_states if ist.get("petacontinuum_met")
    )
    pending_remaining = [
        str(s.get("petacontinuum_id") or "")
        for s in pending_charter_slots(active_charter, petacontinuum_states)
    ]

    if exacontinuum_met and stop_reason in {"exacontinuum_met", "max_rounds"}:
        verdict = "exacontinuum_met"
        ok = True
        stop_reason = "exacontinuum_met"
    elif stop_reason == "rank_only":
        verdict = "exacontinuum_ranked"
        ok = True
    elif stop_reason == "exacontinuum_idle":
        verdict = "exacontinuum_idle"
        ok = True
    elif stop_reason == "dispatch_budget":
        verdict = "exacontinuum_budgeted"
        ok = True
        ...
        verdict = "exacontinuum_completed"
        ok = True
    ...
    receipt: dict[str, Any] = {
        ...
        "exacontinuum_goal": exacontinuum_goal,
        "exacontinuum_met": exacontinuum_met,
        ...
        "round_count": len(institutions),
        "petacontinuums": institutions,
        "petacontinuum_digests": [
            i.get("petacontinuum_digest")
            for i in institutions
            if i.get("petacontinuum_digest")
        ],
        "petacontinuum_states": petacontinuum_states,
        "petacontinuums_admitted": len(petacontinuum_states),
        "petacontinuums_met_count": petacontinuums_met_count,
        "admissions": admissions,
    ...
    receipt["petacontinuum_digests"] = [
        str(i.get("petacontinuum_digest") or "") for i in institutions
    ]
    receipt["exacontinuum_digest"] = _sha256_json(_exacontinuum_digest_payload(receipt))
    atomic_write_json(exacontinuum_dir / "exacontinuum.json", receipt)
    atomic_write_json(
        exacontinuum_dir / "summary.json",
        {
            "verdict": receipt["verdict"],
            "ok": receipt["ok"],
            "stop_reason": receipt["stop_reason"],
            "exacontinuum_id": receipt["exacontinuum_id"],
            "round_count": receipt["round_count"],
            "total_dispatched": receipt["total_dispatched"],
            "total_dispatched_ok": receipt["total_dispatched_ok"],
            "exacontinuum_met": receipt["exacontinuum_met"],
            ...
            "max_active_petacontinuums": receipt["max_active_petacontinuums"],
            ...
            "exacontinuum_digest": receipt["exacontinuum_digest"],
            "resumed": resumed,
        },
    )
    ...
    write_exacontinuum_state(
        ...
        _state_payload(
            ...
            roi_history=roi_history,
            petacontinuum_states=petacontinuum_states,
            petacontinuum_digests=receipt["petacontinuum_digests"],
            charter=active_charter,
            stop_reason=stop_reason,
            exacontinuum_goal=exacontinuum_goal,
            max_active_petacontinuums=active_max,
    ...
    return {
        ...
        "exacontinuum_id": lid,
        "round_count": len(institutions),
        "petacontinuum_digests": list(receipt["petacontinuum_digests"]),
        "total_dispatched": total_dispatched,
        "total_dispatched_ok": total_dispatched_ok,
        "exacontinuum_met": exacontinuum_met,
        "petacontinuums_admitted": len(petacontinuum_states),
...
def _petacontinuum_slot(
    petacontinuum_id: str,
    *,
    priority: int = 0,
    teracontinuums: Sequence[dict[str, Any]] | None = None,
    ...
    """Build a hermetic exacontinuum charter petacontinuum slot.

    Prefer ``teracontinuums=`` (nested giga slots for the petacontinuum plane).
    ``institutions=`` wrap a single auto giga with mega nesting underneath.
    """
    nested: list[dict[str, Any]]
    if teracontinuums is not None:
        nested = list(teracontinuums)
    elif megacontinuums is not None:
        nested = [
            pp._teracontinuum_slot(
                f"{petacontinuum_id[:1]}g",
...
def builtin_upstream_exacontinuum_proof() -> dict[str, Any]:
    """Hermetic end-to-end proof of the multi-petacontinuum exacontinuum plane (no network).

    Uses an injected fast ``petacontinuum_runner`` so the outer plane constitution
    ...
    try:
        # Per-petacontinuum progress for budget/resume/terminal multi-call realism.
        ...
        def _fast_mega_runner(**kwargs: Any) -> dict[str, Any]:
            ...
            mid = str(kwargs.get("petacontinuum_id") or "m")
            nested_charter = pp.normalize_petacontinuum_charter(kwargs.get("charter"))
            if kwargs.get("resume_dir") is not None and not nested_charter:
                state_path = Path(str(kwargs["resume_dir"])) / "petacontinuum_state.json"
                if state_path.is_file():
                    st = json.loads(state_path.read_text(encoding="utf-8"))
                    nested_charter = pp.normalize_petacontinuum_charter(st.get("charter"))
                    mid = str(st.get("petacontinuum_id") or mid)

            goal = str(kwargs.get("petacontinuum_goal") or "all_teracontinuums_met")
            ...
            include_deferred = st["calls"] > 1 or goal == "all_teracontinuums_met"
            ...
            teracontinuum_states = [
                {
                    "teracontinuum_id": str(
                        s.get("teracontinuum_id") or s.get("id") or "g"
                    ),
            ...
            receipt = {
                "schema_version": 1,
                "ok": True,
                "verdict": "petacontinuum_met" if report_met else "petacontinuum_completed",
                "stop_reason": "petacontinuum_met" if report_met else "dispatch_budget",
                "petacontinuum_id": mid,
                "petacontinuum_goal": str(
                    kwargs.get("petacontinuum_goal") or "all_teracontinuums_met"
                ),
                "max_rounds": int(kwargs.get("max_rounds") or 6),
                "max_active_teracontinuums": kwargs.get("max_active_teracontinuums"),
                ...
                "teracontinuums_admitted": max(1, len(nested_charter) or 1),
                "teracontinuums_met_count": (
            ...
            receipt["petacontinuum_digest"] = _sha256_json(
                pp._petacontinuum_digest_payload(receipt)
            )
            digest = receipt["petacontinuum_digest"]
            atomic_write_json(out / "petacontinuum.json", receipt)
            atomic_write_json(
                out / "petacontinuum_state.json",
                {
                    "petacontinuum_id": mid,
                    "round_count": st["calls"],
                    "total_dispatched": st["dispatched"],
            ...
            return {
                ...
                "verdict": receipt["verdict"],
                "stop_reason": receipt["stop_reason"],
                "petacontinuum_dir": str(out),
                "petacontinuum_digest": digest,
                "petacontinuum_id": mid,
                "total_dispatched": st["dispatched"],
                "total_dispatched_ok": st["dispatched_ok"],
                "petacontinuum_met": report_met,
        ...
        confed = run_exacontinuum(
            charter=charter,
            max_rounds=8,
            dispatch_budget=12,
            dispatch=True,
            petacontinuum_runner=_fast_mega_runner,
            exacontinuum_goal="all_petacontinuums_met",
            out_root=scratch / "m",
        )
        multi_exacontinuum_ok = (
            confed["ok"]
            and confed["exacontinuum_met"] is True
            and confed["stop_reason"] == "exacontinuum_met"
            and confed["petacontinuums_admitted"] == 2
            and confed["petacontinuums_met_count"] == 2
            and confed["round_count"] >= 2
            and confed["total_dispatched_ok"] >= 3
            and float((confed.get("coverage_end") or {}).get("coverage_ratio") or 0) == 1.0
        )
        scheduled_ids = {
            i.get("petacontinuum_id")
            for i in (confed.get("petacontinuums") or [])
        }
        multi_exacontinuum_scheduled = multi_exacontinuum_ok and scheduled_ids >= {"a", "b"}

        verified = verify_exacontinuum_receipt(Path(confed["exacontinuum_dir"]))
        seal_ok = bool(verified.get("ok")) and verified.get("round_count") == confed["round_count"]

        confed_path = Path(confed["exacontinuum_dir"]) / "exacontinuum.json"
        receipt = json.loads(confed_path.read_text(encoding="utf-8"))
        receipt["exacontinuum_digest"] = "0" * 64
        confed_path.write_text(json.dumps(receipt, indent=2), encoding="utf-8")
        tampered = verify_exacontinuum_receipt(Path(confed["exacontinuum_dir"]))
        tamper_detected = (
            not tampered["ok"]
            and "exacontinuum_digest" in (tampered.get("mismatched") or [])
        )

        # Budget stop
        _progress.clear()
        budgeted = run_exacontinuum(
            charter=[
        ...
        def _premet_runner(**kwargs: Any) -> dict[str, Any]:
            ...
            receipt = {
                ...
                "petacontinuum_id": kwargs.get("petacontinuum_id"),
                "petacontinuum_goal": "all_teracontinuums_met",
                "max_rounds": 1,
                "max_active_teracontinuums": None,
                ...
                "teracontinuum_digests": [],
                "teracontinuums_admitted": max(1, len(nested_charter) or 1),
                "teracontinuums_met_count": max(1, len(nested_charter) or 1),
                "admission_count": 0,
                "pending_remaining": [],
                "charter_expansion_count": 0,
                "charter_expanded_ids": [],
            ...
            atomic_write_json(
                out / "petacontinuum_state.json",
                {
                    ...
                    "total_dispatched_ok": 0,
                    "federated_portfolio": portfolio,
                    "teracontinuum_states": receipt["teracontinuum_states"],
                    "stop_reason": "petacontinuum_met",
                    "charter": nested_charter,
                },
            )
        ...
        pre_met = run_exacontinuum(
            charter=[_slot("om", initial=[("om", "9.0.0", "omega-merged")])],
            max_rounds=3,
            dispatch=True,
            petacontinuum_runner=_premet_runner,
            exacontinuum_goal="all_petacontinuums_met",
            out_root=scratch / "p",
        )
        premet_ok = (
            pre_met["ok"]
            and pre_met["exacontinuum_met"] is True
            and pre_met["stop_reason"] == "exacontinuum_met"
            and pre_met["petacontinuums_met_count"] == 1
            and pre_met["total_dispatched"] == 0
        )

        ranked = run_exacontinuum(
            charter=[
        ...
        rank_only_ok = (
            ranked["ok"]
            and ranked["verdict"] == "exacontinuum_ranked"
        ...
        except ExacontinuumRefused as exc:
        ...
        state_path = Path(partial["exacontinuum_dir"]) / "exacontinuum_state.json"
        state_exists = state_path.is_file()
        resumed = run_exacontinuum(
            resume_dir=Path(partial["exacontinuum_dir"]),
            max_rounds=4,
            dispatch_budget=4,
            dispatch=True,
            petacontinuum_runner=_fast_mega_runner,
        ...
        resume_ok = (
            ...
            and resumed["exacontinuum_id"] == "rcp"
        ...
        admit_ids = [a.get("petacontinuum_id") for a in (deferred.get("admissions") or [])]
        deferred_ok = (
            deferred["ok"]
            and deferred["exacontinuum_met"] is True
            and deferred["petacontinuums_admitted"] == 3
            and deferred["petacontinuums_met_count"] == 3
            and deferred.get("max_active_petacontinuums") == 1
        ...
        expand_ok = (
            expanded["ok"]
            and expanded["exacontinuum_met"] is True
            and expanded["petacontinuums_admitted"] == 2
            and expanded["petacontinuums_met_count"] == 2
        ...
        merged = merge_exacontinuum_charter(
        ...
        merge_ok = [s["petacontinuum_id"] for s in merged] == ["m1", "m2"]

        # Terminal coverage with premature-met + deferred reopen (ids start with t)
        _progress.clear()
        terminal = run_exacontinuum(
            charter=[
        ...
        terminal_ok = (
            terminal["ok"]
            and terminal["exacontinuum_met"] is True
            and terminal["stop_reason"] == "exacontinuum_met"
            ...
            and terminal["petacontinuums_admitted"] == 2
        ...
        return {
            "ok": ok,
            "exacontinuum_met": multi_exacontinuum_ok,
            "multi_exacontinuum_progressed": multi_exacontinuum_scheduled,
            ...
            "roi_scored": roi_ok,
            "exacontinuum_digest": confed.get("exacontinuum_digest"),
            "round_count": confed.get("round_count"),
            "total_dispatched_ok": confed.get("total_dispatched_ok"),
            "petacontinuums_admitted": confed.get("petacontinuums_admitted"),
            "used_skill_route_discovery": legacy_pipeline_was_used(),
            "flags": {
                "multi_exacontinuum_ok": multi_exacontinuum_ok,
                "multi_exacontinuum_scheduled": multi_exacontinuum_scheduled,
                "seal_ok": seal_ok,
                "tamper_detected": tamper_detected,
                "budget_ok": budget_ok,
                "premet_ok": premet_ok,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.verify:
        result = verify_exacontinuum_receipt(Path(args.verify))
        print(json.dumps(result, indent=2, sort_keys=True))
        return 0 if result.get("ok") else 1
    if args.proof:
        result = builtin_upstream_exacontinuum_proof()
        print(json.dumps(result, indent=2, sort_keys=True))
        return 0 if result.get("ok") else 1
    parser.print_help()
    return 2
