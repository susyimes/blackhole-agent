"""Upstream petacontinuum plane: multi-teracontinuum durable stewardship petacontinuum.

The teracontinuum plane (``upstream_teracontinuum``) closes multi-continuum unions *within one
teracontinuum*. It does not:

1. chain multiple independent teracontinuums under a durable petacontinuum constitution;
2. allocate a shared global dispatch budget across teracontinuums by ROI;
3. admit/retire teracontinuum slots from an petacontinuum charter over time
   (deferred admission under a concurrent-active cap);
4. grow the petacontinuum charter mid-run via ``charter_expand`` (constitution
   growth beyond the initial charter, not just deferred admission of a fixed set);
5. federate multi-teracontinuum portfolio coverage into one petacontinuum world-model;
6. persist petacontinuum state so a later process can resume the union;
7. seal a multi-teracontinuum petacontinuum chronicle linking teracontinuum digests.

The petacontinuum plane closes that outer multi-teracontinuum loop:

1. **admit** — materialize teracontinuum slots from a durable petacontinuum charter
   (each slot owns a nested continuum charter). When ``max_active_teracontinuums``
   is set, only that many *unmet* teracontinuums are concurrent: further charter
   slots stay pending and are admitted as capacity frees after retirements
   (petacontinuum constitution growth over time);
2. **schedule** — pick the next open teracontinuum by priority and historical ROI;
3. **teracontinuum** — call the teracontinuum plane (injected ``teracontinuum_runner``;
   default ``run_teracontinuum``) with a share of the remaining global dispatch
   budget;
4. **federate** — merge per-teracontinuum portfolios into one petacontinuum world-model
   and re-score coverage across all stewarded keys;
5. **retire** — mark teracontinuums met when their teracontinuum_goal is satisfied,
   then re-admit pending charter slots up to the active capacity;
6. **expand** — optional ``charter_expand`` may append new teracontinuum slots when
   the active charter has no pending work and all admitted teracontinuums are met,
   so the petacontinuum constitution can grow after start (not only defer a fixed charter);
7. **persist** — write ``petacontinuum_state.json`` after every teracontinuum round so a
   later ``run_petacontinuum(..., resume_dir=...)`` continues the same union
   (including pending charter and admission history);
8. **stop** when any of:

   - ``max_rounds`` reached
   - global ``dispatch_budget`` exhausted across teracontinuums
   - petacontinuum goal met (``all_teracontinuums_met``: every *admitted*
     teracontinuum is met *and* no pending charter slots remain)
   - consecutive idle/no-progress rounds (``idle_round_limit``)
   - explicit ``stop_when`` predicate returns a reason string

9. **seal** — write an petacontinuum receipt under
   ``artifacts/upstream-petacontinuum/`` with sha256 digests of every
   teracontinuum, portfolio federation, admission history, ROI history, stop
   reason, and an petacontinuum chain digest; ``verify_petacontinuum_receipt``
   re-checks the chain and detects tampering.

No skill-route discovery is used. The plane is petacontinuum-level direction
over the teracontinuum plane, not a new verifier of individual repairs.
"""
...
from blackhole_agent import upstream_teracontinuum as tt
...
ARTIFACTS_ROOT = REPO_ROOT / "artifacts" / "upstream-petacontinuum"

TERMINAL_SUCCESS_OUTCOMES = tt.TERMINAL_SUCCESS_OUTCOMES


class PetacontinuumRefused(Exception):
...
def normalize_petacontinuum_charter(
    charter: Sequence[Mapping[str, Any]] | None,
) -> list[dict[str, Any]]:
    """Normalize a petacontinuum charter into deterministic continuum slots.
    ...
    for raw in charter:
        if not isinstance(raw, Mapping):
            continue
        teracontinuum_id = str(
            raw.get("teracontinuum_id") or raw.get("id") or ""
        ).strip()
        if not teracontinuum_id or teracontinuum_id in seen:
            continue
        seen.add(teracontinuum_id)

        nested = tt.normalize_teracontinuum_charter(
            raw.get("charter")
        ...
        max_active_gigacontinuums = raw.get("max_active_gigacontinuums")
        if max_active_gigacontinuums is None:
            max_active_gigacontinuums = raw.get("max_active_continuums")
        if max_active_gigacontinuums is None:
            max_active_gigacontinuums = raw.get("max_active_civilizations")
        if max_active_gigacontinuums is not None:
            max_active_gigacontinuums = max(1, int(max_active_gigacontinuums))
        ...
        out.append(
            {
                ...
                "teracontinuum_goal": str(
                    raw.get("teracontinuum_goal") or "all_gigacontinuums_met"
                ),
                "kind": str(raw.get("kind") or "stewardship_teracontinuum"),
            }
        )
    return out


def admit_teracontinuum_slot(
    *,
    petacontinuum_dir: Path,
    slot: Mapping[str, Any],
) -> dict[str, Any]:
    """Materialize one institution slot under the league directory.

    Returns admission record with teracontinuum_root and nested charter.
    Stewardship surfaces are created lazily by the institution plane on run.
    """
    teracontinuum_id = str(slot.get("teracontinuum_id") or "")
    if not teracontinuum_id:
        raise PetacontinuumRefused("petacontinuum_invalid", "slot missing teracontinuum_id")

    teracontinuum_root = Path(petacontinuum_dir) / "teracontinuums" / teracontinuum_id
    teracontinuum_root.mkdir(parents=True, exist_ok=True)

    nested_charter = tt.normalize_teracontinuum_charter(slot.get("charter"))
    if not nested_charter:
        raise PetacontinuumRefused(
            "petacontinuum_invalid",
            f"continuum slot {teracontinuum_id!r} has empty nested charter",
        )

    max_active_gigacontinuums = slot.get("max_active_gigacontinuums")
    if max_active_gigacontinuums is None:
        max_active_gigacontinuums = slot.get("max_active_continuums")
    if max_active_gigacontinuums is None:
        max_active_gigacontinuums = slot.get("max_active_civilizations")

    return {
        "teracontinuum_id": teracontinuum_id,
        "teracontinuum_root": str(teracontinuum_root),
        "admitted": True,
        "charter": nested_charter,
        "max_active_gigacontinuums": max_active_gigacontinuums,
        "max_rounds": int(slot.get("max_rounds") or 6),
        "teracontinuum_goal": str(slot.get("teracontinuum_goal") or "all_gigacontinuums_met"),
...
def federate_portfolios(
    ...
    portfolio: dict[str, Any] = {
        ...
        "entries": entries,
        "source": "petacontinuum_federation",
    }
...
def petacontinuum_terminal_coverage(
    *,
    teracontinuum_states: Sequence[Mapping[str, Any]],
    federated_portfolio: Mapping[str, Any] | None,
) -> dict[str, Any]:
    """Coverage across teracontinuum->gigacontinuum->...->program."""
    gigacontinuum_states: list[dict[str, Any]] = []
    for uc in teracontinuum_states:
        if not isinstance(uc, Mapping):
            continue
        nested = (
            list(uc.get("gigacontinuum_states") or [])
        ...
        if nested:
            for hc in nested:
                if isinstance(hc, Mapping):
                    gigacontinuum_states.append(dict(hc))
            continue
        # Pre-run: teracontinuum charter is a list of gigacontinuum slots.
        for cslot in list(uc.get("charter") or []) + list(
            uc.get("gigacontinuums") or []
            ...
            if isinstance(cslot, Mapping):
                gigacontinuum_states.append(dict(cslot))
        if not nested and not list(uc.get("charter") or []):
            for cont in list(uc.get("continuum_states") or []) + list(
                ...
                if isinstance(cont, Mapping):
                    gigacontinuum_states.append(dict(cont))
        for raw in list(uc.get("inventory_keys") or []):
            if isinstance(raw, (list, tuple)) and len(raw) >= 3:
                ...
                gigacontinuum_states.append({"inventory_keys": [raw]})
    return tt.teracontinuum_terminal_coverage(
        gigacontinuum_states=gigacontinuum_states,
        federated_portfolio=federated_portfolio,
    )


def teracontinuums_all_met(teracontinuum_states: Sequence[Mapping[str, Any]]) -> bool:
    if not teracontinuum_states:
        return False
    return all(bool(ist.get("teracontinuum_met")) for ist in teracontinuum_states)


def open_unmet_count(teracontinuum_states: Sequence[Mapping[str, Any]]) -> int:
    """Count admitted institutions that are not yet continuum_met."""
    return sum(1 for ist in teracontinuum_states if not ist.get("teracontinuum_met"))
...
def pending_charter_slots(
    ...
    """Charter slots not yet admitted, priority-desc then teracontinuum_id-asc."""
    known = {str(ist.get("teracontinuum_id") or "") for ist in teracontinuum_states}
    pending = [
        dict(slot)
        for slot in charter
        if str(slot.get("teracontinuum_id") or "")
        and str(slot.get("teracontinuum_id")) not in known
    ]
    pending.sort(
        key=lambda s: (
            -int(s.get("priority") or 0),
            str(s.get("teracontinuum_id") or ""),
        )
    )
    return pending
...
def constitution_satisfied(
    *,
    teracontinuum_states: Sequence[Mapping[str, Any]],
    charter: Sequence[Mapping[str, Any]],
    petacontinuum_goal: str,
    federated_portfolio: Mapping[str, Any] | None = None,
) -> bool:
    """True when the league goal is fully met including pending charter."""
    if petacontinuum_goal == "none":
        return False
    if petacontinuum_goal == "terminal_coverage":
        cov = petacontinuum_terminal_coverage(
            teracontinuum_states=teracontinuum_states,
            federated_portfolio=federated_portfolio,
        )
        return bool(cov.get("met")) and not pending_charter_slots(
            charter, teracontinuum_states
        )
    if petacontinuum_goal == "all_teracontinuums_met":
        if not teracontinuum_states:
            return False
        if pending_charter_slots(charter, teracontinuum_states):
            return False
        return teracontinuums_all_met(teracontinuum_states)
    return False


def reopen_incomplete_teracontinuums(
    teracontinuum_states: list[dict[str, Any]],
    *,
    federated_portfolio: Mapping[str, Any] | None,
) -> list[str]:
    """Clear ``continuum_met`` on children still short of terminal coverage.

    Nested teracontinuums can retire after only partial surface work (e.g. before
    deferred program targets expand). Under a ``terminal_coverage`` petacontinuum
    goal those children must re-run until federated inventory is fully
    terminal-success. Returns the reopened ``teracontinuum_id`` list.
    """
    cov = petacontinuum_terminal_coverage(
        teracontinuum_states=teracontinuum_states,
        federated_portfolio=federated_portfolio,
    ...
    for item in list(cov.get("open_or_missing") or []):
        ...
        if key[0] and key[2]:
            open_keys.add(key)
    reopened: list[str] = []
    for i, ist in enumerate(teracontinuum_states):
        if not ist.get("teracontinuum_met"):
            continue
        ...
        for mv in list(ist.get("gigacontinuum_states") or []) + list(
        ...
        if not owns_open:
            continue
        updated = dict(ist)
        updated["teracontinuum_met"] = False
        teracontinuum_states[i] = updated
        reopened.append(str(updated.get("teracontinuum_id") or ""))
    return [r for r in reopened if r]


def merge_petacontinuum_charter(
    ...
    Existing ``teracontinuum_id`` values win (additions with the same id are
    ignored). Returns a fully re-normalized charter so nested confederation
    charters stay deterministic.
    """
    base = normalize_petacontinuum_charter(existing)
    if not additions:
        return base
    known = {str(s.get("teracontinuum_id") or "") for s in base}
    merged: list[Mapping[str, Any]] = list(base)
    for raw in additions:
        if not isinstance(raw, Mapping):
            continue
        cid = str(raw.get("teracontinuum_id") or raw.get("id") or "").strip()
        if not cid or cid in known:
            continue
        known.add(cid)
        merged.append(raw)
    return normalize_petacontinuum_charter(merged)


def make_petacontinuum_charter_expand(
    ...
    *,
    ...
    """
    pending_growth = normalize_petacontinuum_charter(growth)
    ...
    def _runner(
        *,
        ...
        remaining = [
            ...
            if str(s.get("teracontinuum_id") or "") not in applied_ids
            and str(s.get("teracontinuum_id") or "")
            not in {str(x.get("teracontinuum_id") or "") for x in active_charter}
        ]
        if not remaining:
            return {
                "expanded": False,
        ...
        for s in take:
            applied_ids.add(str(s.get("teracontinuum_id") or ""))
        merged = merge_petacontinuum_charter(active_charter, take)
        state["applied"] = applied_ids
        return {
            "expanded": True,
            "added": [str(s.get("teracontinuum_id") or "") for s in take],
            "charter": merged,
            "detail": "charter_growth_applied",
            "round_index": round_index,
            "realms_met": teracontinuums_all_met(teracontinuum_states),
        }
...
def admit_pending_slots(
    *,
    ...
    charter: Sequence[Mapping[str, Any]],
    teracontinuum_states: list[dict[str, Any]],
    max_active_teracontinuums: int | None,
    round_index: int | None = None,
) -> list[dict[str, Any]]:
    """Admit pending charter slots up to concurrent-active capacity.

    ``max_active_teracontinuums`` caps *unmet* concurrent realms. ``None``
    admits every remaining pending slot. Returns admission records for newly
    admitted slots (also mutates ``teracontinuum_states``).
    """
    pending = pending_charter_slots(charter, teracontinuum_states)
    if not pending:
        return []

    open_n = open_unmet_count(teracontinuum_states)
    if max_active_teracontinuums is None:
        capacity = len(pending)
    else:
        capacity = max(0, int(max_active_teracontinuums) - open_n)
    if capacity <= 0:
        return []

    admissions: list[dict[str, Any]] = []
    for slot in pending[:capacity]:
        admission = admit_teracontinuum_slot(petacontinuum_dir=petacontinuum_dir, slot=slot)
        if round_index is not None:
            admission = dict(admission)
            admission["admitted_at_round"] = round_index
        admissions.append(admission)
        teracontinuum_states.append(
            {
                "teracontinuum_id": admission["teracontinuum_id"],
                "teracontinuum_root": admission["teracontinuum_root"],
                "charter": admission["charter"],
                "max_active_gigacontinuums": admission.get("max_active_gigacontinuums"),
                "max_rounds": admission["max_rounds"],
                "teracontinuum_goal": admission["teracontinuum_goal"],
                "priority": admission["priority"],
                "teracontinuum_met": False,
                "last_teracontinuum_dir": None,
                "last_teracontinuum_digest": None,
                "portfolio": None,
                "omniverse_states": [],
                "multiverse_states": [],
                "cosmos_states": [],
...
def score_teracontinuum_roi(
    *,
    ...
    return {
        ...
        "teracontinuum_met": bool(continuum_result.get("teracontinuum_met")),
        "continuum_digest": continuum_result.get("teracontinuum_digest"),
        "continuums_admitted": int(
            continuum_result.get("gigacontinuums_admitted")
            or continuum_result.get("megacontinuums_admitted")
            or continuum_result.get("ultracontinuums_admitted")
            or continuum_result.get("teracontinuums_admitted")
...
def _roi_summary(roi_history: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for r in roi_history:
        iid = str(r.get("teracontinuum_id") or "")
...
def select_next_teracontinuum(
    ...
    *,
    ...
    Tie-break: lower run_count, then stable teracontinuum_id order. Round-robin
    among equal scores uses ``round_index`` so multi-institution progress is fair.
    """
    open_slots = [
        dict(ist) for ist in teracontinuum_states if not ist.get("teracontinuum_met")
    ]
    if not open_slots:
        return None
    ...
    def sort_key(ist: Mapping[str, Any]) -> tuple[Any, ...]:
        iid = str(ist.get("teracontinuum_id") or "")
...
def allocate_teracontinuum_budget(
    *,
    ...
    hist = (summary.get("by_continuum") or {}).get(
        str(selected.get("teracontinuum_id") or "")
    ) or {}
    mean_eff = float(hist.get("mean_efficiency") or 0.0)
...
def _state_payload(
    *,
    ...
    roi_history: Sequence[Mapping[str, Any]],
    teracontinuum_states: Sequence[Mapping[str, Any]],
    teracontinuum_digests: Sequence[str],
    charter: Sequence[Mapping[str, Any]],
    stop_reason: str | None,
    petacontinuum_goal: str,
    max_active_teracontinuums: int | None = None,
    admissions: Sequence[Mapping[str, Any]] | None = None,
    charter_expansions: Sequence[Mapping[str, Any]] | None = None,
) -> dict[str, Any]:
    return {
        "schema_version": SCHEMA_VERSION,
        "petacontinuum_id": petacontinuum_id,
        ...
        "teracontinuum_states": [dict(ist) for ist in teracontinuum_states],
        "teracontinuum_digests": list(teracontinuum_digests),
        "charter": list(charter),
        "stop_reason": stop_reason,
        "petacontinuum_goal": petacontinuum_goal,
        "max_active_teracontinuums": max_active_teracontinuums,
        "admissions": [dict(a) for a in (admissions or [])],
        "charter_expansions": [dict(e) for e in (charter_expansions or [])],
        "pending_teracontinuum_ids": [
            str(s.get("teracontinuum_id") or "")
            for s in pending_charter_slots(charter, teracontinuum_states)
        ],
    }


def write_petacontinuum_state(petacontinuum_dir: Path, state: Mapping[str, Any]) -> Path:
    path = Path(petacontinuum_dir) / "petacontinuum_state.json"
    atomic_write_json(path, dict(state))
    return path


def load_petacontinuum_state(resume_dir: Path) -> dict[str, Any]:
    path = durable_read_path(Path(resume_dir) / "petacontinuum_state.json")
    if not path.is_file():
        raise PetacontinuumRefused(
            "petacontinuum_state_missing",
            f"no petacontinuum_state.json under {resume_dir}",
        )
    try:
        state = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise PetacontinuumRefused("petacontinuum_state_unreadable", str(exc)) from exc
    if not isinstance(state, dict):
        raise PetacontinuumRefused("petacontinuum_state_invalid", "state root must be object")
    return state
...
def _teracontinuum_round_record(
    *,
    round_index: int,
    teracontinuum_id: str,
    continuum_result: Mapping[str, Any],
    ...
    return {
        ...
        "stop_reason": continuum_result.get("stop_reason"),
        "teracontinuum_dir": continuum_result.get("teracontinuum_dir"),
        "teracontinuum_digest": continuum_result.get("teracontinuum_digest"),
        "continuums_admitted": int(
            continuum_result.get("gigacontinuums_admitted")
            or continuum_result.get("megacontinuums_admitted")
            or continuum_result.get("ultracontinuums_admitted")
...
def _petacontinuum_digest_payload(receipt: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": receipt.get("schema_version"),
        "verdict": receipt.get("verdict"),
        "stop_reason": receipt.get("stop_reason"),
        "petacontinuum_id": receipt.get("petacontinuum_id"),
        "petacontinuum_goal": receipt.get("petacontinuum_goal"),
        "max_rounds": receipt.get("max_rounds"),
        "max_active_teracontinuums": receipt.get("max_active_teracontinuums"),
        ...
        "teracontinuum_digests": list(receipt.get("teracontinuum_digests") or []),
        "total_dispatched": receipt.get("total_dispatched"),
        "total_dispatched_ok": receipt.get("total_dispatched_ok"),
        "petacontinuum_met": receipt.get("petacontinuum_met"),
        "coverage_end": receipt.get("coverage_end"),
        "teracontinuums_met_count": receipt.get("teracontinuums_met_count"),
        "teracontinuums_admitted": receipt.get("teracontinuums_admitted"),
...
def verify_petacontinuum_receipt(petacontinuum_dir: Path) -> dict[str, Any]:
    """Re-check a sealed league receipt for digest integrity."""
    path = durable_read_path(Path(petacontinuum_dir) / "petacontinuum.json")
    if not path.is_file():
    ...
    expected = _sha256_json(_petacontinuum_digest_payload(receipt))
    recorded = str(receipt.get("petacontinuum_digest") or "")
    mismatched: list[str] = []
    if not recorded or recorded != expected:
        mismatched.append("petacontinuum_digest")

    institutions = list(receipt.get("teracontinuums") or receipt.get("realms") or receipt.get("leagues") or [])
    listed = list(receipt.get("teracontinuum_digests") or [])
    if len(listed) != len(institutions):
        mismatched.append("teracontinuum_digests_length")
    else:
        for i, (listed_d, rec) in enumerate(zip(listed, institutions)):
            if listed_d != rec.get("teracontinuum_digest"):
                mismatched.append(f"teracontinuum_digests[{i}]")

    nested_failures: list[str] = []
    for rec in institutions:
        idir = rec.get("teracontinuum_dir")
        if not idir:
            continue
        ip = Path(str(idir))
        if (ip / "teracontinuum.json").is_file():
            nested = tt.verify_teracontinuum_receipt(ip)
    ...
    return {
        "ok": ok,
        "verdict": "petacontinuum_sealed" if ok else "petacontinuum_tampered",
        "petacontinuum_digest": recorded,
...
def run_petacontinuum(
    *,
    ...
    max_active_teracontinuums: int | None = None,
    dispatch: bool = True,
    teracontinuum_runner: Callable[..., dict[str, Any]] | None = None,
    ...
    petacontinuum_goal: str = "all_teracontinuums_met",
    refresh_promotions: Mapping[str, str] | None = None,
    petacontinuum_id: str | None = None,
    ...
    charter:
        ...
        :func:`normalize_petacontinuum_charter`).
    max_rounds:
    ...
    petacontinuum_goal:
        ``all_teracontinuums_met`` (default) stops when every admitted institution
    ...
    resume_dir:
        Load ``petacontinuum_state.json`` from a prior league dir and continue.
        New receipt is written under ``out_root`` (or a fresh stamp).
    """
    if max_rounds < 1:
        raise PetacontinuumRefused("petacontinuum_invalid", "max_rounds must be >= 1")
    if per_wave_dispatch_limit < 0:
        raise PetacontinuumRefused(
            "petacontinuum_invalid", "per_wave_dispatch_limit must be >= 0"
        )
    if max_active_teracontinuums is not None and int(max_active_teracontinuums) < 1:
        raise PetacontinuumRefused(
            "petacontinuum_invalid", "max_active_teracontinuums must be >= 1 when set"
        )
    if petacontinuum_goal not in {"all_teracontinuums_met", "terminal_coverage", "none"}:
        raise PetacontinuumRefused(
            "petacontinuum_invalid",
            f"unknown petacontinuum_goal: {petacontinuum_goal}",
        )

    runner = teracontinuum_runner or tt.run_teracontinuum

    prior_round_count = 0
    roi_history: list[dict[str, Any]] = []
    teracontinuum_digests: list[str] = []
    total_dispatched = 0
    total_dispatched_ok = 0
    resumed = False
    resume_petacontinuum_id: str | None = None
    teracontinuum_states: list[dict[str, Any]] = []
    ...
    if resume_dir is not None:
        state = load_petacontinuum_state(resume_dir)
        resumed = True
        resume_petacontinuum_id = str(state.get("petacontinuum_id") or "") or None
        ...
        teracontinuum_digests = [str(d) for d in (state.get("teracontinuum_digests") or [])]
        teracontinuum_states = [
            dict(ist)
            for ist in (state.get("teracontinuum_states") or [])
        ...
        if isinstance(state.get("charter"), list):
            active_charter = normalize_petacontinuum_charter(
        ...
        if (
            state.get("max_active_teracontinuums") is not None
            and max_active_teracontinuums is None
        ):
            resumed_max_active = int(state["max_active_teracontinuums"])
        # Resume may also merge a caller-supplied charter growth tail.
        if charter:
            active_charter = merge_petacontinuum_charter(active_charter, charter)
    else:
        active_charter = normalize_petacontinuum_charter(charter)

    active_max = (
        max_active_teracontinuums
        if max_active_teracontinuums is not None
        else resumed_max_active
    )

    if not active_charter and not teracontinuum_states:
        raise PetacontinuumRefused(
            "petacontinuum_empty",
    ...
    lid = (
        petacontinuum_id
        or resume_petacontinuum_id
        or f"petacontinuum-{utc_now_iso().replace(':', '').replace('-', '')}"
    )

    stamp = utc_now_iso().replace(":", "").replace("-", "")
    if out_root is not None:
        petacontinuum_dir = Path(out_root)
        if (petacontinuum_dir / "petacontinuum.json").is_file():
            petacontinuum_dir = petacontinuum_dir / stamp
    else:
        petacontinuum_dir = ARTIFACTS_ROOT / stamp
    petacontinuum_dir.mkdir(parents=True, exist_ok=True)
    ...
    # on a flat ultra-short base (not under petacontinuum_dir) so one extra
    ...
    if league_out_root is not None:
        ...
        lg_root = petacontinuum_dir / "e"
    lg_root.mkdir(parents=True, exist_ok=True)
    if _os.name == "nt":
        inst_flat_root = Path("C:/t") / "q"
    else:
        inst_flat_root = petacontinuum_dir / "x"
    inst_flat_root.mkdir(parents=True, exist_ok=True)
    # Per-run namespace so parallel/resumed ultracontinua do not collide.
    # Prefer a short random token over the human lid (often "petacontinuum-…")
    # so Windows paths stay short and never re-enter a prior teracontinuum_dir that
    ...
    initial_admissions = admit_pending_slots(
        petacontinuum_dir=petacontinuum_dir,
        charter=active_charter,
        teracontinuum_states=teracontinuum_states,
        max_active_teracontinuums=active_max,
        round_index=prior_round_count,
    )
    admissions.extend(initial_admissions)

    if not teracontinuum_states and not pending_charter_slots(
        active_charter, teracontinuum_states
    ):
        raise PetacontinuumRefused("petacontinuum_empty", "no realm slots admitted")
    if not teracontinuum_states and pending_charter_slots(
        active_charter, teracontinuum_states
    ):
        raise PetacontinuumRefused(
            "petacontinuum_empty",
            "no realm slots admitted under max_active_teracontinuums policy",
        )

    if federated_portfolio is None:
        federated_portfolio = federate_portfolios(
            [ist.get("portfolio") for ist in teracontinuum_states]
        )
    ...
    idle_streak = 0
    petacontinuum_met = False
    terminal_reopen_count = 0
    max_terminal_reopens = max(2, (len(teracontinuum_states) or 1) * 4)
    coverage_end: dict[str, Any] = petacontinuum_terminal_coverage(
        teracontinuum_states=teracontinuum_states,
        federated_portfolio=federated_portfolio,
    )
    ...
    for local_index in range(max_rounds):
        ...
        coverage_before = petacontinuum_terminal_coverage(
            teracontinuum_states=teracontinuum_states,
            federated_portfolio=federated_portfolio,
        )
        ...
        selected = select_next_teracontinuum(
            teracontinuum_states, roi_history, round_index=round_index
        )
        if selected is None:
            # Children may all report met while federated terminal coverage is
            # still incomplete (e.g. deferred surfaces). Only declare petacontinuum
            # met when the active petacontinuum_goal is actually satisfied.
            if constitution_satisfied(
                teracontinuum_states=teracontinuum_states,
                charter=active_charter,
                petacontinuum_goal=petacontinuum_goal,
                ...
                break
            if (
                petacontinuum_goal == "terminal_coverage"
                and terminal_reopen_count < max_terminal_reopens
            ):
                reopened = reopen_incomplete_teracontinuums(
                    teracontinuum_states,
                    federated_portfolio=federated_portfolio,
                )
                if reopened:
            ...
            if selected is None:
                stop_reason = "petacontinuum_idle"
                coverage_end = coverage_before
                break

        open_count = sum(
            1 for ist in teracontinuum_states if not ist.get("teracontinuum_met")
        )
        allocated = allocate_teracontinuum_budget(
        ...
        teracontinuum_id = str(selected["teracontinuum_id"])
        resume_teracontinuum_dir = selected.get("last_teracontinuum_dir")
        continuum_resume: Path | None = None
        if (
            resume_teracontinuum_dir
            and (Path(str(resume_teracontinuum_dir)) / "teracontinuum_state.json").is_file()
            and not selected.get("teracontinuum_met")
        ):
            continuum_resume = Path(str(resume_teracontinuum_dir))

        # Ultra-short stamp (each outer plane tightens Windows MAX_PATH).
        # Do NOT nest child teracontinuum trees under petacontinuum roots:
        # let the teracontinuum plane use its own flat C:/t/{h,i} bases so the
        # deep continuum→…→wave chain still fits. Only the teracontinuum receipt
        # itself lands on a flat unique out_root.
        safe_id = "".join(c if c.isalnum() else "" for c in teracontinuum_id)[:2] or "i"
        ...
        continuum_kwargs: dict[str, Any] = {
            ...
            "teracontinuum_goal": str(
                selected.get("teracontinuum_goal") or "all_gigacontinuums_met"
            ),
            "teracontinuum_id": teracontinuum_id,
            "out_root": out_dir,
        }
        ...
        max_active_gigacontinuums = selected.get("max_active_gigacontinuums")
        if max_active_gigacontinuums is None:
            max_active_gigacontinuums = selected.get("max_active_continuums")
        if max_active_gigacontinuums is None:
            max_active_gigacontinuums = selected.get("max_active_civilizations")
        if max_active_gigacontinuums is not None:
            continuum_kwargs["max_active_gigacontinuums"] = int(
        ...
        except tt.TeracontinuumRefused as exc:
            if local_index == 0 and not resumed:
                raise PetacontinuumRefused(exc.verdict, exc.detail) from exc
        ...
        nested_teracontinuum_dir = continuum_result.get("teracontinuum_dir")
        nested_multiverse_states: list[dict[str, Any]] = []
        nested_receipt_path = (
            Path(str(nested_teracontinuum_dir)) / "teracontinuum.json"
            if nested_teracontinuum_dir
            else None
        )
        if nested_receipt_path is not None and nested_receipt_path.is_file():
            ...
            for ist in list(
                receipt.get("gigacontinuum_states")
        ...
        if not nested_multiverse_states:
            for ist in list(
                continuum_result.get("gigacontinuum_states")
        ...
        for i, lst in enumerate(teracontinuum_states):
            if str(lst.get("teracontinuum_id")) != teracontinuum_id:
                continue
            updated = dict(lst)
            updated["last_teracontinuum_dir"] = continuum_result.get("teracontinuum_dir")
            updated["last_teracontinuum_digest"] = continuum_result.get("teracontinuum_digest")
            updated["teracontinuum_met"] = bool(continuum_result.get("teracontinuum_met"))
            ...
            if nested_multiverse_states:
                updated["gigacontinuum_states"] = nested_multiverse_states
        ...
        coverage_after = petacontinuum_terminal_coverage(
            teracontinuum_states=teracontinuum_states,
            federated_portfolio=federated_portfolio,
        )

        roi = score_teracontinuum_roi(
            round_index=round_index,
            teracontinuum_id=teracontinuum_id,
        ...
        idigest = str(continuum_result.get("teracontinuum_digest") or "")
        if idigest:
            teracontinuum_digests.append(idigest)

        rec = _teracontinuum_round_record(
            round_index=round_index,
            teracontinuum_id=teracontinuum_id,
            continuum_result=continuum_result,
            coverage_before=coverage_before,
        ...
        write_petacontinuum_state(
            petacontinuum_dir,
            _state_payload(
                petacontinuum_id=lid,
                ...
                roi_history=roi_history,
                teracontinuum_states=teracontinuum_states,
                teracontinuum_digests=teracontinuum_digests,
                charter=active_charter,
                stop_reason=None,
                petacontinuum_goal=petacontinuum_goal,
                max_active_teracontinuums=active_max,
        ...
        if (
            ...
            and not continuum_result.get("teracontinuum_met")
        ):
            idle_streak += 1
        else:
            idle_streak = 0
            ...
            reason = stop_when(
                {
                    ...
                    "roi_history": roi_history,
                    "teracontinuum_states": teracontinuum_states,
                    "last_teracontinuum_id": teracontinuum_id,
                    "federated_portfolio": federated_portfolio,
                    "petacontinuum_dir": str(petacontinuum_dir),
                    "pending_teracontinuum_ids": [
                        str(s.get("teracontinuum_id") or "")
        ...
        # Grow constitution before declaring petacontinuum_met when expand remains.
        if (
            charter_expand is not None
            and not pending_charter_slots(active_charter, teracontinuum_states)
            and teracontinuums_all_met(teracontinuum_states)
        ):
    ...
    coverage_end = petacontinuum_terminal_coverage(
        teracontinuum_states=teracontinuum_states,
        federated_portfolio=federated_portfolio,
    )
    if constitution_satisfied(
    ...
    teracontinuums_met_count = sum(
        1 for ist in teracontinuum_states if ist.get("teracontinuum_met")
    )
    pending_remaining = [
        str(s.get("teracontinuum_id") or "")
        for s in pending_charter_slots(active_charter, teracontinuum_states)
    ]

    if petacontinuum_met and stop_reason in {"petacontinuum_met", "max_rounds"}:
        verdict = "petacontinuum_met"
        ok = True
        stop_reason = "petacontinuum_met"
    elif stop_reason == "rank_only":
        verdict = "petacontinuum_ranked"
        ok = True
    elif stop_reason == "petacontinuum_idle":
        verdict = "petacontinuum_idle"
        ok = True
    elif stop_reason == "dispatch_budget":
        verdict = "petacontinuum_budgeted"
        ok = True
        ...
        verdict = "petacontinuum_completed"
        ok = True
    ...
    receipt: dict[str, Any] = {
        ...
        "petacontinuum_goal": petacontinuum_goal,
        "petacontinuum_met": petacontinuum_met,
        ...
        "round_count": len(institutions),
        "teracontinuums": institutions,
        "teracontinuum_digests": [
            i.get("teracontinuum_digest")
            for i in institutions
            if i.get("teracontinuum_digest")
        ],
        "teracontinuum_states": teracontinuum_states,
        "teracontinuums_admitted": len(teracontinuum_states),
        "teracontinuums_met_count": teracontinuums_met_count,
        "admissions": admissions,
    ...
    receipt["teracontinuum_digests"] = [
        str(i.get("teracontinuum_digest") or "") for i in institutions
    ]
    receipt["petacontinuum_digest"] = _sha256_json(_petacontinuum_digest_payload(receipt))
    atomic_write_json(petacontinuum_dir / "petacontinuum.json", receipt)
    atomic_write_json(
        petacontinuum_dir / "summary.json",
        {
            "verdict": receipt["verdict"],
            "ok": receipt["ok"],
            "stop_reason": receipt["stop_reason"],
            "petacontinuum_id": receipt["petacontinuum_id"],
            "round_count": receipt["round_count"],
            "total_dispatched": receipt["total_dispatched"],
            "total_dispatched_ok": receipt["total_dispatched_ok"],
            "petacontinuum_met": receipt["petacontinuum_met"],
            "teracontinuums_admitted": receipt["teracontinuums_admitted"],
            "teracontinuums_met_count": receipt["teracontinuums_met_count"],
            ...
            "max_active_teracontinuums": receipt["max_active_teracontinuums"],
            ...
            "petacontinuum_digest": receipt["petacontinuum_digest"],
            "resumed": resumed,
        },
    )
    ...
    write_petacontinuum_state(
        ...
        _state_payload(
            ...
            roi_history=roi_history,
            teracontinuum_states=teracontinuum_states,
            teracontinuum_digests=receipt["teracontinuum_digests"],
            charter=active_charter,
            stop_reason=stop_reason,
            petacontinuum_goal=petacontinuum_goal,
            max_active_teracontinuums=active_max,
    ...
    return {
        ...
        "petacontinuum_id": lid,
        "round_count": len(institutions),
        "teracontinuum_digests": list(receipt["teracontinuum_digests"]),
        "total_dispatched": total_dispatched,
        "total_dispatched_ok": total_dispatched_ok,
        "petacontinuum_met": petacontinuum_met,
        "teracontinuums_admitted": len(teracontinuum_states),
...
def _teracontinuum_slot(
    teracontinuum_id: str,
    *,
    priority: int = 0,
    gigacontinuums: Sequence[dict[str, Any]] | None = None,
    ...
    """Build a hermetic petacontinuum charter teracontinuum slot.

    Prefer ``gigacontinuums=`` (nested giga slots for the teracontinuum plane).
    ``institutions=`` wrap a single auto giga with mega nesting underneath.
    """
    nested: list[dict[str, Any]]
    if gigacontinuums is not None:
        nested = list(gigacontinuums)
    elif megacontinuums is not None:
        nested = [
            tt._gigacontinuum_slot(
                f"{teracontinuum_id[:1]}g",
...
def builtin_upstream_petacontinuum_proof() -> dict[str, Any]:
    """Hermetic end-to-end proof of the multi-teracontinuum petacontinuum plane (no network).

    Uses an injected fast ``teracontinuum_runner`` so the outer plane constitution
    ...
    try:
        # Per-teracontinuum progress for budget/resume/terminal multi-call realism.
        ...
        def _fast_mega_runner(**kwargs: Any) -> dict[str, Any]:
            ...
            mid = str(kwargs.get("teracontinuum_id") or "m")
            nested_charter = tt.normalize_teracontinuum_charter(kwargs.get("charter"))
            if kwargs.get("resume_dir") is not None and not nested_charter:
                state_path = Path(str(kwargs["resume_dir"])) / "teracontinuum_state.json"
                if state_path.is_file():
                    st = json.loads(state_path.read_text(encoding="utf-8"))
                    nested_charter = tt.normalize_teracontinuum_charter(st.get("charter"))
                    mid = str(st.get("teracontinuum_id") or mid)

            goal = str(kwargs.get("teracontinuum_goal") or "all_gigacontinuums_met")
            ...
            include_deferred = st["calls"] > 1 or goal == "all_gigacontinuums_met"
            ...
            gigacontinuum_states = [
                {
                    "gigacontinuum_id": str(
                        s.get("gigacontinuum_id") or s.get("id") or "g"
                    ),
            ...
            receipt = {
                "schema_version": 1,
                "ok": True,
                "verdict": "teracontinuum_met" if report_met else "teracontinuum_completed",
                "stop_reason": "teracontinuum_met" if report_met else "dispatch_budget",
                "teracontinuum_id": mid,
                "teracontinuum_goal": str(
                    kwargs.get("teracontinuum_goal") or "all_gigacontinuums_met"
                ),
                "max_rounds": int(kwargs.get("max_rounds") or 6),
                "max_active_gigacontinuums": kwargs.get("max_active_gigacontinuums"),
                "dispatch_budget": kwargs.get("dispatch_budget"),
                "teracontinuum_met": report_met,
                "total_dispatched": st["dispatched"],
                "total_dispatched_ok": st["dispatched_ok"],
                "gigacontinuums": [],
                "gigacontinuum_digests": [],
                "gigacontinuums_admitted": max(1, len(nested_charter) or 1),
                "gigacontinuums_met_count": (
            ...
            receipt["teracontinuum_digest"] = _sha256_json(
                tt._teracontinuum_digest_payload(receipt)
            )
            digest = receipt["teracontinuum_digest"]
            atomic_write_json(out / "teracontinuum.json", receipt)
            atomic_write_json(
                out / "teracontinuum_state.json",
                {
                    "teracontinuum_id": mid,
                    "round_count": st["calls"],
                    "total_dispatched": st["dispatched"],
            ...
            return {
                ...
                "verdict": receipt["verdict"],
                "stop_reason": receipt["stop_reason"],
                "teracontinuum_dir": str(out),
                "teracontinuum_digest": digest,
                "teracontinuum_id": mid,
                "total_dispatched": st["dispatched"],
                "total_dispatched_ok": st["dispatched_ok"],
                "teracontinuum_met": report_met,
        ...
        confed = run_petacontinuum(
            charter=charter,
            max_rounds=8,
            dispatch_budget=12,
            dispatch=True,
            teracontinuum_runner=_fast_mega_runner,
            petacontinuum_goal="all_teracontinuums_met",
            out_root=scratch / "m",
        )
        multi_petacontinuum_ok = (
            confed["ok"]
            and confed["petacontinuum_met"] is True
            and confed["stop_reason"] == "petacontinuum_met"
            and confed["teracontinuums_admitted"] == 2
            and confed["teracontinuums_met_count"] == 2
            and confed["round_count"] >= 2
            and confed["total_dispatched_ok"] >= 3
            and float((confed.get("coverage_end") or {}).get("coverage_ratio") or 0) == 1.0
        )
        scheduled_ids = {
            i.get("teracontinuum_id")
            for i in (confed.get("teracontinuums") or [])
        }
        multi_petacontinuum_scheduled = multi_petacontinuum_ok and scheduled_ids >= {"a", "b"}

        verified = verify_petacontinuum_receipt(Path(confed["petacontinuum_dir"]))
        seal_ok = bool(verified.get("ok")) and verified.get("round_count") == confed["round_count"]

        confed_path = Path(confed["petacontinuum_dir"]) / "petacontinuum.json"
        receipt = json.loads(confed_path.read_text(encoding="utf-8"))
        receipt["petacontinuum_digest"] = "0" * 64
        confed_path.write_text(json.dumps(receipt, indent=2), encoding="utf-8")
        tampered = verify_petacontinuum_receipt(Path(confed["petacontinuum_dir"]))
        tamper_detected = (
            not tampered["ok"]
            and "petacontinuum_digest" in (tampered.get("mismatched") or [])
        )

        # Budget stop
        _progress.clear()
        budgeted = run_petacontinuum(
            charter=[
        ...
        def _premet_runner(**kwargs: Any) -> dict[str, Any]:
            ...
            receipt = {
                ...
                "teracontinuum_id": kwargs.get("teracontinuum_id"),
                "teracontinuum_goal": "all_gigacontinuums_met",
                "max_rounds": 1,
                "max_active_gigacontinuums": None,
                ...
                "gigacontinuum_digests": [],
                "gigacontinuums_admitted": max(1, len(nested_charter) or 1),
                "gigacontinuums_met_count": max(1, len(nested_charter) or 1),
                "admission_count": 0,
                "pending_remaining": [],
                "charter_expansion_count": 0,
                "charter_expanded_ids": [],
            ...
            atomic_write_json(
                out / "teracontinuum_state.json",
                {
                    ...
                    "total_dispatched_ok": 0,
                    "federated_portfolio": portfolio,
                    "gigacontinuum_states": receipt["gigacontinuum_states"],
                    "stop_reason": "teracontinuum_met",
                    "charter": nested_charter,
                },
            )
        ...
        pre_met = run_petacontinuum(
            charter=[_slot("om", initial=[("om", "9.0.0", "omega-merged")])],
            max_rounds=3,
            dispatch=True,
            teracontinuum_runner=_premet_runner,
            petacontinuum_goal="all_teracontinuums_met",
            out_root=scratch / "p",
        )
        premet_ok = (
            pre_met["ok"]
            and pre_met["petacontinuum_met"] is True
            and pre_met["stop_reason"] == "petacontinuum_met"
            and pre_met["teracontinuums_met_count"] == 1
            and pre_met["total_dispatched"] == 0
        )

        ranked = run_petacontinuum(
            charter=[
        ...
        rank_only_ok = (
            ranked["ok"]
            and ranked["verdict"] == "petacontinuum_ranked"
        ...
        except PetacontinuumRefused as exc:
        ...
        partial = run_petacontinuum(
            ...
            teracontinuum_runner=_fast_mega_runner,
            petacontinuum_goal="none",
            petacontinuum_id="rcp",
            out_root=scratch / "a",
        )
        state_path = Path(partial["petacontinuum_dir"]) / "petacontinuum_state.json"
        state_exists = state_path.is_file()
        resumed = run_petacontinuum(
            resume_dir=Path(partial["petacontinuum_dir"]),
            max_rounds=4,
            dispatch_budget=4,
            dispatch=True,
            teracontinuum_runner=_fast_mega_runner,
        ...
        resume_ok = (
            ...
            and resumed["petacontinuum_id"] == "rcp"
        ...
        admit_ids = [a.get("teracontinuum_id") for a in (deferred.get("admissions") or [])]
        deferred_ok = (
            deferred["ok"]
            and deferred["petacontinuum_met"] is True
            and deferred["teracontinuums_admitted"] == 3
            and deferred["teracontinuums_met_count"] == 3
            and deferred.get("max_active_teracontinuums") == 1
        ...
        expand_ok = (
            expanded["ok"]
            and expanded["petacontinuum_met"] is True
            and expanded["teracontinuums_admitted"] == 2
            and expanded["teracontinuums_met_count"] == 2
        ...
        merged = merge_petacontinuum_charter(
        ...
        merge_ok = [s["teracontinuum_id"] for s in merged] == ["m1", "m2"]

        # Terminal coverage with premature-met + deferred reopen (ids start with t)
        _progress.clear()
        terminal = run_petacontinuum(
            charter=[
        ...
        terminal_ok = (
            terminal["ok"]
            and terminal["petacontinuum_met"] is True
            and terminal["stop_reason"] == "petacontinuum_met"
            ...
            and terminal["teracontinuums_admitted"] == 2
        ...
        return {
            "ok": ok,
            "petacontinuum_met": multi_petacontinuum_ok,
            "multi_petacontinuum_progressed": multi_petacontinuum_scheduled,
            ...
            "roi_scored": roi_ok,
            "petacontinuum_digest": confed.get("petacontinuum_digest"),
            "round_count": confed.get("round_count"),
            "total_dispatched_ok": confed.get("total_dispatched_ok"),
            "teracontinuums_admitted": confed.get("teracontinuums_admitted"),
            "used_skill_route_discovery": legacy_pipeline_was_used(),
            "flags": {
                "multi_petacontinuum_ok": multi_petacontinuum_ok,
                "multi_petacontinuum_scheduled": multi_petacontinuum_scheduled,
                "seal_ok": seal_ok,
                "tamper_detected": tamper_detected,
                "budget_ok": budget_ok,
                "premet_ok": premet_ok,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.verify:
        result = verify_petacontinuum_receipt(Path(args.verify))
        print(json.dumps(result, indent=2, sort_keys=True))
        return 0 if result.get("ok") else 1
    if args.proof:
        result = builtin_upstream_petacontinuum_proof()
        print(json.dumps(result, indent=2, sort_keys=True))
        return 0 if result.get("ok") else 1
    parser.print_help()
    return 2
