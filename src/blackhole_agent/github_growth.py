...
def attach_evolution_route_surface(digest: dict[str, Any]) -> dict[str, Any]:
    """Attach the operator-visible evolution route surface for this digest.

    The legacy skill_route_discovery lane pipeline is gone. The only surface a
    digest can emit now is the compact Capability Compounder redirect: it
    reports ledger readiness and points the supervisor next action at
    capability prove/compose. When the ledger is not ready the same compact
    surface reports ``not_ready`` instead of building skill-route lanes.
    """

    from blackhole_agent.evolution_route import (
        build_skill_route_compounder_redirect_pipeline,
        ledger_compound_ready,
    )
    ...
    surface = build_skill_route_compounder_redirect_pipeline(
    ...
    if not ledger_compound_ready(repo_path)["ready"]:
        surface["status"] = "ledger_not_ready"
        surface["supervisor_next_action"] = "grow_capability_ledger_first"
        surface["supervisor_next"] = "grow_capability_ledger_first"
    digest["evolution_route"] = surface
    digest["evolution_surface"] = surface.get("evolution_surface")
    digest["supervisor_next_action"] = surface.get("supervisor_next_action")
    return digest
...
def render_markdown_digest(digest: dict[str, Any]) -> str:
    ...
    evolution_route = digest.get("evolution_route")
    if isinstance(evolution_route, dict) and evolution_route:
        lines.extend(["", "## Evolution Route", ""])
        selected = (
            evolution_route.get("selected_step")
            if isinstance(evolution_route.get("selected_step"), dict)
            else {}
        )
        compounder = (
            evolution_route.get("capability_compounder")
            if isinstance(evolution_route.get("capability_compounder"), dict)
            else {}
        )
        lines.extend(
            [
                f"- Status: `{evolution_route.get('status', '')}`",
                f"- Evolution surface: `{evolution_route.get('evolution_surface', '')}`",
                f"- Policy: `{evolution_route.get('policy', '')}`",
                f"- Selected proposal: `{selected.get('proposal_id') or 'none'}`",
                f"- Route class: `{selected.get('route_class') or 'none'}`",
                f"- Runtime action: `{selected.get('runtime_action') or 'none'}`",
                f"- Capability ledger: `{compounder.get('status') or 'unknown'}`"
                f" ({compounder.get('proved_count') or 0}/{compounder.get('capability_count') or 0} proved)",
                f"- Supervisor next action: `{evolution_route.get('supervisor_next_action', 'none')}`",
            ]
        )
    return "\n".join(lines) + "\n"
