...
def builtin_browser_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="browser-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, BROWSER_ACTUATION_GOAL, BROWSER_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_browser"] = (
        not gate.accepted
        and live_goal != BROWSER_ACTUATION_GOAL
        and BROWSER_ACTUATION_ID not in live_done
