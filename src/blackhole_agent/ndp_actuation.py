...
def builtin_ndp_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="ndp-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, NDP_ACTUATION_GOAL, NDP_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_ndp"] = (
        not gate.accepted
        and live_goal != NDP_ACTUATION_GOAL
        and NDP_ACTUATION_ID not in live_done
