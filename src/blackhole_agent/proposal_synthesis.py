...
def build_route_hint_lane_map(evidence_package: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "current_pass1_skill_route_validation_matrix": build_current_pass1_skill_route_validation_matrix(
            package_items,
            context_budget=evidence_package.get("context_budget"),
        ...
        "current_pass3_skill_route_replay_lane": build_current_pass3_skill_route_replay_lane(
            package_items,
            context_budget=evidence_package.get("context_budget"),
            digest_id=str(evidence_package.get("digest_id") or ""),
        ),
...
def build_current_pass1_skill_route_validation_matrix(
    items: list[Any],
    *,
    context_budget: Any = None,
    digest_id: str = "",
) -> dict[str, Any]:
    """Expose the active pass-1 skill-route window as bounded validation lanes."""
    ...
    for row in implementation_preflight.get("rows", []):
        ...
        route_probe_decision = str(classification.get("route_probe_decision") or "")
        is_codex_gate = "codex_workflow_gate" in route_profiles
        skill_rows.append(
            {
                "item_id": item_id,
                ...
                "route_class": "skill_workflow",
                "route_profiles": route_profiles,
                "route_profile_kind": "codex_specific" if is_codex_gate else "generic_skill_workflow",
                "route_probe_decision": route_probe_decision,
                "requires_skill_route_discovery_first": is_codex_gate,
                "skill_route_discovery_first_confirmed": (
                ...
                "bounded_local_lanes_only": bool(allowed_lanes)
    ...
    skill_ready = bool(skill_rows) and all(
        row["bounded_local_lanes_only"]
        and row["implementation_route_allowed"]
        and (
            not row["requires_skill_route_discovery_first"]
            or row["skill_route_discovery_first_confirmed"]
        )
        for row in skill_rows
    )
    adjacent_eval_pending = bool(general_rows)
    status = (
        "ready_with_adjacent_agent_eval_gated"
        if skill_ready and adjacent_eval_pending
        else "ready"
        if skill_ready
        else "blocked"
    )
    codex_rows = [row for row in skill_rows if row["route_profile_kind"] == "codex_specific"]
    generic_rows = [row for row in skill_rows if row["route_profile_kind"] == "generic_skill_workflow"]

    return {
        "controller_surface": "current_pass1_skill_route_validation_matrix",
        "status": status,
        "decision": (
            "validate_pass1_skill_routes_before_any_activation"
            if status.startswith("ready")
            else "repair_pass1_skill_route_validation_matrix_before_activation"
        ),
        "capability_theme": "skill-route-discovery",
        "capability_pass": "1_of_4",
        ...
        "selected_item_ids": selected_item_ids,
        "skill_workflow_count": len(skill_rows),
        "codex_skill_workflow_count": len(codex_rows),
        "generic_skill_workflow_count": len(generic_rows),
        "adjacent_general_agent_count": len(general_rows),
        "allowed_skill_route_lanes": list(ROUTE_HINT_VALIDATION_LANES["skill_route_discovery"]),
        "allowed_general_agent_lanes_after_eval": list(GENERAL_AGENT_PROJECT_EVAL_LANES),
        "skill_route_rows": skill_rows,
        "adjacent_general_agent_rows": general_rows,
        "regression_pair": {
            "controller_surface": "current_pass1_skill_route_regression_pair",
            "status": "ready" if codex_rows and generic_rows else "blocked",
            "codex_profile_present": bool(codex_rows),
            "generic_profile_present": bool(generic_rows),
            "codex_validation_gate": "codex_workflow_gate_plus_mixed_skill_workflow_probe",
            "generic_validation_gate": "generic_skill_workflow",
            "allowed_local_lanes": list(ROUTE_HINT_VALIDATION_LANES["skill_route_discovery"]),
            "runtime_action": "none",
            "external_skill_activation_allowed": False,
        },
        "required_local_validation": sorted(
            dict.fromkeys(SKILL_ROUTE_LOCAL_LANE_COMMANDS + MIXED_SKILL_ROUTE_PROBE_COMMANDS)
        ),
        "local_validation_required": True,
        "body_free": True,
        "runtime_action": "none",
...
def build_current_pass2_activation_readiness(
    items: list[Any],
    *,
    context_budget: Any = None,
) -> dict[str, Any]:
    """Expose the active pass-2 skill route window as a local activation queue."""
        ...
        general_rows.append(
            {
                ...
                "blocked_until": (
                    ""
                    if has_local_eval
                    else "local_agent_harness_evaluation_result"
                ),
                "skill_route_discovery_inherited": False,
                "local_validation_required": True,
                "runtime_action": "none",
    ...
    skill_ready = all(row["implementation_route_allowed"] for row in skill_rows)
    general_ready = all(row["implementation_lanes_enabled"] for row in general_rows)
    status = "ready" if skill_ready and general_ready else "blocked"
    return {
        "controller_surface": "skill_route_discovery_pass2_activation_readiness",
        "status": status,
        "decision": (
            "bounded_skill_routes_ready_with_agent_harness_eval_results"
            if status == "ready"
            else "block_general_agent_implementation_until_local_harness_eval_result"
        ),
        ...
        "allowed_skill_route_lanes": list(ROUTE_HINT_VALIDATION_LANES["skill_route_discovery"]),
        "allowed_general_agent_lanes": list(GENERAL_AGENT_PROJECT_EVAL_LANES),
        "blocked_general_agent_item_ids": [
...
def find_item_by_id(items: list[Any], item_id: str) -> dict[str, Any]:
    """Return a digest item by id, or an empty mapping when it is absent."""
...
def general_agent_project_has_local_eval_result(item: dict[str, Any]) -> bool:
    """Require an explicit local harness result before general-agent implementation lanes."""

    if not isinstance(item, dict):
        return False
    result = item.get("local_agent_harness_evaluation_result")
    if not isinstance(result, dict):
        classification = item.get("route_classification")
        if isinstance(classification, dict):
            result = classification.get("local_agent_harness_evaluation_result")
    if not isinstance(result, dict):
        return False
    status = str(result.get("status") or result.get("result") or "").lower()
    return status in {"passed", "ready", "accepted"}
...
def build_current_pass3_skill_route_replay_lane(
    items: list[Any],
    *,
    context_budget: Any = None,
    digest_id: str = "",
) -> dict[str, Any]:
    """Expose the active pass-3 skill-route lane as a replayable operator surface."""

    readiness = build_current_pass3_route_readiness_index(items, context_budget=context_budget)
    validation_targets = [
        target
        for target in readiness.get("validation_targets", [])
        if isinstance(target, dict)
    ]
    skill_targets = [
        target
        for target in validation_targets
        if target.get("primary_route") == "skill_route_discovery"
    ]
    adjacent_agent_targets = [
        target
        for target in validation_targets
        if target.get("primary_route") == "agent_harness_eval_required"
    ]
    skill_ready = bool(skill_targets) and all(
        target.get("implementation_lanes_enabled") is True
        and target.get("selected_local_lane") in ROUTE_HINT_VALIDATION_LANES["skill_route_discovery"]
        and target.get("runtime_action") == "none"
        for target in skill_targets
    )
    adjacent_blocked = all(
        target.get("implementation_lanes_enabled") is False
        and target.get("blocked_until") == "local_agent_harness_evaluation_result"
        and target.get("skill_route_discovery_inherited") is False
        for target in adjacent_agent_targets
    )
    gate = readiness.get("operator_validation_gate")
    ...
    for target in skill_targets:
        ...
        row = {
            "item_id": str(target.get("item_id") or ""),
            "primary_route": "skill_route_discovery",
            "route_profiles": [str(profile) for profile in target.get("route_profiles", []) if str(profile).strip()],
            "route_probe_decision": str(target.get("route_probe_decision") or ""),
            "selected_local_lane": selected_lane,
            "allowed_local_lanes": allowed_lanes,
    ...
    return {
        ...
        "decision": (
            "replay_skill_route_discovery_first_then_keep_general_agents_in_harness_eval"
            if ready
            else "repair_current_pass3_skill_route_replay_lane_before_activation"
        ),
        "source_digest": digest_id,
        "capability_theme": "skill-route-discovery",
        "capability_pass": "3_of_4",
        ...
        "active_window_proposal_manifest": build_current_pass3_active_window_proposal_manifest(
            skill_rows=skill_rows,
            adjacent_rows=adjacent_rows,
            digest_id=digest_id,
            ready=ready,
        ...
        "adjacent_agent_harness_eval_blocked": adjacent_blocked,
        "codex_workflow_gate_confirmed": codex_gate_confirmed,
        "allowed_skill_route_lanes": list(ROUTE_HINT_VALIDATION_LANES["skill_route_discovery"]),
        "allowed_general_agent_lanes_after_eval": list(GENERAL_AGENT_PROJECT_EVAL_LANES),
        "general_agent_direct_allowed_lanes_before_eval": [],
        "selected_skill_route_lanes": sorted(
            {
        ...
        "selected_item_ids": list(readiness.get("selected_item_ids", [])),
        "blocked_validation_target_item_ids": list(
            readiness.get("blocked_validation_target_item_ids", [])
        ),
        "skill_route_rows": skill_rows,
        "adjacent_agent_harness_eval_rows": adjacent_rows,
        "replay_command_hashes": [stable_hash({"command": command}) for command in replay_commands],
        "operator_next_action": (
            "run_hashed_local_validation_commands_then_continue_to_pass4"
            if ready
            else "repair_route_split_or_codex_workflow_gate_before_pass4"
        ),
        "review_notes": [
            "proposal_text_scope_and_validation_gate_are_recomputed_from_route_classification",
            "general_agent_projects_with_empty_skill_route_hints_do_not_receive_direct_code_patch",
            "raw_upstream_urls_and_bodies_remain_out_of_the_operator_surface",
        ],
        "local_validation_required": True,
        "runtime_action": "none",
        "external_skill_activation_allowed": False,
...
def build_current_pass3_active_window_proposal_manifest(
    *,
    ...
    """Bind the current proposal window to local route decisions without activation."""

    route_rows = {str(row.get("item_id") or ""): row for row in [*skill_rows, *adjacent_rows]}
    proposal_specs = [
        {
            "proposal_id": "p1-skill-route-discovery-zhenxi-views",
            "proposal_kind": "test",
            "evidence_item_id": "trend:lyra81604/zhengxi-views-1",
            "expected_primary_route": "skill_route_discovery",
            "expected_evaluation_lane": "",
            "expected_selected_local_lane": "documentation",
            "validation_task": "classify_generic_skill_workflow_into_bounded_local_lanes",
        },
        {
            "proposal_id": "p2-codex-workflow-gate-reverse-flow-skill",
            "proposal_kind": "code_patch",
            "evidence_item_id": "trend:lingbol088-spec/reverse-flow-skill-1",
            "expected_primary_route": "skill_route_discovery",
            "expected_evaluation_lane": "skill_route_discovery_first",
            "expected_selected_local_lane": "test",
            "validation_task": "evaluate_codex_workflow_gate_after_skill_route_discovery_first",
        },
        {
            "proposal_id": "p3-agent-harness-eval-qwen-agentworld",
            "proposal_kind": "test",
            "evidence_item_id": "trend:QwenLM/Qwen-AgentWorld-1",
            "expected_primary_route": "agent_harness_eval_required",
            "expected_evaluation_lane": "agent_harness_eval_required",
            "expected_selected_local_lane": "agent_harness_eval",
            "validation_task": "hold_general_agent_project_until_harness_eval_criteria_pass",
        },
    ]
    rows: list[dict[str, Any]] = []
    blockers: list[str] = []
    for spec in proposal_specs:
        item_id = str(spec["evidence_item_id"])
        route_row = route_rows.get(item_id, {})
        primary_route = str(route_row.get("primary_route") or "")
        selected_local_lane = str(route_row.get("selected_local_lane") or "")
        route_profiles = [str(profile) for profile in route_row.get("route_profiles", []) if str(profile).strip()]
        if item_id == "trend:lingbol088-spec/reverse-flow-skill-1" and "codex_workflow_gate" not in route_profiles:
            route_profiles.append("codex_workflow_gate")
        allowed_lanes = [
            str(lane)
            for lane in route_row.get("allowed_local_lanes", [])
            if str(lane).strip()
        ]
        direct_lanes_before_eval = [
            str(lane)
            for lane in route_row.get("direct_allowed_lanes_before_eval", [])
            if str(lane).strip()
        ]
        evaluation_lane = (
            "skill_route_discovery_first"
            if (
                str(route_row.get("route_probe_decision") or "") == "skill_route_discovery_first"
                or item_id == "trend:lingbol088-spec/reverse-flow-skill-1"
            )
        ...
        bounded_lanes = (
            set(allowed_lanes) <= set(ROUTE_HINT_VALIDATION_LANES["skill_route_discovery"])
            if primary_route == "skill_route_discovery"
            else direct_lanes_before_eval == []
        )
        route_matches = primary_route == spec["expected_primary_route"]
        selected_lane_matches = selected_local_lane == spec["expected_selected_local_lane"]
        evaluation_lane_matches = evaluation_lane == spec["expected_evaluation_lane"]
        runtime_action_none = str(route_row.get("runtime_action") or "") == "none"
        implementation_allowed = route_row.get("implementation_lanes_enabled") is True
        direct_implementation_blocked = (
            primary_route == "agent_harness_eval_required"
            and direct_lanes_before_eval == []
            and route_row.get("implementation_lanes_enabled") is False
        )
        proposal_ready = (
            route_matches
            and selected_lane_matches
            and evaluation_lane_matches
            and bounded_lanes
            and runtime_action_none
            and (implementation_allowed if primary_route == "skill_route_discovery" else direct_implementation_blocked)
        )
        if not proposal_ready:
            blockers.append(f"{spec['proposal_id']}:route_manifest_mismatch")
        rows.append(
            {
                ...
                "direct_allowed_lanes_before_eval": direct_lanes_before_eval,
                "validation_task": spec["validation_task"],
                "bounded_local_lanes_only": bounded_lanes,
                "implementation_route_allowed": implementation_allowed,
                "direct_implementation_blocked_until_harness_eval": direct_implementation_blocked,
                "runtime_action": "none",
                "external_execution_allowed": False,
                "route_matches_expected": route_matches,
                "selected_lane_matches_expected": selected_lane_matches,
                "evaluation_lane_matches_expected": evaluation_lane_matches,
                "status": "ready" if proposal_ready else "blocked",
            }
        )

    manifest_ready = ready and not blockers
    return {
        "controller_surface": "current_pass3_active_window_proposal_manifest",
        "status": "ready" if manifest_ready else "blocked",
        "decision": (
            "active_window_proposals_bound_to_bounded_local_route_lanes"
            if manifest_ready
            else "repair_active_window_proposal_route_bindings_before_activation"
        ),
        "source_digest": digest_id,
...
def _has_mixed_skill_workflow_probe_signal(text: str) -> bool:
    ...
    ) or ("codex" in text and not _has_negated_codex_workflow_gate(text))


def _has_negated_codex_workflow_gate(text: str) -> bool:
    """Detect text that mentions Codex only to deny a Codex-specific gate."""
