...
def build_route_hint_policy_preflight(evidence_package: dict[str, Any]) -> dict[str, Any]:
    ...
    provider_lanes = lane_map["validation_lanes"].get("provider_config_preflight", [])
    return {
        "ok": not diagnostics,
        ...
        "provider_config_preflight_lanes": provider_lanes,
        "allowed_provider_config_preflight_lanes": list(ROUTE_HINT_VALIDATION_LANES["provider_config_preflight"]),
...
def build_route_hint_lane_map(evidence_package: dict[str, Any]) -> dict[str, Any]:
    ...
    expected_policy_lanes = {
        ...
        if hint in selected_route_hints
    }
...
def classify_digest_item_route(item: dict[str, Any]) -> dict[str, Any]:
    ...
    The legacy skill_route_discovery / agent_harness_eval lane classes are gone:
    classification now only records which bounded route hints (provider config,
    governance policy) the item text suggests. Everything else is unclassified
    trend evidence with no inherited lane.
    """
            ...
            "allowed_lanes": sorted(
                {
                    ...
                    for lane in ROUTE_HINT_VALIDATION_LANES.get(hint, [])
                }
            ),
            "reasons": ["route_hint_signal"],
            "runtime_action": "none",
            "local_validation_required": True,
        }
    return {
...
def render_proposal_synthesis_prompt(evidence_package: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            "Use item.route_hints when present to propose bounded local validation lanes; route hints may map only to documentation, config, test, or code_patch work.",
            "",
...
def candidate_route_hint_lane_errors(
    ...
    errors: list[str] = []
    enforced_route_hints = {"governance_policy"}
