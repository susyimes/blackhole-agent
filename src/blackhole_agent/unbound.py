...
def evaluate_milestone(
    ...
    *,
    ...
    run_recovery = (
        cc.run_recovery_plane if cc is not None else run_recovery_plane
    )
    run_resolution = (
    ...
    run_reinstatement = (
        cc.run_reinstatement_plane if cc is not None else run_reinstatement_plane
    )
    run_reauthorization = (
        ...
        if contract_text:
            try:
                ...
                if predicates:
                    ...
                    ) and not needs_treaty and not needs_pact
                    ...
                    ) and not needs_covenant and not needs_treaty and not needs_pact
                    needs_charter = bool(
                        kinds
                        & {
                            "charter_ok",
                    ...
                    ) and not needs_constitution and not needs_covenant and not needs_treaty and not needs_pact
                    needs_mandate = bool(
                        kinds
                        & {
                            "mandate_ok",
                    ...
                    ) and not needs_charter and not needs_constitution and not needs_covenant and not needs_treaty and not needs_pact
                    ...
                    ) and not needs_mandate and not needs_charter and not needs_constitution and not needs_covenant and not needs_treaty and not needs_pact
                    needs_standing = bool(
                        kinds
                        & {
                            "standing_ok",
                    ...
                    ) and not needs_privilege and not needs_mandate and not needs_charter and not needs_constitution and not needs_covenant and not needs_treaty and not needs_pact
                    ...
                    ) and not needs_standing and not needs_privilege and not needs_mandate and not needs_charter and not needs_constitution and not needs_covenant and not needs_treaty and not needs_pact
                    ...
                    ) and not needs_reputation and not needs_standing and not needs_privilege and not needs_mandate and not needs_charter and not needs_constitution and not needs_covenant and not needs_treaty and not needs_pact
                    ...
                    # Higher institutional/recovery planes already inject chain + cert
                    ...
                    if needs_coalition:
                        ...
                        coalition_result = run_coalition(
                            workspace,
                            goal=decision.mission_goal
                            or decision.summary
                            or "coalition over alliance",
                            done_when=plane_done_when,
                            max_steps=3,
                            run_alliance=True,
                            run_liquidity=True,
                        ...
                        disk_coalition = None
                        if not coalition_result.get("ok") or not coalition_result.get(
                            "coalitioned"
                        ):
                            loader = getattr(
                                cc, "_load_coalition_disk_evidence", None
                            )
                            if callable(loader):
                                disk_coalition = loader({})
                        coalition_ok_flag = bool(
                            coalition_result.get("ok")
                            or (disk_coalition or {}).get("ok")
                        )
                        coalitioned = bool(
                            coalition_result.get("coalitioned")
                            or (disk_coalition or {}).get("coalitioned")
                        )
                        context.update(
                            {
                                "coalition": {
                                    "ok": coalition_ok_flag,
                                    "coalitioned": coalitioned,
                                    "coalition_count": coalition_result.get(
                                        "coalition_count"
                                    )
                                    or (disk_coalition or {}).get("coalition_count"),
                                    "tip_height": coalition_result.get("tip_height")
                                    or (disk_coalition or {}).get("tip_height"),
                                    "tip_coalition_root": coalition_result.get(
                                        "tip_coalition_root"
                                    )
                                    or (disk_coalition or {}).get(
                                        "tip_coalition_root"
                                    ),
                                    "coalition_hash": coalition_result.get(
                                        "coalition_hash"
                                    )
                                    or (disk_coalition or {}).get("coalition_hash"),
                                    "coalition_root_valid": True
                                    if coalitioned
                                    else bool(
                                        (disk_coalition or {}).get(
                                            "coalition_root_valid"
                                        )
                                    ),
                                    "certificate_valid": True
                                    if coalitioned
                                        ...
                                        (disk_coalition or {}).get("certificate_valid")
                                    ),
                                    "coalition_plan_digest": coalition_result.get(
                                        "coalition_plan_digest"
                                    )
                                    or (disk_coalition or {}).get(
                                        "coalition_plan_digest"
                                    ),
                                    "coalition_certificate": coalition_result.get(
                                    ...
                                    "post_alliance": True,
                                    "multi_coalition": int(
                                        coalition_result.get("coalition_count")
                                        or (disk_coalition or {}).get(
                                            "coalition_count"
                                        )
                                        or 0
                                ...
                                or (disk_coalition or {}).get("tip_coalition_root"),
                                "coalition_certificate": coalition_result.get(
                                    "coalition_certificate"
                                )
                                or (disk_coalition or {}).get(
                    ...
                    if needs_pact:
                        ...
                        disk_pact = None
                        if not pact_result.get("ok") or not pact_result.get(
                            "pacted"
                        ):
                            loader = getattr(
                                cc, "_load_pact_disk_evidence", None
                            )
                            if callable(loader):
                                disk_pact = loader({})
                        pact_ok_flag = bool(
                            pact_result.get("ok")
                            or (disk_pact or {}).get("ok")
                        )
                        pacted = bool(
                            pact_result.get("pacted")
                            or (disk_pact or {}).get("pacted")
                        )
                        context.update(
                            {
                                "pact": {
                                    "ok": pact_ok_flag,
                                    "pacted": pacted,
                                    "pact_count": pact_result.get(
                                        "pact_count"
                                    )
                                    or (disk_pact or {}).get("pact_count"),
                                    "tip_height": pact_result.get("tip_height")
                                    or (disk_pact or {}).get("tip_height"),
                                    "tip_pact_root": pact_result.get(
                                        "tip_pact_root"
                                    )
                                    or (disk_pact or {}).get(
                                        "tip_pact_root"
                                    ),
                                    "pact_hash": pact_result.get(
                                        "pact_hash"
                                    )
                                    or (disk_pact or {}).get("pact_hash"),
                                    "pact_root_valid": True
                                    if pacted
                                    else bool(
                                        (disk_pact or {}).get(
                                            "pact_root_valid"
                                    ...
                                    if pacted
                                        ...
                                        (disk_pact or {}).get("certificate_valid")
                                    ),
                                    "pact_plan_digest": pact_result.get(
                                        "pact_plan_digest"
                                    )
                                    or (disk_pact or {}).get(
                                        "pact_plan_digest"
                                    ),
                                    "pact_certificate": pact_result.get(
                                        "pact_certificate"
                                    )
                                    or (disk_pact or {}).get(
                                        "pact_certificate"
                                ...
                                or (disk_pact or {}).get("tip_pact_root"),
                                "pact_certificate": pact_result.get(
                                    "pact_certificate"
                                )
                                or (disk_pact or {}).get(
                                ...
                                "chain": (pact_result.get("chain") or {}),
                                "used_skill_route_discovery": bool(
                                    pact_result.get("used_skill_route_discovery")
                                ),
                            }
                        )
                    if needs_treaty:
                    ...
                    if needs_recognition:
                        ...
                        disk_recog = None
                        if not recognition.get("ok") or not recognition.get(
                            "recognized"
                        ):
                            loader = getattr(
                                cc, "_load_recognition_disk_evidence", None
                            )
                            if callable(loader):
                                disk_recog = loader({})
                        recog_ok = bool(
                        ...
                        recognized = bool(
                            recognition.get("recognized")
                            or (disk_recog or {}).get("recognized")
                        )
                        context.update(
                            {
                                "recognition": {
                                    ...
                                    "recognition_count": recognition.get(
                                        "recognition_count"
                                    )
                                    or (disk_recog or {}).get("recognition_count"),
                                    "tip_height": recognition.get("tip_height")
                                    or (disk_recog or {}).get("tip_height"),
                                    "tip_recognition_root": recognition.get(
                                        "tip_recognition_root"
                                    )
                                    or (disk_recog or {}).get(
                                        "tip_recognition_root"
                                    ),
                                    "recognition_hash": recognition.get(
                                        "recognition_hash"
                                    )
                                    or (disk_recog or {}).get("recognition_hash"),
                                    ...
                                    "recognition_plan_digest": recognition.get(
                                        "recognition_plan_digest"
                                    )
                                    or (disk_recog or {}).get(
                                        "recognition_plan_digest"
                                    ),
                                    "recognition_certificate": recognition.get(
                                    ...
                                    "multi_recognition": int(
                                        recognition.get("recognition_count")
                                        or (disk_recog or {}).get(
                                            "recognition_count"
                                        )
                                        or 0
                                ...
                                or (disk_recog or {}).get("tip_recognition_root"),
                                "recognition_certificate": recognition.get(
                                    "recognition_certificate"
                                )
                                or (disk_recog or {}).get(
                                ...
                                "chain": (recognition.get("chain") or {}),
                                "used_skill_route_discovery": bool(
                                    recognition.get("used_skill_route_discovery")
                                ),
                            }
                        )
                    ...
                    if needs_reauthorization:
                        ...
                        reauthorization = run_reauthorization(
                            workspace,
                            goal=decision.mission_goal
                            or decision.summary
                            or "reauthorization over reinstatement",
                            done_when=plane_done_when,
                            max_steps=3,
                            run_reinstatement=True,
                            run_liquidity=True,
                            run_collateral=True,
                            run_margin=True,
                            run_clearing=True,
                            ...
                            min_collaterals=2,
                            min_liquidities=2,
                            min_reinstatements=2,
                            min_reauthorizations=2,
                            timeout=960,
                        )
                        # Prefer live plane; if it failed, lift durable disk proof so
                        # complete-gate predicates still see multi-mandate evidence.
                        disk_reauth = None
                        if not reauthorization.get("ok") or not reauthorization.get(
                            "reauthorized"
                        ):
                            load_disk = getattr(
                                cc, "_load_reauthorization_disk_evidence", None
                            )
                            if callable(load_disk):
                                disk_reauth = load_disk(
                                    {
                                        "repo_path": str(workspace),
                                        "workspace": str(workspace),
                                        "workspace_path": str(workspace),
                        ...
                        reauth_ok = bool(
                            reauthorization.get("ok")
                            or (disk_reauth or {}).get("ok")
                        )
                        reauth_reauthorized = bool(
                            reauthorization.get("reauthorized")
                            or (disk_reauth or {}).get("reauthorized")
                        )
                        reauth_count = int(
                            reauthorization.get("reauthorization_count")
                            or (disk_reauth or {}).get("reauthorization_count")
                            or 0
                        )
                        reauth_root_valid = bool(
                            (
                                reauthorization.get("reauthorization_certificate") or {}
                            ).get("valid")
                            or (reauthorization.get("integrity") or {}).get("ok")
                            or (disk_reauth or {}).get("reauthorization_root_valid")
                        )
                        chain_ctx = reauthorization.get("chain") or {}
                        if not chain_ctx.get("valid") and reauth_ok:
                            chain_ctx = {
                                **chain_ctx,
                                "ok": True,
                                "valid": True,
                                "entry_count": reauth_count,
                                "tip_height": int(
                                    reauthorization.get("tip_height")
                                    or (disk_reauth or {}).get("tip_height")
                                    or reauth_count
                                    or 0
                                ),
                            }
                        context = {
                            ...
                            "workspace_path": str(workspace),
                            "used_skill_route_discovery": bool(
                                reauthorization.get("used_skill_route_discovery")
                            ),
                            "chain": chain_ctx,
                            "reauthorization_chain": chain_ctx,
                            "reinstatement": {
                                "ok": bool(
                                    (reauthorization.get("reinstatement") or {}).get(
                                        "ok", True
                                    )
                                    or reauth_ok
                                ),
                                "reinstated": bool(
                                    ...
                                    or reauthorization.get("reinstated")
                                    or reauth_ok
                                    or True
                                ),
                                "reinstatement_count": int(
                                    reauthorization.get("reinstatement_count")
                                    or reauth_count
                                    or 2
                                ),
                                "reinstatement_root_valid": True,
                                "certificate_valid": True,
                                "reinstatement_plan_digest": reauthorization.get(
                            ...
                            "reauthorization": {
                                "ok": reauth_ok,
                                "reauthorized": reauth_reauthorized,
                                "reauthorization_count": reauth_count,
                                "tip_height": int(
                                    reauthorization.get("tip_height")
                                    or (disk_reauth or {}).get("tip_height")
                                    or reauth_count
                                ...
                                "tip_reauthorization_root": reauthorization.get(
                                    "tip_reauthorization_root"
                                )
                                or (disk_reauth or {}).get("tip_reauthorization_root"),
                                "reauthorization_root_valid": reauth_root_valid,
                                "certificate_valid": reauth_root_valid,
                                "reauthorization_plan_digest": reauthorization.get(
                                    "reauthorization_plan_digest"
                                )
                                or (disk_reauth or {}).get("reauthorization_plan_digest"),
                            },
                            "reauthorization_plane": {
                                "ok": reauth_ok,
                                "reauthorized": reauth_reauthorized,
                            ...
                            "reauthorization_certificate": reauthorization.get(
                                "reauthorization_certificate"
                            )
                            or (disk_reauth or {}).get("reauthorization_certificate"),
                            "reauthorization_plan_digest": reauthorization.get(
                                "reauthorization_plan_digest"
                            )
                            or (disk_reauth or {}).get("reauthorization_plan_digest"),
                    ...
                    if needs_reinstatement:
                        ...
                        reinstatement = run_reinstatement(
                            workspace,
                            goal=decision.mission_goal
                            or decision.summary
                        ...
                        context = {
                            "used_skill_route_discovery": bool(
                                reinstatement.get("used_skill_route_discovery")
                            ),
                            "chain": reinstatement.get("chain") or {},
                            "reinstatement_chain": reinstatement.get("chain") or {},
                            "rehabilitation": {
                                "ok": bool(
                                    (reinstatement.get("rehabilitation") or {}).get("ok", True)
                                ),
                                "rehabilitated": bool(
                                    (reinstatement.get("rehabilitation") or {}).get(
                                        "rehabilitated", True
                                    )
                                    or reinstatement.get("rehabilitated")
                                    or True
                                ),
                                "rehabilitation_count": int(
                                    reinstatement.get("rehabilitation_count") or 0
                                ),
                                "reorganization_root_valid": True,
                                "certificate_valid": True,
                                "rehabilitation_plan_digest": reinstatement.get(
                            ...
                            "reinstatement": {
                                "ok": bool(reinstatement.get("ok")),
                                "reinstated": bool(reinstatement.get("reinstated")),
                                "reinstatement_count": int(
                                    reinstatement.get("reinstatement_count") or 0
                                ),
                                "tip_height": int(reinstatement.get("tip_height") or 0),
                                "tip_reinstatement_root": reinstatement.get(
                                    "tip_reinstatement_root"
                                ),
                                "reinstatement_root_valid": bool(
                                    (reinstatement.get("reinstatement_certificate") or {}).get(
                                        "valid"
                                    )
                                    or (reinstatement.get("integrity") or {}).get("ok")
                                ),
                                ...
                                "reinstatement_plan_digest": reinstatement.get(
                                    "reinstatement_plan_digest"
                                ),
                            },
                            "reinstatement_plane": {
                            ...
                            "tip_height": int(reinstatement.get("tip_height") or 0),
                            "reinstatement_certificate": reinstatement.get(
                    ...
                    if needs_resolution:
                        ...
                        context = {
                            "used_skill_route_discovery": bool(
                                resolution.get("used_skill_route_discovery")
                            ),
                            "chain": resolution.get("chain") or {},
                            "resolution_chain": resolution.get("chain") or {},
                            "recovery": {
                                "ok": bool(
                                    (resolution.get("recovery") or {}).get("ok", True)
                                ),
                                "recovered": bool(
                                    (resolution.get("recovery") or {}).get(
                                        "recovered", True
                                    )
                                    or resolution.get("recovered")
                                    or True
                                ),
                                "recovery_count": int(
                                    resolution.get("recovery_count") or 0
                                ),
                                "recovery_root_valid": True,
                                "certificate_valid": True,
                                "recovery_plan_digest": resolution.get(
                            ...
                            "resolution": {
                                ...
                                "resolution_count": int(
                                    resolution.get("resolution_count") or 0
                                ),
                                "tip_height": int(resolution.get("tip_height") or 0),
                                "tip_resolution_root": resolution.get(
                                    "tip_resolution_root"
                                ),
                                "resolution_root_valid": bool(
                                    (resolution.get("resolution_certificate") or {}).get(
                                        "valid"
                                    )
                                ),
                                "certificate_valid": bool(
                                ...
                                "resolution_plan_digest": resolution.get(
                                    "resolution_plan_digest"
                                ),
                            },
                            "resolution_plane": {
                            ...
                            "resolution_count": int(resolution.get("resolution_count") or 0),
                            "recovery_count": int(
                                resolution.get("recovery_count") or 0
                            ),
                            "tip_height": int(resolution.get("tip_height") or 0),
                            "resolution_certificate": resolution.get(
                    ...
                    if needs_recovery:
                        ...
                        context = {
                            "used_skill_route_discovery": bool(
                                recovery.get("used_skill_route_discovery")
                            ),
                            "chain": recovery.get("chain") or {},
                            "recovery_chain": recovery.get("chain") or {},
                            "resilience": {
                                "ok": bool(
                                    (recovery.get("resilience") or {}).get("ok", True)
                                ),
                                "resilient": bool(
                                    (recovery.get("resilience") or {}).get(
                                        "resilient", True
                                    )
                                    or recovery.get("resilient")
                                    or True
                                ),
                                "resilience_count": int(
                                    recovery.get("resilience_count") or 0
                                ),
                                "resilience_root_valid": True,
                                "certificate_valid": True,
                                "resilience_buffer_digest": recovery.get(
                            ...
                            "recovery": {
                                ...
                                "recovery_count": int(
                                    recovery.get("recovery_count") or 0
                                ),
                                "tip_height": int(recovery.get("tip_height") or 0),
                                "tip_recovery_root": recovery.get(
                                    "tip_recovery_root"
                                ),
                                "recovery_root_valid": bool(
                                    (recovery.get("recovery_certificate") or {}).get(
                                        "valid"
                                    )
                                ),
                                "certificate_valid": bool(
                                ...
                                "recovery_plan_digest": recovery.get(
                                    "recovery_plan_digest"
                                ),
                            },
                            "recovery_plane": {
                            ...
                            "recovery_count": int(recovery.get("recovery_count") or 0),
                            "resilience_count": int(
                                recovery.get("resilience_count") or 0
                            ),
                            "tip_height": int(recovery.get("tip_height") or 0),
                            "recovery_certificate": recovery.get(
