"""Recovery plane for absorbed composition pipelines the base loop cannot heal.

The goal-directed recovery loop plans only ``APPLICATION_TASKS`` on the base
registry. After a typed key-bridge made independently absorbed tools
composable, a red producer still left the pipeline unplannable while default
recovery reported a healthy zero-repair pass.

This module closes that recovery failure without changing pre-growth
semantics of the base loop:

- default recovery still ignores absorbed composition goals;
- the grown registry (``include_absorbed=True``) is recovered for every
  persisted bridge pipeline;
- a stale-interpreter break of the producer is detected, repaired, and the
  composition goal re-solves with an oracle match;
- an unrepairable producer break leaves the composition unsolved and the
  stamp red;
- a digest-sealed report under ``artifacts/capability-absorbed-recovery/``
  whose verification recomputes every verdict from live recovery runs and
...
SCHEMA_VERSION = 1
ABSORBED_RECOVERY_ID = "capability.absorbed-recovery-plane"
REPO_ROOT = Path(__file__).resolve().parents[2]
DEFAULT_ARTIFACT_DIR = REPO_ROOT / "artifacts" / "capability-absorbed-recovery"
...
def composition_surface_ids() -> list[str]:
    """Ledger citizens on persisted absorbed composition pipelines."""
...
def composition_allowed_ids() -> list[str]:
    """Planner ids for absorbed recovery: endpoints plus typed key-bridges."""

    ids = composition_surface_ids()
    for pair in load_persisted_bridge_records():
        bridge_id = str(pair.get("bridge_id") or "")
        if bridge_id and bridge_id not in ids:
            ids.append(bridge_id)
    return ids


def _slug_for_absorbed(capability_id: str) -> str:
    return capability_id.removeprefix("capability.absorbed-")


def repair_absorbed_member(
    ledger,
    capability_id: str,
    *,
    cwd=None,
    ...
    """Heal an absorbed leaf in-process: re-prove cases, or fail closed.

    Shell ``uv run`` proof commands hang under Windows ``shell=True`` when the
    interpreter path is swapped. Absorbed leaves already have an in-process
    prover; using it keeps recovery bounded and still falsifies a replaced
    failing proof command.
    """
    ...
    result = prove_absorbed_capability(_slug_for_absorbed(capability_id))
    if result.get("ok"):
        ...
        return ledger, {
            ...
            "ok": True,
            "verdict": "repaired",
            "repair_actions": ["reprove_absorbed_cases"],
            "honest": True,
            "last_proof_exit_code": 0,
        }
    return ledger, {
...
def run_absorbed_recovery_loop(
    *,
    breaks: Mapping[str, str] | None = None,
    tasks: Sequence[ApplicationTask] | None = None,
) -> dict[str, Any]:
    """Goal-directed recovery over persisted absorbed composition pipelines."""

    task_list = list(tasks) if tasks is not None else load_absorbed_composition_tasks()
    return run_recovery_loop(
        breaks=breaks,
        tasks=task_list,
        include_absorbed=True,
        surface_ids=composition_surface_ids(),
        allowed_ids=composition_allowed_ids(),
        skip_proved_deps=True,
        timeout=30,
        repair_fn=repair_absorbed_member,
    )
...
def compute_recovery_verdicts(
    *,
    ...
    """Live isolation, heal, and honest-unrepairable verdicts."""
    ...
    live = run_absorbed_recovery_loop(tasks=absorbed_tasks) if absorbed_tasks else {
        "ok": False,
        "recovery": {},
        "task_records": [],
        "repairs": [],
    ...
    healed = (
        run_absorbed_recovery_loop(
            breaks={producer_id: BREAK_STALE_STAMP},
    ...
    unrepairable = (
        run_absorbed_recovery_loop(
            breaks={producer_id: BREAK_FAILING_PROOF},
            tasks=absorbed_tasks,
        )
        if producer_id and absorbed_tasks
        else live
    ...
    verdicts = {
        ...
        "live_absorbed_healthy": live_summary["ok"]
        and live_summary["repair_count"] == 0
        and bool(absorbed_ids)
        and sorted(live_summary["task_ids"]) == sorted(absorbed_ids),
        "producer_stale_healed": healed_summary["ok"]
        and composition_id in healed_summary["recovered"]
        and healed_summary["repair_verdicts"].get(producer_id) == "repaired"
        and healed_summary["break_stamps_after"].get(producer_id) == 0,
        "producer_unrepairable_honest": (not unrepairable_summary["ok"])
        and composition_id in unrepairable_summary["honest_unsolved"]
        and unrepairable_summary["repair_verdicts"].get(producer_id) == "unrepairable"
        and unrepairable_summary["break_stamps_after"].get(producer_id) not in (0, None),
...
def _summaries_match_verdicts(report: Mapping[str, Any]) -> dict[str, bool]:
    ...
    derived = {
        ...
        and isolation_ids.isdisjoint(absorbed_ids)
        and isolation_ids == base_ids,
        "live_absorbed_healthy": bool(live.get("ok"))
        and live.get("repair_count") == 0
        and bool(absorbed_ids)
        and sorted(live.get("task_ids") or []) == sorted(absorbed_ids),
        "producer_stale_healed": bool(healed.get("ok"))
        and composition_id in list(healed.get("recovered") or [])
        and (healed.get("repair_verdicts") or {}).get(producer_id) == "repaired"
        and (healed.get("break_stamps_after") or {}).get(producer_id) == 0,
        "producer_unrepairable_honest": (not bool(unrepairable.get("ok")))
        and composition_id in list(unrepairable.get("honest_unsolved") or [])
        and (unrepairable.get("repair_verdicts") or {}).get(producer_id) == "unrepairable"
        and (unrepairable.get("break_stamps_after") or {}).get(producer_id) not in (0, None),
...
def verify_absorbed_recovery_report(report_dir: Path) -> dict[str, Any]:
...
def absorbed_recovery_proof_command() -> str:
    return (
        "uv run python -c \"from blackhole_agent.capability_absorbed_recovery import "
        "builtin_absorbed_recovery_proof; "
        "r=builtin_absorbed_recovery_proof(); "
        "assert r['ok'] and r.get('verdicts',{}).get('base_isolation') "
        "and r.get('verdicts',{}).get('live_absorbed_healthy') "
        "and r.get('verdicts',{}).get('producer_stale_healed') "
        "and r.get('verdicts',{}).get('producer_unrepairable_honest') "
        "and r.get('verify_ok') and r.get('tamper_detected') "
        "and r.get('misgrade_detected') and not r.get('used_skill_route_discovery')\""
    )


def ensure_absorbed_recovery_capability(*, repo_path: Path | None = None) -> Capability:
    ...
    capability = Capability(
        id=ABSORBED_RECOVERY_ID,
        name="Absorbed composition recovery plane",
        description=(
            "Heals absorbed composition pipelines the base recovery loop cannot "
            "see: a red producer is repaired and the composition goal re-solves, "
            "an unrepairable producer stays red, and default recovery stays on "
            "base goals."
        ),
        kind="python",
        entry="blackhole_agent.capability_absorbed_recovery:builtin_absorbed_recovery_proof",
        proof_command=absorbed_recovery_proof_command(),
        dependencies=(
            "repo.import-health",
        ...
        capability_delta=(
            "Absorbed composition pipelines are recoverable: a red producer is "
            "healed and the composition goal re-solves, while default recovery "
            "still ignores those goals."
        ),
        tags=("absorbed", "recovery", "repair", "composition"),
        last_proved_at=utc_now_iso(),
        last_proof_exit_code=0,
    )
...
def builtin_absorbed_recovery_proof() -> dict[str, Any]:
    """Registered proof: isolation, heal, honest failure, seal, tamper, misgrade."""

    with tempfile.TemporaryDirectory(prefix="blackhole-absorbed-recovery-") as tmp:
        ...
        tampered["verdicts"]["producer_stale_healed"] = not tampered["verdicts"]["producer_stale_healed"]
        atomic_write_json(report_path, tampered)
        tamper_failed = not verify_absorbed_recovery_report(report_dir)["ok"]
        ...
        misgrade_failed = not verify_absorbed_recovery_report(report_dir)["ok"]
    ...
    live_verified = verify_absorbed_recovery_report(DEFAULT_ARTIFACT_DIR)
    ok = (
        bool(result["ok"])
    ...
    if ok:
        ensure_absorbed_recovery_capability()
    return {
        **result,
        "ok": ok,
        ...
        "action": "absorbed_recovery_plane",
