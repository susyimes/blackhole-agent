...
def builtin_webhook_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="webhook-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, WEBHOOK_ACTUATION_GOAL, WEBHOOK_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_webhook"] = (
        not gate.accepted
        and live_goal != WEBHOOK_ACTUATION_GOAL
        and WEBHOOK_ACTUATION_ID not in live_done
