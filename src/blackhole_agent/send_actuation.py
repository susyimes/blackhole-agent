...
def builtin_send_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="send-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, SEND_ACTUATION_GOAL, SEND_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_send"] = (
        not gate.accepted
        and live_goal != SEND_ACTUATION_GOAL
        and SEND_ACTUATION_ID not in live_done
