...
def builtin_ipv6scope_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="ipv6scope-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, IPV6SCOPE_ACTUATION_GOAL, IPV6SCOPE_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_ipv6scope"] = (
        not gate.accepted
        and live_goal != IPV6SCOPE_ACTUATION_GOAL
        and IPV6SCOPE_ACTUATION_ID not in live_done
