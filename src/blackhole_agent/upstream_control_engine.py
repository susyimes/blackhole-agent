...
* total-spine **post-recovery resolution-versus-strategy** — closes the
  restored-but-unresolved cliff: after atomic RvP seals matching
  recovery books, ``resolution_total_spine(...)``
  (and ``run_total_spine(resolution=True)``) independently confirms a
  second recovery, books each restored pair into a resolution
  register and pairs it with a strategy (RvS), seals a re-verifiable
  atomic resolution certificate, refuses split / one-sided / mismatched /
  failed / wrong-root / tampered resolutions, short-circuits on
  re-resolution, and rebinds the depth-28 tip without skill-route
* total-spine **post-resolution restructuring-versus-mandate** — closes the
  resolved-but-unrestructured cliff: after atomic RvS seals matching
  resolution books, ``restructuring_total_spine(...)``
  (and ``run_total_spine(restructuring=True)``) independently confirms a
  second resolution, books each resolved pair into a restructuring
  register and pairs it with a mandate (RvM), seals a re-verifiable
...
* total-spine **post-emergence reorganization-versus-charter** — closes the
  emerged-but-unreorganized cliff: after atomic EvC seals matching
  emergence books, ``reorganize_total_spine(...)``
  (and ``run_total_spine(reorganization=True)``) independently confirms a
  second emergence, books each emerged pair into a reorganization
  register and pairs it with a charter (RvC), seals a re-verifiable
...
def execution_certificate_path(root: Path) -> Path:
    ...
    from blackhole_agent.upstream_total_spine_logs import (
        execution_certificate_path as _path,
    )
...
def write_total_spine_execution_certificate(
    ...
    *,
    ...
    """
    from blackhole_agent.upstream_total_spine_logs import (
        write_total_spine_execution_certificate as _write,
    )

    return _write(out_root, body, allow_idempotent=allow_idempotent)
...
def load_total_spine_execution_certificate(
    ...
    """
    from blackhole_agent.upstream_total_spine_logs import (
        load_total_spine_execution_certificate as _load,
    )
...
def seal_total_spine_execution_chain(
    *,
    ...
    from blackhole_agent.upstream_total_spine_logs import (
        seal_total_spine_execution_chain as _chain,
    )
...
def annotate_total_spine_execution(
    ...
    *,
    ...
    from blackhole_agent.upstream_total_spine_logs import (
        annotate_total_spine_execution as _annotate,
    )
...
def execute_total_spine(
    ...
    *,
    ...
    Thin log-family engine wrapper. Historical name stays here so
    ``def execute_total_spine`` source probes and imports keep working.
    """
...
# Fallback stamps for deep short-circuit plane blocks (cert absent): constant
# per plane; root values read from the resumed effect's own certificate.
_DEEP_FALLBACK_EXTRA: dict[str, tuple[tuple[str, Any], ...]] = {
    "finality": (),
    "execution": (
        ("total_spine_state_applied", True),
        ("total_spine_state_root", "bound_state_root"),
        ("state_root", "bound_state_root"),
        ("state_applied", True),
    ),
    "actuation": (
        ("total_spine_effects_applied", True),
        ("total_spine_tip_action_root", "bound_action_root"),
        ("action_root", "bound_action_root"),
        ("tip_action_root", "bound_action_root"),
    ),
    "settlement": (
        ("total_spine_settled", True),
        ("total_spine_tip_settlement_root", "bound_settlement_root"),
        ("settlement_root", "bound_settlement_root"),
        ("tip_settlement_root", "bound_settlement_root"),
    ),
    "clearing": (
        ("total_spine_cleared", True),
        ("total_spine_discharged", True),
        ("total_spine_tip_clearing_root", "bound_clearing_root"),
        ("clearing_root", "bound_clearing_root"),
        ("tip_clearing_root", "bound_clearing_root"),
    ),
    "delivery": (
        ("total_spine_delivered", True),
        ("total_spine_dvp_ok", True),
        ("total_spine_tip_delivery_root", "bound_delivery_root"),
        ("delivery_root", "bound_delivery_root"),
        ("tip_delivery_root", "bound_delivery_root"),
    ),
    "custody": (
        ("total_spine_custodied", True),
        ("total_spine_cvt_ok", True),
        ("total_spine_tip_custody_root", "bound_custody_root"),
        ("custody_root", "bound_custody_root"),
        ("tip_custody_root", "bound_custody_root"),
    ),
    "margin": (
        ("total_spine_margined", True),
        ("total_spine_mve_ok", True),
        ("total_spine_tip_margin_root", "bound_margin_root"),
        ("margin_root", "bound_margin_root"),
        ("tip_margin_root", "bound_margin_root"),
    ),
}
...
def _total_spine_short_circuit(
    ...
    *,
    ...
    if cfg.get("deep"):
        recovered = ctx["recovered"] or bool(cert.get("recovered"))
        annotated["total_spine_adaptive"] = ctx["prior_round_count"] > 0 or bool(
            ctx["adaptive_rounds_log"]
        )
        if ctx["adaptive_rounds_log"]:
            annotated["total_spine_adaptive_rounds"] = ctx["adaptive_rounds_log"]
            annotated["total_spine_adaptive_round_count"] = len(
                ctx["adaptive_rounds_log"]
            )
            annotated["total_spine_adaptive_recovered"] = recovered
            annotated["total_spine_adaptive_excluded"] = sorted(ctx["exclude"])
        annotated["total_spine_continuity"] = resume.get("checkpoint") is not None
        if resume.get("checkpoint") is not None:
            checkpoint = resume["checkpoint"]
            annotated["total_spine_continuity_status"] = checkpoint.get("status")
            annotated["total_spine_continuity_recovered"] = recovered
            annotated["total_spine_continuity_digest"] = checkpoint.get(
                "checkpoint_digest"
            )
        planes = cfg.get("planes")
        if planes is None:
            planes = ("finality", "execution", "actuation") + tuple(
                chain[: chain.index(effect)]
            )
        for plane in planes:
            plane_cert = resume.get(plane)
            if plane_cert is not None:
                annotated = globals()[f"annotate_total_spine_{plane}"](
                    annotated,
                    certificate=plane_cert,
                    prior_tip=bound_tip,
                    short_circuit=True,
                )
                bound_tip = str(
                    annotated.get(f"total_spine_{plane}_bound_tip") or bound_tip
                )
            else:
                annotated[f"total_spine_{plane}"] = True
                annotated[f"total_spine_{plane}_irreversible"] = True
                annotated[f"total_spine_{plane}_short_circuit"] = True
                for key, val in _DEEP_FALLBACK_EXTRA[plane]:
                    annotated[key] = cert.get(val) if isinstance(val, str) else val
    else:
        for name in ("finality", "execution", "actuation"):
            annotated[f"total_spine_{name}"] = True
        for name in chain[: chain.index(effect)]:
            annotated[f"total_spine_{name}"] = True
    reann_start = cfg.get("reann_start", "funding")
    for name in (
        ()
        if cfg.get("deep")
        else chain[chain.index(reann_start) : chain.index(effect)]
    ):
...
def builtin_total_spine_execution_proof() -> dict[str, Any]:
    ...
    Thin log-family engine wrapper. Historical name stays here so
    ``def builtin_total_spine_execution_proof`` source probes keep working.
    """
    from blackhole_agent.upstream_total_spine_logs import (
        builtin_total_spine_execution_proof as _proof,
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.cmd == "emergence-proof":
        ...
        print(json.dumps(result, indent=2, default=str))
        return 0 if result.get("ok") else 1
    if args.cmd == "reorganization-proof":
