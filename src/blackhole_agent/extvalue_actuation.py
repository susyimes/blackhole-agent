...
def builtin_extvalue_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="extvalue-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, EXTVALUE_ACTUATION_GOAL, EXTVALUE_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_extvalue"] = (
        not gate.accepted
        and live_goal != EXTVALUE_ACTUATION_GOAL
        and EXTVALUE_ACTUATION_ID not in live_done
