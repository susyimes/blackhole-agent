...
def builtin_datagram_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="datagram-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, DATAGRAM_ACTUATION_GOAL, DATAGRAM_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_datagram"] = (
        not gate.accepted
        and live_goal != DATAGRAM_ACTUATION_GOAL
        and DATAGRAM_ACTUATION_ID not in live_done
