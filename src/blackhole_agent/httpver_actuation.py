...
def builtin_httpver_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="httpver-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, HTTPVER_ACTUATION_GOAL, HTTPVER_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_httpver"] = (
        not gate.accepted
        and live_goal != HTTPVER_ACTUATION_GOAL
        and HTTPVER_ACTUATION_ID not in live_done
