...
def builtin_mcp_resource_subscribe_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="mcp-subscribe-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, MCP_SUBSCRIBE_GOAL, MCP_SUBSCRIBE_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_subscribe"] = (
        not gate.accepted
        and live_goal != MCP_SUBSCRIBE_GOAL
        and MCP_SUBSCRIBE_ID not in live_done
