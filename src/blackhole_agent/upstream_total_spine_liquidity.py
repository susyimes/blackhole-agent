"""Post-collateral liquidity-versus-coverage for the absolute total spine.

Closes the collateralized-but-unfunded cliff: after ``collateral_total_spine``
seals atomic CvO receipts, independently confirm a second collateral, book
each collateralized pair into a liquidity register and pair it with coverage
(LvC), seal hash-chained atomic liquidity receipts bound to the collateral
digests, refuse split / one-sided / mismatched / failed / wrong-root /
tampered liquidities, short-circuit re-fund, and rebind the depth-28 tip
without skill-route.
"""
...
TOTAL_SPINE_LIQUIDITY_IMPL = True
TOTAL_SPINE_LIQUIDITY_KIND: str = "total_spine_liquidity"
TOTAL_SPINE_LIQUIDITY_FILENAME: str = "total-spine-liquidity.json"
TOTAL_SPINE_LIQUIDITY_MIN_LIQUIDITIES: int = 2
...
class StageRefused(Exception):
    """A verdict-bearing refusal from total-spine liquidity."""
...
def _collateral_digest_of(row: Mapping[str, Any]) -> str:
    return str(
...
def _capability_list(row: Mapping[str, Any]) -> list[str]:
    ...
    if caps:
        return caps
    for leg in row.get("collaterals") or row.get("liquidities") or []:
...
def _book_signature(margin: Mapping[str, Any]) -> str:
    ...
    legs = margin.get("collaterals") or []
    sigs: list[str] = []
    ...
    return _sha256_json(
        {
            ...
            "collateral_count": int(margin.get("collateral_count") or 0),
        }
    )


def _lvc_pairs(capabilities: Sequence[str]) -> list[dict[str, Any]]:
...
def _assert_pairs_atomic(pairs: Sequence[Mapping[str, Any]]) -> None:
    if not pairs:
        raise StageRefused(
            "total_spine_liquidity_pairs_empty",
            "liquidity refuses an empty LvC pair book",
        )
    for row in pairs:
        if not isinstance(row, Mapping):
            raise StageRefused(
                "total_spine_liquidity_partial",
                "liquidity refuses a malformed LvC pair",
            )
        liquidity_ok = bool(row.get("liquidity_ok", True))
        coverage_ok = bool(row.get("coverage_ok", True))
        if liquidity_ok != coverage_ok or not bool(row.get("atomic_ok", True)):
            raise StageRefused(
                "total_spine_liquidity_partial",
                "liquidity refuses a split (non-atomic) liquidity-versus-coverage pair",
            )
        if not liquidity_ok or not coverage_ok:
            raise StageRefused(
                "total_spine_liquidity_partial",
                "liquidity refuses an unfunded or uncovered LvC pair",
            )


def _liquidity_certificate_material(body: Mapping[str, Any]) -> dict[str, Any]:
    """Canonical material for total-spine liquidity certificate digests."""
    legs = body.get("liquidities") or body.get("legs") or []
    ...
    if isinstance(legs, list):
        for row in legs:
            ...
            collateral_rows.append(
                {
                    "collateral_index": int(row.get("collateral_index") or 0),
                    "collateral_height": int(row.get("collateral_height") or 0),
                    "collateral_digest": str(row.get("collateral_digest") or ""),
                    "bound_collateral_root": str(
                        row.get("bound_collateral_root") or ""
                    ),
                    "bound_delivery_root": str(
                    ...
                    "parent_liquidity_root": str(
                        row.get("parent_liquidity_root") or ""
                    ),
                    "collateral_root": str(row.get("collateral_root") or ""),
                    "post_collateral": bool(row.get("post_collateral", True)),
                    "deterministic": bool(row.get("deterministic", True)),
                    "lvc": bool(row.get("lvc", True)),
                }
            )
    return {
        "schema_version": int(body.get("schema_version") or SCHEMA_VERSION),
        "kind": str(body.get("kind") or TOTAL_SPINE_LIQUIDITY_KIND),
        ...
        "bound_collateral_root": str(body.get("bound_collateral_root") or ""),
        "bound_custody_root": str(body.get("bound_custody_root") or ""),
        "bound_delivery_root": str(body.get("bound_delivery_root") or ""),
        "collateral_digest": str(body.get("collateral_digest") or ""),
        "delivery_digest": str(body.get("delivery_digest") or ""),
        "parent_liquidity_root": str(body.get("parent_liquidity_root") or ""),
        "tip_liquidity_root": str(body.get("tip_liquidity_root") or ""),
        "liquidity_height": int(body.get("liquidity_height") or 0),
        "liquidity_count": int(body.get("liquidity_count") or 0),
        ...
        "funded": bool(body.get("funded", True)),
        "covered": bool(body.get("covered", True)),
        "atomic_ok": bool(body.get("atomic_ok", True)),
        "lvc_ok": bool(body.get("lvc_ok", True)),
        "one_sided": bool(body.get("one_sided", False)),
        "liquidities_ok": bool(body.get("liquidities_ok", True)),
        "collaterals_ok": bool(body.get("collaterals_ok", True)),
        "post_collateral": bool(body.get("post_collateral", True)),
        "deterministic": bool(body.get("deterministic", True)),
        "irreversible": bool(body.get("irreversible", True)),
        "success": bool(body.get("success", True)),
        "liquidities": collateral_rows,
    }


def compute_total_spine_liquidity_root(
    ...
    for idx, row in enumerate(collaterals):
        body = {
            "collateral_index": int(row.get("collateral_index") or idx),
            "collateral_height": int(row.get("collateral_height") or (idx + 1)),
            "collateral_digest": str(row.get("collateral_digest") or ""),
            "bound_collateral_root": str(row.get("bound_collateral_root") or ""),
...
def book_total_spine_collaterals(
    ...
    *,
    ...
    actuation/settlement/clearing roots and the same collateralized pair book.
    ...
    want = max(int(min_liquidities), TOTAL_SPINE_LIQUIDITY_MIN_LIQUIDITIES)
    verified: list[Mapping[str, Any]] = []
    for raw in margins:
        if not isinstance(raw, Mapping):
            continue
        verify = verify_total_spine_collateral_certificate(raw)
        if not verify.get("ok"):
            raise StageRefused(
                "total_spine_liquidity_margin_tampered",
                "liquidity refuses a margin whose digest/chain does not verify",
            )
        if raw.get("collateralized") is False or raw.get("success") is False:
            raise StageRefused(
                "total_spine_liquidity_margin_uncollateralized",
                "liquidity refuses an uncollateralized margin receipt",
            )
        if raw.get("obligated") is False or raw.get("cvo_ok") is False:
            raise StageRefused(
                "total_spine_liquidity_margin_unobligated",
                "liquidity refuses a margin whose CvO is not complete",
            )
        if raw.get("atomic_ok") is False:
            raise StageRefused(
                "total_spine_liquidity_margin_partial",
                "liquidity refuses a non-atomic margin receipt",
            )
        if int(raw.get("residual") or 0) != 0:
            raise StageRefused(
                "total_spine_liquidity_residual",
                "liquidity refuses a margin with a non-zero residual",
            )
        verified.append(raw)
    if len(verified) < want:
        raise StageRefused(
            "total_spine_liquidity_margins_short",
            f"liquidity requires >= {want} independent collaterals, "
            f"got {len(verified)}",
        )
    ...
    if not book_state or not book_action or not book_actuation:
        raise StageRefused(
            "total_spine_liquidity_root_missing",
            "liquidity requires collateral bound state/action/actuation roots",
        )
    if not book_caps:
        raise StageRefused(
            "total_spine_liquidity_pairs_empty",
            "liquidity refuses a collateral with no collateralized capabilities",
        )

    legs: list[dict[str, Any]] = []
    parent = str(parent_liquidity_root or "")
    for idx, margin in enumerate(verified):
        ...
        if (
            ...
            raise StageRefused(
                "total_spine_liquidity_root_mismatch",
                "liquidity refuses collaterals bound to different "
                "state/action/actuation roots",
            )
        if settlement and book_settlement and settlement != book_settlement:
            raise StageRefused(
                "total_spine_liquidity_root_mismatch",
                "liquidity refuses collaterals bound to different settlement roots",
            )
        if clearing and book_clearing and clearing != book_clearing:
            raise StageRefused(
                "total_spine_liquidity_root_mismatch",
                "liquidity refuses collaterals bound to different clearing roots",
            )
        sig = _book_signature(margin)
        if sig != book_sig:
            raise StageRefused(
                "total_spine_liquidity_lvc_failed",
                "independent collateral books disagree; LvC cannot complete",
            )
        caps = tuple(_capability_list(margin))
        if caps != book_caps:
            raise StageRefused(
                "total_spine_liquidity_one_sided",
                "liquidity refuses one-sided books whose capability sets differ",
            )
        pairs = _lvc_pairs(book_caps)
        _assert_pairs_atomic(pairs)
        height = (
            int(liquidity_height) + idx
            if liquidity_height is not None
            else (idx + 1)
        )
        material = {
            "collateral_index": idx,
            "collateral_height": height,
            "collateral_digest": _collateral_digest_of(margin),
            "bound_collateral_root": str(
                margin.get("tip_collateral_root") or ""
            ),
            "bound_delivery_root": str(
                margin.get("bound_delivery_root")
                or margin.get("tip_delivery_root")
...
def seal_total_spine_liquidity_certificate(
    body: Mapping[str, Any],
) -> dict[str, Any]:
    """Seal post-collateral LvC log into a tamper-evident receipt."""
    sealed_body = dict(body)
    collaterals = list(sealed_body.get("liquidities") or [])
    if not str(sealed_body.get("tip_liquidity_root") or "").strip():
        sealed_body["tip_liquidity_root"] = compute_total_spine_liquidity_root(
            collaterals
        )
    if not int(sealed_body.get("liquidity_count") or 0):
        sealed_body["liquidity_count"] = len(collaterals)
    if not int(sealed_body.get("liquidity_height") or 0):
        sealed_body["liquidity_height"] = len(collaterals)
    material = _liquidity_certificate_material(sealed_body)
    material["tip_liquidity_root"] = str(sealed_body.get("tip_liquidity_root") or "")
    digest = _sha256_json(material)
    sealed = dict(material)
    sealed["liquidity_digest"] = digest
    sealed["certificate_hash"] = digest
    sealed["total_spine_liquidity"] = True
    sealed["total_spine_liquidity_impl"] = TOTAL_SPINE_LIQUIDITY_IMPL
    ...
    if collaterals:
        ...
        for src, dest in zip(collaterals, sealed.get("liquidities") or []):
            if isinstance(src, Mapping) and isinstance(dest, dict):
                if src.get("pairs"):
                    dest["pairs"] = list(src.get("pairs") or [])
                sealed_pairs.append(dest)
        if sealed_pairs:
            sealed["liquidities"] = sealed_pairs
    return sealed


def liquidity_certificate_path(root: Path) -> Path:
    """Resolve ``total-spine-liquidity.json`` under a liquidity/out root."""
    path = Path(root)
    if path.is_file():
        if path.name == TOTAL_SPINE_LIQUIDITY_FILENAME or path.suffix == ".json":
            try:
                probe = json.loads(path.read_text(encoding="utf-8"))
            except (OSError, json.JSONDecodeError):
                probe = None
            if isinstance(probe, Mapping) and (
                str(probe.get("kind") or "") == TOTAL_SPINE_LIQUIDITY_KIND
                or path.name == TOTAL_SPINE_LIQUIDITY_FILENAME
            ):
                return path
        parent = path.parent
        sibling = parent / TOTAL_SPINE_LIQUIDITY_FILENAME
        if sibling.is_file():
            return sibling
        nested = parent / "liquidity" / TOTAL_SPINE_LIQUIDITY_FILENAME
        if nested.is_file():
            return nested
        grand = parent.parent / "liquidity" / TOTAL_SPINE_LIQUIDITY_FILENAME
        if grand.is_file():
            return grand
        grand_sib = parent.parent / TOTAL_SPINE_LIQUIDITY_FILENAME
        if grand_sib.is_file():
            return grand_sib
        return parent / "liquidity" / TOTAL_SPINE_LIQUIDITY_FILENAME
    named = path / TOTAL_SPINE_LIQUIDITY_FILENAME
    if named.is_file():
        return named
    nested = path / "liquidity" / TOTAL_SPINE_LIQUIDITY_FILENAME
    if nested.is_file():
        return nested
    return path / "liquidity" / TOTAL_SPINE_LIQUIDITY_FILENAME


def write_total_spine_liquidity_certificate(
    out_root: Path,
    body: Mapping[str, Any],
    *,
    ...
    sealed = seal_total_spine_liquidity_certificate(body)
    path = liquidity_certificate_path(Path(out_root))
    if path.is_file():
        try:
            existing = load_total_spine_liquidity_certificate(path)
        except StageRefused:
            existing = None
        if existing is not None:
            existing_digest = str(
                existing.get("liquidity_digest")
                or existing.get("certificate_hash")
                or ""
            )
            new_digest = str(
                sealed.get("liquidity_digest")
            ...
            if (
                ...
                existing["liquidity_path"] = str(path)
                existing["total_spine_liquidity_idempotent"] = True
                return existing
            raise StageRefused(
                "total_spine_liquidity_supersession_refused",
                f"irreversible liquidity already sealed at {path} "
                f"(existing={existing_digest!r} attempted={new_digest!r})",
            )
    path.parent.mkdir(parents=True, exist_ok=True)
    atomic_write_json(path, sealed)
    sealed["liquidity_path"] = str(path)
    sealed["total_spine_liquidity_idempotent"] = False
    return sealed


def verify_total_spine_liquidity_certificate(
    certificate: Mapping[str, Any],
) -> dict[str, Any]:
    """Recompute collateral digest and LvC roots; fail closed on tamper."""
    claimed = str(
        certificate.get("liquidity_digest")
        or certificate.get("certificate_hash")
        or ""
    )
    material = _liquidity_certificate_material(certificate)
    expected = _sha256_json(material)
    collaterals = list(certificate.get("liquidities") or [])
    recomputed_tip = compute_total_spine_liquidity_root(collaterals)
    claimed_tip = str(certificate.get("tip_liquidity_root") or "")
    height = int(certificate.get("liquidity_height") or 0)
    count = int(certificate.get("liquidity_count") or 0)
    ...
    cert_parent = str(certificate.get("parent_liquidity_root") or "")
    chain_ok = True
    parent = cert_parent
    book_sig = ""
    for idx, row in enumerate(collaterals):
        ...
        if str(row.get("actuation_digest") or "") != actuation_digest:
            chain_ok = False
            break
        if str(row.get("parent_liquidity_root") or "") != parent:
            chain_ok = False
            break
    ...
    min_ok = count >= TOTAL_SPINE_LIQUIDITY_MIN_LIQUIDITIES and height >= count
    liquidities_ok = all(
        isinstance(row, Mapping)
        and bool(row.get("funded", True))
        and bool(row.get("covered", True))
        and bool(row.get("atomic_ok", True))
        and bool(row.get("lvc", True))
    ...
    ok = (
        ...
        and str(certificate.get("kind") or "") == TOTAL_SPINE_LIQUIDITY_KIND
        and int(certificate.get("schema_version") or 0) == SCHEMA_VERSION
        and certificate.get("irreversible") is True
        and certificate.get("post_collateral") is True
        and certificate.get("deterministic") is True
        and certificate.get("funded") is True
        and certificate.get("covered") is True
        and certificate.get("atomic_ok") is True
        and certificate.get("lvc_ok") is True
    ...
    return {
        "ok": ok,
        "action": "verify_total_spine_liquidity",
        ...
        "recomputed_tip_liquidity_root": recomputed_tip,
        "chain_ok": chain_ok,
        "min_liquidities_ok": min_ok,
        "liquidities_ok": liquidities_ok,
        "lvc_ok": certificate.get("lvc_ok") is True,
        "atomic_ok": certificate.get("atomic_ok") is True,
        "kind_ok": str(certificate.get("kind") or "") == TOTAL_SPINE_LIQUIDITY_KIND,
        "schema_ok": int(certificate.get("schema_version") or 0) == SCHEMA_VERSION,
        "irreversible_ok": certificate.get("irreversible") is True,
        "funded_ok": certificate.get("funded") is True,
        "covered_ok": certificate.get("covered") is True,
        "total_spine_liquidity": True,
        "used_skill_route_discovery": legacy_pipeline_was_used(),
    }


def load_total_spine_liquidity_certificate(
    path: Path | str,
) -> dict[str, Any]:
    """Load and integrity-check a sealed liquidity receipt."""
    file_path = liquidity_certificate_path(Path(path))
    if not file_path.is_file():
        raise StageRefused(
            "total_spine_liquidity_missing",
            f"liquidity certificate not found at {file_path}",
        )
    ...
    try:
        ...
        raise StageRefused(
            "total_spine_liquidity_unreadable",
            f"liquidity certificate unreadable at {file_path}: {exc}",
        ) from exc
    if not isinstance(payload, Mapping):
        raise StageRefused(
            "total_spine_liquidity_invalid",
            "liquidity certificate root must be a JSON object",
        )
    if str(payload.get("kind") or "") != TOTAL_SPINE_LIQUIDITY_KIND and not payload.get(
    ...
    verify = verify_total_spine_liquidity_certificate(payload)
    if not verify.get("ok"):
        raise StageRefused(
            "total_spine_liquidity_tampered",
            f"liquidity certificate digest mismatch at {file_path} "
            f"(claimed={verify.get('claimed_digest')!r} "
            f"expected={verify.get('expected_digest')!r})",
        )
    body = dict(payload)
    body["liquidity_path"] = str(file_path)
    body["liquidity_verify"] = verify
    body["total_spine_liquidity_loaded"] = True
    return body


def seal_total_spine_liquidity_chain(
    *,
    prior_tip: str,
    ...
    """Seal liquidity hop into the absolute-tower tip."""
    tip = str(prior_tip or "").strip() or ("0" * 64)
    md = str(liquidity_digest or "").strip() or ("0" * 64)
    mr = str(tip_liquidity_root or "").strip() or ("0" * 64)
    cr = str(bound_collateral_root or "").strip() or ("0" * 64)
    ...
    cd = str(collateral_digest or "").strip() or ("0" * 64)
    dvd = str(delivery_digest or "").strip() or ("0" * 64)
    material = (
        f"liquidity|{int(bool(short_circuit))}|{int(liquidity_height)}|"
    ...
    return {
        ...
        "tip_liquidity_root": mr,
        "bound_collateral_root": cr,
...
def annotate_total_spine_liquidity(
    body: dict[str, Any],
    *,
    certificate: Mapping[str, Any],
    prior_tip: str,
    ...
    """Stamp post-collateral LvC onto a total-spine result and rebind tip."""
    cst_digest = str(
        certificate.get("liquidity_digest")
        or certificate.get("certificate_hash")
        or ""
    )
    tip_liquidity_root = str(certificate.get("tip_liquidity_root") or "")
    liquidity_height = int(certificate.get("liquidity_height") or 0)
    liquidity_count = int(certificate.get("liquidity_count") or 0)
    ...
    bound_collateral_root = str(certificate.get("bound_collateral_root") or "")
    bound_delivery_root = str(certificate.get("bound_delivery_root") or "")
    actuation_digest = str(certificate.get("actuation_digest") or "")
    collateral_digest = str(certificate.get("collateral_digest") or "")
    delivery_digest = str(certificate.get("delivery_digest") or "")
    chain = seal_total_spine_liquidity_chain(
        prior_tip=prior_tip,
        liquidity_digest=cst_digest,
        tip_liquidity_root=tip_liquidity_root,
        ...
        collateral_digest=collateral_digest,
    ...
    body["total_spine_liquidity"] = True
    body["total_spine_liquidity_impl"] = TOTAL_SPINE_LIQUIDITY_IMPL
    body["total_spine_liquidity_short_circuit"] = bool(short_circuit)
    body["total_spine_liquidity_irreversible"] = True
    body["total_spine_liquidity_post_collateral"] = True
    body["total_spine_liquidity_deterministic"] = True
    body["total_spine_liquidity_lvc"] = True
    body["total_spine_liquidity_certificate"] = dict(certificate)
    body["total_spine_liquidity_digest"] = cst_digest
    body["total_spine_liquidity_chain"] = chain
    body["total_spine_liquidity_tip"] = cst_tip
    body["total_spine_liquidity_bound_tip"] = bound
    body["total_spine_digest_pre_liquidity"] = prior_tip
    body["total_spine_tip_liquidity_root"] = tip_liquidity_root
    body["total_spine_liquidity_height"] = liquidity_height
    body["total_spine_liquidity_count"] = liquidity_count
    body["total_spine_funded"] = bool(certificate.get("funded", True))
    body["total_spine_funded_ok"] = bool(certificate.get("funded", True))
    body["total_spine_covered"] = bool(certificate.get("covered", True))
    body["total_spine_covered_ok"] = bool(certificate.get("covered", True))
    body["total_spine_lvc_ok"] = bool(certificate.get("lvc_ok", True))
    body["total_spine_liquidity_atomic"] = bool(certificate.get("atomic_ok", True))
    body["total_spine_liquidity_one_sided"] = bool(
        certificate.get("one_sided", False)
    )
    body["total_spine_liquidities_ok"] = bool(
        certificate.get("liquidities_ok", True)
    )
    body["total_spine_liquidity_root_valid"] = bool(tip_liquidity_root)
    body["total_spine_liquidity_residual"] = int(certificate.get("residual") or 0)
    body["total_spine_liquidity_pair_count"] = int(
        certificate.get("pair_count") or 0
    )
    body["liquidity_root"] = tip_liquidity_root
    body["tip_liquidity_root"] = tip_liquidity_root
    body["liquidity_count"] = liquidity_count
    body["liquidity_height"] = liquidity_height
    body["funded"] = bool(certificate.get("funded", True))
    body["funded_ok"] = bool(certificate.get("funded", True))
    body["lvc_ok"] = bool(certificate.get("lvc_ok", True))
    body["covered"] = bool(certificate.get("covered", True))
    if certificate.get("liquidity_path"):
        body["total_spine_liquidity_path"] = certificate.get("liquidity_path")
    if bound_state_root:
    ...
    if bound_collateral_root:
        body["total_spine_tip_collateral_root"] = bound_collateral_root
        body["collateral_root"] = bound_collateral_root
        body["tip_collateral_root"] = bound_collateral_root
        body.setdefault("total_spine_collateral", True)
        body.setdefault("total_spine_collateralized", True)
        body.setdefault("total_spine_cvo_ok", True)
    ...
    if collateral_digest:
        body["total_spine_collateral_digest"] = collateral_digest
    if delivery_digest:
    ...
    body["verdict"] = (
        "total_spine_liquidity_ok_short_circuit"
        if short_circuit
        else "total_spine_liquidity_ok"
    )
    body["ok"] = True
    return body
...
def _as_collateral_mapping(value: Any) -> dict[str, Any] | None:
    from blackhole_agent.upstream_total_spine_collateral import (
        StageRefused as CollateralRefused,
        load_total_spine_collateral_certificate,
    )

    if isinstance(value, Mapping):
        kind = str(value.get("kind") or "")
        if kind == TOTAL_SPINE_LIQUIDITY_KIND or value.get("total_spine_liquidity"):
            nested_liq = value.get("total_spine_collateral_certificate")
        ...
        ) or value.get("total_spine_collateral_loaded") or value.get(
            "tip_collateral_root"
        ):
    ...
    try:
        probe_path = path
        if path.is_dir():
            nested = path / "collateral" / "total-spine-collateral.json"
            named = path / "total-spine-collateral.json"
            probe_path = nested if nested.is_file() else named
        if probe_path.is_file():
            probe = json.loads(probe_path.read_text(encoding="utf-8"))
            if isinstance(probe, Mapping):
                kind = str(probe.get("kind") or "")
                if kind and kind != TOTAL_SPINE_COLLATERAL_KIND:
                    return None
    except (OSError, json.JSONDecodeError):
        pass
    try:
        return load_total_spine_collateral_certificate(path)
    except CollateralRefused as exc:
...
def _as_liquidity_mapping(value: Any) -> dict[str, Any] | None:
    if isinstance(value, Mapping):
        ...
        if kind == TOTAL_SPINE_LIQUIDITY_KIND or value.get(
            "total_spine_liquidity"
        ) or value.get("total_spine_liquidity_loaded"):
    ...
    if value is None:
        return None
    try:
        return load_total_spine_liquidity_certificate(Path(str(value)))
...
def _confirm_collateral(
    ...
    *,
    ...
    """Independently re-collateral the same book as a confirmation side."""
    from blackhole_agent.upstream_total_spine_collateral import collateral_total_spine

    confirm_out = None
    if out_root is not None:
        confirm_out = Path(out_root) / "liquidity-confirm"
    tip_collateral = str(primary.get("tip_collateral_root") or "")
    col_height = int(primary.get("collateral_height") or 0)
    bundle: list[Any] = []
    for row in margins:
        bundle.append(row)
    ...
    if not bundle:
        raise StageRefused(
            "total_spine_liquidity_confirmation_missing",
            "single collateral requires margins, custodies, deliveries, "
            "clearings, settlements, or actuation to confirm-liquidity",
        )
    confirmed = collateral_total_spine(
        bundle,
        ...
        collateral_height=col_height + 1 if col_height else None,
        repo_path=repo_path or REPO_ROOT,
        confirm=True,
    )
    cert = confirmed.get("total_spine_collateral_certificate")
    if not isinstance(cert, Mapping):
        raise StageRefused(
            "total_spine_liquidity_confirmation_missing",
            "confirmation collateral did not produce a certificate",
        )
    return dict(cert)
...
def _collect_collaterals(
    ...
    existing = _as_liquidity_mapping(source)
    if existing is None and body is not None:
        existing = _as_liquidity_mapping(body)
    ...
    def _push(item: Any) -> None:
        mapped = _as_collateral_mapping(item)
        if mapped is not None:
            collaterals.append(mapped)
    ...
    seen: set[str] = set()
    for row in collaterals:
        digest = _collateral_digest_of(row)
        tip = str(row.get("tip_collateral_root") or "")
        key = digest or tip
    ...
    return existing, deduped, mgn_deduped, cst_deduped, dlv_deduped, clr_deduped, set_deduped, actuation


def _strip_liquidity_predicates(done_when: str) -> str:
    """Evaluate the pre-fund contract, never liquidity_* (no recurse)."""
...
def liquidity_total_spine(
    ...
    *,
    ...
    """Apply post-collateral atomic LvC liquidity on the absolute total spine."""
    ...
    if not TOTAL_SPINE_LIQUIDITY_IMPL:
        raise StageRefused(
            "total_spine_liquidity_disabled",
            "TOTAL_SPINE_LIQUIDITY_IMPL is False",
        )

    extra_books: list[Any] = []
    extra_books.extend(list(collaterals or []))
    ...
    (
        ...
        found_settlements,
        found_actuation,
    ) = _collect_collaterals(source, body, extra_books)
    ...
    if (
        ...
        and existing.get("tip_liquidity_root")
        and (
            str(existing.get("kind") or "") == TOTAL_SPINE_LIQUIDITY_KIND
            or existing.get("total_spine_liquidity_loaded")
            or existing.get("total_spine_liquidity")
        )
    ):
        tip = str(
            prior_tip
        ...
        result = body if body is not None else {
            "ok": True,
            "action": "liquidity_total_spine",
            "total_spine": True,
        }
        return annotate_total_spine_liquidity(
            result,
    ...
    if len(collected) < want and confirm and collected:
        collected.append(
            _confirm_collateral(
                ...
                prior_tip=str(
                    prior_tip
                    or (body or {}).get("total_spine_collateral_bound_tip")
                    or (body or {}).get("total_spine_digest")
                    or ""
                ),
                repo_path=repo_path,
    ...
    legs = book_total_spine_collaterals(
        collected,
        min_liquidities=want,
        parent_liquidity_root=parent_liquidity_root,
        liquidity_height=liquidity_height,
    )
    first = collected[0]
    ...
    margin_root = str(first.get("tip_collateral_root") or "")
    collateral_digest = _collateral_digest_of(first)
    root_layer = str(
    ...
    pre_liquidity = _strip_liquidity_predicates(done_when)
    if pre_liquidity:
        ctx = {
            "margin": {
                "ok": True,
                ...
                "cvo_ok": True,
                "collateral_count": int(first.get("collateral_count") or 0),
                "tip_collateral_root": margin_root,
            },
            "collateral_count": int(first.get("collateral_count") or 0),
            "tip_collateral_root": margin_root,
        ...
        if contract_machine and contract_met is not True:
            raise StageRefused(
                "total_spine_liquidity_contract_unmet",
                f"done_when not met at collateral: {pre_liquidity!r}",
            )

    tip_liquidity_root = compute_total_spine_liquidity_root(legs)
    cst_height = int(legs[-1]["collateral_height"]) if legs else 0
    tip = str(
        prior_tip
        or (body or {}).get("total_spine_collateral_bound_tip")
        or (body or {}).get("total_spine_digest")
    ...
    cst_body: dict[str, Any] = {
        ...
        "kind": TOTAL_SPINE_LIQUIDITY_KIND,
        ...
        "bound_collateral_root": margin_root,
        ...
        "prior_tip": tip,
        "parent_liquidity_root": str(
            parent_liquidity_root
            or (legs[0].get("parent_liquidity_root") if legs else "")
            or ""
        ),
        "liquidities": legs,
        "liquidity_count": len(legs),
        "liquidity_height": cst_height,
        "tip_liquidity_root": tip_liquidity_root,
    ...
    if write_target is not None:
        certificate = write_total_spine_liquidity_certificate(write_target, cst_body)
    else:
        certificate = seal_total_spine_liquidity_certificate(cst_body)
    ...
    if annotated.get("total_spine_compressed") and root_layer:
        ...
        cst_bound = str(annotated.get("total_spine_liquidity_bound_tip") or tip)
    ...
    annotated["total_spine_liquidity_bound_state_root"] = state_root
    annotated["total_spine_liquidity_bound_action_root"] = action_root
    annotated["total_spine_liquidity_bound_settlement_root"] = settlement_root
    annotated["total_spine_liquidity_bound_clearing_root"] = clearing_root
    annotated["total_spine_liquidity_bound_collateral_root"] = margin_root
    annotated["total_spine_liquidity_actuation_digest"] = actuation_digest
    annotated["used_skill_route_discovery"] = legacy_pipeline_was_used()
    return annotated

def builtin_total_spine_liquidity_proof() -> dict[str, Any]:
    """Hermetic proof: post-collateral atomic LvC on the absolute tower."""
    import shutil
    import tempfile
    ...
    from blackhole_agent.upstream_control_engine import (
        ...
        TOTAL_SPINE_LIQUIDITY_IMPL as ENGINE_LIQ_IMPL,
    ...
    scratch = Path(tempfile.mkdtemp(prefix="total-spine-liquidity-proof-"))
    try:
        from blackhole_agent import upstream_loop_engine as le_facade

        flags_ok = (
            TOTAL_SPINE_LIQUIDITY_IMPL is True
            and ENGINE_LIQ_IMPL is True
            and TOTAL_SPINE_COLLATERAL_IMPL is True
            and TOTAL_SPINE_MARGIN_IMPL is True
            and TOTAL_SPINE_CUSTODY_IMPL is True
            and TOTAL_SPINE_LIQUIDITY_KIND == "total_spine_liquidity"
            and bool(TOTAL_SPINE_LIQUIDITY_FILENAME)
            and TOTAL_SPINE_LIQUIDITY_MIN_LIQUIDITIES >= 2
        )
        ...
        offline_liq = liquidity_total_spine(
            [cert_col1, cert_col2],
        ...
        liq_path = offline_liq.get("total_spine_liquidity_path")
        tip_liquidity = str(offline_liq.get("total_spine_tip_liquidity_root") or "")
        offline_ok = (
            bool(offline_liq.get("ok"))
            and offline_liq.get("total_spine_liquidity") is True
            and offline_liq.get("total_spine_liquidity_post_collateral") is True
            and offline_liq.get("total_spine_liquidity_irreversible") is True
            and offline_liq.get("total_spine_funded") is True
            and offline_liq.get("total_spine_covered") is True
            and offline_liq.get("total_spine_lvc_ok") is True
            and offline_liq.get("total_spine_liquidity_atomic") is True
            and offline_liq.get("total_spine_liquidity_one_sided") is False
            and int(offline_liq.get("total_spine_liquidity_count") or 0) >= 2
            and int(offline_liq.get("total_spine_liquidity_height") or 0) >= 2
            and int(offline_liq.get("total_spine_liquidity_residual") or 0) == 0
            and int(offline_liq.get("total_spine_liquidity_pair_count") or 0) >= 1
            and len(tip_liquidity) >= 32
            and str(offline_liq.get("total_spine_state_root") or "") == state_root
            and str(offline_liq.get("total_spine_tip_action_root") or "")
            == tip_action
            and str(offline_liq.get("total_spine_digest") or "")
            != str(col1.get("total_spine_digest") or "")
            and isinstance(liq_path, str)
            and Path(liq_path).is_file()
            and not legacy_pipeline_was_used()
        )

        loaded = load_total_spine_liquidity_certificate(liq_path or scratch)
        verify_ok = bool(
            loaded.get("total_spine_liquidity_loaded")
            and (loaded.get("liquidity_verify") or {}).get("ok")
            and (loaded.get("liquidity_verify") or {}).get("liquidity_root_ok")
            and (loaded.get("liquidity_verify") or {}).get("chain_ok")
            and (loaded.get("liquidity_verify") or {}).get("liquidities_ok")
            and (loaded.get("liquidity_verify") or {}).get("lvc_ok")
        )

        tampered_path = scratch / "tampered-liquidity.json"
        ...
        tampered_body["liquidity_height"] = 99
        atomic_write_json(tampered_path, tampered_body)
        tamper_ok = False
        try:
            load_total_spine_liquidity_certificate(tampered_path)
        except StageRefused as exc:
            tamper_ok = str(exc.verdict) == "total_spine_liquidity_tampered"
        except Exception:  # noqa: BLE001
            tamper_ok = False
        ...
        try:
            ...
            supersession_ok = (
                str(exc.verdict) == "total_spine_liquidity_supersession_refused"
            )
        except Exception:  # noqa: BLE001
            supersession_ok = False
        ...
        resealed = seal_total_spine_liquidity_certificate(wrong_body)
        wrong_verify = verify_total_spine_liquidity_certificate(resealed)
        wrong_root_ok = wrong_verify.get("ok") is False and (
            wrong_verify.get("chain_ok") is False
            or wrong_verify.get("liquidity_root_ok") is False
        )
        ...
        try:
            ...
            other_col = collateral_total_spine(
                other_mgn.get("total_spine_margin_certificate"),
                out_root=scratch / "col-other",
                prior_tip=str(
                    other_mgn.get("total_spine_margin_bound_tip") or ""
                ),
                actuation=actuated2.get("total_spine_actuation_certificate"),
                settlements=[
                    other_set.get("total_spine_settlement_certificate") or {}
            ...
            other_cert = other_col.get("total_spine_collateral_certificate") or {}
            if not other_cert:
                raise StageRefused(
                    "total_spine_liquidity_root_mismatch",
                    "other-origin collateral missing; cannot form a matching LvC book",
                )
            book_total_spine_collaterals(
                [cert_col1, other_cert],
                min_liquidities=2,
            )
        except StageRefused as exc:
            verdict = str(exc.verdict or "")
            mismatch_ok = verdict in {
                "total_spine_liquidity_root_mismatch",
                "total_spine_liquidity_lvc_failed",
        ...
        try:
            ...
            resealed_one = seal_total_spine_collateral_certificate(forged)
            book_total_spine_collaterals(
                [cert_col1, resealed_one], min_liquidities=2
            )
        except StageRefused as exc:
            one_sided_ok = str(exc.verdict) == "total_spine_liquidity_one_sided"
        except Exception:  # noqa: BLE001
            one_sided_ok = False
        ...
        try:
            ...
            partial_ok = str(exc.verdict) == "total_spine_liquidity_partial"
        except Exception:  # noqa: BLE001
            partial_ok = False

        h2 = liquidity_total_spine(
            [cert_col1, cert_col2],
            out_root=scratch / "liq-h2",
            prior_tip=str(
                offline_liq.get("total_spine_liquidity_bound_tip") or ""
            ),
            parent_liquidity_root=tip_liquidity,
            liquidity_height=int(
                offline_liq.get("total_spine_liquidity_height") or 0
            )
            + 1,
            repo_path=REPO_ROOT,
            confirm=False,
        ...
        multi_height_ok = (
            bool(h2.get("ok"))
            and int(h2.get("total_spine_liquidity_count") or 0) >= 2
            and str(h2.get("total_spine_tip_liquidity_root") or "") != tip_liquidity
            and str(
                (h2.get("total_spine_liquidity_certificate") or {}).get(
                    "parent_liquidity_root"
                )
                or ""
            )
        ...
        recomputed = compute_total_spine_liquidity_root(
            loaded.get("liquidities") or []
        )
        determinism_ok = recomputed == tip_liquidity and bool(recomputed)

        live = run_total_spine(
            root_layer="quettacontinuum",
            out_root=scratch / "live-liq",
            max_rounds=1,
            dispatch=True,
            dispatch_budget=2,
            max_successions=1,
        ...
        live_liq_path = live.get("total_spine_liquidity_path")
        live_ok = (
            bool(live.get("ok"))
            ...
            and live.get("total_spine_funded") is True
            and live.get("total_spine_lvc_ok") is True
            and int(live.get("total_spine_liquidity_count") or 0) >= 2
            and isinstance(live.get("total_spine_tip_liquidity_root"), str)
            and len(str(live.get("total_spine_tip_liquidity_root"))) >= 32
            and int(live.get("total_nest_depth") or 0) == 28
            and isinstance(live_liq_path, str)
            and Path(live_liq_path).is_file()
            and not legacy_pipeline_was_used()
        )

        shorted = run_total_spine(
            root_layer="quettacontinuum",
            out_root=scratch / "short-liq",
            max_rounds=1,
            dispatch=True,
            dispatch_budget=2,
            max_successions=1,
            ...
            collateral=True,
            liquidity=True,
            resume_dir=live_liq_path or (scratch / "live-liq"),
            effect_timeout=90,
            repo_path=REPO_ROOT,
        )
        short_ok = (
            bool(shorted.get("ok"))
            and shorted.get("total_spine_liquidity") is True
            and shorted.get("total_spine_liquidity_short_circuit") is True
            and str(shorted.get("total_spine_tip_liquidity_root") or "")
            == str(live.get("total_spine_tip_liquidity_root") or "")
            and int(shorted.get("total_nest_depth") or 0) == 28
            and not legacy_pipeline_was_used()
        )

        liq_chain = live.get("total_spine_liquidity_chain") or {}
        chain_integrity_ok = False
        if isinstance(liq_chain, Mapping) and liq_chain:
            re_seal = seal_total_spine_liquidity_chain(
                prior_tip=str(liq_chain.get("prior_tip") or ""),
                liquidity_digest=str(liq_chain.get("liquidity_digest") or ""),
                tip_liquidity_root=str(liq_chain.get("tip_liquidity_root") or ""),
                bound_collateral_root=str(
                    liq_chain.get("bound_collateral_root") or ""
                ),
                bound_delivery_root=str(
                    liq_chain.get("bound_delivery_root") or ""
                ),
                bound_clearing_root=str(
                    liq_chain.get("bound_clearing_root") or ""
                ),
                bound_settlement_root=str(
                    liq_chain.get("bound_settlement_root") or ""
                ),
                bound_action_root=str(liq_chain.get("bound_action_root") or ""),
                bound_state_root=str(liq_chain.get("bound_state_root") or ""),
                actuation_digest=str(liq_chain.get("actuation_digest") or ""),
                collateral_digest=str(liq_chain.get("collateral_digest") or ""),
                delivery_digest=str(liq_chain.get("delivery_digest") or ""),
                liquidity_height=int(liq_chain.get("liquidity_height") or 0),
                short_circuit=bool(liq_chain.get("short_circuit")),
            )
            chain_integrity_ok = (
                re_seal.get("digest") == liq_chain.get("digest")
                and re_seal.get("digest") == live.get("total_spine_liquidity_tip")
            )

        differential_ok = (
            offline_ok
            and live_ok
            and str(col1.get("total_spine_digest") or "")
            != str(offline_liq.get("total_spine_digest") or "")
        )

        # Facade exposes this stage's surface (delegation identity;
        # source-text greps predate the thin PEP 562 facade).
        source_ok = (
            getattr(le_facade, "TOTAL_SPINE_LIQUIDITY_IMPL", None) is TOTAL_SPINE_LIQUIDITY_IMPL
            and getattr(le_facade, "builtin_total_spine_liquidity_proof", None) is builtin_total_spine_liquidity_proof
            and getattr(le_facade, "liquidity_total_spine", None) is liquidity_total_spine
            and callable(
                getattr(le_facade, "builtin_total_spine_liquidity_proof", None)
    
        )
            and callable(getattr(le_facade, "liquidity_total_spine", None))
            and getattr(le_facade, "TOTAL_SPINE_LIQUIDITY_IMPL", False) is True
        )
        ...
        engine_source_ok = (
            "TOTAL_SPINE_LIQUIDITY_IMPL" in engine_text
            and "liquidity_total_spine" in engine_text
            and (
                "liquidity=True" in engine_text
                or "liquidity: bool = False" in engine_text
            )
            and "builtin_total_spine_liquidity_proof" in engine_text
        )

        mod_path = Path(__file__).resolve()
        mod_text = mod_path.read_text(encoding="utf-8")
        mod_source_ok = (
            "def liquidity_total_spine" in mod_text
            and "def builtin_total_spine_liquidity_proof" in mod_text
            and "total_spine_liquidity_supersession_refused" in mod_text
            and "total_spine_liquidity_tampered" in mod_text
            and "total_spine_liquidity_one_sided" in mod_text
            and "total_spine_liquidity_lvc_failed" in mod_text
        )
        ...
        try:
            ...
            ledger_ok = (
                entry is not None
                and (
                    "upstream_total_spine_liquidity" in (entry.entry or "")
                    or "upstream_control_engine" in (entry.entry or "")
                )
                and "builtin_total_spine_liquidity_proof" in (entry.entry or "")
                and (
                    "liquidity" in tags_blob
                    or "liquidity" in name_blob
                    or "liquidity" in delta_blob
                )
                and ("total" in tags_blob or "total" in name_blob)
                and (
                    "liquidity_total_spine" in delta_blob
                    or "post-collateral" in delta_blob
                    or "post_collateral" in delta_blob
                    or "lvc" in delta_blob
                )
            )
        except Exception:  # noqa: BLE001
            ledger_ok = False
        ...
        return {
            "ok": ok,
            "action": "total_spine_liquidity_proof",
            "flags_ok": flags_ok,
            "offline_ok": offline_ok,
            "liquidity_path": liq_path,
            ...
            "state_root": state_root,
            "liquidity_count": offline_liq.get("total_spine_liquidity_count"),
            "pair_count": offline_liq.get("total_spine_liquidity_pair_count"),
            "verify_ok": verify_ok,
            "tamper_ok": tamper_ok,
            ...
            "live_ok": live_ok,
            "live_liquidity_path": live_liq_path,
            "live_tip_liquidity_root": live.get("total_spine_tip_liquidity_root"),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    sub.add_parser("proof", help="Alias for liquidity-proof")
    args = parser.parse_args(list(argv) if argv is not None else None)
    if args.cmd in {"liquidity-proof", "proof"}:
