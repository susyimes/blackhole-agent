...
def builtin_evpnbum_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="evpnbum-bind-") as tmp:
        ...
        gate = assess_mission_selection(evpnbum, EVPNBUM_ACTUATION_GOAL, EVPNBUM_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(evpnbum)
    checks["exhausted_catalog_rejects_ledger_only_evpnbum"] = (
        not gate.accepted
        and live_goal != EVPNBUM_ACTUATION_GOAL
        and EVPNBUM_ACTUATION_ID not in live_done
