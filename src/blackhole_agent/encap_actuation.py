...
def builtin_encap_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="encap-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, ENCAP_ACTUATION_GOAL, ENCAP_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_encap"] = (
        not gate.accepted
        and live_goal != ENCAP_ACTUATION_GOAL
        and ENCAP_ACTUATION_ID not in live_done
