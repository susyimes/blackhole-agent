...
def builtin_tcp_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="tcp-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, TCP_ACTUATION_GOAL, TCP_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_tcp"] = (
        not gate.accepted
        and live_goal != TCP_ACTUATION_GOAL
        and TCP_ACTUATION_ID not in live_done
