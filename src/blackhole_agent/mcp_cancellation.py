...
def builtin_mcp_cancellation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="mcp-cancellation-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, MCP_CANCELLATION_GOAL, MCP_CANCELLATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_cancellation"] = (
        not gate.accepted
        and live_goal != MCP_CANCELLATION_GOAL
        and MCP_CANCELLATION_ID not in live_done
