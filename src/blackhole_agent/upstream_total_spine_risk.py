"""Post-solvency risk-versus-appetite for the absolute total spine.

Closes the solvent-but-unrisked cliff: after ``solvency_total_spine``
seals atomic SvR receipts, independently confirm a second solvency, book
each solvent pair into a risk register and pair it with appetite
(RvA), seal hash-chained atomic risk receipts bound to the solvency
digests, refuse split / one-sided / mismatched / failed / wrong-root /
tampered risks, short-circuit re-risk, and rebind the depth-28 tip
without skill-route.
"""
...
TOTAL_SPINE_RISK_IMPL = True
TOTAL_SPINE_RISK_KIND: str = "total_spine_risk"
TOTAL_SPINE_RISK_FILENAME: str = "total-spine-risk.json"
TOTAL_SPINE_RISK_MIN_RISKS: int = 2
...
class StageRefused(Exception):
    """A verdict-bearing refusal from total-spine risk."""
...
def _solvency_digest_of(row: Mapping[str, Any]) -> str:
    return str(
...
def _capability_list(row: Mapping[str, Any]) -> list[str]:
    ...
    if caps:
        return caps
    for leg in row.get("solvencies") or row.get("risks") or []:
...
def _rva_pairs(capabilities: Sequence[str]) -> list[dict[str, Any]]:
        ...
        rows.append(
            {
                ...
                "assessed_ok": bool(row.get("assessed_ok", True)),
                "appetite_ok": bool(row.get("appetite_ok", True)),
...
def _assert_pairs_atomic(pairs: Sequence[Mapping[str, Any]]) -> None:
    if not pairs:
        raise StageRefused(
            "total_spine_risk_pairs_empty",
            "risk refuses an empty LvC pair book",
        )
    for row in pairs:
        if not isinstance(row, Mapping):
            raise StageRefused(
                "total_spine_risk_partial",
                "risk refuses a malformed LvC pair",
            )
        assessed_ok = bool(row.get("assessed_ok", True))
        appetite_ok = bool(row.get("appetite_ok", True))
        if assessed_ok != appetite_ok or not bool(row.get("atomic_ok", True)):
            raise StageRefused(
                "total_spine_risk_partial",
                "risk refuses a split (non-atomic) solvency-versus-requirement pair",
            )
        if not assessed_ok or not appetite_ok:
            raise StageRefused(
                "total_spine_risk_partial",
                "risk refuses an unrisked or uncovered LvC pair",
            )


def _risk_certificate_material(body: Mapping[str, Any]) -> dict[str, Any]:
    """Canonical material for total-spine solvency certificate digests."""
    legs = body.get("risks") or body.get("legs") or []
    ...
    if isinstance(legs, list):
        for row in legs:
            ...
            collateral_rows.append(
                {
                    "solvency_index": int(row.get("solvency_index") or 0),
                    "solvency_height": int(row.get("solvency_height") or 0),
                    "solvency_digest": str(row.get("solvency_digest") or ""),
                    "bound_solvency_root": str(
                        row.get("bound_solvency_root") or ""
                    ),
                    "bound_delivery_root": str(
                    ...
                    "risked": bool(row.get("risked", True)),
                    "appetent": bool(row.get("appetent", True)),
                    "atomic_ok": bool(row.get("atomic_ok", True)),
                    ...
                    "parent_risk_root": str(
                        row.get("parent_risk_root") or ""
                    ),
                    "solvency_root": str(row.get("solvency_root") or ""),
                    "post_risk": bool(row.get("post_risk", True)),
                    "deterministic": bool(row.get("deterministic", True)),
                    "rva": bool(row.get("rva", True)),
                }
            )
    return {
        "schema_version": int(body.get("schema_version") or SCHEMA_VERSION),
        "kind": str(body.get("kind") or TOTAL_SPINE_RISK_KIND),
        ...
        "bound_solvency_root": str(body.get("bound_solvency_root") or ""),
        "bound_custody_root": str(body.get("bound_custody_root") or ""),
        "bound_delivery_root": str(body.get("bound_delivery_root") or ""),
        "solvency_digest": str(body.get("solvency_digest") or ""),
        "delivery_digest": str(body.get("delivery_digest") or ""),
        "parent_risk_root": str(body.get("parent_risk_root") or ""),
        "tip_risk_root": str(body.get("tip_risk_root") or ""),
        "risk_height": int(body.get("risk_height") or 0),
        "risk_count": int(body.get("risk_count") or 0),
        ...
        "risked": bool(body.get("risked", True)),
        "appetent": bool(body.get("appetent", True)),
        "atomic_ok": bool(body.get("atomic_ok", True)),
        "rva_ok": bool(body.get("rva_ok", True)),
        "one_sided": bool(body.get("one_sided", False)),
        "risks_ok": bool(body.get("risks_ok", True)),
        "risks_ok": bool(body.get("risks_ok", True)),
        "post_risk": bool(body.get("post_risk", True)),
        "deterministic": bool(body.get("deterministic", True)),
        "irreversible": bool(body.get("irreversible", True)),
        "success": bool(body.get("success", True)),
        "risks": collateral_rows,
    }


def compute_total_spine_risk_root(
    ...
    if not solvencies:
        return "0" * 64
    last = solvencies[-1]
    tip = str(last.get("solvency_root") or "").strip()
    if tip:
        return tip
    parent = ""
    for idx, row in enumerate(solvencies):
        body = {
            "solvency_index": int(row.get("solvency_index") or idx),
            "solvency_height": int(row.get("solvency_height") or (idx + 1)),
            "solvency_digest": str(row.get("solvency_digest") or ""),
            "bound_solvency_root": str(row.get("bound_solvency_root") or ""),
...
def book_total_spine_solvencies(
    ...
    *,
    ...
    want = max(int(min_risks), TOTAL_SPINE_RISK_MIN_RISKS)
    verified: list[Mapping[str, Any]] = []
    for raw in margins:
        if not isinstance(raw, Mapping):
            continue
        verify = verify_total_spine_solvency_certificate(raw)
        if not verify.get("ok"):
            raise StageRefused(
                "total_spine_risk_margin_tampered",
                "risk refuses a margin whose digest/chain does not verify",
            )
        if raw.get("solvent") is False or raw.get("success") is False:
            raise StageRefused(
                "total_spine_risk_capital_insolvent",
                "risk refuses an insolvent solvency receipt",
            )
        if raw.get("required") is False or raw.get("svr_ok") is False:
            raise StageRefused(
                "total_spine_risk_capital_unrequired",
                "risk refuses a solvency whose SvR is not complete",
            )
        if raw.get("atomic_ok") is False:
            raise StageRefused(
                "total_spine_risk_margin_partial",
                "risk refuses a non-atomic margin receipt",
            )
        if int(raw.get("residual") or 0) != 0:
            raise StageRefused(
                "total_spine_risk_residual",
                "risk refuses a margin with a non-zero residual",
            )
        verified.append(raw)
    if len(verified) < want:
        raise StageRefused(
            "total_spine_risk_margins_short",
            f"solvency requires >= {want} independent solvencies, "
            f"got {len(verified)}",
        )
    ...
    if not book_state or not book_action or not book_actuation:
        raise StageRefused(
            "total_spine_risk_root_missing",
            "solvency requires liquidity bound state/action/actuation roots",
        )
    if not book_caps:
        raise StageRefused(
            "total_spine_risk_pairs_empty",
            "risk refuses a liquidity with no funded capabilities",
        )

    legs: list[dict[str, Any]] = []
    parent = str(parent_risk_root or "")
    for idx, margin in enumerate(verified):
        ...
        if (
            ...
            raise StageRefused(
                "total_spine_risk_root_mismatch",
                "risk refuses collaterals bound to different "
                "state/action/actuation roots",
            )
        if settlement and book_settlement and settlement != book_settlement:
            raise StageRefused(
                "total_spine_risk_root_mismatch",
                "risk refuses collaterals bound to different settlement roots",
            )
        if clearing and book_clearing and clearing != book_clearing:
            raise StageRefused(
                "total_spine_risk_root_mismatch",
                "risk refuses collaterals bound to different clearing roots",
            )
        sig = _book_signature(margin)
        if sig != book_sig:
            raise StageRefused(
                "total_spine_risk_svr_failed",
                "independent liquidity books disagree; LvC cannot complete",
            )
        caps = tuple(_capability_list(margin))
        if caps != book_caps:
            raise StageRefused(
                "total_spine_risk_one_sided",
                "risk refuses one-sided books whose capability sets differ",
            )
        pairs = _rva_pairs(book_caps)
        _assert_pairs_atomic(pairs)
        height = (
            int(risk_height) + idx
            if risk_height is not None
            else (idx + 1)
        )
        material = {
            "solvency_index": idx,
            "solvency_height": height,
            "solvency_digest": _solvency_digest_of(margin),
            "bound_solvency_root": str(
                margin.get("tip_solvency_root") or ""
            ),
            "bound_delivery_root": str(
                margin.get("bound_delivery_root")
                or margin.get("tip_delivery_root")
        ...
        solvency_root = _sha256_json(material)
        row = dict(material)
        row["solvency_root"] = solvency_root
        row["pairs"] = pairs
        row["schema_version"] = SCHEMA_VERSION
        legs.append(row)
        parent = solvency_root
    return legs


def seal_total_spine_risk_certificate(
    body: Mapping[str, Any],
) -> dict[str, Any]:
    """Seal post-risk LvC log into a tamper-evident receipt."""
    sealed_body = dict(body)
    risks = list(sealed_body.get("risks") or [])
    if not str(sealed_body.get("tip_risk_root") or "").strip():
        sealed_body["tip_risk_root"] = compute_total_spine_risk_root(
            risks
        )
    if not int(sealed_body.get("risk_count") or 0):
        sealed_body["risk_count"] = len(risks)
    if not int(sealed_body.get("risk_height") or 0):
        sealed_body["risk_height"] = len(risks)
    material = _risk_certificate_material(sealed_body)
    material["tip_risk_root"] = str(sealed_body.get("tip_risk_root") or "")
    digest = _sha256_json(material)
    sealed = dict(material)
    sealed["risk_digest"] = digest
    sealed["certificate_hash"] = digest
    sealed["total_spine_risk"] = True
    sealed["total_spine_risk_impl"] = TOTAL_SPINE_RISK_IMPL
    sealed["risked_at"] = str(body.get("risked_at") or utc_now_iso())
    sealed["used_skill_route_discovery"] = legacy_pipeline_was_used()
    if risks:
        sealed_pairs: list[Any] = []
        for src, dest in zip(risks, sealed.get("risks") or []):
            if isinstance(src, Mapping) and isinstance(dest, dict):
                if src.get("pairs"):
                    dest["pairs"] = list(src.get("pairs") or [])
                sealed_pairs.append(dest)
        if sealed_pairs:
            sealed["risks"] = sealed_pairs
    return sealed


def risk_certificate_path(root: Path) -> Path:
    """Resolve ``total-spine-risk.json`` under a "capital/"out root."""
    path = Path(root)
    if path.is_file():
        if path.name == TOTAL_SPINE_RISK_FILENAME or path.suffix == ".json":
            try:
                probe = json.loads(path.read_text(encoding="utf-8"))
            except (OSError, json.JSONDecodeError):
                probe = None
            if isinstance(probe, Mapping) and (
                str(probe.get("kind") or "") == TOTAL_SPINE_RISK_KIND
                or path.name == TOTAL_SPINE_RISK_FILENAME
            ):
                return path
        parent = path.parent
        sibling = parent / TOTAL_SPINE_RISK_FILENAME
        if sibling.is_file():
            return sibling
        nested = parent / "risk" / TOTAL_SPINE_RISK_FILENAME
        if nested.is_file():
            return nested
        grand = parent.parent / "risk" / TOTAL_SPINE_RISK_FILENAME
        if grand.is_file():
            return grand
        grand_sib = parent.parent / TOTAL_SPINE_RISK_FILENAME
        if grand_sib.is_file():
            return grand_sib
        return parent / "risk" / TOTAL_SPINE_RISK_FILENAME
    named = path / TOTAL_SPINE_RISK_FILENAME
    if named.is_file():
        return named
    nested = path / "risk" / TOTAL_SPINE_RISK_FILENAME
    if nested.is_file():
        return nested
    return path / "risk" / TOTAL_SPINE_RISK_FILENAME


def write_total_spine_risk_certificate(
    out_root: Path,
    body: Mapping[str, Any],
    *,
    ...
    sealed = seal_total_spine_risk_certificate(body)
    path = risk_certificate_path(Path(out_root))
    if path.is_file():
        try:
            existing = load_total_spine_risk_certificate(path)
        except StageRefused:
            existing = None
        if existing is not None:
            existing_digest = str(
                existing.get("risk_digest")
                or existing.get("certificate_hash")
                or ""
            )
            new_digest = str(
                sealed.get("risk_digest")
            ...
            if (
                ...
                existing["risk_path"] = str(path)
                existing["total_spine_risk_idempotent"] = True
                return existing
            raise StageRefused(
                "total_spine_risk_supersession_refused",
                f"irreversible solvency already sealed at {path} "
                f"(existing={existing_digest!r} attempted={new_digest!r})",
            )
    path.parent.mkdir(parents=True, exist_ok=True)
    atomic_write_json(path, sealed)
    sealed["risk_path"] = str(path)
    sealed["total_spine_risk_idempotent"] = False
    return sealed


def verify_total_spine_risk_certificate(
    certificate: Mapping[str, Any],
) -> dict[str, Any]:
    """Recompute collateral digest and LvC roots; fail closed on tamper."""
    claimed = str(
        certificate.get("risk_digest")
        or certificate.get("certificate_hash")
        or ""
    )
    material = _risk_certificate_material(certificate)
    expected = _sha256_json(material)
    risks = list(certificate.get("risks") or [])
    recomputed_tip = compute_total_spine_risk_root(risks)
    claimed_tip = str(certificate.get("tip_risk_root") or "")
    height = int(certificate.get("risk_height") or 0)
    count = int(certificate.get("risk_count") or 0)
    ...
    cert_parent = str(certificate.get("parent_risk_root") or "")
    chain_ok = True
    parent = cert_parent
    book_sig = ""
    for idx, row in enumerate(risks):
        ...
        if str(row.get("actuation_digest") or "") != actuation_digest:
            chain_ok = False
            break
        if str(row.get("parent_risk_root") or "") != parent:
            chain_ok = False
            break
        ...
        if str(row.get("solvency_root") or "") != expected_root:
            chain_ok = False
            break
    ...
    min_ok = count >= TOTAL_SPINE_RISK_MIN_RISKS and height >= count
    risks_ok = all(
        isinstance(row, Mapping)
        and bool(row.get("risked", True))
        and bool(row.get("appetent", True))
        and bool(row.get("atomic_ok", True))
        and bool(row.get("rva", True))
    ...
    ok = (
        ...
        and str(certificate.get("kind") or "") == TOTAL_SPINE_RISK_KIND
        and int(certificate.get("schema_version") or 0) == SCHEMA_VERSION
        and certificate.get("irreversible") is True
        and certificate.get("post_risk") is True
        and certificate.get("deterministic") is True
        and certificate.get("risked") is True
        and certificate.get("appetent") is True
        and certificate.get("atomic_ok") is True
        and certificate.get("rva_ok") is True
    ...
    return {
        "ok": ok,
        "action": "verify_total_spine_risk",
        ...
        "recomputed_tip_risk_root": recomputed_tip,
        "chain_ok": chain_ok,
        "min_risks_ok": min_ok,
        "risks_ok": risks_ok,
        "rva_ok": certificate.get("rva_ok") is True,
        "atomic_ok": certificate.get("atomic_ok") is True,
        "kind_ok": str(certificate.get("kind") or "") == TOTAL_SPINE_RISK_KIND,
        "schema_ok": int(certificate.get("schema_version") or 0) == SCHEMA_VERSION,
        "irreversible_ok": certificate.get("irreversible") is True,
        "risked_ok": certificate.get("risked") is True,
        "appetent_ok": certificate.get("appetent") is True,
        "total_spine_risk": True,
        "used_skill_route_discovery": legacy_pipeline_was_used(),
    }


def load_total_spine_risk_certificate(
    path: Path | str,
) -> dict[str, Any]:
    """Load and integrity-check a sealed liquidity receipt."""
    file_path = risk_certificate_path(Path(path))
    if not file_path.is_file():
        raise StageRefused(
            "total_spine_risk_missing",
            f"solvency certificate not found at {file_path}",
        )
    ...
    try:
        ...
        raise StageRefused(
            "total_spine_risk_unreadable",
            f"solvency certificate unreadable at {file_path}: {exc}",
        ) from exc
    if not isinstance(payload, Mapping):
        raise StageRefused(
            "total_spine_risk_invalid",
            "solvency certificate root must be a JSON object",
        )
    if str(payload.get("kind") or "") != TOTAL_SPINE_RISK_KIND and not payload.get(
        "total_spine_risk"
    ):
        raise StageRefused(
            "total_spine_risk_missing",
    ...
    verify = verify_total_spine_risk_certificate(payload)
    if not verify.get("ok"):
        raise StageRefused(
            "total_spine_risk_tampered",
            f"solvency certificate digest mismatch at {file_path} "
            f"(claimed={verify.get('claimed_digest')!r} "
            f"expected={verify.get('expected_digest')!r})",
        )
    body = dict(payload)
    body["risk_path"] = str(file_path)
    body["risk_verify"] = verify
    body["total_spine_risk_loaded"] = True
    return body


def seal_total_spine_risk_chain(
    *,
    prior_tip: str,
    risk_digest: str,
    tip_risk_root: str,
    ...
    """Seal solvency hop into the absolute-tower tip."""
    tip = str(prior_tip or "").strip() or ("0" * 64)
    md = str(risk_digest or "").strip() or ("0" * 64)
    mr = str(tip_risk_root or "").strip() or ("0" * 64)
    cr = str(bound_solvency_root or "").strip() or ("0" * 64)
    ...
    cd = str(solvency_digest or "").strip() or ("0" * 64)
    dvd = str(delivery_digest or "").strip() or ("0" * 64)
    material = (
        f"risk|{int(bool(short_circuit))}|{int(risk_height)}|"
...
def annotate_total_spine_risk(
    ...
    *,
    ...
    """Stamp post-risk LvC onto a total-spine result and rebind tip."""
    cst_digest = str(
        certificate.get("risk_digest")
        or certificate.get("certificate_hash")
        or ""
    )
    tip_risk_root = str(certificate.get("tip_risk_root") or "")
    risk_height = int(certificate.get("risk_height") or 0)
    risk_count = int(certificate.get("risk_count") or 0)
    ...
    bound_solvency_root = str(certificate.get("bound_solvency_root") or "")
    bound_delivery_root = str(certificate.get("bound_delivery_root") or "")
    actuation_digest = str(certificate.get("actuation_digest") or "")
    solvency_digest = str(certificate.get("solvency_digest") or "")
    delivery_digest = str(certificate.get("delivery_digest") or "")
    chain = seal_total_spine_risk_chain(
        prior_tip=prior_tip,
        risk_digest=cst_digest,
        tip_risk_root=tip_risk_root,
        ...
        solvency_digest=solvency_digest,
        delivery_digest=delivery_digest,
        risk_height=risk_height,
        short_circuit=short_circuit,
    )
    cst_tip = str(chain.get("digest") or prior_tip)
    bound = _sha256_bytes(f"{prior_tip}|{cst_tip}".encode("utf-8"))
    body["total_spine_risk"] = True
    body["total_spine_risk_impl"] = TOTAL_SPINE_RISK_IMPL
    body["total_spine_risk_short_circuit"] = bool(short_circuit)
    body["total_spine_risk_irreversible"] = True
    body["total_spine_risk_post_risk"] = True
    body["total_spine_risk_deterministic"] = True
    body["total_spine_risk_rva"] = True
    body["total_spine_risk_certificate"] = dict(certificate)
    body["total_spine_risk_digest"] = cst_digest
    body["total_spine_risk_chain"] = chain
    body["total_spine_risk_tip"] = cst_tip
    body["total_spine_risk_bound_tip"] = bound
    body["total_spine_digest_pre_solvency"] = prior_tip
    body["total_spine_tip_risk_root"] = tip_risk_root
    body["total_spine_risk_height"] = risk_height
    body["total_spine_risk_count"] = risk_count
    body["total_spine_risked"] = bool(certificate.get("risked", True))
    body["total_spine_risked_ok"] = bool(certificate.get("risked", True))
    body["total_spine_required"] = bool(certificate.get("appetent", True))
    body["total_spine_appetent_ok"] = bool(certificate.get("appetent", True))
    body["total_spine_rva_ok"] = bool(certificate.get("rva_ok", True))
    body["total_spine_risk_atomic"] = bool(certificate.get("atomic_ok", True))
    body["total_spine_risk_one_sided"] = bool(
        certificate.get("one_sided", False)
    )
    body["total_spine_risks_ok"] = bool(
        certificate.get("risks_ok", True)
    )
    body["total_spine_risk_root_valid"] = bool(tip_risk_root)
    body["total_spine_risk_residual"] = int(certificate.get("residual") or 0)
    body["total_spine_risk_pair_count"] = int(
        certificate.get("pair_count") or 0
    )
    body["risk_root"] = tip_risk_root
    body["tip_risk_root"] = tip_risk_root
    body["risk_count"] = risk_count
    body["risk_height"] = risk_height
    body["risked"] = bool(certificate.get("risked", True))
    body["risked_ok"] = bool(certificate.get("risked", True))
    body["risk_ok"] = bool(certificate.get("risked", True))
    body["rva_ok"] = bool(certificate.get("rva_ok", True))
    body["appetent"] = bool(certificate.get("appetent", True))
    if certificate.get("risk_path"):
        body["total_spine_risk_path"] = certificate.get("risk_path")
    if bound_state_root:
    ...
    if bound_solvency_root:
        body["total_spine_tip_solvency_root"] = bound_solvency_root
        body["solvency_root"] = bound_solvency_root
        body["tip_solvency_root"] = bound_solvency_root
        body.setdefault("total_spine_solvency", True)
        body.setdefault("total_spine_solvent", True)
        body.setdefault("total_spine_svr_ok", True)
    ...
    if solvency_digest:
        body["total_spine_solvency_digest"] = solvency_digest
    if delivery_digest:
    ...
    body["verdict"] = (
        "total_spine_risk_ok_short_circuit"
        if short_circuit
        else "total_spine_risk_ok"
    )
    body["ok"] = True
    return body
...
def _as_solvency_mapping(value: Any) -> dict[str, Any] | None:
    from blackhole_agent.upstream_total_spine_solvency import (
        StageRefused as SolvencyRefused,
        load_total_spine_solvency_certificate,
    )

    if isinstance(value, Mapping):
        kind = str(value.get("kind") or "")
        if kind == TOTAL_SPINE_RISK_KIND or value.get("total_spine_risk"):
            nested_liq = value.get("total_spine_solvency_certificate")
        ...
        ) or value.get("total_spine_solvency_loaded") or value.get(
            "tip_solvency_root"
        ):
    ...
    try:
        probe_path = path
        if path.is_dir():
            nested = path / "solvency" / "total-spine-capital.json"
        ...
        if probe_path.is_file():
            ...
            if isinstance(probe, Mapping):
                ...
                if kind and kind != TOTAL_SPINE_SOLVENCY_KIND:
                    return None
    except (OSError, json.JSONDecodeError):
        pass
    try:
        return load_total_spine_solvency_certificate(path)
    except SolvencyRefused as exc:
...
def _as_risk_mapping(value: Any) -> dict[str, Any] | None:
    if isinstance(value, Mapping):
        ...
        if kind == TOTAL_SPINE_RISK_KIND or value.get(
            "total_spine_risk"
        ) or value.get("total_spine_risk_loaded"):
    ...
    if value is None:
        return None
    try:
        return load_total_spine_risk_certificate(Path(str(value)))
...
def _confirm_solvency(
    ...
    *,
    ...
    """Independently re-solvency the same book as a confirmation side."""
    from blackhole_agent.upstream_total_spine_solvency import solvency_total_spine
    ...
    # MAX_PATH. The confirmation solvency is sealed in-memory.
    confirm_out = None
    tip_solvency = str(primary.get("tip_solvency_root") or "")
    sol_height = int(primary.get("solvency_height") or 0)
    ...
    if not capitals and not bundle and not confirm_body:
        raise StageRefused(
            "total_spine_risk_confirmation_missing",
            "single solvency requires capitals, collaterals, margins, "
            "custodies, deliveries, clearings, settlements, or actuation "
            "to confirm-risk",
        )
    source: Any = capitals[0] if len(capitals) == 1 else (capitals or bundle)
    confirmed = solvency_total_spine(
        source,
        capitals=capitals or None,
        ...
        solvency_height=sol_height + 1 if sol_height else None,
        repo_path=repo_path or REPO_ROOT,
        confirm=True,
    )
    cert = confirmed.get("total_spine_solvency_certificate")
    if not isinstance(cert, Mapping):
        raise StageRefused(
            "total_spine_risk_confirmation_missing",
            "confirmation solvency did not produce a certificate",
        )
    return dict(cert)


def _collect_solvencies(
    ...
    existing = _as_risk_mapping(source)
    if existing is None and body is not None:
        existing = _as_risk_mapping(body)
    solvencies: list[dict[str, Any]] = []
    ...
    def _push(item: Any) -> None:
        mapped = _as_solvency_mapping(item)
        if mapped is not None:
            solvencies.append(mapped)
    ...
    if existing is None:
        if isinstance(source, Sequence) and not isinstance(
            ...
            _push(source)
    if body is not None:
        _push(body.get("total_spine_solvency_certificate"))
    ...
    for row in solvencies:
        digest = _solvency_digest_of(row)
        tip = str(row.get("tip_solvency_root") or "")
        key = digest or tip
        if not key or key in seen:
            continue
        seen.add(key)
...
def _strip_risk_predicates(done_when: str) -> str:
    """Evaluate the pre-risk contract, never solvency_* (no recurse)."""
...
def risk_total_spine(
    ...
    *,
    ...
    """Apply post-risk atomic RvA risk on the absolute total spine."""
    ...
    if not TOTAL_SPINE_RISK_IMPL:
        raise StageRefused(
            "total_spine_risk_disabled",
            "TOTAL_SPINE_RISK_IMPL is False",
        )

    extra_books: list[Any] = []
    extra_books.extend(list(solvencies or []))
    ...
    (
        ...
        found_settlements,
        found_actuation,
    ) = _collect_solvencies(source, body, extra_books)
    ...
    if (
        ...
        and existing.get("tip_risk_root")
        and (
            str(existing.get("kind") or "") == TOTAL_SPINE_RISK_KIND
            or existing.get("total_spine_risk_loaded")
            or existing.get("total_spine_risk")
        )
    ):
        tip = str(
            prior_tip
        ...
        result = body if body is not None else {
            "ok": True,
            "action": "risk_total_spine",
            "total_spine": True,
        }
        return annotate_total_spine_risk(
            result,
    ...
    if len(collected) < want and confirm and collected:
        collected.append(
            _confirm_solvency(
                ...
                prior_tip=str(
                    prior_tip
                    or (body or {}).get("total_spine_solvency_bound_tip")
                    or (body or {}).get("total_spine_digest")
                    or ""
                ),
                repo_path=repo_path,
    ...
    legs = book_total_spine_solvencies(
        collected,
        min_risks=want,
        parent_risk_root=parent_risk_root,
        risk_height=risk_height,
    )
    first = collected[0]
    ...
    margin_root = str(first.get("tip_solvency_root") or "")
    solvency_digest = _solvency_digest_of(first)
    root_layer = str(
    ...
    pre_liquidity = _strip_risk_predicates(done_when)
    if pre_liquidity:
        ctx = {
            "liquidity": {
                "ok": True,
        ...
        if contract_machine and contract_met is not True:
            raise StageRefused(
                "total_spine_risk_contract_unmet",
                f"done_when not met at collateral: {pre_liquidity!r}",
            )

    tip_risk_root = compute_total_spine_risk_root(legs)
    cst_height = int(legs[-1]["solvency_height"]) if legs else 0
    tip = str(
        prior_tip
        or (body or {}).get("total_spine_solvency_bound_tip")
        or (body or {}).get("total_spine_digest")
    ...
    cst_body: dict[str, Any] = {
        ...
        "bound_solvency_root": margin_root,
        ...
        "parent_risk_root": str(
            parent_risk_root
            or (legs[0].get("parent_risk_root") if legs else "")
            or ""
        ),
        "risks": legs,
        "risk_count": len(legs),
        "risk_height": cst_height,
        "tip_risk_root": tip_risk_root,
    ...
    if write_target is not None:
        certificate = write_total_spine_risk_certificate(write_target, cst_body)
    else:
        certificate = seal_total_spine_risk_certificate(cst_body)
    ...
    if annotated.get("total_spine_compressed") and root_layer:
        ...
        cst_bound = str(annotated.get("total_spine_risk_bound_tip") or tip)
    ...
    annotated["total_spine_risk_bound_state_root"] = state_root
    annotated["total_spine_risk_bound_action_root"] = action_root
    annotated["total_spine_risk_bound_settlement_root"] = settlement_root
    annotated["total_spine_risk_bound_clearing_root"] = clearing_root
    annotated["total_spine_risk_bound_solvency_root"] = margin_root
    annotated["total_spine_risk_actuation_digest"] = actuation_digest
    annotated["used_skill_route_discovery"] = legacy_pipeline_was_used()
    return annotated


def builtin_total_spine_risk_proof() -> dict[str, Any]:
    """Hermetic proof: post-risk atomic RvA on the absolute tower."""
    import shutil
    import tempfile
    ...
    from blackhole_agent.upstream_control_engine import (
        ...
        TOTAL_SPINE_RISK_IMPL as ENGINE_CAP_IMPL,
    ...
    scratch = Path(tempfile.mkdtemp(prefix="total-spine-risk-proof-"))
    try:
        from blackhole_agent import upstream_loop_engine as le_facade

        flags_ok = (
            TOTAL_SPINE_RISK_IMPL is True
            ...
            and TOTAL_SPINE_RISK_KIND == "total_spine_risk"
            and bool(TOTAL_SPINE_RISK_FILENAME)
            and TOTAL_SPINE_RISK_MIN_RISKS >= 2
        )
        ...
        cap_path = offline_cap.get("total_spine_risk_path")
        tip_risk = str(offline_cap.get("total_spine_tip_risk_root") or "")
        offline_ok = (
            bool(offline_cap.get("ok"))
            and offline_cap.get("total_spine_risk") is True
            and offline_cap.get("total_spine_risk_post_risk") is True
            and offline_cap.get("total_spine_risk_irreversible") is True
            and offline_cap.get("total_spine_risked") is True
            and offline_cap.get("total_spine_required") is True
            and offline_cap.get("total_spine_rva_ok") is True
            and offline_cap.get("total_spine_risk_atomic") is True
            and offline_cap.get("total_spine_risk_one_sided") is False
            and int(offline_cap.get("total_spine_risk_count") or 0) >= 2
            and int(offline_cap.get("total_spine_risk_height") or 0) >= 2
            and int(offline_cap.get("total_spine_risk_residual") or 0) == 0
            and int(offline_cap.get("total_spine_risk_pair_count") or 0) >= 1
            and len(tip_risk) >= 32
        ...
        loaded = load_total_spine_risk_certificate(cap_path or scratch)
        verify_ok = bool(
            loaded.get("total_spine_risk_loaded")
            and (loaded.get("risk_verify") or {}).get("ok")
            and (loaded.get("risk_verify") or {}).get("risk_root_ok")
            and (loaded.get("risk_verify") or {}).get("chain_ok")
            and (loaded.get("risk_verify") or {}).get("risks_ok")
            and (loaded.get("risk_verify") or {}).get("rva_ok")
        )

        tampered_path = scratch / "tampered-risk.json"
        ...
        tampered_body["risk_height"] = 99
        atomic_write_json(tampered_path, tampered_body)
        tamper_ok = False
        try:
            load_total_spine_risk_certificate(tampered_path)
        except StageRefused as exc:
            tamper_ok = str(exc.verdict) == "total_spine_risk_tampered"
        except Exception:  # noqa: BLE001
            tamper_ok = False
        ...
        try:
            ...
            supersession_ok = (
                str(exc.verdict) == "total_spine_risk_supersession_refused"
            )
        except Exception:  # noqa: BLE001
            supersession_ok = False
        ...
        resealed = seal_total_spine_risk_certificate(wrong_body)
        wrong_verify = verify_total_spine_risk_certificate(resealed)
        wrong_root_ok = wrong_verify.get("ok") is False and (
            wrong_verify.get("chain_ok") is False
            or wrong_verify.get("risk_root_ok") is False
        )

        mismatch_ok = False
        try:
            mixed = dict(cert_sol1)
        ...
        try:
            ...
            resealed_one = seal_total_spine_solvency_certificate(forged)
            book_total_spine_solvencies(
                [cert_sol1, resealed_one], min_risks=2
            )
        except StageRefused as exc:
            one_sided_ok = str(exc.verdict) == "total_spine_risk_one_sided"
        except Exception:  # noqa: BLE001
            one_sided_ok = False
        ...
        try:
            ...
            partial_ok = str(exc.verdict) == "total_spine_risk_partial"
        except Exception:  # noqa: BLE001
            partial_ok = False
        ...
        h2 = risk_total_spine(
            ...
            prior_tip=str(
                offline_cap.get("total_spine_risk_bound_tip") or ""
            ),
            parent_risk_root=tip_risk,
            risk_height=int(
                offline_cap.get("total_spine_risk_height") or 0
            )
            + 1,
            repo_path=REPO_ROOT,
            confirm=False,
        ...
        multi_height_ok = (
            bool(h2.get("ok"))
            and int(h2.get("total_spine_risk_count") or 0) >= 2
            and str(h2.get("total_spine_tip_risk_root") or "") != tip_risk
            and str(
                (h2.get("total_spine_risk_certificate") or {}).get(
                    "parent_risk_root"
                )
                or ""
            )
        ...
        recomputed = compute_total_spine_risk_root(
            loaded.get("risks") or []
        )
        determinism_ok = recomputed == tip_risk and bool(recomputed)

        live = run_total_spine(
            root_layer="quettacontinuum",
            out_root=scratch / "live-rsk",
            max_rounds=1,
            dispatch=True,
            dispatch_budget=2,
            max_successions=1,
        ...
        live_cap_path = live.get("total_spine_risk_path")
        live_ok = (
            bool(live.get("ok"))
            ...
            and live.get("total_spine_risked") is True
            and live.get("total_spine_rva_ok") is True
            and int(live.get("total_spine_risk_count") or 0) >= 2
            and isinstance(live.get("total_spine_tip_risk_root"), str)
            and len(str(live.get("total_spine_tip_risk_root"))) >= 32
        ...
        shorted = run_total_spine(
            ...
            solvency=True,
            risk=True,
            resume_dir=live_cap_path or (scratch / "live-rsk"),
            effect_timeout=90,
            repo_path=REPO_ROOT,
        )
        short_ok = (
            bool(shorted.get("ok"))
            and shorted.get("total_spine_risk") is True
            and shorted.get("total_spine_risk_short_circuit") is True
            and str(shorted.get("total_spine_tip_risk_root") or "")
            == str(live.get("total_spine_tip_risk_root") or "")
            and int(shorted.get("total_nest_depth") or 0) == 28
            and not legacy_pipeline_was_used()
        )

        cap_chain = live.get("total_spine_risk_chain") or {}
        chain_integrity_ok = False
        if isinstance(cap_chain, Mapping) and cap_chain:
            re_seal = seal_total_spine_risk_chain(
                prior_tip=str(cap_chain.get("prior_tip") or ""),
                risk_digest=str(cap_chain.get("risk_digest") or ""),
                tip_risk_root=str(cap_chain.get("tip_risk_root") or ""),
                bound_solvency_root=str(
                    cap_chain.get("bound_solvency_root") or ""
                ),
                bound_delivery_root=str(
                ...
                risk_height=int(cap_chain.get("risk_height") or 0),
                short_circuit=bool(cap_chain.get("short_circuit")),
            )
            chain_integrity_ok = (
                re_seal.get("digest") == cap_chain.get("digest")
                and re_seal.get("digest") == live.get("total_spine_risk_tip")
            )
        ...
        source_ok = (
            getattr(le_facade, "TOTAL_SPINE_RISK_IMPL", None) is TOTAL_SPINE_RISK_IMPL
            and getattr(le_facade, "builtin_total_spine_risk_proof", None) is builtin_total_spine_risk_proof
            and getattr(le_facade, "risk_total_spine", None) is risk_total_spine
            and callable(
                getattr(le_facade, "builtin_total_spine_risk_proof", None)
    
        )
            and callable(getattr(le_facade, "risk_total_spine", None))
            and getattr(le_facade, "TOTAL_SPINE_RISK_IMPL", False) is True
        )
        ...
        engine_source_ok = (
            "TOTAL_SPINE_RISK_IMPL" in engine_text
            and "risk_total_spine" in engine_text
            and (
                "risk=True" in engine_text
                or "risk: bool = False" in engine_text
            )
            and "builtin_total_spine_risk_proof" in engine_text
        )

        mod_path = Path(__file__).resolve()
        mod_text = mod_path.read_text(encoding="utf-8")
        mod_source_ok = (
            "def risk_total_spine" in mod_text
            and "def builtin_total_spine_risk_proof" in mod_text
            and "total_spine_risk_supersession_refused" in mod_text
            and "total_spine_risk_tampered" in mod_text
            and "total_spine_risk_one_sided" in mod_text
            and "total_spine_risk_svr_failed" in mod_text
        )
        ...
        try:
            ...
            ledger_ok = (
                entry is not None
                and (
                    "upstream_total_spine_risk" in (entry.entry or "")
                    or "upstream_control_engine" in (entry.entry or "")
                )
                and "builtin_total_spine_risk_proof" in (entry.entry or "")
                and (
                    "risk" in tags_blob
                    or "risk" in name_blob
                    or "risk" in delta_blob
                )
                and ("total" in tags_blob or "total" in name_blob)
                and (
                    "risk_total_spine" in delta_blob
                    or "post-risk" in delta_blob
                    or "post_risk" in delta_blob
                    or "svr" in delta_blob
                )
            )
        except Exception:  # noqa: BLE001
            ledger_ok = False
        ...
        return {
            ...
            "state_root": state_root,
            "risk_count": offline_cap.get("total_spine_risk_count"),
            "pair_count": offline_cap.get("total_spine_risk_pair_count"),
            "verify_ok": verify_ok,
            "tamper_ok": tamper_ok,
            ...
            "live_ok": live_ok,
            "live_risk_path": live_cap_path,
            "live_tip_risk_root": live.get("total_spine_tip_risk_root"),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    sub.add_parser("proof", help="Alias for risk-proof")
    args = parser.parse_args(list(argv) if argv is not None else None)
    if args.cmd in {"risk-proof", "proof"}:
