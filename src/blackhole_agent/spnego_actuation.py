...
def builtin_spnego_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="spnego-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, SPNEGO_ACTUATION_GOAL, SPNEGO_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_spnego"] = (
        not gate.accepted
        and live_goal != SPNEGO_ACTUATION_GOAL
        and SPNEGO_ACTUATION_ID not in live_done
