...
def _continue_resumed_follow_ons(state: Any, durable: Path) -> None:
    ...
    try:
        ...
        pass
    try:
        from blackhole_agent.kernel_compound_loop import continue_resumed_compound_loop

        continue_resumed_compound_loop(state, repo_path=durable)
    except Exception:  # noqa: BLE001 - resume must still return bound fields
        pass
    try:
        from blackhole_agent.kernel_primitive_compose import continue_resumed_primitive_compose

        continue_resumed_primitive_compose(state, repo_path=durable)
    except Exception:  # noqa: BLE001 - resume must still return bound fields
        pass
    try:
        from blackhole_agent.kernel_composed_program import continue_resumed_composed_program

        continue_resumed_composed_program(state, repo_path=durable)
    except Exception:  # noqa: BLE001 - resume must still return bound fields
        pass
    try:
        from blackhole_agent.kernel_program_stack import continue_resumed_program_stack
    ...
    try:
        ...
        pass
    try:
        from blackhole_agent.kernel_program_lattice import continue_resumed_program_lattice

        continue_resumed_program_lattice(state, repo_path=durable)
    except Exception:  # noqa: BLE001 - resume must still return bound fields
        pass
    try:
        from blackhole_agent.kernel_program_fabric import continue_resumed_program_fabric

        continue_resumed_program_fabric(state, repo_path=durable)
    except Exception:  # noqa: BLE001 - resume must still return bound fields
        pass
    try:
        from blackhole_agent.kernel_program_weave import continue_resumed_program_weave

        continue_resumed_program_weave(state, repo_path=durable)
    except Exception:  # noqa: BLE001 - resume must still return bound fields
        pass
