...
def builtin_mcp_tools_list_changed_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="mcp-tools-changed-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, MCP_TOOLS_CHANGED_GOAL, MCP_TOOLS_CHANGED_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_tools_changed"] = (
        not gate.accepted
        and live_goal != MCP_TOOLS_CHANGED_GOAL
        and MCP_TOOLS_CHANGED_ID not in live_done
