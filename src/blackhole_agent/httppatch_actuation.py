...
def builtin_httppatch_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="httppatch-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, HTTPPATCH_ACTUATION_GOAL, HTTPPATCH_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_httppatch"] = (
        not gate.accepted
        and live_goal != HTTPPATCH_ACTUATION_GOAL
        and HTTPPATCH_ACTUATION_ID not in live_done
