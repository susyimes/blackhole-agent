"""Upstream ronnacontinuum plane: multi-yottacontinuum durable stewardship ronnacontinuum.

The yottacontinuum plane (``upstream_yottacontinuum``) closes multi-continuum unions *within one
yottacontinuum*. It does not:

1. chain multiple independent yottacontinuums under a durable ronnacontinuum constitution;
2. allocate a shared global dispatch budget across yottacontinuums by ROI;
3. admit/retire yottacontinuum slots from an ronnacontinuum charter over time
   (deferred admission under a concurrent-active cap);
4. grow the ronnacontinuum charter mid-run via ``charter_expand`` (constitution
   growth beyond the initial charter, not just deferred admission of a fixed set);
5. federate multi-yottacontinuum portfolio coverage into one ronnacontinuum world-model;
6. persist ronnacontinuum state so a later process can resume the union;
7. seal a multi-yottacontinuum ronnacontinuum chronicle linking yottacontinuum digests.

The ronnacontinuum plane closes that outer multi-yottacontinuum loop:

1. **admit** — materialize yottacontinuum slots from a durable ronnacontinuum charter
   (each slot owns a nested continuum charter). When ``max_active_yottacontinuums``
   is set, only that many *unmet* yottacontinuums are concurrent: further charter
   slots stay pending and are admitted as capacity frees after retirements
   (ronnacontinuum constitution growth over time);
2. **schedule** — pick the next open yottacontinuum by priority and historical ROI;
3. **yottacontinuum** — call the yottacontinuum plane (injected ``yottacontinuum_runner``;
   default ``run_yottacontinuum``) with a share of the remaining global dispatch
   budget;
4. **federate** — merge per-yottacontinuum portfolios into one ronnacontinuum world-model
   and re-score coverage across all stewarded keys;
5. **retire** — mark yottacontinuums met when their yottacontinuum_goal is satisfied,
   then re-admit pending charter slots up to the active capacity;
6. **expand** — optional ``charter_expand`` may append new yottacontinuum slots when
   the active charter has no pending work and all admitted yottacontinuums are met,
   so the ronnacontinuum constitution can grow after start (not only defer a fixed charter);
7. **persist** — write ``ronnacontinuum_state.json`` after every yottacontinuum round so a
   later ``run_ronnacontinuum(..., resume_dir=...)`` continues the same union
   (including pending charter and admission history);
8. **stop** when any of:

   - ``max_rounds`` reached
   - global ``dispatch_budget`` exhausted across yottacontinuums
   - ronnacontinuum goal met (``all_yottacontinuums_met``: every *admitted*
     yottacontinuum is met *and* no pending charter slots remain)
   - consecutive idle/no-progress rounds (``idle_round_limit``)
   - explicit ``stop_when`` predicate returns a reason string

9. **seal** — write an ronnacontinuum receipt under
   ``artifacts/upstream-ronnacontinuum/`` with sha256 digests of every
   yottacontinuum, portfolio federation, admission history, ROI history, stop
   reason, and an ronnacontinuum chain digest; ``verify_ronnacontinuum_receipt``
   re-checks the chain and detects tampering.

No skill-route discovery is used. The plane is ronnacontinuum-level direction
over the yottacontinuum plane, not a new verifier of individual repairs.
"""
...
from blackhole_agent import upstream_yottacontinuum as zz
...
ARTIFACTS_ROOT = REPO_ROOT / "artifacts" / "upstream-ronnacontinuum"

TERMINAL_SUCCESS_OUTCOMES = zz.TERMINAL_SUCCESS_OUTCOMES


class RonnacontinuumRefused(Exception):
...
def normalize_ronnacontinuum_charter(
    charter: Sequence[Mapping[str, Any]] | None,
) -> list[dict[str, Any]]:
    """Normalize an ronnacontinuum charter into deterministic continuum slots.
    ...
    Each slot is::

        {
          ...
          "priority": int,
          "charter": [...zettacontinuum slots...],  # nested yottacontinuum charter
          "max_active_zettacontinuums": int | None,
          "max_rounds": int,
          "yottacontinuum_goal": str,
        }
    """
    if not charter:
        return []
    ...
    for raw in charter:
        if not isinstance(raw, Mapping):
            continue
        yottacontinuum_id = str(
            raw.get("yottacontinuum_id") or raw.get("id") or ""
        ).strip()
        if not yottacontinuum_id or yottacontinuum_id in seen:
            continue
        seen.add(yottacontinuum_id)

        nested = zz.normalize_yottacontinuum_charter(
            raw.get("charter")
            or raw.get("zettacontinuums")
        ...
        max_active_zettacontinuums = raw.get("max_active_zettacontinuums")
        if max_active_zettacontinuums is None:
            max_active_zettacontinuums = raw.get("max_active_continuums")
        if max_active_zettacontinuums is None:
            max_active_zettacontinuums = raw.get("max_active_civilizations")
        if max_active_zettacontinuums is not None:
            max_active_zettacontinuums = max(1, int(max_active_zettacontinuums))

        out.append(
            {
                "yottacontinuum_id": yottacontinuum_id,
                "priority": int(raw.get("priority") or 0),
                "charter": nested,
                "max_active_zettacontinuums": max_active_zettacontinuums,
                "max_rounds": max(1, int(raw.get("max_rounds") or 6)),
                "yottacontinuum_goal": str(
                    raw.get("yottacontinuum_goal") or "all_zettacontinuums_met"
                ),
                "kind": str(raw.get("kind") or "stewardship_yottacontinuum"),
            }
        )
    return out


def admit_yottacontinuum_slot(
    *,
    ronnacontinuum_dir: Path,
    slot: Mapping[str, Any],
) -> dict[str, Any]:
    """Materialize one institution slot under the league directory.

    Returns admission record with yottacontinuum_root and nested charter.
    Stewardship surfaces are created lazily by the institution plane on run.
    """
    yottacontinuum_id = str(slot.get("yottacontinuum_id") or "")
    if not yottacontinuum_id:
        raise RonnacontinuumRefused("ronnacontinuum_invalid", "slot missing yottacontinuum_id")

    yottacontinuum_root = Path(ronnacontinuum_dir) / "yottacontinuums" / yottacontinuum_id
    yottacontinuum_root.mkdir(parents=True, exist_ok=True)

    nested_charter = zz.normalize_yottacontinuum_charter(slot.get("charter"))
    if not nested_charter:
        raise RonnacontinuumRefused(
            "ronnacontinuum_invalid",
            f"continuum slot {yottacontinuum_id!r} has empty nested charter",
        )

    max_active_zettacontinuums = slot.get("max_active_zettacontinuums")
    if max_active_zettacontinuums is None:
        max_active_zettacontinuums = slot.get("max_active_continuums")
    if max_active_zettacontinuums is None:
        max_active_zettacontinuums = slot.get("max_active_civilizations")

    return {
        "yottacontinuum_id": yottacontinuum_id,
        "yottacontinuum_root": str(yottacontinuum_root),
        "admitted": True,
        "charter": nested_charter,
        "max_active_zettacontinuums": max_active_zettacontinuums,
        "max_rounds": int(slot.get("max_rounds") or 6),
        "yottacontinuum_goal": str(slot.get("yottacontinuum_goal") or "all_zettacontinuums_met"),
...
def federate_portfolios(
    ...
    portfolio: dict[str, Any] = {
        ...
        "entries": entries,
        "source": "ronnacontinuum_federation",
    }
...
def ronnacontinuum_terminal_coverage(
    *,
    yottacontinuum_states: Sequence[Mapping[str, Any]],
    federated_portfolio: Mapping[str, Any] | None,
) -> dict[str, Any]:
    """Coverage across yottacontinuum->zettacontinuum->...->program."""
    zettacontinuum_states: list[dict[str, Any]] = []
    for uc in yottacontinuum_states:
        if not isinstance(uc, Mapping):
            continue
        nested = (
            list(uc.get("zettacontinuum_states") or [])
        ...
        if nested:
            for hc in nested:
                if isinstance(hc, Mapping):
                    zettacontinuum_states.append(dict(hc))
            continue
        # Pre-run: yottacontinuum charter is a list of zettacontinuum slots.
        for cslot in list(uc.get("charter") or []) + list(
            uc.get("zettacontinuums") or []
            ...
            if isinstance(cslot, Mapping):
                zettacontinuum_states.append(dict(cslot))
        if not nested and not list(uc.get("charter") or []):
            for cont in list(uc.get("continuum_states") or []) + list(
                ...
                if isinstance(cont, Mapping):
                    zettacontinuum_states.append(dict(cont))
        for raw in list(uc.get("inventory_keys") or []):
            if isinstance(raw, (list, tuple)) and len(raw) >= 3:
                zettacontinuum_states.append(
                    {
                        "inventory_keys": [
                ...
                zettacontinuum_states.append({"inventory_keys": [raw]})
    return zz.yottacontinuum_terminal_coverage(
        zettacontinuum_states=zettacontinuum_states,
        federated_portfolio=federated_portfolio,
    )


def yottacontinuums_all_met(yottacontinuum_states: Sequence[Mapping[str, Any]]) -> bool:
    if not yottacontinuum_states:
        return False
    return all(bool(ist.get("yottacontinuum_met")) for ist in yottacontinuum_states)


def open_unmet_count(yottacontinuum_states: Sequence[Mapping[str, Any]]) -> int:
    """Count admitted institutions that are not yet continuum_met."""
    return sum(1 for ist in yottacontinuum_states if not ist.get("yottacontinuum_met"))
...
def pending_charter_slots(
    ...
    """Charter slots not yet admitted, priority-desc then yottacontinuum_id-asc."""
    known = {str(ist.get("yottacontinuum_id") or "") for ist in yottacontinuum_states}
    pending = [
        dict(slot)
        for slot in charter
        if str(slot.get("yottacontinuum_id") or "")
        and str(slot.get("yottacontinuum_id")) not in known
    ]
    pending.sort(
        key=lambda s: (
            -int(s.get("priority") or 0),
            str(s.get("yottacontinuum_id") or ""),
        )
    )
    return pending
...
def constitution_satisfied(
    *,
    ...
    if ronnacontinuum_goal == "none":
        return False
    if ronnacontinuum_goal == "terminal_coverage":
        cov = ronnacontinuum_terminal_coverage(
            yottacontinuum_states=yottacontinuum_states,
            federated_portfolio=federated_portfolio,
        )
        return bool(cov.get("met")) and not pending_charter_slots(
            charter, yottacontinuum_states
        )
    if ronnacontinuum_goal == "all_yottacontinuums_met":
        if not yottacontinuum_states:
            return False
        if pending_charter_slots(charter, yottacontinuum_states):
            return False
        return yottacontinuums_all_met(yottacontinuum_states)
    return False


def reopen_incomplete_yottacontinuums(
    ...
    *,
    ...
    Nested yottacontinuums can retire after only partial surface work (e.g. before
    deferred program targets expand). Under a ``terminal_coverage`` ronnacontinuum
    goal those children must re-run until federated inventory is fully
    terminal-success. Returns the reopened ``yottacontinuum_id`` list.
    """
    cov = ronnacontinuum_terminal_coverage(
        yottacontinuum_states=yottacontinuum_states,
        federated_portfolio=federated_portfolio,
    ...
    for item in list(cov.get("open_or_missing") or []):
        ...
        if key[0] and key[2]:
            open_keys.add(key)
    reopened: list[str] = []
    for i, ist in enumerate(yottacontinuum_states):
        if not ist.get("yottacontinuum_met"):
            continue
        ...
        for mv in list(ist.get("zettacontinuum_states") or []) + list(
        ...
        if not owns_open:
            continue
        updated = dict(ist)
        updated["yottacontinuum_met"] = False
        yottacontinuum_states[i] = updated
        reopened.append(str(updated.get("yottacontinuum_id") or ""))
    return [r for r in reopened if r]


def merge_ronnacontinuum_charter(
    ...
    Existing ``yottacontinuum_id`` values win (additions with the same id are
    ignored). Returns a fully re-normalized charter so nested confederation
    charters stay deterministic.
    """
    base = normalize_ronnacontinuum_charter(existing)
    if not additions:
        return base
    known = {str(s.get("yottacontinuum_id") or "") for s in base}
    merged: list[Mapping[str, Any]] = list(base)
    for raw in additions:
        if not isinstance(raw, Mapping):
            continue
        cid = str(raw.get("yottacontinuum_id") or raw.get("id") or "").strip()
        if not cid or cid in known:
            continue
        known.add(cid)
        merged.append(raw)
    return normalize_ronnacontinuum_charter(merged)


def make_ronnacontinuum_charter_expand(
    ...
    *,
    ...
    """
    pending_growth = normalize_ronnacontinuum_charter(growth)
    ...
    def _runner(
        *,
        ...
        remaining = [
            ...
            if str(s.get("yottacontinuum_id") or "") not in applied_ids
            and str(s.get("yottacontinuum_id") or "")
            not in {str(x.get("yottacontinuum_id") or "") for x in active_charter}
        ]
        if not remaining:
            return {
                "expanded": False,
        ...
        for s in take:
            applied_ids.add(str(s.get("yottacontinuum_id") or ""))
        merged = merge_ronnacontinuum_charter(active_charter, take)
        state["applied"] = applied_ids
        return {
            "expanded": True,
            "added": [str(s.get("yottacontinuum_id") or "") for s in take],
            "charter": merged,
            "detail": "charter_growth_applied",
            "round_index": round_index,
            "realms_met": yottacontinuums_all_met(yottacontinuum_states),
        }
...
def admit_pending_slots(
    *,
    ...
    ``max_active_yottacontinuums`` caps *unmet* concurrent realms. ``None``
    admits every remaining pending slot. Returns admission records for newly
    admitted slots (also mutates ``yottacontinuum_states``).
    """
    pending = pending_charter_slots(charter, yottacontinuum_states)
    if not pending:
        return []

    open_n = open_unmet_count(yottacontinuum_states)
    if max_active_yottacontinuums is None:
        capacity = len(pending)
    else:
        capacity = max(0, int(max_active_yottacontinuums) - open_n)
    if capacity <= 0:
        return []

    admissions: list[dict[str, Any]] = []
    for slot in pending[:capacity]:
        admission = admit_yottacontinuum_slot(ronnacontinuum_dir=ronnacontinuum_dir, slot=slot)
        ...
        yottacontinuum_states.append(
            {
                "yottacontinuum_id": admission["yottacontinuum_id"],
                "yottacontinuum_root": admission["yottacontinuum_root"],
                "charter": admission["charter"],
                "max_active_zettacontinuums": admission.get("max_active_zettacontinuums"),
                "max_rounds": admission["max_rounds"],
                "yottacontinuum_goal": admission["yottacontinuum_goal"],
                "priority": admission["priority"],
                "yottacontinuum_met": False,
                "last_yottacontinuum_dir": None,
                "last_yottacontinuum_digest": None,
                "portfolio": None,
                "omniverse_states": [],
                "multiverse_states": [],
                "cosmos_states": [],
...
def score_yottacontinuum_roi(
    *,
    ...
    return {
        ...
        "yottacontinuum_met": bool(continuum_result.get("yottacontinuum_met")),
        "continuum_digest": continuum_result.get("yottacontinuum_digest"),
        "continuums_admitted": int(
            continuum_result.get("zettacontinuums_admitted")
            or continuum_result.get("megacontinuums_admitted")
            or continuum_result.get("ultracontinuums_admitted")
            or continuum_result.get("yottacontinuums_admitted")
...
def _roi_summary(roi_history: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for r in roi_history:
        iid = str(r.get("yottacontinuum_id") or "")
...
def select_next_yottacontinuum(
    yottacontinuum_states: Sequence[Mapping[str, Any]],
    roi_history: Sequence[Mapping[str, Any]],
    *,
    round_index: int,
    ...
    Tie-break: lower run_count, then stable yottacontinuum_id order. Round-robin
    among equal scores uses ``round_index`` so multi-institution progress is fair.
    """
    open_slots = [
        dict(ist) for ist in yottacontinuum_states if not ist.get("yottacontinuum_met")
    ]
    if not open_slots:
        return None
    ...
    def sort_key(ist: Mapping[str, Any]) -> tuple[Any, ...]:
        iid = str(ist.get("yottacontinuum_id") or "")
...
def allocate_yottacontinuum_budget(
    *,
    ...
    hist = (summary.get("by_continuum") or {}).get(
        str(selected.get("yottacontinuum_id") or "")
    ) or {}
    mean_eff = float(hist.get("mean_efficiency") or 0.0)
...
def _state_payload(
    *,
    ...
    roi_history: Sequence[Mapping[str, Any]],
    yottacontinuum_states: Sequence[Mapping[str, Any]],
    yottacontinuum_digests: Sequence[str],
    charter: Sequence[Mapping[str, Any]],
    ...
    return {
        ...
        "yottacontinuum_states": [dict(ist) for ist in yottacontinuum_states],
        "yottacontinuum_digests": list(yottacontinuum_digests),
        ...
        "pending_yottacontinuum_ids": [
            str(s.get("yottacontinuum_id") or "")
            for s in pending_charter_slots(charter, yottacontinuum_states)
        ],
    }


def write_ronnacontinuum_state(ronnacontinuum_dir: Path, state: Mapping[str, Any]) -> Path:
    path = Path(ronnacontinuum_dir) / "ronnacontinuum_state.json"
    atomic_write_json(path, dict(state))
    return path


def load_ronnacontinuum_state(resume_dir: Path) -> dict[str, Any]:
    path = durable_read_path(Path(resume_dir) / "ronnacontinuum_state.json")
    if not path.is_file():
        raise RonnacontinuumRefused(
            "ronnacontinuum_state_missing",
            f"no ronnacontinuum_state.json under {resume_dir}",
        )
    try:
        state = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise RonnacontinuumRefused("ronnacontinuum_state_unreadable", str(exc)) from exc
    if not isinstance(state, dict):
        raise RonnacontinuumRefused("ronnacontinuum_state_invalid", "state root must be object")
    return state
...
def _yottacontinuum_round_record(
    *,
    round_index: int,
    yottacontinuum_id: str,
    continuum_result: Mapping[str, Any],
    ...
    return {
        ...
        "stop_reason": continuum_result.get("stop_reason"),
        "yottacontinuum_dir": continuum_result.get("yottacontinuum_dir"),
        "yottacontinuum_digest": continuum_result.get("yottacontinuum_digest"),
        "continuums_admitted": int(
            continuum_result.get("zettacontinuums_admitted")
            or continuum_result.get("megacontinuums_admitted")
            or continuum_result.get("ultracontinuums_admitted")
...
def _ronnacontinuum_digest_payload(receipt: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": receipt.get("schema_version"),
        "verdict": receipt.get("verdict"),
        "stop_reason": receipt.get("stop_reason"),
        "ronnacontinuum_id": receipt.get("ronnacontinuum_id"),
        "ronnacontinuum_goal": receipt.get("ronnacontinuum_goal"),
        "max_rounds": receipt.get("max_rounds"),
        "max_active_yottacontinuums": receipt.get("max_active_yottacontinuums"),
        ...
        "yottacontinuum_digests": list(receipt.get("yottacontinuum_digests") or []),
        "total_dispatched": receipt.get("total_dispatched"),
        "total_dispatched_ok": receipt.get("total_dispatched_ok"),
        "ronnacontinuum_met": receipt.get("ronnacontinuum_met"),
        "coverage_end": receipt.get("coverage_end"),
        "yottacontinuums_met_count": receipt.get("yottacontinuums_met_count"),
        "yottacontinuums_admitted": receipt.get("yottacontinuums_admitted"),
...
def verify_ronnacontinuum_receipt(ronnacontinuum_dir: Path) -> dict[str, Any]:
    """Re-check a sealed league receipt for digest integrity."""
    path = durable_read_path(Path(ronnacontinuum_dir) / "ronnacontinuum.json")
    if not path.is_file():
    ...
    expected = _sha256_json(_ronnacontinuum_digest_payload(receipt))
    recorded = str(receipt.get("ronnacontinuum_digest") or "")
    mismatched: list[str] = []
    if not recorded or recorded != expected:
        mismatched.append("ronnacontinuum_digest")

    institutions = list(receipt.get("yottacontinuums") or receipt.get("realms") or receipt.get("leagues") or [])
    listed = list(receipt.get("yottacontinuum_digests") or [])
    if len(listed) != len(institutions):
        mismatched.append("yottacontinuum_digests_length")
    else:
        for i, (listed_d, rec) in enumerate(zip(listed, institutions)):
            if listed_d != rec.get("yottacontinuum_digest"):
                mismatched.append(f"yottacontinuum_digests[{i}]")

    nested_failures: list[str] = []
    for rec in institutions:
        idir = rec.get("yottacontinuum_dir")
        if not idir:
            continue
        ip = Path(str(idir))
        if (ip / "yottacontinuum.json").is_file():
            nested = zz.verify_yottacontinuum_receipt(ip)
    ...
    return {
        "ok": ok,
        "verdict": "ronnacontinuum_sealed" if ok else "ronnacontinuum_tampered",
        "ronnacontinuum_digest": recorded,
...
def run_ronnacontinuum(
    *,
    ...
    max_active_yottacontinuums: int | None = None,
    dispatch: bool = True,
    yottacontinuum_runner: Callable[..., dict[str, Any]] | None = None,
    ...
    ronnacontinuum_id: str | None = None,
    ...
    charter:
        ...
        :func:`normalize_ronnacontinuum_charter`).
    max_rounds:
    ...
    ronnacontinuum_goal:
        ``all_yottacontinuums_met`` (default) stops when every admitted institution
    ...
    resume_dir:
        Load ``ronnacontinuum_state.json`` from a prior league dir and continue.
        New receipt is written under ``out_root`` (or a fresh stamp).
    """
    if max_rounds < 1:
        raise RonnacontinuumRefused("ronnacontinuum_invalid", "max_rounds must be >= 1")
    if per_wave_dispatch_limit < 0:
        raise RonnacontinuumRefused(
            "ronnacontinuum_invalid", "per_wave_dispatch_limit must be >= 0"
        )
    if max_active_yottacontinuums is not None and int(max_active_yottacontinuums) < 1:
        raise RonnacontinuumRefused(
            "ronnacontinuum_invalid", "max_active_yottacontinuums must be >= 1 when set"
        )
    if ronnacontinuum_goal not in {"all_yottacontinuums_met", "terminal_coverage", "none"}:
        raise RonnacontinuumRefused(
            "ronnacontinuum_invalid",
            f"unknown ronnacontinuum_goal: {ronnacontinuum_goal}",
        )

    runner = yottacontinuum_runner or zz.run_yottacontinuum

    prior_round_count = 0
    roi_history: list[dict[str, Any]] = []
    yottacontinuum_digests: list[str] = []
    total_dispatched = 0
    total_dispatched_ok = 0
    resumed = False
    resume_ronnacontinuum_id: str | None = None
    yottacontinuum_states: list[dict[str, Any]] = []
    ...
    if resume_dir is not None:
        state = load_ronnacontinuum_state(resume_dir)
        resumed = True
        resume_ronnacontinuum_id = str(state.get("ronnacontinuum_id") or "") or None
        ...
        yottacontinuum_digests = [str(d) for d in (state.get("yottacontinuum_digests") or [])]
        yottacontinuum_states = [
            dict(ist)
            for ist in (state.get("yottacontinuum_states") or [])
        ...
        if isinstance(state.get("charter"), list):
            active_charter = normalize_ronnacontinuum_charter(
        ...
        if (
            state.get("max_active_yottacontinuums") is not None
            and max_active_yottacontinuums is None
        ):
            resumed_max_active = int(state["max_active_yottacontinuums"])
        # Resume may also merge a caller-supplied charter growth tail.
        if charter:
            active_charter = merge_ronnacontinuum_charter(active_charter, charter)
    else:
        active_charter = normalize_ronnacontinuum_charter(charter)
    ...
    if not active_charter and not yottacontinuum_states:
        raise RonnacontinuumRefused(
            "ronnacontinuum_empty",
            "continuum charter has no admitable realm slots",
        )

    lid = (
        ronnacontinuum_id
        or resume_ronnacontinuum_id
        or f"ronnacontinuum-{utc_now_iso().replace(':', '').replace('-', '')}"
    )

    stamp = utc_now_iso().replace(":", "").replace("-", "")
    if out_root is not None:
        ronnacontinuum_dir = Path(out_root)
        if (ronnacontinuum_dir / "ronnacontinuum.json").is_file():
            ronnacontinuum_dir = ronnacontinuum_dir / stamp
    else:
        ronnacontinuum_dir = ARTIFACTS_ROOT / stamp
    ronnacontinuum_dir.mkdir(parents=True, exist_ok=True)
    ...
    # on a flat ultra-short base (not under ronnacontinuum_dir) so one extra
    ...
    if league_out_root is not None:
        ...
        lg_root = ronnacontinuum_dir / "e"
    lg_root.mkdir(parents=True, exist_ok=True)
    if _os.name == "nt":
        inst_flat_root = Path("C:/t") / "q"
    else:
        inst_flat_root = ronnacontinuum_dir / "x"
    inst_flat_root.mkdir(parents=True, exist_ok=True)
    # Per-run namespace so parallel/resumed ultracontinua do not collide.
    # Prefer a short random token over the human lid (often "ronnacontinuum-…")
    # so Windows paths stay short and never re-enter a prior yottacontinuum_dir that
    ...
    initial_admissions = admit_pending_slots(
        ronnacontinuum_dir=ronnacontinuum_dir,
        charter=active_charter,
        yottacontinuum_states=yottacontinuum_states,
        max_active_yottacontinuums=active_max,
        round_index=prior_round_count,
    )
    admissions.extend(initial_admissions)

    if not yottacontinuum_states and not pending_charter_slots(
        active_charter, yottacontinuum_states
    ):
        raise RonnacontinuumRefused("ronnacontinuum_empty", "no realm slots admitted")
    if not yottacontinuum_states and pending_charter_slots(
        active_charter, yottacontinuum_states
    ):
        raise RonnacontinuumRefused(
            "ronnacontinuum_empty",
            "no realm slots admitted under max_active_yottacontinuums policy",
        )

    if federated_portfolio is None:
        federated_portfolio = federate_portfolios(
            [ist.get("portfolio") for ist in yottacontinuum_states]
        )
    ...
    max_terminal_reopens = max(2, (len(yottacontinuum_states) or 1) * 4)
    coverage_end: dict[str, Any] = ronnacontinuum_terminal_coverage(
        yottacontinuum_states=yottacontinuum_states,
        federated_portfolio=federated_portfolio,
    )
    ...
    for local_index in range(max_rounds):
        ...
        coverage_before = ronnacontinuum_terminal_coverage(
            yottacontinuum_states=yottacontinuum_states,
            federated_portfolio=federated_portfolio,
        )
        ...
        selected = select_next_yottacontinuum(
            yottacontinuum_states, roi_history, round_index=round_index
        )
        if selected is None:
            # Children may all report met while federated terminal coverage is
            # still incomplete (e.g. deferred surfaces). Only declare ronnacontinuum
            # met when the active ronnacontinuum_goal is actually satisfied.
            if constitution_satisfied(
                yottacontinuum_states=yottacontinuum_states,
                charter=active_charter,
                ronnacontinuum_goal=ronnacontinuum_goal,
                ...
                break
            if (
                ronnacontinuum_goal == "terminal_coverage"
                and terminal_reopen_count < max_terminal_reopens
            ):
                reopened = reopen_incomplete_yottacontinuums(
                    yottacontinuum_states,
                    federated_portfolio=federated_portfolio,
                )
                if reopened:
            ...
            if selected is None:
                stop_reason = "ronnacontinuum_idle"
                coverage_end = coverage_before
                break

        open_count = sum(
            1 for ist in yottacontinuum_states if not ist.get("yottacontinuum_met")
        )
        allocated = allocate_yottacontinuum_budget(
        ...
        yottacontinuum_id = str(selected["yottacontinuum_id"])
        resume_yottacontinuum_dir = selected.get("last_yottacontinuum_dir")
        continuum_resume: Path | None = None
        if (
            resume_yottacontinuum_dir
            and (Path(str(resume_yottacontinuum_dir)) / "yottacontinuum_state.json").is_file()
            and not selected.get("yottacontinuum_met")
        ):
            continuum_resume = Path(str(resume_yottacontinuum_dir))

        # Ultra-short stamp (each outer plane tightens Windows MAX_PATH).
        # Do NOT nest child yottacontinuum trees under ronnacontinuum roots:
        # let the yottacontinuum plane use its own flat C:/t/{h,i} bases so the
        # deep continuum→…→wave chain still fits. Only the yottacontinuum receipt
        # itself lands on a flat unique out_root.
        safe_id = "".join(c if c.isalnum() else "" for c in yottacontinuum_id)[:2] or "i"
        ...
        continuum_kwargs: dict[str, Any] = {
            ...
            "yottacontinuum_goal": str(
                selected.get("yottacontinuum_goal") or "all_zettacontinuums_met"
            ),
            "yottacontinuum_id": yottacontinuum_id,
            "out_root": out_dir,
        }
        ...
        max_active_zettacontinuums = selected.get("max_active_zettacontinuums")
        if max_active_zettacontinuums is None:
            max_active_zettacontinuums = selected.get("max_active_continuums")
        if max_active_zettacontinuums is None:
            max_active_zettacontinuums = selected.get("max_active_civilizations")
        if max_active_zettacontinuums is not None:
            continuum_kwargs["max_active_zettacontinuums"] = int(
                max_active_zettacontinuums
            )
        ...
        except zz.YottacontinuumRefused as exc:
            if local_index == 0 and not resumed:
                raise RonnacontinuumRefused(exc.verdict, exc.detail) from exc
        ...
        nested_yottacontinuum_dir = continuum_result.get("yottacontinuum_dir")
        nested_multiverse_states: list[dict[str, Any]] = []
        nested_receipt_path = (
            Path(str(nested_yottacontinuum_dir)) / "yottacontinuum.json"
            if nested_yottacontinuum_dir
            else None
        )
        if nested_receipt_path is not None and nested_receipt_path.is_file():
            ...
            for ist in list(
                receipt.get("zettacontinuum_states")
        ...
        if not nested_multiverse_states:
            for ist in list(
                continuum_result.get("zettacontinuum_states")
        ...
        for i, lst in enumerate(yottacontinuum_states):
            if str(lst.get("yottacontinuum_id")) != yottacontinuum_id:
                continue
            updated = dict(lst)
            updated["last_yottacontinuum_dir"] = continuum_result.get("yottacontinuum_dir")
            updated["last_yottacontinuum_digest"] = continuum_result.get("yottacontinuum_digest")
            updated["yottacontinuum_met"] = bool(continuum_result.get("yottacontinuum_met"))
            ...
            if nested_multiverse_states:
                updated["zettacontinuum_states"] = nested_multiverse_states
        ...
        coverage_after = ronnacontinuum_terminal_coverage(
            yottacontinuum_states=yottacontinuum_states,
            federated_portfolio=federated_portfolio,
        )

        roi = score_yottacontinuum_roi(
            round_index=round_index,
            yottacontinuum_id=yottacontinuum_id,
        ...
        idigest = str(continuum_result.get("yottacontinuum_digest") or "")
        if idigest:
            yottacontinuum_digests.append(idigest)

        rec = _yottacontinuum_round_record(
            round_index=round_index,
            yottacontinuum_id=yottacontinuum_id,
            continuum_result=continuum_result,
            coverage_before=coverage_before,
        ...
        write_ronnacontinuum_state(
            ronnacontinuum_dir,
            _state_payload(
                ronnacontinuum_id=lid,
                ...
                roi_history=roi_history,
                yottacontinuum_states=yottacontinuum_states,
                yottacontinuum_digests=yottacontinuum_digests,
                charter=active_charter,
                stop_reason=None,
                ronnacontinuum_goal=ronnacontinuum_goal,
                max_active_yottacontinuums=active_max,
        ...
        if (
            ...
            and not continuum_result.get("yottacontinuum_met")
        ):
            idle_streak += 1
        else:
            idle_streak = 0
            ...
            reason = stop_when(
                {
                    ...
                    "roi_history": roi_history,
                    "yottacontinuum_states": yottacontinuum_states,
                    "last_yottacontinuum_id": yottacontinuum_id,
                    "federated_portfolio": federated_portfolio,
                    "ronnacontinuum_dir": str(ronnacontinuum_dir),
                    "pending_yottacontinuum_ids": [
                        str(s.get("yottacontinuum_id") or "")
        ...
        # Grow constitution before declaring ronnacontinuum_met when expand remains.
        if (
            charter_expand is not None
            and not pending_charter_slots(active_charter, yottacontinuum_states)
            and yottacontinuums_all_met(yottacontinuum_states)
        ):
    ...
    coverage_end = ronnacontinuum_terminal_coverage(
        yottacontinuum_states=yottacontinuum_states,
        federated_portfolio=federated_portfolio,
    )
    if constitution_satisfied(
    ...
    yottacontinuums_met_count = sum(
        1 for ist in yottacontinuum_states if ist.get("yottacontinuum_met")
    )
    pending_remaining = [
        str(s.get("yottacontinuum_id") or "")
        for s in pending_charter_slots(active_charter, yottacontinuum_states)
    ]

    if ronnacontinuum_met and stop_reason in {"ronnacontinuum_met", "max_rounds"}:
        verdict = "ronnacontinuum_met"
        ok = True
        stop_reason = "ronnacontinuum_met"
    elif stop_reason == "rank_only":
        verdict = "ronnacontinuum_ranked"
        ok = True
    elif stop_reason == "ronnacontinuum_idle":
        verdict = "ronnacontinuum_idle"
        ok = True
    elif stop_reason == "dispatch_budget":
        verdict = "ronnacontinuum_budgeted"
        ok = True
        ...
        verdict = "ronnacontinuum_completed"
        ok = True
    ...
    receipt: dict[str, Any] = {
        ...
        "ronnacontinuum_goal": ronnacontinuum_goal,
        "ronnacontinuum_met": ronnacontinuum_met,
        ...
        "round_count": len(institutions),
        "yottacontinuums": institutions,
        "yottacontinuum_digests": [
            i.get("yottacontinuum_digest")
            for i in institutions
            if i.get("yottacontinuum_digest")
        ],
        "yottacontinuum_states": yottacontinuum_states,
        "yottacontinuums_admitted": len(yottacontinuum_states),
        "yottacontinuums_met_count": yottacontinuums_met_count,
        "admissions": admissions,
    ...
    receipt["yottacontinuum_digests"] = [
        str(i.get("yottacontinuum_digest") or "") for i in institutions
    ]
    receipt["ronnacontinuum_digest"] = _sha256_json(_ronnacontinuum_digest_payload(receipt))
    atomic_write_json(ronnacontinuum_dir / "ronnacontinuum.json", receipt)
    atomic_write_json(
        ronnacontinuum_dir / "summary.json",
        {
            "verdict": receipt["verdict"],
            "ok": receipt["ok"],
            "stop_reason": receipt["stop_reason"],
            "ronnacontinuum_id": receipt["ronnacontinuum_id"],
            "round_count": receipt["round_count"],
            "total_dispatched": receipt["total_dispatched"],
            "total_dispatched_ok": receipt["total_dispatched_ok"],
            "ronnacontinuum_met": receipt["ronnacontinuum_met"],
            ...
            "max_active_yottacontinuums": receipt["max_active_yottacontinuums"],
            ...
            "ronnacontinuum_digest": receipt["ronnacontinuum_digest"],
            "resumed": resumed,
        },
    )
    ...
    write_ronnacontinuum_state(
        ...
        _state_payload(
            ...
            roi_history=roi_history,
            yottacontinuum_states=yottacontinuum_states,
            yottacontinuum_digests=receipt["yottacontinuum_digests"],
            charter=active_charter,
            stop_reason=stop_reason,
            ronnacontinuum_goal=ronnacontinuum_goal,
            max_active_yottacontinuums=active_max,
    ...
    return {
        ...
        "ronnacontinuum_id": lid,
        "round_count": len(institutions),
        "yottacontinuum_digests": list(receipt["yottacontinuum_digests"]),
        "total_dispatched": total_dispatched,
        "total_dispatched_ok": total_dispatched_ok,
        "ronnacontinuum_met": ronnacontinuum_met,
        "yottacontinuums_admitted": len(yottacontinuum_states),
...
def _yottacontinuum_slot(
    yottacontinuum_id: str,
    *,
    priority: int = 0,
    zettacontinuums: Sequence[dict[str, Any]] | None = None,
    ...
    institutions: Sequence[dict[str, Any]] | None = None,
    max_rounds: int = 6,
    yottacontinuum_goal: str = "all_zettacontinuums_met",
    max_active_zettacontinuums: int | None = None,
) -> dict[str, Any]:
    """Build a hermetic ronnacontinuum charter yottacontinuum slot.

    Prefer ``zettacontinuums=`` (nested giga slots for the yottacontinuum plane).
    ``institutions=`` wrap a single auto giga with mega nesting underneath.
    """
    nested: list[dict[str, Any]]
    if zettacontinuums is not None:
        nested = list(zettacontinuums)
    elif megacontinuums is not None:
        nested = [
            zz._zettacontinuum_slot(
                f"{yottacontinuum_id[:1]}g",
    ...
    return {
        ...
        "charter": nested,
        "max_rounds": max_rounds,
        "yottacontinuum_goal": yottacontinuum_goal,
        "max_active_zettacontinuums": max_active_zettacontinuums,
    }
...
def builtin_upstream_ronnacontinuum_proof() -> dict[str, Any]:
    """Hermetic end-to-end proof of the multi-yottacontinuum ronnacontinuum plane (no network).

    Uses an injected fast ``yottacontinuum_runner`` so the outer plane constitution
    ...
    try:
        # Per-yottacontinuum progress for budget/resume/terminal multi-call realism.
        ...
        def _fast_mega_runner(**kwargs: Any) -> dict[str, Any]:
            ...
            mid = str(kwargs.get("yottacontinuum_id") or "m")
            nested_charter = zz.normalize_yottacontinuum_charter(kwargs.get("charter"))
            if kwargs.get("resume_dir") is not None and not nested_charter:
                state_path = Path(str(kwargs["resume_dir"])) / "yottacontinuum_state.json"
                if state_path.is_file():
                    st = json.loads(state_path.read_text(encoding="utf-8"))
                    nested_charter = zz.normalize_yottacontinuum_charter(st.get("charter"))
                    mid = str(st.get("yottacontinuum_id") or mid)

            goal = str(kwargs.get("yottacontinuum_goal") or "all_zettacontinuums_met")
            ...
            include_deferred = st["calls"] > 1 or goal == "all_zettacontinuums_met"
            ...
            zettacontinuum_states = [
                {
                    "zettacontinuum_id": str(
                        s.get("zettacontinuum_id") or s.get("id") or "g"
                    ),
                    "zettacontinuum_met": report_met,
            ...
            receipt = {
                "schema_version": 1,
                "ok": True,
                "verdict": "yottacontinuum_met" if report_met else "yottacontinuum_completed",
                "stop_reason": "yottacontinuum_met" if report_met else "dispatch_budget",
                "yottacontinuum_id": mid,
                "yottacontinuum_goal": str(
                    kwargs.get("yottacontinuum_goal") or "all_zettacontinuums_met"
                ),
                "max_rounds": int(kwargs.get("max_rounds") or 6),
                "max_active_zettacontinuums": kwargs.get("max_active_zettacontinuums"),
                ...
                "zettacontinuums": [],
                "zettacontinuum_digests": [],
                "zettacontinuums_admitted": max(1, len(nested_charter) or 1),
                "zettacontinuums_met_count": (
            ...
            receipt["yottacontinuum_digest"] = _sha256_json(
                zz._yottacontinuum_digest_payload(receipt)
            )
            digest = receipt["yottacontinuum_digest"]
            atomic_write_json(out / "yottacontinuum.json", receipt)
            atomic_write_json(
                out / "yottacontinuum_state.json",
                {
                    "yottacontinuum_id": mid,
                    "round_count": st["calls"],
                    "total_dispatched": st["dispatched"],
            ...
            return {
                ...
                "verdict": receipt["verdict"],
                "stop_reason": receipt["stop_reason"],
                "yottacontinuum_dir": str(out),
                "yottacontinuum_digest": digest,
                "yottacontinuum_id": mid,
                "total_dispatched": st["dispatched"],
                "total_dispatched_ok": st["dispatched_ok"],
                "yottacontinuum_met": report_met,
                "zettacontinuums_admitted": receipt["zettacontinuums_admitted"],
                "zettacontinuums_met_count": receipt["zettacontinuums_met_count"],
                "coverage_end": receipt["coverage_end"],
                "federated_portfolio": portfolio,
                "zettacontinuum_states": zettacontinuum_states,
        ...
        confed = run_ronnacontinuum(
            charter=charter,
            max_rounds=8,
            dispatch_budget=12,
            dispatch=True,
            yottacontinuum_runner=_fast_mega_runner,
            ronnacontinuum_goal="all_yottacontinuums_met",
            out_root=scratch / "m",
        )
        multi_ronnacontinuum_ok = (
            confed["ok"]
            and confed["ronnacontinuum_met"] is True
            and confed["stop_reason"] == "ronnacontinuum_met"
            and confed["yottacontinuums_admitted"] == 2
            and confed["yottacontinuums_met_count"] == 2
            and confed["round_count"] >= 2
            and confed["total_dispatched_ok"] >= 3
            and float((confed.get("coverage_end") or {}).get("coverage_ratio") or 0) == 1.0
        )
        scheduled_ids = {
            i.get("yottacontinuum_id")
            for i in (confed.get("yottacontinuums") or [])
        }
        multi_ronnacontinuum_scheduled = multi_ronnacontinuum_ok and scheduled_ids >= {"a", "b"}

        verified = verify_ronnacontinuum_receipt(Path(confed["ronnacontinuum_dir"]))
        seal_ok = bool(verified.get("ok")) and verified.get("round_count") == confed["round_count"]

        confed_path = Path(confed["ronnacontinuum_dir"]) / "ronnacontinuum.json"
        receipt = json.loads(confed_path.read_text(encoding="utf-8"))
        receipt["ronnacontinuum_digest"] = "0" * 64
        confed_path.write_text(json.dumps(receipt, indent=2), encoding="utf-8")
        tampered = verify_ronnacontinuum_receipt(Path(confed["ronnacontinuum_dir"]))
        tamper_detected = (
            not tampered["ok"]
            and "ronnacontinuum_digest" in (tampered.get("mismatched") or [])
        )

        # Budget stop
        _progress.clear()
        budgeted = run_ronnacontinuum(
            charter=[
        ...
        def _premet_runner(**kwargs: Any) -> dict[str, Any]:
            ...
            receipt = {
                "schema_version": 1,
                "ok": True,
                "verdict": "yottacontinuum_met",
                "stop_reason": "yottacontinuum_met",
                "yottacontinuum_id": kwargs.get("yottacontinuum_id"),
                "yottacontinuum_goal": "all_zettacontinuums_met",
                "max_rounds": 1,
                "max_active_zettacontinuums": None,
                ...
                "zettacontinuum_digests": [],
                "zettacontinuums_admitted": max(1, len(nested_charter) or 1),
                "zettacontinuums_met_count": max(1, len(nested_charter) or 1),
                "admission_count": 0,
                "pending_remaining": [],
                "charter_expansion_count": 0,
                "charter_expanded_ids": [],
            ...
            atomic_write_json(
                out / "yottacontinuum_state.json",
                {
                    ...
                    "total_dispatched_ok": 0,
                    "federated_portfolio": portfolio,
                    "zettacontinuum_states": receipt["zettacontinuum_states"],
                    "stop_reason": "yottacontinuum_met",
                    "charter": nested_charter,
                },
            )
        ...
        pre_met = run_ronnacontinuum(
            charter=[_slot("om", initial=[("om", "9.0.0", "omega-merged")])],
            max_rounds=3,
            dispatch=True,
            yottacontinuum_runner=_premet_runner,
            ronnacontinuum_goal="all_yottacontinuums_met",
            out_root=scratch / "p",
        )
        premet_ok = (
            pre_met["ok"]
            and pre_met["ronnacontinuum_met"] is True
            and pre_met["stop_reason"] == "ronnacontinuum_met"
            and pre_met["yottacontinuums_met_count"] == 1
            and pre_met["total_dispatched"] == 0
        )

        ranked = run_ronnacontinuum(
            charter=[
        ...
        rank_only_ok = (
            ranked["ok"]
            and ranked["verdict"] == "ronnacontinuum_ranked"
        ...
        except RonnacontinuumRefused as exc:
        ...
        partial = run_ronnacontinuum(
            ...
            yottacontinuum_runner=_fast_mega_runner,
            ronnacontinuum_goal="none",
            ronnacontinuum_id="rcp",
            out_root=scratch / "a",
        )
        state_path = Path(partial["ronnacontinuum_dir"]) / "ronnacontinuum_state.json"
        state_exists = state_path.is_file()
        resumed = run_ronnacontinuum(
            resume_dir=Path(partial["ronnacontinuum_dir"]),
            max_rounds=4,
            dispatch_budget=4,
            dispatch=True,
            yottacontinuum_runner=_fast_mega_runner,
        ...
        resume_ok = (
            ...
            and resumed["ronnacontinuum_id"] == "rcp"
        ...
        admit_ids = [a.get("yottacontinuum_id") for a in (deferred.get("admissions") or [])]
        deferred_ok = (
            deferred["ok"]
            and deferred["ronnacontinuum_met"] is True
            and deferred["yottacontinuums_admitted"] == 3
            and deferred["yottacontinuums_met_count"] == 3
            and deferred.get("max_active_yottacontinuums") == 1
        ...
        expand_ok = (
            expanded["ok"]
            and expanded["ronnacontinuum_met"] is True
            and expanded["yottacontinuums_admitted"] == 2
            and expanded["yottacontinuums_met_count"] == 2
        ...
        merged = merge_ronnacontinuum_charter(
        ...
        merge_ok = [s["yottacontinuum_id"] for s in merged] == ["m1", "m2"]

        # Terminal coverage with premature-met + deferred reopen (ids start with t)
        _progress.clear()
        terminal = run_ronnacontinuum(
            charter=[
        ...
        terminal_ok = (
            terminal["ok"]
            and terminal["ronnacontinuum_met"] is True
            and terminal["stop_reason"] == "ronnacontinuum_met"
            ...
            and terminal["yottacontinuums_admitted"] == 2
        ...
        ok = all(
            [
                multi_ronnacontinuum_ok,
                multi_ronnacontinuum_scheduled,
                seal_ok,
                tamper_detected,
                budget_ok,
                premet_ok,
        ...
        return {
            "ok": ok,
            "ronnacontinuum_met": multi_ronnacontinuum_ok,
            "multi_ronnacontinuum_progressed": multi_ronnacontinuum_scheduled,
            ...
            "roi_scored": roi_ok,
            "ronnacontinuum_digest": confed.get("ronnacontinuum_digest"),
            "round_count": confed.get("round_count"),
            "total_dispatched_ok": confed.get("total_dispatched_ok"),
            "yottacontinuums_admitted": confed.get("yottacontinuums_admitted"),
            "used_skill_route_discovery": legacy_pipeline_was_used(),
            "flags": {
                "multi_ronnacontinuum_ok": multi_ronnacontinuum_ok,
                "multi_ronnacontinuum_scheduled": multi_ronnacontinuum_scheduled,
                "seal_ok": seal_ok,
                "tamper_detected": tamper_detected,
                "budget_ok": budget_ok,
                "premet_ok": premet_ok,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.verify:
        result = verify_ronnacontinuum_receipt(Path(args.verify))
        print(json.dumps(result, indent=2, sort_keys=True))
        return 0 if result.get("ok") else 1
    if args.proof:
        result = builtin_upstream_ronnacontinuum_proof()
        print(json.dumps(result, indent=2, sort_keys=True))
        return 0 if result.get("ok") else 1
    parser.print_help()
    return 2
