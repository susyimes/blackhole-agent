...
def builtin_mcp_plugin_reconnect_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="mcp-reconnect-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, MCP_RECONNECT_GOAL, MCP_RECONNECT_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_reconnect"] = (
        not gate.accepted
        and live_goal != MCP_RECONNECT_GOAL
        and MCP_RECONNECT_ID not in live_done
