...
def builtin_ipv6addr_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="ipv6addr-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, IPV6ADDR_ACTUATION_GOAL, IPV6ADDR_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_ipv6addr"] = (
        not gate.accepted
        and live_goal != IPV6ADDR_ACTUATION_GOAL
        and IPV6ADDR_ACTUATION_ID not in live_done
