...
def builtin_masque_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="masque-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, MASQUE_ACTUATION_GOAL, MASQUE_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_masque"] = (
        not gate.accepted
        and live_goal != MASQUE_ACTUATION_GOAL
        and MASQUE_ACTUATION_ID not in live_done
