...
def builtin_mcp_roots_list_changed_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="mcp-roots-changed-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, MCP_ROOTS_CHANGED_GOAL, MCP_ROOTS_CHANGED_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_roots_changed"] = (
        not gate.accepted
        and live_goal != MCP_ROOTS_CHANGED_GOAL
        and MCP_ROOTS_CHANGED_ID not in live_done
