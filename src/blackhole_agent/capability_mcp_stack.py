"""Stack-health plane for mixed MCP+absorbed pipelines the base stack cannot grade.

Goal-stack health compounds application, watchdog, fragility, recovery, and
synthesis over ``APPLICATION_TASKS``. After a typed key-bridge made a live
MCP tool compose with an absorbed Python leaf, a red MCP hop still left the
composite stack green: those mixed goals never entered the headlines.

This module closes that grading failure without changing pre-growth
semantics of the base stack:

- default stack health still ignores mixed MCP+absorbed goals and stays
  green when the live MCP hop is stamped red;
- mixed application, watchdog, fragility, and recovery headlines are
  graded together on the three-member pipeline surface so hide-one
  analysis cannot explode over the absorbed zoo;
- a live mixed pipeline is healthy only when the composition plans, the
  watchdog reports no drift, the hop is a scored SPOF, and no pipeline
  member needs repair;
...
from blackhole_agent.capability_mcp_fragility import run_mcp_fragility_audit
from blackhole_agent.capability_mcp_recovery import mixed_surface_ids
...
SCHEMA_VERSION = 1
MCP_STACK_ID = "capability.mcp-stack-health-plane"
REPO_ROOT = Path(__file__).resolve().parents[2]
DEFAULT_ARTIFACT_DIR = REPO_ROOT / "artifacts" / "capability-mcp-stack"
...
def mixed_recovery_snapshot(ledger) -> dict[str, Any]:
    """Snapshot: mixed stack recovery is green only when every pipeline member is proved.
    ...
    red_ids: list[str] = []
    for capability_id in mixed_surface_ids():
...
def _watch_mixed(tasks: Sequence[ApplicationTask], ledger) -> dict[str, Any]:
    return run_goal_watchdog(
        ledger=ledger,
        tasks=tasks,
        include_absorbed=True,
        registry=mixed_watch_registry(ledger),
    )
...
def _fragility_headline(
    ...
    *,
    ...
    return {
        ...
        "mcp_hop_blast": blast.get(mcp_id),
        "mcp_hop_is_spof": bool(composition_id) and mcp_id in list(spofs.get(composition_id) or []),
        "blocked_goals": list(matrix.get(mcp_id) or []),
    }


def compute_mcp_stack_health(headlines: Mapping[str, Any]) -> dict[str, Any]:
    """Pure mixed-stack grade from recorded headlines.

    Fragility is topology (the hop is a scored SPOF). Application, watchdog,
    and recovery are the current stamp: a red hop must fail those three and
    therefore the composite grade.
    """
    ...
    planes_green = {
        ...
        "fragility": bool(
            ...
            and (fragility.get("mcp_hop_blast") or 0) >= 1
            and bool(fragility.get("mcp_hop_is_spof"))
        ),
...
def collect_mixed_headlines(
    ...
    *,
    ...
    watchdog = _watch_mixed(tasks, ledger)
    headlines = {
        "application": _application_headline(watchdog),
        "watchdog": _watchdog_headline(watchdog),
        "fragility": _fragility_headline(
            run_mcp_fragility_audit(tasks=tasks, ledger=live_ledger),
            mcp_id=mcp_id,
            composition_id=composition_id,
        ),
        "recovery": mixed_recovery_snapshot(ledger),
    }
    return {
        "headlines": headlines,
        "health": compute_mcp_stack_health(headlines),
    }
...
def compute_stack_verdicts(
    *,
    ...
    """Live isolation, mixed-stack health, and red-hop grade-failure verdicts."""
    ...
    base_recovery = run_recovery_loop(breaks={mcp_id: BREAK_STALE_STAMP} if mcp_id else None)
    live = (
    ...
    red_base_watch = run_goal_watchdog(ledger=red_ledger) if mcp_id else base_watch
    ...
    verdicts = {
        ...
        and base_watch_ids.isdisjoint(mcp_ids)
        ...
        and mcp_id not in (base_grade.get("blast_radius") or {})
        ...
        "live_mcp_stack_healthy": bool(live_health.get("healthy"))
        and bool(mcp_ids)
        ...
        "base_stack_ignores_red_hop": bool(mcp_id)
...
def run_mcp_stack_health_plane(output_dir: Path | None = None) -> dict[str, Any]:
    """Prove isolation + red-hop grade failure, seal a report, persist the live artifact."""

    honesty = compute_stack_verdicts()
    report = {
        "schema_version": SCHEMA_VERSION,
        "kind": "capability_mcp_stack_health",
...
def verify_mcp_stack_health_report(report_dir: Path) -> dict[str, Any]:
    """Recompute live mixed-stack verdicts and re-check the sealed digest."""
    ...
    checks = {
        ...
        "red_hop_fails_match": honesty["verdicts"].get("red_hop_fails_stack")
        is (report.get("verdicts") or {}).get("red_hop_fails_stack"),
        "live_health_recomputed": compute_mcp_stack_health((honesty["live"] or {}).get("headlines") or {})
...
def mcp_stack_health_proof_command() -> str:
    return (
        "uv run python -c \"from blackhole_agent.capability_mcp_stack import "
        "builtin_mcp_stack_health_proof; "
        "r=builtin_mcp_stack_health_proof(); "
        ...
        "and r.get('verdicts',{}).get('base_stack_ignores_red_hop') "
        "and r.get('verify_ok') and r.get('tamper_detected') "
        "and r.get('misgrade_detected') and not r.get('used_skill_route_discovery')\""
    )


def ensure_mcp_stack_health_capability(*, repo_path: Path | None = None) -> Capability:
    ...
    capability = Capability(
        id=MCP_STACK_ID,
        name="MCP mixed-composition stack-health plane",
        description=(
            "Grades mixed MCP+absorbed pipelines the base stack-health "
            "surface cannot see: a red live MCP hop fails the mixed stack "
            "grade, and base-stack pre-growth semantics stay intact."
        ),
        kind="python",
        entry="blackhole_agent.capability_mcp_stack:builtin_mcp_stack_health_proof",
        proof_command=mcp_stack_health_proof_command(),
        dependencies=(
            "repo.import-health",
            ...
            "capability.mcp-recovery-plane",
            "capability.mcp-fragility-plane",
        ),
        behavior_paths=(
        ...
        capability_delta=(
            "Mixed MCP+absorbed pipelines are stack-health visible: a red "
            "MCP hop fails the mixed stack grade, while default stack health "
            "still ignores those goals."
        ),
        tags=("mcp", "absorbed", "stack-health", "grade", "composition"),
...
def builtin_mcp_stack_health_proof() -> dict[str, Any]:
    """Registered proof: isolation, red-hop grade failure, seal, tamper, misgrade."""

    with tempfile.TemporaryDirectory(prefix="blackhole-mcp-stack-") as tmp:
        ...
        tampered["verdicts"]["red_hop_fails_stack"] = not tampered["verdicts"]["red_hop_fails_stack"]
        atomic_write_json(report_path, tampered)
        tamper_failed = not verify_mcp_stack_health_report(report_dir)["ok"]
        ...
        misgrade_failed = not verify_mcp_stack_health_report(report_dir)["ok"]
        atomic_write_json(report_path, report)

    live = run_mcp_stack_health_plane(DEFAULT_ARTIFACT_DIR)
    ok = (
        bool(result["ok"])
    ...
    if ok:
        ensure_mcp_stack_health_capability()
    return {
        **result,
        "ok": ok,
