...
def builtin_gopher_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="gopher-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, GOPHER_ACTUATION_GOAL, GOPHER_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_gopher"] = (
        not gate.accepted
        and live_goal != GOPHER_ACTUATION_GOAL
        and GOPHER_ACTUATION_ID not in live_done
