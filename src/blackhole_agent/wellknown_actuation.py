...
def builtin_wellknown_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="wellknown-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, WELLKNOWN_ACTUATION_GOAL, WELLKNOWN_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_wellknown"] = (
        not gate.accepted
        and live_goal != WELLKNOWN_ACTUATION_GOAL
        and WELLKNOWN_ACTUATION_ID not in live_done
