...
def builtin_mcp_http_transport_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="mcp-http-transport-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, MCP_HTTP_GOAL, MCP_HTTP_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_http"] = (
        not gate.accepted
        and live_goal != MCP_HTTP_GOAL
        and MCP_HTTP_ID not in live_done
