...
def builtin_bhttp_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="bhttp-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, BHTTP_ACTUATION_GOAL, BHTTP_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_bhttp"] = (
        not gate.accepted
        and live_goal != BHTTP_ACTUATION_GOAL
        and BHTTP_ACTUATION_ID not in live_done
