"""Post-resolution restructuring-versus-mandate for the absolute total spine.

Closes the resolved-but-unrestructured cliff: after ``resolution_total_spine``
seals atomic RvS receipts, independently confirm a second resolution, book
each resolved pair into a restructuring register and pair it with a mandate
(RvM), seal hash-chained atomic restructuring receipts bound to the resolution
digests, refuse split / one-sided / mismatched / failed / wrong-root /
tampered restructurings, short-circuit re-restructuring, and rebind the depth-28 tip
without skill-route.
"""
...
TOTAL_SPINE_RESTRUCTURING_IMPL = True
TOTAL_SPINE_RESTRUCTURING_KIND: str = "total_spine_restructuring"
TOTAL_SPINE_RESTRUCTURING_FILENAME: str = "total-spine-restructuring.json"
TOTAL_SPINE_RESTRUCTURING_MIN_RESTRUCTURINGS: int = 2

TOTAL_SPINE_RESOLUTION_KIND: str = "total_spine_resolution"
TOTAL_SPINE_DEFAULT_ROOT: str = "quettacontinuum"


class StageRefused(Exception):
    """A verdict-bearing refusal from total-spine resolution."""
...
def _resolution_digest_of(row: Mapping[str, Any]) -> str:
    return str(
        row.get("resolution_digest")
...
def _capability_list(row: Mapping[str, Any]) -> list[str]:
    ...
    if caps:
        return caps
    for leg in row.get("resolutions") or row.get("recoveries") or row.get("risks") or row.get("restructurings") or []:
...
def _book_signature(margin: Mapping[str, Any]) -> str:
    ...
    legs = margin.get("resolutions") or margin.get("recoveries") or margin.get("risks") or []
    sigs: list[str] = []
...
def _rvm_pairs(capabilities: Sequence[str]) -> list[dict[str, Any]]:
        ...
        rows.append(
            {
                ...
                "restructured_ok": bool(row.get("restructured_ok", True)),
                "mandate_ok": bool(row.get("mandate_ok", True)),
...
def _assert_pairs_atomic(pairs: Sequence[Mapping[str, Any]]) -> None:
    if not pairs:
        raise StageRefused(
            "total_spine_restructuring_pairs_empty",
            "stress refuses an empty LvC pair book",
        )
    for row in pairs:
        if not isinstance(row, Mapping):
            raise StageRefused(
                "total_spine_restructuring_partial",
                "stress refuses a malformed LvC pair",
            )
        restructured_ok = bool(row.get("restructured_ok", True))
        mandate_ok = bool(row.get("mandate_ok", True))
        if restructured_ok != mandate_ok or not bool(row.get("atomic_ok", True)):
            raise StageRefused(
                "total_spine_restructuring_partial",
                "stress refuses a split (non-atomic) risk-versus-requirement pair",
            )
        if not restructured_ok or not mandate_ok:
            raise StageRefused(
                "total_spine_restructuring_partial",
                "stress refuses an unresolved or uncovered LvC pair",
            )


def _restructuring_certificate_material(body: Mapping[str, Any]) -> dict[str, Any]:
    """Canonical material for total-spine risk certificate digests."""
    legs = body.get("restructurings") or body.get("legs") or []
    ...
    if isinstance(legs, list):
        for row in legs:
            ...
            collateral_rows.append(
                {
                    "resolution_index": int(row.get("resolution_index") or 0),
                    "resolution_height": int(row.get("resolution_height") or 0),
                    "resolution_digest": str(row.get("resolution_digest") or ""),
                    "bound_resolution_root": str(
                        row.get("bound_resolution_root") or ""
                    ),
                    "bound_delivery_root": str(
                    ...
                    "restructured": bool(row.get("restructured", True)),
                    "mandated": bool(row.get("mandated", True)),
                    "atomic_ok": bool(row.get("atomic_ok", True)),
                    ...
                    "parent_restructuring_root": str(
                        row.get("parent_restructuring_root") or ""
                    ),
                    "resolution_root": str(row.get("resolution_root") or ""),
                    "post_restructuring": bool(row.get("post_restructuring", True)),
                    "deterministic": bool(row.get("deterministic", True)),
                    "rvm": bool(row.get("rvm", True)),
                }
            )
    return {
        "schema_version": int(body.get("schema_version") or SCHEMA_VERSION),
        "kind": str(body.get("kind") or TOTAL_SPINE_RESTRUCTURING_KIND),
        ...
        "bound_resolution_root": str(body.get("bound_resolution_root") or ""),
        "bound_custody_root": str(body.get("bound_custody_root") or ""),
        "bound_delivery_root": str(body.get("bound_delivery_root") or ""),
        "resolution_digest": str(body.get("resolution_digest") or ""),
        "delivery_digest": str(body.get("delivery_digest") or ""),
        "parent_restructuring_root": str(body.get("parent_restructuring_root") or ""),
        "tip_restructuring_root": str(body.get("tip_restructuring_root") or ""),
        "restructuring_height": int(body.get("restructuring_height") or 0),
        "restructuring_count": int(body.get("restructuring_count") or 0),
        ...
        "restructured": bool(body.get("restructured", True)),
        "mandated": bool(body.get("mandated", True)),
        "atomic_ok": bool(body.get("atomic_ok", True)),
        "rvm_ok": bool(body.get("rvm_ok", True)),
        "one_sided": bool(body.get("one_sided", False)),
        "restructurings_ok": bool(body.get("restructurings_ok", True)),
        "restructurings_ok": bool(body.get("restructurings_ok", True)),
        "post_restructuring": bool(body.get("post_restructuring", True)),
        "deterministic": bool(body.get("deterministic", True)),
        "irreversible": bool(body.get("irreversible", True)),
        "success": bool(body.get("success", True)),
        "restructurings": collateral_rows,
    }


def compute_total_spine_restructuring_root(
    ...
    if not risks:
        return "0" * 64
    last = risks[-1]
    tip = str(last.get("resolution_root") or "").strip()
    if tip:
        return tip
    parent = ""
    for idx, row in enumerate(risks):
        body = {
            "resolution_index": int(row.get("resolution_index") or idx),
            "resolution_height": int(row.get("resolution_height") or (idx + 1)),
            "resolution_digest": str(row.get("resolution_digest") or ""),
            "bound_resolution_root": str(row.get("bound_resolution_root") or ""),
            ...
            "residual": int(row.get("residual") or 0),
            "independent": bool(row.get("independent", True)),
            "parent_restructuring_root": parent,
            "post_restructuring": True,
            "deterministic": True,
            "rvm": True,
        }
...
def book_total_spine_resolutions(
    ...
    *,
    ...
    want = max(int(min_restructurings), TOTAL_SPINE_RESTRUCTURING_MIN_RESTRUCTURINGS)
    verified: list[Mapping[str, Any]] = []
    for raw in margins:
        if not isinstance(raw, Mapping):
            continue
        verify = verify_total_spine_resolution_certificate(raw)
        if not verify.get("ok"):
            raise StageRefused(
                "total_spine_restructuring_margin_tampered",
                "stress refuses a margin whose digest/chain does not verify",
            )
        if raw.get("resolved") is False or raw.get("success") is False:
            raise StageRefused(
                "total_spine_restructuring_capital_unresolved",
                "stress refuses an unresolved risk receipt",
            )
        if raw.get("strategic") is False or raw.get("rvs_ok") is False:
            raise StageRefused(
                "total_spine_restructuring_capital_uncapacitated",
                "stress refuses a risk whose RvA is not complete",
            )
        if raw.get("atomic_ok") is False:
            raise StageRefused(
                "total_spine_restructuring_margin_partial",
                "stress refuses a non-atomic margin receipt",
            )
        if int(raw.get("residual") or 0) != 0:
            raise StageRefused(
                "total_spine_restructuring_residual",
                "stress refuses a margin with a non-zero residual",
            )
        verified.append(raw)
    if len(verified) < want:
        raise StageRefused(
            "total_spine_restructuring_margins_short",
    ...
    if not book_state or not book_action or not book_actuation:
        raise StageRefused(
            "total_spine_restructuring_root_missing",
    ...
    parent = str(parent_restructuring_root or "")
    for idx, margin in enumerate(verified):
        ...
        if (
            ...
            raise StageRefused(
                "total_spine_restructuring_root_mismatch",
        ...
        if sig != book_sig:
            raise StageRefused(
                "total_spine_restructuring_rvs_failed",
                "independent liquidity books disagree; LvC cannot complete",
            )
        caps = tuple(_capability_list(margin))
        if caps != book_caps:
            raise StageRefused(
                "total_spine_restructuring_one_sided",
                "stress refuses one-sided books whose capability sets differ",
            )
        pairs = _rvm_pairs(book_caps)
        _assert_pairs_atomic(pairs)
        height = (
            int(restructuring_height) + idx
            if restructuring_height is not None
            else (idx + 1)
        )
        material = {
            "resolution_index": idx,
            "resolution_height": height,
            "resolution_digest": _resolution_digest_of(margin),
            "bound_resolution_root": str(
                margin.get("tip_resolution_root") or ""
            ),
            "bound_delivery_root": str(
                margin.get("bound_delivery_root")
                or margin.get("tip_delivery_root")
        ...
        resolution_root = _sha256_json(material)
        row = dict(material)
        row["resolution_root"] = resolution_root
        row["pairs"] = pairs
        row["schema_version"] = SCHEMA_VERSION
        legs.append(row)
        parent = resolution_root
    return legs


def seal_total_spine_restructuring_certificate(
    body: Mapping[str, Any],
) -> dict[str, Any]:
    """Seal post-resolution LvC log into a tamper-evident receipt."""
    sealed_body = dict(body)
    restructurings = list(sealed_body.get("restructurings") or [])
    if not str(sealed_body.get("tip_restructuring_root") or "").strip():
        sealed_body["tip_restructuring_root"] = compute_total_spine_restructuring_root(
            restructurings
        )
    if not int(sealed_body.get("restructuring_count") or 0):
        sealed_body["restructuring_count"] = len(restructurings)
    if not int(sealed_body.get("restructuring_height") or 0):
        sealed_body["restructuring_height"] = len(restructurings)
    material = _restructuring_certificate_material(sealed_body)
    material["tip_restructuring_root"] = str(sealed_body.get("tip_restructuring_root") or "")
    digest = _sha256_json(material)
    sealed = dict(material)
    sealed["restructuring_digest"] = digest
    sealed["certificate_hash"] = digest
    sealed["total_spine_restructuring"] = True
    sealed["total_spine_restructuring_impl"] = TOTAL_SPINE_RESTRUCTURING_IMPL
    sealed["restructured_at"] = str(body.get("restructured_at") or utc_now_iso())
    sealed["used_skill_route_discovery"] = legacy_pipeline_was_used()
    if restructurings:
        sealed_pairs: list[Any] = []
        for src, dest in zip(restructurings, sealed.get("restructurings") or []):
            if isinstance(src, Mapping) and isinstance(dest, dict):
                if src.get("pairs"):
                    dest["pairs"] = list(src.get("pairs") or [])
                sealed_pairs.append(dest)
        if sealed_pairs:
            sealed["restructurings"] = sealed_pairs
    return sealed


def restructuring_certificate_path(root: Path) -> Path:
    """Resolve ``total-spine-restructuring.json`` under a resolution/ out root."""
    path = Path(root)
    if path.is_file():
        if path.name == TOTAL_SPINE_RESTRUCTURING_FILENAME or path.suffix == ".json":
            try:
                probe = json.loads(path.read_text(encoding="utf-8"))
            except (OSError, json.JSONDecodeError):
                probe = None
            if isinstance(probe, Mapping) and (
                str(probe.get("kind") or "") == TOTAL_SPINE_RESTRUCTURING_KIND
                or path.name == TOTAL_SPINE_RESTRUCTURING_FILENAME
            ):
                return path
        parent = path.parent
        sibling = parent / TOTAL_SPINE_RESTRUCTURING_FILENAME
        if sibling.is_file():
            return sibling
        nested = parent / "restructuring" / TOTAL_SPINE_RESTRUCTURING_FILENAME
        if nested.is_file():
            return nested
        grand = parent.parent / "restructuring" / TOTAL_SPINE_RESTRUCTURING_FILENAME
        if grand.is_file():
            return grand
        grand_sib = parent.parent / TOTAL_SPINE_RESTRUCTURING_FILENAME
        if grand_sib.is_file():
            return grand_sib
        return parent / "restructuring" / TOTAL_SPINE_RESTRUCTURING_FILENAME
    named = path / TOTAL_SPINE_RESTRUCTURING_FILENAME
    if named.is_file():
        return named
    nested = path / "restructuring" / TOTAL_SPINE_RESTRUCTURING_FILENAME
    if nested.is_file():
        return nested
    return path / "restructuring" / TOTAL_SPINE_RESTRUCTURING_FILENAME


def write_total_spine_restructuring_certificate(
    out_root: Path,
    body: Mapping[str, Any],
    *,
    ...
    sealed = seal_total_spine_restructuring_certificate(body)
    path = restructuring_certificate_path(Path(out_root))
    if path.is_file():
        try:
            existing = load_total_spine_restructuring_certificate(path)
        except StageRefused:
            existing = None
        if existing is not None:
            existing_digest = str(
                existing.get("restructuring_digest")
                or existing.get("certificate_hash")
                or ""
            )
            new_digest = str(
                sealed.get("restructuring_digest")
            ...
            if (
                ...
                existing["restructuring_path"] = str(path)
                existing["total_spine_restructuring_idempotent"] = True
                return existing
            raise StageRefused(
                "total_spine_restructuring_supersession_refused",
    ...
    sealed["restructuring_path"] = str(path)
    sealed["total_spine_restructuring_idempotent"] = False
    return sealed


def verify_total_spine_restructuring_certificate(
    certificate: Mapping[str, Any],
) -> dict[str, Any]:
    """Recompute collateral digest and LvC roots; fail closed on tamper."""
    claimed = str(
        certificate.get("restructuring_digest")
        or certificate.get("certificate_hash")
        or ""
    )
    material = _restructuring_certificate_material(certificate)
    expected = _sha256_json(material)
    restructurings = list(certificate.get("restructurings") or [])
    recomputed_tip = compute_total_spine_restructuring_root(restructurings)
    claimed_tip = str(certificate.get("tip_restructuring_root") or "")
    height = int(certificate.get("restructuring_height") or 0)
    count = int(certificate.get("restructuring_count") or 0)
    ...
    cert_parent = str(certificate.get("parent_restructuring_root") or "")
    chain_ok = True
    parent = cert_parent
    book_sig = ""
    for idx, row in enumerate(restructurings):
        if not isinstance(row, Mapping):
            chain_ok = False
            break
        ...
        if str(row.get("actuation_digest") or "") != actuation_digest:
            chain_ok = False
            break
        if str(row.get("parent_restructuring_root") or "") != parent:
            chain_ok = False
            break
        ...
        if str(row.get("resolution_root") or "") != expected_root:
            chain_ok = False
            break
    ...
    min_ok = count >= TOTAL_SPINE_RESTRUCTURING_MIN_RESTRUCTURINGS and height >= count
    restructurings_ok = all(
        isinstance(row, Mapping)
        and bool(row.get("restructured", True))
        and bool(row.get("mandated", True))
        and bool(row.get("atomic_ok", True))
        and bool(row.get("rvm", True))
    ...
    ok = (
        ...
        and str(certificate.get("kind") or "") == TOTAL_SPINE_RESTRUCTURING_KIND
        and int(certificate.get("schema_version") or 0) == SCHEMA_VERSION
        and certificate.get("irreversible") is True
        and certificate.get("post_restructuring") is True
        and certificate.get("deterministic") is True
        and certificate.get("restructured") is True
        and certificate.get("mandated") is True
        and certificate.get("atomic_ok") is True
        and certificate.get("rvm_ok") is True
        and certificate.get("one_sided") is False
        and bool(certificate.get("success"))
        and height >= 1
        and count >= 1
        and count == len(restructurings)
        and height >= count
        and bool(bound_root)
        ...
        and parent_ok
        and min_ok
        and restructurings_ok
        and int(certificate.get("residual") or 0) == 0
        and TOTAL_SPINE_RESTRUCTURING_IMPL is True
    )
    return {
        "ok": ok,
        "action": "verify_total_spine_restructuring",
        "claimed_digest": claimed,
        "expected_digest": expected,
        "resolution_root_ok": claimed_tip == recomputed_tip and bool(claimed_tip),
        "resolution_root_ok": claimed_tip == recomputed_tip and bool(claimed_tip),
        "recomputed_tip_restructuring_root": recomputed_tip,
        "chain_ok": chain_ok,
        "min_restructurings_ok": min_ok,
        "restructurings_ok": restructurings_ok,
        "rvm_ok": certificate.get("rvm_ok") is True,
        "atomic_ok": certificate.get("atomic_ok") is True,
        "kind_ok": str(certificate.get("kind") or "") == TOTAL_SPINE_RESTRUCTURING_KIND,
        "schema_ok": int(certificate.get("schema_version") or 0) == SCHEMA_VERSION,
        "irreversible_ok": certificate.get("irreversible") is True,
        "restructured_ok": certificate.get("restructured") is True,
        "mandated_ok": certificate.get("mandated") is True,
        "total_spine_restructuring": True,
        "used_skill_route_discovery": legacy_pipeline_was_used(),
    }


def load_total_spine_restructuring_certificate(
    path: Path | str,
) -> dict[str, Any]:
    """Load and integrity-check a sealed liquidity receipt."""
    file_path = restructuring_certificate_path(Path(path))
    if not file_path.is_file():
        raise StageRefused(
            "total_spine_restructuring_missing",
    ...
    try:
        ...
        raise StageRefused(
            "total_spine_restructuring_unreadable",
            f"risk certificate unreadable at {file_path}: {exc}",
        ) from exc
    if not isinstance(payload, Mapping):
        raise StageRefused(
            "total_spine_restructuring_invalid",
            "risk certificate root must be a JSON object",
        )
    if str(payload.get("kind") or "") != TOTAL_SPINE_RESTRUCTURING_KIND and not payload.get(
    ...
    verify = verify_total_spine_restructuring_certificate(payload)
    if not verify.get("ok"):
        raise StageRefused(
            "total_spine_restructuring_tampered",
    ...
    body = dict(payload)
    body["restructuring_path"] = str(file_path)
    body["restructuring_verify"] = verify
    body["total_spine_restructuring_loaded"] = True
    return body


def seal_total_spine_restructuring_chain(
    *,
    prior_tip: str,
    ...
    md = str(restructuring_digest or "").strip() or ("0" * 64)
    mr = str(tip_restructuring_root or "").strip() or ("0" * 64)
    cr = str(bound_resolution_root or "").strip() or ("0" * 64)
    ...
    cd = str(resolution_digest or "").strip() or ("0" * 64)
    dvd = str(delivery_digest or "").strip() or ("0" * 64)
    material = (
        f"risk|{int(bool(short_circuit))}|{int(restructuring_height)}|"
    ...
    return {
        ...
        "tip_restructuring_root": mr,
        "bound_resolution_root": cr,
        ...
        "bound_state_root": st,
        "actuation_digest": ad,
        "resolution_digest": cd,
        "delivery_digest": dvd,
        "restructuring_digest": md,
        "prior_tip": tip,
        "digest": digest,
        "total_spine_restructuring": True,
        "irreversible": True,
...
def annotate_total_spine_restructuring(
    body: dict[str, Any],
    *,
    certificate: Mapping[str, Any],
    prior_tip: str,
    ...
    """Stamp post-resolution LvC onto a total-spine result and rebind tip."""
    cst_digest = str(
        certificate.get("restructuring_digest")
        or certificate.get("certificate_hash")
        or ""
    )
    tip_restructuring_root = str(certificate.get("tip_restructuring_root") or "")
    restructuring_height = int(certificate.get("restructuring_height") or 0)
    restructuring_count = int(certificate.get("restructuring_count") or 0)
    ...
    bound_resolution_root = str(certificate.get("bound_resolution_root") or "")
    bound_delivery_root = str(certificate.get("bound_delivery_root") or "")
    actuation_digest = str(certificate.get("actuation_digest") or "")
    resolution_digest = str(certificate.get("resolution_digest") or "")
    delivery_digest = str(certificate.get("delivery_digest") or "")
    chain = seal_total_spine_restructuring_chain(
        prior_tip=prior_tip,
        restructuring_digest=cst_digest,
        tip_restructuring_root=tip_restructuring_root,
        ...
        restructuring_height=restructuring_height,
        short_circuit=short_circuit,
    )
    cst_tip = str(chain.get("digest") or prior_tip)
    bound = _sha256_bytes(f"{prior_tip}|{cst_tip}".encode("utf-8"))
    body["total_spine_restructuring"] = True
    body["total_spine_restructuring_impl"] = TOTAL_SPINE_RESTRUCTURING_IMPL
    body["total_spine_restructuring_short_circuit"] = bool(short_circuit)
    body["total_spine_restructuring_irreversible"] = True
    body["total_spine_restructuring_post_restructuring"] = True
    body["total_spine_restructuring_deterministic"] = True
    body["total_spine_restructuring_rvm"] = True
    body["total_spine_restructuring_certificate"] = dict(certificate)
    body["total_spine_restructuring_digest"] = cst_digest
    body["total_spine_restructuring_chain"] = chain
    body["total_spine_restructuring_tip"] = cst_tip
    body["total_spine_restructuring_bound_tip"] = bound
    body["total_spine_digest_pre_risk"] = prior_tip
    body["total_spine_tip_restructuring_root"] = tip_restructuring_root
    body["total_spine_restructuring_height"] = restructuring_height
    body["total_spine_restructuring_count"] = restructuring_count
    body["total_spine_restructured"] = bool(certificate.get("restructured", True))
    body["total_spine_restructured_ok"] = bool(certificate.get("restructured", True))
    body["total_spine_mandated"] = bool(certificate.get("mandated", True))
    body["total_spine_mandated"] = bool(certificate.get("mandated", True))
    body["total_spine_mandated_ok"] = bool(certificate.get("mandated", True))
    body["total_spine_rvm_ok"] = bool(certificate.get("rvm_ok", True))
    body["total_spine_restructuring_atomic"] = bool(certificate.get("atomic_ok", True))
    body["total_spine_restructuring_one_sided"] = bool(
        certificate.get("one_sided", False)
    )
    body["total_spine_restructurings_ok"] = bool(
        certificate.get("restructurings_ok", True)
    )
    body["total_spine_restructuring_root_valid"] = bool(tip_restructuring_root)
    body["total_spine_restructuring_residual"] = int(certificate.get("residual") or 0)
    body["total_spine_restructuring_pair_count"] = int(
        certificate.get("pair_count") or 0
    )
    body["resolution_root"] = tip_restructuring_root
    body["tip_restructuring_root"] = tip_restructuring_root
    body["restructuring_count"] = restructuring_count
    body["restructuring_height"] = restructuring_height
    body["restructured"] = bool(certificate.get("restructured", True))
    body["restructured_ok"] = bool(certificate.get("restructured", True))
    body["risk_ok"] = bool(certificate.get("restructured", True))
    body["rvm_ok"] = bool(certificate.get("rvm_ok", True))
    body["mandated"] = bool(certificate.get("mandated", True))
    if certificate.get("restructuring_path"):
        body["total_spine_restructuring_path"] = certificate.get("restructuring_path")
    if bound_state_root:
    ...
    if bound_resolution_root:
        body["total_spine_tip_risk_root"] = bound_resolution_root
        body["resolution_root"] = bound_resolution_root
        body["tip_resolution_root"] = bound_resolution_root
        body.setdefault("total_spine_resolution", True)
        body.setdefault("total_spine_solvent", True)
        body.setdefault("total_spine_rvs_ok", True)
    ...
    if resolution_digest:
        body["total_spine_resolution_digest"] = resolution_digest
    if delivery_digest:
    ...
    body["verdict"] = (
        "total_spine_restructuring_ok_short_circuit"
        if short_circuit
        else "total_spine_restructuring_ok"
    )
    body["ok"] = True
    return body


def _as_resolution_mapping(value: Any) -> dict[str, Any] | None:
    from blackhole_agent.upstream_total_spine_resolution import (
    ...
    if isinstance(value, Mapping):
        ...
        if kind == TOTAL_SPINE_RESTRUCTURING_KIND or value.get("total_spine_restructuring"):
            nested_liq = value.get("total_spine_resolution_certificate")
            if isinstance(nested_liq, Mapping) and nested_liq.get(
                "tip_resolution_root"
            ):
                return dict(nested_liq)
        if kind == TOTAL_SPINE_RESOLUTION_KIND or value.get(
            "total_spine_resolution"
        ) or value.get("total_spine_resolution_loaded") or value.get(
            "tip_resolution_root"
        ):
            nested = value.get("total_spine_resolution_certificate")
            if isinstance(nested, Mapping) and nested.get("tip_resolution_root"):
                return dict(nested)
            return dict(value)
        nested = value.get("total_spine_resolution_certificate")
    ...
    try:
        probe_path = path
        if path.is_dir():
            nested = path / "resolution" / "total-spine-resolution.json"
            named = path / "total-spine-resolution.json"
            probe_path = nested if nested.is_file() else named
        if probe_path.is_file():
            probe = json.loads(probe_path.read_text(encoding="utf-8"))
            if isinstance(probe, Mapping):
                kind = str(probe.get("kind") or "")
                if kind and kind != TOTAL_SPINE_RESOLUTION_KIND:
                    return None
    except (OSError, json.JSONDecodeError):
        pass
    try:
        return load_total_spine_resolution_certificate(path)
    except ResolutionRefused as exc:
        if str(exc.verdict) == "total_spine_resolution_tampered":
...
def _as_restructuring_mapping(value: Any) -> dict[str, Any] | None:
    if isinstance(value, Mapping):
        ...
        if kind == TOTAL_SPINE_RESTRUCTURING_KIND or value.get(
            "total_spine_restructuring"
        ) or value.get("total_spine_restructuring_loaded"):
    ...
    if value is None:
        return None
    try:
        return load_total_spine_restructuring_certificate(Path(str(value)))
...
def _settlements_from(item: Any) -> list[dict[str, Any]]:
    ...
    if isinstance(nested, Mapping) and (
        nested.get("tip_settlement_root") or nested.get("observmtions")
    ):
        found.append(dict(nested))
    if item.get("tip_settlement_root") and item.get("observmtions"):
        found.append(dict(item))
...
def _confirm_resolution(
    ...
    *,
    ...
    from blackhole_agent.upstream_total_spine_resolution import resolution_total_spine
    ...
    confirm_out = None
    tip_risk = str(primary.get("tip_resolution_root") or "")
    sol_height = int(primary.get("resolution_height") or 0)
    ...
    for drop in (
        "total_spine_resolution",
        "total_spine_resolution_certificate",
        "total_spine_resolution_loaded",
        "total_spine_restructuring",
        ...
        if kind in {TOTAL_SPINE_RESOLUTION_KIND, TOTAL_SPINE_RESTRUCTURING_KIND}:
            confirm_body.pop("kind", None)
        confirm_body.pop(drop, None)
    recoveries_in: list[dict[str, Any]] = []
    seen: set[str] = set()
    for src in (primary, body, confirm_body):
        for row in _recoveries_from(src):
            key = str(
                row.get("recovery_digest")
                or row.get("certificate_hash")
                or row.get("tip_recovery_root")
                or ""
            )
    ...
    if not recoveries_in and not bundle and not confirm_body:
        raise StageRefused(
            "total_spine_restructuring_confirmation_missing",
    ...
    confirmed = resolution_total_spine(
        source,
        recoveries=recoveries_in or None,
        ...
        prior_tip=prior_tip,
        parent_resolution_root=tip_risk,
        resolution_height=sol_height + 1 if sol_height else None,
        repo_path=repo_path or REPO_ROOT,
        confirm=True,
    )
    cert = confirmed.get("total_spine_resolution_certificate")
...
def _collect_resolutions(
    ...
    existing = _as_restructuring_mapping(source)
    if existing is None and body is not None:
        existing = _as_restructuring_mapping(body)
    ...
    def _push(item: Any) -> None:
        mapped = _as_resolution_mapping(item)
    ...
    if existing is None:
        if isinstance(source, Sequence) and not isinstance(
            ...
            _push(source)
    if body is not None:
        _push(body.get("total_spine_resolution_certificate"))
    ...
    seen: set[str] = set()
    for row in risks:
        digest = _resolution_digest_of(row)
        tip = str(row.get("tip_resolution_root") or "")
        key = digest or tip
        if not key or key in seen:
            continue
        seen.add(key)
...
def _strip_restructuring_predicates(done_when: str) -> str:
    """Evaluate the pre-restructuring contract, never stress_* (no recurse)."""
...
def restructuring_total_spine(
    ...
    *,
    ...
    """Apply post-resolution atomic RvM restructuring on the absolute total spine."""
    ...
    if not TOTAL_SPINE_RESTRUCTURING_IMPL:
        raise StageRefused(
            "total_spine_restructuring_disabled",
            "TOTAL_SPINE_RESTRUCTURING_IMPL is False",
        )

    extra_books: list[Any] = []
    extra_books.extend(list(resolutions or []))
    ...
    (
        ...
        found_settlements,
        found_actuation,
    ) = _collect_resolutions(source, body, extra_books)
    ...
    if (
        ...
        and existing.get("tip_restructuring_root")
        and (
            str(existing.get("kind") or "") == TOTAL_SPINE_RESTRUCTURING_KIND
            or existing.get("total_spine_restructuring_loaded")
            or existing.get("total_spine_restructuring")
        )
    ):
        tip = str(
            prior_tip
        ...
        result = body if body is not None else {
            "ok": True,
            "action": "restructuring_total_spine",
            "total_spine": True,
        }
        return annotate_total_spine_restructuring(
            result,
    ...
    legs = book_total_spine_resolutions(
        collected,
        min_restructurings=want,
        parent_restructuring_root=parent_restructuring_root,
        restructuring_height=restructuring_height,
    )
    first = collected[0]
    ...
    margin_root = str(first.get("tip_resolution_root") or "")
    resolution_digest = _resolution_digest_of(first)
    root_layer = str(
    ...
    pre_liquidity = _strip_restructuring_predicates(done_when)
    if pre_liquidity:
        ctx = {
            "liquidity": {
                "ok": True,
                ...
                "tip_resolution_root": margin_root,
            },
            "liquidity_count": int(first.get("liquidity_count") or 0),
            "tip_resolution_root": margin_root,
        ...
        if contract_machine and contract_met is not True:
            raise StageRefused(
                "total_spine_restructuring_contract_unmet",
                f"done_when not met at collateral: {pre_liquidity!r}",
            )

    tip_restructuring_root = compute_total_spine_restructuring_root(legs)
    cst_height = int(legs[-1]["resolution_height"]) if legs else 0
    tip = str(
        prior_tip
        or (body or {}).get("total_spine_risk_bound_tip")
        or (body or {}).get("total_spine_digest")
    ...
    cst_body: dict[str, Any] = {
        ...
        "kind": TOTAL_SPINE_RESTRUCTURING_KIND,
        ...
        "bound_resolution_root": margin_root,
        ...
        "prior_tip": tip,
        "parent_restructuring_root": str(
            parent_restructuring_root
            or (legs[0].get("parent_restructuring_root") if legs else "")
            or ""
        ),
        "restructurings": legs,
        "restructuring_count": len(legs),
        "restructuring_height": cst_height,
        "tip_restructuring_root": tip_restructuring_root,
        ...
        "rvm_ok": True,
        "one_sided": False,
        "restructurings_ok": True,
        "restructurings_ok": True,
        "post_restructuring": True,
        "deterministic": True,
        "irreversible": True,
        "success": True,
        "restructured_at": utc_now_iso(),
    }
    ...
    if write_target is not None:
        certificate = write_total_spine_restructuring_certificate(write_target, cst_body)
    else:
        certificate = seal_total_spine_restructuring_certificate(cst_body)
    ...
    if annotated.get("total_spine_compressed") and root_layer:
        ...
        cst_bound = str(annotated.get("total_spine_restructuring_bound_tip") or tip)
    ...
    annotated["total_spine_restructuring_bound_state_root"] = state_root
    annotated["total_spine_restructuring_bound_action_root"] = action_root
    annotated["total_spine_restructuring_bound_settlement_root"] = settlement_root
    annotated["total_spine_restructuring_bound_clearing_root"] = clearing_root
    annotated["total_spine_restructuring_bound_resolution_root"] = margin_root
    annotated["total_spine_restructuring_actuation_digest"] = actuation_digest
    annotated["used_skill_route_discovery"] = legacy_pipeline_was_used()
    return annotated

def builtin_total_spine_restructuring_proof() -> dict[str, Any]:
    ...
    from blackhole_agent.upstream_control_engine import (
        ...
        TOTAL_SPINE_RESTRUCTURING_IMPL as ENGINE_CAP_IMPL,
    ...
    scratch = Path(tempfile.mkdtemp(prefix="tsp-rvm-"))
    try:
        from blackhole_agent import upstream_loop_engine as le_facade

        flags_ok = (
            TOTAL_SPINE_RESTRUCTURING_IMPL is True
            ...
            and TOTAL_SPINE_RESTRUCTURING_KIND == "total_spine_restructuring"
            and bool(TOTAL_SPINE_RESTRUCTURING_FILENAME)
            and TOTAL_SPINE_RESTRUCTURING_MIN_RESTRUCTURINGS >= 2
        )
        ...
        for idx, done_when in enumerate(
            ...
            body = {
                ...
                "goal": "restructuring proof origin",
                "done_when": done_when,
        ...
        offline_cap = restructuring_total_spine(
            [cert_res1, cert_res2],
            out_root=scratch / "res-h1",
        ...
        cap_path = offline_cap.get("total_spine_restructuring_path")
        tip_restructuring = str(offline_cap.get("total_spine_tip_restructuring_root") or "")
        offline_ok = (
            bool(offline_cap.get("ok"))
            and offline_cap.get("total_spine_restructuring") is True
            and offline_cap.get("total_spine_restructuring_post_restructuring") is True
            and offline_cap.get("total_spine_restructuring_irreversible") is True
            and offline_cap.get("total_spine_restructured") is True
            and offline_cap.get("total_spine_mandated") is True
            and offline_cap.get("total_spine_rvm_ok") is True
            and offline_cap.get("total_spine_restructuring_atomic") is True
            and offline_cap.get("total_spine_restructuring_one_sided") is False
            and int(offline_cap.get("total_spine_restructuring_count") or 0) >= 2
            and int(offline_cap.get("total_spine_restructuring_height") or 0) >= 2
            and int(offline_cap.get("total_spine_restructuring_residual") or 0) == 0
            and int(offline_cap.get("total_spine_restructuring_pair_count") or 0) >= 1
            and len(tip_restructuring) >= 32
        ...
        loaded = load_total_spine_restructuring_certificate(cap_path or scratch)
        verify_ok = bool(
            loaded.get("total_spine_restructuring_loaded")
            and (loaded.get("restructuring_verify") or {}).get("ok")
            and (loaded.get("restructuring_verify") or {}).get("resolution_root_ok")
            and (loaded.get("restructuring_verify") or {}).get("chain_ok")
            and (loaded.get("restructuring_verify") or {}).get("restructurings_ok")
            and (loaded.get("restructuring_verify") or {}).get("rvm_ok")
        )

        tampered_path = scratch / "tampered-restructuring.json"
        ...
        tampered_body["restructuring_height"] = 99
        atomic_write_json(tampered_path, tampered_body)
        tamper_ok = False
        try:
            load_total_spine_restructuring_certificate(tampered_path)
        except StageRefused as exc:
            tamper_ok = str(exc.verdict) == "total_spine_restructuring_tampered"
        except Exception:  # noqa: BLE001
            tamper_ok = False
        ...
        try:
            ...
            supersession_ok = (
                str(exc.verdict) == "total_spine_restructuring_supersession_refused"
            )
        except Exception:  # noqa: BLE001
            supersession_ok = False
        ...
        resealed = seal_total_spine_restructuring_certificate(wrong_body)
        wrong_verify = verify_total_spine_restructuring_certificate(resealed)
        wrong_root_ok = wrong_verify.get("ok") is False and (
            wrong_verify.get("chain_ok") is False
            or wrong_verify.get("resolution_root_ok") is False
        )

        mismatch_ok = False
        try:
            mixed = dict(cert_res1)
        ...
        try:
            ...
            resealed_one = seal_total_spine_resolution_certificate(forged)
            book_total_spine_resolutions(
                [cert_res1, resealed_one], min_restructurings=2
            )
        except StageRefused as exc:
            one_sided_ok = str(exc.verdict) == "total_spine_restructuring_one_sided"
        except Exception:  # noqa: BLE001
            one_sided_ok = False
        ...
        try:
            ...
            partial_ok = str(exc.verdict) == "total_spine_restructuring_partial"
        except Exception:  # noqa: BLE001
            partial_ok = False

        h2 = restructuring_total_spine(
            [cert_res1, cert_res2],
            out_root=scratch / "res-h2",
            prior_tip=str(
                offline_cap.get("total_spine_restructuring_bound_tip") or ""
            ),
            parent_restructuring_root=tip_restructuring,
            restructuring_height=int(
                offline_cap.get("total_spine_restructuring_height") or 0
            )
            + 1,
            repo_path=REPO_ROOT,
            confirm=False,
        ...
        multi_height_ok = (
            bool(h2.get("ok"))
            and int(h2.get("total_spine_restructuring_count") or 0) >= 2
            and str(h2.get("total_spine_tip_restructuring_root") or "") != tip_restructuring
            and str(
                (h2.get("total_spine_restructuring_certificate") or {}).get(
                    "parent_restructuring_root"
                )
                or ""
            )
        ...
        recomputed = compute_total_spine_restructuring_root(
            loaded.get("restructurings") or []
        )
        determinism_ok = recomputed == tip_restructuring and bool(recomputed)
        ...
        live_cap_path = live.get("total_spine_restructuring_path")
        live_ok = (
            bool(live.get("ok"))
            ...
            and live.get("total_spine_restructured") is True
            and live.get("total_spine_rvm_ok") is True
            and int(live.get("total_spine_restructuring_count") or 0) >= 2
            and isinstance(live.get("total_spine_tip_restructuring_root"), str)
            and len(str(live.get("total_spine_tip_restructuring_root"))) >= 32
        ...
        shorted = run_total_spine(
            ...
            resolution=True,
            restructuring=True,
            resume_dir=live_cap_path or (scratch / "live-res"),
            effect_timeout=90,
            repo_path=REPO_ROOT,
        )
        short_ok = (
            bool(shorted.get("ok"))
            and shorted.get("total_spine_restructuring") is True
            and shorted.get("total_spine_restructuring_short_circuit") is True
            and str(shorted.get("total_spine_tip_restructuring_root") or "")
            == str(live.get("total_spine_tip_restructuring_root") or "")
            and int(shorted.get("total_nest_depth") or 0) == 28
            and not legacy_pipeline_was_used()
        )

        cap_chain = live.get("total_spine_restructuring_chain") or {}
        chain_integrity_ok = False
        if isinstance(cap_chain, Mapping) and cap_chain:
            re_seal = seal_total_spine_restructuring_chain(
                prior_tip=str(cap_chain.get("prior_tip") or ""),
                restructuring_digest=str(cap_chain.get("restructuring_digest") or ""),
                tip_restructuring_root=str(cap_chain.get("tip_restructuring_root") or ""),
                bound_resolution_root=str(
                    cap_chain.get("bound_resolution_root") or ""
                ),
                bound_delivery_root=str(
                ...
                resolution_digest=str(cap_chain.get("resolution_digest") or ""),
                delivery_digest=str(cap_chain.get("delivery_digest") or ""),
                restructuring_height=int(cap_chain.get("restructuring_height") or 0),
                short_circuit=bool(cap_chain.get("short_circuit")),
            )
            chain_integrity_ok = (
                re_seal.get("digest") == cap_chain.get("digest")
                and re_seal.get("digest") == live.get("total_spine_restructuring_tip")
            )
        ...
        source_ok = (
            getattr(le_facade, "TOTAL_SPINE_RESTRUCTURING_IMPL", None) is TOTAL_SPINE_RESTRUCTURING_IMPL
            and getattr(le_facade, "builtin_total_spine_restructuring_proof", None) is builtin_total_spine_restructuring_proof
            and getattr(le_facade, "restructuring_total_spine", None) is restructuring_total_spine
            and callable(
                getattr(le_facade, "builtin_total_spine_restructuring_proof", None)
    
        )
            and callable(getattr(le_facade, "restructuring_total_spine", None))
            and getattr(le_facade, "TOTAL_SPINE_RESTRUCTURING_IMPL", False) is True
        )
        ...
        engine_source_ok = (
            "TOTAL_SPINE_RESTRUCTURING_IMPL" in engine_text
            and "restructuring_total_spine" in engine_text
            and (
                "restructuring=True" in engine_text
                or "restructuring: bool = False" in engine_text
            )
            and "builtin_total_spine_restructuring_proof" in engine_text
        )

        mod_path = Path(__file__).resolve()
        mod_text = mod_path.read_text(encoding="utf-8")
        mod_source_ok = (
            "def restructuring_total_spine" in mod_text
            and "def builtin_total_spine_restructuring_proof" in mod_text
            and "total_spine_restructuring_supersession_refused" in mod_text
            and "total_spine_restructuring_tampered" in mod_text
            and "total_spine_restructuring_one_sided" in mod_text
            and "total_spine_restructuring_rvs_failed" in mod_text
        )
        ...
        try:
            ...
            ledger_ok = (
                entry is not None
                and (
                    "upstream_total_spine_restructuring" in (entry.entry or "")
                    or "upstream_control_engine" in (entry.entry or "")
                )
                and "builtin_total_spine_restructuring_proof" in (entry.entry or "")
                and (
                    "restructuring" in tags_blob
                    or "restructuring" in name_blob
                    or "restructuring" in delta_blob
                )
                and ("total" in tags_blob or "total" in name_blob)
                and (
                    "restructuring_total_spine" in delta_blob
                    or "post-restructuring" in delta_blob
                    or "post_restructuring" in delta_blob
                    or "rvm" in delta_blob
                )
            )
        except Exception:  # noqa: BLE001
            ledger_ok = False
        ...
        return {
            "ok": ok,
            "action": "total_spine_restructuring_proof",
            "flags_ok": flags_ok,
            "offline_ok": offline_ok,
            "restructuring_path": cap_path,
            "tip_restructuring_root": tip_restructuring,
            ...
            "state_root": state_root,
            "restructuring_count": offline_cap.get("total_spine_restructuring_count"),
            "pair_count": offline_cap.get("total_spine_restructuring_pair_count"),
            "verify_ok": verify_ok,
            "tamper_ok": tamper_ok,
            ...
            "live_ok": live_ok,
            "live_restructuring_path": live_cap_path,
            "live_tip_restructuring_root": live.get("total_spine_tip_restructuring_root"),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    sub.add_parser(
        "restructuring-proof",
        help=(
            "Total spine restructuring proof: post-resolution atomic RvM seals "
            "matching resolution books into irreversible restructuring receipts"
        ),
    )
    sub.add_parser("proof", help="Alias for restructuring-proof")
    args = parser.parse_args(list(argv) if argv is not None else None)
    if args.cmd in {"restructuring-proof", "proof"}:
