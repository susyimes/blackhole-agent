...
def builtin_mcp_cursor_pagination_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="mcp-cursor-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, MCP_CURSOR_GOAL, MCP_CURSOR_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_cursor"] = (
        not gate.accepted
        and live_goal != MCP_CURSOR_GOAL
        and MCP_CURSOR_ID not in live_done
