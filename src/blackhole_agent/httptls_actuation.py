...
def builtin_httptls_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="httptls-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, HTTPTLS_ACTUATION_GOAL, HTTPTLS_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_httptls"] = (
        not gate.accepted
        and live_goal != HTTPTLS_ACTUATION_GOAL
        and HTTPTLS_ACTUATION_ID not in live_done
