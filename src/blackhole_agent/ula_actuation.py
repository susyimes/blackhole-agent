...
def builtin_ula_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="ula-bind-") as tmp:
        ...
        gate = assess_mission_selection(root, ULA_ACTUATION_GOAL, ULA_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(root)
    checks["exhausted_catalog_rejects_ledger_only_ula"] = (
        not gate.accepted
        and live_goal != ULA_ACTUATION_GOAL
        and ULA_ACTUATION_ID not in live_done
