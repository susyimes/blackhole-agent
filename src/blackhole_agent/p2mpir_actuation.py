...
def builtin_p2mpir_actuation_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="p2mpir-bind-") as tmp:
        ...
        gate = assess_mission_selection(p2mpir, P2MPIR_ACTUATION_GOAL, P2MPIR_ACTUATION_DONE_WHEN, history=[])
        live_goal, live_done, live_source = bind_gate_passing_successor(p2mpir)
    checks["exhausted_catalog_rejects_ledger_only_p2mpir"] = (
        not gate.accepted
        and live_goal != P2MPIR_ACTUATION_GOAL
        and P2MPIR_ACTUATION_ID not in live_done
