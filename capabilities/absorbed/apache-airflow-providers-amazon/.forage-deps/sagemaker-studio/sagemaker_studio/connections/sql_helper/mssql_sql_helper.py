...
class MSSQLHelper(SqlHelper):

    @staticmethod
    def to_sql_config(connection: Connection, **kwargs) -> Dict[str, Any]:
        """
        Transform DataZone MSSQL connection data into SQL interface configuration.

        Extracts MSSQL-specific parameters including region, host, port and database. Reads the contents of the
