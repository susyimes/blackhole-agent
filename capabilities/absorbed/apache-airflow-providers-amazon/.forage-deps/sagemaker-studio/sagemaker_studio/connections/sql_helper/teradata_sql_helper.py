...
class TeraDataSQLHelper(SqlHelper):
    @staticmethod
    def to_sql_config(connection: Connection, **kwargs) -> Dict[str, Any]:
        """
        Transform DataZone Teradata connection data into SQL interface configuration.

        Extracts Teradata-specific parameters including host, port and database. Reads the contents of the
        ...
        Returns:
            Dict[str, Any]: Configuration dictionary containing:
                - host: Host address of the Teradata database
                - port: Port number of the Teradata database
