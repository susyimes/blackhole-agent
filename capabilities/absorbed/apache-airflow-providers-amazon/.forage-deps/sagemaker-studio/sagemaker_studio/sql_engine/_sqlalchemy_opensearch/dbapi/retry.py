"""
Retry mechanisms and error recovery patterns for OpenSearch.
...
from .exceptions import OperationalError, TransientError, map_opensearch_exception
...
class RetryConfig:
    ...
    def __init__(
        ...
        self.retryable_exceptions = retryable_exceptions or (
            TransientError,
            # Add specific OpenSearch exceptions that are transient
        )
    ...
    def should_retry(self, exception: Exception, attempt: int) -> bool:
        ...
        # Check for specific OpenSearch exceptions that are transient
        try:
            from opensearchpy import ConnectionError as OSConnectionError
            from opensearchpy import TransportError

            if isinstance(exception, (TransportError, OSConnectionError)):
...
def with_retry(
    ...
    Usage:
        @with_retry(RetryConfig(max_attempts=5), "execute_query")
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            context.log_failure()  # pragma: no cover
            raise context.last_exception  # pragma: no cover
...
def retry_on_transient_error(
    ...
    for attempt in range(config.max_attempts):
        try:
            ...
            # Map OpenSearch exceptions to DB-API exceptions with context
