...
:Revision: $Revision: 10331 $
:Date: $Date: 2026-05-08 16:57:28 +0200 (Fr, 08. Mai 2026) $
...
`LaTeX packages`_ (see [clsguide]_ for an introduction on LaTeX
package writing).
...
Admonitions__ are specially styled "topics" that can appear anywhere an
ordinary `body element`_ can.

__ ../ref/rst/directives.html#admonitions
.. _body element: ../ref/doctree.html#body-elements
...
LaTeX font encodings are described in detail in [encguide]_ which is
...
By default, `footnote references`_ and footnotes_ are set with the
macros ``\DUfootnotemark`` and ``\DUfootnotetext`` using the footnote
labels from the source or generated by Docutils.

If the setting latex_footnotes_ is True (or with the command line
option ``--latex-footnotes``), numbering and placement of footnotes
is handled by LaTeX:

* Footnotes are set with the standard LaTeX ``\footnote`` command at the
  place of the first matching `footnote reference`_.
* LaTeX does its best to place the footnote at the same page as the
  footnote reference.
...
.. important::
   ...
   LaTeX footnotes in a sidebar_ and corner cases like footnotes
   referenced from a title and containing a footnote reference lead to
   compilation errors!

LaTeX offers a wide range of footnote configuration options
(cf. the `LaTeX WikiBook`__ and the `TeX FAQ`__).
Additionally, the ``\DUfootnotemark`` and ``\DUfootnotetext`` commands
can be configured in a custom `style sheet`_ or the `LaTeX preamble`_.

__ https://texfaq.org/FAQ-ftncapt
__ https://en.wikibooks.org/wiki/LaTeX/Footnotes_and_Margin_Notes
   #Common_problems_and_workarounds
__ https://texfaq.org/tags#footnotes
...
Example 1:
  ...
  * This is the default with KOMA-script_ classes, e.g::
    ...
    (for further configuration, see [scrguide]_),

  * `LaTeX preamble`_ code for hanging indent with the footmisc_ package::
...
Example 2:
  ...
  (See [scrguide]_ for details and other options.)
...
.. _latex_footnotes: config.html#latex-footnotes
.. _hanging: https://ctan.org/pkg/hanging
.. _symbol footnotes: ../ref/rst/restructuredtext.html#auto-symbol-footnotes
...
a) The `typearea`_ package (part of the `KOMA-script`_ bundle) calculates a
   *good* page layout (based on rules and recommendations of typography
   ...
   See [scrguide]_ for details on what is a *good* layout and
...
__ ../ref/rst/directives.html#sidebar
...
* The ToC is generated by LaTeX (via the ``\tableofcontents`` command).
  ...
  The layout depends on the chosen document class and can be configured
  in a custom `style sheet`_ (see, e.g., [scrguide]_ for the `KOMA-script`_
  classes).
...
footnote text close together or use the latex_footnotes_ setting.

empty line before footnote content
``````````````````````````````````

If footnotes_ start with any `body element`_ other than a paragraph,
there is a spurious vertical space after the footnote number/symbol.
`This is a LaTeX limitation.`__

__ https://tex.stackexchange.com/a/237391/288060
...
.. [clsguide] `LaTeX for package and class authors`,
   https://mirrors.ctan.org/macros/latex/base/clsguide.pdf
.. [encguide] `LaTeX font encodings`,
   https://mirrors.ctan.org/macros/latex/base/encguide.pdf
.. [grfguide] `Packages in the ‘graphics’ bundle`
   https://mirrors.ctan.org/macros/latex/required/graphics/grfguide.pdf
.. [scrguide] `KOMA - Script a versatile LATEX 2ε bundle`,
   https://mirrors.ctan.org/macros/latex/contrib/koma-script/doc/scrguide-en.pdf
.. [usrguide] `LaTeX2e for authors`,
   https://www.latex-project.org/guides/usrguide.pdf
