"""Standalone repro for the footnote scaling quadratic (complexity).

Discovered autonomously by blackhole_agent.upstream_discovery (generator
'footnote_defs': n caret-footnote references with n caret-footnote
definitions). Two root causes in the footnotes extension, repaired by one
patch: the footnote block processor re-searches a shrinking block once per
definition, and the footnote inline processor builds a list of every
definition key (definition order) or linearly scans the reference-order list
(reference order) once per reference. Runs a doubling ladder against the
source tree given as argv[1]; exits 1 while the defect is present, 0 once
repaired. Usage: python <this file> <path-to-src-dir>
"""
